...
class PccCmdHelp : public PccCmd {
  ...
  public:
	...
	virtual dgt_sint32 run()
	{
		...
		dg_print("pcp_credentials generate -svc demo -user dgadmin -password abc -key this_is_key -[svc_home|ip|mac|instance|db_name|db_user|os_user|program|org_user]\n");
		dg_print("pcp_credentials parse -key this_is_key -credentials hdfdsf123+93dkf\n");
		return 0;
	};
};
...
class PccCmdGenerate : public PccCmd {
  ...
  public:
	...
	virtual dgt_sint32 run()
	{
		...
		PcaCredentials	pc;
		//
		// added by chchung 2012.11.2 for adding open_session attributes into credentials
		//
		const dgt_schar*	val;
...
class PccCmdParse : public PccCmd {
  ...
  public:
	...
	virtual dgt_sint32 run()
	{
		...
		if (*pc.instanceName()) printf("Instance Name => %s\n",pc.instanceName());
...
class PccCredentials {
  ...
  protected :
  public :
	PccCredentials(dgt_sint32 argc,dgt_schar** argv) : Cmd(argv[1]), CmdOption(argc,argv) {};
	~PccCredentials() {};
