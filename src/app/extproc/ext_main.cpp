...
int main(dgt_sint32 argc,dgt_schar** argv)
{
	...
	if (DgcDbProcess::openDatabase(svc_name,DGC_LD_ATTACH) != 0) {
		...
		DgcWorker::PLOG.tprintf(0,*e,"attachDatabase[%s] failed due to the below:\n",svc_name);
		delete e;
		exit(1);
	}
	DgcDbProcess::sess()->setDbUser(DGC_SYS_OWNER);
	if (argc > 2 && argv[2] && *argv[2]) dg_strcpy(DgcDbProcess::pa().owner,argv[2]);
	else dg_strcpy(DgcDbProcess::pa().owner,DGC_SYS_OWNER);
	...
	if (argc > 2 && argv[2] && *argv[2]) DgcProcServer::p()->setOwner(argv[2]);
	else DgcProcServer::p()->setOwner(DGC_SYS_OWNER);
	DgcProcServer::p()->setParentPID(ppid);
	if (DgcProcServer::p()->start() != 0) {
		DgcExcept*	e=EXCEPTnC;
		DgcWorker::PLOG.tprintf(0,*e,"start[proc_server] failed due to the below:\n");
		delete e;
