 ...
 *   date                    modification
 *   180713                  modified build parameter because dir_pttn,
file_pttn is moved dependency to job_tgt_dir
--------------------------------------------------------------------
...
const dgt_schar PfccAgentParamBuilder::SCHEDULE_PARAM_FORMAT[] =
    "(schedule=(id=%lld)(start_date=%u)(end_date=%u)(week_map=%u)(start_hour=%"
    "u)(end_hour=%u)(start_min=%u)(end_min=%u)(use_cores=%u)(buffer_size=%u))"
    "\n";

const dgt_schar PfccAgentParamBuilder::CRYPT_DIR_PARAM_FORMAT[] =
    "(crypt_dir=(id=%lld)(zone_id=%lld)(status=%u)(src_dir=\"%s\")(dst_dir=\"%"
    "s\")(dir_rule=(version=%d)(sfd=%d)(sld=%d)(efd=%d)(eld=%d)))\n";

const dgt_schar PfccAgentParamBuilder::ENC_ZONE_PARAM_FORMAT[] =
    "(zone=(id=%lld)(last_update=%lld)(name=\"%s\")(crypt_mode=%s)(file_format="
    "%u)(close_after=%u)(header_flag=%u)(out_extension=%s)(backup_flag=%u)("
    "overwrite_flag=%u)(ext_ip=%s)(ext_port=%u)(s_limit=%f)(s_enable=%d)(g_"
    "sigma=%f)(g_enable=%d)(c_enable=%d)(r_angle=%d)(r_enable=%d))\n";

const dgt_schar PfccAgentParamBuilder::SYSTEM_INFO_PARAM_FORMAT[] =
    "(system_info=(zone_id=%lld)(system_id=\"%lld\")(system_name=\"%s\")("
    "system_ip=\"%s\"))\n";

const dgt_schar PfccAgentParamBuilder::DELIMITER_PARAM_FORMAT[] =
    "(delimiter=(zone_id=%lld)(max_line_len=%u)(max_pattern_len=%u)(chars=\"%"
    "s\")(row_delimiter=\"%s\")(continue_delimiter=\"%s\"))\n";

const dgt_schar PfccAgentParamBuilder::FIXED_PARAM_FORMAT[] =
    "(fixed=(zone_id=%lld)(max_line_len=%u)(max_pattern_len=%u)(row_delimiter="
    "\"%s\")(continue_delimiter=\"%s\")(col_lengths=%s)(out_col_lengths=%s)("
    "lead_space_trim=%s)(tail_space_trim=%s))\n";

PfccAgentParamBuilder::PfccAgentParamBuilder(dgt_sint64 agent_id,
                                             dgt_uint8 init_flag)
...
dgt_sint32 PfccAgentParamBuilder::buildDirPttnParams(
    DgcMemRows* enc_job_tgt_id_rows) throw(DgcExcept) {
    if (!enc_job_tgt_id_rows)
        THROWnR(DgcDgExcept(DGC_EC_DG_INVALID_STAT,
                            new DgcError(SPOS, "no enc_job_tgt_id_rows")),
                -1);
    ...
    if (sql_handle.execute(stext, dg_strlen(stext), enc_job_tgt_id_rows) < 0)
        ATHROWnR(DgcError(SPOS, "execute failed"), -1);
    ...
    while (!(ret = sql_handle.fetch(rtn_row)) && rtn_row) {
        ...
        if (pre_enc_job_tgt_id != name_pttn->enc_job_tgt_id) {
            ...
            sprintf(DirPttnParam + idx, "(dir_pttn=(enc_job_tgt_id=%lld)",
                    name_pttn->enc_job_tgt_id);
            idx += dg_strlen(DirPttnParam + idx);
        ...
        pttn_idx++;
        sprintf(DirPttnParam + idx, "(%u=\"%s\")", pttn_idx,
                name_pttn->pttn_expr);
        idx += dg_strlen(DirPttnParam + idx);
    }
...
dgt_sint32 PfccAgentParamBuilder::buildFilePttnParams(
    DgcMemRows* enc_job_tgt_id_rows) throw(DgcExcept) {
    if (!enc_job_tgt_id_rows)
        THROWnR(DgcDgExcept(DGC_EC_DG_INVALID_STAT,
                            new DgcError(SPOS, "no enc_job_tgt_id_rows")),
    ...
    while (!(ret = sql_handle.fetch(rtn_row)) && rtn_row) {
        ...
        if (pre_enc_job_tgt_id != name_pttn->enc_job_tgt_id) {
            ...
            sprintf(FilePttnParam + idx, "(file_pttn=(enc_job_tgt_id=%lld)",
                    name_pttn->enc_job_tgt_id);
            idx += dg_strlen(FilePttnParam + idx);
            pre_enc_job_tgt_id = name_pttn->enc_job_tgt_id;
            pttn_idx = 0;
        ...
        pttn_idx++;
        sprintf(FilePttnParam + idx, "(%u=\"%s\")", pttn_idx,
                name_pttn->pttn_expr);
        idx += dg_strlen(FilePttnParam + idx);
    }
...
dgt_sint32 PfccAgentParamBuilder::buildEncZoneParams(
    DgcMemRows* zone_id_rows) throw(DgcExcept) {
    EncZoneParamSize = 0;
    if (!zone_id_rows)
        THROWnR(DgcDgExcept(DGC_EC_DG_INVALID_STAT,
                            new DgcError(SPOS, "no zone_id_rows")),
                -1);
    // 0. get system info
    if (getSystemInfo() < 0) {
    ...
    len = zone_id_rows->numRows() * ENC_ZONE_PARAM_MAX_LEN_PER_ROW +
          dg_strlen(SystemIP) + dg_strlen(SystemName);
    ...
    zone_id_rows->rewind();
    if (sql_handle.execute(stext, dg_strlen(stext), zone_id_rows) < 0)
        ATHROWnR(DgcError(SPOS, "execute failed"), -1);
    ...
    while (!(ret = sql_handle.fetch(rtn_row)) && rtn_row) {
        ...
        sprintf(ZoneParam + idx, ENC_ZONE_PARAM_FORMAT, enc_zone->enc_zone_id,
                enc_zone->last_update, enc_zone->name, crypt_mode,
                enc_zone->file_format, enc_zone->close_after,
                enc_zone->header_flag, enc_zone->out_extension,
                enc_zone->backup_flag, enc_zone->overwrite_flag, ext_data.ip,
                ext_data.port, ms_data.s_limit, ms_data.s_enable,
                ms_data.g_sigma, ms_data.g_enable, ms_data.c_enable,
                ms_data.r_angle, ms_data.r_enable);
        idx += dg_strlen(ZoneParam + idx);

        // set num_rows
        if (enc_zone->file_format == PFCC_TYPE_FILE_FORMAT_PTTN)
            pttn_num_rows++;
        else if (enc_zone->file_format == PFCC_TYPE_FILE_FORMAT_DELI)
            deli_num_rows++;
        else if (enc_zone->file_format == PFCC_TYPE_FILE_FORMAT_FIXED)
            fixed_num_rows++;

        // build system info parameter
        sprintf(SystemInfoParam + sys_info_idx, SYSTEM_INFO_PARAM_FORMAT,
                enc_zone->enc_zone_id, SystemID, SystemName, SystemIP);
    ...
    memset(stext, 0, 256);
    sprintf(stext,
            "select k.enc_zone_id, k.enc_col_id, k.pattern_id, k.column_no, "
            "c.column_name enc_col_name, d.table_name table_name from "
            "pfct_enc_zone_col_key k, pct_enc_column c, pct_enc_table d "
    ...
    while (!(ret = sql_handle.fetch(rtn_row)) && rtn_row) {
        ...
        if (pre_enc_zone_id != col_key_out->enc_zone_id) {
            ...
            sprintf(KeyParam + idx, "(key=(zone_id=%lld)",
        ...
        if (col_key_out->pattern_id) {
            ...
            memcpy(pttn_col_key_rows.data(), col_key_out,
                   sizeof(pfct_apb_get_col_key_out));
        } else {
            key_no++;
        }

        sprintf(KeyParam + idx, "(%u=(name=%s.%s)(columns=%u))", key_no,
                col_key_out->table_name, col_key_out->enc_col_name,
                col_key_out->column_no);
        idx += dg_strlen(KeyParam + idx);
    }
    ...
    if (pttn_num_rows > 0) {
        ...
        dgt_uint32 pttn_idx = 0;
        while (pttn_col_key_rows.next() &&
               (col_key_out =
                    (pfct_apb_get_col_key_out*)pttn_col_key_rows.data())) {
            ...
            if (pre_enc_zone_id != col_key_out->enc_zone_id) {
                ...
                sprintf(RegularParam + idx,
                        "(regular=(zone_id=%lld)(max_line_len=%u)(max_pattern_"
                        "len=%u)",
                        col_key_out->enc_zone_id, file_format->max_line_len,
            ...
            while (!(ret = sql_handle.fetch(rtn_row)) && rtn_row) {
                ...
                // check buffer size
                if (RegularParamLen - idx <
                    (dgt_uint32)(dg_strlen(pattern_expr->pattern_expr) + 20)) {
                    dgt_uint32 optimal_len =
                        RegularParamLen + REGULAR_PARAM_MAX_LEN_PER_ROW + 1;
                ...
                if (pttn_idx == 1) {
                    strcpy(RegularParam + idx, "(");
                    idx++;  // column no start
                    sprintf(RegularParam + idx, "%d=", col_key_out->column_no);
                    idx += dg_strlen(RegularParam + idx);  // col_no set
                }

                sprintf(RegularParam + idx, "(%u=%s)", pttn_idx,
    ...
    if (deli_num_rows > 0) {
        ...
        while (!(ret = sql_handle.fetch(rtn_row)) && rtn_row) {
            ...
            sprintf(DelimiterParam + idx, DELIMITER_PARAM_FORMAT,
                    file_format->enc_zone_id, file_format->max_line_len,
                    file_format->max_pttn_len, file_format->delimiter,
    ...
    if (fixed_num_rows > 0) {
        ...
        while (!(ret = sql_handle.fetch(rtn_row)) && rtn_row) {
            ...
            sprintf(FixedParam + idx, FIXED_PARAM_FORMAT,
                    file_format->enc_zone_id, file_format->max_line_len,
                    file_format->max_pttn_len, file_format->row_delimiter,
                    file_format->continue_delimiter, file_format->col_lengths,
    ...
    EncZoneParamSize = zone_param_size + sys_info_param_size + key_param_size +
                       reg_param_size + deli_param_size + fixed_param_size;
...
dgt_sint32 PfccAgentParamBuilder::buildScheduleParams(
    dgt_sint64 schedule_date_id) throw(DgcExcept) {
    ...
    if (schedule_date_id) {
        ...
        DgcTableSegment* schedule_date_tab =
            PetraTableHandler->getTable("pfct_schedule_date");
        if (!schedule_date_tab)
            ATHROWnR(DgcError(SPOS, "getTable[pfct_schedule_date] failed"), -1);
        DgcIndexSegment* schedule_date_tab_idx =
            PetraTableHandler->getIndex("pfct_schedule_date_idx1");
        if (!schedule_date_tab_idx)
            ATHROWnR(DgcError(SPOS, "getIndex[pfct_schedule_date_idx1] failed"),
                     -1);
        ...
        if (schedule_date_tab_idx->find((dgt_uint8*)&search_date_row,
                                        schedule_date_row_list, 1) < 0) {
            ATHROWnR(DgcError(SPOS, "schedule_date_tab_idx search failed"), -1);
        }

        if (schedule_date_row_list.numRows() == 0)
            THROWnR(
                DgcDgExcept(DGC_EC_DG_INVALID_STAT,
                            new DgcError(
                                SPOS, "pfct_schedule_date[%lld] data not found",
                                schedule_date_id)),
                -1);
        schedule_date_row_list.rewind();

        pfct_type_schedule_date* date = 0;
        if (schedule_date_row_list.next() &&
            (date = (pfct_type_schedule_date*)schedule_date_row_list.data())) {
            start_date = date->schedule_start_date;
            end_date = date->schedule_end_date;
        }

        // 2. search pfct_weekly_work_schedule
        DgcTableSegment* weekly_work_schedule_tab =
            PetraTableHandler->getTable("pfct_weekly_work_schedule");
        if (!weekly_work_schedule_tab)
            ATHROWnR(
                DgcError(SPOS, "getTable[pfct_weekly_work_schedule] failed"),
                -1);
        DgcIndexSegment* weekly_work_schedule_idx2 =
            PetraTableHandler->getIndex("pfct_weekly_work_schedule_idx2");
        if (!weekly_work_schedule_idx2)
            ATHROWnR(
                DgcError(SPOS,
                         "getIndex[pfct_weekly_work_schedule_idx2] failed"),
                -1);
        ...
        if (weekly_work_schedule_idx2->find((dgt_uint8*)&work_schedule_row,
                                            work_schedule_row_list, 1) < 0) {
            ATHROWnR(DgcError(SPOS, "weekly_work_schedule_idx2 search failed"),
                     -1);
        }
        if (work_schedule_row_list.numRows() == 0)
            THROWnR(
                DgcDgExcept(
                    DGC_EC_DG_INVALID_STAT,
                    new DgcError(
                        SPOS, "pfct_weekly_work_schedule[%lld] data not found",
                        schedule_date_id)),
                -1);
        work_schedule_row_list.rewind();

        dgt_uint32 len =
            work_schedule_row_list.numRows() * SCHEDULE_PARAM_MAX_LEN_PER_ROW;
        ...
        while (work_schedule_row_list.next() &&
               (work_schedule = (pfct_type_weekly_work_schedule*)
                                    work_schedule_row_list.data())) {
            ...
            sprintf(ScheduleParam + schedule_param_idx, SCHEDULE_PARAM_FORMAT,
                    schedule_id, start_date, end_date, week_map, start_hour,
                    end_hour, start_min, end_min, use_cores, buffer_size);
        ...
        memset(ScheduleParam, 0, ScheduleParamLen);
        schedule_id = 1;
        sprintf(ScheduleParam, SCHEDULE_PARAM_FORMAT, schedule_id, start_date,
                end_date, week_map, start_hour, end_hour, start_min, end_min,
                use_cores, 0);
    }
...
dgt_sint32 PfccAgentParamBuilder::buildJobParams(dgt_sint64 job_id) throw(
    DgcExcept) {
    // 0. search pfct_enc_job_tgt for enc_zone_id, target_path and ouput_path
    DgcTableSegment* job_tgt_tab =
        PetraTableHandler->getTable("pfct_enc_job_tgt");
    if (!job_tgt_tab)
        ATHROWnR(DgcError(SPOS, "getTable[pfct_enc_job_tgt] failed"), -1);
    DgcIndexSegment* enc_job_tab_idx2 =
        PetraTableHandler->getIndex("pfct_enc_job_tgt_idx2");
    if (!enc_job_tab_idx2)
        ATHROWnR(DgcError(SPOS, "getIndex[pfct_enc_job_tgt_idx2] failed"), -1);
    ...
    if (enc_job_tab_idx2->find((dgt_uint8*)&job_tgt_row, job_tgt_row_list, 1) <
        0) {
        ATHROWnR(DgcError(SPOS, "enc_job_tab_idx2 search failed"), -1);
    }

    // this exception is returned -901130,
    // because default configuration job is not error to be pfct_enc_job_tgt
    // data not found therefore -901130 return values have checking if default
    // configuration job
    if (job_tgt_row_list.numRows() == 0)
        THROWnR(DgcDgExcept(
                    DGC_EC_DG_INVALID_STAT,
                    new DgcError(SPOS, "pfct_enc_job_tgt[%lld] data not found",
                                 job_id)),
                -901130);
    ...
    while (job_tgt_row_list.next() &&
           (job_tgt = (pfct_type_enc_job_tgt*)job_tgt_row_list.data())) {
    ...
    while (job_tgt_row_list.next() &&
        ...
        //
        if ((InitParamFlag && job_tgt->status == PCC_STATUS_TYPE_DELETED) ||
            job_tgt->enc_zone_id == 0)
            continue;
        ...
        DgcTableSegment* dir_rule_tab =
            PetraTableHandler->getTable("pfct_enc_zone_dir_rule");
        if (!dir_rule_tab)
            ATHROWnR(DgcError(SPOS, "getTable[pfct_enc_zone_dir_rule] failed"),
                     -1);
        DgcIndexSegment* dir_rule_tab_idx =
            PetraTableHandler->getIndex("pfct_enc_zone_dir_rule_idx2");
        if (!dir_rule_tab_idx)
            ATHROWnR(DgcError(SPOS, "getIndex[pfct_schedule_date_idx2] failed"),
                     -1);
        ...
        if (dir_rule_tab_idx->find((dgt_uint8*)&search_dir_rule_row,
                                   dir_rule_row_list, 1) < 0) {
        ...
        if (dir_rule_row_list.numRows() > 0) {
            ...
            if (dir_rule_row_list.next() &&
                (dir_rule =
                     (pfct_type_enc_zone_dir_rule*)dir_rule_row_list.data())) {
        ...
        sprintf(CryptDirParam + idx, CRYPT_DIR_PARAM_FORMAT,
                job_tgt->enc_job_tgt_id, job_tgt->enc_zone_id, job_tgt->status,
                job_tgt->target_path, job_tgt->output_path,
                dir_rule_row.version, dir_rule_row.search_first_depth,
                dir_rule_row.search_last_depth, dir_rule_row.enc_first_depth,
        ...
        if (enc_job_tgt_id_hash.findNode(job_tgt->enc_job_tgt_id) == 0) {
            enc_job_tgt_id_hash.addNode(0, job_tgt->enc_job_tgt_id,
                                        (dgt_void*)0);
    ...
    if (zone_id_rows.numRows() == 0)
        THROWnR(
            DgcDgExcept(DGC_EC_DG_INVALID_STAT,
                        new DgcError(SPOS, "zone_id_rows data not found [%lld]",
                                     job_id)),
            -901130);

    if (enc_job_tgt_id_rows.numRows() == 0)
        THROWnR(
            DgcDgExcept(
                DGC_EC_DG_INVALID_STAT,
                new DgcError(SPOS, "enc_job_tgt_id_rows data not found [%lld]",
                             job_id)),
            -1);

    // 5. build zone parameters
    if (buildEncZoneParams(&zone_id_rows) < 0)
        ATHROWnR(DgcError(SPOS, "buildEncZoneParam failed"), -1);

    // 5. build dir_pttn parameters
    if (buildDirPttnParams(&enc_job_tgt_id_rows) < 0)
        ATHROWnR(DgcError(SPOS, "buildDirPttnParams failed"), -1);

    // 6. build file_pttn parameters
    if (buildFilePttnParams(&enc_job_tgt_id_rows) < 0)
        ATHROWnR(DgcError(SPOS, "buildFilePttnParams failed"), -1);

    return 0;
}

dgt_sint32 PfccAgentParamBuilder::buildAgentParams(
    dgt_sint64 job_id, dgt_sint64 schedule_date_id) throw(DgcExcept) {
    dgt_sint32 rtn = 0;
    AgentParamSize = 0;
    // 1. build schedule parameter
    if (buildScheduleParams(schedule_date_id) < 0)
        ATHROWnR(DgcError(SPOS, "buildScheduleParams failed"), -1);

    DgcSqlHandle sql_handle(DgcDbProcess::sess());
    dgt_schar sql_text[256] = {0};
    sprintf(sql_text,
            "select job_type from pfct_enc_job where enc_job_id = %lld",
            job_id);
    if (sql_handle.execute(sql_text) < 0)
        ATHROWnR(DgcError(SPOS, "execute failed"), -1);

    dgt_void* rtn_row = 0;
    if (sql_handle.fetch(rtn_row) < 0)
        ATHROWnR(DgcError(SPOS, "fetch failed"), -1);
    ...
    pr_debug(
        "schedule_param_size[%u] EncZoneParamSize[%u] crypt_dir_param_size[%u] "
        "dir_pttn_param_size[%u] file_pttn_param_size[%u]\n",
        schedule_param_size, EncZoneParamSize, crypt_dir_param_size,
        dir_pttn_param_size, file_pttn_param_size);

    AgentParamSize = schedule_param_size + EncZoneParamSize +
                     crypt_dir_param_size + dir_pttn_param_size +
                     file_pttn_param_size;
