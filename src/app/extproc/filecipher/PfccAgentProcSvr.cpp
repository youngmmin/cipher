...
PfccAgentSessChecker::PfccAgentSessChecker(
    dgt_worker* wa, PfccAgentSessionPool& agent_sess_pool)
    : DgcDbWorker(DGC_WT_CIPHER_AGENT_PROC_SVR, "agent_sess_checker", wa),
      AgentSessPool(agent_sess_pool),
      StopFlag(0) {}
            ...
            if (session->isBroken()) {
                ...
                if (e) {
                    DgcWorker::PLOG.tprintf(
                        0, *e, "agent[%lld] session[%d] is broken.\n",
...
PfccAgentListener::PfccAgentListener(dgt_worker* wa, const dgt_schar* ip,
                                     dgt_uint16 port)
    : DgcDbWorker(DGC_WT_CIPHER_AGENT_PROC_SVR, "pfcc_proc_svr", wa),
      ListenPort(port),
      CommServer(20, 20) {
...
dgt_sint32 PfccAgentListener::run() throw(DgcExcept) {
    ...
    if (e) {
        DgcWorker::PLOG.tprintf(
            0, *e, "exception while listener is running to the below:\n");
        delete e;
    }
    return 0;
}
...
dgt_void PfccAgentListener::out() throw(DgcExcept) {
    ...
    if (AgentSessChecker && AgentSessChecker->isAlive())
        AgentSessChecker->stop();
