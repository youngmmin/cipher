	...
	if ((rtn=MsgStream->recvMessage(1)) < 0) {
		...
		if (MsgStream->currMsg()->opi() != PTDGILETTER) {
			ATHROWnR(DgcError(SPOS,"invalid request message[%d:%d:%d]",
						MsgStream->currMsg()->tti(),MsgStream->currMsg()->opi(),MsgStream->currMsg()->seq()),-1);
		}
...
dgt_sint32 PfccAgentSession::setInitParams() throw(DgcExcept)
{
	...
	DgcIndexSegment* enc_job_idx2 = PetraTableHandler->getIndex("pfct_enc_job_idx2");
	if (!enc_job_idx2) ATHROWnR(DgcError(SPOS,"getIndex[pfct_enc_job_idx2] failed"),-1);
	...
	while (job_row_list.next() && (enc_job=(pfct_type_enc_job*)job_row_list.data())) {
		...
		dgt_sint32 rtn = 0;
		// build param
		if ((rtn=param_builder.buildAgentParams(enc_job->enc_job_id,enc_job->schedule_date_id)) < 0) {
			if (rtn == -901130 && strncmp(enc_job->name,"default",sizeof(enc_job->name)) == 0) { //when pfct_enc_job_tgt not found about job
				DgcExcept*	e = EXCEPTnC;
				delete e;
				DgcWorker::PLOG.tprintf(0,"setInitParams default configuration job don't has enc_job_tgt, this message is not Error\n");
				continue;
			} else {
...
//pr_debug("param:\n%s\n",param_builder.agentParam());
		...
		while (remain > 0) {
			...
			if (remain >= 1024) {
				memcpy(set_params->data,param_builder.agentParam()+offset,1024);
				offset += 1024;
				remain -= 1024;
			} else {
				memcpy(set_params->data,param_builder.agentParam()+offset,remain);
				offset += remain;
				remain = 0;
			}
		}
...
dgt_sint32 PfccAgentSession::connect(DgcCommStream* stream) throw(DgcExcept)
{
	...
	if (AgentInfo.last_update == 0 && AgentInfo.sess_id == 1 && setInitParams()) {
		DgcExcept*	e=EXCEPTnC;
		if (e) {
...
dgt_sint32 PfccAgentSessionPool::addSession(PfccAgentSession* session) throw(DgcExcept)
{
	// check prev session is exist
	if (lock() == 0) {
		for(dgt_sint32 i=0; i<PFCC_MAX_AGENT_SESS_NUM; i++) {
			if (AgentSess[i] &&
					AgentSess[i]->agentID() == session->agentID() &&
	...
	if (session) {
		THROWnR(DgcDgExcept(DGC_EC_DG_INVALID_STAT,new DgcError(SPOS,"agent[%lld] session[%d] pool is full.\n",session->agentID(),session->sessID())),-1);
	}
	return 0;
}
...
PfccAgentSession* PfccAgentSessionPool::getSession(dgt_sint64 agent_id)
{
	...
	if (lock() == 0) {
		for(dgt_sint32 i=0; i<PFCC_MAX_AGENT_SESS_NUM; i++) {
			if (AgentSess[i] &&
					AgentSess[i]->agentID() == agent_id &&
					AgentSess[i]->isUsing() == 0 &&
					AgentSess[i]->isBroken() == 0) {
				session = AgentSess[i];
				session->setUseFlag();
				break;
			}
