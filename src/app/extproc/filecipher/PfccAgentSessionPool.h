...
class PfccAgentSession : public DgcObject {
   ...
   public:
    ...
    inline dgt_uint8 isUsing() {
        if (UseFlag) return 1;
        return 0;
    };
    inline dgt_void setUseFlag() { UseFlag = 1; };
    inline dgt_void unsetUseFlag() { UseFlag = 0; };
    inline dgt_uint8 isBroken() {
        if (BrokenFlag) return 1;
        return 0;
    };
    inline dgt_void setBrokenFlag() { BrokenFlag = 1; };
    inline dgt_void unsetBrokenFlag() { BrokenFlag = 0; };

    inline DgcCliStmt* getStmt() throw(DgcExcept) {
        return Connection.getStmt();
    }
    inline dgt_sint32 checkConn() throw(DgcExcept) {
