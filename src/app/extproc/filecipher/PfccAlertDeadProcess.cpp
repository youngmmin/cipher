...
dgt_sint32 PfccAlertDeadProcess::execute() throw(DgcExcept)
{
	...
	sprintf(msg,"system[%s] found a dead process[%lld:%s].\n",tgt_sys.name,param->dead_pid,param->pname);
	if(PcAlert::p()->sendAlert(DgcDbProcess::sess(),PT_ALERT_CATEGORY1_PETRA,PT_ALERT_CATEGORY2_PETRA_PROCESS,PT_ALERT_EVENT_PETRA_PROCESS_DEAD,msg) != 0) {
		delete msg;
