...
PfccCryptStatCollector::PfccCryptStatCollector(
    dgt_worker* wa, PfccAgentListener* agent_listener,
    dgt_sint32 collecting_interval)
    : DgcPetraWorker(DGC_WT_CIPHER_AGENT_PROC_SVR, "pfcc_crypt_stat_collector",
                     wa) {
    AgentListener = agent_listener;
    if (collecting_interval > 0)
        CollectingInterval = collecting_interval;
    else
        CollectingInterval = 5;
}
...
#define MAXSQLLEN 1024
    ...
    DgcSqlHandle sql_handle(
        DgcDbProcess::
            sess());  // for select
                      // agent_id,enc_job_id,enc_zone_id,enc_job_tgt_id
    ...
    if (sql_handle.execute(stext, dg_strlen(stext)) < 0)
    ...
    while (!(ret = sql_handle.fetch(rtn_row)) && rtn_row) {
        ...
        if (rtn_row)
            get_dir_crypt_stat_in = (pfcc_get_dir_crypt_stat_in*)rtn_row;
#if 1 /*{{{*/
        // check connection agent session
        AgentSession = AgentListener->agentSessPool().getSession(
            get_dir_crypt_stat_in->agent_id);
        if (AgentSession == 0) {
            napAtick();
            continue;
            // THROWnR(DgcDgExcept(DGC_EC_DG_INVALID_STAT,new
            // DgcError(SPOS,"agent[%lld] has no available
        ...
        memset(stext, 0, MAXSQLLEN);
        sprintf(stext,
                "select * from  PFC_GET_DIR_CRYPT_STAT(%lld, %lld, %lld, %lld)",
                get_dir_crypt_stat_in->agent_id, get_dir_crypt_stat_in->job_id,
                get_dir_crypt_stat_in->enc_zone_id,
                get_dir_crypt_stat_in->enc_job_tgt_id);
        if (procedure_handle.execute(stext, dg_strlen(stext)) < 0)
            ATHROWnR(DgcError(SPOS, "execute failed"), -1);
        ...
        while (!(ret2 = procedure_handle.fetch(rtn_row2)) && rtn_row2) {
            ...
            if (dml_handle.execute(stext, dg_strlen(stext)) < 0)
                ATHROWnR(DgcError(SPOS, "execute failed"), -1);
            // insert new row
            memset(stext, 0, MAXSQLLEN);
            sprintf(stext,
                    "insert into pfct_crypt_stat_temp values("
                    "%lld, %lld, %lld, %lld, %lld, %lld, %lld, %lld, %lld, "
                    "%lld, 	" /* job_id ~ target_files 	*/
                    "%lld, %lld, %lld, %lld, %lld, %lld, %lld, %lld, 	 %u,   "
                    "%u,	" /* input_files ~ end_time */
                    "	 %d,   %d, %lld, %lld) 								"
                    "		" /* job_status ~ reserved	*/
                    ,
                    get_dir_crypt_stat_out->job_id,
