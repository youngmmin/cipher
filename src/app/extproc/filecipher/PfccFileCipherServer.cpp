...
PfccFileCipherFactory::~PfccFileCipherFactory()
{
	...
	if (AgentListener && AgentListener->isAlive())
		AgentListener->stop();
...
dgt_sint32 PfccFileCipherFactory::initialize() throw(DgcExcept)
{
	...
	ATHROWnR(DgcError(SPOS,"parsing listener.list failed",listener_conf_file),-1);

	DgcBgrammer* listen_info = 0;
	while((listen_info=listen_info_list.getNext()) != 0){
		dgt_schar* val = 0;
		if((val=listen_info->getValue("listen.client_type")) != 0 && !(dg_strcmp(val,"pfcc_proc_svr"))){
			if ((val=listen_info->getValue("listen.host")) != 0) dg_strncpy(listen_ip,val,64);
			if ((val=listen_info->getValue("listen.port")) != 0) listen_port=(dgt_uint16)strtol(val,0,10);
			if ((val=listen_info->getValue("listen.in_timeout")) != 0) in_timeout=strtol(val,0,10);
			if ((val=listen_info->getValue("listen.out_timeout")) != 0) out_timeout=strtol(val,0,10);
		}
	}
	...
	wa->PID=DgcDbProcess::pa().pid;
	wa->LWID=wa->WID;
	CryptStatCollector = new PfccCryptStatCollector(wa,AgentListener,collecting_interval);
	...
	if (addProcedure(new PfccGetFileInfo("PFC_GET_FILE_INFO",AgentListener)) != 0) {
		ATHROWnR(DgcError(SPOS,"addProcedure failed"),-1);
	}

	if (addProcedure(new PfccGetAgentStat("PFC_GET_AGENT_STAT",AgentListener)) != 0) {
		ATHROWnR(DgcError(SPOS,"addProcedure failed"),-1);
	}

	if (addProcedure(new PfccGetDirCryptStat("PFC_GET_DIR_CRYPT_STAT",AgentListener)) != 0) {
		ATHROWnR(DgcError(SPOS,"addProcedure failed"),-1);
	}

	if (addProcedure(new PfccSetAgentParam("PFC_SET_AGENT_PARAM",AgentListener)) != 0) {
		ATHROWnR(DgcError(SPOS,"addProcedure failed"),-1);
	}
	...
	if (addProcedure(new PfccRemoveFile("PFC_REMOVE_FILE",AgentListener)) != 0) {
		ATHROWnR(DgcError(SPOS,"addProcedure failed"),-1);
	}

	//added by shson 2018.03.27
	if (addProcedure(new PfccGetTargetList("PFC_GET_TARGET_LIST",AgentListener)) != 0) {
		ATHROWnR(DgcError(SPOS,"addProcedure failed"),-1);
	}
#if 1
	if (addProcedure(new PfccRecollectCryptDir("PFC_RECOLLECT_CRYPT_DIR",AgentListener)) != 0) {
		ATHROWnR(DgcError(SPOS,"addProcedure failed"),-1);
	}

	if (addProcedure(new PfccValidationFile("PFC_VALIDATION_FILE",AgentListener)) != 0) {
		ATHROWnR(DgcError(SPOS,"addProcedure failed"),-1);
	}
#endif
	// normal procedure
...
#if 1 // added by chchung 2018.7.17 for PCFS
        if (addProcedure(new PfccPcfsGetList("PFC_PCFS_GET_LIST",AgentListener)) != 0) {
                ATHROWnR(DgcError(SPOS,"addProcedure failed"),-1);
        }
        if (addProcedure(new PfccPcfsMount("PFC_PCFS_MOUNT",AgentListener)) != 0) {
                ATHROWnR(DgcError(SPOS,"addProcedure failed"),-1);
        }
        if (addProcedure(new PfccPcfsGetStat("PFC_PCFS_GET_STAT",AgentListener)) != 0) {
                ATHROWnR(DgcError(SPOS,"addProcedure failed"),-1);
        }
#endif
	...
	if (addProcedure(new PfccGetDetectInfo("PFC_GET_DETECT_INFO",AgentListener)) != 0) {
                ATHROWnR(DgcError(SPOS,"addProcedure failed"),-1);
	}
	if (addProcedure(new PfccVerifyDetectInfo("PFC_VERIFY_DETECT_INFO",AgentListener)) != 0) {
                ATHROWnR(DgcError(SPOS,"addProcedure failed"),-1);
	}
#endif
...
dgt_sint32 PfccFileCipherFactory::finalize() throw(DgcExcept)
{
	return 0;
}
...
dgt_void initServerFactory()
{
	...
	DgcProcServer::initFactory(&FileCipherFactory);        // register the SyncMgrFactory with the Procedure Server
}
