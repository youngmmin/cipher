...
PfccGetDetectInfo::PfccGetDetectInfo(const dgt_schar* name, PfccAgentListener* agent_listener)
...
PfccGetDetectInfo::~PfccGetDetectInfo()
{
...
dgt_sint32 PfccGetDetectInfo::initialize() throw(DgcExcept) 
{
	return 0; 
}
...
dgt_sint32 PfccGetDetectInfo::execute() throw(DgcExcept)
{
	...
	pfcc_get_detect_info_in* param_in = (pfcc_get_detect_info_in*) BindRows->data();
	...
	sprintf(stext,"select char_set from pfct_agent where agent_id = %lld",param_in->agent_id);
	...
	bind_vars.add();
	bind_vars.next();
	memcpy(bind_vars.getColPtr(1), param_in->file_name, strlen(param_in->file_name));
	memcpy(bind_vars.getColPtr(2), param_in->parameter, strlen(param_in->parameter));
	...
	if (convert_flag) {
pr_debug("before file_name[%s] file_name_len[%d] param[%s] param_len[%d]\n", info_in->file_name, (dgt_sint32)strlen(info_in->file_name, info_in->parameter, (dgt_sint32)strlen(info_in->parameter)));
		memset(conv_file_name,0,sizeof(conv_file_name));
		memset(conv_parameter,0,sizeof(conv_parameter));
		if ((rtn = charset->convert(info_in->file_name,strlen(info_in->file_name),conv_file_name,sizeof(conv_file_name))) > 0) {
			memset(info_in->file_name,0,sizeof(info_in->file_name));
			memcpy(info_in->file_name,conv_file_name,sizeof(info_in->file_name));
		}
		if ((rtn = charset->convert(info_in->parameter,strlen(info_in->parameter),conv_parameter,sizeof(conv_parameter))) > 0) {
			memset(info_in->parameter,0,sizeof(info_in->parameter));
			memcpy(info_in->parameter,conv_parameter,sizeof(info_in->parameter));
		}
		//for fetch - when fetch, file_name must be converting to utf-8
		charset->setCharSet("UTF-8",agent_charset);
	}
pr_debug("file_name[%s] file_name_len[%d] param[%s] param_len[%d] converting_flag [%d]\n", info_in->file_name, (dgt_sint32)strlen(info_in->file_name), info_in->parameter, (dgt_sint32)strlen(info_in->parameter), convert_flag);
	...
	while (rtn_rows && rtn_rows->next()) {
		pcct_detect_info_out* info_out = (pcct_detect_info_out*)rtn_rows->data();
		if (convert_flag) {
			dgt_schar conv_expr[1024]={0};
			dgt_schar conv_data[1024]={0};
			if ((rtn = charset->convert(info_out->expr,strlen(info_out->expr),conv_expr,sizeof(conv_expr))) > 0) {
				memset(info_out->expr,0,strlen(info_out->expr));
				memcpy(info_out->expr,conv_expr,strlen(conv_expr));
			}
			if ((rtn = charset->convert(info_out->data,strlen(info_out->data),conv_data,sizeof(conv_data))) > 0) {
...
pr_debug("so[%lld] eo[%lld] data_seq[%d] expr[%s] data[%s]\n", info_out->start_offset, info_out->end_offset, info_out->data_seq, info_out->expr, info_out->data);
	}
