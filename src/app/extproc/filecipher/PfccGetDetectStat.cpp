...
dgt_sint32 PfccGetDetectStat::initialize() throw(DgcExcept) { return 0; }
...
dgt_sint32 PfccGetDetectStat::execute() throw(DgcExcept) {
    ...
    sprintf(stext,
            "select file_id, pttn_num, file_name, parameter from "
            "pct_file_detect_hist where pttn_num != 0 and job_id = %lld",
            job_id);
...
dgt_sint32 PfccGetDetectStat::fetch() throw(DgcExcept) { return 0; }
