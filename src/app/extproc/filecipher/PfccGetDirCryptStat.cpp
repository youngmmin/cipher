...
PfccGetDirCryptStat::PfccGetDirCryptStat(const dgt_schar* name, PfccAgentListener* agent_listener)
...
dgt_sint32 PfccGetDirCryptStat::initialize() throw(DgcExcept)
{
	return 0; 
}
...
dgt_sint32 PfccGetDirCryptStat::execute() throw(DgcExcept)
{
	...
	pfcc_get_dir_crypt_stat_in* stat_in = (pfcc_get_dir_crypt_stat_in*) BindRows->data();
	...
	bind_vars.add();
	bind_vars.next();
	pcct_get_dir_crypt_stat* dir_crypt_stat = (pcct_get_dir_crypt_stat*)bind_vars.data();
	...
	if (cli_stmt->execute(10,&bind_vars) < 0) {
		...
		//therefore when occur error "Interrupted system call" so change message to "agent[%lld] sess_id[%lld] finalized"
		DgcError*       err=e->getErr();
		const dgt_schar*    err_msg="";
		while(err->next()) err=err->next();
		err_msg=(dgt_schar*)err->message();
		if (strncmp(err_msg,"Interrupted system call",strlen("Interrupted system call")) == 0) {
		DgcWorker::PLOG.tprintf(0,"agent[%lld] sess_id[%lld] finalized\n",AgentSession->agentID(),AgentSession->sessID());
		delete e;
		return 0;
		} else
