...
PfccGetFileInfo::PfccGetFileInfo(const dgt_schar* name, PfccAgentListener* agent_listener)
...
PfccGetFileInfo::~PfccGetFileInfo()
{
...
dgt_sint32 PfccGetFileInfo::initialize() throw(DgcExcept) 
{
	return 0; 
}
...
dgt_sint32 PfccGetFileInfo::execute() throw(DgcExcept)
{
	...
	strncpy(dir_entry_in->dir_path, info_in->dir_path, strlen(info_in->dir_path));
...
pr_debug("before dir_entry_in->dir_path [%s], dir_path_len [%d]\n", dir_entry_in->dir_path, strlen(dir_entry_in->dir_path));
		...
		rtn = charset->convert(dir_entry_in->dir_path,strlen(dir_entry_in->dir_path),conv_name,1024);
		if (rtn > 0) {
			memset(dir_entry_in->dir_path,0,sizeof(dir_entry_in->dir_path));
			memcpy(dir_entry_in->dir_path,conv_name,sizeof(dir_entry_in->dir_path));
		}
		//for fetch - when fetch, file_name must be converting to utf-8
		charset->setCharSet("UTF-8",agent_charset);
	}

pr_debug("dir_entry_in->dir_path [%s], strlen(dir_entry_in->dir_path) [%d], dir_entry_in->offset [%lld] dir_entry_in->fetch_count [%d] converting_flag [%d]\n", dir_entry_in->dir_path, strlen(dir_entry_in->dir_path), dir_entry_in->offset, dir_entry_in->fetch_count,convert_flag);
...
pr_debug("first fetch start\n");
	...
	//8. fetch file name converting
				if (convert_flag) {
					memset(conv_name,0,1024);
					rtn = charset->convert(de->name,strlen(de->name),conv_name,1024);
					if (rtn > 0) {
