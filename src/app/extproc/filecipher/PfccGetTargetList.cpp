...
PfccGetTargetList::PfccGetTargetList(const dgt_schar* name,
...
dgt_sint32 PfccGetTargetList::initialize() throw(DgcExcept) { return 0; }
...
dgt_sint32 PfccGetTargetList::execute() throw(DgcExcept) {
    ...
    if (frows < 0) {
        ...
        if (rtn_rows) {
            rtn_rows->rewind();
            while (rtn_rows->next()) {
                pcct_target_list_out* target_list_out =
                    (pcct_target_list_out*)rtn_rows->data();
                target_list_out->job_id = param_in->job_id;
                ReturnRows->add();
                ReturnRows->next();
                if (ReturnRows->data()) {
                    memcpy(ReturnRows->data(), target_list_out,
                           sizeof(pcct_target_list_out));
                }
            }
        }
    }
