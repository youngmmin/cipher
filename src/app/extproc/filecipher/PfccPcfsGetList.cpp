...
PfccPcfsGetList::PfccPcfsGetList(const dgt_schar* name, PfccAgentListener* agent_listener)
...
dgt_sint32 PfccPcfsGetList::initialize() throw(DgcExcept)
{
	return 0; 
}
	...
	if (frows < 0) {
		...
		if (rtn_rows) {
			...
			while(rtn_rows->next()) {
				...
				ReturnRows->next();
				if (ReturnRows->data()) {
					memcpy(ReturnRows->data(),pcfs_list,sizeof(pcfst_fs_list));
				}
			}
		}
	}
...
{
	if (CliStmt->check() < 0) ATHROWnR(DgcError(SPOS,"client stmt check failed"),-1);
	dgt_sint32      frows = CliStmt->fetch(500);
	if (frows < 0) {
		DgcExcept*      e = EXCEPTnC;
		RTHROWnR(e,DgcError(SPOS,"fetch failed"),-1);
