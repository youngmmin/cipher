...
PfccPcfsMount::PfccPcfsMount(const dgt_schar* name, PfccAgentListener* agent_listener)
...
dgt_sint32 PfccPcfsMount::initialize() throw(DgcExcept)
{
	return 0; 
}
...
dgt_sint32 PfccPcfsMount::execute() throw(DgcExcept)
{
	...
	AgentSession = AgentListener->agentSessPool().getSession(mount_in->agent_id);
	if (AgentSession == 0) {
