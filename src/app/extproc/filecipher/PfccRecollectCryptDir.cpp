...
PfccRecollectCryptDir::PfccRecollectCryptDir(const dgt_schar* name,
...
dgt_sint32 PfccRecollectCryptDir::initialize() throw(DgcExcept) { return 0; }
...
dgt_sint32 PfccRecollectCryptDir::execute() throw(DgcExcept) {
    ...
    pcct_recollect_crypt_dir_in* param_in =
        (pcct_recollect_crypt_dir_in*)BindRows->data();
    ...
    if (frows < 0) {
        ...
        if (rtn_rows) {
            rtn_rows->rewind();
            if (rtn_rows->next()) {
                pcct_recollect_crypt_dir_out* recollect_out =
                    (pcct_recollect_crypt_dir_out*)rtn_rows->data();
                ReturnRows->add();
                ReturnRows->next();
                ...
                if (result_out) {
                    ...
                    if (result_out->rtn_code) {
                        dgt_sint32 err_msg_len =
                            dg_strlen(recollect_out->error_message);
                        strncpy(result_out->error_message,
                                recollect_out->error_message,
