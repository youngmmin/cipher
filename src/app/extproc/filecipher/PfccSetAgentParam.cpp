...
PfccSetAgentParam::PfccSetAgentParam(const dgt_schar* name,
...
dgt_sint32 PfccSetAgentParam::initialize() throw(DgcExcept) { return 0; }
...
dgt_sint32 PfccSetAgentParam::execute() throw(DgcExcept) {
    ...
    pfct_set_agent_param_in* param_in =
        (pfct_set_agent_param_in*)BindRows->data();
    ...
    DgcIndexSegment* enc_job_idx =
        PetraTableHandler->getIndex("pfct_enc_job_idx1");
    if (!enc_job_idx)
        ATHROWnR(DgcError(SPOS, "getIndex[pfct_enc_job_idx1] failed"), -1);
    ...
    if (job_row_list.numRows() == 0)
        THROWnR(
            DgcDgExcept(DGC_EC_DG_INVALID_STAT,
                        new DgcError(SPOS, "pfct_enc_job[%lld] data not found",
                                     param_in->job_id)),
            -1);
    job_row_list.rewind();

    pfct_type_enc_job* enc_job = 0;
    if (job_row_list.next() &&
    ...
    if (agent_last_update < last_update) {
        ...
        if (!(rtn = sql_handle.fetch(rtn_row)) && rtn_row) {
            ...
            if (strlen(agent_charset) &&
                ((strncasecmp(agent_charset, "EUC-KR", sizeof("EUC-KR")) ==
                  0) ||
        ...
        if (convert_flag) {
            PcaCharSetCnvt* charset =
                new PcaCharSetCnvt(agent_charset, "UTF-8");
            rtn = charset->convert(param_builder.agentParam(), agent_param_size,
                                   conv_agent_param, agent_param_size);
            if (rtn < 0) {
                ...
                THROWnR(
                    DgcDgExcept(
                        ...
                        new DgcError(
                            SPOS,
                            "agent[%lld] char conversion failed: param [%s]\n",
                            param_in->agent_id, param_builder.agentParam())),
                    -1);
            }
            ...
            memcpy(conv_agent_param, param_builder.agentParam(),
                   agent_param_size);

        // printf("conv_agent_param [%s], agent_param_size [%d],
        // strlen(conv_agent_param) [%d] rtn [%d]
        ...
        if (enc_job) {
            ...
            job_row_list.rewind();
            if (enc_job_tab->updateCommit(DgcDbProcess::sess(), job_row_list) <
                0) {
