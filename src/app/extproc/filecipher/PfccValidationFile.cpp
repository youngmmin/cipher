...
PfccValidationFile::PfccValidationFile(const dgt_schar* name,
...
dgt_sint32 PfccValidationFile::initialize() throw(DgcExcept) { return 0; }
...
dgt_sint32 PfccValidationFile::execute() throw(DgcExcept) {
    ...
    pfct_validation_file_in* param_in =
        (pfct_validation_file_in*)BindRows->data();
    ...
    bind_vars.add();
    bind_vars.next();
    pcct_validation_file_in* validation_file_in =
        (pcct_validation_file_in*)bind_vars.data();
    validation_file_in->ptu_id = param_in->ptu_id;
    strncpy(validation_file_in->client_ip, param_in->client_ip,
            dg_strlen(param_in->client_ip));
    strncpy(validation_file_in->validation_file_path, param_in->file_path,
            dg_strlen(param_in->file_path) > 2048
                ? 2048
                : dg_strlen(param_in->file_path));

    // DgcWorker::PLOG.tprintf(0,"procedure validation_file_path : %s
    // param_in->file_path : %s param_in->client_ip %s param_in->ptu_id
