...
dgt_sint32 PfccVerifyCryptParam::checkKeyExists(const dgt_schar* key_name) throw(DgcExcept)
{
	...
	dgt_uint8* ret = 0;
	sprintf(sql_text,"select ENC_TAB_ID from PCT_ENC_TABLE where TABLE_NAME like '%s'", key_tab);
	...
	sprintf(sql_text,"select ENC_COL_ID from PCT_ENC_COLUMN where ENC_TAB_ID = %lld and COLUMN_NAME like '%s'", enc_tab_id, key_col);
	if (sql_handle.execute(sql_text) < 0) ATHROWnR(DgcError(SPOS,"execute failed"), -1);
...
dgt_sint32 PfccVerifyCryptParam::execute() throw(DgcExcept)
{
	...
	pfcc_verify_crypt_param_in* param_in = (pfcc_verify_crypt_param_in*) BindRows->data();
	...
	while(param_in->crypt_param[idx] != '(') {
		if(param_in->crypt_param[idx] == ' ') {
			...
			sprintf(param_out.err_msg,"invalid param format [%s]", param_in->crypt_param);
	...
	while((bg=ParamList->getNext())) {
		if (bg->getNode("key")) {
			for (dgt_sint32 col_no=1;;col_no++) {
				...
				if ((enc_name=bg->getValue(node_name))) {
					...
					if (!(col_string=bg->getValue(node_name))) {
						...
						sprintf(param_out.err_msg,"not define columns in key node [col_no:%d]",col_no);
						memcpy(ReturnRows->data(),&param_out,sizeof(pfcc_verify_crypt_param_out));
						ReturnRows->rewind();
						return 0;
					}
					if (checkKeyExists(enc_name)) {	// check if key exists
						...
						sprintf(param_out.err_msg,"not found key node[col_no:%d]'s key[%s] on server",col_no,enc_name);
						memcpy(ReturnRows->data(),&param_out,sizeof(pfcc_verify_crypt_param_out));
						ReturnRows->rewind();
						return 0;
					}
					...
					if (col_no == 1) {
						...
						sprintf(param_out.err_msg,"not define name in key node [col_no:%d]",col_no);
						memcpy(ReturnRows->data(),&param_out,sizeof(pfcc_verify_crypt_param_out));
						ReturnRows->rewind();
						return 0;
					}
			...
			if (bg->getValue("mode.crypt")) {
				...
				sprintf(param_out.err_msg,"unsupported parameter \"mode.crypt\"");
				memcpy(ReturnRows->data(),&param_out,sizeof(pfcc_verify_crypt_param_out));
				ReturnRows->rewind();
				return 0;
			}
			...
			for(col_no=1;;col_no++) {
				...
				for(expr_no=1;;expr_no++) {
					...
					if ((val=bg->getValue(expr_string))) { 	//check expr_string
						regex_t regex;
						dgt_sint32 ret = 0;
						dgt_schar err_msg[100]={0};
						if ((ret=regcomp(&regex, val, REG_EXTENDED))) {
							regerror(ret, &regex, err_msg, 100);
							param_out.rtn_code = PFC_VRFY_ERR_REGUALR_REGEX_ERROR;
