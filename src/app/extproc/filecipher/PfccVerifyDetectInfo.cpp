...
PfccVerifyDetectInfo::PfccVerifyDetectInfo(const dgt_schar* name, PfccAgentListener* agent_listener)
...
dgt_sint32 PfccVerifyDetectInfo::initialize() throw(DgcExcept) 
{
	return 0; 
}
...
dgt_sint32 PfccVerifyDetectInfo::execute() throw(DgcExcept)
{
	...
	pfcc_verify_detect_info_in* param_in = (pfcc_verify_detect_info_in*) BindRows->data();
	...
	// prepare bind data
	memset(stext, 0, sizeof(stext));
	sprintf(stext,"select file_name,file_size,file_mtime from pct_file_detect_hist where job_id = %lld and dir_id = %lld and file_id = %lld", param_in->job_id, param_in->dir_id, param_in->file_id);
	if (sql_handle.execute(stext) < 0) ATHROWnR(DgcError(SPOS,"execute failed"), -1);
	if (sql_handle.fetch(rtn_row) < 0 || !rtn_row) ATHROWnR(DgcError(SPOS,"fetch failed"), -1);
	...
	pcct_verify_detect_info_in* info_in = (pcct_verify_detect_info_in*)bind_vars.data();
...
pr_debug("file_name[%s] file_name_len[%d] converting_flag[%d]\n", info_in->file_name, (dgt_sint32)strlen(info_in->file_name), convert_flag);
	...
	if (frows < 0) {		
		...
		if (rtn_rows) {
			...
			if(rtn_rows->next()) {
				ReturnRows->add();
				ReturnRows->next();
				memcpy(ReturnRows->data(),rtn_rows->data(),sizeof(pcct_verify_detect_info_out));
