...
PfccVerifyExpr::PfccVerifyExpr(const dgt_schar* name)
...
dgt_sint32 PfccVerifyExpr::execute() throw(DgcExcept)
{
	...
	regex_t regex;
	dgt_sint32 ret = 0;
	if ((ret=regcomp(&regex, (dgt_schar*)param_in->expr, REG_EXTENDED)) == 0) {
		regmatch_t	pttn_match[1];
		if ((ret=regexec(&regex,param_in->reg,1,pttn_match,0)) == 0) {
			if ((pttn_match[0].rm_eo - pttn_match[0].rm_so) != (dgt_sint32)strlen(param_in->reg)) {
				ret = REG_NOMATCH;
			}
		}
	}
