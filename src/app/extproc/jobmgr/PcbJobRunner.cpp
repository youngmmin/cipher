...
dgt_sint32 PcbJobRunner::startJob(dgt_schar* arg1,dgt_schar* arg2) throw(DgcExcept)
{
	pid_t		pid=0;
	if ((pid=fork()) == 0) {
		//
		...
		// execv error
		//
		printf("execv[pcb_job %s %s] failed due to [%s].\n", arg1, arg2, strerror(errno));
		exit(0);
	} else if (pid < 0) {
		THROWnR(DgcDgExcept(DGC_EC_PD_NOT_FOUND,new DgcError(SPOS,"fork failed [%s]",strerror(errno))),-1);
	}
	return 0;
}
...
dgt_sint32 PcbJobRunner::startJob(dgt_sint64 enc_tab_id,dgt_sint16 target_step) throw(DgcExcept)
{
	dgt_schar	arg1[32];
