...
PccEncrypt::PccEncrypt(const dgt_schar* name)
...
DgcExtProcedure* PccEncrypt::clone()
{
	return new PccEncrypt(procName());
}
...
dgt_sint32 PccEncrypt::execute() throw(DgcExcept)
{
	...
	sprintf(sql_text,"select max(step_no) from pct_script where enc_tab_id=%lld", *enc_tab_id);
	...
	if (PcbJobRunner::startJob(*enc_tab_id,max_step_no)) {
		ATHROWnR(DgcError(SPOS,"startJob[%lld,%d] failed", *enc_tab_id, max_step_no),-1);
	}
	ReturnRows->reset();
	ReturnRows->add();
	ReturnRows->next();
	*(ReturnRows->data())=0;
	dg_sprintf((dgt_schar*)ReturnRows->data(),"encrypting[%d] started",max_step_no);
	ReturnRows->rewind();
	return 0;
}
