...
dgt_sint32 PccJobMgrFactory::initialize() throw(DgcExcept) {
    ...
    if ((wa = DgcDbProcess::db().getWorker(DgcDbProcess::sess())) == 0) {
        ...
        DgcWorker::PLOG.tprintf(
            0, *e, "getWorker for scheduler failed due to the below:\n");
        delete e;
    } else {
        ...
        if (Scheduler->initialize()) {
            ...
            DgcWorker::PLOG.tprintf(0, *e,
                                    "initialize failed due to the below:\n");
            delete e;
            delete Scheduler;
            Scheduler = 0;
            ...
            DgcExcept* e = EXCEPTnC;
            DgcWorker::PLOG.tprintf(
                0, *e, "start scheduler failed due to the below exception:");
    ...
    if (addProcedure(new PccRunVerifySchedule("PCP_RUN_VERIFY_SCHEDULE")) !=
        0) {
        ATHROWnR(DgcError(SPOS, "addProcedure failed"), -1);
    }
    if (addProcedure(
            new PccRunMigVerifySchedule("PCP_RUN_MIG_VERIFY_SCHEDULE")) != 0) {
        ATHROWnR(DgcError(SPOS, "addProcedure failed"), -1);
    }
...
dgt_sint32 PccJobMgrFactory::finalize() throw(DgcExcept) { return 0; }

dgt_void initServerFactory() {
    static PccJobMgrFactory JobMgrFactory;       // create a JobMgrFactory
    DgcProcServer::initFactory(&JobMgrFactory);  // register the JobMgrFactory
