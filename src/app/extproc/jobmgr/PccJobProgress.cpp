...
PccJobProgress::PccJobProgress(const dgt_schar* name)
	...
	*sel_str=0;
	...
	if(!rtn_row) {
		...
		*(ReturnRows->data())=0;
		...
		dg_memcpy(ReturnRows->data(),&progress_result,sizeof(pct_type_job_progress_result));
		ReturnRows->rewind();
		if(sel_str) delete sel_str;
		return 0;
	}

	pct_type_job_progress_curr_info* job_info=(pct_type_job_progress_curr_info*)rtn_row;
	if(!job_info->curr_status){
		...
		*(ReturnRows->data())=0;
		progress_result.progress_status=-1;
		dg_sprintf(progress_result.remark,"an encryption of job_id[%lld] not in progress.",job_id);
		dg_memcpy(ReturnRows->data(),&progress_result,sizeof(pct_type_job_progress_result));
		ReturnRows->rewind();
		if(sel_str) delete sel_str;
		return 0;
	...
	if(init_enc_type == 0){
		...
		if(!rtn_row) {
			...
			*(ReturnRows->data())=0;
			progress_result.progress_status=-1;
			dg_sprintf(progress_result.remark,"a job_id[%lld] has no workers.",job_id);
			dg_memcpy(ReturnRows->data(),&progress_result,sizeof(pct_type_job_progress_result));
			ReturnRows->rewind();
			if(sel_str) delete sel_str;
			return 0;
		...
		progress_result.progress_status=(dgt_float64)processed_rows/(dgt_float64)total_rows*(dgt_float64)100;
		...
		sprintf(sel_str,
				"select c.admin_link, b.schema_name "
                                 ...
                                 " where a.enc_tab_id = %lld and a.schema_id = b.schema_id and b.db_id = c.db_id",enc_tab_id);
		if (SqlHandle.execute(sel_str) < 0) {
			if(sel_str) delete sel_str;
			ATHROWnR(DgcError(SPOS,"SqlHandle execute failed."),-1);
		}
		...
		if(!rtn_row) {
			...
			*(ReturnRows->data())=0;
			progress_result.progress_status=-1;
			dg_sprintf(progress_result.remark,"no admin_link[%lld] in the table[PCT_DB_AGENT]",enc_tab_id);
			dg_memcpy(ReturnRows->data(),&progress_result,sizeof(pct_type_job_progress_result));
			ReturnRows->rewind();
			if(sel_str) delete sel_str;
			return 0;
		}
		memcpy(&con_info,(pc_type_conn_info*)rtn_row,sizeof(pc_type_conn_info));

		// 2.get a table info
		dg_memset(sel_str,0,512);
		rtn_row=0;
		sprintf(sel_str,
				"select * from pct_enc_table where enc_tab_id=%lld ",enc_tab_id);
		...
		if(!rtn_row) {
			...
			*(ReturnRows->data())=0;
			progress_result.progress_status=-1;
			dg_sprintf(progress_result.remark,"no enc_tab_id[%lld] in the table[PCT_ENC_TABLE]",enc_tab_id);
			dg_memcpy(ReturnRows->data(),&progress_result,sizeof(pct_type_job_progress_result));
			ReturnRows->rewind();
			if(sel_str) delete sel_str;
			return 0;
		...
		switch(DatabaseLink->getDbType()){
				case DGC_DB_TYPE_SOHA:
					if(sel_str) delete sel_str;
					THROWnR(DgcLdbExcept(DGC_EC_LD_STMT_ERR,new DgcError(SPOS,"[%s] db type is not in service yet.", con_info.db_link)),-1);
				...
				case DGC_DB_TYPE_ORACLE:
				{
				    // get original table`s bytes
                                    ...
                                    *ora_str=0;
                                        sprintf(ora_str,
                                                ...
                                                "WHERE OWNER='%s' "
                                                "AND SEGMENT_NAME='%s' "
                                                "AND SEGMENT_TYPE IN ('TABLE','TABLE PARTITION','NESTED TABLE') "
                                    ...
                                    if (ora_stmt->execute(ora_str,strlen(ora_str),1) < 0) {
                                        ...
                                        if(sel_str) delete sel_str;
                                        if(ora_str) delete ora_str;
                                        RTHROWnR(e,DgcError(SPOS,"execute[%*.*s] failed",64,64,ora_str),-1);
                                    }
                                    DgcMemRows* rtn_rows=ora_stmt->returnRows();
                                    if(rtn_rows && rtn_rows->numRows() > 0) {
                                        rtn_rows->rewind();
                                        rtn_rows->next();
                                        org_table_size=dg_strtoll((dgt_schar*)rtn_rows->getColPtr(1),0,10);
                                    } else {
                                        //no data found
                                        progress_result.progress_status=-100;
                                        dg_sprintf(progress_result.remark,"no segment info[%s:%s] in oracle",con_info.owner,enc_table.table_name);
                                    }
                                    if(ora_stmt) delete ora_stmt;
                                    if(ora_str) delete ora_str;
				    ...
				    *ora_str=0;
				    ...
				    if(rtn_rows && rtn_rows->numRows() > 0) {
				    	rtn_rows->rewind();
				    	rtn_rows->next();
				    	curr_table_size=dg_strtoll((dgt_schar*)rtn_rows->getColPtr(1),0,10);
				    	progress_result.progress_status=(dgt_float64)curr_table_size/(dgt_float64)org_table_size*(dgt_float64)100;
				    } else {
				    	//no data found
				    	progress_result.progress_status=-100;
				    	dg_sprintf(progress_result.remark,"no segment info[%s:%s] in oracle",con_info.owner,table_name);
				    }
				    if(ora_stmt) delete ora_stmt;
				    if(ora_str) delete ora_str;
					break;
				...
				default:
					if(sel_str) delete sel_str;
					THROWnR(DgcLdbExcept(DGC_EC_LD_STMT_ERR,new DgcError(SPOS,"[%s] db type is invaild.", con_info.db_link)),-1);
					break;
		}
	} else {
		//
		...
		*(ReturnRows->data())=0;
		...
		dg_sprintf(progress_result.remark,"invalid init_enc_type[%d]",init_enc_type);
		dg_memcpy(ReturnRows->data(),&progress_result,sizeof(pct_type_job_progress_result));
		ReturnRows->rewind();
		if(sel_str) delete sel_str;
		return 0;
	...
	if(progress_result.curr_status == 20000) progress_result.progress_status=100;
	//else if(progress_result.progress_status>100) progress_result.progress_status=99.99;
