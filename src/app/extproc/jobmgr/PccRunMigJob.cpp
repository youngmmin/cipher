...
PccRunMigJob::PccRunMigJob(const dgt_schar* name)
	: DgcExtProcedure(name)
{
}


PccRunMigJob::~PccRunMigJob()
{
}


DgcExtProcedure* PccRunMigJob::clone()
{
	return new PccRunMigJob(procName());
}
...
dgt_sint32 PccRunMigJob::execute() throw(DgcExcept)
{
