...
PccRunMigSchedule::PccRunMigSchedule(const dgt_schar* name)
    : DgcExtProcedure(name) {}

PccRunMigSchedule::~PccRunMigSchedule() {}

DgcExtProcedure* PccRunMigSchedule::clone() {
    return new PccRunMigSchedule(procName());
}

#include "PetraCipherSchedule.h"

dgt_sint32 PccRunMigSchedule::execute() throw(DgcExcept) {
