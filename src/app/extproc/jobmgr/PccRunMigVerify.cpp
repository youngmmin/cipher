...
dgt_sint32 PccRunMigVerify::execute() throw(DgcExcept)
{
	...
	sprintf(sql_text,"select b.* from pct_verify_job a, pct_schedule b where a.verify_job_id=%lld and a.schedule_id=b.schedule_id", *job_id);
	...
	delete sql_stmt;
        ...
        sprintf(sql_text,"update PCT_VERIFY_JOB set(ROW_VERIFY_CNT_RESULT,DATA_VERIFY_RESULT)=(0,0) where VERIFY_JOB_ID=%lld",*job_id);
	...
	PetraCipherMigVerifySchedule*	pcs=new PetraCipherMigVerifySchedule(schedule_id,wa,*job_id);
	if (pcs->initialize()) {
		DgcExcept*	e=EXCEPTnC;
		delete pcs;
