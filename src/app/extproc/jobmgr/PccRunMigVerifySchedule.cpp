...
dgt_sint32 PccRunMigVerifySchedule::execute() throw(DgcExcept)
{
	...
	memset(sql_text,0,512);
        sprintf(sql_text,"update PCT_VERIFY_JOB set(ROW_VERIFY_CNT_RESULT,DATA_VERIFY_RESULT)=(0,0) where SCHEDULE_ID=%lld",*schedule_id);
	...
	PetraCipherMigVerifySchedule*	pcs=new PetraCipherMigVerifySchedule(*schedule_id,wa,0);
	if (pcs->initialize()) {
		DgcExcept*	e=EXCEPTnC;
		delete pcs;
