...
PccRunVerify::PccRunVerify(const dgt_schar* name)
	: DgcExtProcedure(name)
{
}


PccRunVerify::~PccRunVerify()
{
}


DgcExtProcedure* PccRunVerify::clone()
{
	return new PccRunVerify(procName());
}
...
dgt_sint32 PccRunVerify::execute() throw(DgcExcept)
{
	...
	sprintf(sql_text,"select b.* from pct_job a, pct_schedule b where a.job_id=%lld and a.schedule_id=b.schedule_id", *job_id);
	...
	PetraCipherSchedule*	pcs=new PetraCipherSchedule(schedule_id,wa,*job_id);
	if (pcs->initialize()) {
		DgcExcept*	e=EXCEPTnC;
		delete pcs;
		RTHROWnR(e,DgcError(SPOS,"initialize[Schedule:%lld:%lld] failed",schedule_id,*job_id),-1);
	}
	if (pcs->start() != 0) {
		DgcExcept*	e=EXCEPTnC;
		if (e->classid() == DGC_EXT_WORKER) delete pcs;
		RTHROWnR(e,DgcError(SPOS,"start[Schedule:%lld:%lld] failed",schedule_id,*job_id),-1);
	}
	ReturnRows->reset();
	ReturnRows->add();
	ReturnRows->next();
	*(ReturnRows->data())=0;
	dg_sprintf((dgt_schar*)ReturnRows->data(),"schedule[%lld:%lld] started",schedule_id,*job_id);
	ReturnRows->rewind();
	return 0;
}
