...
PccStopJob::PccStopJob(const dgt_schar* name)
...
DgcExtProcedure* PccStopJob::clone()
{
	return new PccStopJob(procName());
}
...
dgt_sint32 PccStopJob::execute() throw(DgcExcept)
{
	...
	delete sql_stmt;
	if (job.curr_status > 0 && job.curr_status <  PetraCipherSchedule::PCB_JOB_STATUS_PENDING) {
		//
		// job in processing
		//
		kill(job.process_id, 9);
		sprintf(sql_text,"update pct_job set(curr_status,curr_err_msg)=(%d,'stoped by user') where job_id=%lld",
		...
		delete sql_stmt;
		sprintf(sql_text,"update pct_enc_table set(curr_enc_step,curr_enc_stmt)=(%d,%d) where enc_tab_id=%lld",
                         job.curr_enc_step, job.curr_enc_stmt-1, job.enc_tab_id);
                sql_stmt=Database->getStmt(Session, sql_text, strlen(sql_text));
                if (sql_stmt == 0 || sql_stmt->execute() < 0) {
                        DgcExcept*      e=EXCEPTnC;
                        delete sql_stmt;
                        RTHROWnR(e,DgcError(SPOS,"updating pct_enc_table`s curr_enc_step failed"), -1);
                }
	} else {
		THROWnR(DgcLdbExcept(DGC_EC_LD_STMT_ERR,new DgcError(SPOS,"The Job[%lld] not in processing",*job_id)),-1);
	}
	ReturnRows->reset();
	ReturnRows->add();
	ReturnRows->next();
