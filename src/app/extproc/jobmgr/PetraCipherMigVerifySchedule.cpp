...
PetraCipherMigVerifySchedule::PetraCipherMigVerifySchedule(
    dgt_sint64 schedule_id, dgt_worker* wa, dgt_sint64 job_id)
...
RETRY_SCHEDULE:
    ...
    if ((idx = (DgcIndexSegment*)DgcDbProcess::db().pdb()->idxMgr()->getIndex(
             "PCT_VERIFY_JOB_IDX1")) == 0) {
        ScheduleRowPtr->curr_status = 0;
        ATHROW(DgcError(SPOS, "getIndex failed"));
        THROW(
            DgcLdbExcept(DGC_EC_PD_NOT_FOUND,
                         new DgcError(SPOS, "index[VERIFY_JOB_ID] not found")));
    }
    job = 0;
    memset(soha_str, 0, 1024);
    sprintf(soha_str,
            "select * from pct_verify_job where schedule_id=%lld and "
            "exe_order>%d order by exe_order",
            ScheduleID, exe_order);
    ...
    while (!(ret = SqlHandle.fetch(rtn_row)) && rtn_row) {
        ...
        if (JobRows.numRows() == 0) {
            ScheduleRowPtr->curr_status = 0;
            THROW(DgcDgExcept(
                DGC_EC_DG_TAB_NOT_FOUND,
                new DgcError(SPOS, "no jobs in schedule[%lld]", ScheduleID)));
        }
        JobRows.rewind();

        while (JobRows.next() &&
               ((job = (pct_type_verify_job*)JobRows.data()))) {
            if (job_order->verify_job_id == job->verify_job_id &&
        ...
        if (PcbJobRunner::startMigVerify(job->verify_job_id)) {
            ...
            DgcWorker::PLOG.tprintf(0, *e,
                                    "startJob[%lld] failed due to the below:\n",
                                    job->verify_job_id);
            delete e;
        } else {
            //
            ...
            //
            for (dgt_sint32 i = 0; i < 100 && job->process_id == 0; i++)
                napAtick();
            if (job->process_id == 0) {
                //
                ...
                DgcWorker::PLOG.tprintf(0,
                                        "startJob[%lld] failed due to delaied "
                                        "process id setting.\n",
                                        job->verify_job_id);
            } else {
                while (!(kill(job->process_id, 0) != 0 && errno == ESRCH))
                    napAtick();
            }
        }
    ...
    if (JobID == 0) {
        memset(soha_str, 0, 1024);
        sprintf(soha_str,
                "select count() from pct_verify_job where schedule_id=%lld and "
                "exe_order>%d",
                ScheduleID, exe_order);
        if (SqlHandle.execute(soha_str) < 0) {
            ATHROW(DgcError(SPOS, "SqlHandle execute failed."));
    ...
    while (JobRows.next() && ((job = (pct_type_verify_job*)JobRows.data()))) {
        switch (job->curr_status) {
            ...
            case PetraCipherMigVerifyJob::PCB_JOB_STATUS_SCHEDULING:
                if (ScheduleRowPtr->curr_status ==
                    PetraCipherMigVerifyJob::PCB_JOB_STATUS_DONE) {
                    ScheduleRowPtr->curr_status =
                        PetraCipherMigVerifyJob::PCB_JOB_STATUS_SCHEDULING;
                }
                break;
            case PetraCipherMigVerifyJob::PCB_JOB_STATUS_PENDING:
                ScheduleRowPtr->curr_status =
                    PetraCipherMigVerifyJob::PCB_JOB_STATUS_PENDING;
                break;
            default:
                //
                // must change this job's status to PCB_JOB_STATUS_PENDING:
                // update pct_verify_job
                // set(curr_status)=(PetraCipherMigVerifyJob::PCB_JOB_STATUS_PENDING)
                // where job_id=job->job_id; except a default schedule!
                //
                if (ScheduleRowPtr->schedule_id != 1) {
                    ...
                    sprintf(soha_str,
                            "update pct_verify_job set(curr_status)=(%d) where "
                            "verify_job_id=%lld",
                            PetraCipherMigVerifyJob::PCB_JOB_STATUS_PENDING,
                            job->verify_job_id);
                    if (SqlHandle.execute(soha_str) < 0) {
...
dgt_sint32 PetraCipherMigVerifySchedule::initialize() throw(DgcExcept) {
    ...
    if (ScheduleSeg == 0) {
        ...
        THROWnR(
            DgcDgExcept(DGC_EC_DG_TAB_NOT_FOUND,
                        new DgcError(SPOS, "table[PCT_SCHEDULE] not found")),
            -1);
    }
    ...
    while (AllScheduleRows.next() &&
           (ScheduleRowPtr = (pct_type_schedule*)AllScheduleRows.data())) {
        if (ScheduleRowPtr->schedule_id == ScheduleID) {
            if (ScheduleRowPtr->schedule_id != 1 &&
                ...
                THROWnR(
                    DgcDgExcept(DGC_EC_DG_TAB_NOT_FOUND,
                                new DgcError(SPOS, "Schedule[%lld] is running",
                                             ScheduleID)),
                    -1);
            }
            ScheduleRowPtr->curr_status = 10;
            ScheduleRows.add(AllScheduleRows.bno(), AllScheduleRows.rno(),
                             AllScheduleRows.data());
            break;
        }
        ScheduleRowPtr = 0;
    }
    if (!ScheduleRowPtr) {
        ...
        THROWnR(DgcDgExcept(
                    DGC_EC_DG_TAB_NOT_FOUND,
                    new DgcError(SPOS, "Schedule[%lld] not found", ScheduleID)),
                -1);
    }
    ...
    if (JobSeg == 0) {
        ...
        THROWnR(
            DgcDgExcept(DGC_EC_DG_TAB_NOT_FOUND,
                        new DgcError(SPOS, "table[PCT_VERIFY_JOB] not found")),
            -1);
    }
