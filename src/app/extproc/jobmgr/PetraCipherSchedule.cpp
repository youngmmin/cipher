...
PetraCipherSchedule::PetraCipherSchedule(dgt_sint64 schedule_id, dgt_worker* wa,
                                         dgt_sint64 job_id)
...
dgt_void PetraCipherSchedule::in() throw(DgcExcept) {
    ...
    RETRY_SCHEDULE:
        ...
        if ((idx =
                 (DgcIndexSegment*)DgcDbProcess::db().pdb()->idxMgr()->getIndex(
                     "PCT_JOB_IDX1")) == 0) {
            ScheduleRowPtr->curr_status = 0;
            ATHROW(DgcError(SPOS, "getIndex failed"));
            THROW(DgcLdbExcept(
                DGC_EC_PD_NOT_FOUND,
                new DgcError(SPOS, "index[PCT_JOB_IDX1] not found")));
        }

        job = 0;
        memset(soha_str, 0, 1024);
        sprintf(soha_str,
                "select * from pct_job where schedule_id=%lld and exe_order>%d "
                "order by exe_order",
        ...
        while (!(ret = SqlHandle.fetch(rtn_row)) && rtn_row) {
            ...
            if (JobRows.numRows() == 0) {
                ...
                THROW(
                    DgcDgExcept(DGC_EC_DG_TAB_NOT_FOUND,
                                new DgcError(SPOS, "no jobs in schedule[%lld]",
                                             ScheduleID)));
            }
            JobRows.rewind();

            while (JobRows.next() && ((job = (pct_type_job*)JobRows.data()))) {
                if (job_order->job_id == job->job_id &&
            ...
            if (PcbJobRunner::startJob(job->job_id)) {
                ...
                DgcWorker::PLOG.tprintf(
                    0, *e, "startJob[%lld] failed due to the below:\n",
                    job->job_id);
                delete e;
            } else {
                //
                ...
                if (job->process_id == 0) {
                    ...
                    DgcWorker::PLOG.tprintf(0,
                                            "startJob[%lld] failed due to "
                                            "delaied process id setting.\n",
                                            job->job_id);
                } else {
        ...
        if (JobID == 0) {
            memset(soha_str, 0, 1024);
            sprintf(soha_str,
                    "select count() from pct_job where schedule_id=%lld and "
                    "exe_order>%d",
                    ScheduleID, exe_order);
            if (SqlHandle.execute(soha_str) < 0) {
                ATHROW(DgcError(SPOS, "SqlHandle execute failed."));
    ...
    while (JobRows.next() && ((job = (pct_type_job*)JobRows.data()))) {
        switch (job->curr_status) {
            ...
            default:
                ...
                // update pct_job set(curr_status)=(PCB_JOB_STATUS_PENDING)
                ...
                if (ScheduleRowPtr->schedule_id != 1) {
                    ...
                    dg_memset(soha_str, 0, 1024);
                    sprintf(soha_str,
                            "update pct_job set(curr_status)=(%d) where "
                            "job_id=%lld",
