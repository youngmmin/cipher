...
dgt_sint32 PetraCipherScheduler::run() throw(DgcExcept) {
    ...
    while (AllScheduleRows.next() &&
           ((schedule = (pct_type_schedule*)AllScheduleRows.data()))) {
        dgt_time ct = dgtime(&ct);
        if (!strcasecmp(schedule->executing_mode, "time-base") &&
            schedule->curr_status == 0 && ct >= schedule->start_time &&
            (ct - schedule->start_time) < 3) {
            //
            // find a schedule reaching its starting time
            //
            dgt_worker* wa;
            if ((wa = DgcDbProcess::db().getWorker(DgcDbProcess::sess())) ==
                0) {
                DgcExcept* e = EXCEPTnC;
                PLOG.tprintf(
                    0, *e,
                    "getWorker for schedule[%lld] failed due to the below:\n",
                ...
                PetraCipherSchedule* pcs =
                    new PetraCipherSchedule(schedule->schedule_id, wa);
                if (pcs->initialize()) {
                    DgcExcept* e = EXCEPTnC;
                    delete pcs;
                    PLOG.tprintf(0, *e,
                                 "initialize[%lld] failed due to the below:\n",
                                 schedule->schedule_id);
                    delete e;
                } else if (pcs->start() != 0) {
                    DgcExcept* e = EXCEPTnC;
                    PLOG.tprintf(0, *e,
                                 "start schedule[%lld] failed due to the below "
                                 "exception:",
                                 schedule->schedule_id);
