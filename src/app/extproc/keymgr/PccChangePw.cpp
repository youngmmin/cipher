...
PccChangePw::PccChangePw(const dgt_schar* name) : DgcExtProcedure(name) {}

PccChangePw::~PccChangePw() {}

DgcExtProcedure* PccChangePw::clone() { return new PccChangePw(procName()); }
...
dgt_sint32 PccChangePw::execute() throw(DgcExcept) {
    ...
    if (key_rows.next()) {
        ...
        dgt_sint64 last_update = iud_log.nextLastUpdate(
            (dgt_schar*)"pct_key", key_row->key_id,
