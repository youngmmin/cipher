 ...
 *   Description        :       close key set- master, encryption key set, ecryption key set signature
...
PccCloseKey::PccCloseKey(const dgt_schar* name)
...
DgcExtProcedure* PccCloseKey::clone()
{
	return new PccCloseKey(procName());
}
...
dgt_sint32 PccCloseKey::execute() throw(DgcExcept)
{
	...
	if (key_rows.next()) {
		...
		dgt_sint32	rtn=0;
		if ((rtn=PCI_closeKey(pw, key_row->smk, strlen(key_row->smk), key_row->seks,
			...
			THROWnR(DgcLdbExcept(DGC_EC_LD_STMT_ERR,new DgcError(SPOS,"%d:%s",rtn,PCI_getKmgrErrMsg())),-1);
                ...
                sprintf(sql_text,"delete pct_key_stat where key_id=%lld",key_row->key_id);
