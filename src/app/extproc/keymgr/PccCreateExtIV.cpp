...
PccCreateExtIV::PccCreateExtIV(const dgt_schar* name)
...
dgt_sint32 PccCreateExtIV::execute() throw(DgcExcept)
{
	...
	if (ek.createIV(eki->iv,eki->format_no,ext_iv.iv_id,&ext_iv.iv_no,ext_iv.seiv,&seiv_len,ext_iv.seivs,&seivs_len)) {
	...
	ReturnRows->next();
	*(ReturnRows->data())=0;
	dg_sprintf((dgt_schar*)ReturnRows->data(),"an external iv[%u:%s] created",ext_iv.iv_no,ext_iv.name);
	ReturnRows->rewind();
	return 0;
}
