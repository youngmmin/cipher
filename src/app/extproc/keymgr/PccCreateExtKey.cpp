...
dgt_sint32 PccCreateExtKey::execute() throw(DgcExcept) {
    ...
    pc_type_create_ext_key_in* eki =
        (pc_type_create_ext_key_in*)BindRows->data();
    //
    // get a key id
    //
    ...
    if (ek.createKey(eki->key, eki->format_no, ext_key.key_id, &ext_key.key_no,
                     ext_key.sek, &sek_len, ext_key.seks, &seks_len)) {
    ...
    ReturnRows->next();
    *(ReturnRows->data()) = 0;
    dg_sprintf((dgt_schar*)ReturnRows->data(), "an external key[%u:%s] created",
               ext_key.key_no, ext_key.name);
    ReturnRows->rewind();
    return 0;
}
