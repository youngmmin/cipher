 ...
 *   Description        :       create key set- master, encryption key set,
...
PccCreateKey::PccCreateKey(const dgt_schar* name) : DgcExtProcedure(name) {}

PccCreateKey::~PccCreateKey() {}

DgcExtProcedure* PccCreateKey::clone() { return new PccCreateKey(procName()); }
...
dgt_sint32 PccCreateKey::execute() throw(DgcExcept) {
    ...
    if ((rtn = PCI_createKey(pw, (void*)this, key_row->smk, &smk_len,
                             key_row->seks, &seks_len, key_row->sks, &sks_len,
    ...
    *(ReturnRows->data()) = 0;
    ...
    sprintf(sql_text,
            "select nextLastUpdate('PCT_KEY',1,1) last_update from dual");
    DgcSqlStmt* sql_stmt =
