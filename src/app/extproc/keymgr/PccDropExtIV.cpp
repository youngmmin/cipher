...
PccDropExtIV::PccDropExtIV(const dgt_schar* name) : DgcExtProcedure(name) {}

PccDropExtIV::~PccDropExtIV() {}

DgcExtProcedure* PccDropExtIV::clone() { return new PccDropExtIV(procName()); }
...
dgt_sint32 PccDropExtIV::execute() throw(DgcExcept) {
    ...
    while (rows.next() && (iv_row = (pct_type_ext_iv*)rows.data())) {
        if (iv_row->iv_id == iv_id) {
            ...
            sprintf(sql_text,
                    "select iv_type from pct_encrypt_key where iv_type = %u",
                    iv_row->iv_no);
            DgcSqlStmt* sql_stmt =
            ...
            if ((kno_ptr = (dgt_uint16*)sql_stmt->fetch())) {
                ...
                THROWnR(DgcLdbExcept(
                            DGC_EC_LD_STMT_ERR,
                            new DgcError(SPOS, "the iv[%lld] is being used",
                                         iv_id)),
                        -1);
            }
            delete EXCEPTnC;
            ...
            memset(sql_text, 0, 256);
            sprintf(sql_text,
                    "DELETE PCT_EXT_IV WHERE iv_id = %lld and "
                    "nextLastUpdate('PCT_EXT_IV', %lld, 3) > 0",
                    iv_id, iv_id);
    ...
    ReturnRows->next();
    *(ReturnRows->data()) = 0;
    dg_sprintf((dgt_schar*)ReturnRows->data(), "the external iv[%lld] dropped",
               iv_id);
    ReturnRows->rewind();
    return 0;
}
