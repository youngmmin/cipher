...
PccDropExtKey::PccDropExtKey(const dgt_schar* name) : DgcExtProcedure(name) {}
...
dgt_sint32 PccDropExtKey::execute() throw(DgcExcept) {
    ...
    while (rows.next() && (key_row = (pct_type_ext_key*)rows.data())) {
        if (key_row->key_id == key_id) {
            ...
            sprintf(sql_text,
                    "select key_no from pct_encrypt_key where key_no = %u",
                    key_row->key_no);
            DgcSqlStmt* sql_stmt =
            ...
            if ((kno_ptr = (dgt_uint16*)sql_stmt->fetch())) {
                ...
                THROWnR(DgcLdbExcept(
                            DGC_EC_LD_STMT_ERR,
                            new DgcError(SPOS, "the key[%lld] is being used",
                                         key_id)),
                        -1);
            }
            delete EXCEPTnC;
            ...
            memset(sql_text, 0, 256);
            sprintf(sql_text,
                    "DELETE PCT_EXT_KEY WHERE KEY_ID = %lld and "
                    "nextLastUpdate('PCT_EXT_KEY', %lld, 3) > 0",
                    key_id, key_id);
    ...
    if (key_row == 0) {
        THROWnR(
            DgcLdbExcept(
                DGC_EC_LD_STMT_ERR,
                new DgcError(SPOS, "the external key[%lld] not found", key_id)),
            -1);
    }
    ReturnRows->reset();
    ReturnRows->add();
    ReturnRows->next();
    *(ReturnRows->data()) = 0;
    dg_sprintf((dgt_schar*)ReturnRows->data(), "the external key[%lld] dropped",
               key_id);
    ReturnRows->rewind();
    return 0;
}
