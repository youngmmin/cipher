 ...
 *   Description        :       drop key set- master, encryption key set, ecryption key set signature
...
PccDropKey::PccDropKey(const dgt_schar* name)
...
DgcExtProcedure* PccDropKey::clone()
{
	return new PccDropKey(procName());
}
...
dgt_sint32 PccDropKey::execute() throw(DgcExcept)
{
	...
	if (rows.next()) {
		...
		if ((rtn=PCI_checkPassword(pw, key_row->smk, strlen(key_row->smk), key_row->seks,
		...
		delete EXCEPTnC;
                delete sql_stmt; sql_stmt=0;
		memset(sql_text,0,256);
		sprintf(sql_text,"DELETE PCT_KEY WHERE nextLastUpdate('PCT_KEY', %lld, 3) > 0",key_row->key_id);
		...
		THROWnR(DgcLdbExcept(DGC_EC_LD_STMT_ERR,new DgcError(SPOS,"there's no key to drop")),-1);
	}
	ReturnRows->reset();
	ReturnRows->add();
	ReturnRows->next();
