...
PccExportExtIV::PccExportExtIV(const dgt_schar* name)
...
dgt_sint32 PccExportExtIV::execute() throw(DgcExcept)
{
	...
	while (iv_rows.next() && (iv_row=(pct_type_ext_iv*)iv_rows.data())) {
		if (iv_row->iv_id == iv_id) {
			...
			dg_sprintf(tmp_ivs,"(ext_iv=(iv_id=%lld)(create_time=%u)(iv_no=%u)(seiv=\"%s\")(seivs=\"%s\")(name=\"%s\")(description=%s)(reserved=%s))",
				iv_row->iv_id,
				iv_row->create_time,
				iv_row->iv_no,
				iv_row->seiv,
