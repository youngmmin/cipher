...
dgt_sint32 PccExportExtKey::execute() throw(DgcExcept) {
    ...
    while (key_rows.next() && (key_row = (pct_type_ext_key*)key_rows.data())) {
        if (key_row->key_id == key_id) {
            ...
            dg_sprintf(
                tmp_keys,
                "(ext_key=(key_id=%lld)(create_time=%u)(key_no=%u)(sek=\"%s\")("
                "seks=\"%s\")(name=\"%s\")(description=%s)(reserved=%s))",
                key_row->key_id, key_row->create_time, key_row->key_no,
                key_row->sek, key_row->seks, key_row->name,
                key_row->description, key_row->reserved);
