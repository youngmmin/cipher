 ...
 *   Description        :       export key set- master, encryption key set,
...
PccExportKey::PccExportKey(const dgt_schar* name) : DgcExtProcedure(name) {}

PccExportKey::~PccExportKey() {}

DgcExtProcedure* PccExportKey::clone() { return new PccExportKey(procName()); }
...
dgt_sint32 PccExportKey::execute() throw(DgcExcept) {
    ...
    if (key_rows.next()) {
        ...
        if (*new_pw &&
            (rtn = PCI_changePassword(old_pw, new_pw, new_smk, strlen(new_smk),
                                      key_row->seks, strlen(key_row->seks),
                                      key_row->sks, strlen(key_row->sks),
                                      hsm_mode, hsm_password)) < 0) {
            THROWnR(DgcLdbExcept(
        ...
        //				 key_row->key_id, new_smk, key_row->seks, key_row->sks,
        //key_row->description, key_row->create_date);
        dg_sprintf(tmp_keys,
                   "(key=\n(key_id=%lld)\n(smk=\"%s\")\n(seks=\"%s\")\n(sks=\"%"
                   "s\")\n(description=%s)\n)",
                   key_row->key_id, new_smk, key_row->seks, key_row->sks,
