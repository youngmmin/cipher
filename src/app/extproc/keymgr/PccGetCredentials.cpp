...
dgt_sint32 PccGetCredentials::execute() throw(DgcExcept) {
    ...
    if (*param == 0) {
        THROWnR(DgcLdbExcept(DGC_EC_LD_STMT_ERR,
                             new DgcError(SPOS, "null input param.")),
                -1);
    }
    ...
    while ((credentials_info = credentials_list.getNext()) != 0) {
        dgt_schar* val = 0;
        if ((val = credentials_info->getValue("credentials.svc")) != 0)
            svc = val;
        if ((val = credentials_info->getValue("credentials.user")) != 0)
            user = val;
        if ((val = credentials_info->getValue("credentials.password")) != 0)
            password = val;
        if ((val = credentials_info->getValue("credentials.key")) != 0)
            key = val;
        if ((val = credentials_info->getValue("credentials.svc_home")) != 0)
            pc.setSvcHome(val);
        if ((val = credentials_info->getValue("credentials.ip")) != 0)
            pc.setIP(val);
        if ((val = credentials_info->getValue("credentials.mac")) != 0)
            pc.setMAC(val);
        if ((val = credentials_info->getValue("credentials.instance")) != 0)
            pc.setInstanceName(val);
        if ((val = credentials_info->getValue("credentials.db_name")) != 0)
            pc.setDbName(val);
        if ((val = credentials_info->getValue("credentials.db_user")) != 0)
            pc.setDbUser(val);
        if ((val = credentials_info->getValue("credentials.os_user")) != 0)
            pc.setOsUser(val);
        if ((val = credentials_info->getValue("credentials.program")) != 0)
            pc.setProgram(val);
        if ((val = credentials_info->getValue("credentials.org_user")) != 0)
            pc.setOrgUserID(val);
    }
    ...
    ret.errcode = rtn;
    if (rtn)
        dg_sprintf((dgt_schar*)ret.result, pc.errMsg());
    else
        dg_sprintf((dgt_schar*)ret.result, pc.credentials());
