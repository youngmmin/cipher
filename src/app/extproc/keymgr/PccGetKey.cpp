...
PccGetKey::PccGetKey(const dgt_schar *name) : DgcExtProcedure(name) {}

PccGetKey::~PccGetKey() {}

DgcExtProcedure *PccGetKey::clone() { return new PccGetKey(procName()); }

static const dgt_uint8 PCI_PIVS[64] =  // Predefined Initial Vector seed
    {123, 8,   94,  71,  230, 9,   12, 70,  95,  71,  84, 84,  3,
     78,  5,   63,  4,   83,  123, 95, 47,  56,  91,  34, 123, 46,
     73,  214, 38,  24,  1,   239, 76, 237, 8,   39,  81, 80,  238,
     90,  81,  3,   123, 89,  74,  1,  239, 81,  230, 67, 51,  236,
     51,  59,  238, 74,  61,  92,  37, 84,  125, 180, 36, 237};
...
dgt_sint32 PccGetKey::execute() throw(DgcExcept) {
    ...
    if (key_rows.next()) {
        ...
        dgt_sint32 rtn = 0;
        if ((rtn = PCI_checkPassword(pwd, key_row->smk, strlen(key_row->smk),
    ...
    dg_sprintf(sql_text, "select * from pct_encrypt_key where key_id=%lld",
               key_id);
    DgcSqlStmt *sql_stmt =
    ...
    if (enc_key.key_no >= 10000) {
        ...
        // external key
        //
        dg_sprintf(sql_text, "select * from pct_ext_key where key_no = %u",
                   enc_key.key_no);
        DgcSqlStmt *sql_stmt =
            Database->getStmt(Session, sql_text, strlen(sql_text));
        DgcExcept *e = 0;
        ...
        if ((ext_key = (pct_type_ext_key *)sql_stmt->fetch())) {
            PccExternalKey ek;
            ek.getKey(ext_key->key_no, ext_key->sek, ext_key->seks,
                      enc_key.key_size / 8, key);
        }
        e = EXCEPTnC;
        delete sql_stmt;
        if (e) {
            RTHROWnR(e,
                     DgcError(SPOS, "fetch external key_no[%u] failed",
                              enc_key.key_no),
                     -1);
        }
    } else {
        ...
        // internal key
        //
        if ((rtn = PCI_getEncryptKey(enc_key.key_no, enc_key.key_size / 8,
                                     key)) < 0) {
            THROWnR(DgcLdbExcept(
                        DGC_EC_LD_STMT_ERR,
                        new DgcError(SPOS,
                                     "getEncryptKey[%lld] failed due to %d:%s",
                                     key_id, rtn, PCI_getKmgrErrMsg())),
                    -1);
        }
    }
    dgt_uint8 iv[64];
    ...
    if (enc_key.iv_type > 0 && enc_key.iv_type <= 10) {
        ...
        //
        // external iv
        // The "IV_TYPE" column in the "PCT_ENCRYPT_KEY" table is identical to
        // the "IV_NO" value in the "PCT_EXT_IV" table.
        //
        dg_sprintf(sql_text, "select * from pct_ext_iv where iv_no = %u",
                   enc_key.iv_type);
        DgcSqlStmt *sql_stmt =
            Database->getStmt(Session, sql_text, strlen(sql_text));
        DgcExcept *e = 0;
        ...
        if (e) {
            RTHROWnR(e,
                     DgcError(SPOS, "fetch external iv_no[%u] failed",
                              enc_key.iv_type),
                     -1);
        }
    }
