...
PccImportExtIV::PccImportExtIV(const dgt_schar* name)
...
dgt_sint32 PccImportExtIV::execute() throw(DgcExcept)
{
	...
	dgt_schar*	tmp_ivs = new dgt_schar[BindRows->numRows()*BindRows->rowSize()+1];
	*tmp_ivs = 0;
	...
	if (bg.parse(tmp_ivs) < 0 || bg.pstatus() != DGC_BGPS_FINISH) {
		...
		THROWnR(DgcBgmrExcept(DGC_EC_BG_INCOMPLETE,new DgcError(SPOS,"incomplete iv")),-1);
	}
	...
	if (!iv_id) strncpy(tmp_emsg,"iv_id not found",127);
	else if (!create_time) strncpy(tmp_emsg,"create_time not found",127);
	else if (!iv_no) strncpy(tmp_emsg,"iv_no not found",127);
	else if (!seiv) strncpy(tmp_emsg,"seiv not found",127);
	else if (!seivs) strncpy(tmp_emsg,"seivs not found",127);
	else if (!name) strncpy(tmp_emsg,"name not found",127);
	else if (!description) strncpy(tmp_emsg,"description not found",127);
	else if (!reserved) strncpy(tmp_emsg,"reserved not found",127);
	if (*tmp_emsg) {
		delete tmp_ivs;
	...
	ext_iv.last_update = iud_log.nextLastUpdate((dgt_schar*)"pct_ext_iv", ext_iv.iv_id, PcSyncIudLogInserter::PTC_SYNC_SQL_TYPE_INSERT);
