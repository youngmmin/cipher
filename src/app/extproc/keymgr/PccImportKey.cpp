 ...
 *   Description        :       import key set- master, encryption key set, ecryption key set signature
...
PccImportKey::PccImportKey(const dgt_schar* name)
...
DgcExtProcedure* PccImportKey::clone()
{
	return new PccImportKey(procName());
}
...
dgt_sint32 PccImportKey::execute() throw(DgcExcept)
{
	...
	dgt_schar*	old_pw=0;
	dgt_schar*	new_pw=0;
	dgt_schar*	tmp_keys=new dgt_schar[BindRows->numRows()*BindRows->rowSize()+1];
	*tmp_keys=0;
	...
	if (bg.parse(tmp_keys) < 0 || bg.pstatus() != DGC_BGPS_FINISH) {
		...
		THROWnR(DgcBgmrExcept(DGC_EC_BG_INCOMPLETE,new DgcError(SPOS,"incomplete key")),-1);
	}
...
//	dgt_schar*	create_date=bg.getValue("key.create_date");
	dgt_schar	tmp_emsg[128]={0,};
	if (!key_id) strncpy(tmp_emsg,"key_id not found",127);
	else if (!smk) strncpy(tmp_emsg,"smk not found",127);
	else if (!seks) strncpy(tmp_emsg,"seks not found",127);
	else if (!sks) strncpy(tmp_emsg,"sks not found",127);
//	else if (!create_date) strncpy(tmp_emsg,"create_date not found",127);
	if (*tmp_emsg) {
		delete tmp_keys;
		THROWnR(DgcLdbExcept(DGC_EC_LD_STMT_ERR,new DgcError(SPOS,tmp_emsg)),-1);
	}
	//
	...
	dgt_sint32	rtn=0;
	if ((rtn=PCI_checkPassword(old_pw, smk, strlen(smk), seks, strlen(seks), sks, strlen(sks), hsm_mode, hsm_password)) < 0) {
		delete tmp_keys;
...
//	key_row.create_date=strtoul(create_date,0,10);
	...
	// change password
	//
	if (*new_pw && (rtn=PCI_changePassword(old_pw, new_pw, key_row.smk, strlen(key_row.smk),
	...
	if (tab->pinInsert(Session,rows,1) != 0) {
        ...
        key_row.last_update  = iud_log.nextLastUpdate((dgt_schar*)"pct_key", key_row.key_id, PcSyncIudLogInserter::PTC_SYNC_SQL_TYPE_INSERT);
