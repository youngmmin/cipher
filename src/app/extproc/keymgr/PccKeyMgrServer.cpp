 ...
 *   Description        :       petra cipher key managing external procedure server
...
dgt_sint32 PccKeyMgrFactory::initialize() throw(DgcExcept)
{
	...
	if (!key_stash) {
		key_stash=DgcDbProcess::db().pdb()->segMgr()->createStream(DgcDbProcess::sess(),1,"_PCS_KEY_STASH");
        ...
        //
        dgt_sint32      rtn=0;
        if ((rtn=PCI_setKeyStash((PCT_KEY_STASH*)((dgt_uint8*)key_stash + key_stash->totalSize()))) < 0 && rtn != PCC_ERR_KMGR_KEY_STASH_SET_ALREADY) {
	...
	if (EXCEPT) {
		...
		if ((rtn=pc_fs.recvData((dgt_uint8*)credentials,256)) < 0) {
			...
			if ((rtn=pc.parse(credentials))) {
				DgcWorker::PLOG.tprintf(0,"parse credentials failed due to:[%d:%s]\n",rtn,pc.errMsg());
			} else if (strcasecmp(pc.svcName(),getenv("SOHA_SVC")) || strcasecmp(pc.userID(),"SAVED_PW")) {
				DgcWorker::PLOG.tprintf(0,"invalid svc[%s] or user id[%s]\n",pc.svcName(),pc.userID());
			} else {
				DgcTableSegment* key_tab=(DgcTableSegment*)DgcDbProcess::db().pdb()->segMgr()->getTable("PCT_KEY");
				if (key_tab == 0) {
					DgcExcept*	e = EXCEPTnC;
					if (e) {
						DgcWorker::PLOG.tprintf(0,*e,"read credentials failed due to:\n");
					...
					if (key_rows.next()) {
						...
						if (key_row->open_mode == 1) {
							...
							if ((rtn=PCI_openKey(pc.password(), key_row->smk, strlen(key_row->smk), key_row->seks,
									     strlen(key_row->seks), key_row->sks, strlen(key_row->sks),hsm_mode, hsm_password)) < 0) {
								DgcWorker::PLOG.tprintf(0,"open key failed due to [%d:%s]\n",rtn,PCI_getKmgrErrMsg());
							} else {
						                dgt_uint32      open_date;
						                open_date=dgtime(&open_date);
					        	        dgt_schar       sql_text[256];
					                	memset(sql_text,0,256);
						                sprintf(sql_text,"insert into pct_key_stat values(%lld,%d,%d)",key_row->key_id,open_date,1);
								...
								DgcWorker::PLOG.tprintf(0,"the key set opened automatically.\n");
							}
						} else {
							DgcWorker::PLOG.tprintf(0,"no key found for auto-open.\n");
						}
					}
				}
			}
...
dgt_void initServerFactory()
{
	...
	DgcProcServer::initFactory(&KeyMgrFactory);        // register the KeyMgrFactory with the Procedure Server
}
