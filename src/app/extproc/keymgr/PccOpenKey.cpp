...
PccOpenKey::PccOpenKey(const dgt_schar* name)
...
DgcExtProcedure* PccOpenKey::clone()
{
	return new PccOpenKey(procName());
}
...
dgt_sint32 PccOpenKey::execute() throw(DgcExcept)
{
	...
	if (key_rows.next()) {
		...
		if ((rtn=PCI_openKey(pw, key_row->smk, strlen(key_row->smk), key_row->seks,
