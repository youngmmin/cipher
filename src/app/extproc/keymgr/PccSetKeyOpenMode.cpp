...
dgt_sint32 PccSetKeyOpenMode::execute() throw(DgcExcept) {
    ...
    if (key_rows.next()) {
        ...
        if ((rtn = PCI_checkPassword(
                 set_in->passwd, key_row->smk, strlen(key_row->smk),
    ...
    if (strncasecmp(set_in->open_mode, "AUTO", 4) == 0) {
        ...
        PcaCredentials pc;
        if ((rtn =
                 pc.generate(getenv("SOHA_SVC"), "SAVED_PW", set_in->passwd))) {
            THROWnR(DgcLdbExcept(DGC_EC_LD_STMT_ERR,
                                 new DgcError(SPOS, "%d:%s", rtn, pc.errMsg())),
                    -1);
        }
        //
        // save the credential
        ...
        sprintf(sql_text,
                "update pct_key "
                "set(open_mode,last_update)=(1,nextlastupdate('PCT_KEY',1,2)) "
                "where key_id=1");
        ...
        *(ReturnRows->data()) = 0;
    ...
    } else if (*set_in->open_mode == 0 ||
               strncasecmp(set_in->open_mode, "MANUAL", 6) == 0) {
        //
        // remove credentials
        //
        if (remove(pc_file_path) < 0) {
            THROWnR(DgcLdbExcept(
                        DGC_EC_LD_STMT_ERR,
                        new DgcError(SPOS, "remove[%s] failed due to [%d:%s]",
                                     pc_file_path, errno, strerror(errno))),
                    -1);
        }
        //
        ...
        sprintf(sql_text,
                "update pct_key "
                "set(open_mode,last_update)=(0,nextlastupdate('PCT_KEY',1,2)) "
                "where key_id=1");
        if (sql_handle.execute(sql_text) < 0) {
            DgcExcept* e = EXCEPTnC;
            if (e) {
        ...
        ReturnRows->next();
        *(ReturnRows->data()) = 0;
        dg_sprintf((dgt_schar*)ReturnRows->data(),
                   "key open mode set as manual");
    } else {
        THROWnR(DgcLdbExcept(DGC_EC_LD_STMT_ERR,
                             new DgcError(SPOS, "unsupported open mode[%s]",
                                          set_in->open_mode)),
                -1);
    }
    ReturnRows->rewind();
