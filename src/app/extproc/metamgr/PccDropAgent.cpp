...
PccDropAgent::PccDropAgent(const dgt_schar* name) : PccMetaProcedure(name) {}

PccDropAgent::~PccDropAgent() {}

DgcExtProcedure* PccDropAgent::clone() { return new PccDropAgent(procName()); }
...
dgt_sint32 PccDropAgent::execute() throw(DgcExcept) {
    ...
    PccScriptBuilder* cipher_builder =
        getScriptBuilder(InRow->service_id, PCC_ID_TYPE_AGENT);
    if (!cipher_builder) {
    ...
    sprintf(sql_text, "select * from pct_db_agent where service_id=%lld",
            InRow->service_id);
    DgcSqlStmt* sql_stmt =
    ...
    if (agent.inst_step >= 4) {
        ...
        //
        for (stmt_no = 0; (rtn = cipher_builder->getScript(
                               InRow->service_id, 0, -2, stmt_no + 1)) > 0;
             stmt_no++) {
    ...
    for (stmt_no = 0; (rtn = cipher_builder->getScript(InRow->service_id, 0, -1,
                                                       stmt_no + 1)) > 0;
         stmt_no++) {
        if (cipher_builder->runScript(cipher_builder->scriptText(), conn))
            break;
