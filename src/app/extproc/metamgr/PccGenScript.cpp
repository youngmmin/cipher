...
PccGenScript::PccGenScript(const dgt_schar* name) : PccMetaProcedure(name) {}

PccGenScript::~PccGenScript() {}

DgcExtProcedure* PccGenScript::clone() { return new PccGenScript(procName()); }

dgt_sint32 PccGenScript::execute() throw(DgcExcept) {
    ...
    // build new scripts
    //
    if (script_builder->buildScript(*enc_tab_id, 0)) {
        DgcExcept* e = EXCEPTnC;
