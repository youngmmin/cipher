...
PccGenScript2::PccGenScript2(const dgt_schar* name) : PccMetaProcedure(name) {}
...
dgt_sint32 PccGenScript2::execute() throw(DgcExcept) {
    ...
    if ((tmp_ptr = (pc_type_schema_info*)sql_stmt->fetch()) == 0) {
        DgcExcept* e = EXCEPTnC;
        delete sql_stmt;
        RTHROWnR(e, DgcError(SPOS, "schema not found by id[%lld]", *enc_tab_id),
                 0);
    }
    ...
    script_builder =
        new PccOraScriptBuilder(Database, Session, schema_info.schema_link);
    //
    // delete old scripts
    //
