...
PccGenScriptAddCol::PccGenScriptAddCol(const dgt_schar* name)
    : PccMetaProcedure(name) {}

PccGenScriptAddCol::~PccGenScriptAddCol() {}

DgcExtProcedure* PccGenScriptAddCol::clone() {
    return new PccGenScriptAddCol(procName());
}

dgt_sint32 PccGenScriptAddCol::execute() throw(DgcExcept) {
    ...
    sprintf(sql_text, "delete pct_script where enc_tab_id=%lld and step_no=3",
            *enc_tab_id);
    DgcSqlStmt* sql_stmt =
    ...
    // build new scripts
    //
    if (script_builder->buildScriptAddCol(*enc_tab_id, 0)) {
        DgcExcept* e = EXCEPTnC;
