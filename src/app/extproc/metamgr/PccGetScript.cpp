...
PccGetScript::PccGetScript(const dgt_schar* name) : PccMetaProcedure(name) {}

PccGetScript::~PccGetScript() {}

DgcExtProcedure* PccGetScript::clone() { return new PccGetScript(procName()); }
...
dgt_sint32 PccGetScript::execute() throw(DgcExcept) {
    ...
    ReturnRows->reset();
    if (getScript(InRow->enc_tab_id, InRow->version_no, InRow->step_no,
                  InRow->stmt_no, ReturnRows) < 0) {
