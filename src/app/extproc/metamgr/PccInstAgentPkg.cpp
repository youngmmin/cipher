...
dgt_sint32 PccInstAgentPkg::execute() throw(DgcExcept)
{
	...
	memset(sql_text,0,1024);
	sprintf(sql_text,"select max(step_no) from pct_script where enc_tab_id=%lld",InRow->db_agent_id);
...
DgcWorker::PLOG.tprintf(0,"SQL[%s]\n",cipher_builder->scriptText());
                ...
                if (e) {
                        ...
                        sprintf(sql_text,"update pct_db_agent set(curr_install_step,curr_install_stmt,inst_step,last_update)=(%d,%d,-4,nextLastUpdate('PCT_DB_AGENT', %lld, 2)) where db_agent_id=%lld", 
	...
	//
        sprintf(sql_text,"update pct_db_agent set(curr_install_step,curr_install_stmt,inst_step,last_update)=(%d,0,4,nextLastUpdate('PCT_DB_AGENT', %lld, 2)) where db_agent_id=%lld",
	...
	delete cipher_builder;
        ...
        ReturnRows->next();
        memset(ReturnRows->data(),0,ReturnRows->rowSize());
        sprintf(out_param.result_msg,"%s",(dgt_schar*)"Package Created Successfully");
