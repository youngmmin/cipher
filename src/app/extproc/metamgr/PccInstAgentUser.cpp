...
dgt_sint32 PccInstAgentUser::execute() throw(DgcExcept)
{
	...
	dgt_schar sql_text[1024];
        ...
        sprintf(sql_text,"select a.db_type "
                         "from pt_db_instance a, pct_db_agent b "
                         "where a.instance_id = b.instance_id "
                         "and   b.db_agent_id  = %lld",InRow->db_agent_id);
	...
	db_type=*ptr_db_type;
        ...
        switch(db_type){
                ...
                default:
                        ...
                        sprintf(out_param.result_msg,"%s",(dgt_schar*)"not Supported dbms type\n");
        ...
        //
        memset(sql_text,0,1024);
        sprintf(sql_text,"select * from pct_db_agent where db_agent_id=%lld",InRow->db_agent_id);
	...
	//
        PccScriptBuilder*       cipher_builder=getScriptBuilder(InRow->db_agent_id,PCC_ID_TYPE_AGENT);
        if (!cipher_builder) {
	...
	if (agent.curr_install_step == 0 && agent.curr_install_stmt <= 1) {
		if (InRow->inst_mode == 0) {
			if (cipher_builder->checkDB(InRow->db_agent_id,InRow->sys_uid,InRow->sys_passwd,InRow->agent_uid,ReturnRows) < 0) {
				DgcExcept*      e=EXCEPTnC;
				delete cipher_builder;
				if (e) {
					RTHROWnR(e,DgcError(SPOS,"PccInstAgentUser CheckDB failed"),-1);
				}
				return 0;
			}
		}
	        ...
	        // delete old scripts
       		//
		memset(sql_text,0,1024);
        	sprintf(sql_text,"delete pct_script where enc_tab_id=%lld",InRow->db_agent_id);
	        sql_stmt=Database->getStmt(Session,sql_text,strlen(sql_text));
        	if (sql_stmt == 0 || sql_stmt->execute() < 0) {
			delete cipher_builder;
	                DgcExcept*      e=EXCEPTnC;
        	...
        	// build new scripts
	        //
		if (cipher_builder->buildInstallScript(InRow->db_agent_id,InRow->agent_uid,InRow->agent_passwd,InRow->soha_home) < 0) {
			delete cipher_builder;
			DgcExcept*      e=EXCEPTnC;
			if (e) {
	...
	if (InRow->inst_mode == 1) {
		...
		ReturnRows->next();
		memset(ReturnRows->data(),0,ReturnRows->rowSize());
		sprintf(out_param.result_msg,"%s",(dgt_schar*)"User Install Script Created Successfully");
		out_param.result_code=0;
		ReturnRows->rewind();
		return 0;
	}
	...
	//
        memset(sql_text,0,1024);
        sprintf(sql_text,"select a.instance_name, c.listen_ip, c.listen_port, a.db_type "
	...
	if (agent.curr_install_stmt == 0) {
        ...
        if (agent.curr_install_step <= 1) {
                ...
                //
                for(; (rtn=cipher_builder->getScript(InRow->db_agent_id, 0, step_no, stmt_no)) > 0; stmt_no++) {
                ...
                if (e) {
                        ...
                        sprintf(sql_text,"update pct_db_agent set(curr_install_stmt,inst_step,last_update)=(%d,-1,nextLastUpdate('PCT_DB_AGENT', %lld, 2)) where db_agent_id=%lld", 
						stmt_no, InRow->db_agent_id, InRow->db_agent_id);
                } else {
			if ((rtn=cipher_builder->getScript(InRow->db_agent_id, 0, 1, 1)) > 0) {
			...
			delete cipher_builder;
                        ...
                        sprintf(sql_text,"update pct_db_agent set(curr_install_step,curr_install_stmt,inst_step,last_update)=(2,0,1,nextLastUpdate('PCT_DB_AGENT', %lld, 2)) where db_agent_id=%lld",
	...
	delete conn;
        ...
        ReturnRows->next();
        memset(ReturnRows->data(),0,ReturnRows->rowSize());
        sprintf(out_param.result_msg,"%s",(dgt_schar*)"User Created Successfully");
        out_param.result_code=0;
        memcpy(ReturnRows->data(),&out_param,sizeof(pc_type_inst_agent_user_out));
        ReturnRows->rewind();
        return 0;
