...
dgt_sint32 PccMetaMgrFactory::initialize() throw(DgcExcept)
{
	...
	if (addProcedure(new PccGenScriptColAdmin("PCP_GEN_SCRIPT_COL_ADMIN")) != 0) {
		ATHROWnR(DgcError(SPOS,"addProcedure failed"),-1);
	}
...
dgt_void initServerFactory()
{
	...
	DgcProcServer::initFactory(&MetaMgrFactory);        // register the MetaMgrFactory with the Procedure Server
}
