...
PccScriptBuilder* PccMetaProcedure::getScriptBuilder(
    dgt_sint64 id, dgt_uint8 id_type) throw(DgcExcept) {
    return PccScriptBuilderFactory::getScriptBuilder(Database, Session, id,
                                                     id_type);
}

dgt_sint32 PccMetaProcedure::getScript(dgt_sint64 enc_tab_id,
    ...
    if (builder->getScript(enc_tab_id, version_no, step_no, stmt_no, rtn_rows,
                           1) < 0) {
