...
dgt_sint32 PccMigValidTest::setItTabRows() throw(DgcExcept) {
    ...
    while (1) {
        if (sql_handle.fetch(rtn_row_ptr) < 0) {
            ...
            ItTabRows.add();
            ItTabRows.next();
            memcpy(ItTabRows.data(), (it_tab_st*)rtn_row_ptr,
                   sizeof(it_tab_st));
        } else {
            break;
        }
    ...
    while (ItTabRows.next()) {
        ...
        if (1) {
            if (sql_handle.fetch(rtn_row_ptr) < 0) {
                ...
                memcpy(&target_table, (target_table_st*)rtn_row_ptr,
                       sizeof(target_table_st));
            } else {
                break;
            }
        }
        if (target_table.enc_tab_id > 0) {
            ...
            sprintf(sql_text,
                    ...
                    it_tab_id, target_table.enc_tab_id);
            if (sql_handle.execute(sql_text) < 0) {
                DgcExcept* e = EXCEPTnC;
                if (e) {
                    RTHROWnR(e, DgcError(SPOS, "sql_handle execute Failed"),
            ...
            while (1) {
                if (sql_handle.fetch(rtn_row_ptr) < 0) {
                    ...
                    memcpy(&target_column, (target_column_st*)rtn_row_ptr,
                           sizeof(target_column_st));
                    if (target_column.c_column_name == 0) {
                        ...
                        sprintf(err_msg, "[%s.%s] column not found",
                                PetraNamePool->getNameString(
                                    target_table.table_name),
                                PetraNamePool->getNameString(
                                    target_column.t_column_name));
                        if (logging(it_tab_id, err_msg) < 0) {
                            DgcExcept* e = EXCEPTnC;
                            if (e) {
                                RTHROWnR(e,
                                         DgcError(SPOS, "err logging failed"),
                                         -1);
                            }
                        }
                    }
            ...
            dgt_schar err_msg[256];
            memset(err_msg, 0, 256);
            sprintf(err_msg, "[%s] table not found",
                    PetraNamePool->getNameString(target_table.table_name));
            if (logging(it_tab_id, err_msg) < 0) {
                DgcExcept* e = EXCEPTnC;
                if (e) {
        ...
        while (1) {
            ...
            if (sql_handle.fetch(rtn_row_ptr) < 0) {
                ...
                memcpy(&src_table, (src_table_st*)rtn_row_ptr,
                       sizeof(src_table_st));
                if (src_table.enc_tab_id > 0) {
                    ...
                    sprintf(sql_text,
                            ...
                            "and    b.enc_tab_id = %lld "
                            "and    a.column_name = b.column_name",
                            it_tab_id, src_table.is_tab_id,
                            src_table.enc_tab_id);
                    DgcSqlHandle col_handle(Session);
                    if (col_handle.execute(sql_text) < 0) {
                        DgcExcept* e = EXCEPTnC;
                        if (e) {
                            RTHROWnR(
                                e, DgcError(SPOS, "col_handle execute Failed"),
                                -1);
                        }
                    }
                    typedef struct {
                    ...
                    while (1) {
                        if (col_handle.fetch(rtn_row_ptr) < 0) {
                            ...
                            if (e) {
                                RTHROWnR(
                                    e,
                                    DgcError(SPOS, "col_handle execute failed"),
                                    -1);
                            }
                        } else if (rtn_row_ptr) {
                            memcpy(&src_column, (src_column_st*)rtn_row_ptr,
                                   sizeof(src_column_st));
                    ...
                    memset(err_msg, 0, 256);
                    sprintf(err_msg, "[%s] table not found",
                            PetraNamePool->getNameString(src_table.table_name));
                    if (logging(it_tab_id, err_msg) < 0) {
                        DgcExcept* e = EXCEPTnC;
                        if (e) {
                            RTHROWnR(e, DgcError(SPOS, "err logging failed"),
                                     -1);
                        }
                    }
                }
...
dgt_sint32 PccMigValidTest::validExpression() throw(DgcExcept) {
    ...
    if (DictItTabRows.numRows() > 0) {
        ...
        if (db_id > 0) {
            ...
            while (1) {
                if (sql_handle.fetch(rtn_row_ptr) < 0) {
                    ...
                    memcpy(&db_agent_id, (dgt_sint64*)rtn_row_ptr,
                           sizeof(dgt_sint64));
                } else {
                    break;
                }
            ...
            if (db_agent_id == 0) {
                ...
                memset(err_msg, 0, 256);
                sprintf(err_msg,
                        "not found db_agent_id in PCT_DB_AGENT[db_id=%lld]",
                        db_id);
                if (logging(it_tab_id, err_msg) < 0) {
                    DgcExcept* e = EXCEPTnC;
                    if (e) {
        ...
        PccScriptBuilder* cipher_builder =
            getScriptBuilder(db_agent_id, PCC_ID_TYPE_AGENT);
        if (!cipher_builder) {
...
dgt_sint32 PccMigValidTest::registMtTabInfo() throw(DgcExcept) {
    ...
    while (ExpItTabRows.next()) {
        ...
        sprintf(sql_text,
                "insert into "
                "pct_mt_table(enc_tab_id,it_tab_id,db_id,schema_name,table_"
                "name,where_clause,join_clause,schedule_id) "
                "select  b.enc_tab_id, "
                "a.it_tab_id, "
                "a.db_id, "
                "a.schema_name, "
                ...
                "and     a.schema_name = b.schema_name "
                "and     a.table_name = b.table_name",
                *(dgt_sint64*)ExpItTabRows.getColPtr(1));
        if (sql_handle.execute(sql_text) < 0) {
            DgcExcept* e = EXCEPTnC;
            if (e) {
                RTHROWnR(e, DgcError(SPOS, "sql_handle execute Failed"), -1);
        ...
        sprintf(
            sql_text,
            "insert into "
            "pct_mt_column(enc_col_id,it_tab_id,enc_tab_id,db_id,schema_name,"
            "column_name,map_col_id,col_expression,key_id,column_order) "
            "select  d.enc_col_id, "
            "a.it_tab_id, "
            "c.enc_tab_id, "
            "a.db_id, "
        ...
        sprintf(sql_text,
                "insert into "
                "pct_ms_table(enc_tab_id,it_tab_id,db_id,schema_name,table_"
                "name,alias_name,dblink_name) "
                "select  b.enc_tab_id, "
                "a.it_tab_id, "
                "a.db_id, "
                "a.schema_name, "
        ...
        sprintf(sql_text,
                "insert into "
                "pct_ms_column(enc_col_id,it_tab_id,enc_tab_id,db_id,schema_"
                "name,column_name,col_expression) "
                "select c.is_col_id, "
                "a.it_tab_id, "
                "b.enc_tab_id, "
                "a.db_id, "
...
dgt_sint32 PccMigValidTest::registEncTabInfo() throw(DgcExcept) {
    ...
    while (ExpItTabRows.next()) {
        memset(sql_text, 0, 1024);
        sprintf(sql_text,
                "insert into pct_enc_schema(schema_id, last_update, db_id, "
                "schema_name) "
                ...
                "from "
                "( "
                "select distinct a.db_id db_id, a.schema_name schema_name, "
                "b.schema_name c "
                "from   pct_mt_table   a, "
                "pct_enc_schema (+) b "
                "where  a.it_tab_id = %lld "
        ...
        memset(sql_text, 0, 1024);
        sprintf(sql_text,
                "insert into pct_enc_table(enc_tab_id, last_update, schema_id, "
        ...
        sprintf(
            sql_text,
            "insert into "
            "pct_enc_column(enc_col_id,last_update,enc_tab_id,key_id,data_"
            "length,data_precision,data_scale,status,column_name,data_type) "
            "select b.enc_col_id, "
        ...
        sprintf(sql_text,
                "insert into "
                "pct_enc_col_index(enc_col_id,enc_tab_id,index_name,column_"
                "position,"
                "index_owner,uniqueness,partitioned,generated,target_"
                "tablespace_name, "
                "index_type,index_tablespace,logging,degree,column_expression,"
                "descend_flag,renamed_index_name1,renamed_index_name2,renamed_"
                "org_name1) "
                "select "
                "a.enc_col_id,a.enc_tab_id,index_name,column_position,index_"
                "owner,uniqueness,partitioned,generated,index_tablespace, "
                "index_type,index_tablespace,logging,degree,column_expression,"
                "descend_flag,getnameid(getname(index_name) || '_n$$'),"
                "getnameid(getname(index_name) || "
                "'_f$$'),getnameid(getname(index_name) || '$$') "
...
dgt_sint32 PccMigValidTest::updateStatus() throw(DgcExcept) {
    ...
    if (TargetTabID) {
        sprintf(sql_text,
                "update pct_it_table set(status)=(-1) where it_tab_id = %lld",
                TargetTabID);
    } else {
        sprintf(sql_text,
...
dgt_sint32 PccMigValidTest::clearLogTables() throw(DgcExcept) {
    ...
    if (ItTabRows.numRows() > 0) {
        sprintf(sql_text, "delete pct_it_error where it_tab_id = :1");
...
dgt_sint32 PccMigValidTest::logging(dgt_sint64 it_tab_id,
                                    dgt_schar* err_msg) throw(DgcExcept) {
    DgcSqlHandle sql_handle(Session);
    dgt_schar sql_text[1024];
    memset(sql_text, 0, 1024);
    sprintf(sql_text,
            "insert into pct_it_error(it_tab_id, err_msg) values(%lld, '%s')",
            it_tab_id, err_msg);
    if (sql_handle.execute(sql_text) < 0) {
        DgcExcept* e = EXCEPTnC;
        if (e) {
