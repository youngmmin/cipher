...
dgt_sint32 PccMyScriptBuilder::step1() throw(DgcExcept)
{
	...
	*TextBuf=0;
	...
	if (TabInfo.enc_type == 0) {
		sprintf(TextBuf,"alter table %s.%s rename %s.%s",SchemaName, TabInfo.table_name,SchemaName,TabInfo.renamed_tab_name);
		if (saveSqlText() < 0) {
	...
	*TextBuf=0;
	...
	while(ColInfoRows.next() && (col_info=(pc_type_col_info*)ColInfoRows.data())) {
		if (col_info->status == 1) {
			...
			else if (!strcasecmp(col_info->data_type,"MEDIUMINT")) enc_len+=(col_info->data_precision+2);
			else if (!strcasecmp(col_info->data_type,"SMALLINT")) enc_len+=(col_info->data_precision+2);
			else if (!strcasecmp(col_info->data_type,"DECIMAL")) enc_len+=(col_info->data_precision+2);
			else if (!strcasecmp(col_info->data_type,"DOUBLE")) enc_len+=(col_info->data_precision+2);
			else if (!strcasecmp(col_info->data_type,"FLOAT")) enc_len+=(col_info->data_precision+2);
			else if (!strcasecmp(col_info->data_type,"TINYINT")) enc_len+=(col_info->data_precision+2);
			else if (!strcasecmp(col_info->data_type,"HEXADECIMAL")) enc_len+=(col_info->data_precision+2);
			else if (!strcasecmp(col_info->data_type,"DATETIME")) enc_len+=14;
			else if (!strcasecmp(col_info->data_type,"DATE")) enc_len+=14;
			else if (!strcasecmp(col_info->data_type,"TIME")) enc_len+=14;
			else if (!strcasecmp(col_info->data_type,"TIMESTAMP")) enc_len+=14;
			else enc_len+=col_info->data_length;
			PCI_Context	ctx;
			PCI_initContext(&ctx, 0, col_info->key_size, col_info->cipher_type, col_info->enc_mode,
					col_info->iv_type, col_info->n2n_flag, 1, col_info->enc_start_pos, col_info->enc_length);
	...
	*TextBuf=0;
	if (TabInfo.enc_type == 0) {
		sprintf(TextBuf,"create view %s.%s_upd as select ",SchemaName, TabInfo.table_name);
		ColInfoRows.rewind();
		while(ColInfoRows.next() && (col_info=(pc_type_col_info*)ColInfoRows.data())) {
			if (col_info->status > 0) {
				*TmpBuf=0;
				sprintf(TmpBuf,"%s %s_upd,",col_info->renamed_col_name, col_info->col_name);
				strcat(TextBuf,TmpBuf);
	...
	*TextBuf=0;
	if (TabInfo.enc_type == 0) {
		sprintf(TextBuf,"create view %s.%s_ins as select ",SchemaName, TabInfo.table_name);
		ColInfoRows.rewind();
		while(ColInfoRows.next() && (col_info=(pc_type_col_info*)ColInfoRows.data())) {
			if (col_info->status == 1) {
				*TmpBuf=0;
	...
	*TmpBuf=0;
	if (TabInfo.enc_type == 0) {
		sprintf(TextBuf,"update %s.%s set",SchemaName, TabInfo.renamed_tab_name);
	} else {
	...
	while(ColInfoRows.next() && (col_info=(pc_type_col_info*)ColInfoRows.data())) {
		if (col_info->status == 1) {
			*TmpBuf=0;
			sprintf(TmpBuf,"\n\t%s=%s,", col_info->renamed_col_name, getFname(col_info->enc_col_id,1));
			strcat(TextBuf,TmpBuf);
		}
	}
	...
	*TextBuf=0;
	...
	//
	// replace the view for the encrypted column to be used as the original column
	//
	*TextBuf=0;
	if (TabInfo.enc_type == 0) {
	...
	*TmpBuf=0;
	...
	while(ColInfoRows.next() && (col_info=(pc_type_col_info*)ColInfoRows.data())) {
		if (col_info->status > 0 && col_info->nullable_flag == 0) {
			...
			*TextBuf=0;
			if (TabInfo.enc_type == 0) {
				sprintf(TextBuf,"alter table %s.%s modify %s varchar(%d) not null",
						SchemaName, TabInfo.renamed_tab_name, col_info->renamed_col_name, enc_len);
			} else {
				sprintf(TextBuf,"alter table %s.%s modify %s varchar(%d) not null",
						SchemaName, TabInfo.table_name, col_info->renamed_col_name, enc_len);
			...
			*TextBuf=0;
			if (TabInfo.enc_type == 0) {
				if ( !strcasecmp(col_info->data_type,"varchar") || !strcasecmp(col_info->data_type,"char")) {
					sprintf(TextBuf,"alter table %s.%s modify %s %s(%d) null",
							SchemaName, TabInfo.renamed_tab_name, col_info->col_name, col_info->data_type, col_info->data_length);
					if (saveSqlText() < 0) {
						ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
					}
				} else {
				...
				if ( !strcasecmp(col_info->data_type,"varchar") || !strcasecmp(col_info->data_type,"char")) {
					sprintf(TextBuf,"alter table %s.%s modify %s %s(%d) null",
							SchemaName, TabInfo.table_name, col_info->col_name, col_info->data_type, col_info->data_length);
					if (saveSqlText() < 0) {
						ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
					}
				} else {
	...
	while(ColInfoRows.next() && (col_info=(pc_type_col_info*)ColInfoRows.data())) {
		if (col_info->status > 0) {
			*TextBuf=0;
			if (TabInfo.enc_type == 0) {
				sprintf(TextBuf,"update %s.%s set %s = null",SchemaName, TabInfo.renamed_tab_name, col_info->col_name);
			} else {
				sprintf(TextBuf,"update %s.%s set %s = null",SchemaName, TabInfo.table_name, col_info->col_name);
			}
			if (saveSqlText() < 0) {
				ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
			}
...
dgt_sint32 PccMyScriptBuilder::reverse_step1() throw(DgcExcept)
{
	...
	if (TabInfo.user_view_flag == 1 || TabInfo.enc_type == 0) {
		*TextBuf=0;
		sprintf(TextBuf,"drop trigger %s.%s_insert",SchemaName, TabInfo.view_trigger_name);
		if (saveSqlText() < 0) {
			ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		}
		*TextBuf=0;
		sprintf(TextBuf,"drop trigger %s.%s_update",SchemaName, TabInfo.view_trigger_name);
		if (saveSqlText() < 0) {
			ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		}
	}
	...
	if (TabInfo.enc_type == 0) {
		*TextBuf=0;
		sprintf(TextBuf,"alter table %s.%s rename %s.%s",SchemaName, TabInfo.renamed_tab_name, SchemaName, TabInfo.table_name);
		if (saveSqlText() < 0) {
			ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		}
	}
	...
	while(ColInfoRows.next() && (col_info=(pc_type_col_info*)ColInfoRows.data())) {
		if (col_info->status > 0) {
			*TextBuf=0;
			sprintf(TextBuf,"alter table %s.%s drop column %s",SchemaName,TabInfo.table_name,col_info->renamed_col_name);
			if (saveSqlText() < 0) {
				ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
			}
		}
	...
	while(ColInfoRows.next() && (col_info=(pc_type_col_info*)ColInfoRows.data())) {
		if (col_info->status > 0 && col_info->nullable_flag == 0) {
			*TextBuf=0;
			if (TabInfo.enc_type == 0) {
				if ( !strcasecmp(col_info->data_type,"varchar") || !strcasecmp(col_info->data_type,"char")) {
					sprintf(TextBuf,"alter table %s.%s modify %s %s(%d) not null",
					        SchemaName, TabInfo.renamed_tab_name, col_info->col_name, col_info->data_type, col_info->data_length);
					if (col_info->col_default) {
						sprintf(TextBuf,"%s default '%s'",TextBuf,PetraNamePool->getNameString(col_info->col_default));
					}
					if (saveSqlText() < 0) {
						ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
					}
...
dgt_sint32 PccMyScriptBuilder::insteadOfTrigger(dgt_sint8 is_final) throw(DgcExcept)
{
	//
	// insert dml trigger
	//
	...
	*TmpBuf=0;
	if (TabInfo.enc_type == 0) {
		sprintf(TmpBuf,"create trigger %s.%s_insert\nbefore insert on %s\nfor each row\nbegin",
	        SchemaName, TabInfo.view_trigger_name, TabInfo.renamed_tab_name);
	} else {
		sprintf(TmpBuf,"create trigger %s.%s_insert\nbefore insert on %s\nfor each row\nbegin",
	...
	while(ColInfoRows.next() && (col_info=(pc_type_col_info*)ColInfoRows.data())) {
		if (col_info->status > 0) {
			if (is_final) {
				if (col_info->col_default) {
					*TmpBuf=0;
					sprintf(TmpBuf,"\n\tset new.%s = petra.pls_encrypt_b64(ifnull(new.%s,'%s'),%lld);",
					...
					*TmpBuf=0;
					sprintf(TmpBuf,"\n\tset new.%s = petra.pls_encrypt_b64(new.%s,%lld);",
							col_info->renamed_col_name,col_info->renamed_col_name,col_info->enc_col_id);
					strcat(TextBuf,TmpBuf);
				}
			} else {
				if (col_info->col_default) {
	...
	*TmpBuf=0;
	if (TabInfo.enc_type == 0) {
		sprintf(TmpBuf,"create trigger %s.%s_update\nbefore update on %s\nfor each row\nbegin",
	        SchemaName, TabInfo.view_trigger_name, TabInfo.renamed_tab_name);
	} else {
		sprintf(TmpBuf,"create trigger %s.%s_update\nbefore update on %s\nfor each row\nbegin",
	        SchemaName, TabInfo.view_trigger_name, TabInfo.table_name);
	...
	*TmpBuf=0;
	...
	while(ColInfoRows.next() && (col_info=(pc_type_col_info*)ColInfoRows.data())) {
		if (col_info->status > 0) {
			if (is_final) {
				*TmpBuf=0;
				sprintf(TmpBuf,"\n\tif ifnull(old.%s,'petra_null') != ifnull(new.%s,'petra_null') then",
						col_info->renamed_col_name, col_info->renamed_col_name);
				strcat(TextBuf,TmpBuf);
				if (col_info->col_default) {
					*TmpBuf=0;
...
{
	dgt_schar sql_text[2048];
	sprintf(sql_text,"select * from pct_enc_tab_priv where enc_tab_id=%lld ",TabInfo.enc_tab_id);
	...
	while ((priv_info_tmp=(pct_type_enc_tab_priv*)sql_stmt->fetch())) {
		...
		sprintf(privSql,"grant %s on %s.%s to %s", privilege, SchemaName, TabInfo.table_name,priv_info_tmp->grantee);
		sprintf(tmp_sql.enc2,"grant %s on %s.%s to %s", privilege, SchemaName, TabInfo.table_name,priv_info_tmp->grantee);
		PrivSqlRows.add();
		PrivSqlRows.next();
		...
		if (TabInfo.enc_type == 0) {
			...
			sprintf(tmp_sql.enc1,"grant %s on %s.%s to %s", privilege, SchemaName,
			        TabInfo.renamed_tab_name, priv_info_tmp->grantee);
			sprintf(tmp_sql.org1,"revoke %s on %s.%s from %s", privilege, SchemaName,
			        TabInfo.renamed_tab_name, priv_info_tmp->grantee);
			PrivSqlRows.add();
			PrivSqlRows.next();
			memcpy(PrivSqlRows.data(), privSql, 1024);
			...
			if (seq == 1) {
				...
				if (TabInfo.enc_type == 0) {
					sprintf(tmp_sql.org2,"alter table %s.%s drop index %s",
					        SchemaName,TabInfo.renamed_tab_name,PetraNamePool->getName(idxcol->index_name));
					sprintf(tmp_sql.enc2,"alter table %s.%s drop index %s",
					        SchemaName,TabInfo.renamed_tab_name,PetraNamePool->getName(idxcol->index_name));
				} else {
					sprintf(tmp_sql.org2,"alter table %s.%s drop index %s",
					        SchemaName,TabInfo.table_name,PetraNamePool->getName(idxcol->index_name));
					sprintf(tmp_sql.enc2,"alter table %s.%s drop index %s",
					        SchemaName,TabInfo.table_name,PetraNamePool->getName(idxcol->index_name));
				}
			} else {
		...
		while ((pk_row=(pc_type_pk_info*)pk_stmt->fetch())) {
			...
			if (seq == 1) {
				if (pk_row->enc_type == 0) {
					if (pk_row->constraint_type == 1) {
						if (ispkfk_tab) {
							sprintf(pkfkSql.org1,"alter table %s.%s add constraint primary key("
							        ,PetraNamePool->getNameString(pk_row->schema_name)
							        ,PetraNamePool->getNameString(pk_row->renamed_tab_name));
							sprintf(pkfkSql.enc1,"alter table %s.%s add constraint primary key("
							        ,PetraNamePool->getNameString(pk_row->schema_name)
							        ,PetraNamePool->getNameString(pk_row->renamed_tab_name));
						} else {
							if (IsPkFk) {
			...
			if (ispkfk_tab) {
				if (pk_row->enc_type == 0) {
					if (pk_row->constraint_type == 1) {
						...
						sprintf(pkfkSql.org2,"alter table %s.%s drop index %s",
						        PetraNamePool->getNameString(pk_row->schema_name),
						        PetraNamePool->getNameString(pk_row->renamed_tab_name),
						        PetraNamePool->getNameString(pk_row->constraint_name));
					}
			...
			if (TabInfo.enc_type == 0) {
				if (pk_row->constraint_type == 1) {
					...
					sprintf(pkfkSql.enc2,"alter table %s.%s drop index %s",
					        PetraNamePool->getNameString(pk_row->schema_name),
					        PetraNamePool->getNameString(pk_row->renamed_tab_name),
					        PetraNamePool->getNameString(pk_row->constraint_name));
				}
...
dgt_sint32 PccMyScriptBuilder::prepareCt2Info() throw(DgcExcept)
{
	...
	while ((const_name=(dgt_sint64*)sql_stmt->fetch())) {
		...
		memset(sql_text,0,2048);
		sprintf(sql_text,
		        "select constraint_name, a.schema_name, a.table_name, b.column_name, d.renamed_tab_name, c.ref_pk_owner, c.ref_pk_table, c.ref_pk_column, c.position "
		        "from   ceea_table a, "
		        "ceea_column b, "
		        "ceea_col_ct c, "
		        "ceea_enc_table d "
		...
		while ((def_fksql=(pc_type_def_fksql2*)fkStmt->fetch())) {
			...
			if (enc_tab_flag) {
				...
				sprintf(def_enc_sql.enc4,"alter table %s.%s drop foreign key %s",PetraNamePool->getNameString(def_fksql->schema_name),
				        PetraNamePool->getNameString(def_fksql->renamed_tab_name),
				        PetraNamePool->getNameString(def_fksql->constraint_name));
				sprintf(def_enc_sql.enc3,"alter table %s.%s drop index %s",PetraNamePool->getNameString(def_fksql->schema_name),
				        PetraNamePool->getNameString(def_fksql->renamed_tab_name),
				        PetraNamePool->getNameString(def_fksql->constraint_name));
				sprintf(def_enc_sql.enc2,"alter table %s.%s drop foreign key %s",PetraNamePool->getNameString(def_fksql->schema_name),
				        PetraNamePool->getNameString(def_fksql->table_name),
				        PetraNamePool->getNameString(def_fksql->constraint_name));
				sprintf(def_enc_sql.enc1,"alter table %s.%s drop index %s",PetraNamePool->getNameString(def_fksql->schema_name),
				        PetraNamePool->getNameString(def_fksql->table_name),
				        PetraNamePool->getNameString(def_fksql->constraint_name));
				if (TabInfo.enc_type == 0) {
					sprintf(def_enc_sql.org2,"alter table %s.%s$$ drop foreign key %s",
					        PetraNamePool->getNameString(def_fksql->schema_name),
					        PetraNamePool->getNameString(def_fksql->table_name),
					        PetraNamePool->getNameString(def_fksql->constraint_name));
					sprintf(def_enc_sql.org1,"alter table %s.%s$$ drop index %s",
					        PetraNamePool->getNameString(def_fksql->schema_name),
					        PetraNamePool->getNameString(def_fksql->table_name),
					        PetraNamePool->getNameString(def_fksql->constraint_name));
				} else {
...
PccMyScriptBuilder::PccMyScriptBuilder(DgcDatabase* db, DgcSession* sess,dgt_schar* schema_link)
	: PccScriptBuilder(db,sess,schema_link), PrivSqlRows(1), PrivSqlRows2(3), PkSqlRows(4), FkSqlRows(4),
	  IdxSqlRows(4), IdxColRows(1),
	  DefFkDropSqlRows(6)
{
...
dgt_schar* PccMyScriptBuilder::getFname(dgt_sint64 enc_col_id,dgt_uint8 fun_type) throw(DgcExcept)
{
	memset(Fname,0,256);
	ColInfoRows2.rewind();
	pc_type_col_info* col_info;
	...
	if (fun_type == 1) {
		while (ColInfoRows2.next() && (col_info=(pc_type_col_info*)ColInfoRows2.data())) {
			if (col_info->enc_col_id==enc_col_id) {
				sprintf(Fname,"petra.pls_encrypt_b64(ifnull(%s,''),%lld)",col_info->col_name, col_info->enc_col_id);
			}
		}
	} else if (fun_type == 2) {
		while (ColInfoRows2.next() && (col_info=(pc_type_col_info*)ColInfoRows2.data())) {
			if (col_info->enc_col_id==enc_col_id) {
				if (TabInfo.cast_flag == 0) {
					sprintf(Fname,"petra.pls_decrypt_b64(%s,%lld)",col_info->renamed_col_name, col_info->enc_col_id);
				} else {
					if (!strcasecmp(col_info->data_type,"date") || !strcasecmp(col_info->data_type,"datetime") ||
					    !strcasecmp(col_info->data_type,"time") || !strcasecmp(col_info->data_type,"timestamp")) {
					    sprintf(Fname,"cast(petra.pls_decrypt_b64(%s,%lld) as date)",
					            col_info->renamed_col_name, col_info->enc_col_id);
					} else if (!strcasecmp(col_info->data_type,"smallint") || !strcasecmp(col_info->data_type,"int") ||
					           !strcasecmp(col_info->data_type,"tinyint") || !strcasecmp(col_info->data_type,"bigint") ||
					           !strcasecmp(col_info->data_type,"mediumint")) {
						sprintf(Fname,"cast(petra.pls_decrypt_b64(%s,%lld) as SIGNED)",
						        col_info->renamed_col_name, col_info->enc_col_id);
					} else if (!strcasecmp(col_info->data_type,"decimal") || !strcasecmp(col_info->data_type,"float") ||
					           !strcasecmp(col_info->data_type,"hexadecimal") || !strcasecmp(col_info->data_type,"double")) {
						sprintf(Fname,"cast(petra.pls_decrypt_b64(%s,%lld) as DECIMAL)",
						        col_info->renamed_col_name, col_info->enc_col_id);
					} else {
						sprintf(Fname,"cast(petra.pls_decrypt_b64(%s,%lld) as char(%d))",
						        col_info->renamed_col_name, col_info->enc_col_id, col_info->data_length);
					}
				}
			}
		}
...
DgcCliConnection* PccMyScriptBuilder::connect(dgt_schar* uid,dgt_schar* pw) throw(DgcExcept)
{
	...
	if (conn->connect(port, link_info->host, uid, pw, link_info->db_name) != 0) {
		DgcExcept*	e=EXCEPTnC;
		delete conn;
...
dgt_sint32 PccMyScriptBuilder::buildScript(dgt_sint64 enc_tab_id,dgt_uint16 version_no) throw(DgcExcept)
{
	//
	// enc_type = 0 (view)
	// enc_type = 1 (non view)
	...
	if (reverse_step1() < 0) ATHROWnR(DgcError(SPOS,"reverse step1 failed."),-1);
	if (reverse_step2() < 0) ATHROWnR(DgcError(SPOS,"reverse step2 failed."),-1);
	return 0;
}
