...
DgcCliConnection* PccOraScriptBuilder::connect(dgt_schar* uid,dgt_schar* pw) throw(DgcExcept)
{
        //
...
"where enc_tab_id=%lld ",TabInfo.enc_tab_id);
        ...
        while ((priv_info_tmp=(pct_type_enc_tab_priv*)sql_stmt->fetch())) {
                ...
                sprintf(privSql,"grant %s on %s.%s to %s %s", privilege, SchemaName, TabInfo.table_name 
		...
		if (TabInfo.enc_type == 0) {
			memset(privSql,0,1024);
			sprintf(privSql,"grant %s on %s.%s to %s %s", privilege, SchemaName, TabInfo.renamed_tab_name, 
								   priv_info_tmp->grantee, GrantOption);
        	        PrivSqlRows.add();
                	PrivSqlRows.next();
	                memcpy(PrivSqlRows.data(), privSql, 1024);
				...
				if (!strcasecmp(type,"SYNONYM")) {
					continue;
		                ...
		                if (!strcasecmp(type,"PACKAGE BODY")) {
                		        ...
                		        sprintf(objSql,"alter PACKAGE %s.%s compile body", owner, name);
		                } else {
...
dgt_sint32 PccOraScriptBuilder::prepareIdxInfo() throw(DgcExcept)
{
	...
	if (TabInfo.synonym_flag == 0) {
	        sprintf(sql_text,
"select a.enc_col_id, b.column_name, b.data_type, a.index_type, b.domain_index_name, b.fbi_index_name, "
"b.normal_index_name, a.tablespace_name, a.normal_idx_flag, b.index_col_name "
//", a.sql_text, a.normal_sql_text, a.sql_text2, a.normal_sql_text2, a.normal_sql_text, a.normal_sql_text "
"from pct_enc_index a, pct_enc_column b, pct_enc_table c "
"where a.enc_col_id = b.enc_col_id "
"and   b.enc_tab_id = c.enc_tab_id " 
"and   b.enc_tab_id = %lld",TabInfo.enc_tab_id);
	} else {
	        sprintf(sql_text,
"select d.key_id, b.column_name, b.data_type, a.index_type, b.domain_index_name, b.fbi_index_name, "
"b.normal_index_name, a.tablespace_name, a.normal_idx_flag, b.index_col_name "
"from pct_enc_index a, pct_enc_column b, pct_enc_table c, pct_encrypt_key d "
"where a.enc_col_id = b.enc_col_id "
"and   b.enc_tab_id = c.enc_tab_id " 
...
"and   b.enc_tab_id = %lld",TabInfo.enc_tab_id);
	...
	pc_type_index_row*    idx_info=0;
		...
		if (idx_info->index_type == 1) {
			if (TabInfo.enc_type == 0 && idx_info->normal_idx_flag == 1) {
				// normal_idx_flag == 1 create the domain index
							...
							SchemaName,TabInfo.renamed_tab_name,idx_info->column_name,
							AgentName);
				...
				if (TabInfo.partitioned) {
					sprintf(idx_col_idx1,"create index %s.%s on %s.%s(%s) tablespace %s parallel %d nologging local",
        	                			SchemaName,idx_info->fbi_index_name,
	                        	                ...
	                        	                idx_info->column_name,idx_info->tablespace_name,
        	                        	        ParallelDegree);
				} else {
			...
			delete EXCEPTnC;
				...
				// create the domain index
                                if (!strcasecmp(idx_info->data_type,"NUMBER")) {
                                        sprintf(idx_sql,"create index %s.%s on %s.%s(%s) indextype is %s.PC_IDX2_TYP2",
                                                        SchemaName,idx_info->domain_index_name,
                                                        SchemaName,TabInfo.renamed_tab_name,idx_info->column_name,
                                                        AgentName);
                                } else {
                                        sprintf(idx_sql,"create index %s.%s on %s.%s(%s) indextype is %s.PC_IDX2_TYP1",
                                                        SchemaName,idx_info->domain_index_name,
                                                        SchemaName,TabInfo.renamed_tab_name,idx_info->column_name,
                                                        AgentName);
                                }
			...
			// create domain index`s fbi(PLS_OPHUEK_B64) index
                        if (TabInfo.partitioned) {
                                sprintf(idx_col_idx1,"create index %s.%s on %s.%s(PLS_OPHUEK_B64(%s,%lld,1)) tablespace %s parallel %d nologging local",
                                                SchemaName,idx_info->fbi_index_name,
                                                SchemaName,TabInfo.renamed_tab_name,
                                                idx_info->column_name, idx_info->enc_col_id, 
						idx_info->tablespace_name, 
                                                ParallelDegree);
                        } else {
                                sprintf(idx_col_idx1,"create index %s.%s on %s.%s(PLS_OPHUEK_B64(%s,%lld,1)) tablespace %s parallel %d nologging",
                                                SchemaName,idx_info->fbi_index_name,
                                                SchemaName,TabInfo.renamed_tab_name,
                                                idx_info->column_name, idx_info->enc_col_id,
						idx_info->tablespace_name,
			...
			//
                        if (TabInfo.enc_type == 0) {
                                // normal_idx_flag == 1 create the domain index
                                if (!strcasecmp(idx_info->data_type,"NUMBER")) {
                                        sprintf(idx_sql,"create index %s.%s on %s.%s(%s) indextype is %s.PC_IDX3_TYP2",
                                                        SchemaName,idx_info->domain_index_name,
                                                        SchemaName,TabInfo.renamed_tab_name,idx_info->column_name,
                                                        AgentName);
                                } else if (!strcasecmp(idx_info->data_type,"DATE") || !strcasecmp(idx_info->data_type,"TIMESTAMP")) {
                                        sprintf(idx_sql,"create index %s.%s on %s.%s(%s) indextype is %s.PC_IDX3_TYP3",
                                                        SchemaName,idx_info->domain_index_name,
                                                        SchemaName,TabInfo.renamed_tab_name,idx_info->column_name,
                                                        AgentName);
                                } else {
                                        sprintf(idx_sql,"create index %s.%s on %s.%s(%s) indextype is %s.PC_IDX3_TYP1",
                                                        SchemaName,idx_info->domain_index_name,
                                                        SchemaName,TabInfo.renamed_tab_name,idx_info->column_name,
                                                        AgentName);
                                }
		...
		if (idx_info->index_type || idx_info->normal_idx_flag) {
			...
			strncpy(pt_idx_info.normal_sql_text,normal_sql,strlen(normal_sql));
			strncpy(pt_idx_info.sql_text2,idx_sql2,strlen(idx_sql2));
	                strncpy(pt_idx_info.normal_sql_text2,normal_sql2,strlen(normal_sql2));
			strncpy(pt_idx_info.idx_col_idx1,idx_col_idx1,strlen(idx_col_idx1));
			strncpy(pt_idx_info.idx_col_idx2,idx_col_idx2,strlen(idx_col_idx2));
			PetraIdxInfoRows.add();
			PetraIdxInfoRows.next();
			memcpy(PetraIdxInfoRows.data(),&pt_idx_info,sizeof(pc_type_petra_index));
		}
	}
	DgcExcept* e=EXCEPTnC;
	if (e) {
...
"and   c.idx_name2 = 0 ",TabInfo.enc_tab_id,TabInfo.enc_tab_id);
#endif
        sql_stmt=Database->getStmt(Session,sql_text,strlen(sql_text));
        if (sql_stmt == 0 || sql_stmt->execute() < 0) {
                DgcExcept*      e=EXCEPTnC;
...
"end;");
		...
		if (stmt->execute(sql_text,strlen(sql_text),10) < 0) {
        	...
        	delete stmt;
                sprintf(sql_text, "select dbms_metadata.get_ddl('INDEX','%s','%s') from dual",
...
//				strcat(idx_sql3," NOLOGGING ");
                ...
                sprintf(idx_sql2,"ALTER INDEX %s.%s PARALLEL %d LOGGING",PetraNamePool->getNameString(idx_tmp->index_owner),
       	                                                         PetraNamePool->getNameString(idx_tmp->renamed_org_name),
               	                                                 idx_tmp->degree);
                IdxSqlRows3.add();
       	        IdxSqlRows3.next();
...
"and   b.enc_tab_id = c.enc_tab_id "
"and   a.enc_tab_id=%lld "
"and   getname(a.search_condition) like '%%IS NOT NULL%%' ", TabInfo.enc_tab_id);
	...
	memset(&check_sql,0,sizeof(pc_type_check_sql));
		...
		if (TabInfo.enc_type == 0) {
			if (TabInfo.init_enc_type >= 1) {
				sprintf(check_sql.org1,"alter table %s.petra_%lld modify %s not null",SchemaName,TabInfo.enc_tab_id,
                                                                                                        check_row_tmp->column_name);
                                sprintf(check_sql.enc1,"alter table %s.%s modify %s not null",SchemaName,TabInfo.renamed_tab_name,
                                                                                                        check_row_tmp->column_name);
			} else {
				sprintf(check_sql.org1,"alter table %s.%s modify %s not null",SchemaName,TabInfo.renamed_tab_name,
													check_row_tmp->column_name);
				sprintf(check_sql.org2,"alter table %s.%s modify %s null",SchemaName,TabInfo.renamed_tab_name,
                        	                                                                        check_row_tmp->column_name);
				sprintf(check_sql.enc1,"alter table %s.%s modify %s not null",SchemaName,TabInfo.renamed_tab_name,
													check_row_tmp->column_name);
				sprintf(check_sql.enc2,"alter table %s.%s modify %s null",SchemaName,TabInfo.renamed_tab_name,
                        	                                                                        check_row_tmp->column_name);
			}
		} else {
			if (TabInfo.init_enc_type >= 1) {
				...
				sprintf(check_sql.org1,"alter table %s.%s modify %s not null",SchemaName,TabInfo.table_name,
													check_row_tmp->column_name);
				sprintf(check_sql.org2,"alter table %s.%s modify %s null",SchemaName,TabInfo.table_name,
                        	                                                                        check_row_tmp->column_name);
				sprintf(check_sql.enc1,"alter table %s.%s modify %s not null",SchemaName,TabInfo.table_name,
													check_row_tmp->renamed_col_name);
				sprintf(check_sql.enc2,"alter table %s.%s modify %s null",SchemaName,TabInfo.table_name,
                                                                                                check_row_tmp->renamed_col_name);
			}
		}
...
"and   a.constraint_type =3",TabInfo.enc_tab_id);
        ...
        while ((tmp_search=(type_check*)sql_stmt->fetch())) {
                ...
                sprintf(tmp_st_con,"%s",PetraNamePool->getNameString(tmp_search->search_condition));
...
dgt_schar* PccOraScriptBuilder::getFname(dgt_schar* col_name,dgt_uint8 fun_type,dgt_uint8 instead_of_trigger_flag) throw(DgcExcept)
{
        memset(fname,0,256);
        ColInfoRows2.rewind();
					...
					if (col_info->col_default) {
						if (!strcasecmp(col_info->data_type,"date") || !strcasecmp(col_info->data_type,"timestamp")) {
							...
							if (col_info->coupon_id) {
	                                        	        sprintf(fname,"pls_encrypt_cpn_id(nvl(trim(:new.%s),%s),%lld)",
									col_info->col_name,PetraNamePool->getNameString(col_info->col_default),
									col_info->enc_col_id);
							} else {
							...
							if (col_info->coupon_id) {
	                                                        sprintf(fname,"pls_encrypt_cpn_id(nvl(:new.%s,%s),%lld)",
                                                                        col_info->col_name,PetraNamePool->getNameString(col_info->col_default),
                                                                        col_info->enc_col_id);
							} else {
						...
						if (!strcasecmp(col_info->data_type,"date") || !strcasecmp(col_info->data_type,"timestamp")) {
							...
							if (col_info->coupon_id) {
	        	                                        sprintf(fname,"pls_encrypt_cpn_id(trim(:new.%s),%lld)",col_info->col_name,col_info->enc_col_id);
							} else {
	        	                                        sprintf(fname,"pls_encrypt_b64_id(trim(:new.%s),%lld)",col_info->col_name,col_info->enc_col_id);
							}
						} else if (!strcasecmp(col_info->data_type,"nchar")) {
							if (col_info->coupon_id) {
	                                                        sprintf(fname,"pls_encrypt_cpn_id(trim(:new.%s),%lld)",
							...
							if (col_info->coupon_id) {
	                                                        sprintf(fname,"pls_encrypt_cpn_id(:new.%s,%lld)",
                                                                        col_info->col_name,col_info->enc_col_id);
							} else {
							...
							if (col_info->coupon_id) {
	        	                                        sprintf(fname,"pls_encrypt_cpn_id(:new.%s,%lld)",col_info->col_name,col_info->enc_col_id);
							} else {
						...
						if (col_info->coupon_id) {
	                                	        sprintf(fname,"pls_encrypt_cpn_id(trim(%s),%lld)",col_info->col_name,col_info->enc_col_id);
						} else {
						...
						if (col_info->coupon_id) {
                                	        	sprintf(fname,"pls_encrypt_cpn_id(%s,%lld)",col_info->col_name,col_info->enc_col_id);
						} else {
			...
			if (!strcasecmp(col_info->col_name,col_name)) {
                        //if (col_info->enc_col_id==enc_col_id) {
                                if (col_info->normal_idx_flag == 1) {
                                        if (!strcasecmp(col_info->data_type,"date") || !strcasecmp(col_info->data_type,"timestamp")) {
                                                sprintf(fname,"pls_idx1_date(%s,%lld)",renamed_col_name,col_info->enc_col_id);
                                        } else if (!strcasecmp(col_info->data_type,"number")) {
                                                sprintf(fname,"pls_idx1_num(%s,%lld)",renamed_col_name,col_info->enc_col_id);
                                        } else {
                                                sprintf(fname,"pls_idx1_str(%s,%lld)",renamed_col_name,col_info->enc_col_id);
                                        }
                                } else {
							...
							if (col_info->coupon_id) {
	                                                        sprintf(fname,"pls_decrypt_cpn_id(%s,%lld)",renamed_col_name,col_info->enc_col_id);
							} else {
							...
							if (col_info->coupon_id) {
                                                        	sprintf(fname,"cast(pls_decrypt_cpn_id(%s,%lld) as %s(%d))",
                                                                	renamed_col_name, col_info->enc_col_id,
...
dgt_sint32 PccOraScriptBuilder::insteadOfTrigger(dgt_sint8 is_final,dgt_sint32 uniq_flag) throw(DgcExcept)
{
        //
...
"end;");
	...
	delete stmt;
        sprintf(sql_text,
                "select dbms_metadata.get_ddl('TABLE','%s','%s') from dual",TabInfo.table_name,SchemaName);
        stmt=Connection->getStmt();
        if (!stmt) {
                ATHROWnR(DgcError(SPOS,"getStmt failed."),-1);
        }
	...
	if (TabInfo.tran_trg_flag == 1 && TranIdxColRows.numRows() > 0) {
		...
		*TextBuf=0;
	        *TmpBuf=0;
		sprintf(TextBuf,"CREATE TABLE %s.%s as select 1 petra_id, '          ' petra_type, "
				"alias.* from %s.%s alias where 1=0",AgentName,TabInfo.tran_tab_name,SchemaName,TabInfo.table_name);
		if (saveSqlText() < 0) {
			ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		}
		*TextBuf=0;
	        *TmpBuf=0;
		sprintf(TextBuf,"create index %s.tran_%lld_idx on %s.%s(petra_id)",AgentName,TabInfo.enc_tab_id,AgentName,TabInfo.tran_tab_name);
		if (saveSqlText() < 0) {
        	        ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	        }
		//
		...
		*TextBuf=0;
	        *TmpBuf=0;
        	sprintf(TextBuf,"CREATE OR REPLACE TRIGGER %s.%s \n"
				...
				"begin \n"
				"\tif inserting then \n"
				"\t\tinsert into %s.%s(PETRA_ID,PETRA_TYPE,",AgentName,TabInfo.tab_trigger_name,SchemaName,TabInfo.table_name,
								 AgentName,TabInfo.tran_tab_name);
		ColInfoRows.rewind();
		while(ColInfoRows.next() && (col_info=(pc_type_col_info*)ColInfoRows.data())) {
			*TmpBuf=0;
			if (col_info->status == 0 && !strcasecmp(col_info->data_type,"LONG")) continue;
			sprintf(TmpBuf,"%s,",col_info->col_name);
			strcat(TextBuf,TmpBuf);
		}
		TextBuf[strlen(TextBuf)-1]=0;
		...
		*TmpBuf=0;
		sprintf(TmpBuf,"\t\tinsert into %s.%s(PETRA_ID,PETRA_TYPE,",AgentName,TabInfo.tran_tab_name);
		strcat(TextBuf,TmpBuf);
		ColInfoRows.rewind();
	        while(ColInfoRows.next() && (col_info=(pc_type_col_info*)ColInfoRows.data())) {
        	        *TmpBuf=0;
	...
	if (lob_flag == 0) {
		...
		*TextBuf=0;
                sprintf(TextBuf,"ALTER TABLE %s.%s nologging", SchemaName, TabInfo.renamed_tab_name);
                if (saveSqlText() < 0) {
                        ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                }
	}
...
" and subobject_name is not null ",SchemaName, TabInfo.table_name);
		stmt=Connection->getStmt();
		if (!stmt) {
			ATHROWnR(DgcError(SPOS,"getStmt failed."),-1);
		}
		...
		while(rows->next() && (part_name=(dgt_schar*)rows->data())) {
			...
			*TmpBuf=0;
			ColInfoRows.rewind();
	                sprintf(TmpBuf,"insert /*+ APPEND PARALLEL(%s,%d) */ into %s.%s partition(%s) %s \n select /*+ FULL(%s) PARALLEL(%s,%d) */ ",
        	                TabInfo.renamed_tab_name, ParallelDegree, SchemaName,TabInfo.renamed_tab_name, part_name, TabInfo.renamed_tab_name,
			...
			dgt_sint32      count_fun_use_flag=0;
	                while(ColInfoRows.next() && (col_info=(pc_type_col_info*)ColInfoRows.data())) {
							...
							if (col_info->coupon_id) {
	                                        	        sprintf(TmpBuf,"PLS_ENCRYPT_CPN_ID(TRIM(%s),%lld)",col_info->col_name,col_info->enc_col_id);
							} else {
	                                        	        sprintf(TmpBuf,"PLS_ENCRYPT_B64_ID(TRIM(%s),%lld)",col_info->col_name,col_info->enc_col_id);
	                                                	count_fun_use_flag=1;
							}
	                                        } else if (!strcasecmp(col_info->data_type,"VARCHAR") || !strcasecmp(col_info->data_type,"VARCHAR2")) {
							if (col_info->coupon_id) {
        	                                        	sprintf(TmpBuf,"PLS_ENCRYPT_CPN_ID(%s,%lld)",col_info->col_name,col_info->enc_col_id);
							} else {
        	                                        	sprintf(TmpBuf,"PLS_ENCRYPT_B64_ID(%s,%lld)",col_info->col_name,col_info->enc_col_id);
	                	                                count_fun_use_flag=1;
							}
			...
			TextBuf[strlen(TextBuf)-1]=0;
        	       	*TmpBuf=0;
	                sprintf(TmpBuf," from %s.%s partition(%s) %s",SchemaName,TabInfo.table_name,part_name,TabInfo.table_name);
        	        strcat(TextBuf,TmpBuf);
                	if (saveSqlText() < 0) {
	                        ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        	        }
	...
	*TextBuf=0;
	sprintf(TextBuf,"ALTER TABLE %s.%s logging", SchemaName, TabInfo.renamed_tab_name);
	if (saveSqlText() < 0) {
		ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	}
                ...
                *TmpBuf=0;
                ...
                while(ColInfoRows.next() && (col_info=(pc_type_col_info*)ColInfoRows.data())) {
                        if (col_info->status == 1 && (!strcasecmp(col_info->data_type, "LONG") || !strcasecmp(col_info->data_type, "LONG RAW"))) {
                                dgt_sint32 idx_flag=col_info->index_type;
                                *TmpBuf=0;
                                sprintf(TmpBuf,"\n\t\t%s=%s,", col_info->col_name, getFname(col_info->col_name,1));
                                strcat(TextBuf,TmpBuf);
                                *TmpBuf=0;
                        }
                }
	...
	*TmpBuf=0;
	...
	if (TabInfo.tran_trg_flag == 1 && TranIdxColRows.numRows() > 0) {
		...
		*TmpBuf=0;
		ColInfoRows.rewind();
		sprintf(TextBuf,"create or replace procedure %s.proc1_%lld\n"
				"is\n\tcursor tr_table_cursor is \n\t\tselect petra_id,petra_type,",AgentName,TabInfo.enc_tab_id);
		while (ColInfoRows.next() && (col_info=(pc_type_col_info*)ColInfoRows.data())) {
			*TmpBuf=0;
			if (col_info->status == 0 && !strcasecmp(col_info->data_type,"LONG")) continue;
			if (col_info->status == 1) sprintf(TmpBuf,"%s %s,",getFname(col_info->col_name,1),col_info->col_name);
			else sprintf(TmpBuf,"%s,",col_info->col_name);
			strcat(TextBuf,TmpBuf);
		}
#if 0
		ColInfoRows.rewind();
		...
		*TmpBuf=0;
		...
		while (ColInfoRows.next() && (col_info=(pc_type_col_info*)ColInfoRows.data())) {
			*TmpBuf=0;	
			...
			if (col_info->status == 0) sprintf(TmpBuf,"\tv_%s %s.%s.%s%%TYPE;\n",col_info->col_name,AgentName,TabInfo.tran_tab_name,col_info->col_name);
		...
		*TmpBuf=0;
		...
		while (ColInfoRows.next() && (col_info=(pc_type_col_info*)ColInfoRows.data())) {
			*TmpBuf=0;
			if (col_info->status == 0 && !strcasecmp(col_info->data_type,"LONG")) continue;
			if (col_info->status == 1) sprintf(TmpBuf,"%s,",col_info->col_name);
			else sprintf(TmpBuf,"%s,",col_info->col_name);
			strcat(TextBuf,TmpBuf);
		}
		TextBuf[strlen(TextBuf)-1]=0;
		...
		strcat(TextBuf,");\n");
		*TmpBuf=0;
		sprintf(TmpBuf,"\t\t\tdelete %s.%s where petra_id = v_value.petra_id;\n",AgentName,TabInfo.tran_tab_name);
		strcat(TextBuf,TmpBuf);
		*TmpBuf=0;
		...
		strcat(TextBuf,TmpBuf);
                ...
                if (TranIdxColRows.numRows() > 0) {
                        dgt_schar* col_name=0;
                        while (TranIdxColRows.next() && (col_name=(dgt_schar*)TranIdxColRows.data())) {
                                *TmpBuf=0;
                        ...
                        while (ColInfoRows.next() && (col_info=(pc_type_col_info*)ColInfoRows.data())) {
                                *TmpBuf=0;
                                if (!strcasecmp(col_info->data_type,"LONG")) continue;
                                if (col_info->status == 1) {
                                        sprintf(TmpBuf,"\t\t\tv_%s := nvl(v_value.%s,'=p=e=t=r=a=');\n",col_info->col_name,col_info->col_name);
                                } else {
                                        sprintf(TmpBuf,"\t\t\tv_%s := nvl(v_value.%s,'=p=e=t=r=a=');\n",col_info->col_name,col_info->col_name);
                                }
                                strcat(TextBuf,TmpBuf);
		...
		*TmpBuf=0;
		...
		ColInfoRows.rewind();
			...
			*TmpBuf=0;
			if (col_info->status == 0 && !strcasecmp(col_info->data_type,"LONG")) continue;
			if (col_info->status == 1) {
				sprintf(TmpBuf," %s = v_value.%s,",col_info->col_name,col_info->col_name);
			} else {
				sprintf(TmpBuf," %s = v_value.%s,",col_info->col_name,col_info->col_name);
			}
                	strcat(TextBuf,TmpBuf);
			...
			dgt_sint32 seq=0;
				...
				*TmpBuf=0;
				...
				if (seq == 1) {
					if (!strcasecmp(col_info->data_type,"CLOB")) {
						if (col_info->status == 1) {
							sprintf(TmpBuf," dbms_lob.compare(nvl(%s,'=p=e=t=r=a='),v_%s) = 0 ",col_info->col_name,col_info->col_name);
						} else {
							sprintf(TmpBuf," dbms_lob.compare(nvl(%s,'=p=e=t=r=a='),v_%s) = 0 ",col_info->col_name,col_info->col_name);
						}
					} else if (!strcasecmp(col_info->data_type,"BLOB")){
						if (col_info->status == 1) {
							sprintf(TmpBuf," %s = v_%s ",col_info->col_name,col_info->col_name);
						} else {
							sprintf(TmpBuf," %s = v_%s ",col_info->col_name,col_info->col_name);
						}
					} else {
						if (col_info->status == 1) {
							sprintf(TmpBuf," nvl(%s,'=p=e=t=r=a=') = v_%s ",col_info->col_name,col_info->col_name);
						} else {
							sprintf(TmpBuf," nvl(%s,'=p=e=t=r=a=') = v_%s ",col_info->col_name,col_info->col_name);
						}
					}
					...
					if (!strcasecmp(col_info->data_type,"CLOB")) {
						if (col_info->status == 1) {
							sprintf(TmpBuf," and dbms_lob.compare(nvl(%s,'=p=e=t=r=a='),v_%s) = 0 ",col_info->col_name,col_info->col_name);
						} else {
							sprintf(TmpBuf," and dbms_lob.compare(nvl(%s,'=p=e=t=r=a='),v_%s) = 0 ",col_info->col_name,col_info->col_name);
						}
	                	        } else if (!strcasecmp(col_info->data_type,"BLOB")){
						if (col_info->status == 1) {
		        	                        sprintf(TmpBuf," and %s = v_%s ",col_info->col_name,col_info->col_name);
						} else {
		        	                        sprintf(TmpBuf," and %s = v_%s ",col_info->col_name,col_info->col_name);
						}
        	        	        } else {
						if (col_info->status == 1) {
	                	        	        sprintf(TmpBuf," and nvl(%s,'=p=e=t=r=a=') = v_%s ",col_info->col_name,col_info->col_name);
						} else {
	                	        	        sprintf(TmpBuf," and nvl(%s,'=p=e=t=r=a=') = v_%s ",col_info->col_name,col_info->col_name);
						}
	                	        }
		...
		*TmpBuf=0;
		...
		strcat(TextBuf,TmpBuf);
		*TmpBuf=0;
		sprintf(TmpBuf,"\t\t\tdelete %s.%s where ",SchemaName,TabInfo.renamed_tab_name);
		strcat(TextBuf,TmpBuf);
		TranIdxColRows.rewind();
	        if (TranIdxColRows.numRows() > 0) {
        	        dgt_schar* col_name=0;
	                ...
	                while (TranIdxColRows.next() && (col_name=(dgt_schar*)TranIdxColRows.data())) {
        	                *TmpBuf=0;
                        	...
                        	if (seq == 1) {
                	                ...
                	                sprintf(TmpBuf," and %s = v_value.%s ",col_name,col_name);
                        	}
	                        strcat(TextBuf,TmpBuf);
        	        }
		} else {	
			...
			dgt_sint32 seq=0;
				...
				if (!strcasecmp(col_info->data_type,"LONG")) continue;
					...
					if (!strcasecmp(col_info->data_type,"CLOB")) {
						if (col_info->status == 1) {
			                	        sprintf(TmpBuf," dbms_lob.compare(nvl(%s,'=p=e=t=r=a='),nvl(v_value.%s,'=p=e=t=r=a=')) = 0 ",
									col_info->col_name,col_info->col_name);
						} else {
			                	        sprintf(TmpBuf," dbms_lob.compare(nvl(%s,'=p=e=t=r=a='),nvl(v_value.%s,'=p=e=t=r=a=')) = 0 ",
									col_info->col_name,col_info->col_name);
						...
						if (col_info->status == 1) {
							sprintf(TmpBuf," nvl(%s,'=p=e=t=r=a=') = nvl(v_value.%s,'=p=e=t=r=a=') ",col_info->col_name,col_info->col_name);
						} else {
							sprintf(TmpBuf," nvl(%s,'=p=e=t=r=a=') = nvl(v_value.%s,'=p=e=t=r=a=') ",col_info->col_name,col_info->col_name);
						}
					}
					...
					if (!strcasecmp(col_info->data_type,"CLOB")) {
						if (col_info->status == 1) {
	        	                		sprintf(TmpBuf," and dbms_lob.compare(nvl(%s,'=p=e=t=r=a='),nvl(v_value.%s,'=p=e=t=r=a=')) = 0 ",
									col_info->col_name, col_info->col_name);
						} else {
	        	                		sprintf(TmpBuf," and dbms_lob.compare(nvl(%s,'=p=e=t=r=a='),nvl(v_value.%s,'=p=e=t=r=a=')) = 0 ",
									col_info->col_name, col_info->col_name);
						...
						if (col_info->status == 1) {
							sprintf(TmpBuf," and %s = v_value.%s ",col_info->col_name,col_info->col_name);
						} else {
							sprintf(TmpBuf," and %s = v_value.%s ",col_info->col_name,col_info->col_name);
						}
					} else {
						if (col_info->status == 1) {
							sprintf(TmpBuf," and nvl(%s,'=p=e=t=r=a=') = nvl(v_value.%s,'=p=e=t=r=a=') ",
									col_info->col_name,col_info->col_name);
						} else {
							sprintf(TmpBuf," and nvl(%s,'=p=e=t=r=a=') = nvl(v_value.%s,'=p=e=t=r=a=') ",
									col_info->col_name,col_info->col_name);
		...
		*TextBuf=0;
		*TmpBuf=0;
		sprintf(TextBuf,"create or replace procedure %s.proc2_%lld\nis\n\tv_cnt number;\nbegin\n\tloop\n",AgentName,TabInfo.enc_tab_id);
		sprintf(TmpBuf,"\t\tselect count(rowid) into v_cnt from %s.%s;\n",AgentName,TabInfo.tran_tab_name);
		strcat(TextBuf,TmpBuf);
		*TmpBuf=0;
		sprintf(TmpBuf,"\t\t%s.proc1_%lld;\n\texit when(v_cnt <= 0);\n\tend loop;\nend;",AgentName,TabInfo.enc_tab_id);
		strcat(TextBuf,TmpBuf);
		if (saveSqlText() < 0) {
	                ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        	}
...
dgt_sint32 PccOraScriptBuilder::step2() throw(DgcExcept)
{
	...
	*TextBuf=0;
	...
	if (TabInfo.tran_trg_flag == 1 && TranIdxColRows.numRows() > 0) {
                ...
                *TextBuf=0;
                *TmpBuf=0;
                sprintf(TextBuf,"drop trigger %s.%s",AgentName,TabInfo.tab_trigger_name);
                if (saveSqlText() < 0) {
                        ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                }
                *TextBuf=0;
                *TmpBuf=0;
		if (!strcasecmp(DbVersion,"9i")) sprintf(TextBuf,"drop table %s.%s",AgentName,TabInfo.tran_tab_name);
		else sprintf(TextBuf,"drop table %s.%s purge",AgentName,TabInfo.tran_tab_name);
                if (saveSqlText() < 0) {
                        ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                }
                *TextBuf=0;
                *TmpBuf=0;
                sprintf(TextBuf,"drop procedure %s.proc1_%lld",AgentName,TabInfo.enc_tab_id);
                if (saveSqlText() < 0) {
                        ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                }
                *TextBuf=0;
                *TmpBuf=0;
                sprintf(TextBuf,"drop procedure %s.proc2_%lld",AgentName,TabInfo.enc_tab_id);
                if (saveSqlText() < 0) {
                        ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                }
	}
                                ...
                                if (col_info->status == 1) {
                                        if (col_info->cipher_type == 4) {
	                                        ...
	                                        strcat(TextBuf,TmpBuf);
                                                ...
                                                *TmpBuf=0;
	                                       	sprintf(TmpBuf,"%s %s,", getFname(col_info->col_name,2),col_info->col_name);
        	                    		strcat(TextBuf,TmpBuf);
					}
                                } else {
                                        sprintf(TmpBuf,"%s,",col_info->col_name);
...
"end;");
		...
		*TextBuf=0;
		...
		if (TabInfo.enc_type == 0) {
					...
					TabInfo.renamed_tab_name, TabInfo.renamed_tab_name, ParallelDegree);
		} else {
	                sprintf(TmpBuf,"insert /*+ APPEND PARALLEL(petra_%lld,%d) */ into %s.%s_%lld \n select /*+ FULL(%s) PARALLEL(%s,%d) */ ",
        	                	TabInfo.enc_tab_id, ParallelDegree,  SchemaName,"petra",TabInfo.enc_tab_id,
					TabInfo.table_name, TabInfo.table_name, ParallelDegree);
...
"and   b.status = 2",TabInfo.enc_tab_id);
		...
		sprintf(sql_text,
"select d.key_id, b.renamed_col_name, b.data_type, a.index_type, b.domain_index_name, b.fbi_index_name, "
"b.normal_index_name, a.tablespace_name, a.normal_idx_flag, b.column_name "
"from pct_enc_index a, pct_enc_column b, pct_enc_table c, pct_encrypt_key d "
"where a.enc_col_id = b.enc_col_id "
"and   b.enc_tab_id = c.enc_tab_id "
...
PccOraScriptBuilder::PccOraScriptBuilder(DgcDatabase* db, DgcSession* sess,dgt_schar* schema_link)
	: PccScriptBuilder(db,sess,schema_link), 
	  ...
	  IdxSqlRows2(1), IdxSqlRows3(1), IdxSqlRows4(1),IdxSqlRows5(1), IdxSqlRows6(1), 
...
dgt_sint32 PccOraScriptBuilder::getTablespace(DgcMemRows* rtn_rows) throw(DgcExcept)
{
	if (!getConnection()) {
		ATHROWnR(DgcError(SPOS,"getConnection failed."),-1);
	}
...
dgt_sint32 PccOraScriptBuilder::buildScript(dgt_sint64 enc_tab_id,dgt_uint16 version_no) throw(DgcExcept)
{
	//
	// enc_type = 0 (view)
        // enc_type = 1 (non view)
...
dgt_sint32 PccOraScriptBuilder::buildScript2(dgt_sint64 enc_tab_id,dgt_uint16 version_no) throw(DgcExcept)
{
        //
        // enc_type = 0 (view)
        // enc_type = 1 (non view)
...
dgt_sint32 PccOraScriptBuilder::buildScriptMig(dgt_sint64 enc_tab_id,dgt_uint16 version_no) throw(DgcExcept)
{
        VersionNo=version_no;
        if (prepareTabInfo(enc_tab_id) < 0) ATHROWnR(DgcError(SPOS,"prepareTabInfo failed."),-1);
        if (prepareColInfo() < 0) ATHROWnR(DgcError(SPOS,"prepareColInfo failed."),-1);
...
dgt_sint32 PccOraScriptBuilder::runVerifyMig(dgt_sint64 enc_tab_id, pct_type_verify_job* job_row_ptr) throw(DgcExcept)
{
        if (prepareTabInfo(enc_tab_id) < 0) ATHROWnR(DgcError(SPOS,"prepareTabInfo failed."),-1);
        if (prepareColInfo() < 0) ATHROWnR(DgcError(SPOS,"prepareColInfo failed."),-1);
	if (prepareIdxInfo() < 0) ATHROWnR(DgcError(SPOS,"prepareIdxInfo failed."),-1);
...
dgt_sint32 PccOraScriptBuilder::migInsertSql(dgt_sint64 it_tab_id, dgt_uint8 gen_flag) throw(DgcExcept)
{
	//
                                ...
                                if (seq == 1) {
                                        ...
                                        sprintf(insert_part,"INSERT /*+ APPEND PARALLEL(%s,%d) */ INTO %s.%s \n( %s,",
										       ...
										       PetraNamePool->getNameString(insert_st_tmp->schema_name),
                                                                                       ...
                                                                                       PetraNamePool->getNameString(insert_st_tmp->column_name));
					if (insert_st_tmp->key_id > 0) {
		                        	sprintf(insert_select_part,"\nSELECT \n %s,",getFname(PetraNamePool->getNameString(insert_st_tmp->column_name),1));
					} else {
	                                       	sprintf(insert_select_part,"\nSELECT \n %s,",
								PetraNamePool->getNameString(insert_st_tmp->column_name));
					}
...
dgt_sint32 PccOraScriptBuilder::buildScriptAddCol(dgt_sint64 enc_tab_id,dgt_uint16 version_no) throw(DgcExcept)
{
        VersionNo=version_no;
        if (prepareTabInfo(enc_tab_id) < 0) ATHROWnR(DgcError(SPOS,"prepareTabInfo failed."),-1);
        if (prepareColInfo() < 0) ATHROWnR(DgcError(SPOS,"prepareColInfo failed."),-1);
...
dgt_sint32 PccOraScriptBuilder::buildScriptColAdmin(dgt_sint64 enc_tab_id,dgt_uint16 version_no) throw(DgcExcept)
{
        VersionNo=version_no;
        if (prepareTabInfo(enc_tab_id) < 0) ATHROWnR(DgcError(SPOS,"prepareTabInfo failed."),-1);
        if (prepareColInfo() < 0) ATHROWnR(DgcError(SPOS,"prepareColInfo failed."),-1);
...
dgt_sint32 PccOraScriptBuilder::checkDB(dgt_sint64 db_agent_id,dgt_schar* sys_uid,dgt_schar* sys_pass,dgt_schar* agent_uid,DgcMemRows* rtn_rows) 
throw(DgcExcept)
{
        //
...
dgt_sint32 PccOraScriptBuilder::setCharset(dgt_sint64 db_agent_id) throw(DgcExcept)
{
        if (!getConnection()) {
                ATHROWnR(DgcError(SPOS,"getConnection failed."),-1);
        }
        dgt_schar       sql_text[256];
        sprintf(sql_text,
                "select value$ from sys.props$ where name = 'NLS_LANGUAGE' or name = 'NLS_TERRITORY' or name = 'NLS_CHARACTERSET'");
        DgcCliStmt*     stmt=Connection->getStmt();
        if (!stmt) {
		Connection->disconnect();
                ATHROWnR(DgcError(SPOS,"getStmt failed."),-1);
	...
	*(nls_char+strlen(nls_char)-1)=0;
        ...
        sprintf(sql_text,"update pct_db_agent set(nls_lang,last_update)=('%s',nextLastUpdate('PCT_DB_AGENT', %lld, 2)) where db_agent_id=%lld",nls_char,db_agent_id,db_agent_id);
        DgcSqlStmt*     sql_stmt=Database->getStmt(Session,sql_text,strlen(sql_text));
        if (sql_stmt == 0 || sql_stmt->execute() < 0) {
                DgcExcept*      e=EXCEPTnC;
                delete sql_stmt;
...
dgt_sint32 PccOraScriptBuilder::agentTest(dgt_sint64 db_agent_id,DgcMemRows* rtn_rows) throw(DgcExcept)
{
...
dgt_sint32 PccOraScriptBuilder::agentTableTest(dgt_sint64 db_agent_id,DgcMemRows* rtn_rows) throw(DgcExcept)
{
	...
	if (cpu_count > 1) {
		memset(sql_text,0,1024);
		sprintf(sql_text,"CREATE TABLE BASE_TABLE "
				 "(	DATA1 NUMBER,"
  				       "DATA2 VARCHAR(15)"
				 ")");
		stmt=Connection->getStmt();
		...
		memset(sql_text,0,1024);
		sprintf(sql_text,"INSERT INTO BASE_TABLE "
				"SELECT LEVEL "
		...
		memset(sql_text,0,1024);
        	sprintf(sql_text,"CREATE TABLE PARALLEL_TABLE "
				 "(  DATA1 VARCHAR(30), "
				 "   DATA2 VARCHAR(30) )");
		stmt=Connection->getStmt();
	        if (stmt->execute(sql_text,strlen(sql_text),10) < 0) {
		...
		delete stmt;
		memset(sql_text,0,1024);
                sprintf(sql_text,"insert /*+ APPEND PARALLEL(PARALLEL_TABLE,%d) */ into PARALLEL_TABLE "
				 "SELECT /*+ FULL(BASE_TABLE) PARALLEL(BASE_TABLE,%d) */ "
				 "PLS_ENCRYPT_B64_ID(DATA1,1) , "
				 "PLS_ENCRYPT_B64_ID(DATA2,1) "
				 "FROM BASE_TABLE",cpu_count, cpu_count);
		stmt=Connection->getStmt();
                if (stmt->execute(sql_text,strlen(sql_text),10) < 0) {
                        DgcExcept*      e=EXCEPTnC;
                	if (e) {
		...
		if (currTime.tv_sec == startTime.tv_sec) {
                	...
                	lapse_time = (dgt_sint64)(currTime.tv_sec - startTime.tv_sec)*1000000 + currTime.tv_usec - startTime.tv_usec;
                }
		cmp_time=lapse_time;
		...
		gettimeofday(&startTime,0);
                ...
                sprintf(sql_text,"insert /*+ APPEND PARALLEL(PARALLEL_TABLE,%d) */  into PARALLEL_TABLE "
                                 ...
                                 "PLS_ENCRYPT_B64_ID(DATA1,1) , "
                                 "PLS_ENCRYPT_B64_ID(DATA2,1) "
                                 "FROM BASE_TABLE",cpu_count/2, cpu_count/2);
		stmt=Connection->getStmt();
                if (stmt->execute(sql_text,strlen(sql_text),10) < 0) {
                        DgcExcept*      e=EXCEPTnC;
                        if (e) {
			...
			out_param.parallel_degree=cpu_count/2;
                ...
                sprintf(sql_text,"insert /*+ APPEND PARALLEL(PARALLEL_TABLE,%d) */ into PARALLEL_TABLE "
                                 ...
                                 "PLS_ENCRYPT_B64_ID(DATA1,1) , "
                                 "PLS_ENCRYPT_B64_ID(DATA2,1) "
                                 "FROM BASE_TABLE", (dgt_sint32)cpu_count/2 + (dgt_sint32)ceil(cpu_count/4),
		...
		stmt=Connection->getStmt();
			...
			Connection->disconnect();
                ...
                if (cmp_time > lapse_time) {
			cmp_time=lapse_time;
			out_param.parallel_degree=(dgt_sint16)cpu_count/2 + (dgt_sint16)ceil(cpu_count/4);
			if (out_param.parallel_degree > 2) out_param.parallel_degree = out_param.parallel_degree - 1;
		}
		...
		memset(sql_text,0,256);
        	sprintf(sql_text,"update pct_db_agent set(parallel_degree,last_update)=(%d,nextLastUpdate('PCT_DB_AGENT', %lld, 2)) where db_agent_id=%lld",out_param.parallel_degree,db_agent_id,db_agent_id);
	        DgcSqlStmt*     sql_stmt=Database->getStmt(Session,sql_text,strlen(sql_text));
        	if (sql_stmt == 0 || sql_stmt->execute() < 0) {
                	DgcExcept*      e=EXCEPTnC;
	                delete sql_stmt;
	...
	memset(sql_text,0,1024);
        sprintf(sql_text,"CREATE TABLE DATA_TYPE_TABLE "
	...
	memset(sql_text,0,1024);
        sprintf(sql_text,"INSERT INTO DATA_TYPE_TABLE "
	...
	memset(sql_text,0,1024);
        sprintf(sql_text,"SELECT COUNT(*) "
			 ...
			 "OR    dbms_lob.compare(A.LOB_TYPE,PLS_DECRYPT_CLOB(B.LOB_TYPE,1)) != 0 "
	...
	//
	memset(sql_text,0,1024);
        sprintf(sql_text,"CREATE TABLE ALGO_TABLE "
			 "(  AES_COL VARCHAR2(30) "
	...
	memset(sql_text,0,1024);
        sprintf(sql_text,"INSERT INTO ALGO_TABLE "
	...
	//
	// update the current inst_step = 6(sample table test success) in pct_db_agent table
	//
        memset(sql_text,0,256);
        sprintf(sql_text,"update pct_db_agent set(inst_step,last_update)=(6,nextLastUpdate('PCT_DB_AGENT', %lld, 2)) where db_agent_id=%lld",db_agent_id,db_agent_id);
        DgcSqlStmt*     sql_stmt=Database->getStmt(Session,sql_text,strlen(sql_text));
        if (sql_stmt == 0 || sql_stmt->execute() < 0) {
        	DgcExcept*      e=EXCEPTnC;
                delete sql_stmt;
...
dgt_sint32 PccOraScriptBuilder::buildInstallScript(dgt_sint64 agent_id,dgt_schar* agent_uid,dgt_schar* agent_pass,dgt_schar* soha_home) throw(DgcExcept)
{
	...
	*TextBuf=0;
	sprintf(TextBuf,
		...
		"    v_seq number;\n"
		"begin\n"
		"    select seq into v_seq from dba_java_policy where GRANTEE = 'PUBLIC' and TYPE_NAME like '%soracle.aurora.rdbms.security.PolicyTablePermission%s';\n"
		"    dbms_java.disable_permission(v_seq);\n"
		"    dbms_java.grant_permission( upper('%s'), 'SYS:java.lang.RuntimePermission', 'loadLibrary.PcaOracle', '' );\n"
	...
	if (is.os_type != 7) {
		...
		if (!strcasecmp(is.db_version,"8i")) {
		        sprintf(TextBuf,"CREATE OR REPLACE DIRECTORY petra_dir AS '%s/lib/cipher/oracle_class/8i/unix'",soha_home);
		        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		} else {
			sprintf(TextBuf,"CREATE OR REPLACE DIRECTORY petra_dir AS '%s/lib/cipher/oracle_class/9i_above/unix'",soha_home);
                        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		}
	} else {
		//
...
#if 1
		if (!strcasecmp(is.db_version,"8i")) {
                        sprintf(TextBuf,"CREATE OR REPLACE DIRECTORY petra_dir AS 'C:\\Program Files\\SINSIWAY\\Petra\\api'");
                        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                } else {
                        sprintf(TextBuf,"CREATE OR REPLACE DIRECTORY petra_dir AS 'C:\\Program Files\\SINSIWAY\\Petra\\api'");
                        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	...
	*TextBuf=0;
        sprintf(TextBuf,"CREATE OR REPLACE JAVA CLASS USING BFILE (petra_dir , 'PcaOracle.class')");
 	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	...
	*TextBuf=0;
	sprintf(TextBuf,
		...
		"RETURN RAW parallel_enable deterministic IS "
		"BEGIN "
		"RETURN PLS_ENCRYPT_RAW_ID(TO_CHAR(DATA,'YYYYMMDDHH24MISS'),ENC_COL_ID);"
		"END;");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0;
	sprintf(TextBuf,
	...
	*TextBuf=0;
		...
		"BEGIN "
		"RETURN TO_DATE(PLS_DECRYPT_RAW_ID(DATA,ENC_COL_ID),'YYYYMMDDHH24MISS');"
		"END;");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0;
        sprintf(TextBuf,
	...
	*TextBuf=0;
        ...
        *TextBuf=0;
		...
		"RETURN VARCHAR2 parallel_enable deterministic IS "
		"BEGIN "
		"RETURN PLS_ENCRYPT_B64_ID(TO_CHAR(DATA,'YYYYMMDDHH24MISS'),ENC_COL_ID);"
		"END;");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        *TextBuf=0;
        sprintf(TextBuf,
	...
	*TextBuf=0;
		...
		"RETURN DATE parallel_enable deterministic IS "
		"BEGIN "
		"RETURN TO_DATE(PLS_DECRYPT_B64_ID(DATA,ENC_COL_ID),'YYYYMMDDHH24MISS');"
		"END;");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0;
        sprintf(TextBuf,
	...
	*TextBuf=0;
		...
		"LANGUAGE JAVA " 
		"NAME 'PcaOracle.EXT_ENC_CLOB(oracle.sql.BLOB,int) return oracle.sql.CLOB';");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0;
        sprintf(TextBuf,
                ...
                "RETURN CLOB parallel_enable IS "
                "LANGUAGE JAVA "
                "NAME 'PcaOracle.EXT_ENC_CLOB(oracle.sql.BLOB,java.lang.String) return oracle.sql.CLOB';");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0;
        sprintf(TextBuf,
		"CREATE OR REPLACE FUNCTION PLS_ENCRYPT_CLOB(DATA IN CLOB,ENC_COL_ID NUMBER) "
		...
		" IF dbms_lob.compare(nvl(DATA,'==null=='),v_nullcheck) = 0  then "
    		...
    		" DBMS_LOB.ConvertToBlob(v_blob, DATA, DBMS_LOB.GetLength(DATA), v_dstPos, v_srcPos, DBMS_LOB.default_csid, v_langContext, v_warning ); "
    		" return PLS_ENCRYPT_CLOB_M(v_blob, ENC_COL_ID); "
		" END;");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0;
        sprintf(TextBuf,
                "CREATE OR REPLACE FUNCTION PLS_ENCRYPT_CLOB_NM(DATA IN CLOB,ECN VARCHAR2) "
                "RETURN CLOB parallel_enable deterministic IS "
                " v_blob blob; "
                " v_srcPos number := 1; "
                " v_dstPos number := 1; "
	...
	*TextBuf=0;
		...
		"RETURN CLOB parallel_enable IS "
		"LANGUAGE JAVA "
		"NAME 'PcaOracle.EXT_DEC_CLOB(oracle.sql.CLOB,int) return oracle.sql.CLOB';");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0;
        sprintf(TextBuf,
                ...
                "RETURN CLOB parallel_enable IS "
                "LANGUAGE JAVA "
                "NAME 'PcaOracle.EXT_DEC_CLOB(oracle.sql.CLOB,java.lang.String) return oracle.sql.CLOB';");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0;
        sprintf(TextBuf,
		...
		"LANGUAGE JAVA "
		"NAME 'PcaOracle.EXT_ENC_BLOB(oracle.sql.BLOB,int) return oracle.sql.BLOB';");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0;
        sprintf(TextBuf,
                ...
                "RETURN BLOB parallel_enable IS "
                "LANGUAGE JAVA "
                "NAME 'PcaOracle.EXT_ENC_BLOB(oracle.sql.BLOB,java.lang.String) return oracle.sql.BLOB';");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0;
        sprintf(TextBuf,
		...
		"RETURN BLOB parallel_enable IS "
		"LANGUAGE JAVA "
		"NAME 'PcaOracle.EXT_DEC_BLOB(oracle.sql.BLOB,int) return oracle.sql.BLOB';");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0;
        sprintf(TextBuf,
                ...
                "RETURN BLOB parallel_enable IS "
                "LANGUAGE JAVA "
                "NAME 'PcaOracle.EXT_DEC_BLOB(oracle.sql.BLOB,java.lang.String) return oracle.sql.BLOB';");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0;
        sprintf(TextBuf,
	...
	StepNo=5;
	StmtNo=0;
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_ENCRYPT_RAW TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_ENCRYPT_RAW_NM TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_ENCRYPT_RAW_ID TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_ENCRYPT_RAW_ID_RAW TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_ENCRYPT_RAW_ID_DATE TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_DECRYPT_RAW TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_DECRYPT_RAW_NM TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_DECRYPT_RAW_ID TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_DECRYPT_RAW_ID_RAW TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_DECRYPT_RAW_ID_DATE TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_DECRYPT_RAW_ID_NUM TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_ENCRYPT_B64 TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_ENCRYPT_B64_NM TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_ENCRYPT_B64_ID TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_ENCRYPT_B64_ID_C TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_ENCRYPT_B64_ID_RAW TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_ENCRYPT_B64_ID_DATE TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_DECRYPT_B64 TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_DECRYPT_B64_NM TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_DECRYPT_B64_ID TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_DECRYPT_B64_ID_RAW TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_DECRYPT_B64_ID_DATE TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_DECRYPT_B64_ID_NUM TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_ENCRYPT_CLOB TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_ENCRYPT_CLOB_M TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_ENCRYPT_BLOB TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_DECRYPT_CLOB TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_DECRYPT_BLOB TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_ENCRYPT_CPN_ID TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"GRANT EXECUTE ON PLS_DECRYPT_CPN_ID TO PUBLIC WITH GRANT OPTION");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	...
	*TextBuf=0; sprintf(TextBuf,"create sequence tr_table_id increment by 1 start with 1 cache 10000");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	//
	...
	StmtNo=0;
	if (!strcasecmp(is.db_version,"8i")) {
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_ENCRYPT_RAW FOR %s.PLS_ENCRYPT_RAW",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_ENCRYPT_RAW_NM FOR %s.PLS_ENCRYPT_RAW_NM",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_ENCRYPT_RAW_ID FOR %s.PLS_ENCRYPT_RAW_ID",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_ENCRYPT_RAW_ID_RAW FOR %s.PLS_ENCRYPT_RAW_ID_RAW",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_ENCRYPT_RAW_ID_DATE FOR %s.PLS_ENCRYPT_RAW_ID_DATE",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_DECRYPT_RAW FOR %s.PLS_DECRYPT_RAW",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_DECRYPT_RAW_NM FOR %s.PLS_DECRYPT_RAW_NM",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_DECRYPT_RAW_ID FOR %s.PLS_DECRYPT_RAW_ID",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_DECRYPT_RAW_ID_RAW FOR %s.PLS_DECRYPT_RAW_ID_RAW",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_DECRYPT_RAW_ID_DATE FOR %s.PLS_DECRYPT_RAW_ID_DATE",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_DECRYPT_RAW_ID_NUM FOR %s.PLS_DECRYPT_RAW_ID_NUM",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_ENCRYPT_B64 FOR %s.PLS_ENCRYPT_B64",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_ENCRYPT_B64_NM FOR %s.PLS_ENCRYPT_B64_NM",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_ENCRYPT_B64_ID FOR %s.PLS_ENCRYPT_B64_ID",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_ENCRYPT_B64_ID_C FOR %s.PLS_ENCRYPT_B64_ID_C",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_ENCRYPT_B64_ID_N FOR %s.PLS_ENCRYPT_B64_ID_N",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_ENCRYPT_B64_ID_RAW FOR %s.PLS_ENCRYPT_B64_ID_RAW",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_ENCRYPT_B64_ID_DATE FOR %s.PLS_ENCRYPT_B64_ID_DATE",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_DECRYPT_B64 FOR %s.PLS_DECRYPT_B64",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_DECRYPT_B64_NM FOR %s.PLS_DECRYPT_B64_NM",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_DECRYPT_B64_ID FOR %s.PLS_DECRYPT_B64_ID",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_DECRYPT_B64_ID_RAW FOR %s.PLS_DECRYPT_B64_ID_RAW",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_DECRYPT_B64_ID_DATE FOR %s.PLS_DECRYPT_B64_ID_DATE",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_DECRYPT_B64_ID_NUM FOR %s.PLS_DECRYPT_B64_ID_NUM",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_ENCRYPT_CLOB FOR %s.PLS_ENCRYPT_CLOB",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_ENCRYPT_CLOB_M FOR %s.PLS_ENCRYPT_CLOB_M",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_ENCRYPT_BLOB FOR %s.PLS_ENCRYPT_BLOB",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_DECRYPT_CLOB FOR %s.PLS_DECRYPT_CLOB",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_DECRYPT_BLOB FOR %s.PLS_DECRYPT_BLOB",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_LCR FOR %s.PLS_LCR",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_ENCRYPT_CPN_ID FOR %s.PLS_ENCRYPT_CPN_ID",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE PUBLIC SYNONYM PLS_DECRYPT_CPN_ID FOR %s.PLS_DECRYPT_CPN_ID",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	} else {
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_ENCRYPT_RAW FOR %s.PLS_ENCRYPT_RAW",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_ENCRYPT_RAW_NM FOR %s.PLS_ENCRYPT_RAW_NM",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_ENCRYPT_RAW_ID FOR %s.PLS_ENCRYPT_RAW_ID",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_ENCRYPT_RAW_ID_RAW FOR %s.PLS_ENCRYPT_RAW_ID_RAW",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_ENCRYPT_RAW_ID_DATE FOR %s.PLS_ENCRYPT_RAW_ID_DATE",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_DECRYPT_RAW FOR %s.PLS_DECRYPT_RAW",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_DECRYPT_RAW_NM FOR %s.PLS_DECRYPT_RAW_NM",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_DECRYPT_RAW_ID FOR %s.PLS_DECRYPT_RAW_ID",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_DECRYPT_RAW_ID_RAW FOR %s.PLS_DECRYPT_RAW_ID_RAW",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_DECRYPT_RAW_ID_DATE FOR %s.PLS_DECRYPT_RAW_ID_DATE",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_DECRYPT_RAW_ID_NUM FOR %s.PLS_DECRYPT_RAW_ID_NUM",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_ENCRYPT_B64 FOR %s.PLS_ENCRYPT_B64",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_ENCRYPT_B64_NM FOR %s.PLS_ENCRYPT_B64_NM",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_ENCRYPT_B64_ID FOR %s.PLS_ENCRYPT_B64_ID",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_ENCRYPT_B64_ID_C FOR %s.PLS_ENCRYPT_B64_ID_C",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_ENCRYPT_B64_ID_N FOR %s.PLS_ENCRYPT_B64_ID_N",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_ENCRYPT_B64_ID_RAW FOR %s.PLS_ENCRYPT_B64_ID_RAW",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_ENCRYPT_B64_ID_DATE FOR %s.PLS_ENCRYPT_B64_ID_DATE",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_DECRYPT_B64 FOR %s.PLS_DECRYPT_B64",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_DECRYPT_B64_NM FOR %s.PLS_DECRYPT_B64_NM",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_DECRYPT_B64_ID FOR %s.PLS_DECRYPT_B64_ID",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_DECRYPT_B64_ID_RAW FOR %s.PLS_DECRYPT_B64_ID_RAW",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_DECRYPT_B64_ID_DATE FOR %s.PLS_DECRYPT_B64_ID_DATE",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_DECRYPT_B64_ID_NUM FOR %s.PLS_DECRYPT_B64_ID_NUM",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_ENCRYPT_CLOB FOR %s.PLS_ENCRYPT_CLOB",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_ENCRYPT_CLOB_M FOR %s.PLS_ENCRYPT_CLOB_M",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_ENCRYPT_BLOB FOR %s.PLS_ENCRYPT_BLOB",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_DECRYPT_CLOB FOR %s.PLS_DECRYPT_CLOB",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_DECRYPT_BLOB FOR %s.PLS_DECRYPT_BLOB",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_LCR FOR %s.PLS_LCR",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_ENCRYPT_CPN_ID FOR %s.PLS_ENCRYPT_CPN_ID",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_DECRYPT_CPN_ID FOR %s.PLS_DECRYPT_CPN_ID",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	}
	...
	if (strcasecmp(is.db_version,"8i")) {
		*TextBuf=0; 
		sprintf(TextBuf,"CREATE OR REPLACE FUNCTION PLS_OPHUEK_RAW "
		...
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0;
                sprintf(TextBuf,"CREATE OR REPLACE FUNCTION PLS_OPHUEK_B64 "
                                "(DATA IN  VARCHAR2 , ENC_COL_ID IN NUMBER, SRC_ENC_FLAG IN NUMBER) "
                                "RETURN VARCHAR2 parallel_enable deterministic IS "
                                "LANGUAGE JAVA "
                                "NAME 'PcaOracle.EXT_OPHUEK(byte[],int,int) return byte[]';");
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0;
                sprintf(TextBuf,"GRANT EXECUTE ON PLS_OPHUEK_RAW TO PUBLIC WITH GRANT OPTION");
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0;
                sprintf(TextBuf,"GRANT EXECUTE ON PLS_OPHUEK_B64 TO PUBLIC WITH GRANT OPTION");
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0;
                sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_OPHUEK_RAW FOR %s.PLS_OPHUEK_RAW",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0;
                sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_OPHUEK_B64 FOR %s.PLS_OPHUEK_B64",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0;
                sprintf(TextBuf,"create operator pls_idx1_str "
                                "binding (varchar2,number) "
                                "return varchar2 using pls_decrypt_b64_id");
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0;
                sprintf(TextBuf,"create operator pls_idx1_num "
  				"binding (varchar2,number) "
  				"return number using pls_decrypt_b64_id_num");
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0;
                sprintf(TextBuf,"create operator pls_idx1_date "
  				"binding (varchar2,number) "
  				"return date using pls_decrypt_b64_id_date");
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0;
                sprintf(TextBuf,"grant execute on pls_idx1_str to public with grant option");
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0;
                sprintf(TextBuf,"grant execute on pls_idx1_num to public with grant option");
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0;
                sprintf(TextBuf,"grant execute on pls_idx1_date to public with grant option");
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
#if 1
                *TextBuf=0;
                sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM pls_idx1_str FOR %s.pls_idx1_str",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
#else
                *TextBuf=0;
                sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM pls_idx1_num FOR %s.pls_idx1_num",agent_uid);
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0;
                sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM pls_idx1_date FOR %s.pls_idx1_date",agent_uid);
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0;
#endif
                sprintf(TextBuf,"create or replace package DgCipherIndex$Const \n"
				"is \n"
		...
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0;
                sprintf(TextBuf,"grant execute on DgCipherIndex$Const to public with grant option");
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0;
                sprintf(TextBuf,"create or replace type DgCipherIndex$LikeExprInfo as object\n"
				"(\n"
		...
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0;
                sprintf(TextBuf,"create or replace type body DgCipherIndex$LikeExprInfo\n"
				"is\n"
  				"constructor function DgCipherIndex$LikeExprInfo\n"
      				"(expr varchar2)\n"
    				"return self as result\n"
		...
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0;
                sprintf(TextBuf,"create or replace type DgCipherIndex$QueryInfo as object\n"
				"(\n"
  				"cursorNo        number,\n"
  				"howMany         number,\n"
  				"colType         number,\n"
		...
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0;
                sprintf(TextBuf,"create or replace type body DgCipherIndex$QueryInfo\n"
				"is\n"
				"constructor function DgCipherIndex$QueryInfo\n"
    				"return self as result\n"
  				"is\n"
				...
				"self.operatorList(1) := '<=';\n"
   				...
   				"self.operatorList(1) := '>';\n"
    				...
    				"end if;\n"
    				"for i in self.bindValueList.First .. self.bindValueList.Last loop\n"
		...
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0;
                sprintf(TextBuf,"create or replace type DgCipherIndex$PrimaryIndex as object\n"
				"(\n"
		...
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0;
                sprintf(TextBuf,"create or replace type body DgCipherIndex$PrimaryIndex\n"
"is\nconstructor function DgCipherIndex$PrimaryIndex\n(ia  sys.ODCIIndexInfo,env sys.ODCIEnv)\n"
"return self as result\n"
"is\nbegin\n"
"self.indexInfo := ia;\nreturn;\nend DgCipherIndex$PrimaryIndex;\n"
"member function startIndex(self in out DgCipherIndex$PrimaryIndex,cipherId number,strt varchar2,stop varchar2) return number\n"
...
"exception when OTHERS then dbms_sql.close_cursor(cnum); return DgCipherIndex$Const.Failed; end startIndex;\n"
"member function fetchIndex(self IN OUT DgCipherIndex$PrimaryIndex,nrows number,rids OUT sys.ODCIRidList)\n"
"return number\nis\n"
"i number; cnum integer;fetched number;\n"
"rlist   sys.ODCIRidList := sys.ODCIRidList();\n"
"rid_tab dbms_sql.varchar2_table;\n"
"begin\n cnum := self.queryInfo.CursorNo;\n if self.queryInfo.HowMany = 0 then\n"
...
"else\n rlist.extend(fetched + 1);\n end if;\n dbms_sql.column_value(cnum, 1, rid_tab);\n for i in 1..fetched loop\n"
"rlist(i) := rid_tab(i);\n end loop;\n self.queryInfo.HowMany := self.queryInfo.HowMany + fetched;\n rids := rlist;\n"
"return DgCipherIndex$Const.Success;\n end fetchIndex;\n"
"member function closeIndex(self IN OUT DgCipherIndex$PrimaryIndex) return number\n"
"is\n begin\n if self.queryInfo.cursorNo != 0 then\n dbms_sql.close_cursor(self.queryInfo.cursorNo);\n"
"end if;\n return DgCipherIndex$Const.Success;\n end closeIndex;\n"
"member procedure initQuery(self IN OUT DgCipherIndex$PrimaryIndex,pr   sys.ODCIPredInfo,qi   sys.ODCIQueryInfo,\n"
"strt varchar2,stop varchar2)\nis\nbegin\n"
"self.queryInfo := DgCipherIndex$QueryInfo(pr, strt, stop);\n end initQuery;\n"
"member function getQueryStatement (self in out DgCipherIndex$PrimaryIndex, cipherId number) return varchar2\n"
"is\nstmt varchar2(4000);\nwhereClause varchar2(4000);\n opList sys.ODCIVarchar2List;\n"
"phList      sys.ODCIVarchar2List; indexedColName varchar2(4000);  begin\n"
"opList := self.queryInfo.operatorList; phList := self.queryInfo.placeHolderList;\n"
"declare func varchar2(4000); begin indexedColName := self.indexInfo.indexCols(1).colname;\n"
"whereClause := 'where ' || indexedColName || ' ';\n if opList(1) = 'between' then\n whereClause := whereClause || "
"opList(1) || ' ' || 'PLS_ENCRYPT_B64_ID(' || phList(1) || ',' || cipherId || ') ' || opList(2) || ' ' || "
"'PLS_ENCRYPT_B64_ID(' || phList(2) || ',' || cipherId || ')'; elsif opList(1) = 'like' then if (self.queryInfo.likeExprInfo.allMarkPos.count > 0 and "
"self.queryInfo.likeExprInfo.allMarkPos(1) != 1) then declare bindStr varchar2(4000); bindStr2 varchar2(4000); lastChr varchar2(10); "
"strLen  number; whileCount number; begin bindStr := replace(self.queryInfo.bindValueList(1),'%%',''); strLen := length(bindStr); "
"lastChr := substr(bindStr, strLen, 1); if strLen = 1 then bindStr2 := chr(ascii(lastChr) + 1); else "
"bindStr2 := substr(bindStr, 1, strLen - 1) || chr(ascii(lastChr) + 1); end if; "
"self.queryInfo.bindValueList.extend(1); self.queryInfo.bindValueList(1) := bindStr; self.queryInfo.bindValueList(2) := bindStr2; "
"whereClause := 'where ' || indexedColName || ' >= PLS_ENCRYPT_B64_ID(:v1,' || cipherId || ')' || ' and ' || indexedColName || "
"' < PLS_ENCRYPT_B64_ID(:v2,' || cipherId || ')'; end; elsif (self.queryInfo.likeExprInfo.singleMarkPos.count > 0 and "
"self.queryInfo.likeExprInfo.singleMarkPos(1) != 1) then declare bindStr varchar2(4000); bindStr2 varchar2(4000); "
"lastChr varchar2(10); strLen  number; blanktmp varchar2(100); whileCount number; begin bindStr := replace(self.queryInfo.bindValueList(1),'_',' '); "
"bindStr2 := replace(self.queryInfo.bindValueList(1),'_','');  strLen := length(bindStr2); lastChr := substr(bindStr2, strLen, 1); "
"if strLen = 1 then bindStr2 := chr(ascii(lastChr) + 1); else bindStr2 := substr(bindStr2, 1, strLen - 1) || chr(ascii(lastChr) + 1); "
"end if; for i in 1..self.queryInfo.likeExprInfo.singleMarkPos.count loop bindStr2 := bindStr2 || ' '; "
"end loop; self.queryInfo.bindValueList.extend(1); self.queryInfo.bindValueList(1) := bindStr; self.queryInfo.bindValueList(2) := bindStr2; "
"whereClause := 'where ' || indexedColName || ' >= PLS_ENCRYPT_B64_ID(:v1,' || cipherId || ')' || ' and ' || indexedColName || "
"' < PLS_ENCRYPT_B64_ID(:v2,' || cipherId || ')'; end;\n "
"else whereClause := whereClause || opList(1) || ' ' || 'PLS_ENCRYPT_B64_ID(' || phList(1) || ',' || cipherId || ')';\n "
"end if; elsif opList.last = 1 then if opList(1) = '=' then whereClause := whereClause || "
"opList(1) || ' ' || 'PLS_ENCRYPT_B64_ID(' || phList(1) || ',' || cipherId || ')'; else whereClause := whereClause || opList(1) || ' ' || "
"'PLS_ENCRYPT_B64_ID(' || phList(1) || ',' || cipherId || ')'; end if; else whereClause := whereClause || opList(1) || ' ' || "
"'PLS_ENCRYPT_B64_ID(' || phList(1) || ',' || cipherId || ')' || ' and ' || indexedColName || ' ' || opList(2) || ' ' || 'PLS_ENCRYPT_B64_ID(' || phList(2) || ',' || cipherId || ')'; "
"end if; end; stmt := 'select /*+ first_rows index(a) */ rowid,a.* ' || 'from ' || self.indexInfo.indexCols(1).tableSchema || '.' ||  self.indexInfo.indexCols(1).tableName || ' a ' || "
"whereClause ; return stmt; end getQueryStatement; end;");
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0;
                sprintf(TextBuf,"create or replace type pc_index_impl as object\n"
				"(primaryIndex   DgCipherIndex$PrimaryIndex, static function ODCIGetInterfaces(ifclist OUT sys.ODCIObjectList)\n"
    				"return number,\n"
		...
		*TextBuf=0;
		sprintf(TextBuf,
"create or replace type body pc_index_impl is static function ODCIGetInterfaces(ifclist OUT sys.ODCIObjectList)\n"
"return number is begin ifclist := sys.ODCIObjectList(sys.ODCIObject('SYS','ODCIINDEX2')); return ODCIConst.Success; end ODCIGetInterfaces;\n"
"static function ODCIIndexCreate(ia       sys.ODCIIndexInfo,params   varchar2,env      sys.ODCIEnv) return number is E_INDEX_EXIST_OBJ exception;\n"
"pragma exception_init(E_INDEX_EXIST_OBJ, -955); begin begin return ODCIConst.Success; exception when E_INDEX_EXIST_OBJ then NULL;\n"
"when OTHERS then raise_application_error(-20900,'Error creating base index ' || ' of the domain index' || chr(10) || dbms_utility.format_error_stack);\n"
"end; end ODCIIndexCreate; static function ODCIIndexDrop(ia  sys.ODCIIndexInfo,env sys.ODCIEnv) return number is E_INDEX_NOT_EXIST exception;\n"
"E_TABLE_NOT_EXIST exception; pragma exception_init(E_INDEX_NOT_EXIST, -01418); pragma exception_init(E_TABLE_NOT_EXIST, -00942); begin\n"
"return ODCIConst.Success; exception when E_TABLE_NOT_EXIST then NULL; when OTHERS then raise_application_error(-20901,'Error dropping base index ' ||\n"
"' of the domain index' || chr(10) || dbms_utility.format_error_stack); end ODCIIndexDrop;\n"
"static function ODCIIndexAlter(ia sys.ODCIIndexInfo, parms IN OUT varchar2, alter_option number, env sys.ODCIEnv) return number is begin\n"
"return ODCIConst.Success; end ODCIIndexAlter; static function ODCIIndexTruncate(ia  sys.ODCIIndexInfo,env sys.ODCIEnv) return number\n"
"is begin return ODCIConst.Success; end ODCIIndexTruncate; static function ODCIIndexInsert(ia     sys.ODCIIndexInfo,rid    varchar2,\n"
"newval varchar2, env    sys.ODCIEnv) return number is begin return 0; end ODCIIndexInsert;\n"
"static function ODCIIndexDelete(ia     sys.ODCIIndexInfo,rid    varchar2,oldval varchar2,env    sys.ODCIEnv) return number is begin return 0;\n"
"end ODCIIndexDelete; static function ODCIIndexUpdate(ia     sys.ODCIIndexInfo,rid    varchar2,oldval varchar2,newval varchar2,env    sys.ODCIEnv)\n"
"return number is begin return 0; end ODCIIndexUpdate;\n"
"static function ODCIIndexStart(sctx IN OUT pc_index_impl,ia sys.ODCIIndexInfo,pr sys.ODCIPredInfo,qi sys.ODCIQueryInfo,strt varchar2,\n"
"stop varchar2,cipherId number,env sys.ODCIEnv) return number is ret       number; primaryIndex  DgCipherIndex$PrimaryIndex; begin\n"
"primaryIndex := DgCipherIndex$PrimaryIndex(ia, env); primaryIndex.initQuery(pr, qi, strt, stop); ret := primaryIndex.startIndex(cipherId,strt,stop);\n"
"sctx := pc_index_impl(primaryIndex); return ODCIConst.Success; exception when OTHERS then raise; end ODCIIndexStart; member function ODCIIndexFetch(\n"
"self IN OUT pc_index_impl, nrows       number, rids OUT    sys.ODCIRidList, env         sys.ODCIEnv) return number\n"
"is ret     number; begin ret := self.primaryIndex.fetchIndex(nrows, rids); return ODCIConst.Success; end ODCIIndexFetch; member function ODCIIndexClose(\n"
"self IN OUT pc_index_impl, env sys.ODCIEnv) return number is ret number; begin ret := self.primaryIndex.closeIndex; return ODCIConst.Success;\n"
"end ODCIIndexClose; end;");
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0;
                sprintf(TextBuf,"create or replace indextype pc_idx1_typ1 "
				"for pls_idx1_str(varchar2,number) using pc_index_impl");
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0;
                sprintf(TextBuf,"create or replace indextype pc_idx1_typ2 "
                                "for pls_idx1_num(varchar2,number) using pc_index_impl");
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0;
                sprintf(TextBuf,"create or replace indextype pc_idx1_typ3 "
                                "for pls_idx1_date(varchar2,number) using pc_index_impl");
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0;
                sprintf(TextBuf,"grant execute on pc_idx1_typ1 to public with grant option");
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0;
                sprintf(TextBuf,"grant execute on pc_idx1_typ2 to public with grant option");
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		*TextBuf=0;
                sprintf(TextBuf,"grant execute on pc_idx1_typ3 to public with grant option");
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		...
		//
                *TextBuf=0;
                sprintf(TextBuf,"create or replace type DgCipherIndex$PrimaryIndex2 as object\n"
                                "(\n"
                                "queryInfo DgCipherIndex$QueryInfo,\n"
                                "indexInfo sys.ODCIIndexInfo,\n"
                ...
                *TextBuf=0;
                sprintf(TextBuf,"create or replace type body DgCipherIndex$PrimaryIndex2\n"
"is\nconstructor function DgCipherIndex$PrimaryIndex2\n(ia  sys.ODCIIndexInfo,env sys.ODCIEnv)\n"
"return self as result\n"
"is\nbegin\n"
"self.indexInfo := ia;\nreturn;\nend DgCipherIndex$PrimaryIndex2;\n"
"member function startIndex(self in out DgCipherIndex$PrimaryIndex2,cipherId number,strt varchar2,stop varchar2) return number\n"
"is\ni number; cnum integer; stmt varchar2(4000);\n"
"bvList sys.ODCIVarchar2List;\nbegin\n"
"stmt := self.getQueryStatement(cipherId);\n"
"cnum := dbms_sql.open_cursor;\n"
...
"return DgCipherIndex$Const.Success;\n"
"exception when OTHERS then dbms_sql.close_cursor(cnum); return DgCipherIndex$Const.Failed; end startIndex;\n"
"member function fetchIndex(self IN OUT DgCipherIndex$PrimaryIndex2,nrows number,rids OUT sys.ODCIRidList)\n"
"return number\nis\n"
"i number; cnum integer;fetched number;\n"
"rlist   sys.ODCIRidList := sys.ODCIRidList();\n"
"rid_tab dbms_sql.varchar2_table;\n"
...
"rlist(i) := rid_tab(i);\n end loop;\n self.queryInfo.HowMany := self.queryInfo.HowMany + fetched;\n rids := rlist;\n"
"return DgCipherIndex$Const.Success;\n end fetchIndex;\n"
"member function closeIndex(self IN OUT DgCipherIndex$PrimaryIndex2) return number\n"
"is\n begin\n if self.queryInfo.cursorNo != 0 then\n dbms_sql.close_cursor(self.queryInfo.cursorNo);\n"
"end if;\n return DgCipherIndex$Const.Success;\n end closeIndex;\n"
"member procedure initQuery(self IN OUT DgCipherIndex$PrimaryIndex2,pr   sys.ODCIPredInfo,qi   sys.ODCIQueryInfo,\n"
"strt varchar2,stop varchar2)\nis\nbegin\n"
"self.queryInfo := DgCipherIndex$QueryInfo(pr, strt, stop);\n end initQuery;\n"
"member function getQueryStatement(self in out DgCipherIndex$PrimaryIndex2,cipherId number)\n"
"return varchar2\nis\nstmt        varchar2(4000);\nwhereClause varchar2(4000);\nopList      sys.ODCIVarchar2List;\n"
"phList sys.ODCIVarchar2List;\n indexedColName varchar2(4000);\n begin\n"
"opList := self.queryInfo.operatorList;\n phList := self.queryInfo.placeHolderList;\n"
"declare\n func varchar2(4000);\n begin\n indexedColName := self.indexInfo.indexCols(1).colname;\n"
"whereClause := ' where PLS_OPHUEK_B64(' || indexedColName || ',' || cipherId || ',1) ';\n"
"if opList(1) = 'between' then\n whereClause := whereClause || opList(1) || ' ' || 'PLS_OPHUEK_B64(' || phList(1) || ',' || cipherId || ',0) ' ||\n"
"opList(2) || ' ' || 'PLS_OPHUEK_B64(' || phList(2) || ',' || cipherId || ',0)';\n"
"elsif opList(1) = 'like' then if (self.queryInfo.likeExprInfo.allMarkPos.count > 0 and self.queryInfo.likeExprInfo.allMarkPos(1) != 1) then\n"
"declare bindStr varchar2(4000); bindStr2 varchar2(4000); lastChr varchar2(10); strLen  number; whileCount number;\n"
"begin\n bindStr := replace(self.queryInfo.bindValueList(1),'%%',''); strLen := length(bindStr); lastChr := substr(bindStr, strLen, 1);\n"
"if strLen = 1 then\n bindStr2 := chr(ascii(lastChr) + 1);\n else\n bindStr2 := substr(bindStr, 1, strLen - 1) || chr(ascii(lastChr) + 1);\n"
"end if;\n self.queryInfo.bindValueList.extend(1);\n self.queryInfo.bindValueList(1) := bindStr;\n self.queryInfo.bindValueList(2) := bindStr2;\n"
"whereClause := 'where ' || 'PLS_OPHUEK_B64(' || indexedColName || ',' || cipherId || ',1) ' || ' >= PLS_OPHUEK_B64(:v1,' || cipherId || ',0)' || ' and ' || 'PLS_OPHUEK_B64(' || indexedColName || ',' || cipherId || ',1) ' ||\n"
"' < PLS_OPHUEK_B64(:v2,' || cipherId || ',0)';\nend; \n elsif (self.queryInfo.likeExprInfo.singleMarkPos.count > 0 and\n"
"self.queryInfo.likeExprInfo.singleMarkPos(1) != 1) then\n declare bindStr varchar2(4000); bindStr2 varchar2(4000); lastChr varchar2(10);\n"
...
"strLen := length(bindStr2);\n lastChr := substr(bindStr2, strLen, 1); if strLen = 1 then bindStr2 := chr(ascii(lastChr) + 1);\n"
"else bindStr2 := substr(bindStr2, 1, strLen - 1) || chr(ascii(lastChr) + 1); end if; for i in 1..self.queryInfo.likeExprInfo.singleMarkPos.count loop\n"
"bindStr2 := bindStr2 || ' ';\n end loop; self.queryInfo.bindValueList.extend(1); self.queryInfo.bindValueList(1) := bindStr;\n"
"self.queryInfo.bindValueList(2) := bindStr2; whereClause := 'where ' || 'PLS_OPHUEK_B64(' || indexedColName || ',' || cipherId || ',1) ' || ' >= PLS_OPHUEK_B64(:v1,' || cipherId || ',0)' ||\n"
"' and ' || 'PLS_OPHUEK_B64(' || indexedColName || ',' || cipherId || ',1) ' || ' < PLS_OPHUEK_B64(:v2,' || cipherId || ',0)'; end;\n "
"else whereClause := whereClause || opList(1) || ' ' || 'PLS_OPHUEK_B64(' || phList(1) || ',' || cipherId || ',0)';\n end if;\n "
"elsif opList.last = 1 then if opList(1) = '=' then whereClause := whereClause || opList(1) || ' ' || \n"
"'PLS_OPHUEK_B64(' || phList(1) || ',' || cipherId || ',0)'; else whereClause := whereClause || opList(1) || ' ' ||\n"
"'PLS_OPHUEK_B64(' || phList(1) || ',' || cipherId || ',0)'; end if; else whereClause := whereClause || opList(1) || ' ' ||\n"
"'PLS_OPHUEK_B64(' || phList(1) || ',' || cipherId || ',0)' || ' and ' || 'PLS_OPHUEK_B64(' || indexedColName || ',' || cipherId || ',1) ' || opList(2) || ' ' || 'PLS_OPHUEK_B64(' || phList(2) || ',' || cipherId || ',0)';\n"
"end if; end; stmt := 'select /*+ first_rows index(a) */ rowid,a.* ' || 'from ' || self.indexInfo.indexCols(1).tableSchema || '.' ||\n"
"self.indexInfo.indexCols(1).tableName || ' a ' || whereClause ; return stmt; end getQueryStatement; end;");
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);

                *TextBuf=0;
                sprintf(TextBuf,"create or replace type pc_index_impl2 as object\n"
                                "(primaryIndex   DgCipherIndex$PrimaryIndex2, static function ODCIGetInterfaces(ifclist OUT sys.ODCIObjectList)\n"
                                "return number,\n"
                                "static function ODCIIndexCreate(\n"
                                "ia       sys.ODCIIndexInfo,\n"
                                "params   varchar2,\n"
                ...
                *TextBuf=0;
                sprintf(TextBuf,
"create or replace type body pc_index_impl2 is static function ODCIGetInterfaces(ifclist OUT sys.ODCIObjectList)\n"
"return number is begin ifclist := sys.ODCIObjectList(sys.ODCIObject('SYS','ODCIINDEX2')); return ODCIConst.Success; end ODCIGetInterfaces;\n"
"static function ODCIIndexCreate(ia       sys.ODCIIndexInfo,params   varchar2,env      sys.ODCIEnv) return number is E_INDEX_EXIST_OBJ exception;\n"
"pragma exception_init(E_INDEX_EXIST_OBJ, -955); begin begin return ODCIConst.Success; exception when E_INDEX_EXIST_OBJ then NULL;\n"
"when OTHERS then raise_application_error(-20900,'Error creating base index ' || ' of the domain index' || chr(10) || dbms_utility.format_error_stack);\n"
...
"end ODCIIndexDelete; static function ODCIIndexUpdate(ia     sys.ODCIIndexInfo,rid    varchar2,oldval varchar2,newval varchar2,env    sys.ODCIEnv)\n"
"return number is begin return 0; end ODCIIndexUpdate;\n"
"static function ODCIIndexStart(sctx IN OUT pc_index_impl2,ia sys.ODCIIndexInfo,pr sys.ODCIPredInfo,qi sys.ODCIQueryInfo,strt varchar2,\n"
"stop varchar2,cipherId number,env sys.ODCIEnv) return number is ret       number; primaryIndex  DgCipherIndex$PrimaryIndex2; begin\n"
"primaryIndex := DgCipherIndex$PrimaryIndex2(ia, env); primaryIndex.initQuery(pr, qi, strt, stop); ret := primaryIndex.startIndex(cipherId,strt,stop);\n"
"sctx := pc_index_impl2(primaryIndex); return ODCIConst.Success; exception when OTHERS then raise; end ODCIIndexStart; member function ODCIIndexFetch(\n"
"self IN OUT pc_index_impl2, nrows       number, rids OUT    sys.ODCIRidList, env         sys.ODCIEnv) return number\n"
"is ret     number; begin ret := self.primaryIndex.fetchIndex(nrows, rids); return ODCIConst.Success; end ODCIIndexFetch; member function ODCIIndexClose(\n"
"self IN OUT pc_index_impl2, env sys.ODCIEnv) return number is ret number; begin ret := self.primaryIndex.closeIndex; return ODCIConst.Success;\n"
"end ODCIIndexClose; end;");
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);

                *TextBuf=0;
                sprintf(TextBuf,"create or replace indextype pc_idx2_typ1 "
                                "for pls_idx1_str(varchar2,number) using pc_index_impl2");
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0;
                sprintf(TextBuf,"create or replace indextype pc_idx2_typ2 "
                                "for pls_idx1_num(varchar2,number) using pc_index_impl2");
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0;
                sprintf(TextBuf,"grant execute on pc_idx2_typ1 to public with grant option");
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0;
                sprintf(TextBuf,"grant execute on pc_idx2_typ2 to public with grant option");
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
		...
		//
                *TextBuf=0;
                sprintf(TextBuf,"create or replace type DgCipherIndex$PrimaryIndex3 as object\n"
                                "(\n"
                                "queryInfo DgCipherIndex$QueryInfo,\n"
                                "indexInfo sys.ODCIIndexInfo,\n"
		...
		*TextBuf=0;
		sprintf(TextBuf,
"CREATE OR REPLACE TYPE BODY PETRA.DgCipherIndex$PrimaryIndex3 is constructor function DgCipherIndex$PrimaryIndex3 (ia sys.ODCIIndexInfo, env sys.ODCIEnv) "
" return self as result is begin self.indexInfo := ia; return; end DgCipherIndex$PrimaryIndex3; " 
" member function startIndex(self in out DgCipherIndex$PrimaryIndex3, cipherId number, strt varchar2, stop varchar2) "
" return number "
" is i number; cnum integer; stmt varchar2(4000); bvList sys.ODCIVarchar2List; begin stmt := self.getQueryStatement(cipherId); "
" cnum := dbms_sql.open_cursor; dbms_sql.parse(cnum, stmt, dbms_sql.native); bvList := self.queryInfo.bindValueList; "
" for i in bvList.First .. bvList.Last loop dbms_sql.bind_variable(cnum, 'v' || i, bvList(i)); end loop; self.queryInfo.cursorNo := cnum; "
" self.queryInfo.howMany := 0; return DgCipherIndex$Const.Success; exception when OTHERS then dbms_sql.close_cursor(cnum); "
" return DgCipherIndex$Const.Failed; end startIndex; "
" member function fetchIndex(self IN OUT DgCipherIndex$PrimaryIndex3, nrows number, rids OUT sys.ODCIRidList) "
" return number is i number; cnum integer; fetched number; rlist sys.ODCIRidList := sys.ODCIRidList(); rid_tab dbms_sql.varchar2_table; "
" begin cnum := self.queryInfo.CursorNo; if self.queryInfo.HowMany = 0 then i := dbms_sql.execute(cnum); end if; "
" dbms_sql.define_array(cnum, 1, rid_tab, nrows, 1); fetched := dbms_sql.fetch_rows(cnum); if fetched = nrows then rlist.extend(fetched); "
" else rlist.extend(fetched + 1); end if; dbms_sql.column_value(cnum, 1, rid_tab); for i in 1..fetched loop rlist(i) := rid_tab(i); "
" end loop; self.queryInfo.HowMany := self.queryInfo.HowMany + fetched; rids := rlist; return DgCipherIndex$Const.Success; "
" end fetchIndex;"
" member function closeIndex(self IN OUT DgCipherIndex$PrimaryIndex3) "
" return number is begin if self.queryInfo.cursorNo != 0 then dbms_sql.close_cursor(self.queryInfo.cursorNo); "
" end if; return DgCipherIndex$Const.Success; end closeIndex;"
" member procedure initQuery(self IN OUT DgCipherIndex$PrimaryIndex3, pr sys.ODCIPredInfo, qi sys.ODCIQueryInfo, strt varchar2, stop varchar2) " 
" is begin self.queryInfo := DgCipherIndex$QueryInfo(pr, strt, stop); end initQuery; "
" member function getQueryStatement (self in out DgCipherIndex$PrimaryIndex3, cipherId number) return varchar2 "
" is stmt varchar2(4000); whereClause varchar2(4000); opList sys.ODCIVarchar2List; phList sys.ODCIVarchar2List; "
" indexedColName varchar2(4000); func varchar2(4000); begin opList := self.queryInfo.operatorList; phList := self.queryInfo.placeHolderList; "
" indexedColName := self.indexInfo.indexCols(1).colname; whereClause := 'where ' || indexedColName || ' '; if opList(1) = 'between' then "
" whereClause := whereClause || opList(1) || ' ' || phList(1) || ' ' || opList(2) || ' ' || phList(2); "
" elsif opList(1) = 'like' then whereClause := whereClause || opList(1) || ' ' || phList(1); "
" elsif opList.last = 1 then if opList(1) = '=' then "
" whereClause := whereClause || opList(1) || ' ' || 'PLS_ENCRYPT_B64_ID(' || phList(1) || ',' || cipherId || ')'; "
" else whereClause := whereClause || opList(1) || ' ' || phList(1); end if; else "
" whereClause := whereClause || opList(1) || ' ' || 'PLS_ENCRYPT_B64_ID(' || phList(1) || ',' || cipherId || ')' || ' and ' || indexedColName || ' ' || opList(2) || ' ' || 'PLS_ENCRYPT_B64_ID(' || phList(2) || ',' || cipherId || ')'; "
" end if; "
"stmt := 'select /*+ first_rows index(a) */ rowid,a.* ' || 'from ' || self.indexInfo.indexCols(1).tableSchema || '.' || self.indexInfo.indexCols(1).tableName || ' a ' || whereClause ; "
" return stmt; end getQueryStatement; end;");
		if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
 
                *TextBuf=0;
                sprintf(TextBuf,"create or replace type pc_index_impl3 as object\n"
                                "(primaryIndex   DgCipherIndex$PrimaryIndex3, static function ODCIGetInterfaces(ifclist OUT sys.ODCIObjectList)\n"
                                "return number,\n"
                                "static function ODCIIndexCreate(\n"
                                "ia       sys.ODCIIndexInfo,\n"
                                "params   varchar2,\n"
		...
		*TextBuf=0;
                sprintf(TextBuf,
"create or replace type body pc_index_impl3 is static function ODCIGetInterfaces(ifclist OUT sys.ODCIObjectList)\n"
"return number is begin ifclist := sys.ODCIObjectList(sys.ODCIObject('SYS','ODCIINDEX2')); return ODCIConst.Success; end ODCIGetInterfaces;\n"
"static function ODCIIndexCreate(ia       sys.ODCIIndexInfo,params   varchar2,env      sys.ODCIEnv) return number is E_INDEX_EXIST_OBJ exception;\n"
"pragma exception_init(E_INDEX_EXIST_OBJ, -955); begin begin return ODCIConst.Success; exception when E_INDEX_EXIST_OBJ then NULL;\n"
"when OTHERS then raise_application_error(-20900,'Error creating base index ' || ' of the domain index' || chr(10) || dbms_utility.format_error_stack);\n"
...
"end ODCIIndexDelete; static function ODCIIndexUpdate(ia     sys.ODCIIndexInfo,rid    varchar2,oldval varchar2,newval varchar2,env    sys.ODCIEnv)\n"
"return number is begin return 0; end ODCIIndexUpdate;\n"
"static function ODCIIndexStart(sctx IN OUT pc_index_impl3,ia sys.ODCIIndexInfo,pr sys.ODCIPredInfo,qi sys.ODCIQueryInfo,strt varchar2,\n"
"stop varchar2,cipherId number,env sys.ODCIEnv) return number is ret       number; primaryIndex  DgCipherIndex$PrimaryIndex3; begin\n"
"primaryIndex := DgCipherIndex$PrimaryIndex3(ia, env); primaryIndex.initQuery(pr, qi, strt, stop); ret := primaryIndex.startIndex(cipherId,strt,stop);\n"
"sctx := pc_index_impl3(primaryIndex); return ODCIConst.Success; exception when OTHERS then raise; end ODCIIndexStart; member function ODCIIndexFetch(\n"
"self IN OUT pc_index_impl3, nrows       number, rids OUT    sys.ODCIRidList, env         sys.ODCIEnv) return number\n"
"is ret     number; begin ret := self.primaryIndex.fetchIndex(nrows, rids); return ODCIConst.Success; end ODCIIndexFetch; member function ODCIIndexClose(\n"
"self IN OUT pc_index_impl3, env sys.ODCIEnv) return number is ret number; begin ret := self.primaryIndex.closeIndex; return ODCIConst.Success;\n"
"end ODCIIndexClose; end;");
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);

                *TextBuf=0;
                sprintf(TextBuf,"create or replace indextype pc_idx3_typ1 "
                                "for pls_idx1_str(varchar2,number) using pc_index_impl3");
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0;
                sprintf(TextBuf,"create or replace indextype pc_idx3_typ2 "
                                "for pls_idx1_num(varchar2,number) using pc_index_impl3");
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0;
                sprintf(TextBuf,"grant execute on pc_idx3_typ1 to public with grant option");
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
                *TextBuf=0;
                sprintf(TextBuf,"grant execute on pc_idx3_typ2 to public with grant option");
                if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	...
	StepNo=-2;
	StmtNo=0;
        *TextBuf=0; sprintf(TextBuf,"DROP PUBLIC SYNONYM PLS_ENCRYPT_RAW");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        *TextBuf=0; sprintf(TextBuf,"DROP PUBLIC SYNONYM PLS_ENCRYPT_RAW_NM");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        *TextBuf=0; sprintf(TextBuf,"DROP PUBLIC SYNONYM PLS_ENCRYPT_RAW_ID");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        *TextBuf=0; sprintf(TextBuf,"DROP PUBLIC SYNONYM PLS_ENCRYPT_RAW_ID_RAW");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        *TextBuf=0; sprintf(TextBuf,"DROP PUBLIC SYNONYM PLS_ENCRYPT_RAW_ID_DATE");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        *TextBuf=0; sprintf(TextBuf,"DROP PUBLIC SYNONYM PLS_DECRYPT_RAW");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        *TextBuf=0; sprintf(TextBuf,"DROP PUBLIC SYNONYM PLS_DECRYPT_RAW_NM");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        *TextBuf=0; sprintf(TextBuf,"DROP PUBLIC SYNONYM PLS_DECRYPT_RAW_ID");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        *TextBuf=0; sprintf(TextBuf,"DROP PUBLIC SYNONYM PLS_DECRYPT_RAW_ID_RAW");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        *TextBuf=0; sprintf(TextBuf,"DROP PUBLIC SYNONYM PLS_DECRYPT_RAW_ID_DATE");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        *TextBuf=0; sprintf(TextBuf,"DROP PUBLIC SYNONYM PLS_DECRYPT_RAW_ID_NUM");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        *TextBuf=0; sprintf(TextBuf,"DROP PUBLIC SYNONYM PLS_ENCRYPT_B64");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        *TextBuf=0; sprintf(TextBuf,"DROP PUBLIC SYNONYM PLS_ENCRYPT_B64_NM");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        *TextBuf=0; sprintf(TextBuf,"DROP PUBLIC SYNONYM PLS_ENCRYPT_B64_ID");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        *TextBuf=0; sprintf(TextBuf,"DROP PUBLIC SYNONYM PLS_ENCRYPT_B64_ID_RAW");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        *TextBuf=0; sprintf(TextBuf,"DROP PUBLIC SYNONYM PLS_ENCRYPT_B64_ID_DATE");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        *TextBuf=0; sprintf(TextBuf,"DROP PUBLIC SYNONYM PLS_DECRYPT_B64");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        *TextBuf=0; sprintf(TextBuf,"DROP PUBLIC SYNONYM PLS_DECRYPT_B64_NM");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        *TextBuf=0; sprintf(TextBuf,"DROP PUBLIC SYNONYM PLS_DECRYPT_B64_ID");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        *TextBuf=0; sprintf(TextBuf,"DROP PUBLIC SYNONYM PLS_DECRYPT_B64_ID_RAW");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        *TextBuf=0; sprintf(TextBuf,"DROP PUBLIC SYNONYM PLS_DECRYPT_B64_ID_DATE");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        *TextBuf=0; sprintf(TextBuf,"DROP PUBLIC SYNONYM PLS_DECRYPT_B64_ID_NUM");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        *TextBuf=0; sprintf(TextBuf,"DROP PUBLIC SYNONYM PLS_ENCRYPT_CLOB");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        *TextBuf=0; sprintf(TextBuf,"DROP PUBLIC SYNONYM PLS_ENCRYPT_BLOB");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        *TextBuf=0; sprintf(TextBuf,"DROP PUBLIC SYNONYM PLS_DECRYPT_CLOB");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        *TextBuf=0; sprintf(TextBuf,"DROP PUBLIC SYNONYM PLS_DECRYPT_BLOB");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
        *TextBuf=0; sprintf(TextBuf,"DROP PUBLIC SYNONYM PLS_LCR");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	...
	StepNo=-1;
	StmtNo=0;	
	*TextBuf=0; sprintf(TextBuf,"DROP USER %s CASCADE",agent_uid);
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
