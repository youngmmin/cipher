...
DgcCliConnection* PccPostgresScriptBuilder::connect(
    ...
    if (conn->connect(conn_string, link_info->host, uid, pw,
                      link_info->db_name) != 0) {
...
dgt_sint32 PccPostgresScriptBuilder::preparePrivInfo() throw(DgcExcept) {
    ...
    while ((priv_info_tmp = (pct_type_enc_tab_priv*)sql_stmt->fetch())) {
        ...
        sprintf(privSql.org1, "GRANT %s ON %s.%s TO %s", privilege, SchemaName,
                TabInfo.org_renamed_tab_name, priv_info_tmp->grantee);
        sprintf(privSql.enc1, "GRANT %s ON %s.%s TO %s", privilege, SchemaName,
...
dgt_sint32 PccPostgresScriptBuilder::prepareCommentInfo() throw(DgcExcept) {
    dgt_schar sql_text[2048];
    sprintf(sql_text,
            "select getname(comments) from pct_enc_tab_comment where "
            "enc_tab_id=%lld ",
            TabInfo.enc_tab_id);
    DgcSqlStmt* sql_stmt =
        Database->getStmt(Session, sql_text, strlen(sql_text));
    ...
    while ((comment_tmp = (dgt_schar*)sql_stmt->fetch())) {
        ...
        memset(&comments, 0, sizeof(pc_type_col_comment));
        sprintf(comments.org1, "COMMENT ON TABLE %s.%s IS '%s'", SchemaName,
                TabInfo.org_renamed_tab_name, comment_tmp);
        sprintf(comments.enc1, "COMMENT ON TABLE %s.%s IS '%s'", SchemaName,
...
dgt_sint32 PccPostgresScriptBuilder::prepareIdxInfo() throw(DgcExcept) {
    ...
    while ((idxname = (dgt_sint64*)sql_stmt->fetch())) {
        ...
        sprintf(
            sql_text,
            "select count() from pct_enc_col_ct where constraint_name = %lld ",
            *idxname);
        DgcSqlStmt* pk_stmt =
            Database->getStmt(Session, sql_text, strlen(sql_text));
        ...
        while ((idxcol = (idx_type*)idx_stmt->fetch())) {
            ...
            if (seq == 1) {
                ...
                sprintf(tmp_sql.org2, "DROP INDEX %s.%s", SchemaName,
                        PetraNamePool->getName(idxcol->index_name));
                sprintf(tmp_sql.enc2, "DROP INDEX %s.%s", SchemaName,
                        PetraNamePool->getName(idxcol->index_name));
            } else {
...
dgt_sint32 PccPostgresScriptBuilder::prepareCt2Info() throw(DgcExcept) {
    ...
    memset(sql_text, 0, 2048);
    sprintf(sql_text,
            "select a.enc_tab_id, a.schema_name, a.table_name, "
            "a.renamed_tab_name, a.org_renamed_tab_name "
            "from   ceea_enc_table a, "
            "(select distinct ref_pk_owner, ref_pk_table from pct_enc_col_ct "
            "where enc_tab_id = %lld and constraint_type=2) b "
            "where a.schema_name = b.ref_pk_owner and a.table_name = "
            "b.ref_pk_table ",
            TabInfo.enc_tab_id);
    sql_stmt = Database->getStmt(Session, sql_text, strlen(sql_text));
    if (sql_stmt == 0 || sql_stmt->execute() < 0) {
...
dgt_schar* PccPostgresScriptBuilder::getFname(
    ...
    while (ColInfoRows2.next() &&
        ...
        sprintf(sql_text,
                "select t.table_name, c.column_name from pct_enc_table t, "
                "pct_enc_column c "
                "where c.enc_col_id = %lld and t.enc_tab_id = c.enc_tab_id",
                col_info->enc_col_id);
        sql_stmt = Database->getStmt(Session, sql_text, strlen(sql_text));
        if (sql_stmt == 0 || sql_stmt->execute() < 0) {
            delete sql_stmt;
        ...
        if (fun_type == 1) {
            if (instead_of_trigger_flag) {
                if (col_info->col_default &&
                    ...
                    sprintf(fname, "pca_encrypt(:new.%s,'%s')",
                            col_info->col_name, enc_name);
                }
            } else {
                if (col_info->col_default &&
                    strlen(PetraNamePool->getNameString(
                    ...
                    sprintf(fname, "pca_encrypt(%s,'%s')", col_info->col_name,
                            enc_name);
                }
            }
        } else if (fun_type == 2) {
            if (TabInfo.cast_flag == 0) {
                sprintf(fname, "pca_decrypt(%s,'%s')", col_info->col_name,
                        enc_name);
            } else {
                if (!strcasecmp(col_info->data_type, "numeric")) {
                    ...
                    if (col_info->data_precision && col_info->data_scale) {
                        sprintf(fname, "(%d,%d)", col_info->data_precision,
                                col_info->data_scale);
                    } else if (col_info->data_precision) {
                        sprintf(fname, "(%d)", col_info->data_precision);
                    }
                    sprintf(fname, "cast(pca_decrypt(%s,'%s') as %s%.*s)",
                            col_info->col_name, enc_name, col_info->data_type,
                            (dgt_sint32)strlen(type), type);
                } else if (!strncasecmp(col_info->data_type, "time", 4)) {
                    sprintf(fname, "cast(pca_decrypt(%s,'%s') as %s)",
                            col_info->col_name, enc_name, col_info->data_type);
                } else if (col_info->data_length) {
                    sprintf(fname, "cast(pca_decrypt(%s,'%s') as %s(%d))",
                            col_info->col_name, enc_name, col_info->data_type,
...
dgt_sint32 PccPostgresScriptBuilder::insteadOfTrigger(
    ...
    *TmpBuf = 0;
    if (TabInfo.user_view_flag == 1 ||
        ...
        sprintf(TextBuf,
                "CREATE TRIGGER %s \nINSTEAD OF INSERT OR UPDATE ON %s.%s FOR "
                "EACH ROW \nDECLARE",
        ...
        sprintf(TextBuf,
                "CREATE TRIGGER %s \nINSTEAD OF INSERT OR UPDATE ON %s.%s FOR "
                "EACH ROW \nDECLARE",
                TabInfo.view_trigger_name, SchemaName,
    ...
    *TmpBuf = 0;
    ...
    while (ColInfoRows.next() &&
        ...
        if (col_info->status >= 1 && col_info->status < 3) {
            ...
            if (!strcasecmp(col_info->data_type, "date") ||
                !strncasecmp(col_info->data_type, "time", 4))
                enc_len = 24;
            else if (col_info->data_precision)
    ...
    while (CheckTrgRows.next() &&
        ...
        *TmpBuf = 0;
        ...
        strcat(TextBuf, TmpBuf);
        *TmpBuf = 0;
        sprintf(TmpBuf, "(:new.%s",
                tmp_search->search_condition +
                    1);  // (val < 1000) -> (:new.val < 1000)
        //		sprintf(TmpBuf,"(:new.%s)",tmp_search->search_condition);
        strcat(TextBuf, TmpBuf);
        *TmpBuf = 0;
        sprintf(TmpBuf,
                " THEN\n\t RAISE EXCEPTION 'new row for relation \"%s\" "
                "violates check constraint %s';\n END IF;",
                TabInfo.table_name, tmp_search->search_condition);
        strcat(TextBuf, TmpBuf);
    }
    *TmpBuf = 0;
    sprintf(TmpBuf, "\n   IF inserting THEN\n");
        ...
        *TmpBuf = 0;
        ...
        while (ColInfoRows.next() &&
            ...
            if (col_info->col_default && strlen(PetraNamePool->getNameString(
                                             col_info->col_default)) > 0) {
                *TmpBuf = 0;
                sprintf(TmpBuf, "coalesce(:new.%s,%s),", col_info->col_name,
                        PetraNamePool->getNameString(col_info->col_default));
                strcat(TextBuf, TmpBuf);
            } else {
                *TmpBuf = 0;
        ...
        *TmpBuf = 0;
        if (TabInfo.double_flag && IdxColRows.numRows() == 0) {
            sprintf(
                ...
                "\n\t\tv_sql_main := v_sql_main || v_sql_set || ' WHERE ctid = "
                "''' || :old.ctid || '''';"
...
dgt_sint32 PccPostgresScriptBuilder::step1() throw(DgcExcept) {
    ...
    *TextBuf = 0;
    ...
    sprintf(TextBuf,
            ...
            "INCLUDING STORAGE "
            "INCLUDING COMMENTS)",
            SchemaName, TabInfo.renamed_tab_name, SchemaName,
            TabInfo.table_name);
#endif
    ...
    *TextBuf = 0;
    *TmpBuf = 0;
    sprintf(TextBuf, "ALTER TABLE %s.%s OWNER TO %s", SchemaName,
            TabInfo.renamed_tab_name, SchemaName);
    if (saveSqlText() < 0) {
        ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    }
    ...
    while (ColInfoRows.next() &&
        ...
        if (col_info->status == 1) {
            ...
            *TextBuf = 0;
            sprintf(
                TextBuf,
                "ALTER TABLE %s.%s ALTER COLUMN %s TYPE character varying(%d)",
    ...
    while (CheckSqlRows.next() &&
           (cksql = (pc_type_checksql*)CheckSqlRows.data())) {
        *TextBuf = 0;
    ...
    *TmpBuf = 0;
    ...
    if (TabInfo.grant_flag) {
        while (PrivSqlRows.next() &&
               (priv_sql = (pc_type_col_priv*)PrivSqlRows.data())) {
            *TextBuf = 0;
    ...
    *TmpBuf = 0;
    ...
    while (CommentInfoRows.next() &&
           (comment_sql = (pc_type_col_comment*)CommentInfoRows.data())) {
        *TextBuf = 0;
...
dgt_sint32 PccPostgresScriptBuilder::step2() throw(DgcExcept) {
                ...
                *TmpBuf = 0;
                if (col_info->status == 1) {
                    if (col_info->cipher_type == 4) {
                        *TmpBuf = 0;
                        sprintf(TmpBuf, "%s as %s,", col_info->col_name,
                                col_info->col_name);
                        strcat(TextBuf, TmpBuf);
                    } else {
                        *TmpBuf = 0;
                        sprintf(TmpBuf, "%s as %s,",
    ...
    *TmpBuf = 0;
    if (TabInfo.keep_org_tab_flag == 0) {
        ...
        ref_table_sql* refsql;
        DefTabDropSqlRows.rewind();
        while (DefTabDropSqlRows.next() &&
               (refsql = (ref_table_sql*)DefTabDropSqlRows.data())) {
            *TextBuf = 0;
    ...
    *TmpBuf = 0;
    ...
    if (TabInfo.keep_org_tab_flag == 0) {
        ...
        sprintf(TextBuf,
                ...
                "INCLUDING STORAGE "
                "INCLUDING COMMENTS)",
                SchemaName, TabInfo.org_renamed_tab_name, SchemaName,
                TabInfo.renamed_tab_name);
        if (TabInfo.target_tablespace_name[0]) {
            sprintf(TmpBuf, " TABLESPACE %s", TabInfo.target_tablespace_name);
            strcat(TextBuf, TmpBuf);
        ...
        *TmpBuf = 0;
        sprintf(TextBuf, "ALTER TABLE %s.%s OWNER TO %s", SchemaName,
                TabInfo.org_renamed_tab_name, SchemaName);
        if (saveSqlText() < 0) {
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        }
        ...
        while (ColInfoRows.next() &&
            ...
            *TmpBuf = 0;
            ...
            if (!strcasecmp(col_info->data_type, "numeric")) {
                if (col_info->data_precision && col_info->data_scale) {
                    sprintf(TmpBuf, " numeric(%d,%d)", col_info->data_precision,
                            col_info->data_scale);
                } else if (col_info->data_precision) {
                    ...
                    sprintf(TmpBuf, " numeric");
                                   ...
                                   "time with time zone")) {
                if (col_info->data_precision)
                    sprintf(type, "(%d)", col_info->data_precision);
                sprintf(TmpBuf, " time%.*s with time zone",
                        (dgt_sint32)strlen(type), type);
            } else if (!strcasecmp(col_info->data_type,
                                   "time without time zone")) {
                if (col_info->data_precision)
                    sprintf(type, "(%d)", col_info->data_precision);
                sprintf(TmpBuf, " time%.*s without time zone",
                        (dgt_sint32)strlen(type), type);
            } else if (!strcasecmp(col_info->data_type,
                                   "timestamp with time zone")) {
                if (col_info->data_precision)
                    sprintf(type, "(%d)", col_info->data_precision);
                sprintf(TmpBuf, " timestamp%.*s with time zone",
                        (dgt_sint32)strlen(type), type);
            } else if (!strcasecmp(col_info->data_type,
                                   "timestamp without time zone")) {
                if (col_info->data_precision)
                    sprintf(type, "(%d)", col_info->data_precision);
                sprintf(TmpBuf, " timestamp%.*s without time zone",
                        (dgt_sint32)strlen(type), type);
            } else if (col_info->data_length) {
                sprintf(TmpBuf, " %s(%d)", col_info->data_type,
                        col_info->data_length);
            } else {
            ...
            sprintf(TextBuf,
                    ...
                    SchemaName, TabInfo.org_renamed_tab_name,
                    col_info->col_name, TmpBuf, col_info->col_name, TmpBuf);
...
PccPostgresScriptBuilder::PccPostgresScriptBuilder(DgcDatabase* db,
                                                   DgcSession* sess,
...
dgt_sint32 PccPostgresScriptBuilder::getTablespace(DgcMemRows* rtn_rows) throw(
    DgcExcept) {
    if (!getConnection()) {
...
dgt_sint32 PccPostgresScriptBuilder::buildScript(
    ...
    // if (prepareObjInfo() < 0) ATHROWnR(DgcError(SPOS,"prepareObjInfo
    // failed."),-1);
    if (prepareIdxInfo() < 0)
        ATHROWnR(DgcError(SPOS, "prepareIdxInfo failed."), -1);
    if (prepareCommentInfo() < 0)
        ATHROWnR(DgcError(SPOS, "prepareCommentInfo failed."), -1);
    // if (prepareSynonymInfo() < 0) ATHROWnR(DgcError(SPOS,"prepareSynonymInfo
    // failed."),-1);
    if (prepareCtInfo() < 0)
...
dgt_sint32 PccPostgresScriptBuilder::buildInstallScript(
    ...
    *TextBuf = 0;
    sprintf(TextBuf, "CREATE USER %s WITH SUPERUSER PASSWORD '%s'", agent_uid,
            agent_pass);
    ...
    *TextBuf = 0;
    sprintf(TextBuf,
            "CREATE OR REPLACE PUBLIC SYNONYM PCA_ENCRYPT FOR %s.PCA_ENCRYPT",
            agent_uid);
    if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    *TextBuf = 0;
    sprintf(TextBuf,
            "CREATE OR REPLACE PUBLIC SYNONYM PCA_DECRYPT FOR %s.PCA_DECRYPT",
            agent_uid);
    if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    ...
    *TextBuf = 0;
    sprintf(TextBuf, "DROP PUBLIC SYNONYM PCA_ENCRYPT");
    if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    *TextBuf = 0;
    sprintf(TextBuf, "DROP PUBLIC SYNONYM PCA_DECRYPT");
    if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);

    *TextBuf = 0;
    sprintf(TextBuf, "DROP FUNCTION %s.PCA_ENCRYPT", agent_uid);
    if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    *TextBuf = 0;
    sprintf(TextBuf, "DROP FUNCTION %s.PCA_DECRYPT", agent_uid);
    if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
