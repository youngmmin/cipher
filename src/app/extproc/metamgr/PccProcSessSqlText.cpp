...
//printf("sid[%lld], serial#[%lld]\n",param->sid,param->serial_no);
	dgt_schar sel_str[1024]={0,};

	switch(DatabaseLink->getDbType()){
			case DGC_DB_TYPE_SOHA:
				THROWnR(DgcLdbExcept(DGC_EC_LD_STMT_ERR,new DgcError(SPOS,"[%s] db type is not in service yet.", param->dblink_name)),-1);
...
"and    s.sid=%lld "
"order by piece",param->sid);
				break;
			default:
				THROWnR(DgcLdbExcept(DGC_EC_LD_STMT_ERR,new DgcError(SPOS,"[%s] db type is invaild.", param->dblink_name)),-1);
				break;
	}
    ...
    while(rtn_rows && rtn_rows->numRows() > 0) {
    	...
    	while(rtn_rows->next()) {
    		...
    		if(tmp_len > 0) dg_strncpy(sess_sql_text_info.sql_text,tmp,tmp_len>64?64:tmp_len);
    		sess_sql_text_info.piece=dg_strtoll((dgt_schar*)rtn_rows->getColPtr(2),0,10);

    		ReturnRows->add();
    		ReturnRows->next();
    		dg_memcpy(ReturnRows->data(),&sess_sql_text_info,sizeof(pct_type_sess_sql_text));
    	}
    	ReturnRows->rewind();
    	rtn_rows->reset();
    }
