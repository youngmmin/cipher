...
dgt_sint32 PccProcSessionMonitor::execute() throw(DgcExcept)
{
	...
	dgt_schar sel_str[2048]={0,};

	switch(DatabaseLink->getDbType()){
			case DGC_DB_TYPE_SOHA:
				THROWnR(DgcLdbExcept(DGC_EC_LD_STMT_ERR,new DgcError(SPOS,"[%s] db type is not in service yet.", dblink_name)),-1);
				//Stmt = new CeeaSohaSqlStmt(DatabaseLink,TraceLevel);
				break;
			case DGC_DB_TYPE_ORACLE:
				sprintf(sel_str,
				"SELECT SS.* FROM ("
						"SELECT /*+ use_hash(s t) */nvl(s.username, decode(s.type, 'USER', decode(bgp.name, NULL,'UNDEFINED '))) username, "
							"s.sid, "
							"s.serial#, "
							"s.paddr, "
							"s.osuser, "
							"act.name sql_type, "
							"s.status, "
							"decode(s.event, 'latch free', s.event || ' (' || l.name || ')', s.event) event, "
							"s.machine, "
							"s.module, "
							"s.action, "
							"s.program, "
...
"select schemaname, "
       ...
       " ipaddr, "
       " osuser, "
       " decode(command,1,'SELECT',2,'INSERT',3,'UPDATE',4,'DELETE','CALL'), "
       " status, "
       " state, "
       " machine, "
       " module, "
...
"from v$session ");
			...
			default:
				THROWnR(DgcLdbExcept(DGC_EC_LD_STMT_ERR,new DgcError(SPOS,"[%s] db type is invaild.", dblink_name)),-1);
				break;
	}
    ...
    if (sql_stmt->execute(sel_str,strlen(sel_str),1000) < 0) {
    	...
    	delete sql_stmt;
    	RTHROWnR(e,DgcError(SPOS,"execute[%*.*s] failed",64,64,sel_str),-1);
    }
    ...
    while(rtn_rows && rtn_rows->numRows() > 0) {
    	...
    	while(rtn_rows->next()) {
    		...
    		if(tmp_len > 0) dg_strncpy(sess_mon_info.username,tmp,tmp_len>33?33:tmp_len);
    		sess_mon_info.sid=dg_strtoll((dgt_schar*)rtn_rows->getColPtr(2),0,10);
    		sess_mon_info.serial_no=dg_strtoll((dgt_schar*)rtn_rows->getColPtr(3),0,10);
    		tmp=(dgt_schar*)rtn_rows->getColPtr(4);
    		tmp_len=dg_strlen(tmp);
    		if(tmp_len > 0) dg_strncpy(sess_mon_info.paddr,tmp,tmp_len>33?33:tmp_len);
    		tmp=(dgt_schar*)rtn_rows->getColPtr(5);
    		tmp_len=dg_strlen(tmp);
    		if(tmp_len > 0) dg_strncpy(sess_mon_info.osuser,tmp,tmp_len>33?33:tmp_len);
    		tmp=(dgt_schar*)rtn_rows->getColPtr(6);
    		tmp_len=dg_strlen(tmp);
    		if(tmp_len > 0) dg_strncpy(sess_mon_info.sql_type,tmp,tmp_len>10?10:tmp_len);
    		tmp=(dgt_schar*)rtn_rows->getColPtr(7);
    		tmp_len=dg_strlen(tmp);
    		if(tmp_len > 0) dg_strncpy(sess_mon_info.status,tmp,tmp_len>20?20:tmp_len);
    		tmp=(dgt_schar*)rtn_rows->getColPtr(8);
    		tmp_len=dg_strlen(tmp);
    		if(tmp_len > 0) dg_strncpy(sess_mon_info.sess_event,tmp,tmp_len>100?100:tmp_len);
    		tmp=(dgt_schar*)rtn_rows->getColPtr(9);
    		tmp_len=dg_strlen(tmp);
    		if(tmp_len > 0) dg_strncpy(sess_mon_info.machine,tmp,tmp_len>50?50:tmp_len);
    		tmp=(dgt_schar*)rtn_rows->getColPtr(10);
    		tmp_len=dg_strlen(tmp);
    		if(tmp_len > 0) dg_strncpy(sess_mon_info.module,tmp,tmp_len>50?50:tmp_len);
    		tmp=(dgt_schar*)rtn_rows->getColPtr(11);
    		tmp_len=dg_strlen(tmp);
    		if(tmp_len > 0) dg_strncpy(sess_mon_info.action,tmp,tmp_len>30?30:tmp_len);
    		tmp=(dgt_schar*)rtn_rows->getColPtr(12);
    		tmp_len=dg_strlen(tmp);
    		if(tmp_len > 0) dg_strncpy(sess_mon_info.program,tmp,tmp_len>50?50:tmp_len);
    		tmp=(dgt_schar*)rtn_rows->getColPtr(13);
    		tmp_len=dg_strlen(tmp);
    		if(tmp_len > 0) dg_strncpy(sess_mon_info.logon_time,tmp,tmp_len>30?30:tmp_len);
    		sess_mon_info.session_logical_read=dg_strtoll((dgt_schar*)rtn_rows->getColPtr(14),0,10);

    		ReturnRows->add();
    		ReturnRows->next();
    		dg_memcpy(ReturnRows->data(),&sess_mon_info,sizeof(pct_type_session_monitor));
    	}
    	ReturnRows->rewind();
    	rtn_rows->reset();
    }
