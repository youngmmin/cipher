...
PccRunScript::PccRunScript(const dgt_schar* name) : PccMetaProcedure(name) {}

PccRunScript::~PccRunScript() {}

DgcExtProcedure* PccRunScript::clone() { return new PccRunScript(procName()); }
...
dgt_sint32 PccRunScript::execute() throw(DgcExcept) {
    ...
    PccScriptBuilder* script_builder =
        getScriptBuilder(InRow->enc_tab_id, PCC_ID_TYPE_AGENT);
    if (!script_builder) {
    ...
    //
    dgt_sint32 rtn;
    if ((rtn = script_builder->getScript(InRow->enc_tab_id, InRow->version_no,
                                         InRow->step_no, InRow->stmt_no)) > 0) {
        //
        // run script
        //
