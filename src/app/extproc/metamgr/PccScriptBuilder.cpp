...
dgt_sint32 PccScriptBuilder::prepareTabInfo(dgt_sint64 enc_tab_id) throw(
    DgcExcept) {
    ...
    //
    sprintf(sql_text,
            "select max(d.parallel_degree),max(e.db_version),max(c.db_id) from "
            "pct_enc_table a, pct_enc_schema b, pt_database c, pct_db_agent d, "
            "pt_db_instance e "
    ...
    sprintf(sql_text,
            ...
            "pct_db_agent a, pct_enc_table b, pct_enc_schema c, "
            "pt_database_link_info e "
...
dgt_sint32 PccScriptBuilder::saveSqlText() throw(DgcExcept) {
    ...
    for (dgt_sint32 i = 0; i < 2; i++) {
        if ((cs_tab = (DgcTableSegment*)Database->pdb()->segMgr()->getTable(
            ...
            THROWnR(
                DgcPdbExcept(DGC_EC_PDO_NOT_FOUND,
        ...
        while (remains > 0) {
            ...
            if (i == 0)
                csp->version_no = VersionNo;
            else
                csp->version_no = VersionNo + 10;
            csp->step_no = StepNo;
            csp->stmt_no = StmtNo;
            csp->seg_no = ++seg_no;
            if (remains < 64) {
            ...
            if (count % 10 == 0) {
                ...
                if (cs_tab->insertCommit(Session, cs_rows) != 0) {
                    ...
                    if (cs_tab->pinRollback(cs_rows)) delete EXCEPTnC;
                    RTHROWnR(e,
                             DgcError(SPOS, "insertCommit[PCT_SCRIPT] failed"),
                             -1);
                }
                cs_rows.reset();
            }
        ...
        if (count % 10 != 0) {
            ...
            if (cs_tab->insertCommit(Session, cs_rows) != 0) {
                DgcExcept* e = EXCEPTnC;
                if (cs_tab->pinRollback(cs_rows)) delete EXCEPTnC;
                RTHROWnR(e, DgcError(SPOS, "insertCommit[PCT_SCRIPT] failed"),
                         -1);
            }
        }
    }
...
PccScriptBuilder::PccScriptBuilder(DgcDatabase* db, DgcSession* sess,
...
dgt_sint32 PccScriptBuilder::getScript(
    dgt_sint64 enc_tab_id, dgt_uint16 version_no, dgt_sint16 step_no,
    dgt_sint16 stmt_no, DgcMemRows* rtn_rows,
    ...
    for (rtn = 0; ((curr = (pct_type_script*)sql_stmt->fetch())); rtn++) {
        if (curr->step_no != prev.step_no || curr->stmt_no != prev.stmt_no) {
            ...
            *tmp = 0;
            if (comment_flag) {
                if (rtn > 0)
                    sprintf(tmp, "\n/* step:%u stmt:%u */", curr->step_no,
                            curr->stmt_no);
                else
                    sprintf(tmp, "/* step:%u stmt:%u */", curr->step_no,
                            curr->stmt_no);
            } else {
...
dgt_sint32 PccScriptBuilder::runScript(
    const dgt_schar* script_text, DgcCliConnection* conn) throw(DgcExcept) {
    if (conn) {
...
dgt_sint32 PccScriptBuilder::runVerifyScript(
    dgt_schar* sql_text, DgcMemRows* rtn_rows) throw(DgcExcept) {
    if (!getConnection()) {
        ATHROWnR(DgcError(SPOS, "getConnection failed."), -1);
    }
    DgcCliStmt* stmt = Connection->getStmt();
