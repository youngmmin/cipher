...
PccScriptBuilder* PccScriptBuilderFactory::scriptBuilder(
    DgcDatabase* db, DgcSession* sess, dgt_sint64 id,
    dgt_uint8 id_type) throw(DgcExcept) {
    dgt_schar sql_text[256];
    if (id_type == PCC_ID_TYPE_AGENT) {
        sprintf(sql_text,
                "select b.db_type, a.admin_link, b.db_version"
    ...
    if (schema_info.dbms_type == 11) {
        script_builder =
            new PccOraScriptBuilder(db, sess, schema_info.schema_link);
    } else if (schema_info.dbms_type == DGC_DB_TYPE_TDS) {
        if (!strncasecmp(schema_info.db_version, "2000", 32)) {
            script_builder =
                new PccTds2000ScriptBuilder(db, sess, schema_info.schema_link);
        } else {
            script_builder =
                new PccTdsScriptBuilder(db, sess, schema_info.schema_link);
        }
    } else if (schema_info.dbms_type == DGC_DB_TYPE_TIBERO) {
        script_builder =
            new PccTiberoScriptBuilder(db, sess, schema_info.schema_link);
        ...
        script_builder =
            new PccMyScriptBuilder(db, sess, schema_info.schema_link);
#ifdef linux
    } else if (schema_info.dbms_type == DGC_DB_TYPE_POSTGRESQL) {
        script_builder =
            new PccPostgresScriptBuilder(db, sess, schema_info.schema_link);
#endif
    } else {
        THROWnR(DgcLdbExcept(DGC_EC_LD_STMT_ERR,
                             new DgcError(SPOS, "unsupported dbms type[%s]",
                                          schema_info.dbms_type)),
                0);
    }
