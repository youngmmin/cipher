...
DgcCliConnection* PccTdsScriptBuilder::connect(dgt_schar* uid,
    ...
    while ((idxname = (dgt_sint64*)sql_stmt->fetch())) {
        ...
        while ((idxcol = (idx_type*)idx_stmt->fetch())) {
            ...
            if (seq == 1) {
                ...
                if (TabInfo.enc_type == 0) {
                    sprintf(tmp_sql.org2, "drop index %s.%s.%s", SchemaName,
                            TabInfo.renamed_tab_name,
                            PetraNamePool->getName(idxcol->index_name));
                    sprintf(tmp_sql.enc2, "drop index %s.%s.%s", SchemaName,
                            TabInfo.renamed_tab_name,
                            PetraNamePool->getName(idxcol->index_name));
                } else {
                    sprintf(tmp_sql.org2, "drop index %s.%s.%s", SchemaName,
...
dgt_sint32 PccTdsScriptBuilder::prepareCtInfo() throw(DgcExcept) {
    ...
    if (IsPkFk == 1) {
        ...
        sprintf(
            ...
            "from pct_working_set a, pct_enc_column b, pct_enc_table c, "
            "pct_enc_schema d "
        ...
        while ((tmp_ptr = (type_column*)sql_stmt->fetch())) {
            ...
            dgt_schar sql[512];
            memset(sql, 0, 512);
            sprintf(sql, "alter table %s.%s drop column %s",
                    tmp_ptr->schema_name, tmp_ptr->table_name,
                    tmp_ptr->col_name);
        ...
        while ((row_ptr = (pc_type_working_set*)sql_stmt->fetch())) {
            ...
            while ((pk_row = (pc_type_pk_info*)pk_stmt->fetch())) {
                ...
                if (ispkfk_tab) {
                    sprintf(
                        pkfkSql.org2, "alter table %s.%s drop constraint %s",
                ...
                if (TabInfo.enc_type == 0) {
                    sprintf(
                        pkfkSql.enc2, "alter table %s.%s drop constraint %s",
                        PetraNamePool->getNameString(pk_row->schema_name),
                        PetraNamePool->getNameString(pk_row->renamed_tab_name),
                        PetraNamePool->getNameString(pk_row->constraint_name));
                } else {
            ...
            while ((constraint_name_tmp = (dgt_sint64*)fk_sql_stmt->fetch())) {
                ...
                while ((fk_row = (pc_type_pk_info*)fk_stmt->fetch())) {
                    ...
                    if ((tmp_ptr = (ref_pk_row*)sqlstmt->fetch())) {
                        ...
                        if (TabInfo.enc_type == 0) {
                            sprintf(pktmp.renamed_table,
                                    tmp_ptr->renamed_tab_name);
                        } else {
                            sprintf(pktmp.renamed_table, tmp_ptr->table_name);
                        }
                        sprintf(pktmp.renamed_column,
                                tmp_ptr->renamed_col_name);
                    } else {
                        sprintf(pktmp.owner, PetraNamePool->getNameString(
                                                 fk_row->ref_pk_owner));
                        sprintf(
                            pktmp.table, "%s",
                            PetraNamePool->getNameString(fk_row->ref_pk_table));
                        sprintf(pktmp.column, "%s",
                                PetraNamePool->getNameString(
                                    fk_row->ref_pk_column));
                        if (TabInfo.enc_type == 0) {
                            sprintf(pktmp.renamed_table, "%s$$",
                                    PetraNamePool->getNameString(
                                        fk_row->ref_pk_table));
                        } else {
                            sprintf(pktmp.renamed_table, "%s",
                        ...
                        sprintf(pktmp.renamed_column, "%s$$",
                                PetraNamePool->getNameString(
                                    fk_row->ref_pk_column));
                    }
                    DgcExcept* e = EXCEPTnC;
...
dgt_schar* PccTdsScriptBuilder::getFname(
                    ...
                    if (TabInfo.cast_flag == 0) {
                        ...
                        if (!strcasecmp(col_info->data_type, "numeric")) {
                            ...
                            sprintf(fname,
                                    ...
                                    "s,%lld) as %s)",
                                    col_info->renamed_col_name,
                                    col_info->enc_col_id, col_info->data_type);
...
dgt_sint32 PccTdsScriptBuilder::insteadOfTrigger(
    ...
    *TmpBuf = 0;
    ...
    while (ColInfoRows.next() &&
        ...
        if (col_info->col_default) {
            *TmpBuf = 0;
            sprintf(TmpBuf, "isnull(I.%s,%s),", col_info->col_name,
    ...
    *TmpBuf = 0;
    ...
    while (ColInfoRows.next() &&
        ...
        *TmpBuf = 0;
        ...
        if (col_info->data_length == -1) {
            sprintf(TmpBuf, "\n\tdeclare @s_%s nvarchar(max)",
                    col_info->col_name);
        } else {
            sprintf(TmpBuf, "\n\tdeclare @s_%s nvarchar(%d)",
        ...
        *TmpBuf = 0;
        sprintf(TmpBuf,
                ...
                "\n\t\tbegin select @s_%s = %s from inserted I end",
                col_info->col_name, col_info->col_name,
                getFname(col_info->enc_col_id, 1, 1), col_info->col_name,
                col_info->col_name);
        strcat(TextBuf, TmpBuf);
    }
    strcat(TextBuf, "\n\n");
        ...
        if (col_info->status >= 1 && col_info->status < 3) {
            *TmpBuf = 0;
            if (col_info->cipher_type == 4) {  // sha
                sprintf(TmpBuf, "\n\t\t%s.%s.%s=@s_%s,", SchemaName,
                        TabInfo.renamed_tab_name, col_info->renamed_col_name,
                        col_info->col_name);
            } else {
                sprintf(TmpBuf, "\n\t\t%s.%s.%s=%s,", SchemaName,
                        TabInfo.renamed_tab_name, col_info->renamed_col_name,
...
dgt_sint32 PccTdsScriptBuilder::step1() throw(DgcExcept) {
    ...
    *TextBuf = 0;
    ...
    if (TabInfo.enc_type == 0 && IdxColRows.numRows() <= 0) {
        sprintf(TextBuf, "alter table %s.%s add rowid bigint identity(1,1)",
                SchemaName, TabInfo.table_name);
        if (saveSqlText() < 0) {
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        }
    ...
    *TextBuf = 0;
    ...
    if (TabInfo.enc_type == 0 && IdxColRows.numRows() <= 0) {
        ...
        *TmpBuf = 0;
        if (IdxColRows.numRows() == 0) {
            sprintf(TmpBuf, ",rowid from %s.%s;", SchemaName,
                    TabInfo.renamed_tab_name);
            strcat(TextBuf, TmpBuf);
        } else {
            sprintf(TmpBuf, " from %s.%s;", SchemaName,
                    TabInfo.renamed_tab_name);
            strcat(TextBuf, TmpBuf);
        }
        if (saveSqlText() < 0) {
        ...
        if (col_info->status == 1) {
            *TmpBuf = 0;
            sprintf(TmpBuf, "\n\t%s=%s,", col_info->renamed_col_name,
                    getFname(col_info->enc_col_id, 1, 0));
            strcat(TextBuf, TmpBuf);
        }
    }
    TextBuf[strlen(TextBuf) - 1] = 0;
        ...
        *TextBuf = 0;
        if (col_info->nullable_flag == 0) {
            if (TabInfo.enc_type == 0) {
                if (col_info->data_length == -1) {
                    sprintf(TextBuf,
                            "alter table %s.%s alter column %s nvarchar(max) "
                            "not null",
                            SchemaName, TabInfo.renamed_tab_name,
                            col_info->renamed_col_name);
                } else {
                    sprintf(TextBuf,
                            "alter table %s.%s alter column %s nvarchar(%d) "
                            "not null",
                            SchemaName, TabInfo.renamed_tab_name,
                            col_info->renamed_col_name, enc_len);
                }
            ...
            *TextBuf = 0;
            if (TabInfo.enc_type == 0) {
                if (!strcasecmp(col_info->data_type, "numeric")) {
                    if (col_info->data_precision == 0) {
                        sprintf(TextBuf,
                                "alter table %s.%s alter column %s %s null",
                                SchemaName, TabInfo.renamed_tab_name,
                                col_info->col_name, col_info->data_type);
                    } else {
                        sprintf(
                            TextBuf,
                            "alter table %s.%s alter column %s %s(%d,%d) null",
                            SchemaName, TabInfo.renamed_tab_name,
                            col_info->col_name, col_info->data_type,
                    ...
                    sprintf(TextBuf,
                            "alter table %s.%s alter column %s %s(%d) null",
                            SchemaName, TabInfo.renamed_tab_name,
                            col_info->col_name, col_info->data_type,
                            col_info->data_length);
                } else if (col_info->data_length == -1) {
                    sprintf(TextBuf,
                            "alter table %s.%s alter column %s %s(max) null",
                            SchemaName, TabInfo.renamed_tab_name,
                            col_info->col_name, col_info->data_type);
                } else {
                    sprintf(TextBuf,
...
dgt_sint32 PccTdsScriptBuilder::reverse_step1() throw(DgcExcept) {
    ...
    *TextBuf = 0;
    ...
    pc_type_col_info* col_info;
    if (TabInfo.enc_type == 0 && IdxColRows.numRows() <= 0) {
        sprintf(TextBuf, "alter table %s.%s drop column rowid", SchemaName,
                TabInfo.table_name);
        if (saveSqlText() < 0) {
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        }
        ...
        if (col_info->nullable_flag == 0) {
            if (TabInfo.enc_type == 0) {
                if (col_info->data_length == -1) {
                    sprintf(
                        TextBuf,
                        "alter table %s.%s alter column %s nvarchar(max) null",
                        SchemaName, TabInfo.renamed_tab_name,
                        col_info->renamed_col_name);
                } else {
                    sprintf(
                        TextBuf,
                        "alter table %s.%s alter column %s nvarchar(%d) null",
                        SchemaName, TabInfo.renamed_tab_name,
                        col_info->renamed_col_name, enc_len);
                }
            } else {
            ...
            if (TabInfo.enc_type == 0) {
                if (!strcasecmp(col_info->data_type, "numeric")) {
                    if (col_info->data_precision == 0) {
                        ...
                        sprintf(TextBuf,
                                "alter table %s.%s alter column %s %s(%d,%d) "
                                "not null",
                                SchemaName, TabInfo.renamed_tab_name,
                                col_info->col_name, col_info->data_type,
                                col_info->data_precision, col_info->data_scale);
                    ...
                    sprintf(TextBuf,
                            "alter table %s.%s alter column %s %s(%d) not null",
                            SchemaName, TabInfo.renamed_tab_name,
                            col_info->col_name, col_info->data_type,
                            col_info->data_length);
                } else if (col_info->data_length == -1) {
                    sprintf(
                        TextBuf,
                        "alter table %s.%s alter column %s %s(max) not null",
                        SchemaName, TabInfo.renamed_tab_name,
                        col_info->col_name, col_info->data_type);
                } else {
                    sprintf(TextBuf,
...
PccTdsScriptBuilder::PccTdsScriptBuilder(DgcDatabase* db, DgcSession* sess,
...
dgt_sint32 PccTdsScriptBuilder::getTablespace(DgcMemRows* rtn_rows) throw(
    DgcExcept) {
    return 0;
}
...
dgt_sint32 PccTdsScriptBuilder::buildScript(
...
dgt_sint32 PccTdsScriptBuilder::buildScriptMig(
    dgt_sint64 enc_tab_id, dgt_uint16 version_no) throw(DgcExcept) {
    return 0;
}

dgt_sint32 PccTdsScriptBuilder::runVerifyMig(
    dgt_sint64 enc_tab_id, pct_type_verify_job* job_row_ptr) throw(DgcExcept) {
    return 0;
}

dgt_sint32 PccTdsScriptBuilder::migInsertSql(
    dgt_sint64 it_tab_id, dgt_uint8 gen_flag) throw(DgcExcept) {
    return 0;
}

dgt_sint32 PccTdsScriptBuilder::buildScriptAddCol(
    dgt_sint64 enc_tab_id, dgt_uint16 version_no) throw(DgcExcept) {
    VersionNo = version_no;
    if (prepareTabInfo(enc_tab_id) < 0)
        ATHROWnR(DgcError(SPOS, "prepareTabInfo failed."), -1);
...
dgt_sint32 PccTdsScriptBuilder::buildScriptColAdmin(
    dgt_sint64 enc_tab_id, dgt_uint16 version_no) throw(DgcExcept) {
#if 0
        VersionNo=version_no;
...
dgt_sint32 PccTdsScriptBuilder::checkDB(dgt_sint64 db_agent_id,
...
dgt_sint32 PccTdsScriptBuilder::setCharset(dgt_sint64 db_agent_id) throw(
    DgcExcept) {
    return 0;
}

dgt_sint32 PccTdsScriptBuilder::agentTest(
    dgt_sint64 db_agent_id, DgcMemRows* rtn_rows) throw(DgcExcept) {
    return 0;
}

dgt_sint32 PccTdsScriptBuilder::agentTableTest(
    dgt_sint64 db_agent_id, DgcMemRows* rtn_rows) throw(DgcExcept) {
    return 0;
}

dgt_sint32 PccTdsScriptBuilder::buildInstallScript(
    ...
    memset(sql_text, 0, 1024);
    sprintf(sql_text,
            "select a.db_name from pt_database a, pct_db_agent b where a.db_id "
            "= b.db_id and db_agent_id=%lld",
            agent_id);
    DgcSqlStmt* sql_stmt =
        Database->getStmt(Session, sql_text, strlen(sql_text));
    if (sql_stmt == 0 || sql_stmt->execute() < 0) {
    ...
    if (dg_strcmp(db_name, "master") == 0) {
        *TextBuf = 0;
        sprintf(TextBuf,
                "CREATE LOGIN [%s] WITH PASSWORD=N\'%s\', DEFAULT_DATABASE "
                "=[master]",
                agent_uid, agent_pass);
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    }

    *TextBuf = 0;
    sprintf(TextBuf, "USE [%s]", db_name);
    ...
    *TextBuf = 0;
    sprintf(
        TextBuf,
        "GO\nCREATE USER [petra] FOR LOGIN [%s] WITH DEFAULT_SCHEMA=[petra]",
        agent_uid);
    if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    *TextBuf = 0;
...
// grant permission (sysadmin or not)
    ...
    if (dg_strcmp(db_name, "master") == 0) {
        *TextBuf = 0;
        sprintf(TextBuf, "ALTER SERVER ROLE [sysadmin] ADD MEMBER [%s]",
                agent_uid);
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    }
    ...
    *TextBuf = 0;
    sprintf(TextBuf, "CREATE ROLE [db_petra]");
    if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    *TextBuf = 0;
    sprintf(TextBuf, "GRANT CONNECT TO [db_petra]");
    if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    *TextBuf = 0;
    sprintf(TextBuf, "GRANT CREATE SYNONYM TO [db_petra]");
    if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    *TextBuf = 0;
    sprintf(TextBuf, "GRANT CREATE TYPE TO [db_petra]");
    if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    *TextBuf = 0;
    sprintf(TextBuf, "GRANT CREATE TABLE TO [db_petra]");
    if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    *TextBuf = 0;
    sprintf(TextBuf, "GRANT CREATE VIEW TO [db_petra]");
    if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    *TextBuf = 0;
    sprintf(TextBuf, "GRANT CREATE PROCEDURE TO [db_petra]");
    if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    *TextBuf = 0;
    sprintf(TextBuf, "GRANT ALTER TO [db_petra]");
    if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    *TextBuf = 0;
    sprintf(TextBuf, "GRANT REFERENCES TO [db_petra]");
    if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    *TextBuf = 0;
    sprintf(TextBuf, "GRANT ALTER ANY SCHEMA TO [db_petra]");
    if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    *TextBuf = 0;
    sprintf(TextBuf, "GRANT ALTER ANY DATABASE DDL TRIGGER TO [db_petra]");
    if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    *TextBuf = 0;
    sprintf(TextBuf, "GRANT DELETE TO [db_petra]");
    if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    *TextBuf = 0;
    sprintf(TextBuf, "GRANT INSERT TO [db_petra]");
    if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    *TextBuf = 0;
    sprintf(TextBuf, "GRANT UPDATE TO [db_petra]");
    if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    *TextBuf = 0;
    sprintf(TextBuf, "GRANT SELECT TO [db_petra]");
    if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    *TextBuf = 0;
    sprintf(TextBuf, "GRANT EXECUTE TO [db_petra]");
    if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    *TextBuf = 0;
    sprintf(TextBuf, "ALTER ROLE [db_petra] ADD MEMBER [PETRA]");
    if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
#endif
    ...
    if (dg_strcmp(db_name, "master") == 0) {
        *TextBuf = 0;
        sprintf(TextBuf, "exec sp_configure \'clr enabled\' ,1");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(TextBuf, "RECONFIGURE");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(TextBuf, "alter database master set trustworthy on");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(
            TextBuf,
            "create assembly petra_cipher AUTHORIZATION [petra] "
            "from 'C:\\Program Files\\sinsiway\\petra\\api\\PcaSqlServer.dll' "
            "with permission_set = UNSAFE");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(TextBuf, "GRANT VIEW SERVER STATE TO public");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    }
    ...
    // master     : Create Enc/Dec Function & Grant Execute on Function to
    // PUBLIC not master : Create Synonym
    StepNo = 2;
    StmtNo = 0;
    ...
    if (dg_strcmp(db_name, "master") == 0) {
        *TextBuf = 0;
        sprintf(
            ...
            "as external name "
            "petra_cipher.\"sqlserver.StoredProcedures\".pls_encrypt_bin_b64");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(
            ...
            "as external name "
            "petra_cipher.\"sqlserver.StoredProcedures\".pls_decrypt_bin_b64");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(TextBuf,
                ...
                "as external name "
                "petra_cipher.\"sqlserver.StoredProcedures\".pls_encrypt_b64");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(TextBuf,
                ...
                "ReTURNS nvarchar(368) "
                "as external name "
                "petra_cipher.\"sqlserver.StoredProcedures\".pls_decrypt_b64 ");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(TextBuf,
        ...
        *TextBuf = 0;
        sprintf(TextBuf, "GRANT EXECUTE ON petra.pls_encrypt_b64 TO PUBLIC");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(TextBuf, "GRANT EXECUTE ON petra.pls_decrypt_b64 TO PUBLIC");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(TextBuf,
                "GRANT EXECUTE ON petra.pls_encrypt_bin_b64 TO PUBLIC");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(TextBuf,
                "GRANT EXECUTE ON petra.pls_decrypt_bin_b64 TO PUBLIC");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(TextBuf,
                "GRANT EXECUTE ON petra.pls_encrypt_b64_max TO PUBLIC");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(TextBuf,
                "GRANT EXECUTE ON petra.pls_decrypt_b64_max TO PUBLIC");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    } else {  // not master
        *TextBuf = 0;
        sprintf(
            TextBuf,
            "CREATE SYNONYM PLS_ENCRYPT_B64 FOR master.petra.pls_encrypt_b64");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(
            TextBuf,
            "CREATE SYNONYM PLS_DECRYPT_B64 FOR master.petra.pls_decrypt_b64");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(TextBuf,
                "CREATE SYNONYM PLS_ENCRYPT_BIN_B64 FOR "
                "master.petra.pls_encrypt_bin_b64");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(TextBuf,
                "CREATE SYNONYM PLS_DECRYPT_BIN_B64 FOR "
                "master.petra.pls_decrypt_bin_b64");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(TextBuf,
                "CREATE SYNONYM PLS_ENCRYPT_B64_MAX FOR "
                "master.petra.pls_encrypt_b64_max");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(TextBuf,
                "CREATE SYNONYM PLS_DECRYPT_B64_MAX FOR "
                "master.petra.pls_decrypt_b64_max");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    }
    ...
    *TextBuf = 0;
    ...
    if (dg_strcmp(db_name, "master") == 0) {
        *TextBuf = 0;
        sprintf(TextBuf, "DROP function  petra.pls_encrypt_b64");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(TextBuf, "DROP function  petra.pls_decrypt_b64");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(TextBuf, "DROP function  petra.pls_encrypt_bin_b64");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(TextBuf, "DROP function  petra.pls_decrypt_bin_b64");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(TextBuf, "DROP function  petra.pls_encrypt_b64_max");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(TextBuf, "DROP function  petra.pls_decrypt_b64_max");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    } else {
        *TextBuf = 0;
        sprintf(TextBuf, "drop synonym PETRA.PLS_ENCRYPT_B64");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(TextBuf, "drop synonym PETRA.PLS_DECRYPT_B64");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(TextBuf, "drop synonym PETRA.PLS_ENCRYPT_BIN_B64");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(TextBuf, "drop synonym PETRA.PLS_DECRYPT_BIN_B64");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(TextBuf, "drop synonym PETRA.PLS_ENCRYPT_B64_MAX");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(TextBuf, "drop synonym PETRA.PLS_DECRYPT_B64_MAX");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
        *TextBuf = 0;
        sprintf(TextBuf, "drop assembly petra_cipher");
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    }
    ...
    *TextBuf = 0;
    sprintf(TextBuf, "drop schema petra");
    if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    *TextBuf = 0;
    sprintf(TextBuf, "drop user petra");
    if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);

    if (dg_strcmp(db_name, "master") == 0) {
        *TextBuf = 0;
        sprintf(TextBuf, "drop login %s", agent_uid);
        if (saveSqlText() < 0)
            ATHROWnR(DgcError(SPOS, "saveSqlText failed."), -1);
    }
