		...
		if (idx_info->index_type == 1) {
			if (TabInfo.enc_type == 0 && idx_info->normal_idx_flag == 1) {
				...
				if (!strcasecmp(idx_info->data_type,"NUMBER")) {
                                        sprintf(idx_sql,"create index %s.%s on %s.%s(%s) indextype is %s.PC_IDX1_TYP2",
                                                        SchemaName,idx_info->domain_index_name,
                                                        SchemaName,TabInfo.table_name,idx_info->column_name,
                                                        AgentName);
                                } else if (!strcasecmp(idx_info->data_type,"DATE") || !strcasecmp(idx_info->data_type,"TIMESTAMP")) {
                                        sprintf(idx_sql,"create index %s.%s on %s.%s(%s) indextype is %s.PC_IDX1_TYP3",
                                                        SchemaName,idx_info->domain_index_name,
	...
	if (TabInfo.tran_trg_flag == 1 && TranIdxColRows.numRows() > 0) {
		...
		*TmpBuf=0;
		...
		while (ColInfoRows.next() && (col_info=(pc_type_col_info*)ColInfoRows.data())) {
			*TmpBuf=0;
			if (col_info->status == 0 && !strcasecmp(col_info->data_type,"LONG")) continue;
			if (col_info->status == 1) sprintf(TmpBuf,"%s %s,",getFname(col_info->enc_col_id,1),col_info->col_name);
		...
		while (ColInfoRows.next() && (col_info=(pc_type_col_info*)ColInfoRows.data())) {
                        if (col_info->status == 1 && col_info->index_type == 2) {
                        	*TmpBuf=0;
				sprintf(TmpBuf,"%s %s,",getFname(col_info->enc_col_id,3),col_info->index_col_name);
	                        strcat(TextBuf,TmpBuf);
			}
                }
#endif
...
dgt_sint32 PccTiberoScriptBuilder::buildInstallScript(
	...
	*TextBuf=0;
	sprintf(TextBuf, "GRANT DBA TO %s", agent_uid);
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0;
	sprintf(TextBuf,"CONNECT %s/%s",agent_uid, agent_pass);
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0;
	sprintf(TextBuf,"CREATE LIBRARY PT_CIPHER_LIB IS \n'&TB_HOME/client/lib/libPcaTibero.so';/");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0;
	sprintf(TextBuf,
"CREATE OR REPLACE FUNCTION \nPLS_ENCRYPT_RAW_ID(data varchar2,enc_col_id pls_integer) "
"\nRETURN RAW deterministic "
"\nAS LANGUAGE C "
"\nLIBRARY PT_CIPHER_LIB "
"\nNAME ""EXT_ENC_RAW"" "
"\nPARAMETERS(CONTEXT, "
	 " \ndata string, "
	 " \ndata LENGTH, "
...
"\n/ ");
	...
	*TextBuf=0;
	sprintf(TextBuf,
"CREATE OR REPLACE FUNCTION \nPLS_ENCRYPT_RAW_ID_RAW(data RAW,enc_col_id pls_integer) "
"\nRETURN RAW deterministic "
"\nAS LANGUAGE C "
"\nLIBRARY PT_CIPHER_LIB "
"\nNAME ""EXT_ENC_RAW_RAW"" "
"\nPARAMETERS(CONTEXT, "
         " \ndata, "
         " \ndata LENGTH, "
         " \nenc_col_id int, "
...
"\n/ ");
	...
	*TextBuf=0;
	sprintf(TextBuf,
"CREATE OR REPLACE FUNCTION \nPLS_ENCRYPT_RAW_ID_DATE(data DATE,enc_col_id pls_integer) "
"\nRETURN RAW deterministic "
"\nAS "
"\nBEGIN "
...
"\n/ ");
	...
	*TextBuf=0;
	sprintf(TextBuf,
"CREATE OR REPLACE FUNCTION \nPLS_DECRYPT_RAW_ID(data RAW,enc_col_id pls_integer) "
"\nRETURN varchar2 deterministic "
"\nAS LANGUAGE C "
"\nLIBRARY PT_CIPHER_LIB "
"\nNAME ""EXT_DEC_RAW"" "
"\nPARAMETERS(CONTEXT,data,data LENGTH,enc_col_id int,RETURN LENGTH ); "
"\n/ ");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);

	*TextBuf=0;
	sprintf(TextBuf,
"CREATE OR REPLACE FUNCTION \nPLS_DECRYPT_RAW_ID_RAW(data RAW,enc_col_id pls_integer) "
"\nRETURN RAW deterministic "
"\nAS LANGUAGE C "
"\nLIBRARY PT_CIPHER_LIB "
"\nNAME ""EXT_DEC_RAW_RAW"" "
"\nPARAMETERS(CONTEXT,data , data LENGTH, enc_col_id int,RETURN LENGTH ); "
"\n/ ");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	...
	*TextBuf=0;
	sprintf(TextBuf,
"\nCREATE OR REPLACE FUNCTION \nPLS_DECRYPT_RAW_ID_NUM(data RAW,enc_col_id pls_integer) "
...
"\n/ ");
	...
	*TextBuf=0;
	sprintf(TextBuf,
"CREATE OR REPLACE FUNCTION \nPLS_DECRYPT_RAW_ID_DATE(data RAW,enc_col_id pls_integer) "
...
"\n/ ");
	...
	*TextBuf=0;
	sprintf(TextBuf,
"CREATE OR REPLACE FUNCTION \nPLS_ENCRYPT_B64_ID(data varchar2,enc_col_id pls_integer) "
"\nRETURN varchar2 deterministic "
"\nAS LANGUAGE C "
"\nLIBRARY PT_CIPHER_LIB "
"\nNAME ""EXT_ENC_B64"" "
...
"\n/ ");
	...
	*TextBuf=0;
	sprintf(TextBuf,
"CREATE OR REPLACE FUNCTION \nPLS_ENCRYPT_B64_ID_C(data varchar2,enc_col_id pls_integer) "
"\nRETURN varchar2 deterministic"
"\nAS LANGUAGE C"
"\nLIBRARY PT_CIPHER_LIB"
"\nNAME ""EXT_ENC_B64_C"" "
...
"\n/");
	...
	*TextBuf=0;
	sprintf(TextBuf,
"CREATE OR REPLACE FUNCTION \nPLS_ENCRYPT_B64_ID_RAW(data RAW,enc_col_id pls_integer) "
"\nRETURN varchar2 deterministic "
"\nAS LANGUAGE C "
"\nLIBRARY PT_CIPHER_LIB "
"\nNAME ""EXT_ENC_B64"" "
...
"\n/ ");
	...
	*TextBuf=0;
	sprintf(TextBuf,
"CREATE OR REPLACE FUNCTION \nPLS_ENCRYPT_B64_ID_DATE(data DATE,enc_col_id pls_integer) "
"\nRETURN VARCHAR2 deterministic "
"\nAS "
"\nBEGIN "
...
"\n/ ");
	...
	*TextBuf=0;
	sprintf(TextBuf,
"CREATE OR REPLACE FUNCTION \nPLS_DECRYPT_B64_ID(data varchar2,enc_col_id pls_integer)"
"\nRETURN varchar2 deterministic "
"\nAS LANGUAGE C "
"\nLIBRARY PT_CIPHER_LIB "
"\nNAME ""EXT_DEC_B64"" "
"\nPARAMETERS(CONTEXT,data string,data LENGTH,enc_col_id int,RETURN LENGTH );"
"\n/ ");
        if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);

	*TextBuf=0;
	sprintf(TextBuf,
"CREATE OR REPLACE FUNCTION \nPLS_DECRYPT_B64_ID_RAW(data varchar2,enc_col_id pls_integer) "
"\nRETURN RAW deterministic "
"\nAS LANGUAGE C "
"\nLIBRARY PT_CIPHER_LIB "
"\nNAME ""EXT_DEC_B64"" "
...
"\n/ ");
	...
	*TextBuf=0;
	sprintf(TextBuf,
"CREATE OR REPLACE FUNCTION \nPLS_DECRYPT_B64_ID_NUM(data varchar2,enc_col_id pls_integer) "
"\nRETURN NUMBER deterministic "
"\nAS "
"\nBEGIN "
...
"\n/ ");
	...
	*TextBuf=0;
	sprintf(TextBuf,
"CREATE OR REPLACE FUNCTION \nPLS_DECRYPT_B64_ID_DATE(data varchar2,enc_col_id pls_integer) "
"\nRETURN DATE deterministic "
"\nAS "
"\nBEGIN "
...
"\nEND; ");
	...
	*TextBuf=0;
	sprintf(TextBuf,
"create or replace function \npls_encrypt_blob(data BLOB, enc_col_id number) "
"\nreturn blob "
"\nas "
	"\ntmp_blob	BLOB; "
	"\ntmp_buffer	raw(512); "
	"\namount 	BINARY_INTEGER := 512; "
	"\nret_buffer	raw(2000); "
	"\ndata_len	BINARY_INTEGER;	"
        "\nenc_len	BINARY_INTEGER; "
	"\nposition	BINARY_INTEGER; "
	"\nblob_position	BINARY_INTEGER; "
"\nbegin "
	"\nDBMS_LOB.CREATETEMPORARY(tmp_blob, false); "
	"\ndata_len := nvl(DBMS_LOB.getlength(data),0); "
	"\nif data_len = 0 then "
		"\ntmp_buffer := pls_encrypt_raw_id_raw(null,enc_col_id); "
		"\nreturn tmp_buffer; "
	"\nelsif data_len <= amount then "
		"\nDBMS_LOB.READ(data, data_len, 1, tmp_buffer); "
		"\nret_buffer := pls_encrypt_raw_id_raw(tmp_buffer,enc_col_id); "
        	"\nenc_len := utl_raw.length(ret_buffer); "
		"\nDBMS_LOB.WRITEAPPEND(tmp_blob, enc_len, ret_buffer); "
	"\nelse "
		"\nblob_position := 1; "
		"\nposition := 1; "
		"\nwhile position <= data_len   LOOP "
			"\nDBMS_LOB.READ(data, amount, position, tmp_buffer); "
			"\nret_buffer := pls_encrypt_raw_id_raw(tmp_buffer,enc_col_id); "
        		"\nenc_len := utl_raw.length(ret_buffer); "
			"\nDBMS_LOB.WRITEAPPEND(tmp_blob, enc_len, ret_buffer); "
                        "\nposition := position + amount; "
			"\nblob_position := blob_position + enc_len; "
		"\nEND LOOP; "
	"\nend if; "
	"\nreturn tmp_blob; "
"\nend; "
"\n/ ");
        ...
        *TextBuf=0;
        sprintf(TextBuf,
"create or replace function pls_encrypt_clob(data CLOB, enc_col_id number) "
"return clob "
"as "
        "tmp_clob       CLOB; "
        "tmp_buffer     varchar2(512); "
        "amount         BINARY_INTEGER := 512; "
        "ret_buffer     varchar2(2000); "
        "data_len       BINARY_INTEGER; "
        "enc_len        BINARY_INTEGER; "
        "position       BINARY_INTEGER; "
        "clob_position  BINARY_INTEGER; "
"begin "
        "DBMS_LOB.CREATETEMPORARY(tmp_clob, false); "
        "data_len := nvl(DBMS_LOB.getlength(data),0); "
        "if data_len = 0 then "
                "tmp_buffer := pls_encrypt_b64_id(null,enc_col_id); "
                "return tmp_buffer; "
        "elsif data_len <= amount then "
                "DBMS_LOB.READ(data, data_len, 1, tmp_buffer); "
                "ret_buffer := pls_encrypt_b64_id(tmp_buffer,enc_col_id); "
                "enc_len := length(ret_buffer); "
                "DBMS_LOB.WRITEAPPEND(tmp_clob, enc_len, ret_buffer); "
        "else "
                "clob_position := 1; "
                "position := 1; "
                "while position <= data_len   LOOP "
                        ...
                        "DBMS_LOB.WRITEAPPEND(tmp_clob, enc_len, ret_buffer); "
                        "position := position + amount; "
                        "clob_position := clob_position + enc_len; "
                "END LOOP; "
        "end if; "
        "return tmp_clob; "
"end; "
"/ ");
	...
	*TextBuf=0;
	sprintf(TextBuf,
"create or replace function pls_decrypt_blob(data BLOB, enc_col_id number) "
"return blob "
"as "
	"tmp_blob	BLOB; "
	"tmp_buffer	raw(516); "
	"amount 	BINARY_INTEGER := 516; "
	"ret_buffer	raw(2000); "
	"data_len	BINARY_INTEGER;	"
        "dec_len	BINARY_INTEGER; "
	"position	BINARY_INTEGER; "
	"blob_position	BINARY_INTEGER; "
"begin "
	"DBMS_LOB.CREATETEMPORARY(tmp_blob, false); "
	...
	"elsif data_len <= amount then "
		"DBMS_LOB.READ(data, data_len, 1, tmp_buffer); "
		"ret_buffer := pls_decrypt_raw_id_raw(tmp_buffer,enc_col_id); "
        	"dec_len := utl_raw.length(ret_buffer); "
		"DBMS_LOB.WRITEAPPEND(tmp_blob, dec_len, ret_buffer); "
	"else "
		"blob_position := 1; "
		"position := 1; "
		"while position <= data_len   LOOP "
			...
			"ret_buffer := pls_decrypt_raw_id_raw(tmp_buffer,enc_col_id); "
        		"dec_len := utl_raw.length(ret_buffer); "
			"DBMS_LOB.WRITEAPPEND(tmp_blob, dec_len, ret_buffer); "
			"blob_position := blob_position + dec_len; "
			"position := position + amount; "
		"END LOOP; "
	"end if; "
	"return tmp_blob; "
...
"/ ");
        ...
        *TextBuf=0;
        sprintf(TextBuf,
"create or replace function pls_decrypt_clob(data CLOB, enc_col_id number) "
"return clob "
"as "
        "tmp_clob       CLOB; "
        "tmp_buffer     varchar2(689); "
        "amount         BINARY_INTEGER := 689; "
        "ret_buffer     varchar2(2000); "
        "data_len       BINARY_INTEGER; "
        "dec_len        BINARY_INTEGER; "
        "position       BINARY_INTEGER; "
...
"begin "
        ...
        "elsif data_len <= amount then "
                "DBMS_LOB.READ(data, data_len, 1, tmp_buffer); "
                "ret_buffer := pls_decrypt_b64_id(tmp_buffer,enc_col_id); "
                "dec_len := length(ret_buffer); "
                "DBMS_LOB.WRITEAPPEND(tmp_clob, dec_len, ret_buffer); "
        "else "
                "clob_position := 1; "
                "position := 1; "
                "while position <= data_len   LOOP "
                        ...
                        "dec_len := length(ret_buffer); "
                        "DBMS_LOB.WRITEAPPEND(tmp_clob, dec_len, ret_buffer); "
                        "clob_position := clob_position + dec_len; "
                        "position := position + amount; "
                "END LOOP; "
        "end if; "
        "return tmp_clob; "
...
"/ ");
	...
	//
	// Grant Execute on Function to PULBIC
	//
	*TextBuf=0; sprintf(TextBuf,"grant execute on pls_encrypt_raw_id to public;");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"grant execute on pls_encrypt_raw_id_date to public;");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"grant execute on pls_encrypt_b64_id to public;");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"grant execute on pls_encrypt_b64_id_c to public;");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"grant execute on pls_encrypt_b64_id_date to public;");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"grant execute on pls_decrypt_raw_id to public;");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"grant execute on pls_decrypt_raw_id_num to public;");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"grant execute on pls_decrypt_raw_id_date to public;");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"grant execute on pls_decrypt_raw_id_raw to public;");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"grant execute on pls_decrypt_b64_id to public;");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"grant execute on pls_decrypt_b64_id_num to public;");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"grant execute on pls_encrypt_b64_id_date to public;");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"grant execute on pls_encrypt_b64_id_raw to public;");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"grant execute on pls_encrypt_blob to public;");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"grant execute on pls_decrypt_blob to public;");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"grant execute on pls_encrypt_clob to public;");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"grant execute on pls_decrypt_clob to public;");
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	...
	*TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_DECRYPT_RAW_ID FOR %s.PLS_DECRYPT_RAW_ID;",agent_uid);
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_DECRYPT_RAW_NUM FOR %s.PLS_DECRYPT_RAW_NUM;",agent_uid);
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_DECRYPT_RAW_DATE FOR %s.PLS_DECRYPT_RAW_DATE;",agent_uid);
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	...
	*TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_DECRYPT_B64_ID_RAW FOR %s.PLS_DECRYPT_B64_ID_RAW;",agent_uid);
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_ENCRYPT_BLOB FOR %s.PLS_DECRYPT_BLOB;",agent_uid);
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
	*TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_DECRYPT_BLOB FOR %s.PLS_DECRYPT_BLOB;",agent_uid);
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);

	*TextBuf=0; sprintf(TextBuf,"CREATE OR REPLACE PUBLIC SYNONYM PLS_ENCRYPT_CLOB FOR %s.PLS_DECRYPT_CLOB;",agent_uid);
	if (saveSqlText() < 0) ATHROWnR(DgcError(SPOS,"saveSqlText failed."),-1);
