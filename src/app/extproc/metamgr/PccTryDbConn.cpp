...
PccTryDbConn::PccTryDbConn(const dgt_schar* name)
...
DgcExtProcedure* PccTryDbConn::clone()
{
	return new PccTryDbConn(procName());
}
...
dgt_sint32 PccTryDbConn::execute() throw(DgcExcept)
{
	...
	for (;;) {
		if (in->dbms_type == DGC_DB_TYPE_ORACLE) {
			...
			if (conn->connect(conn_string, nul, in->user, in->password, 0) != 0) {
				e = EXCEPTnC;
			}
			break;
			...
			if (conn->connect(nul, host, in->user, in->password, in->db_name) != 0) {
				e = EXCEPTnC;
			}
			break;
		} else if (in->dbms_type == DGC_DB_TYPE_SYBASEIQ || in->dbms_type == DGC_DB_TYPE_SYBASEASE) {
			// tds connection
			...
			sprintf(conn_string,"host=%s;service=%d;database=%s;protocol=onsoctcp;server=%s;uid=%s;pwd=%s;CLIENT_LOCALE=en_us.utf8",
				in->ip_addr, in->port, in->db_name, in->instance_name, in->user, in->password);

			if (conn->connect(conn_string, nul,in->user, in->password, nul) != 0) {
				e = EXCEPTnC;
			}
			break;
#endif
			...
			sprintf(conn_string, "DATABASE=%s;HOSTNAME=%s;PORT=%d;UID=%s;PWD=%s;PROTOCOL=TCPIP;",
			...
			sprintf(conn_string,"DSN=%s;UID=%s;PWD=%s;CONNTYPE=1;PORT_NO=%d;NLS_USE=UTF8;",
			...
			if (conn->connect(conn_string, in->ip_addr, in->user, in->password, in->db_name) != 0) {
				e = EXCEPTnC;
			}
			break;
			...
			sprintf(conn_string, "PROTOCOL=TCP;HOST=%s;PORT=%d;UID=%s;PWD=%s;", in->ip_addr, in->port, in->user, in->password);
			if (conn->connect(conn_string, nul, in->user, in->password, nul) != 0) {
				e = EXCEPTnC;
			}
			break;
	        ...
	        if (!conn) break;
	        /*
	        sprintf(conn_string, "DSN=399;UID=%s;PWD=%s;", in->user, in->password);
			if (conn->connect(conn_string, nul, in->user, in->password, nul) != 0) {
				e = EXCEPTnC;
			}
			*/
			...
			sprintf(out_param.result_msg,"[not supported dbms_type[%d]]",in->dbms_type);
	...
	if (!conn) {
		...
		ReturnRows->next();
		out_param.result_code = -1;
		sprintf(out_param.result_msg,"[connection was not allocated : db_type[%d]]",in->dbms_type);
		memcpy(ReturnRows->data(),&out_param,sizeof(pc_type_try_db_conn_out));
		ReturnRows->rewind();
	} else {
		ReturnRows->reset();
