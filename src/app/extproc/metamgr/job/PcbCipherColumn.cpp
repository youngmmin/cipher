...
{
	dgt_schar	sql_txt[256]={0,};

	//
	// get encrypt column info
	//
	sprintf(sql_txt,"select * from pct_enc_column where enc_col_id=%lld",EncColumnID);
	DgcSqlStmt*	sql_stmt=DgcDbProcess::db().getStmt(DgcDbProcess::sess(), sql_txt, strlen(sql_txt));
	if (!sql_stmt) {
	...
	// 
	sprintf(sql_txt,"select count() from pct_enc_index where enc_col_id=%lld and index_type =1",EncColumnID);
        sql_stmt=DgcDbProcess::db().getStmt(DgcDbProcess::sess(), sql_txt, strlen(sql_txt));
        if (!sql_stmt) {
                ATHROWnR(DgcError(SPOS,"getStmt failed"),-1);
        }
        if (sql_stmt->execute() >= 0) {
	...
	//
	sprintf(sql_txt,"select b.* from pct_enc_column a, pct_encrypt_key b where a.key_id=b.key_id and a.enc_col_id=%lld",EncColumnID);
	sql_stmt=DgcDbProcess::db().getStmt(DgcDbProcess::sess(), sql_txt, strlen(sql_txt));
	if (!sql_stmt) {
		ATHROWnR(DgcError(SPOS,"getStmt failed"),-1);
	}
	...
	delete sql_stmt;
	sql_stmt=0;
	if (e) RTHROWnR(e,DgcError(SPOS,"PCT_ENCRYPT_KEY[%lld] execute/fetch failed",EncColumnID), -1);
	...
	if ((rtn=PCI_initContext(&CipherContext, Key, EncKey.key_size,
				...
				EncKey.b64_txt_enc_flag, EncKey.enc_start_pos, EncKey.enc_length,DomainIndexFlag)) < 0) {
                THROWnR(DgcLdbExcept(DGC_EC_LD_STMT_ERR,
...
dgt_sint32 PcbCipherColumn::initializeIndexColumns(const pct_type_enc_column*	enc_column,const pct_type_encrypt_key* enc_key,const PCI_Context* cipher_context) throw(DgcExcept)
{
...
dgt_uint8 PcbCipherColumn::colType()
{
	if (!strcasecmp(EncColumn.data_type,"VARCHAR") ||
	    ...
	    !strcasecmp(EncColumn.data_type,"CHAR")) return PCB_COL_TYPE_CHR;
	else if (!strcasecmp(EncColumn.data_type,"NUMBER") ||
			!strcasecmp(EncColumn.data_type,"LONG") ||
			!strcasecmp(EncColumn.data_type,"SHORT")) return PCB_COL_TYPE_NUM;
	else if (!strcasecmp(EncColumn.data_type,"DATE")) return PCB_COL_TYPE_DATE;
	else if (!strcasecmp(EncColumn.data_type,"RAW")) return PCB_COL_TYPE_BIN;
...
dgt_sint32 PcbCipherColumn::encrypt(PcbDataColumn* data_column) throw(DgcExcept)
{
	dgt_sint32	rtn;
	dgt_sint32	src_len;
	dgt_uint32	dst_len;
	for(dgt_uint32 rno=0; rno<data_column->numRows(); rno++) {
		...
		if (DomainIndexFlag == 1) {
				...
				(dgt_uint8*)data_column->colData(rno), src_len,
                                (dgt_uint8*)data_column->encColData(rno), &ophDstLen, EncColumnID, PCI_SRC_TYPE_CHAR,0,1)) < 0) {
				THROWnR(DgcLdbExcept(DGC_EC_LD_STMT_ERR,
                                        new DgcError(SPOS,"PCI_OPHUEK[%lld] failed due to %d:%s",EncColumnID, rtn, PCI_getErrMsg(&CipherContext))),-1);
			}
                        dgt_uint32 encDstLen=dst_len;
                        if ((rtn=PCI_encrypt(&CipherContext, 
				(dgt_uint8*)data_column->colData(rno), src_len, 
				(dgt_uint8*)data_column->encColData(rno)+ophDstLen, &encDstLen, (dgt_uint8)ophDstLen)) < 0) {
				THROWnR(DgcLdbExcept(DGC_EC_LD_STMT_ERR,
                                        new DgcError(SPOS,"PCI_encrypt[%lld] failed due to %d:%s",EncColumnID, rtn, PCI_getErrMsg(&CipherContext))),-1);
			}
			*(data_column->encColLen(rno))=(dgt_uint16)ophDstLen+encDstLen;
		} else {
			if ((rtn=PCI_encrypt(&CipherContext,
					(dgt_uint8*)data_column->colData(rno), src_len,
					(dgt_uint8*)data_column->encColData(rno), &dst_len)) < 0) {
                		THROWnR(DgcLdbExcept(DGC_EC_LD_STMT_ERR,
					new DgcError(SPOS,"PCI_encrypt[%lld] failed due to %d:%s",EncColumnID, rtn, PCI_getErrMsg(&CipherContext))),-1);
			}
...
dgt_sint32 PcbCipherColumn::decrypt(PcbDataColumn* data_column,dgt_uint8 verify_flag) throw(DgcExcept)
{
	dgt_sint32	rtn;
	dgt_sint32	src_len;
	dgt_uint32	dst_len;
	...
	for(dgt_uint32 rno=0; rno<data_column->numRows(); rno++) {
		...
		if (DomainIndexFlag == 1 && src_len) {
			dgt_sint32 ophLen=0;
			dgt_uint8* b64_tailer = (dgt_uint8*)data_column->encColData(rno) + src_len - 4; // 4 = PCC_ENC_TRAILER_LENGTH;
                        dgt_uint8  trailer[3] = {0,0,0};
                        DgcBase64::decode((dgt_schar*)b64_tailer, 4, trailer, 3);
			if (trailer[2]) {
				ophLen=trailer[2];
				if ((rtn=PCI_decrypt(&CipherContext,
						(dgt_uint8*)data_column->encColData(rno)+ophLen, src_len-ophLen, tmp_buf, &dst_len)) < 0) {
					if(verify_flag){
						//
						//although PCI_decrypt failed, process must be go on while a verification.
						//
						...
						delete tmp_buf;
						THROWnR(DgcLdbExcept(DGC_EC_LD_STMT_ERR,
						new DgcError(SPOS,"PCI_decrypt[%lld] failed due to %d:%s",EncColumnID, rtn, PCI_getErrMsg(&CipherContext))),-1);
					}
				}
			} else {
				if ((rtn=PCI_decrypt(&CipherContext,
                                        (dgt_uint8*)data_column->encColData(rno), src_len, tmp_buf, &dst_len)) < 0) {
	                                if(verify_flag){
        	                                //
                	                        //although PCI_decrypt failed, process must be go on while a verification.
                        	                //
		...
		if (dst_len > data_column->maxColLen()) *(data_column->colLen(rno))=data_column->maxColLen();
		else *(data_column->colLen(rno))=(dgt_uint16)dst_len;
		memcpy(data_column->colData(rno), tmp_buf, *(data_column->colLen(rno)));
		if (dst_len) *data_column->colInd(rno)=0;
		else *data_column->colInd(rno)=1;
	}
	delete tmp_buf;
	return 0;
}
...
dgt_uint32 PcbCipherColumn::ophuekLength(dgt_sint32 src_length, dgt_uint8 src_type)
{
	dgt_sint32 src_len=0;

	//max column length
	if(src_type == PCI_SRC_TYPE_NUM) src_len=22;
	else if(src_type == PCI_SRC_TYPE_DATE) src_len=7;
	else src_len=src_length;
...
dgt_sint32 PcbCipherColumn::ophuek(PcbDataColumn* index_data_column, PcbDataColumn* data_column) throw(DgcExcept)
{
	dgt_sint32	rtn;
	dgt_sint32	src_len;
	dgt_uint32	dst_len;
...
if(rno==0){
	...
	printf("################################\n");
		...
		if ((rtn=PCI_OPHUEK(&CipherContext,
				(dgt_uint8*)data_column->colData(rno), src_len,
				(dgt_uint8*)index_data_column->encColData(rno), &dst_len, EncColumnID, col_type,0,0)) < 0) {
                	THROWnR(DgcLdbExcept(DGC_EC_LD_STMT_ERR,
				new DgcError(SPOS,"PCI_OPHUEK[%lld] failed due to %d:%s",EncColumnID, rtn, PCI_getErrMsg(&CipherContext))),-1);
		}
