...
class PcbCipherTable : public DgcObject {
   private:
    ...
    dgt_uint8
        DecryptFlag;  // decrypt flag 0:encrypt / 1:decrypt / 2:null update
   ...
   public:
    ...
    inline PcbCipherColumn* cipherColumn(dgt_uint16 idx) {
        if (idx < (NumColumns + NumIndexes))
            return CipherColumns[idx];
        else
            return 0;
    };
    ...
    //
    // get encryption table info, schema table, schema link info, create
    // PcbCipherColumns
    //
