...
PcbCollector::PcbCollector(PcbSelectStmt* select_stmt,
                           PcbDataChunkPool* chunk_pool,
                           pct_type_worker* worker_ptr)
    : PcbWorker(chunk_pool, worker_ptr), SelectStmt(select_stmt) {
    strncpy(WorkerPtr->assigned_role, "collector", 10);
}

PcbCollector::~PcbCollector() { delete SelectStmt; }

dgt_sint32 PcbCollector::run() throw(DgcExcept) {
    dgt_sint32 rtn;
    PcbDataChunk* data_chunk;
    while ((data_chunk = ChunkPool->getEmptyChunk()) &&
        ...
        if ((rtn = SelectStmt->fetch(data_chunk)) < 0) {
            ...
            if (etime.tv_sec == stime.tv_sec)
                WorkerPtr->working_time += etime.tv_usec - stime.tv_usec;
            else
                WorkerPtr->working_time +=
                    (dgt_sint64)(etime.tv_sec - stime.tv_sec) * 1000000 +
