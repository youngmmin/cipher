...
class PcbDataColumn : public DgcObject {
   private:
    ...
    dgt_schar*
        ColData;  // the array of column data with the size of "MaxColLen"
    dgt_sint16* ColInd;     // the array of the null indicator of column data
    dgt_uint16* ColLen;     // the array of the length of column data
    dgt_schar* EncColData;  // the array of encrypted column data with the size
                            // of "MaxEncColLen"
   ...
   protected:
   public:
    PcbDataColumn(dgt_uint32 max_rows, dgt_uint16 max_col_len,
                  dgt_uint16 max_enc_col_len = 0)
        : MaxRows(max_rows),
    ...
    inline dgt_schar* colData(dgt_uint32 row_no = 0) {
        return ColData + row_no * MaxColLen;
    };
    inline dgt_sint16* colInd(dgt_uint32 row_no = 0) {
        return ColInd + row_no;
    };
    inline dgt_uint16* colLen(dgt_uint32 row_no = 0) {
        return ColLen + row_no;
    };
    inline dgt_schar* encColData(dgt_uint32 row_no = 0) {
        if (EncColData) return EncColData + row_no * MaxEncColLen;
        return 0;
    };
    inline dgt_uint16* encColLen(dgt_uint32 row_no = 0) {
        if (EncColLen) return EncColLen + row_no;
        return 0;
    };
    ...
    inline dgt_uint32 putData(const dgt_uint32 num_rows, dgt_schar* col_data,
                              dgt_sint16* col_ind, dgt_uint16* col_len) {
    ...
    inline dgt_uint32 putData(const dgt_schar* col_data, dgt_sint16 col_ind,
                              dgt_uint16 col_len) {
        if (NumRows >= MaxRows) return 0;
        if (col_len >= MaxColLen) col_len = MaxColLen - 1;
        if (col_len != 0)
            strncpy(ColData + NumRows * MaxColLen, col_data, col_len);
        ColInd[NumRows] = col_ind;
        ColLen[NumRows] = col_len;
        return NumRows++;
    };

    inline dgt_uint32 putEncData(const dgt_uint32 num_rows, dgt_schar* col_data,
                                 dgt_sint16* col_ind, dgt_uint16* col_len) {
        if (num_rows > MaxRows) return 0;
    ...
    inline dgt_uint32 putEncData(const dgt_schar* col_data, dgt_sint16 col_ind,
                                 dgt_uint16 col_len) {
        if (NumRows >= MaxRows) return 0;
        if (col_len >= MaxEncColLen) col_len = MaxEncColLen - 1;
        if (col_len != 0)
            strncpy(EncColData + NumRows * MaxEncColLen, col_data, col_len);
        ColInd[NumRows] = col_ind;
...
class PcbDataChunk : public DgcObject {
   private:
    dgt_uint16 ChunkID;  // chunk ID, which is the descriptor of the chunk array
                         // in the ChunkPool
   ...
   public:
    ...
    static const dgt_uint8 PCB_CHUNK_STAT_LOADING =
        2;  // after a collector gets it
    static const dgt_uint8 PCB_CHUNK_STAT_LOADED =
        3;  // after the collector returns it
    static const dgt_uint8 PCB_CHUNK_STAT_UPDATING =
        4;  // after updater gets it

    PcbDataChunk(dgt_uint16 chunk_id, dgt_uint32 max_rows,
                 dgt_uint16 num_columns, dgt_uint16 num_idxcolumns = 0);
    ...
    inline PcbDataColumn* dataColumn(dgt_uint32 col_no) {
        if (col_no < NumColumns + NumIdxColumns) return DataColumns[col_no];
        return 0;
    };
    inline dgt_void reset() {
        for (dgt_uint16 i = 0; i < NumColumns; i++) DataColumns[i]->reset();
    };

    inline PcbDataChunk* setStat(dgt_uint8 set_stat,
                                 dgt_uint8 test_stat = PCB_CHUNK_STAT_NULL) {
    ...
    dgt_uint16 addColumn(dgt_uint16 max_col_len,
                         dgt_uint16 max_enc_col_len = 0);
    dgt_uint16 addIdxColumn(dgt_uint16 max_col_len,
                            dgt_uint16 max_enc_col_len = 0);
};
