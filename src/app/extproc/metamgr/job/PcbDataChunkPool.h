...
class PcbDataChunkPool : public DgcObject {
   ...
   public:
    ...
    inline PcbDataChunk* getEmptyChunk() {
        ...
        for (dgt_uint16 cid = 0; cid < NumChunks; cid++) {
            if (ChunkPool[cid]->stat() == PcbDataChunk::PCB_CHUNK_STAT_EMPTY) {
                if ((data_chunk = ChunkPool[cid]->setStat(
                         PcbDataChunk::PCB_CHUNK_STAT_LOADING,
                         PcbDataChunk::PCB_CHUNK_STAT_EMPTY)))
                    break;
            }
        }
        return data_chunk;
    ...
    inline PcbDataChunk* getLoadedChunk() {
        ...
        for (dgt_uint16 cid = 0; cid < NumChunks; cid++) {
            if (ChunkPool[cid]->stat() == PcbDataChunk::PCB_CHUNK_STAT_LOADED) {
                if ((data_chunk = ChunkPool[cid]->setStat(
                         PcbDataChunk::PCB_CHUNK_STAT_UPDATING,
                         PcbDataChunk::PCB_CHUNK_STAT_LOADED)))
                    break;
            }
        }
        return data_chunk;
    ...
    dgt_void initialize(PcbCipherTable* cipher_table,
