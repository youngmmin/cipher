...
class PcbOracleConnection : public DgcObject {
  ...
  public:
	...
	inline dgt_sint32 connect(pt_database_link_info* link_info) throw(DgcExcept)
	{
		//
		...
		DgcOracleConnection* conn=new DgcOracleConnection();
        	if (conn->connect(con_string, nul, link_info->user_name, link_info->passwd, 0) != 0) {
	                delete conn;
			ATHROWnR(DgcError(SPOS, "connect failed"), -1);
	        }
                const dgt_schar* sel_str="select parameter, value from v$nls_parameters "
		...
		memset(con_string,0,1024);
		sprintf(con_string,"(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=%s)(PORT=%u))(CONNECT_DATA=(SERVER=DEDICATED)(SID=%s)))",
			link_info->host,
			link_info->port,
			link_info->ora_service);
		if (Connection.connect(
			...
			link_info->passwd,
			con_string,
			link_info->ora_privilege)) {
			ATHROWnR(DgcError(SPOS,"connet[%s] failed",con_string),-1);
		}
		dgt_schar alter_txt[128];
		dg_sprintf(alter_txt, "alter session set NLS_DATE_FORMAT = 'YYYYMMDDHH24MISS'");
		DgcDbifCoreOciCursor    alter_stmt(&Connection);
		if (alter_stmt.declare(alter_txt,1)) ATHROWnR(DgcError(SPOS,"declare[%s] failed",alter_txt),-1);
		if (alter_stmt.open()) ATHROWnR(DgcError(SPOS,"open[%s] failed",alter_txt),-1);
		if (alter_stmt.execute()) ATHROWnR(DgcError(SPOS,"execute[%s] failed",alter_txt),-1);
		if (alter_stmt.close()) ATHROWnR(DgcError(SPOS,"close[%s] failed",alter_txt),-1);
		return 0;
	};

	inline DgcDbifCoreOciCursor* getStmt(const dgt_schar* sql_text,dgt_uint16 max_bind_cols,dgt_uint16 max_rtn_cols)
	{
		return new DgcDbifCoreOciCursor(&Connection, max_bind_cols, max_rtn_cols);
	};
	...
	inline dgt_void disconnect() throw(DgcExcept)
	{
