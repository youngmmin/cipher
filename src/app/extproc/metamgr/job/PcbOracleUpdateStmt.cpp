...
PcbOracleUpdateStmt::PcbOracleUpdateStmt(PcbCipherTable* ciphet_table,dgt_uint32 array_size)
	: PcbUpdateStmt(ciphet_table,array_size),
	  UpdateStmt(0),
	  IsDefined(0)
{
}
	...
	for(dgt_uint16 idx_cno=num_columns; idx_cno<(num_columns+num_indexes) && (cipher_idx_column=CipherTable->cipherColumn(idx_cno)); idx_cno++) {
		...
		dg_sprintf(tmp,", %s=hextoraw(:%d)",
				cipher_idx_column->indexColName()?
						cipher_idx_column->indexColName():default_idx_name,idx_cno+1);
		dg_strcat(SqlText, tmp);
	}
...
dgt_sint32 PcbOracleUpdateStmt::update(PcbDataChunk* data_chunk) throw(DgcExcept)
{
#if 0 // this should be moved to SelectStmt because cipher lib is not thread-safe.
	//
	...
	if (CipherTable->getDecryptFlag()==PCB_DECRYPT_FLAG_NULLUPDATE){
		...
		//
		if (!IsDefined) {
			rtn=UpdateStmt->defineBind(rowid_cno,"",SQLT_CHR,data_chunk->dataColumn(rowid_cno)->maxColLen());
		...
		if (!IsDefined) {
			for(dgt_uint16 cno=0; cno<CipherTable->numColumns(); cno++) {
				if (CipherTable->getDecryptFlag()==PCB_DECRYPT_FLAG_DECRYPT) {
					if (CipherTable->cipherColumn(cno)->colType() == PcbCipherColumn::PCB_COL_TYPE_BIN) {
						rtn=UpdateStmt->defineBind(cno,"",SQLT_BIN,data_chunk->dataColumn(cno)->maxColLen());
					} else {
						rtn=UpdateStmt->defineBind(cno,"",SQLT_CHR,data_chunk->dataColumn(cno)->maxColLen());
					}
				} else {
					if (CipherTable->cipherColumn(cno)->isTextEncoded()) {

						rtn=UpdateStmt->defineBind(cno,"",SQLT_CHR,data_chunk->dataColumn(cno)->maxEncColLen());
					} else {
						rtn=UpdateStmt->defineBind(cno,"",SQLT_BIN,data_chunk->dataColumn(cno)->maxEncColLen());
					}
				}
				if (rtn) ATHROWnR(DgcError(SPOS,"defineBind failed"),-1);
				if( CipherTable->numIndexes() > 0){
					dgt_uint16 idx_cno=CipherTable->cipherColumn(cno)->getIdxColumnOrder();
					if(idx_cno > 0)	//rtn=UpdateStmt->defineBind(idx_cno,"",SQLT_CHR,data_chunk->dataColumn(idx_cno)->maxEncColLen());
						rtn=UpdateStmt->defineBind(idx_cno,"",SQLT_BIN,data_chunk->dataColumn(idx_cno)->maxEncColLen());
				}
				if (rtn) ATHROWnR(DgcError(SPOS,"defineBind failed"),-1);
			}
			rtn=UpdateStmt->defineBind(rowid_cno,"",SQLT_CHR,data_chunk->dataColumn(rowid_cno)->maxColLen());
	...
	for(dgt_uint32 rno=0; rno<data_chunk->numRows(); rno++) {
		if (CipherTable->getDecryptFlag()!=PCB_DECRYPT_FLAG_NULLUPDATE){
			for(dgt_uint16 cno=0; cno<CipherTable->numColumns(); cno++) {
				...
				if (CipherTable->getDecryptFlag()==PCB_DECRYPT_FLAG_DECRYPT) {
					...
					if (UpdateStmt->setBind(cno,rno,data_column->colData(rno),*data_column->colLen(rno),0)) {
						ATHROWnR(DgcError(SPOS,"setBind failed"),-1);
					}
				} else {
					if (UpdateStmt->setBind(cno,rno,data_column->encColData(rno),*data_column->encColLen(rno),0)) {
						ATHROWnR(DgcError(SPOS,"setBind failed"),-1);
					}
				}
				if( CipherTable->numIndexes() > 0){
					dgt_uint16 idx_cno=CipherTable->cipherColumn(cno)->getIdxColumnOrder();
					if(idx_cno > 0){
						PcbDataColumn*	idx_data_column=data_chunk->dataColumn(idx_cno);
						if (UpdateStmt->setBind(idx_cno,rno,idx_data_column->encColData(rno),*idx_data_column->encColLen(rno),0)) {
							ATHROWnR(DgcError(SPOS,"setBind failed"),-1);
						}
					}
				}
		...
		if (UpdateStmt->setBind(rowid_cno, rno,
					data_chunk->dataColumn(rowid_cno)->colData(rno),
...
dgt_sint32 PcbOracleUpdateStmt::verifUpdate(dgt_uint32 partition_number) throw(DgcExcept)
{
	return 0;
}
