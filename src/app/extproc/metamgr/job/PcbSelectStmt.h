...
class PcbSelectStmt : public PcbStmt {
  ...
  protected:
	...
	{
		FetchColAttrs[NumFetchCols++]=new DgcAttr(DGC_SCHR,max_len,col_name);
	};
  public:
	...
	{
		if (idx < NumFetchCols) return FetchColAttrs[idx];
		else return 0;
	};

	inline dgt_sint32 numFetchedRows() { return NumFetchedRows; };

	virtual dgt_sint32 initialize(dgt_schar* where_clause=0) throw(DgcExcept) = 0;
	virtual dgt_sint32 fetch() throw(DgcExcept) = 0;
	virtual dgt_sint32 fetch(PcbDataChunk* data_chunk) throw(DgcExcept) = 0;
	virtual dgt_sint32 getFetchData(dgt_uint32 col_order,dgt_void** buf,dgt_sint16** ind,dgt_uint16** len) throw(DgcExcept) = 0;
};
