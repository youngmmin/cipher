...
PcbUpdater::PcbUpdater(PcbUpdateStmt* update_stmt, PcbDataChunkPool* chunk_pool,
                       pct_type_worker* worker_ptr)
    : PcbWorker(chunk_pool, worker_ptr), UpdateStmt(update_stmt) {
    strncpy(WorkerPtr->assigned_role, "updater", 10);
}

PcbUpdater::~PcbUpdater() { delete UpdateStmt; }
