...
PcbVerifOraUpdateStmt::PcbVerifOraUpdateStmt(dgt_sint64 job_id,
                                             PcbCipherTable* cipher_table,
                                             dgt_uint32 array_size)
    : PcbUpdateStmt(cipher_table, array_size), UpdateStmt(0), IsDefined(0) {
    JobID = job_id;
}
...
dgt_sint32 PcbVerifOraUpdateStmt::initialize() throw(DgcExcept) {
    ...
    if (!rtn_row)
        THROWnR(
            DgcLdbExcept(DGC_EC_LD_STMT_ERR,
                         new DgcError(SPOS, "no data found [enc_tab_id:%lld]",
                                      CipherTable->encTabId())),
            0);
    schema_info = (pct_verif_schema_info*)rtn_row;

    PccOraScriptBuilder* script_builder = new PccOraScriptBuilder(
        DgcDbProcess::dbPtr(), DgcDbProcess::sess(), schema_info->schema_link);
    ...
    *SqlText = 0;
    ...
    for (dgt_uint16 cno = 0;
        ...
        dgt_schar tmp[128];
        dg_sprintf(
            tmp, "%s=%s", cipher_column->encColName(),
        ...
        // DgcWorker::PLOG.tprintf(0,"PcbVerifOraUpdateStmt::initialize
...
dgt_sint32 PcbVerifOraUpdateStmt::update(PcbDataChunk* data_chunk) throw(
    DgcExcept) {
    return 0;
}

dgt_sint32 PcbVerifOraUpdateStmt::verifUpdate(
    dgt_uint32 partition_number) throw(DgcExcept) {
    dgt_sint32 rtn = 0;
    dgt_schar empty_string[1] = {
        0,
    };
    //
    ...
    dg_sprintf(soha_str,
               "select row_id from pct_verif_invalid_%lld::partition(%d)",
               JobID, partition_number);
    if (SqlHandle.execute(soha_str) < 0) {
        ATHROWnR(DgcError(SPOS, "SqlHandle execute failed."), -1);
    }
    dgt_uint32 rno = 0;
    while (!(ret = SqlHandle.fetch(rtn_row)) && rtn_row) {
        ...
        if (UpdateStmt->setBind(0, rno, row_id, dg_strlen(row_id), 0)) {
