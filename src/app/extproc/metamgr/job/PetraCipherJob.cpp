	  ...
	  NumStartedUpdaters(0)
{
	...
	if (ParallelDegree == 0) ParallelDegree=PCB_PARALLEL_DEGREE;
	else if (ParallelDegree > PCB_MAX_UPDATERS) ParallelDegree=PCB_MAX_UPDATERS;
...
dgt_sint32 PetraCipherJob::initJobRows() throw(DgcExcept)
{
	...
	if (JobID) {
		...
		//
		AllJobRows.rewind();
		while(AllJobRows.next() && (JobRowPtr=(pct_type_job*)AllJobRows.data())) {
		...
		if (JobRowPtr->parallel_degree) ParallelDegree=JobRowPtr->parallel_degree;
		...
		if (JobSeg->pinInsert(DgcDbProcess::sess(), JobRows, 1)) {
			ATHROWnR(DgcError(SPOS,"pinInsert[PCT_JOB] for Job[%lld] failed", JobID),-1);
		}
		JobRows.rewind();
		JobRows.next();
		...
		if (JobSeg->insertCommit(DgcDbProcess::sess(), JobRows)) {
			...
			RTHROWnR(e,DgcError(SPOS,"insertCommit[PCT_JOB] for Job[%lld] failed",JobID),-1);
		}
		JobRows.rewind();
	}
	return 0;
...
dgt_sint32 PetraCipherJob::initResource(dgt_uint8 decrypt_flag) throw(DgcExcept)
{
	//
	...
	if (!KeyStash) {
		KeyStash=DgcDbProcess::db().pdb()->segMgr()->getStream("_PCS_KEY_STASH");
		if (!KeyStash) {
		...
		} else if ((rtn=PCI_setKeyStash((PCT_KEY_STASH*)((dgt_uint8*)KeyStash + KeyStash->totalSize()))) < 0) {
	...
	//
	PcbSelectStmt*	select_stmt=PcbStmtFactory::getSelectStmt(CipherTable, JobRowPtr->array_size);
	...
	//
	JobWkrSeg=(DgcTableSegment*)DgcDbProcess::db().pdb()->segMgr()->getTable("PCT_WORKER");
	if (JobWkrSeg == 0) {
		ATHROWnR(DgcError(SPOS,"getTable[PCT_WORKER] failed"),-1);
		THROWnR(DgcDgExcept(DGC_EC_DG_TAB_NOT_FOUND,new DgcError(SPOS,"table[PCT_WORKER] not found")),-1);
	}
	JobWkrRows.setFsegment(JobWkrSeg);
	JobWkrSeg->unlockShare();
	JobWkrRows.reset();
	if (JobWkrSeg->pinInsert(DgcDbProcess::sess(), JobWkrRows, JobRowPtr->parallel_degree+1)) {
		ATHROWnR(DgcError(SPOS,"pinInsert[PCT_WORKER] for Job[%lld] failed",JobRowPtr->job_id),-1);
	}
	JobWkrRows.rewind();
	pct_type_worker*	job_wkr_ptr;
	while(JobWkrRows.next() && (job_wkr_ptr=(pct_type_worker*)JobWkrRows.data())) {
	...
	if (JobWkrSeg->insertCommit(DgcDbProcess::sess(),JobWkrRows)) {
		...
		RTHROWnR(e,DgcError(SPOS,"insertCommit[PCT_WORKER] for Job[%lld] failed",JobRowPtr->job_id),-1);
	}
	//
	...
	Collector=new PcbCollector(select_stmt, ChunkPool, (pct_type_worker*)JobWkrRows.data());
	...
	for(dgt_uint16 i=0; i<JobRowPtr->parallel_degree; i++) {
		...
		//
		PcbUpdateStmt*	update_stmt=PcbStmtFactory::getUpdateStmt(CipherTable, JobRowPtr->array_size);
		...
		JobWkrRows.next();
		Updaters[NumUpdaters++]=new PcbUpdater(update_stmt, ChunkPool, (pct_type_worker*)JobWkrRows.data());
	}
	return 0;
}
...
{
	printf("JobID[%lld] TableID[%lld] Table[%s] TotalRows[%lld] PID[%u] Lapse[%u]\n",
		JobRowPtr->job_id, JobRowPtr->enc_tab_id, CipherTable->tableName(),
		JobRowPtr->total_rows, JobRowPtr->process_id, JobRowPtr->end_time ? (JobRowPtr->end_time-JobRowPtr->start_time):SleepCount);
	JobWkrRows.rewind();
	pct_type_worker*	worker;
	while(JobWkrRows.next() && (worker=(pct_type_worker*)JobWkrRows.data())) {
		printf("\tTID[%u] Role[%s] Status[%d] Chunks[%u] Rows[%lld] Waits[%u] Time[%f]\n",
			worker->thread_id, worker->assigned_role, worker->curr_status,
			worker->processed_chunks, worker->processed_rows, worker->chunk_waits, worker->working_time/1000000.0);
	}
}
...
dgt_sint16 PetraCipherJob::getMaxStmtNo(dgt_sint16 step_no)
{
	dgt_sint16	rtn=0;
	dgt_schar       sql_text[128];
	sprintf(sql_text,"select max(stmt_no) from pct_script where enc_tab_id=%lld and step_no=%d", JobRowPtr->enc_tab_id, step_no);
	DgcSqlStmt*	sql_stmt=DgcDbProcess::db().getStmt(DgcDbProcess::sess(),sql_text,strlen(sql_text));
	if (sql_stmt == 0 || sql_stmt->execute() < 0) {
		delete EXCEPTnC;
	} else {
...
dgt_sint16 PetraCipherJob::getMinStmtNo(dgt_sint16 step_no)
{
        dgt_sint16      rtn=0;
        dgt_schar       sql_text[128];
        sprintf(sql_text,"select min(stmt_no) from pct_script where enc_tab_id=%lld and step_no=%d", JobRowPtr->enc_tab_id, step_no);
        DgcSqlStmt*     sql_stmt=DgcDbProcess::db().getStmt(DgcDbProcess::sess(),sql_text,strlen(sql_text));
        if (sql_stmt == 0 || sql_stmt->execute() < 0) {
                delete EXCEPTnC;
        } else {
...
dgt_sint32 PetraCipherJob::doCryptoWork(dgt_uint8 decrypt_flag) throw(DgcExcept)
{
	//
	// inltialize resources like the key stash, cipher table, statements, worker etc
	//
	...
	DgcExcept*	e=0;
	for(NumStartedUpdaters=0; NumStartedUpdaters<NumUpdaters; NumStartedUpdaters++) {
		//
		// start updaters
		//
	...
	for(;;) {
		...
		if (Collector->isAlive()) num_alive_jobs++;
		else if(Collector->getWorkerPtr()->curr_status == PCB_WKR_STATUS_ERROR){
			for(dgt_uint16 i=0; i<NumUpdaters; i++) {
				if(Updaters[i]->getWorkerPtr()->curr_status == PCB_WKR_STATUS_WAITING) Updaters[i]->stop();
			}
		}
		...
		if (num_alive_jobs == 0) {
			if(Collector->getWorkerPtr()->curr_status == PCB_WKR_STATUS_DONE){
				...
				for(dgt_uint16 i=0; i<NumUpdaters; i++) {  // if more than half of updaters finished normally, doCryptoWork is not pending
					if(Updaters[i]->getWorkerPtr()->curr_status == PCB_WKR_STATUS_DONE)  ret_updater+=1;
					else ret_updater-=1;
				}
				if(ret_updater > 0) break; // all job workers finished
				else {
					THROWnR(DgcDgExcept(DGC_EC_DG_INVALID_STAT,new DgcError(SPOS,"The updaters got an exception.")),-1);
				}
			} else {
				THROWnR(DgcDgExcept(DGC_EC_DG_INVALID_STAT,new DgcError(SPOS,"The collector got an exception.")),-1);
			}
		}
		sleep(1);
		++SleepCount;
...
dgt_void PetraCipherJob::logJobExecution(dgt_sint16 step_no,dgt_sint16 stmt_no,struct timeval* stime,struct timeval* etime,DgcExcept* e)
{
	dgt_sint64	lapse=0;
	...
	if (etime->tv_sec == stime->tv_sec) lapse = etime->tv_usec - stime->tv_usec;
	else lapse = (dgt_sint64)(etime->tv_sec - stime->tv_sec)*1000000 + etime->tv_usec - stime->tv_usec;
	...
	if (err_code == 1091 || err_code == 1347) {
		...
		sprintf(sql_text,"insert into pct_job_exe_log(job_id,exe_count,step_no,stmt_no,lapse_time,err_code,err_msg) "
				 "values(%lld,%d,%d,%d,%lld,%d,'%*s')",
				JobRowPtr->job_id,JobRowPtr->exe_count,step_no,stmt_no,lapse,err_code,128,fixed_err_msg);
	} else {
		sprintf(sql_text,"insert into pct_job_exe_log(job_id,exe_count,step_no,stmt_no,lapse_time,err_code,err_msg) "
				 "values(%lld,%d,%d,%d,%lld,%d,'%*s')",
				JobRowPtr->job_id,JobRowPtr->exe_count,step_no,stmt_no,lapse,err_code,128,err_msg);
	}
	DgcSqlStmt*	sql_stmt=DgcDbProcess::db().getStmt(DgcDbProcess::sess(),sql_text,strlen(sql_text));
	...
	if (step_no == 3 && stmt_no == 8001) {
		...
		sprintf(sql_text,"update pct_enc_column set(status)=(1) where status = 2 and enc_tab_id = %lld and nextlastupdate('PCT_ENC_COLUMN',0,5,'set(status)=(1)','where status =2 and enc_tab_id=%lld') > 1",JobRowPtr->enc_tab_id,JobRowPtr->enc_tab_id);
		sql_stmt=DgcDbProcess::db().getStmt(DgcDbProcess::sess(),sql_text,strlen(sql_text));
		if (sql_stmt == 0 || sql_stmt->execute() < 0) delete EXCEPTnC;
		delete sql_stmt;
		memset(sql_text,0,512);
                sprintf(sql_text,"update pct_enc_column set(status)=(0) where status >= 3 and status < 5 and enc_tab_id = %lld and nextlastupdate('PCT_ENC_COLUMN',0,5,'set(status)=(0)','where status >= 3 and status < 5 and enc_tab_id = %lld') > 0",JobRowPtr->enc_tab_id,JobRowPtr->enc_tab_id);
                sql_stmt=DgcDbProcess::db().getStmt(DgcDbProcess::sess(),sql_text,strlen(sql_text));
                if (sql_stmt == 0 || sql_stmt->execute() < 0) delete EXCEPTnC;
                delete sql_stmt;
		memset(sql_text,0,512);
                sprintf(sql_text,"delete pct_enc_column where status = 5 and enc_tab_id = %lld and nextlastupdate('PCT_ENC_COLUMN',0,7,'','where status=5 and enc_tab_id=%lld') > 0 ",JobRowPtr->enc_tab_id,JobRowPtr->enc_tab_id);
                sql_stmt=DgcDbProcess::db().getStmt(DgcDbProcess::sess(),sql_text,strlen(sql_text));
                if (sql_stmt == 0 || sql_stmt->execute() < 0) delete EXCEPTnC;
                delete sql_stmt;
		memset(sql_text,0,512);
                sprintf(sql_text,"update pct_enc_table set(curr_enc_step,curr_enc_stmt,last_update)=(3,0,nextlastupdate('PCT_ENC_TABLE',%lld,2)) where enc_tab_id=%lld",JobRowPtr->enc_tab_id,JobRowPtr->enc_tab_id);
                sql_stmt=DgcDbProcess::db().getStmt(DgcDbProcess::sess(),sql_text,strlen(sql_text));
                if (sql_stmt == 0 || sql_stmt->execute() < 0) delete EXCEPTnC;
                delete sql_stmt;
	}
...
dgt_void PetraCipherJob::in() throw(DgcExcept)
{
	...
	if (JobRowPtr->target_step != 0) {
	        sprintf(sql_text,"delete pct_job_exe_log where (job_id=%lld and step_no > %d) or (job_id=%lld and step_no = %d and stmt_no >= %d)",
				  JobID, EncTabRow.curr_enc_step, JobID, EncTabRow.curr_enc_step, EncTabRow.curr_enc_stmt);
	} else {
	        sprintf(sql_text,"delete pct_job_exe_log where job_id=%lld and step_no < 0",JobID);
	}
        DgcSqlStmt*     sql_stmt=DgcDbProcess::db().getStmt(DgcDbProcess::sess(),sql_text,strlen(sql_text));
       	if (sql_stmt == 0 || sql_stmt->execute() < 0) delete EXCEPTnC;
        delete sql_stmt;
...
{
//dg_print("PetraCipherJob::run() JobRowPtr->target_step[%d], EncTabRow.curr_enc_step[%d]\n",JobRowPtr->target_step, EncTabRow.curr_enc_step);
//DgcWorker::PLOG.tprintf(0,"PetraCipherJob::run() JobRowPtr->target_step[%d], EncTabRow.curr_enc_step[%d]\n",JobRowPtr->target_step, EncTabRow.curr_enc_step);
	...
	if (EncTabRow.curr_enc_step >=0 && JobRowPtr->target_step > EncTabRow.curr_enc_step) {
		//
		// encrypting process
		//
		for (cstep_no = EncTabRow.curr_enc_step; cstep_no < JobRowPtr->target_step; cstep_no++) {
			if (cstep_no == EncTabRow.curr_enc_step) cstmt_no = EncTabRow.curr_enc_stmt + 1; // start from middle
			else cstmt_no = 1;
			last_stmt_no = cstmt_no - 1;

		    dgt_schar       sql_text[128];
		    dg_memset(sql_text,0,128);
		    sprintf(sql_text,"select distinct stmt_no from pct_script "
			...
			//for (; cstmt_no <= getMaxStmtNo(cstep_no); cstmt_no++) { /////
				...
				if ((rtn = script_builder->getScript(JobRowPtr->enc_tab_id, 0, cstep_no, cstmt_no)) > 0) {

DgcWorker::PLOG.tprintf(0,"ENC_TAB_ID[%lld]: [%d]Step-[%d]Stmt Execute....\n",EncTabRow.enc_tab_id, cstep_no,cstmt_no);
DgcWorker::PLOG.tprintf(0,"[%s]\n",script_builder->scriptText());
					...
					if (cstep_no == 1 && cstmt_no == 6001) {
						...
						//
       						dgt_schar       sql_text[128];
					        sprintf(sql_text,"delete pct_worker where job_id=%lld", JobRowPtr->job_id);
					        DgcSqlStmt*     sql_stmt=DgcDbProcess::db().getStmt(DgcDbProcess::sess(),sql_text,strlen(sql_text));
					        if (sql_stmt == 0 || sql_stmt->execute() < 0) {
					                delete EXCEPTnC;
        						delete sql_stmt;
					...
					} else if (!strcasecmp(script_builder->scriptText(),"Decrypt")) {
						rtn = doCryptoWork(PCB_DECRYPT_FLAG_DECRYPT);
					} else if (!strcasecmp(script_builder->scriptText(),"Update Null")) {
						rtn = doCryptoWork(PCB_DECRYPT_FLAG_NULLUPDATE);
					} else {
						rtn = script_builder->runScript(script_builder->scriptText());
					}
					e = EXCEPTnC;
					...
					if (e) {
						...
						if (cstep_no == 2 && cstmt_no >=2000 && cstmt_no < 3000) {
							if (errcode == 117 || errcode == 0 || errcode == 942 || errcode == -7071 || errcode == 2443 ||
							    errcode == 1347 || errcode == 1091) {
								delete e;
								e=0;
							}
						} else if (errcode == 117 || errcode == 0 || errcode == 234 ||
						    errcode == 1408 || errcode == 2443 || errcode == 1347 || errcode == 1091) {
							delete e;
							e = 0;
						} else {
                                                	break;
		...
		if (EncTabRow.curr_enc_step < 0) {
                        ...
                        for (cstep_no = EncTabRow.curr_enc_step; cstep_no < JobRowPtr->target_step; cstep_no++) {
                                if (EncTabRow.curr_enc_stmt < 0 && cstep_no == EncTabRow.curr_enc_step) {
                                        last_step_no=cstep_no;
                                        last_stmt_no = EncTabRow.curr_enc_stmt;
                                        if (sequence == 0 && EncTabRow.curr_enc_stmt > 4000) {
                                                //
                                                // 4001 stmt = alter table org_tab rename org_renamed_tab_name
                                                //
                                                cstmt_no = getMinStmtNo(cstep_no);
                                                last_stmt_no = cstmt_no-1;
                                                sequence=1;
                                        } else cstmt_no = EncTabRow.curr_enc_stmt + 1;
                                } else {
                                        last_stmt_no = cstmt_no;
                                        cstmt_no = getMinStmtNo(cstep_no);
                                }
                            ...
                            while((rtn_row=sql_stmt->fetch())) {
                                cstmt_no=*(dgt_sint16*)rtn_row;
                                        ...
                                        if ((rtn = script_builder->getScript(JobRowPtr->enc_tab_id,     0, cstep_no, cstmt_no)) > 0) {
                                                ...
                                                struct timeval etime;
                                                gettimeofday(&etime, 0);
                                                //logJobExecution(cstep_no, cstmt_no, &stime, &etime,   EXCEPT);
                                                logJobExecution(cstep_no, cstmt_no, &stime, &etime,     e);
							...
							if (cstep_no == -1) {
								...
								// -7071 (tibero dose not exist)
	                                                        if (errcode == 117 || errcode == 0 || errcode == 234 ||
        	                                                    errcode == 4080 || errcode == 942 || errcode==4043 || errcode==1418 ||
								    errcode == -7071 || errcode == 2443 || errcode == 1091 || errcode == 1347) {
                	                                                delete e;
                        	                                        e = 0;
								} else {
                                                                	break;
			...
			//
			dgt_sint32	sequence=0;
			for (cstep_no = -1 * EncTabRow.curr_enc_step; cstep_no < JobRowPtr->target_step; cstep_no++) {
				if (cstep_no == -1 * EncTabRow.curr_enc_step) {
					...
					if (sequence == 0 && EncTabRow.curr_enc_stmt > 4000) {
						...
						last_stmt_no = cstmt_no-1;
						sequence=1;
					} else cstmt_no = -1 * EncTabRow.curr_enc_stmt + 1;
				} else {
					last_stmt_no = cstmt_no;
					cstmt_no = getMinStmtNo(cstep_no);
				}
	...
	if (last_step_no != EncTabRow.curr_enc_step || last_stmt_no != EncTabRow.curr_enc_stmt) {
		//
		...
		if (last_step_no == 4) {
			...
			sprintf(sql_text,"update pct_enc_table set(curr_enc_step,curr_enc_stmt,last_update)=(%d,%d,nextlastupdate('PCT_ENC_TABLE',%lld,2)) where enc_tab_id=%lld",
				last_step_no, last_stmt_no, JobRowPtr->enc_tab_id, JobRowPtr->enc_tab_id);
			DgcSqlStmt*	sql_stmt=DgcDbProcess::db().getStmt(DgcDbProcess::sess(),sql_text,strlen(sql_text));
			if (sql_stmt == 0 || sql_stmt->execute() < 0) delete EXCEPTnC;
			delete sql_stmt;
#if 0
...
int main(int argc,char** argv)
{
	...
	if ((val=cmd_line.getValue("job_id"))) {
		...
		// encrypt table id
		//
		if((val=cmd_line.getValue("table_id"))) enc_tab_id=dg_strtoll(val,0,10);
		//
		// target step
		//
		if((val=cmd_line.getValue("target_step"))) target_step=(dgt_sint16)strtol(val,0,10);
		//
		// arrya size
		//
		if((val=cmd_line.getValue("array_size"))) array_size=strtol(val,0,10);
		//
		// the number of chunks
		//
		if((val=cmd_line.getValue("chunks"))) num_chunks=(dgt_uint16)strtol(val,0,10);
		//
		// parallel degree
		//
		if((val=cmd_line.getValue("parallel"))) parallel_degree=(dgt_uint16)strtol(val,0,10);
		//
		// where clause
		//
		...
		// print interval
		//
		if((val=cmd_line.getValue("print_interval"))) print_interval=(dgt_uint16)strtol(val,0,10);
	}
