...
PetraCipherMigVerifyJob::PetraCipherMigVerifyJob(dgt_sint64 job_id,
                                                 dgt_sint64 enc_tab_id)
    : JobID(job_id), EncTabID(enc_tab_id) {}
...
dgt_sint32 PetraCipherMigVerifyJob::initJobRows() throw(DgcExcept) {
    ...
    if (JobID) {
        ...
        while (AllJobRows.next() &&
               (JobRowPtr = (pct_type_verify_job*)AllJobRows.data())) {
        ...
        if (JobSeg->pinInsert(DgcDbProcess::sess(), JobRows, 1)) {
            ATHROWnR(
                DgcError(SPOS, "pinInsert[PCT_VERIFY_JOB] for Job[%lld] failed",
                         JobID),
                -1);
        }
        JobRows.rewind();
        ...
        if (JobSeg->insertCommit(DgcDbProcess::sess(), JobRows)) {
            ...
            RTHROWnR(
                e,
                DgcError(SPOS,
                         "insertCommit[PCT_VERIFY_JOB] for Job[%lld] failed",
                         JobID),
                -1);
        }
        JobRows.rewind();
...
dgt_void PetraCipherMigVerifyJob::in() throw(DgcExcept) {
    ...
    sprintf(sql_text,
            "delete PCT_VERIFY_JOB_ROW_CNT_RESULT where VERIFY_JOB_ID=%lld",
            JobID);
    DgcSqlStmt* sql_stmt = DgcDbProcess::db().getStmt(
        DgcDbProcess::sess(), sql_text, strlen(sql_text));
    ...
    memset(sql_text, 0, 512);
    sprintf(sql_text,
            "delete PCT_VERIFY_JOB_DATA_RESULT where VERIFY_JOB_ID=%lld",
            JobID);
...
dgt_sint32 PetraCipherMigVerifyJob::run() throw(DgcExcept) {
    ...
    dgt_sint32 rtn = 0;
    if ((rtn = script_builder->runVerifyMig(JobRowPtr->enc_tab_id, JobRowPtr)) <
        0) {
...
int main(int argc, char** argv) {
    ...
    //
    PetraCipherMigVerifyJob pcb(job_id,     // job id
