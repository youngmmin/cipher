...
void print_usage()
{
	...
	printf("  -e, --encrypt		Encrypt the input file (requires: -in, -out, -keyname)\n");
	printf("  -d, --decrypt		Decrypt the input file (requires: -in, -out, -keyname)\n");
	printf("  -c, --check		Check if the input file is encrypted (requires: -in)\n");
	printf("\n");
	
	printf("  (REQUIRED - if -e, -d, or -c is selected)\n");
	printf("  -in <file>		Specify the input file\n");
	printf("  -out <file>		Specify the output file\n");
	printf("  -keyname <name>	Specify the 'enc_col_name' value in petra cipher kms\n");
	printf("\n");
...
int main(int argc, char *argv[])
{
	...
	int i;
	int has_encrypt = 0, has_decrypt = 0, has_check = 0, has_in = 0, has_out = 0, has_keyname = 0;
	...
	for (i = 1; i < argc; ++i)
	{
		...
		if (strcmp(argv[i], "-keyname") == 0)
		{
			...
			{
				fprintf(stderr, "ERROR: -keyname option requires an argument\n");
				return 1;
			}
		}
	}
	...
	{
		fprintf(stderr, "ERROR: Only one option among -e, -d, -c can be set.\n");
		print_usage();
		return 1; // failure
	}
	else if ((has_encrypt + has_decrypt + has_check) == 0)
	{
		fprintf(stderr, "ERROR: At least one option among -e, -d, -c must be set.\n");
		print_usage();
		return 1; // failure
	}
	...
	{
		fprintf(stderr, "ERROR: -in, -out, and -keyname options are required\n");
		print_usage();
		return 1;
	}
