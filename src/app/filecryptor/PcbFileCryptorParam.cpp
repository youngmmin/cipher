...
PcbFileCryptorParam::PcbFileCryptorParam()
    : user(NULL), key_name(NULL), in_file(NULL), out_file(NULL), mode(FCB_MODE_NONE)
{
}
...
void PcbFileCryptorParam::setEncryptMode()
{
    mode = FCB_MODE_ENCRYPT;
}

void PcbFileCryptorParam::setDecryptMode()
{
    mode = FCB_MODE_DECRYPT;
}

void PcbFileCryptorParam::setCheckMode()
{
    mode = FCB_MODE_CHECK;
}
...
int PcbFileCryptorParam::toString(char* param_list, int param_list_size)
{
    ...
    sprintf(param_list, "(key=(1=(name=%s)(columns=1)))(mode=(crypt=%s)(overwrite_flag=on))(session=(program_id=%s run pcb_filecryptor))", key_name, mode, user);
