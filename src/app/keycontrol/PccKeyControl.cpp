...
template<class C> class PccAutoPtr {
  private:
	C*	Pointer;
  public:
  ...
  public :
	...
	inline const dgt_schar* optionValue(const dgt_schar* option_name)
	{
		for(dgt_sint32 i=0; i<NumOptions; i++) {
			if (!strcasecmp(option_name,Options[i].name)) return Options[i].value;
		}
		return 0;
	}
};
...
class PccCmd {
  ...
  protected :
	...
	inline dgt_sint32 connect()
	{
		...
		if (!proto) {
			THROWnR(DgcCipherExcept(-10001,new DgcError(SPOS,"protocol not found")),-1);
		}
		...
		if (!strcasecmp(proto,"beq")) {
			...
			if (!host) {
				THROWnR(DgcCipherExcept(-10002,new DgcError(SPOS,"host not found")),-1);
			}
			if (!port) {
				THROWnR(DgcCipherExcept(-10003,new DgcError(SPOS,"port not found")),-1);
			}
			dg_sprintf(con_string,"(address=(protocol=tcp)(host=%s)(port=%s))", host, port);
		} else {
			THROWnR(DgcCipherExcept(-10004,new DgcError(SPOS,"unsupported protocol[%s]",proto)),-1);
		}
		const dgt_schar*	svc=CmdOption.optionValue("svc");
		if (!svc) {
			THROWnR(DgcCipherExcept(-10005,new DgcError(SPOS,"svc not found")),-1);
		}
		const dgt_schar*	user=CmdOption.optionValue("user");
		if (!user) {
			THROWnR(DgcCipherExcept(-10006,new DgcError(SPOS,"user not found")),-1);
		}
		dgt_schar	pw[33];
		dg_print("%s ",user);
		fflush(stdout);
		...
		if (UI_UTIL_read_pw_string(pw, 32,"passwd: ",0)) {
			THROWnR(DgcCipherExcept(-10007,new DgcError(SPOS,"UI_UTIL_read_pw_string failed")),-1);
		}
	...
	inline dgt_sint32 runSQL(dgt_schar* sql_text,DgcMemRows* bind_rows=0,DgcFileStream* fs=0) throw(DgcExcept)
	{
		if (connect()) {
			ATHROWnR(DgcError(SPOS,"connect failed"),-1);
		}
		...
		dgt_sint32	frows=0;
		if ((frows=stmt->execute(sql_text, strlen(sql_text), 1, bind_rows)) < 0) {
		...
		while (frows > 0) {
			...
			if (rows == 0) {
				...
				while(rows->next()) {
					if (fs) {
						if (fs->sendData(rows->data(),strlen((dgt_schar*)rows->data())) < 0) {
							DgcExcept*	e=EXCEPTnC;
							delete stmt;
							RTHROWnR(e,DgcError(SPOS,"sendData failed"),-1);
						}
					} else dg_print("%s",(dgt_schar*)rows->data());
					prows++;
				}
				rows->reset();
			}
...
class PccCmdHelp : public PccCmd {
  ...
  public:
	...
	{
		dg_print("\npcp_key_ctrl help\n");
		dg_print("pcp_key_ctrl create -proto tcp|beq -host 192.168.1.20 -port 6688 -svc demo -user dgadmin\n");
		dg_print("pcp_key_ctrl drop -proto tcp|beq -host 192.168.1.20 -port 6688 -svc demo -user dgadmin\n");
		dg_print("pcp_key_ctrl open -proto tcp|beq -host 192.168.1.20 -port 6688 -svc demo -user dgadmin\n");
		dg_print("pcp_key_ctrl close -proto tcp|beq -host 192.168.1.20 -port 6688 -svc demo -user dgadmin\n");
		dg_print("pcp_key_ctrl chpasswd -proto tcp|beq -host 192.168.1.20 -port 6688 -svc demo -user dgadmin\n");
		dg_print("pcp_key_ctrl export -proto tcp|beq -host 192.168.1.20 -port 6688 -svc demo -user dgadmin -out file.txt\n");
		dg_print("pcp_key_ctrl import -proto tcp|beq -host 192.168.1.20 -port 6688 -svc demo -user dgadmin -in file.txt\n");
		dg_print("pcp_key_ctrl set -proto tcp|beq -host 192.168.1.20 -port 6688 -svc demo -user dgadmin -key_open_mode <auto|manual>\n");
		return 0;
	};
};
...
class PccCmdCreate : public PccCmd {
  ...
  public:
	...
	virtual dgt_sint32 run()
	{
		...
		if (UI_UTIL_read_pw_string(tmp_pw, 32, "retype master key passwd: ", 0)) {
			THROWnR(DgcCipherExcept(-10007,new DgcError(SPOS,"UI_UTIL_read_pw_string failed")),-1);
		}
		if (memcmp(passwd, tmp_pw, 32)) {
			THROWnR(DgcCipherExcept(-10007,new DgcError(SPOS,"password mismatch")),-1);
		}
...
class PccCmdDrop : public PccCmd {
  ...
  public:
	...
	virtual dgt_sint32 run()
	{
		...
		memset(passwd, 0, 33);
		if (UI_UTIL_read_pw_string(passwd, 32, "master key passwd: ", 0)) {
			THROWnR(DgcCipherExcept(-10008,new DgcError(SPOS,"UI_UTIL_read_pw_string failed")),-1);
		}
		dgt_schar	sql_text[256];
...
class PccCmdClose : public PccCmd {
  ...
  public:
	...
	virtual dgt_sint32 run()
	{
		...
		memset(passwd,0,33);
		if (UI_UTIL_read_pw_string(passwd, 32, "master key passwd: ", 0)) {
			THROWnR(DgcCipherExcept(-10009,new DgcError(SPOS,"UI_UTIL_read_pw_string failed")),-1);
		}
		dgt_schar	sql_text[256];
...
class PccCmdExport : public PccCmd {
  ...
  public:
	...
	virtual dgt_sint32 run()
	{
		...
		if (!out) {
			THROWnR(DgcCipherExcept(-10011,new DgcError(SPOS,"out not found")),-1);
		}
		...
		if (UI_UTIL_read_pw_string(old_pw,32,"master key passwd: ",0)) {
			THROWnR(DgcCipherExcept(-10012,new DgcError(SPOS,"UI_UTIL_read_pw_string failed")),-1);
		}
		if (UI_UTIL_read_pw_string(new_pw,32,"export master key passwd: ",0)) {
			THROWnR(DgcCipherExcept(-10013,new DgcError(SPOS,"UI_UTIL_read_pw_string failed")),-1);
		}
		if (*new_pw) {
			dgt_schar	tmp_pw[33];
			memset(tmp_pw,0,33);
			if (UI_UTIL_read_pw_string(tmp_pw,32,"retype new master key passwd: ",0)) {
				THROWnR(DgcCipherExcept(-10013,new DgcError(SPOS,"UI_UTIL_read_pw_string failed")),-1);
			}
...
class PccCmdImport : public PccCmd {
  ...
  public:
	...
	virtual dgt_sint32 run()
	{
		...
		if (!in) {
			THROWnR(DgcCipherExcept(-10014,new DgcError(SPOS,"out not found")),-1);
		}
		...
		//
		// read the master key password and a new one if user wants to change it.
		//
		dgt_schar	old_pw[33];
		dgt_schar	new_pw[33];
		memset(old_pw,0,33);
  ...
  public:
	...
	virtual dgt_sint32 run()
	{
		...
		if ((option=CmdOption.optionValue("key_open_mode"))) {
			dg_sprintf(sql_text,"call pcp_set_key_open_mode('%s','%s')", passwd, option);
		} else {
			THROWnR(DgcCipherExcept(-10008,new DgcError(SPOS,"unsupported option")),-1);
		}
		if (runSQL(sql_text)) {
			ATHROWnR(DgcError(SPOS,"runSQL failed"),-1);
		}
...
class PccKeyControl {
  ...
  protected :
  public :
	PccKeyControl(dgt_sint32 argc,dgt_schar** argv) : Cmd(argv[1]), CmdOption(argc,argv) {};
	~PccKeyControl() {};
