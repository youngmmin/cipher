...
dgt_sint8 PccKredService::doRequest(DgcMsgDgiSqlRq* srm) throw(DgcExcept) {
    ...
    if ((srm->operation() & DGC_OPF_PARSE) != 0) {  // new sql text
        ...
        if (srm->sqlLength() == 0 || srm->sqlText() == 0) {
            THROWnR(DgcDbNetExcept(
                        ...
                        new DgcError(SPOS, "parse request with no text")),
                    -1);
        }
        if (srm->cursor() > 0) {
            //
            // just release the statement associated with the cursor,
            // becasue this cursor will be reused associating with a new
            // statement.
            //
        ...
        if (stmt_term->tid() != DgcSqlLang::DGC_SQL_DML_CALL) {
            THROWnR(DgcDbNetExcept(DGC_EC_DN_INVALID_ST,
                                   new DgcError(SPOS, "not a call statement")),
                    -1);
        } else {
            ...
            if (!strcasecmp(proc_name, "open_session")) {
                CurrStmt = new PccKredStmtOpenSess(DgcDbProcess::db().pdb(),
                                                   &Session, 0);
            } else if (!strcasecmp(proc_name, "get_key")) {
                CurrStmt = new PccKredStmtGetKey(DgcDbProcess::db().pdb(),
                                                 &Session, 0);
            } else if (!strcasecmp(proc_name, "get_priv")) {
                CurrStmt = new PccKredStmtGetPriv(DgcDbProcess::db().pdb(),
                                                  &Session, 0);
            } else if (!strcasecmp(proc_name, "get_eci")) {
                CurrStmt = new PccKredStmtGetEci(DgcDbProcess::db().pdb(),
                                                 &Session, 0);
            } else if (!strcasecmp(proc_name, "log_request")) {
                CurrStmt = new PccKredStmtLogRequest(DgcDbProcess::db().pdb(),
                                                     &Session, 0);
            } else if (!strcasecmp(proc_name, "alert")) {
                CurrStmt =
                    new PccKredStmtAlert(DgcDbProcess::db().pdb(), &Session, 0);
            } else if (!strcasecmp(proc_name, "approve")) {
                CurrStmt = new PccKredStmtApprove(DgcDbProcess::db().pdb(),
                                                  &Session, 0);
            } else if (!strcasecmp(proc_name, "crypt")) {
                CurrStmt =
                    new PccKredStmtCrypt(DgcDbProcess::db().pdb(), &Session, 0);
            } else if (!strcasecmp(proc_name, "enc_count")) {
                CurrStmt = new PccKredStmtEncCount(DgcDbProcess::db().pdb(),
                                                   &Session, 0);
            } else if (!strcasecmp(proc_name, "post")) {
                CurrStmt =
                    new PccKredStmtPost(DgcDbProcess::db().pdb(), &Session, 0);
#if 1  // 2015.8.8 added by chchung for sending key set for cipher agent
            } else if (!strcasecmp(proc_name, "get_key_set")) {
                CurrStmt = new PccKredStmtGetKeySet(DgcDbProcess::db().pdb(),
                                                    &Session, 0);
#endif
#if 1  // 2015.8.9 added by chchung for sending external initial vector for
       // cipher agent
            } else if (!strcasecmp(proc_name, "get_iv")) {
                CurrStmt =
                    new PccKredStmtGetIV(DgcDbProcess::db().pdb(), &Session, 0);
#endif
#if 1  // 2016.3.4 added by mwpark for putting external key for cipher agent
            } else if (!strcasecmp(proc_name, "put_ext_key")) {
                CurrStmt = new PccKredStmtPutExtKey(DgcDbProcess::db().pdb(),
                                                    &Session, 0);
#endif
#if 1  // 2016.3.15 added by mwpark for get trailer for cipher agent
            } else if (!strcasecmp(proc_name, "get_trailer")) {
                CurrStmt = new PccKredStmtGetTrailer(DgcDbProcess::db().pdb(),
                                                     &Session, 0);
#endif
#if 1  // 2017.2.7 added by mwpark for file log request
            } else if (!strcasecmp(proc_name, "file_request")) {
                CurrStmt = new PccKredStmtFileLogRequest(
                    DgcDbProcess::db().pdb(), &Session, 0);
#endif
#if 1  // 2018.6.15 added by shson for user file log request
            } else if (!strcasecmp(proc_name, "user_file_request")) {
                CurrStmt = new PccKredStmtUserFileLogRequest(
                    DgcDbProcess::db().pdb(), &Session, 0);
#endif
#if 1  // 2017.07.06 added by jhpark for check approval privilege with virtual
       // key
            } else if (!strcasecmp(proc_name, "get_vkey_db_priv")) {
                CurrStmt = new PccKredStmtGetVKeyDbPriv(
                    DgcDbProcess::db().pdb(), &Session, 0);
            } else if (!strcasecmp(proc_name, "get_vkey_file_priv")) {
                CurrStmt = new PccKredStmtGetVKeyFilePriv(
                    DgcDbProcess::db().pdb(), &Session, 0);
#endif
#if 1  // 2017.8.8 added by jhpark for file cryption api
            } else if (!strcasecmp(proc_name, "get_zone_param")) {
                CurrStmt = new PccKredStmtGetZoneParam(DgcDbProcess::db().pdb(),
                                                       &Session, 0);
#endif
            } else if (!strcasecmp(proc_name, "get_reg_engine")) {
                CurrStmt = new PccKredStmtGetRegEngine(DgcDbProcess::db().pdb(),
                                                       &Session, 0);
            } else if (!strcasecmp(proc_name, "get_crypt_param")) {
                CurrStmt = new PccKredStmtGetCryptParam(
                    DgcDbProcess::db().pdb(), &Session, 0);
            } else if (!strcasecmp(
                           proc_name,
                           "get_zone_id")) {  // added by shson 2018.06.01
                CurrStmt = new PccKredStmtGetZoneId(DgcDbProcess::db().pdb(),
                                                    &Session, 0);
            } else if (!strcasecmp(
                           proc_name,
                           "get_reg_engine_id")) {  // added by shson 2018.06.01
                CurrStmt = new PccKredStmtGetRegEngineId(
                    DgcDbProcess::db().pdb(), &Session, 0);
            } else if (!strcasecmp(
                           proc_name,
                           "detect_file_data")) {  // added by mjkim 2019.05.29
                CurrStmt = new PccKredStmtDetectFileLogData(
                    DgcDbProcess::db().pdb(), &Session, 0);
            } else if (!strcasecmp(proc_name,
                                   "detect_file_request")) {  // added by mjkim
                                                              // 2019.05.29
                CurrStmt = new PccKredStmtDetectFileLogRequest(
                    DgcDbProcess::db().pdb(), &Session, 0);
            } else if (!strcasecmp(
                           proc_name,
                           "get_detect_file_request")) {  // added by mjkim
                                                          // 2019.05.29
                CurrStmt = new PccKredStmtDetectFileGetRequest(
                    DgcDbProcess::db().pdb(), &Session, 0);
            } else if (!strcasecmp(proc_name, "get_rsa_key")) {
                CurrStmt = new PccKredStmtGetRsaKey(DgcDbProcess::db().pdb(),
                                                    &Session, 0);
            } else {
                THROWnR(
                    DgcDbNetExcept(
                        DGC_EC_DN_INVALID_ST,
                        new DgcError(SPOS, "unknown procedure[%s]", proc_name)),
                    -1);
            }
            ((PccKredStmt*)CurrStmt)->setTraceLog(&TLOG);
        }
    } else if ((srm->operation() & DGC_OPF_EXECUTE) != 0 ||
               (srm->operation() & DGC_OPF_FETCH) != 0) {
        //
    ...
    if ((srm->operation() & DGC_OPF_BIND) != 0) {
        ...
        if ((rtn = MsgStream->recvMessage(10)) < 0) {
            ...
            if (MsgStream->currMsg()->opi() != DGICLASS) {
                THROWnR(
                    DgcDbNetExcept(DGC_EC_DN_INVALID_MSG,
                                   new DgcError(SPOS, "not DGICLASS[%d]",
            ...
            THROWnR(DgcDbNetExcept(
                        DGC_EC_DN_TIMEOUT,
                        new DgcError(SPOS,
                                     "timeout for bind row format[DGICLASS]")),
                    -1);
        }
        TLOG.tprintf(3, MsgStream->currMsg(), "receive a row header:\n");
        UserVarRows = new DgcMemRows(
            ((DgcMsgDgiClass*)MsgStream->currMsg())->cutClass(), 1);
        //
        ...
        if ((rtn = MsgStream->recvMessage(10)) < 0) {
            ...
            if (MsgStream->currMsg()->opi() != DGIROWS) {
                ...
                THROWnR(
                    DgcDbNetExcept(DGC_EC_DN_INVALID_MSG,
                                   new DgcError(SPOS, "not DGIROWS[%d]",
                                                MsgStream->currMsg()->opi())),
                    -1);
            }
            TLOG.tprintf(3, MsgStream->currMsg(), "receive a row message:\n");
            if (((DgcMsgDgiRows*)MsgStream->currMsg())
                    ->decodeRows(UserVarRows) < 0) {
                if (UserVarRows != 0) delete UserVarRows;
            ...
            THROWnR(DgcDbNetExcept(DGC_EC_DN_TIMEOUT,
                                   new DgcError(SPOS, "timeout for DGICLASS")),
                    -1);
        }
        PktStream->resetIBuf();
    } else {
    ...
    //
    if ((srm->operation() & DGC_OPF_EXECUTE) != 0 &&
        CurrStmt->execute(UserVarRows) < 0) {
...
dgt_sint8 PccKredService::doResponse(DgcMsgDgiSqlRq* srm) throw(DgcExcept) {
    ...
    //
    DgcMsgDgiSqlRs srs(((srm->operation() & DGC_OPF_DESC_SL) |
                        (srm->operation() & DGC_OPF_FETCH)),
                       srm->cursor());
    ...
    if (srs.operation() != 0) {
        ...
        if ((srm->operation() & DGC_OPF_FETCH) != 0) {
            ...
            while ((srm->moveUnit() == 0 || rnum < srm->moveUnit()) &&
                   (rdata = CurrStmt->fetch()) != 0) {
                if (rmsg.encodeRows(++rnum, rh_format, rdata) != 0) {
                    ATHROWnR(DgcError(SPOS, "encodeRows failed"), -1);
                }
            }
...
dgt_sint32 PccKredService::run() throw(DgcExcept) DGC_TRY_BEGIN
    if (checkTrace() != 0) {
    if (ctraceLevel() > 0) {
    ...
    if (MsgStream->currMsg()->opi() == DGISQLRQ) {
        ...
        if (OpenFlag) {
            ...
            if (e == 0 && (srm->operation() & DGC_OPF_PARSE)) {
                ...
                // new sql statement
                //
                if ((new_cursor = CursorTable->openCursor(
                         CurrStmt, srm->cursor())) == 0) {
                    e = EXCEPTnC;
            ...
            if (openDatabase(srm->sqlLength(), srm->sqlText())) {
                ...
                DgcStreamSegment* key_stash =
                    DgcDbProcess::db().pdb()->segMgr()->getStream(
                        "_PCS_KEY_STASH");
                if (!key_stash) {
                    if ((e = EXCEPTnC))
                        ...
                        e = new DgcLdbExcept(
                            ...
                            new DgcError(SPOS, "_PCS_KEY_STASH not found"));
                                ...
                                PCT_KEY_STASH*)((dgt_uint8*)key_stash +
                ...
                if (DgcDgRepository::p()->load() != 0) {
                    if ((e = EXCEPTnC))
                        e->addErr(new DgcError(SPOS, "load failed"));
                }
                //
                // initialize KRED session pool which has gateway & agent
                // connections
                //
        ...
        // *****************************************
        //
        e = new DgcDbNetExcept(
            DGC_EC_DN_INVALID_MSG,
...
if (e) {
    ...
    DgcPktMarker* marker =
        (DgcPktMarker*)PktStream->newPacket(DgcPacket::DGC_PKT_MARKER);
    if (PktStream->setPacket() != 0) {
        delete marker;
        ATHROWnR(DgcError(SPOS, "setPacket failed"), -1);
    }
