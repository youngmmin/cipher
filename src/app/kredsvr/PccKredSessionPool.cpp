...
dgt_sint32 PccKredSessionPool::getConnection(pksp_type_link* link) throw(DgcExcept)
{
	//
	// get link info
	//
	...
	if (link_info == 0) {
		ATHROWnR(DgcError(SPOS,"getDatabaseLinkInfo[%s] failed",link->name),-1);
	}
	...
	dg_sprintf(sql_text,"(ADDRESS=(PROTOCOL=TCP)(HOST=%s)(PORT=%u)(CONN_TIMEOUT=%u)(IN_TIMEOUT=%u)(OUT_TIMEOUT=%u)(DB_NAME=%s))",
		link_info->host,link_info->port,link_info->conn_timeout,
		link_info->in_timeout,link_info->out_timeout,link_info->db_name);
	link->connection=DgcConnectionMgr::conMgr().getConnection("soha");
	if (link->connection->connect(sql_text,link_info->db_name,link_info->user_name,link_info->passwd) != 0) {
		DgcExcept*	e=EXCEPTnC;
		delete link->connection;
		link->connection=0;
		RTHROWnR(e,DgcError(SPOS,"connect to [%s:%u] with [%s:%s] failed",
			link_info->host,link_info->port,link_info->db_name,link_info->user_name),-1);
	}
	return 0;
}
...
#else
	//dg_sprintf(sql_text, "select b.* from (select link_name from pt_database_link_info where host=:1) a, pct_db_agent b where b.primary_gw_link = a.link_name or b.secondary_gw_link = a.link_name or b.agent_link = a.link_name");
	dg_sprintf(sql_text, "select b.* from pct_db_agent b where b.kred_link=:1");
#endif

if (LogDumpMap == 255) DgcWorker::PLOG.tprintf(0,"shson sqltext[%s] clientIp [%s]\n",sql_text,kred_session->clientCommIP());

	while ((sql_stmt=DgcDbProcess::db().getStmt(DgcDbProcess::sess(), sql_text, strlen(sql_text))) && sql_stmt->execute(&v_bind, 0) >= 0) {
		dgt_uint8*	tmp=0;
		if ((tmp=sql_stmt->fetch())) memcpy(&agent_row, tmp, sizeof(pct_type_db_agent));
		break;
	}
	DgcExcept*	e=EXCEPTnC;
	delete sql_stmt;
if (LogDumpMap == 255) DgcWorker::PLOG.tprintf(0,"shson agent_row.primary_gw_link [%s] agent_row.agent_link [%s]\n",agent_row.primary_gw_link,agent_row.agent_link);
	if (e) {
if (LogDumpMap == 255)DgcWorker::PLOG.tprintf(0,*e,"shson initialization for KREDAgent[%s] failed:",kred_session->clientCommIP());
		delete e;
	} else {
		//
		// open sessions
		...
		if (*agent_row.primary_gw_link) {
			...
			if (getConnection(&Gateway1)) {
				DgcExcept*	e=EXCEPTnC;
				DgcWorker::PLOG.tprintf(0,*e,"getConnection[%s] failed:",agent_row.primary_gw_link);
				delete e;
			}
		}
		if (*agent_row.secondary_gw_link) {
			...
			if (getConnection(&Gateway2)) {
				DgcExcept*	e=EXCEPTnC;
				DgcWorker::PLOG.tprintf(0,*e,"getConnection[%s] failed:",agent_row.secondary_gw_link);
				delete e;
			}
		}
		if (*agent_row.agent_link) {
			...
			if (getConnection(&Agent)) {
				DgcExcept*	e=EXCEPTnC;
				DgcWorker::PLOG.tprintf(0,*e,"getConnection[%s] failed:",agent_row.agent_link);
				delete e;
			}
		}
	}
...
dgt_sint64 PccKredSessionPool::getUserSID(pc_type_open_sess_in* uinfo, dgt_sint32* auth_fail_code) throw(DgcExcept)
{
	//added by shson 2018.02.07 for debugging
	dgt_dg_sys_param*   param = 0;
	dgt_sint32 LogDumpMap = 0;
	...
	dgt_schar	sql_text[512];
        *sql_text = 0;
		...
		db_v.rewind();
                ...
                if (sql_stmt && sql_stmt->execute(&db_v,0) >= 0 && (db_svc=(db_inst_id_type*)sql_stmt->fetch())) {
...
if (LogDumpMap == 255) DgcWorker::PLOG.tprintf(0,"shson uinfo->db_sid [%lld] ps_user.instance_id [%lld] uinfo->client_ip [%s] uinfo->instance_name [%s]\n",uinfo->db_sid,ps_user.instance_id,uinfo->client_ip,uinfo->instance_name);
	if (uinfo->db_sid) {
		//
		...
		if ((sql_stmt=DgcDbProcess::db().getStmt(DgcDbProcess::sess(), sql_text, strlen(sql_text))) && sql_stmt->execute(&v_bind,0) >= 0) {
			dgt_uint8*	tmp_row = 0;
			if ((tmp_row=sql_stmt->fetch())) memcpy(&sess_user, tmp_row, sizeof(sess_user));
		}
		...
		if (sess_user.psu_id == 0) {
			...
			if ((*uinfo->client_ip == 0 || !strcasecmp(uinfo->client_ip,"127.0.0.1")) && *Agent.name) {
				link = &Agent;
			} else if (*uinfo->client_ip) {
				if (!strcasecmp(uinfo->client_ip, Gateway1.ip)) link = &Gateway1;
				else if (!strcasecmp(uinfo->client_ip, Gateway2.ip)) link = &Gateway2;
				else if (!strcasecmp(uinfo->client_ip, Agent.ip)) link = &Agent;
			}
			DgcCliStmt*	stmt = 0;
			dgt_sint32	ntry = 0;
			dgt_sint32	is_link_change = 0; //added by shson 2019.02.27 for #519 issue
		DgcMemRows      remote_bind(2);
                ...
                remote_bind.add();
                remote_bind.next();
                memcpy(remote_bind.getColPtr(1),&uinfo->db_sid, sizeof(dgt_uint32));
...
RETRY_GET_CONNECTION:
			for (;link;) {
				...
				if (!stmt) break;
				if (stmt->execute(sql_text,strlen(sql_text),0,&remote_bind) < 0) break;
				DgcMemRows*	rtn_rows = stmt->returnRows();
				if (rtn_rows && rtn_rows->next()) memcpy(&sess_user, rtn_rows->data(), sizeof(ptt_sess_user));
				break;
			}
			e = EXCEPTnC;
			delete stmt;
			if (e) {
				delete link->connection;
				link->connection = 0;
				DgcWorker::PLOG.tprintf(0,*e,"getSessUser[%d] failed:", uinfo->db_sid);
				delete e;
            ...
            if (is_link_change == 0 && !strcasecmp(uinfo->client_ip,"127.0.0.1") && *Agent.name && *Gateway1.name && sess_user.psu_id == 0) {
                link = &Gateway1;
				is_link_change = 1;//added by shson 2019.02.27 for #519 issue
				ntry = 0;
if (LogDumpMap == 255) DgcWorker::PLOG.tprintf(0,"shson not found active agent, change stand by agent(gateway1) [%s] \n",link->name);
                goto RETRY_GET_CONNECTION;
			} //if (strncmp(link->name ,Agent.name, sizeof(Agent.name)) == 0 && sess_user.psu_id == 0) end
	...
	// build ps_user
	//
if (LogDumpMap == 255) DgcWorker::PLOG.tprintf(0,"shson sess_user.psu_id [%lld] sess_user.clientip [%s] \n",sess_user.psu_id,sess_user.client_ip);
...
DgcWorker::PLOG.tprintf(0,"shson not found psu_id so be bypassed\n");
        if (sess_user.psu_id) {
                ...
                //
		if (sess_user.auth_user && *sess_user.auth_user) strncpy(ps_user.auth_user, sess_user.auth_user, 64);
...
#else
		//
		// db_user, os_user, client_program = information gatherd by cipher library
		//
                if (uinfo->db_user) strncpy(ps_user.db_sess_user, uinfo->db_user, 32);
                if (uinfo->os_user) strncpy(ps_user.os_user, uinfo->os_user, 32);
                if (uinfo->client_program) strncpy(ps_user.client_program, uinfo->client_program, 128);
#endif
        	if (sess_user.client_mac && *sess_user.client_mac) strncpy(ps_user.client_mac, sess_user.client_mac, 64);
                ...
                if (uinfo->os_user) strncpy(ps_user.os_user, uinfo->os_user, 32);
                if (uinfo->client_program) strncpy(ps_user.client_program, uinfo->client_program, 128);
        	if (uinfo->client_mac && *uinfo->client_mac) strncpy(ps_user.client_mac, uinfo->client_mac, 64);
	...
	if ((user_tab=(DgcTableSegment*)DgcDbProcess::db().pdb()->segMgr()->getTable(
				"PT_SESS_USER",DGC_SEG_TABLE,DgcDbProcess::sess()->databaseUser())) == 0) {
		ATHROWnR(DgcError(SPOS,"getTable failed"),0);
		THROWnR(DgcLdbExcept(DGC_EC_PD_NOT_FOUND,new DgcError(SPOS,"table[PT_SESS_USER_NEW] not found")),0);
	}
	user_tab->unlockShare();
	DgcIndexSegment*	idx=0;
	if ((idx=(DgcIndexSegment*)DgcDbProcess::db().pdb()->idxMgr()->getIndex("PT_SESS_USER_IDX2")) == 0) {
		ATHROWnR(DgcError(SPOS,"getIndex failed"),0);
		THROWnR(DgcLdbExcept(DGC_EC_PD_NOT_FOUND,new DgcError(SPOS,"index[PT_SESS_USER_IDX2] not found")),0);
	}
	...
	if ((sess_seq=DgcDbProcess::db().pdb()->seqMgr()->getSequence(DgcDbProcess::sess()->databaseUser(),"PTS_SESS_SEQ")) == 0) {
	...
	//
        ...
        if (user_tab->insertCommit(DgcDbProcess::sess(), rows) != 0) {
                ...
                if (e->errCode() == DGC_EC_PD_DUP_KEY) {
                        ...
                        e = 0;
                        rows.rewind();
                        if (idx->find((dgt_uint8*)&ps_user, rows)) { // search again for psu_id
                                e=EXCEPTnC;
                ...
                //
                THROWnR(DgcLdbExcept(DGC_EC_PD_NOT_FOUND,new DgcError(SPOS,"index[PT_SESS_USER_IDX2] search failed")),0);
        }
	DgcTableSegment*        ins_user_tab=0;
        if ((ins_user_tab=(DgcTableSegment*)DgcDbProcess::db().pdb()->segMgr()->getTable(
                                "PT_SESS_USER_NEW",DGC_SEG_TABLE,DgcDbProcess::sess()->databaseUser())) == 0) {
                ATHROWnR(DgcError(SPOS,"getTable failed"),0);
                THROWnR(DgcLdbExcept(DGC_EC_PD_NOT_FOUND,new DgcError(SPOS,"table[PT_SESS_USER_NEW] not found")),0);
        }
...
pt_type_sess_user* PccKredSessionPool::getSessUser(dgt_sint64 psu_id) throw(DgcExcept)
{
	DgcTableSegment*	user_tab=0;
	if ((user_tab=(DgcTableSegment*)DgcDbProcess::db().pdb()->segMgr()->getTable(
				"PT_SESS_USER",DGC_SEG_TABLE,DgcDbProcess::sess()->databaseUser())) == 0) {
		ATHROWnR(DgcError(SPOS,"getTable failed"),0);
		THROWnR(DgcLdbExcept(DGC_EC_PD_NOT_FOUND,new DgcError(SPOS,"table[PT_SESS_USER] not found")),0);
	}
	user_tab->unlockShare();
	DgcIndexSegment*	idx=0;
	if ((idx=(DgcIndexSegment*)DgcDbProcess::db().pdb()->idxMgr()->getIndex("PT_SESS_USER_IDX1")) == 0) {
		ATHROWnR(DgcError(SPOS,"getIndex failed"),0);
		THROWnR(DgcLdbExcept(DGC_EC_PD_NOT_FOUND,new DgcError(SPOS,"index[PT_SESS_USER_IDX1] not found")),0);
	}
	DgcRowList		rows(user_tab);
	...
	if (!rows.next()) {
		THROWnR(DgcLdbExcept(DGC_EC_PD_NOT_FOUND,new DgcError(SPOS,"session user[%lld] not found",psu_id)),0);
	}
