...
dgt_sint32 PccKredStmt::getPrivilege(pc_type_get_priv_in* priv_in,
                                     pc_type_get_priv_out* priv_out,
                                     pct_type_priv_request_hist* rqst_hist) {
    pt_type_sess_user* sess_user =
        PccKredSessionPool::getSessUser(priv_in->user_sid);
    if (sess_user == 0) {
        ATHROWnR(DgcError(SPOS, "getSessUser[%lld] failed.", priv_in->user_sid),
                 -1);
    }
    ...
    if (user_grp.initialize() < 0) {
        ATHROWnR(DgcError(SPOS, "getPrivilege[PccUserGroup] initialize failed"),
                 -1);
    }
    ...
    if (enc_priv.initialize() < 0) {
        ATHROWnR(
            DgcError(SPOS, "getPrivilege[PccEncPrivilege] initialize failed"),
            -1);
    }
    if (dec_priv.initialize() < 0) {
        ATHROWnR(
            DgcError(SPOS, "getPrivilege[PccDecPrivilege] initialize failed"),
            -1);
    }
    if (dec_count_priv.initialize() < 0) {
        ATHROWnR(
            DgcError(SPOS,
                     "getPrivilege[PccDecCountPrivilege] initialize failed"),
            -1);
    }
    enc_priv.reset();
    dec_priv.reset();
    ...
    if (e) {
        DgcWorker::PLOG.tprintf(
            0, *e, "privilege search for sid[%lld]-enc_col_id[%lld] failed:",
            priv_in->user_sid, priv_in->enc_col_id);
        RTHROWnR(e,
                 DgcError(SPOS, "getPrivilege[%lld:%lld] failed.",
                          priv_in->user_sid, priv_in->enc_col_id),
                 -1);
    }
...
PccKredStmt::PccKredStmt(DgcPhyDatabase* pdb, DgcSession* session,
                         DgcSqlTerm* stmt_term)
    : DgcSqlStmt(pdb, stmt_term, session),
      UserVarRows(0),
      SelectListDef(0),
      PrivStmt(0),
      TLOG(0) {}
...
dgt_sint8 PccKredStmt::describe(DgcClass* def) throw(DgcLdbExcept) { return 0; }

DgcClass* PccKredStmt::fetchListDef() throw(DgcLdbExcept) {
    if (SelectListDef == 0) {
        THROWnR(DgcDbNetExcept(
                    DGC_EC_DN_INVALID_ST,
                    new DgcError(SPOS, "fetch row format not defined yet")),
                0);
    }
