...
PccKredStmtAlert::PccKredStmtAlert(DgcPhyDatabase* pdb, DgcSession* session,
...
dgt_sint32 PccKredStmtAlert::execute(
    ...
    if (brow->op_type == 3) {
        memset(sql_text, 0, 1024);
        dg_sprintf(sql_text,
                   "select level_id, level_count from pct_alert_level where "
                   "level_count<=%lld order by level_count desc",
                   brow->dec_count);
        sql_stmt = DgcDbProcess::db().getStmt(DgcDbProcess::sess(), sql_text,
                                              strlen(sql_text));
        e = 0;
        if (sql_stmt == 0 || sql_stmt->execute() < 0) {
            delete EXCEPTnC;
            delete sql_stmt;
            //                	RTHROWnR(e,DgcError(SPOS,"execute[security_level
            //                query] failed."),-1);
        } else {
            if ((tmp_row = sql_stmt->fetch()) == 0) {
                delete EXCEPTnC;
    ...
    //
    // for cipher alerting
    // brow->op_type             /* 1 -> encrypt no priv, 2 -> decrypt no priv,
    // 3 -> too many decrypt */
#if 0
	dgt_schar desc[1025];
	memset(desc,0,1025);
