...
PccKredStmtApprove::PccKredStmtApprove(DgcPhyDatabase* pdb, DgcSession* session,
...
dgt_sint32 PccKredStmtApprove::execute(
    ...
    dg_sprintf(sql_text, "select * from pct_enc_column where enc_col_id=%lld",
               brow->enc_col_id);
    ...
    if (sql_stmt == 0 || sql_stmt->execute() < 0) {
        e = EXCEPTnC;
        delete sql_stmt;
        RTHROWnR(e, DgcError(SPOS, "execute[%lld] failed.", brow->user_sid),
                 -1);
    }
