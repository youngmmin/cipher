...
PccKredStmtDetectFileGetRequest::PccKredStmtDetectFileGetRequest(DgcPhyDatabase* pdb,DgcSession* session,DgcSqlTerm* stmt_term)
...
dgt_sint32 PccKredStmtDetectFileGetRequest::execute(DgcMemRows* mrows, dgt_sint8 delete_flag) throw(DgcLdbExcept,DgcPdbExcept)
{
	if (!mrows || !mrows->next()) {
	...
	dg_sprintf(sql_text,"select dir_id, file_id, pttn_num, file_name from pct_file_detect_hist where dir_id = %lld\n", dir_id);
...
dgt_uint8* PccKredStmtDetectFileGetRequest::fetch() throw(DgcLdbExcept,DgcPdbExcept)
{
	if (IsExecuted == 0) {
