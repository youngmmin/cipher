...
PccKredStmtDetectFileLogRequest::PccKredStmtDetectFileLogRequest(DgcPhyDatabase* pdb,DgcSession* session,DgcSqlTerm* stmt_term)
...
dgt_sint32 PccKredStmtDetectFileLogRequest::execute(DgcMemRows* mrows,dgt_sint8 delete_flag) throw(DgcLdbExcept,DgcPdbExcept)
{
	if (!mrows || !mrows->next()) {
		THROWnR(DgcLdbExcept(DGC_EC_LD_STMT_ERR,new DgcError(SPOS,"no bind row")),-1);
        }
	defineUserVars(mrows);
	DgcTableSegment*	tab=(DgcTableSegment*)DgcDbProcess::db().pdb()->segMgr()->getTable("PCT_FILE_DETECT_HIST_TEMP");
	if (tab == 0) {
		DgcExcept*	e=EXCEPTnC;
		if (e) {
			DgcWorker::PLOG.tprintf(0,*e,"getTable[PCT_FILE_DETECT_HIST_TEMP] failed:\n");
			delete e;
		} else {
			DgcWorker::PLOG.tprintf(0,"Table[PCT_FILE_DETECT_HIST_TEMP] not found.\n");
		}
	} else {
		tab->unlockShare();
		DgcRowList	rows(tab);
		...
		if (tab->pinInsert(DgcDbProcess::sess(),rows,1) != 0) {
			DgcExcept*	e=EXCEPTnC;
			DgcWorker::PLOG.tprintf(0,*e,"pinInsert[PCT_FILE_DETECT_HIST_TEMP] failed:\n");
			delete e;
		} else {
			rows.rewind();
			rows.next();
			...
			if (tab->insertCommit(DgcDbProcess::sess(), rows) != 0) {
				...
				DgcWorker::PLOG.tprintf(0,*e,"insertCommit[PCT_FILE_DETECT_HIST_TEMP] failed:\n");
				delete e;
			}
		}
	}
...
dgt_uint8* PccKredStmtDetectFileLogRequest::fetch() throw(DgcLdbExcept,DgcPdbExcept)
{
	if (IsExecuted == 0) {
