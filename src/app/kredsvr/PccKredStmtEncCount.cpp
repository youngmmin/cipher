...
PccKredStmtEncCount::PccKredStmtEncCount(DgcPhyDatabase* pdb,
                                         DgcSession* session,
...
dgt_sint32 PccKredStmtEncCount::execute(
    ...
    if (job_id) {
        ...
        dg_sprintf(
            sql_text,
            "insert into pct_worker(job_id,PROCESSED_ROWS) values(%lld,%lld)",
            job_id, brow->enc_count);
        DgcSqlStmt* sql_stmt = DgcDbProcess::db().getStmt(
            DgcDbProcess::sess(), sql_text, strlen(sql_text));
        DgcExcept* e = 0;
        if (sql_stmt == 0 || sql_stmt->execute() < 0) {
