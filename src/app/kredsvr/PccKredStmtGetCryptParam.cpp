...
PccKredStmtGetCryptParam::PccKredStmtGetCryptParam(DgcPhyDatabase* pdb,DgcSession* session,DgcSqlTerm* stmt_term)
...
dgt_sint32 PccKredStmtGetCryptParam::execute(DgcMemRows* mrows,dgt_sint8 delete_flag) throw(DgcLdbExcept,DgcPdbExcept)
{
	if (!mrows || !mrows->next()) {
...
"where   param_name = '%s' ", crypt_param_name);
