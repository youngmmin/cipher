...
PccKredStmtGetEci::PccKredStmtGetEci(DgcPhyDatabase* pdb,DgcSession* session,DgcSqlTerm* stmt_term)
...
dgt_sint32 PccKredStmtGetEci::execute(DgcMemRows* mrows,dgt_sint8 delete_flag) throw(DgcLdbExcept,DgcPdbExcept)
{
	if (!mrows || !mrows->next()) {
	...
	if (strncmp(ec_name,PCI_DFLT_HASH_COL_NAME,131) == 0) {
		...
		if (*ec_name) {
			...
			for(dgt_sint32 i=1; i<=3 && (names[i]=strtok_r(0, ".", &last)); i++) num_names++;
			if (num_names < 4) for(dgt_sint32 i=0; i<4; i++) if (i < num_names) names[3-i] = names[num_names-i-1]; else names[3-i] = "";
		}
		DgcMemRows      v_bind(4);
		v_bind.addAttr(DGC_SCHR,130,"col_name");
        	...
        	v_bind.rewind();
		sprintf(sql_text,
			"select a.enc_col_id from pct_enc_column a, pct_encrypt_key f, pct_enc_table b, pct_enc_schema c, pt_database d "
			" where upper(a.column_name) = upper(decode(:1,'',a.column_name,:1))"
...
//			"   and f.cipher_type != 4"
			...
			"   and upper(c.schema_name) = upper(decode(:3,'',c.schema_name,:3))"
			"   and c.db_id = d.db_id"
