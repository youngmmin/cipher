...
PccKredStmtGetIV::PccKredStmtGetIV(DgcPhyDatabase* pdb,DgcSession* session,DgcSqlTerm* stmt_term)
...
dgt_sint32 PccKredStmtGetIV::execute(DgcMemRows* mrows,dgt_sint8 delete_flag) throw(DgcLdbExcept,DgcPdbExcept)
{
	if (!mrows || !mrows->next()) {
	...
	if ((ext_iv=(pct_type_ext_iv*)sql_stmt->fetch())) {
		...
		eiv.getIV(ext_iv->iv_no,ext_iv->seiv,ext_iv->seivs,IVInfo.iv_size,IVInfo.iv);
	}
	e=EXCEPTnC;
	delete sql_stmt;
	if (e) {
		RTHROWnR(e,DgcError(SPOS,"fetch external iv_no[%u] failed",iv_in->iv_type),-1);
	}
	IsExecuted=1;
	NumRtnRows=0;
	return 0;
