...
PccKredStmtGetKey::PccKredStmtGetKey(DgcPhyDatabase* pdb, DgcSession* session,
...
dgt_sint32 PccKredStmtGetKey::execute(
    ...
    memcpy(v_bind.getColPtr(1), &key_id, sizeof(dgt_sint64));
    v_bind.rewind();
    dg_sprintf(sql_text, "select * from pct_encrypt_key where key_id=:1");
    DgcSqlStmt* sql_stmt = DgcDbProcess::db().getStmt(
        DgcDbProcess::sess(), sql_text, strlen(sql_text));
    DgcExcept* e = 0;
    if (sql_stmt == 0 || sql_stmt->execute(&v_bind, 0) < 0) {
    ...
    if (enc_key->expire_action && enc_key->expire_date) {
        ...
        if (curr_time > enc_key->expire_date) {
            THROWnR(DgcLdbExcept(DGC_EC_PD_NOT_FOUND,
                                 new DgcError(SPOS, "encrypt key expired!!")),
                    0);
        }
    }
    delete sql_stmt;
    ...
    if (key_no >= 10000) {
        ...
        if ((ext_key = (pct_type_ext_key*)sql_stmt->fetch())) {
            ...
            ek.getKey(ext_key->key_no, ext_key->sek, ext_key->seks,
                      KeyInfo.key_size / 8, KeyInfo.key);
        }
        e = EXCEPTnC;
        delete sql_stmt;
        if (e) {
            RTHROWnR(e,
                     DgcError(SPOS, "fetch external key_no[%u] failed", key_no),
                     -1);
        }
    } else {
        if ((rtn = PCI_getEncryptKey(key_no, KeyInfo.key_size / 8,
                                     KeyInfo.key)) < 0) {
