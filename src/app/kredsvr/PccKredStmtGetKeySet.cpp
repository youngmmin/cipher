...
PccKredStmtGetKeySet::PccKredStmtGetKeySet(DgcPhyDatabase* pdb,
                                           DgcSession* session,
...
dgt_sint32 PccKredStmtGetKeySet::execute(
    ...
    if ((rtn = PCI_getKeyStash(&key_stash)) < 0) {
        THROWnR(DgcLdbExcept(
                    DGC_EC_LD_STMT_ERR,
                    new DgcError(SPOS, "PCI_getKeyStash failed due to %d:%s",
    ...
    if (key_stash->open_status == 0) {
        THROWnR(
            DgcLdbExcept(
                ...
                new DgcError(
                    SPOS,
                    "PCI_getKeyStash failed due to %d:The key set not opened",
                    PCC_ERR_KMGR_KEY_NOT_OPEN)),
            -1);
    }
    memcpy(KeySet.key_set_signature, key_stash->key_set_signature,
