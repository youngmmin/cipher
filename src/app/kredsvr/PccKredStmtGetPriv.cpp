...
PccKredStmtGetPriv::PccKredStmtGetPriv(DgcPhyDatabase* pdb, DgcSession* session,
...
dgt_sint32 PccKredStmtGetPriv::execute(
    ...
    if (sql_stmt == 0 || sql_stmt->execute(&v_bind, 0) < 0) {
        e = EXCEPTnC;
        delete sql_stmt;
        RTHROWnR(e, DgcError(SPOS, "execute[%lld] failed.", brow->enc_col_id),
                 -1);
    }
    ...
    if (enc_col.multi_byte_flag)
        PrivInfo.max_col_len = enc_col.data_length * 3;
    else
        PrivInfo.max_col_len = enc_col.data_length;
    if (strcasecmp(enc_col.data_type, "NUMBER") == 0)
        PrivInfo.col_type = PCI_SRC_TYPE_NUM;
    else if (strcasecmp(enc_col.data_type, "DATE") == 0)
        PrivInfo.col_type = PCI_SRC_TYPE_DATE;
    else if (strcasecmp(enc_col.data_type, "TIMESTAMP") == 0)
        PrivInfo.col_type = PCI_SRC_TYPE_DATE;
    else if (strcasecmp(enc_col.data_type, "RAW") == 0)
        PrivInfo.col_type = PCI_SRC_TYPE_RAW;
    else if (strcasecmp(enc_col.data_type, "CHAR") == 0)
        PrivInfo.col_type = PCI_SRC_TYPE_CHAR;
    else if (strcasecmp(enc_col.data_type, "NCHAR") == 0)
        PrivInfo.col_type = PCI_SRC_TYPE_CHAR;
    else
    ...
    if ((getPrivilege((pc_type_get_priv_in*)mrows->data(), &PrivInfo,
                      &rqst_hist)) < 0) {
        ATHROWnR(DgcError(SPOS, "getPrivilege[%lld] failed.", brow->enc_col_id),
                 -1);
    }
    IsExecuted = 1;
    NumRtnRows = 0;
