...
PccKredStmtGetRegEngine::PccKredStmtGetRegEngine(DgcPhyDatabase* pdb,DgcSession* session,DgcSqlTerm* stmt_term)
...
dgt_sint32 PccKredStmtGetRegEngine::execute(DgcMemRows* mrows,dgt_sint8 delete_flag) throw(DgcLdbExcept,DgcPdbExcept)
{
	if (!mrows || !mrows->next()) {
...
"  and   a.pattern_id = '%lld'",reg_engine_id);
...
dgt_uint8* PccKredStmtGetRegEngine::fetch() throw(DgcLdbExcept,DgcPdbExcept)
{
	...
	THROWnR(DgcLdbExcept(DGC_EC_PD_NOT_FOUND,new DgcError(SPOS,"not found pttn")),0);
	return 0;
}
