...
PccKredStmtGetRegEngineId::PccKredStmtGetRegEngineId(DgcPhyDatabase* pdb,DgcSession* session,DgcSqlTerm* stmt_term)
...
dgt_sint32 PccKredStmtGetRegEngineId::execute(DgcMemRows* mrows,dgt_sint8 delete_flag) throw(DgcLdbExcept,DgcPdbExcept)
{
	if (!mrows || !mrows->next()) {
	...
	dgt_schar	sql_text[1024];
		sprintf(sql_text,
			...
			" where pattern_name = '%s' ", reg_engine_name);
...
dgt_uint8* PccKredStmtGetRegEngineId::fetch() throw(DgcLdbExcept,DgcPdbExcept)
{
	if (IsExecuted == 0) {
