...
PccKredStmtGetRsaKey::PccKredStmtGetRsaKey(DgcPhyDatabase* pdb,DgcSession* session,DgcSqlTerm* stmt_term)
...
dgt_sint32 PccKredStmtGetRsaKey::execute(DgcMemRows* mrows,dgt_sint8 delete_flag) throw(DgcLdbExcept,DgcPdbExcept)
{
	if (!mrows || !mrows->next()) {
...
"from    pct_rsa_key "
"where   upper(key_name) = upper('%s') ", key_name);
	...
	if (!sql_handle.fetch(rtn_row) && rtn_row) {
		...
		memcpy(ResultParam,(dgt_schar*)rtn.key_str,strlen((dgt_schar*)rtn.key_str));
	} else {
		THROWnR(DgcLdbExcept(DGC_EC_LD_STMT_ERR,new DgcError(SPOS,"get rsa key[%s] failed",key_name)),-1);
	}
