...
PccKredStmtGetVKeyDbPriv::PccKredStmtGetVKeyDbPriv(DgcPhyDatabase* pdb,
                                                   DgcSession* session,
...
dgt_sint32 PccKredStmtGetVKeyDbPriv::execute(
    ...
    pc_type_get_vkey_db_priv_in* brow =
        (pc_type_get_vkey_db_priv_in*)mrows->data();
    ...
    if (EncColumn.enc_col_id) {
        ...
        if (target_row_list.numRows() > 0) {
            ...
            while (
                ...
                // check db_name
                if ((ret = matchTargetName(db_name, vkey_target->name1, 0)) <
                    0) {
                    DgcExcept* e = EXCEPTnC;
                    if (e) {
                        DgcWorker::PLOG.tprintf(
                            0, *e, "matchTargetName failed[db_name:%s]\n",
                            db_name);
                        delete e;
                    }
                    continue;
                ...
                // check schema_name
                if ((ret = matchTargetName(schema_name, vkey_target->name2,
                                           0)) < 0) {
                    DgcExcept* e = EXCEPTnC;
                    if (e) {
                        DgcWorker::PLOG.tprintf(
                            0, *e, "matchTargetName failed[schema_name:%s]\n",
                            schema_name);
                        delete e;
                    }
                    continue;
                ...
                if ((ret = matchTargetName(db_sess_user_name,
                                           vkey_target->name3, 0)) < 0) {
                    DgcExcept* e = EXCEPTnC;
                    if (e) {
                        DgcWorker::PLOG.tprintf(
                            0, *e,
                            "matchTargetName failed[db_sess_user_name:%s]\n",
                            db_sess_user_name);
                        delete e;
                    }
                    continue;
                ...
                // check table_name
                if ((ret = matchTargetName(table_name, vkey_target->name4, 0)) <
                    0) {
                    DgcExcept* e = EXCEPTnC;
                    if (e) {
                        DgcWorker::PLOG.tprintf(
                            0, *e, "matchTargetName failed[table_name:%s]\n",
                            table_name);
                        delete e;
                    }
                    continue;
                ...
                // check column_name
                if ((ret = matchTargetName(column_name, vkey_target->name5,
                                           0)) < 0) {
                    DgcExcept* e = EXCEPTnC;
                    if (e) {
                        DgcWorker::PLOG.tprintf(
                            0, *e, "matchTargetName failed[column_name:%s]\n",
                            column_name);
                        delete e;
                    }
                    continue;
