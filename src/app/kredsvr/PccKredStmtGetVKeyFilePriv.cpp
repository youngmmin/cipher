...
PccKredStmtGetVKeyFilePriv::PccKredStmtGetVKeyFilePriv(DgcPhyDatabase* pdb,DgcSession* session,DgcSqlTerm* stmt_term)
...
dgt_sint32 PccKredStmtGetVKeyFilePriv::execute(DgcMemRows* mrows,dgt_sint8 delete_flag) throw(DgcLdbExcept,DgcPdbExcept)
{
	if (!mrows || !mrows->next()) {
		THROWnR(DgcLdbExcept(DGC_EC_LD_STMT_ERR,new DgcError(SPOS,"no bind row")),-1);
	}
	defineUserVars(mrows);
	pc_type_get_vkey_file_priv_in*	brow = (pc_type_get_vkey_file_priv_in*)mrows->data();
	...
	if (EncColumn.enc_col_id) {
		...
		if (target_row_list.numRows() > 0) {
			...
			while (target_row_list.next() && (vkey_target=(pvat_type_cipher_vkey_target*)target_row_list.data())) {
				// check host_name
				if ((ret=matchTargetName(host_name,vkey_target->name1,1)) < 0) {
					DgcExcept* e = EXCEPTnC;
					if (e) {
						DgcWorker::PLOG.tprintf(0,*e,"matchTargetName failed[host_name:%s]\n",host_name);
						delete e;
					}
					continue;
				} else if(ret == 0) {
				...
				// check os_user
				if ((ret=matchTargetName(os_user,vkey_target->name2,1)) < 0) {
					DgcExcept* e = EXCEPTnC;
					if (e) {
						DgcWorker::PLOG.tprintf(0,*e,"matchTargetName failed[os_user:%s]\n",os_user);
						delete e;
					}
					continue;
				} else if(ret == 0) {
				...
				// check file_path
				if ((ret=matchTargetName(file_path,vkey_target->name3,1)) < 0) {
					DgcExcept* e = EXCEPTnC;
					if (e) {
						DgcWorker::PLOG.tprintf(0,*e,"matchTargetName failed[file_path:%s]\n",file_path);
						delete e;
					}
					continue;
				} else if(ret == 0) {
				...
				// check file_name
				if ((ret=matchTargetName(file_name,vkey_target->name4,1)) < 0) {
					DgcExcept* e = EXCEPTnC;
					if (e) {
						DgcWorker::PLOG.tprintf(0,*e,"matchTargetName failed[file_name:%s]\n",file_name);
						delete e;
					}
					continue;
				} else if(ret == 0) {
