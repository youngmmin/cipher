...
PccKredStmtGetVKeyPriv::PccKredStmtGetVKeyPriv(DgcPhyDatabase* pdb,
                                               DgcSession* session,
...
dgt_sint32 PccKredStmtGetVKeyPriv::getEncColumn(
    dgt_sint64 virtual_key_id, dgt_uint64 user_sid,
    dgt_uint8 crypt_type) throw(DgcExcept) {
    //
    // get virtual key
    //
    DgcMemRows v_bind(1);
    ...
    dg_sprintf(sql_text,
               "select * from pvat_cipher_virtual_key where virtual_key_id=:1");
    ...
    if (sql_stmt == 0 || sql_stmt->execute(&v_bind, 0) < 0) {
        ...
        RTHROWnR(
            e,
            DgcError(SPOS, "virtual key execute[%lld] failed.", virtual_key_id),
            -1);
    }
    ...
    if ((virtual_key = (pvat_type_cipher_virtual_key*)sql_stmt->fetch()) == 0) {
        ...
        delete sql_stmt;
        RTHROWnR(
            e, DgcError(SPOS, "virtual key fetch[%lld] failed", virtual_key_id),
            -1);
    }

    if (virtual_key->crypt_type && virtual_key->crypt_type != crypt_type)
        return 10;
    ...
    if (virtual_key->valid_from && virtual_key->valid_to) {
        ...
        if (str_time <= end_time &&
            (timeinfo->tm_hour < str_time || timeinfo->tm_hour >= end_time))
            rtn = 3;
        if (str_time > end_time &&
            !((timeinfo->tm_hour >= str_time && timeinfo->tm_hour < 24) ||
              (timeinfo->tm_hour < end_time && timeinfo->tm_hour >= 0)))
            rtn = 3;
        if (rtn) {
    ...
    if (virtual_key->check_ip_flag) {
        pt_type_sess_user* sess_user =
            PccKredSessionPool::getSessUser(user_sid);
        if (sess_user == 0) {
    ...
    if (sql_stmt == 0 || sql_stmt->execute(&v_bind, 0) < 0) {
        ...
        delete sql_stmt;
        RTHROWnR(e,
                 DgcError(SPOS, "enc column execute[%lld] failed.", enc_col_id),
                 -1);
    }
    ...
    if ((enc_column = (pct_type_enc_column*)sql_stmt->fetch()) == 0) {
        e = EXCEPTnC;
        delete sql_stmt;
        RTHROWnR(e, DgcError(SPOS, "enc column fetch[%lld] failed", enc_col_id),
                 -1);
    }
    ...
    if (EncColumn.multi_byte_flag)
        PrivInfo.max_col_len = EncColumn.data_length * 3;
    else
        PrivInfo.max_col_len = EncColumn.data_length;
    if (strcasecmp(EncColumn.data_type, "NUMBER") == 0)
        PrivInfo.col_type = PCI_SRC_TYPE_NUM;
    else if (strcasecmp(EncColumn.data_type, "DATE") == 0)
        PrivInfo.col_type = PCI_SRC_TYPE_DATE;
    else if (strcasecmp(EncColumn.data_type, "TIMESTAMP") == 0)
        PrivInfo.col_type = PCI_SRC_TYPE_DATE;
    else if (strcasecmp(EncColumn.data_type, "RAW") == 0)
        PrivInfo.col_type = PCI_SRC_TYPE_RAW;
    else if (strcasecmp(EncColumn.data_type, "CHAR") == 0)
        PrivInfo.col_type = PCI_SRC_TYPE_CHAR;
    else if (strcasecmp(EncColumn.data_type, "NCHAR") == 0)
        PrivInfo.col_type = PCI_SRC_TYPE_CHAR;
    else
        PrivInfo.col_type = PCI_SRC_TYPE_VARCHAR;
    delete sql_stmt;
    ...
    memset(sql_text, 0, 256);
    dg_sprintf(sql_text,
               "select enc_col_id from pct_enc_index where enc_col_id=:1 and "
               "index_type =1");
    sql_stmt = DgcDbProcess::db().getStmt(DgcDbProcess::sess(), sql_text,
                                          strlen(sql_text));
    e = 0;
...
dgt_sint32 PccKredStmtGetVKeyPriv::matchTargetName(
    dgt_schar* target_name, dgt_sint64 cmp_name_id,
    dgt_uint8 case_sensitive) throw(DgcExcept) {
    ...
    if (cmp_name_id) {
        if ((name_len = PetraNamePool->getNameLen(cmp_name_id)) > 0) {
            ...
            if (rtv == 0) {
                ATHROWnR(
                    DgcError(SPOS, "getName failed[name_id:%lld]", cmp_name_id),
                    -1);
            } else {
                dgt_sint32 ret = DgcExprLang::getStrStr(target_name, NameStr,
                                                        case_sensitive);
                if (ret) {
                    NameStr[name_len] = 0;
                    match_flag = 1;
