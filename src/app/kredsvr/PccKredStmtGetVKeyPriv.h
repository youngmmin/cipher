...
class PccKredStmtGetVKeyPriv : public PccKredStmt {
   ...
   protected:
    ...
    dgt_sint32 getEncColumn(dgt_sint64 virtual_key_id, dgt_uint64 user_sid,
                            dgt_uint8 crypt_type) throw(DgcExcept);
    dgt_sint32 matchTargetName(dgt_schar* target_name, dgt_sint64 cmp_name_id,
                               dgt_uint8 case_sensitive = 0) throw(DgcExcept);

   public:
    PccKredStmtGetVKeyPriv(DgcPhyDatabase* pdb, DgcSession* session,
                           DgcSqlTerm* stmt_term);
    virtual ~PccKredStmtGetVKeyPriv();

    virtual dgt_sint32 execute(
        DgcMemRows* mrows = 0,
        dgt_sint8 delete_flag = 1) throw(DgcLdbExcept, DgcPdbExcept) = 0;
