...
const dgt_schar PccKredStmtGetZoneParam::FIXED_PARAM_FORMAT[] =
    "(fixed=(zone_id=%lld)(col_lengths=%s)(out_col_lengths=%s)(lead_space_trim="
    "%s)(tail_space_trim=%s))\n";

PccKredStmtGetZoneParam::PccKredStmtGetZoneParam(DgcPhyDatabase* pdb,
                                                 DgcSession* session,
...
dgt_sint32 PccKredStmtGetZoneParam::execute(
    ...
    if (!rtn_row) {
        THROWnR(DgcLdbExcept(
                    DGC_EC_PD_NOT_FOUND,
                    new DgcError(SPOS,
                                 "enc_zone not found : pfct_enc_zone.id[%lld]",
                                 zone_id)),
                -1);
    }
    ...
    *((dgt_sint64*)zone_id_rows.data()) = enc_zone->enc_zone_id;
    ...
    // set num_rows
    if (enc_zone->file_format == 2 /*PFCC_TYPE_FILE_FORMAT_PTTN*/)
        pttn_num_rows++;
    else if (enc_zone->file_format == 3 /*PFCC_TYPE_FILE_FORMAT_DELI*/)
        deli_num_rows++;
    else if (enc_zone->file_format == 4 /*PFCC_TYPE_FILE_FORMAT_FIXED*/)
        fixed_num_rows++;
    ...
    sprintf(stext,
            ...
            "c.column_name enc_col_name from pfct_enc_zone_col_key k, "
            "pct_enc_column c "
    ...
    while (!(ret = sql_handle.fetch(rtn_row)) && rtn_row) {
        ...
        if (col_key_out->pattern_id) {
            ...
            memcpy(pttn_col_key_rows.data(), col_key_out,
                   sizeof(pct_kred_apb_get_col_key_out));
        } else {
            key_no++;
        }

        sprintf(KeyParam + idx, "(%u=(name=%s)(columns=%u))", key_no,
                col_key_out->enc_col_name, col_key_out->column_no);
        idx += dg_strlen(KeyParam + idx);
    }
    if (idx) {
    ...
    if (pttn_num_rows > 0) {
        ...
        while (pttn_col_key_rows.next() &&
               (col_key_out =
                    (pct_kred_apb_get_col_key_out*)pttn_col_key_rows.data())) {
    ...
    if (fixed_num_rows > 0) {
        ...
        memset(stext, 0, 256);
        sprintf(stext,
                "select max(strlen(out_col_lengths)) from "
                "pfct_enc_fixed_file_format "
                "where enc_zone_id = :1 ");
        zone_id_rows.rewind();
        if (sql_handle.execute(stext, dg_strlen(stext), &zone_id_rows) < 0)
            ATHROWnR(DgcError(SPOS, "execute failed"), -1);
        ...
        if (max_out_col_len > 0)
            len = fixed_num_rows * (max_out_col_len * 2 + PARAM_MIN_LEN);
        else
            len =
                fixed_num_rows * (FIXED_PARAM_MAX_LEN_PER_ROW + PARAM_MIN_LEN);
        ...
        while (!(ret = sql_handle.fetch(rtn_row)) && rtn_row) {
            ...
            sprintf(FixedParam + idx, FIXED_PARAM_FORMAT,
                    file_format->enc_zone_id, file_format->col_lengths,
    ...
    dgt_uint32 param_len =
        key_param_size + reg_param_size + deli_param_size + fixed_param_size;

    if (param_len > 2048)
        THROWnR(DgcLdbExcept(DGC_EC_LD_STMT_ERR,
                             new DgcError(SPOS, "reslut param size too big[%u]",
                                          param_len)),
                -1);
