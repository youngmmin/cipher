...
PccKredStmtLogRequest::PccKredStmtLogRequest(DgcPhyDatabase* pdb,
                                             DgcSession* session,
...
dgt_sint32 PccKredStmtLogRequest::execute(
    ...
    if (tab == 0) {
        ...
        if (e) {
            DgcWorker::PLOG.tprintf(0, *e,
                                    "getTable[PCT_CRYPT_HIST_TEMP] failed:\n");
            delete e;
        } else {
            DgcWorker::PLOG.tprintf(0,
                                    "Table[PCT_CRYPT_HIST_TEMP] not found.\n");
        }
    } else {
        tab->unlockShare();
        DgcRowList rows(tab);
        ...
        if (tab->pinInsert(DgcDbProcess::sess(), rows, 1) != 0) {
            DgcExcept* e = EXCEPTnC;
            DgcWorker::PLOG.tprintf(0, *e,
                                    "pinInsert[PCT_CRYPT_HIST_TEMP] failed:\n");
            delete e;
        } else {
            rows.rewind();
            rows.next();
            ...
            if (tab->insertCommit(DgcDbProcess::sess(), rows) != 0) {
                ...
                DgcWorker::PLOG.tprintf(
                    0, *e, "insertCommit[PCT_CRYPT_HIST_TEMP] failed:\n");
                delete e;
            }
        }
    }
