...
PccKredStmtOpenSess::PccKredStmtOpenSess(DgcPhyDatabase* pdb,
                                         DgcSession* session,
...
dgt_sint32 PccKredStmtOpenSess::execute(
    ...
    if (uinfo->os_user && *uinfo->os_user) {
        ...
        if (ConvCharset.convCharSet(uinfo->os_user, 33, outbuffer, 33) < 0) {
            ...
            memset(outbuffer, 0, 33);
            if (ConvCharset.convCharSet(uinfo->os_user, 33, outbuffer, 33) <
                0) {
                delete EXCEPTnC;
            }
        }
    ...
    if (uinfo->client_program && *uinfo->client_program) {
        ...
        if (ConvCharset.convCharSet(uinfo->client_program, 128, outbuffer,
                                    128) < 0) {
            delete EXCEPTnC;
            ConvCharset.reset("UTF-8", "CP949");
            memset(outbuffer, 0, 128);
    ...
    //
    if (strcasecmp(uinfo->client_ip, "local") == 0)
        strncpy(uinfo->client_ip, Session->clientCommIP(), 64);
    ...
    //	if (auth_fail_code == PAUC_AUTH_ERRCODE_CANCEL || auth_fail_code ==
    //PAUC_AUTH_ERRCODE_AUTH_FAIL) {
    if (auth_fail_code < 0) {
