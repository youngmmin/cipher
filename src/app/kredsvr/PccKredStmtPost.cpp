...
PccKredStmtPost::PccKredStmtPost(DgcPhyDatabase* pdb,DgcSession* session,DgcSqlTerm* stmt_term)
...
dgt_sint32 PccKredStmtPost::execute(DgcMemRows* mrows,dgt_sint8 delete_flag) throw(DgcLdbExcept,DgcPdbExcept)
{
...
//                      RTHROWnR(e,DgcError(SPOS,"fetch[%lld] failed",brow->user_sid),-1);
	...
	//
	memset(sql_text,0,1024);
	dg_sprintf(sql_text, "select b.table_name, a.column_name "
	...
	if (brow->err_code == -30301) {
		...
		//
		sprintf(msg,"\r\n\r\n[%s.%s] Encryption Rejected by Petra Cipher.",col_info.table_name,col_info.column_name);
		post_ctl.postCipherMsg(msg);
	} else if (brow->err_code == -30389) {
		//
		// src_Data Length > max Column Length exceed
		//
		sprintf(msg,"\r\n\r\n[%s.%s] data max length exceeds.",col_info.table_name,col_info.column_name);
		post_ctl.postCipherMsg(msg);
	} else if (brow->err_code == -30115) {
		//
		// double encryption  error
		//
		sprintf(msg,"\r\n\r\n[%s.%s] already encrypted data.",col_info.table_name,col_info.column_name);
		post_ctl.postCipherMsg(msg);
	}
