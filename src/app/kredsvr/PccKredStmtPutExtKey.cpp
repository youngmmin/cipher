...
PccKredStmtPutExtKey::PccKredStmtPutExtKey(DgcPhyDatabase* pdb,
                                           DgcSession* session,
...
dgt_sint32 PccKredStmtPutExtKey::execute(
    ...
    dg_sprintf(sql_text,
               "select soha_link from ppmt_product where product_id = 0");
    DgcSqlStmt* sql_stmt =
    ...
    if ((link_name = (dgt_schar*)sql_stmt->fetch())) {
        ...
        dgt_schar sql_text[512];
        memset(sql_text, 0, 512);
        sprintf(sql_text, "select * from PCP_CREATE_EXT_KEY('%s','%s',%u)",
                key_in->key_name, key_in->key, key_in->format_no);
        if (cli_stmt->execute(sql_text, strlen(sql_text)) < 0) {
            e = EXCEPTnC;
            delete cli_stmt;
            RTHROWnR(e,
                     DgcError(SPOS, "master sql[%s] execute failed", sql_text),
                     -1);
        }
        e = EXCEPTnC;
        delete cli_stmt;
