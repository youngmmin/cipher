...
dgt_sint32 PccKredStmtUserFileLogRequest::execute(
    ...
    pc_type_user_file_request_in* brow =
        (pc_type_user_file_request_in*)mrows->data();
    DgcTableSegment* tab =
        (DgcTableSegment*)DgcDbProcess::db().pdb()->segMgr()->getTable(
            "PCT_USER_FILE_CRYPT_HIST_TEMP");
    if (tab == 0) {
        DgcExcept* e = EXCEPTnC;
        if (e) {
            DgcWorker::PLOG.tprintf(
                0, *e, "getTable[PCT_USER_FILE_CRYPT_HIST_TEMP] failed:\n");
            delete e;
        } else {
            DgcWorker::PLOG.tprintf(
                0, "Table[PCT_USER_FILE_CRYPT_HIST_TEMP] not found.\n");
        }
    } else {
        tab->unlockShare();
        DgcRowList rows(tab);
        ...
        if (tab->pinInsert(DgcDbProcess::sess(), rows, 1) != 0) {
            DgcExcept* e = EXCEPTnC;
            DgcWorker::PLOG.tprintf(
                0, *e, "pinInsert[PCT_USER_FILE_CRYPT_HIST_TEMP] failed:\n");
            delete e;
        } else {
            rows.rewind();
            rows.next();
            ...
            if (tab->insertCommit(DgcDbProcess::sess(), rows) != 0) {
                ...
                DgcWorker::PLOG.tprintf(
                    0, *e,
                    "insertCommit[PCT_USER_FILE_CRYPT_HIST_TEMP] failed:\n");
                delete e;
            }
        }
    }
...
dgt_uint8* PccKredStmtUserFileLogRequest::fetch() throw(DgcLdbExcept,
                                                        DgcPdbExcept) {
    if (IsExecuted == 0) {
        THROWnR(
