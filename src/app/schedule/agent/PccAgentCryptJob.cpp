...
PccAgentCryptJob::PccAgentCryptJob(dgt_sint64 job_id, dgt_sint32 trace_level,
                                   dgt_sint32 fail_file_queue_size,
                                   dgt_sint32 nullity_file_queue_size)
    : JobID(job_id),
      StartTime(0),
      EndTime(0),
      Repository(trace_level),
      Collector(0) {
    NSL = 0;
    ELF = 0;
...
dgt_sint32 PccAgentCryptJob::lock() throw(DgcExcept) {
    if (DgcSpinLock::lock(&Lock) != 0) {
        THROWnR(DgcBgmrExcept(
                    ...
                    new DgcError(SPOS, "job_id[%lld] lock timeout.", JobID)),
                -1);
    }
    return 0;
}
        ...
        if (max_nap > 0 && nap_count >= max_nap) {
            THROWnR(
                DgcBgmrExcept(
                    ...
                    new DgcError(
                        SPOS,
                        "job_id[%lld] lockExclusive[ELF:%u][NSL:%d] timeout.",
                        JobID, ELF, NSL)),
                -1);
        }
        napAtick();
        ...
        if (max_nap > 0 && nap_count >= max_nap) {
            THROWnR(
                DgcBgmrExcept(
                    DGC_EC_BG_INCOMPLETE,
                    new DgcError(
                        SPOS, "job_id[%lld] lockShare[ELF:%u][NSL:%d] timeout.",
                        JobID, ELF, NSL)),
                -1);
        }
        napAtick();
    ...
    if (lock() != 0) {
        ...
        if (e) {
            DgcWorker::PLOG.tprintf(
                0, *e,
                "job_id[%lld] lock failed while unlockShare[ELF:%u][NSL:%d]\n",
                JobID, ELF, NSL);
            delete e;
        }
        return 0;
...
dgt_sint32 PccAgentCryptJob::start(
    dgt_uint8 job_type, dgt_uint8 job_status, dgt_sint32 file_queue_size,
    dgt_sint32 collecting_interval) throw(DgcExcept) {
    DgcWorker::PLOG.tprintf(0, "CryptJob Library Version test\n");
    // DgcWorker::PLOG.tprintf(0,"CryptJob Library Version
    // [%d][%d][%d]\n",getBogoModule()->msGetMajorVersion(),
    // getBogoModule()->msGetMinorVersion(),
    // getBogoModule()->msGetPatchVersion());
    ...
    if (file_queue_size > 0) {
        if (Repository.fileQueue().initializeQueue(file_queue_size) == 0) {
            THROWnR(DgcBgmrExcept(
                        DGC_EC_BG_INCOMPLETE,
                        new DgcError(SPOS,
                                     "initializeQueue failed : queue_size[%d]",
                                     file_queue_size)),
                    0);
        }
        if (Repository.migrationFileQueue().initializeQueue(file_queue_size) ==
            0) {
            THROWnR(DgcBgmrExcept(
                        DGC_EC_BG_INCOMPLETE,
                        new DgcError(
                            SPOS,
                            "initializeMigrationQueue failed : queue_size[%d]",
                            file_queue_size)),
                    0);
        }
    }
    ...
    if (FailFileQueueSize > 0) {
        if (Repository.failFileQueue().initializeQueue(FailFileQueueSize) ==
            0) {
            THROWnR(DgcBgmrExcept(
                        DGC_EC_BG_INCOMPLETE,
                        new DgcError(
                            SPOS, "initializeFailQueue failed : queue_size[%d]",
                            FailFileQueueSize)),
                    0);
        }
    }
    if (NullityFileQueueSize > 0) {
        if (Repository.nullityFileQueue().initializeQueue(
                NullityFileQueueSize) == 0) {
            THROWnR(
                DgcBgmrExcept(
                    DGC_EC_BG_INCOMPLETE,
                    new DgcError(
                        SPOS, "initializeNullityQueue failed : queue_size[%d]",
    ...
    Collector = new PccCryptTargetCollector(Repository, collecting_interval,
                                            job_status);
