...
PccAgentStmt* PccAgentCursorTable::getStmt(dgt_uint32 cursor) throw(
    DgcDbNetExcept) {
    if (cursor == 0 || cursor > MaxOpenCursors ||
        OpenCursors[cursor - 1].agentStmt() == 0) {
        THROWnR(
            DgcDbNetExcept(DGC_EC_DN_INVALID_CUR,
                           new DgcError(SPOS, "invalid cursor[%d]", cursor)),
            0);
    }
    return OpenCursors[cursor - 1].agentStmt();
}

dgt_uint32 PccAgentCursorTable::openCursor(
    PccAgentStmt* agent_stmt, dgt_uint32 cursor) throw(DgcDbNetExcept) {
    if (cursor == 0) {
        ...
        THROWnR(
            DgcDbNetExcept(DGC_EC_DN_INVALID_CUR,
                           new DgcError(SPOS, "reach the MaxOpenCursors[%d]",
                                        MaxOpenCursors)),
            0);
    }
...
dgt_void PccAgentCursorTable::closeStmt(dgt_uint32 cursor) {
    if (cursor > 0 && cursor <= MaxOpenCursors)
        OpenCursors[cursor - 1].closeStmt();
}

dgt_void PccAgentCursorTable::closeAllCursors() {
    for (dgt_uint32 cursor = 0; cursor < MaxOpenCursors; cursor++)
        OpenCursors[cursor].close();
}
