...
PccCipherAgentSvrSession::PccCipherAgentSvrSession(
    PccAgentCryptJobPool& job_pool, dgt_sint32 sess_id,
    dgt_sint32 no_sess_sleep_cnt, dgt_schar* p_ip, dgt_uint16 p_port,
    dgt_schar* s_ip, dgt_uint16 s_port)
    : JobPool(job_pool),
      SessID(sess_id),
      CommStream(0),
      PktStream(0),
    ...
    SkipCheckConn = 0;
    NoSessionSleepCount =
        no_sess_sleep_cnt ? no_sess_sleep_cnt : NO_SESSION_SLEEP_COUNT;
    CurrSleepCount = 0;
    PrimaryIP = p_ip;
    PrimaryPort = p_port;
    SecondaryIP = s_ip;
...
dgt_sint32 PccCipherAgentSvrSession::talkType() throw(DgcExcept) {
    if (!MsgStream)
        THROWnR(DgcDbNetExcept(
                    DGC_EC_DN_NOT_FOUND,
                    new DgcError(SPOS, "message stream is not allocated")),
                -1);
    ...
    if (MsgStream->recvMessage(10) <= 0) {
        ...
        THROWnR(DgcDbNetExcept(DGC_EC_DN_TIMEOUT,
                               new DgcError(SPOS, "timeout waiting for ACK")),
                -1);
    }
    DgcMessage* msg = MsgStream->currMsg();
    if (!msg) {
        ATHROWnR(DgcError(SPOS, "message is not allocated"), -1);
        THROWnR(DgcDbNetExcept(DGC_EC_DN_NOT_FOUND,
                               new DgcError(SPOS, "message is not allocated")),
                -1);
    }
    if (msg->opi() != DGIEXT) {
        THROWnR(DgcDbNetExcept(
                    DGC_EC_DN_TIMEOUT,
                    new DgcError(SPOS, "not DGIEXT message[%d]", msg->opi())),
                -1);
    }
    DgcMsgDgiExt* ext = (DgcMsgDgiExt*)msg;
    if (ext->getExcept()->classid() != 0) {
        RTHROWnR(
            ((DgcMsgDgiExt*)msg)->cutExcept(),
            DgcError(SPOS, "client returned an exception[%u] err_code[%d]",
                     ext->getExcept()->classid(), ext->getExcept()->errCode()),
            -1);
    }
    MsgStream->resetIBuf();
    return 0;
...
dgt_sint32 PccCipherAgentSvrSession::makeConnection(
    dgt_schar* ip, dgt_uint16 port) throw(DgcExcept) {
    if (MsgStream) delete MsgStream;
    if (PktStream) delete PktStream;
    if (CommStream) delete CommStream;
    MsgStream = 0;
        ...
        if (*SecondaryIP) {
            ...
            if (e) {
                DgcWorker::PLOG.tprintf(0, *e,
                                        ...
                                        "connecServer to primary failed:\n",
                                        JobPool.agentID(), SessID);
                delete e;
            }
            if (makeConnection(SecondaryIP, SecondaryPort) < 0) {
                ATHROWnR(DgcError(SPOS, "connectServer to secondary failed."),
                         -1);
            }
        } else {
...
dgt_sint32 PccCipherAgentSvrSession::doRequest(DgcMsgDgiSqlRq* srm) throw(
    DgcExcept) {
...
dgt_sint32 PccCipherAgentSvrSession::doResponse(DgcMsgDgiSqlRq* srm) throw(
    DgcExcept) {
...
dgt_sint32 PccCipherAgentSvrSession::run() throw(DgcExcept) {
    ...
    if ((rtn = MsgStream->recvMessage(1)) < 0) {
        ...
        DgcWorker::PLOG.tprintf(
            0, *e, "agent[%lld] agent_svr_session[%d] recvMessage failed:\n",
            JobPool.agentID(), SessID);
        delete e;
        BrokenConnFlag = 1;
        return 0;
        ...
        if (MsgStream->currMsg()->opi() == DGISQLRQ) {
            ...
            CurrStmt = 0;
            pr_debug(
                "srm->tti() [%d], srm->opi() [%d] srm->seq() [%d], "
                "srm->operation() [%d], srm->sqlText() [%s]\n",
                srm->tti(), srm->opi(), srm->seq(), srm->operation(),
                srm->sqlText());
            // 1. do request job
            if (doRequest(srm) != 0) {
                e = EXCEPTnC;
            ...
            if (e == 0 && (srm->operation() & DGC_OPF_PARSE)) {
                // new sql statement
                if ((new_cursor = CursorTable.openCursor(CurrStmt,
                                                         srm->cursor())) == 0) {
                    e = EXCEPTnC;
            ...
            if (CurrStmt && (srm->operation() & DGC_OPF_PARSE) &&
                unreg_stmt_flag) {
                delete CurrStmt;
                CurrStmt = 0;
            }
            ...
            BrokenConnFlag = 1;
            return 0;
        } else if (CheckConnFlag &&
                   MsgStream->currMsg()->opi() == PTDGILETTER) {
            CheckConnFlag = 0;
            SkipCheckConn = 0;
            return 0;
        } else {
            // invalid request
            DgcWorker::PLOG.tprintf(0, "invalid request message[%d:%d:%d]",
        ...
        if (e) {
            ...
            DgcWorker::PLOG.tprintf(
                0, *e,
                "agent[%lld] agent_svr_session[%d] exception occurred:\n",
                JobPool.agentID(), SessID);
            DgcPktMarker* marker =
                (DgcPktMarker*)newPacket(DgcPacket::DGC_PKT_MARKER);
            if (!marker) {
                delete e;
                e = EXCEPTnC;
                if (e) {
                    DgcWorker::PLOG.tprintf(
                        0, *e,
                        "agent[%lld] agent_svr_session[%d] newPacket failed:\n",
                        JobPool.agentID(), SessID);
                    delete e;
                    BrokenConnFlag = 1;
                    return 0;
            ...
            if (setPacket() != 0) {
                ...
                if (e) {
                    DgcWorker::PLOG.tprintf(
                        0, *e,
                        "agent[%lld] agent_svr_session[%d] setPacket failed:\n",
                        JobPool.agentID(), SessID);
                    delete e;
                    BrokenConnFlag = 1;
                    return 0;
            ...
            for (;;) {
                if ((rtn = moveNext()) < 0) {
                    ...
                    if (e) {
                        DgcWorker::PLOG.tprintf(
                            0, *e,
                            "agent[%lld] agent_svr_session[%d] moveNext "
                            "failed:\n",
                            JobPool.agentID(), SessID);
                        delete e;
                        BrokenConnFlag = 1;
    ...
    if (EXCEPT) {
        ...
        if (e) {
            DgcWorker::PLOG.tprintf(
                0, *e, "agent[%lld] agent_svr_session[%d] got an exception :\n",
                JobPool.agentID(), SessID);
            delete e;
            BrokenConnFlag = 1;
            return 0;
