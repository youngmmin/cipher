	...
	: PrimaryPort(0),SecondaryPort(0)
{
	...
	for (dgt_sint32 i=0; i<MAX_NUM_AGENT_SVR_SESS; i++) AgentSvrSession[i] = 0;
...
dgt_sint32 PccCipherAgentSvrSessionPool::cleanBrokenSessions() throw(DgcExcept)
{
...
dgt_sint32 PccCipherAgentSvrSessionPool::startNewSessions(PccAgentCryptJobPool& job_pool, dgt_sint32 no_sess_sleep_cnt) throw(DgcExcept)
{
	dgt_sint32 new_session_count = 0;
	for (dgt_sint32 i=0; i<NumAgentSvrSess; i++) {
		if (AgentSvrSession[i] == 0) {
			AgentSvrSession[i] = new PccCipherAgentSvrSession(job_pool,i+1,no_sess_sleep_cnt,PrimaryIP,PrimaryPort,SecondaryIP,SecondaryPort);
			if (AgentSvrSession[i]->connect() < 0) {
				delete AgentSvrSession[i];
				AgentSvrSession[i] = 0;
				ATHROWnR(DgcError(SPOS,"agent[%lld] agent_svr_session[%d] connect failed while starting\n",job_pool.agentID(),i+1),-1);
			}
			if (AgentSvrSession[i]->start(1)) {
				delete AgentSvrSession[i];
				AgentSvrSession[i] = 0;
				ATHROWnR(DgcError(SPOS,"start agent_svr_session[%d] failed",i+1),-1);
			}
...
dgt_sint32 PccCipherAgentSvrSessionPool::startSessions(PccAgentCryptJobPool& job_pool, dgt_sint32 no_sess_sleep_cnt) throw(DgcExcept)
{
	for (dgt_sint32 i=0; i<NumAgentSvrSess; i++) {
		if (AgentSvrSession[i]) {
			AgentSvrSession[i]->askStop();
		...
		if (AgentSvrSession[i]->connect() < 0) {
			...
			if (e) {
				DgcWorker::PLOG.tprintf(0,*e,"agent[%lld] agent_svr_session[%d] connect failed while starting\n",job_pool.agentID(),i+1);
				delete e;
			}
		}
		if (AgentSvrSession[i]->start(1)) {
