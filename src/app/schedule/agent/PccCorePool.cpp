...
PccCorePool::PccCorePool() : TotalCores(0), FreeCores(0), Cores(0) {
    Cores = new PccCores[MAX_CORES];
    for (dgt_sint32 i = 0; i < MAX_CORES - 1; i++)
        Cores[i].setNext(&(Cores[i + 1]));
    FirstFree = &(Cores[0]);
