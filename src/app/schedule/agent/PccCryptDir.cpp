 ...
 *   date                    modification
 *   180713					 add isTargetFile, isTargetDir, compileDirPttns, compileFilePttns member
 *   						 remove statdir
--------------------------------------------------------------------
...
PccCryptDir::PccCryptDir(dgt_sint64 dir_id,PccCryptSchedule& schedule,PccCryptTargetFileQueue& file_queue,PccCryptTargetFileQueue& migration_file_queue, PccCryptZone* crypt_zone,const dgt_schar* src_dir,const dgt_schar* dst_dir, pct_crypt_zone_dir_rule* dir_rule, dgt_sint32 trace_level, dgt_uint8 status)
	: DirID(dir_id),Schedule(schedule),FileQueue(file_queue),MigrationFileQueue(migration_file_queue), CryptZone(crypt_zone),CryptMir(0)
{
	CurrDirDepth = 0;
...
dgt_sint8 PccCryptDir::cryptStatus(const dgt_schar* file_path) throw(DgcExcept)
{
	...
	if (CryptZone->headerFlag()) {
		...
		if((rtn=(dgt_sint8)PccHeaderManager::isEncrypted(file_path,&header_info)) < 0) {
			if (rtn == -3) ATHROWnR(DgcError(SPOS,"encrypting is currently underway"),-3); //this case is currently underway ,file encryptiong
			else if (rtn == -1) ATHROWnR(DgcError(SPOS,"header checking crypt status failed"),-1);
		}
#if 1
		...
		if (rtn == 1 && curr_key_id != 10 && curr_key_id < 255) { //chageable key
			dgt_uint8 enc_file_key_id = 0;
			//get file key id
			if ((header_info.version == 2 || header_info.version == 3 || header_info.version == 4) && header_info.key_id) { //exist key_id in header
				enc_file_key_id = header_info.key_id;
			} else { //not exist key_id in header, key_id is lastbyte of encrypted file
		...
		// rtn should be different from crypt_flag even it occur duplicating cryption
		if (CryptZone->encryptFlag()) rtn = 0;
		else rtn = 1;
	}
	return rtn;
}

dgt_sint32 PccCryptDir::countCryptFile(dgt_schar* src_dir, dgt_sint8 encrypt_flag) throw(DgcExcept)
{
	...
	for (;(rtn=readdir_r(dir_ptr,entry,&result)) == 0;) { // success
		...
		if (S_ISDIR(fstat.st_mode)) { // directory
			...
			if (enc_check < 0){
				...
				continue;
			} else if (enc_check == encrypt_flag) crypt_count++;
...
dgt_void PccCryptDir::filter0(PccCryptMir* parent_mir,dgt_schar* src_dir,dgt_schar* dst_dir, dgt_sint32 is_target_dir) throw(DgcExcept)
{
	...
	for (;(rtn=readdir_r(dir_ptr,entry,&result)) == 0;) { // success
		...
		if (S_ISDIR(fstat.st_mode)) { // directory
			...
			//
			// need method which returns crypting status, cryptStatus(const dgt_schar* file_path)
			// with return values: 0 -> text, 1 -> encrypted
			//
			...
			if (enc_check < 0) {
				...
				if (e) {
					if (enc_check == -3) {
						if (TraceLevel > 10) DgcWorker::PLOG.tprintf(0,"encrypting is current underway[%s]\n",src_file);
					} else {
						DgcWorker::PLOG.tprintf(0,*e,"isEncrypted[%s] failed:\n",src_file);
						TmpCryptStat.check_errors++;
					}
				...
				while(MigrationFileQueue.put(
							...
							src_dir_len,
							src_file,
							src_dir_len) == 0) napAtick();
				file_node->check_migration = 1;
if(TraceLevel > 20) DgcWorker::PLOG.tprintf(0,"input migration file queue src_file[%s]\n",src_file);
				}
				continue;
...
dgt_void PccCryptDir::filter1(PccCryptMir* parent_mir,const dgt_schar* src_dir,const dgt_schar* dst_dir, dgt_uint8 is_target_dir) throw(DgcExcept)
{
	...
	if (CurrDirDepth >= MAX_DIR_DEPTH) {
		DgcWorker::PLOG.tprintf(0,"[%s] current depth is too much deep to search.\n",src_dir);
		TmpCryptStat.check_errors++;
		CurrDirDepth--;
		return;
	}
	...
	for (;(rtn=readdir_r(dir_ptr,entry,&result)) == 0;) { // success
		...
		if (DirFileList[CurrDirDepth].dst_file_blen < dst_file_len) {
			if (DirFileList[CurrDirDepth].dst_file) delete DirFileList[CurrDirDepth].dst_file;
			dgt_sint32 buf_len = dg_strlen(dst_dir) + (dg_strlen(entry->d_name)>MAX_FILE_LEN ? dg_strlen(entry->d_name) : MAX_FILE_LEN) + 1;
			DirFileList[CurrDirDepth].dst_file = new dgt_schar[buf_len];
			DirFileList[CurrDirDepth].dst_file_blen = buf_len;
		}
		memset(DirFileList[CurrDirDepth].dst_file,0,DirFileList[CurrDirDepth].dst_file_blen);
		if (S_ISDIR(fstat.st_mode)) { // directory
			dgt_uint8 is_target_dir_flag = is_target_dir;
			if (strcmp(entry->d_name,".") && strcmp(entry->d_name,"..")) {
				if (isSearchDirDepth(CurrDirDepth)) {
					if (isTargetDir(entry->d_name)) is_target_dir_flag = 1;
					else is_target_dir_flag = 0;
				} 
...
pr_debug("search dir src_file[%s]\n",DirFileList[CurrDirDepth].src_file);
			...
			// with return values: 0 -> text, 1 -> encrypted
			//
			dgt_sint8 enc_check = cryptStatus(DirFileList[CurrDirDepth].src_file);
			if (enc_check < 0){
				DgcExcept* e=EXCEPTnC;
				if (e) {
					DgcWorker::PLOG.tprintf(0,*e,"isEncrypted[%s] failed:\n",DirFileList[CurrDirDepth].src_file);
				...
				TmpCryptStat.check_errors++;
				continue;
			} else if (enc_check == CryptZone->encryptFlag()) continue;
			TmpCryptStat.check_files++;
...
dgt_sint64 PccCryptDir::loadDetectRqst(PccCryptMir* parent_mir)
{
	...
	dgt_sint32 sid = -1;
	if ((sid=PcaApiSessionPool::getApiSession("","","","","","",0)) < 0) return 0;
...
dgt_void PccCryptDir::filter2(PccCryptMir* parent_mir,dgt_schar* src_dir, dgt_sint32 is_target_dir) throw(DgcExcept)
{
	TmpCryptStat.check_dirs++;
	DIR*	dir_ptr = opendir(src_dir);
	if (dir_ptr == NULL) {
...
{
	if (Status == PCC_STATUS_TYPE_DELETED || Status == PCC_STATUS_TYPE_PAUSE) return 0;
	if (lock() == 0) {
		...
		if (Schedule.isWorkingTime()) {
			...
			switch (dirRuleVersion()) {
				case 1: // Scoped directory depth filter (crypt)
					CurrDirDepth = -1;
					filter1(CryptMir,SrcDir,DstDir && *DstDir ? DstDir : SrcDir, 1);
					break;
				...
				default:// Full directory depth filter (crypt)
					filter0(CryptMir,TmpSrcPath,TmpDstPath, isEncDirDepth(0)); // old filter version
					break;
			}
...
dgt_sint32 PccCryptDir::compileDirPttns(DgcBgrammer* bg,dgt_schar* err_string) throw(DgcExcept)
{       
	if (DirExprs) {
		delete DirExprs;
		DirExprs = 0;
	...
	for(expr_no=1;;expr_no++) {
		...
		if ((val=bg->getValue(expr_string)) && *val) {
			...
			if ((rtn=DirExprs->compileStr(tmp_val,err_string)) < 0) {
				THROWnR(DgcOsExcept(errno,new DgcError(SPOS,"compileStr[%s] failed",val)),rtn);
			}
		} else {
			break;
		}
...
dgt_sint32 PccCryptDir::compileFilePttns(DgcBgrammer* bg,dgt_schar* err_string) throw(DgcExcept)
{
	if (FileExprs) {
		delete FileExprs;
		FileExprs = 0;
...
dgt_sint32 PccCryptDir::isTargetDir(const dgt_schar* src_dir)
{
	...
	while((preg=DirExprs->nextPttn())) { // each dir pattern
		if (regexec(&preg->reg,src_dir,1,pttn_match,0) == 0) { 
				//&& (pttn_match[0].rm_eo - pttn_match[0].rm_so) == (dgt_sint32)strlen(src_dir)) {
...
dgt_sint32 PccCryptDir::isTargetFile(const dgt_schar* src_file)
{
	...
	while((preg=FileExprs->nextPttn())) { // each file pattern
		if (regexec(&preg->reg,src_file,1,pttn_match,0) == 0 &&
		    (pttn_match[0].rm_eo - pttn_match[0].rm_so) == (dgt_sint32)strlen(src_file)) {
			return 1;
		}
	}
