...
class PccCryptDir : public DgcObject {
  private:
	...
	dgt_void filter0(PccCryptMir* parent_mir,dgt_schar* src_dir,dgt_schar* dst_dir, dgt_sint32 is_target_dir=1) throw(DgcExcept);	// Full directory depth filter (crypt)
	dgt_void filter1(PccCryptMir* parent_mir,const dgt_schar* src_dir,const dgt_schar* dst_dir, dgt_uint8 is_target_dir=0) throw(DgcExcept);	// Scoped directory depth filter (crypt)
	dgt_void filter2(PccCryptMir* parent_mir,dgt_schar* src_dir, dgt_sint32 is_target_dir=1) throw(DgcExcept);	// Full directory depth filter (pattern detect)
	dgt_sint64 loadDetectRqst(PccCryptMir* parent_mir);
  protected:
  public:
	PccCryptDir(dgt_sint64 dir_id,PccCryptSchedule& schedule,PccCryptTargetFileQueue& file_queue,PccCryptTargetFileQueue& migration_file_queue, PccCryptZone* crypt_zone,const dgt_schar* src_dir,const dgt_schar* dst_dir=0, pct_crypt_zone_dir_rule* dir_rule = 0, dgt_sint32 trace_level = 0, dgt_uint8 status = 1);
	virtual ~PccCryptDir();
	...
	inline dgt_uint8 isSearchDirDepth(dgt_sint32 curr_depth) {
		if (curr_depth >= DirRule.search_first_depth && curr_depth <= DirRule.search_last_depth) return 1;
		return 0; 
	};
	inline dgt_uint8 isEncDirDepth(dgt_sint32 curr_depth) {
		if (curr_depth >= DirRule.enc_first_depth && curr_depth <= DirRule.enc_last_depth) return 1;
		return 0; 
	};
	...
	dgt_sint32 countCryptFile(dgt_schar* src_dir, dgt_sint8 encrypt_flag=1) throw(DgcExcept);
	dgt_sint32 compileDirPttns(DgcBgrammer* bg,dgt_schar* err_string) throw(DgcExcept);
	dgt_sint32 compileFilePttns(DgcBgrammer* bg,dgt_schar* err_string) throw(DgcExcept);
