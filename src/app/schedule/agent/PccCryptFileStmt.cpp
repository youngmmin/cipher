...
dgt_sint32 PccCryptFileStmt::filter(const dgt_schar* src_dir,
                                    const dgt_schar* dst_dir) throw(DgcExcept) {
    DIR* DirPtr;
    if ((DirPtr = opendir(src_dir)) == NULL) {
        THROWnR(DgcOsExcept(errno,
                            new DgcError(SPOS, "opendir[%s] failed", src_dir)),
                -1);
    }
    ...
    if (JobPool.traceLevel() > 10)
        DgcWorker::PLOG.tprintf(0, "src_dir [%s] filter start  \n", src_dir);
    struct stat fstat;
    for (; (rtn = readdir_r(DirPtr, entry, &result)) == 0;) {  // success
        ...
        if (stat(src_file, &fstat) < 0) {
            THROWnR(DgcOsExcept(errno, new DgcError(SPOS, "stat[%s] failed",
                                                    entry->d_name)),
                    -1);
            delete src_file;
            delete dst_file;
        ...
        if (S_ISDIR(fstat.st_mode) == 1) {  // when directory
            if (strcmp(entry->d_name, ".") && strcmp(entry->d_name, "..")) {
                if (JobPool.traceLevel() > 10)
                    DgcWorker::PLOG.tprintf(0, "src_file : %s  is directory \n",
                                            src_file);
                if (src_dir == dst_dir) {
            ...
            if (JobPool.traceLevel() > 10)
                DgcWorker::PLOG.tprintf(0, "src_file : %s  is regular file \n",
                                        src_file);
            if ((rtn = (dgt_sint8)PccHeaderManager::isEncrypted(src_file)) <
                0) {
            ...
            if ((rtn == 1 && (CryptFileIn->crypt_flag == 2)) ||
                ...
                strncpy(target_list->in_file_name, src_file,
                        dg_strlen(src_file));
                strncpy(target_list->out_file_name, dst_file,
                        dg_strlen(dst_file));
                ...
                } else if (CryptFileIn->crypt_flag == 2)
                    strcat(target_list->out_file_name, ".dec");
            }
...
dgt_sint32 PccCryptFileStmt::buildParam() throw(DgcExcept) {
    ...
    if (rtn)
        THROWnR(DgcBgmrExcept(
                    ...
                    new DgcError(
                        SPOS, "get zone param failed [%lld]: Error Code - %d",
                        CryptFileIn->enc_zone_id, rtn)),
                -1);
    ...
    if (JobPool.traceLevel() > 10)
        DgcWorker::PLOG.tprintf(0, "get zone param :%d - %s \n", out_param_len,
                                out_param);
    ...
    sprintf(logging_param,
            "(logging_info=(ptu_id=%lld)(client_ip=%s))(mode=(crypt=%s)(user_"
            "logging=on))",
            CryptFileIn->ptu_id, CryptFileIn->client_ip,
            (CryptFileIn->crypt_flag == 1) ? "encrypt" : "decrypt");
    strcat(CryptParam, logging_param);
    if (JobPool.traceLevel() > 10)
        DgcWorker::PLOG.tprintf(0, "crypt_param:\n%s\n", CryptParam);

    return 0;
}

dgt_sint32 PccCryptFileStmt::execute(DgcMemRows* mrows,
    ...
    if (JobPool.traceLevel() > 10) {
        DgcWorker::PLOG.tprintf(
            0,
            "enc_zone_id: %lld crypt_flag: %d InFileName: %s OutFileName %s\n",
            CryptFileIn->enc_zone_id, CryptFileIn->crypt_flag,
            CryptFileIn->in_file_name, CryptFileIn->out_file_name);
    }
    ...
    if (stat(CryptFileIn->in_file_name, &fstat) == 0) {
        file_type =
            S_ISDIR(fstat.st_mode) ? 1 : 2;  // 1 -> direcroty, 2 -> file
    } else {                                 // not exist file
    }

    pcct_target_list* target_list;
    if (file_type == 1)
        filter(CryptFileIn->in_file_name,
               CryptFileIn->out_file_name);  // is directory
    else if (file_type == 2) {
        TargetList->add();
        TargetList->next();
        target_list = (pcct_target_list*)TargetList->data();
        strncpy(target_list->in_file_name, CryptFileIn->in_file_name,
                dg_strlen(CryptFileIn->in_file_name));
        strncpy(target_list->out_file_name, CryptFileIn->out_file_name,
                dg_strlen(CryptFileIn->out_file_name));
    }
    if (JobPool.traceLevel() > 10) {
    ...
    while (TargetList->next()) {
        ...
        if ((rtn = cryptor->crypt(SessionId, CryptParam,
                                  target_list->in_file_name,
                                  target_list->out_file_name)) < 0) {
            CryptFileOut->rtn_code = rtn;
            dgt_sint32 len = dg_strlen(cryptor->errString());
            strncpy(CryptFileOut->error_message, cryptor->errString(),
                    len > 1024 ? 1024 : len);
            delete cryptor;
            break;
        }
