...
PccCryptManager::PccCryptManager(PccAgentCryptJobPool& job_pool,
                                 PccCorePool& core_pool, dgt_sint32 manager_id,
                                 dgt_sint32 agent_mode, dgt_schar* enc_col_name,
                                 dgt_schar* header_flag, dgt_sint32 buffer_size)
    : JobPool(job_pool),
      CorePool(core_pool),
      SessionID(-1),
      CryptParams(0),
...
dgt_sint32 PccCryptManager::buildStreamParam(PccCryptTargetFile& tf,
                                             PccAgentCryptJob* curr_job) {
    PccCryptZone* crypt_zone = 0;
    if ((crypt_zone =
             curr_job->repository().zonePool().getZoneByID(tf.zoneID())) == 0) {
    ...
    if ((rtn = crypt_zone->buildParam(CryptParams, &remain_buf_len)) < 0) {
        ATHROWnR(DgcError(SPOS, "buildParam[zone:%lld] failed", tf.zoneID()),
                 rtn);
    }

    if (OutFileNameLen < dg_strlen(tf.dstFileName())) {
        delete OutFileName;
        OutFileNameLen = dg_strlen(tf.dstFileName()) +
                         PccCryptTargetFile::EXTRA_FILE_NAME_LEN;
        OutFileName = new dgt_schar[OutFileNameLen];
    }
    ...
    if (tf.isSameFile()) {
        if (crypt_zone->hasOutExtension()) {
            sprintf(OutFileName, "%s.%s", TargetFile.dstFileName(),
                    crypt_zone->outExtension());
        } else {
            THROWnR(DgcBgmrExcept(
                        DGC_EC_BG_INCOMPLETE,
                        new DgcError(SPOS, "dst file[%s] is same to src file",
...
dgt_sint32 PccCryptManager::buildParam(
    PccCryptTargetFile& tf, PccAgentCryptJob* curr_job, dgt_sint32 threads,
    dgt_sint32 migration_flag)  // return the number of cores when success
{
    PccCryptZone* crypt_zone = 0;
    if ((crypt_zone =
             curr_job->repository().zonePool().getZoneByID(tf.zoneID())) == 0) {
    ...
    memset(CryptParams, 0, MAX_PARAM_LEN);
    if (AgentMode <= 1) {
        if ((rtn = crypt_zone->buildParam(CryptParams, &remain_buf_len,
                                          migration_flag)) < 0) {
            ATHROWnR(
                DgcError(SPOS, "buildParam[zone:%lld] failed", tf.zoneID()),
                rtn);
        }
    }
    if (OutFileNameLen < dg_strlen(tf.dstFileName())) {
    ...
    if (migration_flag == 0) {
        if (crypt_zone->hasOutExtension()) {
            sprintf(OutFileName, "%s.%s", tf.dstFileName(),
                    crypt_zone->outExtension());
    ...
    if (AgentMode <= 1) {
        dgt_schar* cp = CryptParams + strlen(CryptParams);
        remain_buf_len = MAX_PARAM_LEN - remain_buf_len - 1;
        if (threads > 0 && (rtn = curr_job->repository().schedule().buildParam(
                                threads, tf, cp, &remain_buf_len)) < 0) {
...
dgt_sint32 PccCryptManager::fileCryption(PccAgentCryptJob* curr_job) throw(
    DgcExcept) {
    WorkStage = 5;
    ...
    threads = curr_job->repository().fileQueue().get(
        &TargetFile);  // get a crypt target file queued by the Collector
    if (threads == 0) {
        // get migrationfilequeue
        threads = curr_job->repository().migrationFileQueue().get(
            &TargetFile);  // get a crypt migration file queued by the Collector
    ...
    if (threads > 0) {
        ...
        for (dgt_sint32 i = 0;
             i < MAX_GET_CORE_TRY &&
             (cores = CorePool.getCores(threads ? threads : 1)) == 0;
             i++) {
            if (threads > 1)
                threads--;  // fail to get cores and decrease threads by 1
            napAtick();
        }
        if (JobPool.traceLevel() > 1)
            DgcWorker::PLOG.tprintf(
                0, "schedule cores : [%d] allocate cores : [%d]\n",
                curr_job->repository().schedule().usingCores(),
                cores ? cores->numCores() : 0);
        if (cores) {
            WorkStage = 7;

            if ((buildParam(TargetFile, curr_job, cores->numCores(),
                            migration_flag)) <
                0) {  // build crypt parameter string,  rtn>0 => # of threads,
                      // rtn<0 => exception
            ...
            if (JobPool.traceLevel() > 1) {
                DgcWorker::PLOG.tprintf(0,
                                        ...
                                        "src[%s] dst[%s]\n"
                                        "parameter[%s]\n",
                                        TargetFile.srcFileName(), OutFileName,
                                        CryptParams);
            }
            // create Cryptor
            if (Cryptor) {
            ...
            struct passwd* user;
            if (AgentMode > 0) {
                if ((file_rtn = stat(TargetFile.srcFileName(), &file_info)) !=
                    -1) {
            ...
            dgt_sint32 rtn = 0;
            // crypting
            if ((rtn = Cryptor->crypt(SessionID, CryptParams,
                                      TargetFile.srcFileName(), OutFileName,
                                      AgentMode, EncColName, HeaderFlag,
                                      BufferSize)) < 0) {
                //
                // rtn = last_error_code
                // error_string from cryptor couldn't be contained in exception
                // because of it's size = 1025
                //
#if 1
                if (curr_job->repository().failFileQueue().isFull())
                    curr_job->repository().failFileQueue().get(
                        0);  // delete oldest node when file queue is full
                curr_job->repository().failFileQueue().put(
                    TargetFile.dirID(), TargetFile.zoneID(),
                    TargetFile.cryptMir(), TargetFile.fileNode(),
                    TargetFile.cryptStat(), TargetFile.srcFileName(),
                    TargetFile.srcFileNamePos(), TargetFile.dstFileName(),
                    TargetFile.dstFileNamePos(), rtn, Cryptor->errString());
#endif
                TargetFile.cryptStat()->crypt_errors++;
                if (JobPool.traceLevel() > 0)
                    DgcWorker::PLOG.tprintf(0, "crypt failed [%s] : [%d:%s]\n",
                                            TargetFile.srcFileName(), rtn,
                                            Cryptor->errString());
                if (EXCEPT) {
                    DgcExcept* e = EXCEPTnC;
                    if (e) {
                        DgcWorker::PLOG.tprintf(
                            0, *e, "exception occured while crypt : \n");
                        delete e;
                    }
                }
                WorkStage = 10;
                ...
                EncFileCnt++;
                if (JobPool.traceLevel() > 0) {
                    if (EncFileCnt % 1000 == 0 || EncFileCnt == 1 ||
                        EncFileCnt % 1000 == 1)
                        DgcWorker::PLOG.tprintf(
                            0, "[%lld]files success crypt \n", EncFileCnt);
                }
            ...
            if (WorkStage == 8) {
                ...
                if (TargetFile.backupFlag()) {
                    ...
                    if (TargetFile.outExtensionFlag() == 0 &&
                        TargetFile.isSameFile()) {
                        memset(OutFileName, 0, OutFileNameLen);
                        sprintf(OutFileName, "%s_PFC_ORG",
                                TargetFile.dstFileName());
                        if (rename(TargetFile.srcFileName(), OutFileName) <
                            0) {  // rename the output file
                            DgcWorker::PLOG.tprintf(
                                0, "rename[%s] srcfile[%s] failed:%s\n",
                                OutFileName, TargetFile.srcFileName(),
                                strerror(errno));
                            WorkStage = 10;
                        }
                    }
                    ...
                    // remove file node
                    PccCryptDir* crypt_dir =
                        curr_job->repository().dirPool().getCryptDirWithDid(
                            TargetFile.dirID());
                    if (crypt_dir && crypt_dir->cryptMir() &&
                        TargetFile.fileNode()) {
                        dgt_uint64 file_id = TargetFile.fileNode()->file_id;
                        crypt_dir->cryptMir()->removeFileNode(file_id);
                    }
                    // remove source file
                    if (unlink(TargetFile.srcFileName()) <
                        0) {  // remove the source file
                    ...
                    if (JobPool.traceLevel() > 0)
                        DgcWorker::PLOG.tprintf(0, "remove src file : [%s]\n",
                                                TargetFile.srcFileName());
                    TargetFile.cryptStat()->target_files--;
                }
            }
            if (WorkStage == 9 && TargetFile.outExtensionFlag() == 0 &&
                TargetFile.isSameFile()) {
                //
                // need to rename the output file which has a extension "._PFC"
                // only if backing up or unlinking the source file successes.
                // and create a file node for it which should inherit some
                // attributes from the source file node.
                //
                memset(OutFileName, 0, OutFileNameLen);
                ...
                if (stat(OutFileName, &fstat) == 0) {
                    pcct_file_node* out_file_node =
                        TargetFile.cryptMir()->checkFileNode(&fstat);
                    ...
                    out_file_node->encrypt_flag =
                        TargetFile.fileNode()->encrypt_flag;
                    if (rename(OutFileName, TargetFile.dstFileName()) <
                        0) {  // rename the output file
...
dgt_void PccCryptManager::in() throw(DgcExcept) {
...
#endif
    SessionID = PcaApiSessionPool::getApiSession(
        sess_in.client_ip, sess_in.user_id, sess_in.client_program,
        sess_in.client_mac, sess_in.db_user, sess_in.os_user, sess_in.protocol);
#if 0
    ...
    if (SessionID < 0) {
        ...
        // THROW(DgcBgmrExcept(DGC_EC_BG_INCOMPLETE,new
        // DgcError(SPOS,"openSession failed")));
    }
    WorkStage = 3;
