...
class PccCryptManager : public DgcWorker {
   private:
    ...
    dgt_sint32 buildStreamParam(PccCryptTargetFile& tf,
                                PccAgentCryptJob* curr_job);
    dgt_sint32 buildParam(PccCryptTargetFile& tf, PccAgentCryptJob* curr_job,
                          dgt_sint32 threads = 0,
                          dgt_sint32 migration_flag =
                              0);  // return the number of cores when success
   protected:
   ...
   public:
    PccCryptManager(PccAgentCryptJobPool& job_pool, PccCorePool& core_pool,
                    dgt_sint32 manager_id, dgt_sint32 agent_mode = 0,
                    dgt_schar* enc_col_name = 0, dgt_schar* header_flag = 0,
