...
PccCryptManagerPool::PccCryptManagerPool(PccAgentCryptJobPool& job_pool, PccCorePool& core_pool)
...
dgt_sint32 PccCryptManagerPool::addManagers(dgt_sint32 num_managers, dgt_sint32 agent_mode, dgt_schar* enc_col_name, dgt_schar* header_flag, dgt_sint32 buffer_size) throw(DgcExcept)
{
	...
	for(dgt_sint32 i=0; i<num_managers; i++) {
		if (NumManagers < MAX_MANAGERS) {
			...
			if (agent_mode > 0) {
				manager = new PccCryptManager(JobPool,CorePool,i+1,agent_mode,enc_col_name,header_flag,buffer_size);
			} else {
			...
			if (manager->start(1)) {
				...
				delete manager;
				RTHROWnR(e, DgcError(SPOS,"start[PccCryptManager] failed"), -1);
			}
...
dgt_sint32 PccCryptManagerPool::cleanManagers(dgt_sint32 force_flag) throw(DgcExcept)
{
