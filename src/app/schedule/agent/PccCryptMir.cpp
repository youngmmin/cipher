...
PccCryptMir::PccCryptMir(dgt_uint64 file_id)
	: ChildMirList(CHILD_MIR_HASH_SIZE),FileNodeList(FILE_NODE_HASH_SIZE),FileID(file_id),
	  LastModified(0),CheckSeq(0),ParentSeq(0),ChangeFlag(1),DropFlag(0),RefCount(0)
{
...
PccCryptMir* PccCryptMir::getChildMir(dgt_uint64 file_id,dgt_time last_modified)
{
	for(;;) {
		PccCryptMir*	mir = 0;
...
dgt_void PccCryptMir::closeMir()
{
	...
	for(;;) {
		if (DgcSpinLock::lock(&NodeListLock) == 0) {
			...
			hnp = 0;
			FileNodeList.rewind();
			while((hnp=FileNodeList.nextNode()) && num_rfis < MAX_REMOVE_FILE) {
				pcct_file_node*	file_node = (pcct_file_node*)hnp->value();
				if (file_node->check_seq < CheckSeq && file_node->ref_count == 0) { // dropped file
	...
	for(;;) {
		if (DgcSpinLock::lock(&MirListLock) == 0) {
			...
			num_rfis = 0;
			hnp = 0;
			while((hnp=ChildMirList.nextNode()) && num_rfis < MAX_REMOVE_FILE) {
				PccCryptMir*	mir = (PccCryptMir*)hnp->value();
				if ((mir->dropFlag() || mir->parentSeq() != CheckSeq) && !mir->isRefered()) { // dropped file
					remove_file_ids[num_rfis++] = mir->fileID();
					delete mir;
					hnp->setValue(0);
				}
			}
			for(dgt_sint32 i=0; i<num_rfis; i++) ChildMirList.removeNode(remove_file_ids[i]);
			DgcSpinLock::unlock(&MirListLock);
		}
		if (num_rfis < MAX_REMOVE_FILE) break;
	}
...
PccCryptMir* PccCryptMir::findCryptMir(dgt_uint64 file_id,dgt_sint32 is_child_search)
{
	for(;;) {
		PccCryptMir* mir = 0;
		if (DgcSpinLock::lock(&MirListLock) == 0) {
			...
			if (hnp) {
				...
				ChildMirList.rewind();
				while((hnp=ChildMirList.nextNode())) {
					if ((mir=((PccCryptMir*)hnp->value())->findCryptMir(file_id,is_child_search))) break;
					mir = 0;
				}
			}
			DgcSpinLock::unlock(&MirListLock);
