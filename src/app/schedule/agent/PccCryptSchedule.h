...
typedef struct {
    ...
    dgt_sint64 run_size;  // the size per threads which is a base to compute the
                          // number of threads for a file
} pcct_week_schedule;
...
class PccCryptSchedule : public DgcObject {
   private:
    ...
    dgt_sint32 weekSchedule(
        dgt_sint32 fit = 0);  // fit : 0 -> first fit, 1 -> best pit
   ...
   public:
    ...
    dgt_sint32 buildParam(dgt_sint32 threads, PccCryptTargetFile& tf,
                          dgt_schar* buf, dgt_uint32* buf_len) throw(DgcExcept);
};
