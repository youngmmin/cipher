...
PccCryptTargetCollector::PccCryptTargetCollector(PccJobRepository& repository,
                                                 dgt_sint32 collecting_interval,
                                                 dgt_uint8 job_status)
    : Repository(repository),
      CollectingInterval(collecting_interval ? collecting_interval
                                             : COLLECT_INTERVAL),
...
dgt_sint32 PccCryptTargetCollector::run() throw(DgcExcept) {
    ...
    if (JobStatus == PCC_STATUS_TYPE_RUN) {
        ...
        for (dgt_sint32 i = 0;
             i < Repository.dirPool().numDirs() &&
             (crypt_dir = Repository.dirPool().getCryptDir(i));
             i++) {
            if (crypt_dir->status() == PCC_STATUS_TYPE_DELETED) {
                if (Repository.dirPool().dropCryptDir(crypt_dir->dirID()) < 0) {
                    ...
                    DgcWorker::PLOG.tprintf(
                        0, *e, "PccCryptDir[%lld:%s]::dropCryptDir Failed\n",
            ...
            if (crypt_dir->status() == PCC_STATUS_TYPE_DELETED ||
                crypt_dir->status() == PCC_STATUS_TYPE_PAUSE)
                continue;
            if (Repository.getJobType() == PCC_AGENT_TYPE_DETECT_JOB)
                crypt_dir->dirRule()->version = 2;
            if (crypt_dir->filter() < 0) {
                DgcExcept* e = EXCEPTnC;
                DgcWorker::PLOG.tprintf(
                    0, *e, "PccCryptDir[%lld:%s]::filter:\n",
                    crypt_dir->dirID(), crypt_dir->srcDir());
                delete e;
            }
        }
        if (Repository.getJobType() == PCC_AGENT_TYPE_TEMPORARY_JOB &&
            collect_flag == 1)
            return 1;
        if (Repository.getJobType() == PCC_AGENT_TYPE_DETECT_JOB &&
            collect_flag == 1) {
    ...
    for (dgt_sint32 i = 0; i < CollectingInterval; i++) {
        ...
        }      // for(dgt_sint32 i=0; i<Repository.dirPool().numDirs() &&
           // (crypt_dir=Repository.dirPool().getCryptDir(i)); i++) end
        sleep(1);
