...
class PccCryptTargetCollector : public DgcWorker {
   ...
   public:
    PccCryptTargetCollector(PccJobRepository& repository,
                            dgt_sint32 collecting_interval,
                            dgt_uint8 job_status = PCC_STATUS_TYPE_RUN);
    virtual ~PccCryptTargetCollector();

    inline dgt_void setJobStatus(dgt_uint8 job_status) {
        JobStatus = job_status;
    };
    inline dgt_void setCollectingInterval(dgt_sint32 itv) {
        CollectingInterval = itv;
    };
