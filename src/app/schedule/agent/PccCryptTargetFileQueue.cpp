...
dgt_void PccCryptTargetFile::reset(
    dgt_sint64 dir_id, dgt_sint64 zone_id, PccCryptMir* crypt_mir,
    pcct_file_node* file_node, pcct_crypt_stat* cs,
    const dgt_schar* src_file_name, dgt_sint32 src_file_name_pos,
    const dgt_schar* dst_file_name, dgt_sint32 dst_file_name_pos,
    dgt_sint32 error_code, const dgt_schar* error_msg) {
    DirID = dir_id;
    ZoneID = zone_id;
    CryptMir = crypt_mir;
    FileNode = file_node;
    ...
    // pr_debug("SrcFileNameLen[%d] src_file_name_len[%d]
...
dgt_void PccCryptTargetFile::copy(PccCryptTargetFile* tf) {
    if (!tf) return;
    reset(tf->dirID(), tf->zoneID(), tf->cryptMir(), tf->fileNode(),
          tf->cryptStat(), tf->srcFileName(), tf->srcFileNamePos(),
          tf->dstFileName(), tf->dstFileNamePos(), tf->errCode(), tf->errMsg());
    Next = tf->next();
...
dgt_sint32 PccCryptTargetFileQueue::initializeQueue(dgt_sint32 num_files) {
    if (DgcSpinLock::lock(&ListLock) == 0) {
        if (num_files > 0) {
            if (TargetFiles && TargetFiles != &NoHeapQueue)
                delete[] TargetFiles;
            while (num_files > 0 &&
                   (TargetFiles = new PccCryptTargetFile[num_files]) == 0)
                num_files = num_files / 2;  // insufficient heap
        }
...
dgt_sint32 PccCryptTargetFileQueue::queueCopy(DgcMemRows* tf) {
    ...
    if (DgcSpinLock::lock(&ListLock) == 0) {
        ...
        while (tmp) {
            ...
            strncpy(target_info->in_file_name, tmp->srcFileName(),
                    tmp->srcFileNameLen());
            strncpy(target_info->out_file_name, tmp->dstFileName(),
                    tmp->dstFileNameLen());  // gogo
            target_info->input_time = tmp->inputTime();
            if (tmp->errCode()) {
                target_info->error_code = tmp->errCode();
                strncpy(target_info->error_msg, tmp->errMsg(),
                        PccCryptTargetFile::MAX_ERROR_MSG);  // gogo
            }
            tmp = tmp->next();
        }
        DgcSpinLock::unlock(&ListLock);
...
dgt_sint32 PccCryptTargetFileQueue::get(PccCryptTargetFile* tf) {
    ...
    if (DgcSpinLock::lock(&ListLock) == 0) {
        // pr_debug("get FileCount[%d] First[%p] Last[%p] FirstFree[%p]
...
dgt_sint32 PccCryptTargetFileQueue::put(
    ...
    if (DgcSpinLock::lock(&ListLock) == 0) {
        // pr_debug("put FileCount[%d] First[%p] Last[%p] FirstFree[%p]
        // LastFree[%p]\n",FileCount,First,Last,FirstFree,LastFree);
        if ((free_file = FirstFree)) {
            if ((FirstFree = free_file->next()) == 0)
                FirstFree = LastFree = 0;  // now empty
            free_file->reset(dir_id, zone_id, crypt_mir, file_node, cs,
                             src_file_name, src_file_name_pos, dst_file_name,
                             dst_file_name_pos, error_code, error_msg);
            if (Last) {
    ...
    return free_file ? 1 : 0;  // 0 -> no more free target file which means the
                               // pool is full and caller might try again
}
...
dgt_void PccCryptTargetFileQueue::dump() {
    if (DgcSpinLock::lock(&ListLock) == 0) {
        ...
        while (tmp) {
            printf(
                "src[%s] dst[%s] dir_id[%lld] zone_id[%lld] mir[%p] "
                "file_node[%p] stat[%p]\n",
                tmp->srcFileName(), tmp->dstFileName(), tmp->dirID(),
                tmp->zoneID(), tmp->cryptMir(), tmp->fileNode(),
                tmp->cryptStat());
            if (tmp->errCode())
