...
class PccCryptTargetFile : public DgcObject {
  private:
	...
	dgt_sint32		SameFileFlag; // indicator if source and ouput file names are the same
	dgt_uint8		BackupFlag;	// sorcue file bakcup flag
	dgt_uint8		OutExtensionFlag; // out extension flag
	dgt_sint32		ErrCode; // indicator if source and ouput file names are the same
  ...
  public:
	...
	inline dgt_void setBackupFlag(dgt_uint8 bkup_flag) { BackupFlag = bkup_flag; };
	inline dgt_void setOutExtensionFlag(dgt_uint8 out_extension_flag) { OutExtensionFlag = out_extension_flag; };
	...
	dgt_void reset(
			...
			dgt_sint32 dst_file_name_pos=0,
			dgt_sint32 error_code=0,
...
class PccCryptTargetFileQueue : public DgcObject {
  ...
  public:
	...
	inline dgt_sint32 isFull() { if (NumFiles <= FileCount) return 1; else return 0; }
