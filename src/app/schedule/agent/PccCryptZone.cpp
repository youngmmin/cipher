 ...
 *   date                    modification
 *   180713					 move function dir_rule, isTargetDir, isTargetFile,
compileDirPttn, compileFilePttn to PccCryptDir
--------------------------------------------------------------------
...
PccCryptZone::PccCryptZone(dgt_sint64 zone_id) : ZoneID(zone_id) {
    RegularParam = 0;
    DelimiterParam = 0;
    FixedParam = 0;
    ErrString = 0;
...
dgt_sint32 PccCryptZone::lock() throw(DgcExcept) {
    if (DgcSpinLock::lock(&Lock) != 0) {
        THROWnR(DgcBgmrExcept(
                    ...
                    new DgcError(SPOS, "zone_id[%lld] lock timeout.", ZoneID)),
                -1);
    }
    return 0;
}

dgt_void PccCryptZone::unlock() throw(DgcExcept) { DgcSpinLock::unlock(&Lock); }
        ...
        if (max_nap > 0 && nap_count >= max_nap) {
            THROWnR(
                DgcBgmrExcept(
                    ...
                    new DgcError(
                        SPOS,
                        "zone_id[%lld] lockExclusive[ELF:%u][NSL:%d] timeout.",
                        ZoneID, ELF, NSL)),
                -1);
        }
        napAtick();
    ...
    if (lock() != 0) {
        ...
        if (e) {
            DgcWorker::PLOG.tprintf(0, *e,
                                    "zone_id[%lld] lock failed while "
                                    "unlockExclusive[ELF:%u][NSL:%d]\n",
                                    ZoneID, ELF, NSL);
            delete e;
        }
        return 0;
    ...
    for (;;) {
        ...
        if (max_nap > 0 && nap_count >= max_nap) {
            THROWnR(DgcBgmrExcept(
                        ...
                        new DgcError(
                            SPOS,
                            "zone_id[%lld] lockShare[ELF:%u][NSL:%d] timeout.",
                            ZoneID, ELF, NSL)),
                    -1);
        }
        napAtick();
    ...
    if (lock() != 0) {
        ...
        if (e) {
            DgcWorker::PLOG.tprintf(
                0, *e,
                "zone_id[%lld] lock failed while unlockShare[ELF:%u][NSL:%d]\n",
                ZoneID, ELF, NSL);
            delete e;
        }
        return 0;
...
dgt_sint32 PccCryptZone::checkParams(dgt_schar* err_string) throw(DgcExcept) {
    if (lockShare() < 0)
        ATHROWnR(DgcError(SPOS, "checkParams: lockShare failed"), -1);
    if (*KeyParam == 0) {
...
dgt_sint32 PccCryptZone::setZoneParams(DgcBgrammer* bg,
                                       dgt_schar* err_string) throw(DgcExcept) {
    if (lockExclusive(ELF_TYPE_SET_ZONE_PARAM) < 0)
        ATHROWnR(DgcError(SPOS, "setZoneParams: lockExclusive failed"), -1);
    dgt_sint32 rtn = 0;
    for (;;) {
        dgt_schar* val;
        if ((val = bg->getValue("zone.name")) == 0 || *val == 0) {
            sprintf(err_string, "name not found");
            rtn = -77002;
            break;
        }
        strncpy(Name, val, 32);
        ...
        if ((val = bg->getValue("zone.crypt_mode.detect"))) {
            ...
            sprintf(err_string, "mode not found");
            rtn = -77003;
            break;
        } else {
        ...
        if ((val = bg->getValue("zone.close_after")) && *val)
            CloseAfter = strtol(val, 0, 10);
        if ((val = bg->getValue("zone.header_flag")) && *val)
            HeaderFlag = (dgt_uint8)strtol(val, 0, 10);
        if ((val = bg->getValue("zone.backup_flag")) && *val)
            BackupFlag = (dgt_uint8)strtol(val, 0, 10);
        if ((val = bg->getValue("zone.overwrite_flag")) && *val)
            OverWriteFlag = (dgt_uint8)strtol(val, 0, 10);
        if ((val = bg->getValue("zone.out_extension")) && *val)
            strncpy(OutExtension, val, 32);
        if ((val = bg->getValue("zone.ext_ip")) && *val)
            strncpy(ExtIp, val, 65);
        if ((val = bg->getValue("zone.ext_port")) && *val)
            ExtPort = (dgt_uint16)strtol(val, 0, 10);
#ifdef linux
        if ((val = bg->getValue("zone.s_limit")) && *val)
            Slimit = (dgt_float64)strtof(val, 0);
        if ((val = bg->getValue("zone.s_enable")) && *val)
            Senable = (dgt_sint32)strtol(val, 0, 10);
        if ((val = bg->getValue("zone.g_sigma")) && *val)
            Gsigma = (dgt_float64)strtof(val, 0);
#else
        if ((val = bg->getValue("zone.s_limit")) && *val)
            Slimit = (dgt_float64)atof(val);
        if ((val = bg->getValue("zone.s_enable")) && *val)
            Senable = (dgt_sint32)strtol(val, 0, 10);
        if ((val = bg->getValue("zone.g_sigma")) && *val)
            Gsigma = (dgt_float64)atof(val);
#endif /* linux */
        if ((val = bg->getValue("zone.g_enable")) && *val)
            Genable = (dgt_sint32)strtol(val, 0, 10);
        if ((val = bg->getValue("zone.c_enable")) && *val)
            Cenable = (dgt_sint32)strtol(val, 0, 10);
        if ((val = bg->getValue("zone.r_angle")) && *val)
            Rangle = (dgt_sint32)strtol(val, 0, 10);
        if ((val = bg->getValue("zone.r_enable")) && *val)
            Renable = (dgt_sint32)strtol(val, 0, 10);
        ...
        if (FileFormat == 1) {         // all
            ...
            sprintf(err_string, "unknown file format[%u]", FileFormat);
            rtn = -77008;
        }
        break;
    }
...
dgt_sint32 PccCryptZone::setKeyParams(DgcBgrammer* bg,
                                      dgt_schar* err_string) throw(DgcExcept) {
    if (lockExclusive(ELF_TYPE_SET_KEY_PARAM) < 0)
        ATHROWnR(DgcError(SPOS, "setKeyParams: lockExclusive failed"), -1);
    if (FileFormat == 1) {  // whole crypt
        ...
        dgt_sint32 sid = -1;
        if ((sid = PcaApiSessionPool::getApiSession("", "", "", "", "", "",
                                                    0)) < 0)
            THROWnR(
                DgcBgmrExcept(
                    DGC_EC_BG_INCOMPLETE,
                    new DgcError(
                        SPOS, "getApiSession failed[%d] in cryptStatus", sid)),
                -1);
        ...
        if ((session = PcaApiSessionPool::getApiSession(sid)) == 0)
            THROWnR(
                DgcBgmrExcept(
                    DGC_EC_BG_INCOMPLETE,
                    new DgcError(
                        SPOS, "getApiSession[%d] failed in cryptStatus", sid)),
                -1);
...
dgt_sint32 PccCryptZone::setRegularParams(
    DgcBgrammer* bg, dgt_schar* err_string) throw(DgcExcept) {
    if (lockExclusive(ELF_TYPE_SET_REG_PARAM) < 0)
        ATHROWnR(DgcError(SPOS, "setRegularParams: lockExclusive failed"), -1);
...
dgt_sint32 PccCryptZone::setDelimiterParams(
    DgcBgrammer* bg, dgt_schar* err_string) throw(DgcExcept) {
    if (lockExclusive(ELF_TYPE_SET_DELI_PARAM) < 0)
        ATHROWnR(DgcError(SPOS, "setDelimiterParams: lockExclusive failed"),
                 -1);
...
dgt_sint32 PccCryptZone::setFixedParams(
    DgcBgrammer* bg, dgt_schar* err_string) throw(DgcExcept) {
    if (lockExclusive(ELF_TYPE_SET_FIXED_PARAM) < 0)
        ATHROWnR(DgcError(SPOS, "setFixedParams: lockExclusive failed"), -1);
    strncpy(FixedParam, bg->getText(), 256);
    unlockExclusive();
    return 0;
}

dgt_sint32 PccCryptZone::setSystemInfoParams(
    DgcBgrammer* bg, dgt_schar* err_string) throw(DgcExcept) {
    if (lockExclusive(ELF_TYPE_SET_SYSINFO_PARAM) < 0)
        ATHROWnR(DgcError(SPOS, "setSystemInfoParams: lockExclusive failed"),
                 -1);
    dgt_sint32 rtn = 0;
    for (;;) {
        dgt_schar* val;
...
dgt_sint32 PccCryptZone::buildParam(
    dgt_schar* buf, dgt_uint32* buf_len,
    dgt_sint32 migration_flag) throw(DgcExcept) {
    dgt_sint32 rtn = 0;
    memset(ErrString, 0, 1025);
    if ((rtn = checkParams(ErrString))) {
        THROWnR(DgcBgmrExcept(
                    DGC_EC_BG_INCOMPLETE,
                    new DgcError(SPOS, "checkParams failed:%s", ErrString)),
                -1);
    }
    ...
    dgt_schar tmp[129];
    if (lockShare() < 0)
        ATHROWnR(DgcError(SPOS, "buildParam: lockShare failed"), -1);
    for (;;) {
        memset(tmp, 0, 129);
        ...
        if (EncryptFlag == 1) {
            if (migration_flag == 1)
                sprintf(crypt_mode, "%s", "migration");
            else
        ...
        if (DetectFlag) {
            sprintf(tmp, "(mode=(detect=%d)(overwrite_flag=%s))", EncryptFlag,
                    OverWriteFlag ? "on" : "off");
        } else if (ExtPort == 0) {  // added by mwpark 2018.11.14 for fp masking
            sprintf(tmp, "(mode=(crypt=%s)(header_flag=%s)(overwrite_flag=%s))",
                    crypt_mode, header_version, OverWriteFlag ? "on" : "off");
        } else {
            sprintf(tmp,
                    "(mode=(crypt=%s)(header_flag=%s)(overwrite_flag=%s)(ext_"
                    "ip=%s)(ext_port=%u))",
                    crypt_mode, header_version, OverWriteFlag ? "on" : "off",
                    ExtIp, ExtPort);
        }
        ...
        *buf_len -= tmp_len;
        if (FileFormat == 2) {  // regular
            ...
            *buf_len -= tmp_len;
            ...
            strcat(buf, FixedParam);
            *buf_len -= tmp_len;
        }
        if ((tmp_len = strlen(SystemInfoParam)) > *buf_len) {  // add
                                                               // system_info
            rtn = -1;
            break;
        }
        strcat(buf, SystemInfoParam);
        *buf_len -= tmp_len;
        break;
    }
    unlockShare();
    if (rtn < 0) {
        THROWnR(DgcBgmrExcept(
                    DGC_EC_BG_INCOMPLETE,
                    new DgcError(SPOS, "not enough buffer length for [%d]",
                                 tmp_len)),
                -1);
    }
    return 0;
