...
dgt_sint32 PccDropJobStmt::execute(DgcMemRows* mrows,
    ...
    if (job_id == 1) {
        THROWnR(DgcDbNetExcept(DGC_EC_DN_INVALID_ST,
                               new DgcError(SPOS, "job[1] can not be dropped")),
                -1);
    }
    ...
    //		THROWnR(DgcDbNetExcept(DGC_EC_DN_INVALID_ST,new
    //DgcError(SPOS,"job[%lld] does not finish yet",job_id)),-1);
    //	}
