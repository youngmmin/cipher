...
dgt_sint32 PccGetCryptStatStmt::execute(
    ...
    if (job_id == 0) {  // all jobs
        ...
        if ((curr_job = JobPool.getJob(job_id))) {
            for (dgt_sint32 j = 0;
                 (curr_dir = curr_job->repository().dirPool().getCryptDir(j));
                 j++) {
                CryptStat->add();
                CryptStat->next();
                curr_stat = (pcct_crypt_stat*)CryptStat->data();
                ...
                if (curr_job->collector() &&
                    ...
                    curr_stat->job_status = curr_job->collector()->jobStatus();
            }
            curr_job->unlockShare();
        } else {
