...
dgt_sint32 PccGetDirCryptStatStmt::execute(DgcMemRows* mrows, dgt_sint8 delete_flag) throw(DgcExcept)
{
	if (!mrows || !mrows->next()) {
	...
	if ((curr_job=JobPool.getJob(param->job_id)) && 
		(curr_dir = curr_job->repository().dirPool().getCryptDirWithDid(param->dir_id))) {
				// found dir
				...
				if (curr_job->collector() && curr_job->collector()->jobStatus() != PCC_STATUS_TYPE_RUN) CryptStat.job_status = curr_job->collector()->jobStatus();
		...
		DgcWorker::PLOG.tprintf(0,"not found crypt dir  job_id [%lld] dir_id [%lld]\n",param->job_id,param->dir_id);
