...
dgt_sint32 PccGetTargetListStmt::execute(DgcMemRows* mrows, dgt_sint8 delete_flag) throw(DgcExcept)
{
	...
	{
		case 1 : FileQueue = &(job->repository().fileQueue()); 
				 break;
		case 2 : FileQueue = &(job->repository().failFileQueue()); 
				 break;
		case 3 : FileQueue = &(job->repository().nullityFileQueue()); 
				 break;
		default : 
		THROWnR(DgcDbNetExcept(DGC_EC_DN_INVALID_ST,new DgcError(SPOS,"invalied target_type [%d].",param_in->target_type)),-1);
...
dgt_uint8* PccGetTargetListStmt::fetch() throw(DgcExcept)
{
	...
	if (TargetList->next()) return (dgt_uint8*)TargetList->data();
	else return 0;
	return 0;
}
