...
class PccPcfsStat : public DgcObject {
   ...
   public:
    ...
    PccPcfsStat(dgt_uint32 pcfs_id,
                const dgt_schar* stat_file_path = DFLT_PCFS_STAT_PATH)
        : PcfsID(pcfs_id) {
    ...
    dgt_sint32 putStat(pcfst_fs_stat* fs_stat = 0) throw(DgcExcept) {
        ...
        if (stat_file.sendData((dgt_uint8*)fs_stat, sizeof(pcfst_fs_stat)) <
            0) {
    ...
    dgt_void getStat(pcfst_fs_stat* fs_stat = 0) {
        ...
        if (EXCEPT == 0) {
            if (stat_file.seek(PcfsID * sizeof(pcfst_fs_stat), SEEK_SET) >= 0) {
                ...
                if (fs_stat->pid == 0 ||
                    (kill((pid_t)fs_stat->pid, 0) && errno == ESRCH)) {
                    // no pcfs daemon
                    memset(fs_stat, 0, sizeof(pcfst_fs_stat));
                }
            }
