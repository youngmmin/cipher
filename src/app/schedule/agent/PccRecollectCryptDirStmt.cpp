...
dgt_sint32 PccRecollectCryptDirStmt::execute(
    ...
    pcct_recollect_crypt_dir_in* param_in =
        (pcct_recollect_crypt_dir_in*)mrows->data();
    ...
    PccCryptDir* crypt_dir =
        job->repository().dirPool().getCryptDirWithDid(param_in->dir_id);
    if (crypt_dir == 0) {
        job->unlockShare();
        THROWnR(DgcDbNetExcept(DGC_EC_DN_INVALID_ST,
                               new DgcError(SPOS, "not found crypt_dir [%lld].",
                                            param_in->dir_id)),
                -1);
    }
    org_dir_status = crypt_dir->status();
    if (org_dir_status == PCC_STATUS_TYPE_RUN)
        crypt_dir->setStatus(PCC_STATUS_TYPE_PAUSE);
    ...
    for (dgt_sint32 i = 1; i < 4; i++) {
        switch (i) {
            case 1:
                FileQueue = &(job->repository().fileQueue());  // target queue
                break;
            case 2:
                FileQueue = &(job->repository().failFileQueue());  // fail queue
                break;
            case 3:
                FileQueue =
                    &(job->repository().nullityFileQueue());  // nullity queue
                break;
            default:
                job->unlockShare();
                ...
                THROWnR(
                    DgcDbNetExcept(
                        ...
                        new DgcError(SPOS, "invalied target_type [%d].", i)),
                    -1);
        }
        while (FileQueue->get(&TargetFile)) {
            if (TargetFile.dirID() != param_in->dir_id)
                FileQueue->put(TargetFile.dirID(), TargetFile.zoneID(),
                               ...
                               TargetFile.errCode(), TargetFile.errMsg());
        }  // while end
    }      // for end
    ...
    //	strncpy(RecollectCryptDirOut.error_message, "successful!",
    //strlen("successful!"));
    IsExecuted = 1;
    FetchFlag = 0;
    return 0;
