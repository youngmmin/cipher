...
dgt_sint32 PccRemoveFileStmt::execute(DgcMemRows* mrows, dgt_sint8 delete_flag) throw(DgcExcept)
{
	if (!mrows || !mrows->next()) {
	...
	if (JobPool.traceLevel() > 0) DgcWorker::PLOG.tprintf(0,"rm_file_path : %s\n",rm_file_path);

	// remove file
	if (unlink(rm_file_path) < 0) { // remove the file
		if (JobPool.traceLevel() > 0) DgcWorker::PLOG.tprintf(0,"unlink[%s] failed : %s\n",rm_file_path,strerror(errno));
