...
dgt_sint32 PccSetParamsStmt::execute(DgcMemRows* mrows,
    ...
    do {
        ...
        if (params->max_target_files > 0)
            file_queue_size = params->max_target_files;
        if (params->collecting_interval > 0)
            collect_interval = params->collecting_interval;
    ...
    if (JobPool.traceLevel() > 10) {
        DgcWorker::PLOG.tprintf(
            0,
            "job_id[%lld] last_update[%lld] agent_param_size[%u] "
            "file_queue_size[%d] collect_interval[%d] job_type[%d] "
            "job_status[%u]\n",
            job_id, last_update, dg_strlen(ParamText), file_queue_size,
            collect_interval, job_type, job_status);
    ...
    if (job_status == PCC_STATUS_TYPE_DELETED ||
        ...
        if ((job = JobPool.getJob(job_id)) == 0) {
            ...
            new_job_flag = 1;
            if (JobPool.traceLevel() > 10)
                DgcWorker::PLOG.tprintf(0, "create new job:%lld\n", job_id);
        }
        ...
        while ((bg = params.getNext())) {
            ...
            if (EXCEPT) {
                DgcExcept* e = EXCEPTnC;
                DgcWorker::PLOG.tprintf(
                    0, *e, "job[%lld] occur exception:", job->jobID());
                job->unlockShare();
                if (new_job_flag) {
                    if (JobPool.dropJob(job->jobID()) < 0) {
                        ...
                        DgcWorker::PLOG.tprintf(
                            0, *drop_job_e,
                            "dropJob[%lld] failed while executing setParams:",
                            job->jobID());
                        delete drop_job_e;
                    }
                }
    ...
    if (new_job_flag) {
        if (job->start(job_type, job_status, file_queue_size,
                       collect_interval) < 0) {
            DgcExcept* e = EXCEPTnC;
            job->unlockShare();
        ...
        if (job->collector()) {
            if (job_status == PCC_STATUS_TYPE_DELETED) {
                ...
                job = 0;
                if (JobPool.traceLevel() > 10)
                    DgcWorker::PLOG.tprintf(0, "drop job:%lld\n", job_id);
            } else {
