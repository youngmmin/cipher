...
PccStreamFile* PccStreamFileStatistic::updateFileNode(
    dgt_sint64 file_id, dgt_sint64 dir_id, dgt_sint64 zone_id,
    dgt_sint64 src_file_size, dgt_sint64 dst_file_size, dgt_time lm_time,
    const dgt_schar* src_file_name, const dgt_schar* dst_file_name,
    ...
    for (;;) {
        ...
        if (DgcSpinLock::lock(&NodeListLock) == 0) {
            ...
            if (hnp == 0) {  // not found and create one
                ...
                file_node->reset(file_id, dir_id, zone_id, src_file_size,
                                 dst_file_size, lm_time, src_file_name,
                                 dst_file_name, error_code, error_msg);
...
dgt_sint32 PccStreamFileStatistic::statisticCopy(DgcMemRows* sf,
    ...
    if (DgcSpinLock::lock(&NodeListLock) == 0) {
        ...
        while (node) {
            ...
            if ((file_type == 1 && tmp->errCode() == 0) ||
                (file_type == 2 && tmp->errCode()) || (file_type == 3)) {
                sf->add();
                sf->next();
                ...
                strncpy(stream_node->in_file_name, tmp->srcFileName(),
                        tmp->INIT_FILE_NAME_LEN);
                strncpy(stream_node->out_file_name, tmp->dstFileName(),
                        tmp->INIT_FILE_NAME_LEN);  // gogo
                if (tmp->errCode()) {
                    stream_node->error_code = tmp->errCode();
                    strncpy(stream_node->error_msg, tmp->errMsg(),
                            PccStreamFile::MAX_ERROR_MSG);  // gogo
                }
