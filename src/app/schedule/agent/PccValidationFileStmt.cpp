...
dgt_sint32 PccValidationFileStmt::execute(DgcMemRows* mrows, dgt_sint8 delete_flag) throw(DgcExcept)
{
	if (!mrows || !mrows->next()) {
	...
	if (JobPool.traceLevel() > 10) DgcWorker::PLOG.tprintf(0,"validation_file_path : %s\n",validation_file_path);
	...
	if ((header_flag = hm.checkHeader(validation_file_path)) < 0) { //error
		...
		session_id = PcaApiSessionPool::getApiSession(sess_in.client_ip,sess_in.user_id,sess_in.client_program,sess_in.client_mac,sess_in.db_user,sess_in.os_user,sess_in.protocol);
		...
		sprintf(parameters,"(mode=(crypt=validation)(overwrite_flag=on)(user_logging=on))(logging_info=(ptu_id=%lld)(client_ip=%s))",param_in->ptu_id, param_in->client_ip);
		strcat(parameters,out_param);
		if (JobPool.traceLevel() > 10) DgcWorker::PLOG.tprintf(0,"crypt parameters :%s \n",parameters);
		PccFileCryptor* cryptor = new PccFileCryptor(0,0,JobPool.traceLevel());
		if ((rtn=cryptor->crypt(session_id, parameters,validation_file_path,out_file_path)) < 0) {
			DgcWorker::PLOG.tprintf(0,"crypt failed: %s - %d\n",cryptor->errString(),cryptor->errCode());
			CryptFileOut->rtn_code = -1;
			sprintf(CryptFileOut->error_message,"decrypt failed [%d]: [%s]",cryptor->errCode(),cryptor->errString());
		}
		delete cryptor;
		struct stat     fstat;
		if (stat(out_file_path,&fstat) == 0) {
			if (hm.inFileSize() == fstat.st_size) {
				...
				CryptFileOut->rtn_code = -1;
				strcpy(CryptFileOut->error_message,"incompleteness encrypted file");
			}
			if (unlink(out_file_path) < 0) { // remove the file
				if (JobPool.traceLevel() > 10) DgcWorker::PLOG.tprintf(0,"unlink[%s] failed : %s\n",out_file_path,strerror(errno));
				CryptFileOut->rtn_code = -1;
				dgt_sint32 len = dg_strlen(strerror(errno));
				strncpy(CryptFileOut->error_message,strerror(errno),len>1024?1024:len);
			}
		}
