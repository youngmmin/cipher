...
const dgt_schar	PccCipherAgentManager::SOHA_CONN_STRING[] =
		"(ADDRESS=(PROTOCOL=TCP)(HOST=%s)(PORT=%d)(SID=%s)(CONN_TIMEOUT=%d)(IN_TIMEOUT=%d)(OUT_TIMEOUT=%d))";
...
{
	for (dgt_uint8 i=0; i<MAX_AGENTS; i++) memset(&AgentList[i],0,sizeof(pcamt_agent_list));
	NumAgents = 0;
...
dgt_sint32 PccCipherAgentManager::connectMaster() throw(DgcExcept)
{
	...
	dg_sprintf(conn_string,
			SOHA_CONN_STRING,
			PrimarySohaConnIP,
			PrimarySohaConnPort,
			PrimarySohaSvc,
			SOHA_CONN_TIMEOUT,SOHA_CONN_TIMEOUT,SOHA_CONN_TIMEOUT);
	if (SohaClient.connectDB(conn_string,"dgadmin","petra@one1","attach","pcp_crypt_manager") < 0) {
		// connect secondary
		if (*SecondarySohaConnIP && SecondarySohaConnPort) {
			DgcExcept* e = EXCEPTnC;
			DgcWorker::PLOG.tprintf(0,*e,"connectDB to primary[%s:%u:%s]failed\n",PrimarySohaConnIP,PrimarySohaConnPort,PrimarySohaSvc);
			delete e;
			...
			dg_sprintf(conn_string,
					SOHA_CONN_STRING,
					SecondarySohaConnIP,
					SecondarySohaConnPort,
					SecondarySohaSvc,
					SOHA_CONN_TIMEOUT,SOHA_CONN_TIMEOUT,SOHA_CONN_TIMEOUT);
			if (SohaClient.connectDB(conn_string,"dgadmin","petra@one1","attach","pcp_crypt_manager") < 0) {
				if (conn_string) delete conn_string;
				ATHROWnR(DgcError(SPOS,"connectDB to secondary[%s:%u:%s] failed",SecondarySohaConnIP,SecondarySohaConnPort,SecondarySohaSvc),-1);
			}
		} else {
			if (conn_string) delete conn_string;
			ATHROWnR(DgcError(SPOS,"connectDB[%s:%u:%s]failed\n",PrimarySohaConnIP,PrimarySohaConnPort,PrimarySohaSvc),-1);
		}
	}
	if (conn_string) delete conn_string;
...
dgt_sint32 PccCipherAgentManager::getAgentList() throw(DgcExcept)
{
	...
	if (rtn_rows == 0) {
		THROWnR(DgcDbNetExcept(DGC_EC_DN_INVALID_ST, new DgcError(SPOS,"no agent_id pfct_agent.enc_tgt_sys_id[%lld]",EncTgtSysID)),-1);
	}
...
dgt_sint64 PccCipherAgentManager::getCryptAgentPID(dgt_sint64 agent_id) throw(DgcExcept)
{
	...
	if (client_stream.connectServer(agent_uds_addr,10) < 0) {
		ATHROWnR(DgcError(SPOS,"connectServer to agent_service[%lld]:[%s] failed:\n",agent_id,agent_uds_addr),-1);
	}
	...
	if (msg_stream.sendMessage(&result_ext) != 0) {
		ATHROWnR(DgcError(SPOS,"sendMessage to agent_service[%lld]:[%s] failed:\n",agent_id,agent_uds_addr),-1);
	}

	// recv message
	DgcMessage*	msg = 0;
	if (msg_stream.recvMessage(10) <= 0) {
		ATHROWnR(DgcError(SPOS,"recvMessage from agent_service[%lld]:[%s] failed:\n",agent_id,agent_uds_addr),-1);
	}
	if((msg=msg_stream.currMsg())->opi() != PTDGILETTER) {
		THROWnR(DgcDbNetExcept(DGC_EC_DN_INVALID_ST, new DgcError(SPOS,"not PTDGILETTER message[%d]\n",msg->opi())),-1);
	}
...
dgt_void PccCipherAgentManager::in() throw(DgcExcept)
{
	...
	if (NumAgents > 0) {
		...
		DgcWorker::PLOG.tprintf(0,"agent_manager[%lld] has no pcp_crypt_agent to monitor\n",EncTgtSysID);
		StopFlag = 1;
	}
}
...
dgt_sint32 PccCipherAgentManager::run() throw(DgcExcept)
{
	...
	switch(RunStep) {
		...
		{
			DgcWorker::PLOG.tprintf(0,"agent_manager[%lld]: waiting for agents to be ready \n",EncTgtSysID,NumAgents);
			...
			while (list_idx < NumAgents) {
				if ((agent_pid=getCryptAgentPID(AgentList[list_idx].agent_id)) < 0) {
					if (retry_cnt++ < max_retry_cnt) {
						...
						DgcExcept* e = EXCEPTnC;
						if (e) {
							DgcWorker::PLOG.tprintf(0,*e,"agent_manager[%lld]: getCryptAgentPID[%lld] failed\n",EncTgtSysID,AgentList[list_idx].agent_id);
							delete e;
						}
					}
				} else {
					if (agent_pid > 0) {
						...
						num_monitor_proc++;
						DgcWorker::PLOG.tprintf(0,"agent_manager[%lld]: start the agents[%lld] pid[%lld]\n",EncTgtSysID,AgentList[list_idx].agent_id,AgentList[list_idx].pid);
					}
				}
				list_idx++;
				retry_cnt = 0;
			}
			if (num_monitor_proc == 0) RunStep = STEP_END;
			else RunStep = STEP_MONITOR_AGENT;
			break;
		}
			...
			for (dgt_uint8 i=0; i<NumAgents; i++) {
				if (AgentList[i].pid > 0 && AgentList[i].monitoring_flag) {
					...
					if (kill((pid_t)AgentList[i].pid,0) != 0 && errno == ESRCH) { // find a dead process
						...
						dgt_sint32 addr_len = dg_strlen(UdsListenDir) + 100;
						dgt_schar agent_uds_addr[addr_len];
						sprintf(agent_uds_addr,CRYPT_AGENT_CONN_STRING,UdsListenDir,AgentList[i].agent_id);
						...
						if (agent_pid > 0) {
							...
							DgcWorker::PLOG.tprintf(0,"agent_manager[%lld] : restart agent[%lld:%lld]\n",EncTgtSysID,AgentList[i].agent_id,AgentList[i].pid);
						}
					}
				}
			}
...
dgt_void PccCipherAgentManager::out() throw(DgcExcept)
{
	...
	dgt_sint32 rtn=0;
	while (list_idx < NumAgents) {
		if (AgentList[list_idx].pid > 0 && AgentList[list_idx].monitoring_flag) {
			if ((rtn=kill((pid_t)AgentList[list_idx].pid,0)) == 0) {
				// the process is not terminated yet
				DgcWorker::PLOG.tprintf(0,"agent_manager[%lld] : waiting until agent[%lld:%lld] stopped.\n",EncTgtSysID,AgentList[list_idx].agent_id,AgentList[list_idx].pid);
				sleep(3);
				continue;
			}
		}
...
dgt_void PccCipherAgentManager::getCryptManagerStatus(pcct_manager_status* status)
{
	if (status) {
		...
		memcpy(status->primary_soha_ip,PrimarySohaConnIP,sizeof(PrimarySohaConnIP));
		status->primary_soha_port = PrimarySohaConnPort;
		memcpy(status->secondary_soha_svc,SecondarySohaSvc,sizeof(SecondarySohaSvc));
		memcpy(status->secondary_soha_ip,SecondarySohaConnIP,sizeof(SecondarySohaConnIP));
...
void help_message()
{
	printf("Usage: pcp_crypt_manager [OPTIONS] <start|stop|status>\n");
        printf("\n Examples:\n");
        printf("    pcp_crypt_manager start\t\t\t\t# start the manager using manager.conf\n");
        printf("    pcp_crypt_manager -c manager2.conf start\t\t# start the manager using manager2.conf \n");
        printf("    pcp_crypt_manager -c manager2.conf stop\t\t# stop the manager using manager2.conf \n");
        printf("    pcp_crypt_manager -c manager2.conf -v status\t# display the manager's detailed status using manager2.conf \n");

        printf("\n Options:\n");
        printf("    -h\t display this help and exit\n");
        printf("    -c\t set configure file path [default: manager.conf]\n");
        printf("    -v\t display detailed manager agent status (with status command)\n");
}
...
int main(dgt_sint32 argc,dgt_schar** argv)
{
	...
	if (argc < 2) {
//		printf("usage: pcp_crypt_manager <conf_file_path> <start|stop|status> [all(default) | crypt_agent | client_agent]\n");
		help_message();
		return -1;
	}
	...
	if (remain_param == 1) {	
		cmd = argv[optind];
	} else if (remain_param == 2 || remain_param == 3) {	// for compatibility; pcp_crypt_manager <conf file path> <start|stop|staus> [all(default] | crypt_agent | client_agent]
	...
	if (strcasecmp(cmd,"start")) {
		...
		DgcUnixClient client_stream;
		if (client_stream.connectServer(agent_manager->udsListenAddr(),10) < 0) {
		...
		if (!strcasecmp(cmd,"stop")) {
			for (;;) {
				...
				if (!result_e) {
					printf("stop agent_manager[%lld] failed : [unknown error]\n",agent_manager->encTgtSysID());
					break;
				}
				if (result_e->classid() == 0 && result_e->errCode() == 0) {
					...
					// do nothing
					//printf("waiting[%d] until agent_manager[%lld] stopped...\n",result_e->errCode(),agent_manager->encTgtSysID());
				}
			}
		} else if(!strcasecmp(cmd,"status")) {
			if (msg_stream.recvMessage(10) <= 0) {
			...
			dgt_sint32 num_agents = (dgt_sint32)dg_strtoll(letter->getPs(),0,10);
			if (num_agents > 0) {
	...
	if (server_stream.listenServer(agent_manager->udsListenAddr()) == 0) {
		...
		if (e) {
			e->print();
			DgcWorker::PLOG.tprintf(0,*e,"agent_manager[%lld] listenServer[%s] failed:\n",agent_manager->encTgtSysID(),agent_manager->udsListenAddr());
			delete e;
		} else {
			DgcWorker::PLOG.tprintf(0,"agent_manager[%lld] listenServer[%s] failed: [unknown error]\n",agent_manager->encTgtSysID(),agent_manager->udsListenAddr());
		}
		return 4;
	}
	...
	if (agent_manager->start() < 0) {
		...
		if (e) {
			e->print();
			DgcWorker::PLOG.tprintf(0,*e,"starting agent_manager[%lld] failed:\n",agent_manager->encTgtSysID());
			delete e;
		}
	...
	while(1) {
		...
		if ((stream=(DgcCommStream*)server_stream.acceptConnection(5))) {
			...
			if (msg_type == PCC_MANAGER_UDS_MSG_TYPE_STOP) {
				...
				while (agent_manager->isAlive()) {
					if (wait_time > 300) {
						if (agent_manager->stop() < 0) {
							...
							DgcExcept* e = EXCEPTnC;
							DgcWorker::PLOG.tprintf(0,*e,"agent_manager[%lld] : stop failed:\n",agent_manager->encTgtSysID());
							delete e;
							exit(-1);
						}
					}
				...
				dgt_schar body[513];
				dgt_schar ps[65];
				sprintf(body,"manager[%lld] pid[%lld] num_agents[%d] soha_conn_status[%d] primary_soha_conn[%s:%d/%s] secondary_soha_conn[%s:%d/%s]",
						manager_status.manager_id,
						manager_status.manager_pid,
				...
				if (manager_status.num_agents > 0) {
					...
					for(dgt_sint32 i=0; i<manager_status.num_agents; i++) {
						pcamt_agent_list* agent_list = agent_manager->agentList(i);
				...
				DgcWorker::PLOG.tprintf(0,"agent_manager[%lld] invalid msg[%u] :\n",agent_manager->encTgtSysID(),msg_type);
			}
		}
		if (!agent_manager->isAlive()) {
			DgcWorker::PLOG.tprintf(0,"pcp_crypt_manager stopped cause agent_manager[%lld] is not alive\n",agent_manager->encTgtSysID());
			break;
		}
		napAtick();
	}
