...
class PccExternalIV {
   private:
    ...
    inline dgt_sint32 initContext(dgt_uint8 iv_no) throw(DgcExcept) {
        ...
        if ((rtn = PCI_getEncryptKey(iv_no, PARENT_KEY_SIZE / 8, ParentKey)) <
            0) {
            THROWnR(
                DgcPciExcept(
                    DGC_EC_PCI_ERR,
                    new DgcError(SPOS, "PCI_getEncryptKey[%u] failed, %d:%s",
                                 iv_no, rtn, PCI_getKmgrErrMsg())),
                -1);
        }
   ...
   public:
    ...
    dgt_sint32 createIV(const dgt_schar* str_iv, dgt_uint16 format_no,
                        dgt_sint64 iv_id, dgt_uint8* iv_no, dgt_schar* seiv,
                        dgt_uint32* seiv_len, dgt_schar* seivs,
                        dgt_uint32* seivs_len) throw(DgcExcept);

    dgt_sint32 checkIV(dgt_uint8 iv_no, const dgt_schar* seiv,
                       const dgt_schar* seivs) throw(DgcExcept);

    dgt_sint32 getIV(dgt_uint8 iv_no, const dgt_schar* seiv,
