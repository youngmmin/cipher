...
dgt_sint32 PccExternalKey::createKey(const dgt_schar* str_key,
                                     dgt_uint16 format_no, dgt_sint64 key_id,
                                     dgt_uint16* key_no, dgt_schar* sek,
...
DgcWorker::PLOG.tprintf(0,"createKey => str_key[%s],format_no[%u],key_id[%lld]\n",str_key,format_no,key_id);
    ...
    if (format_no == 128) {
        ...
        if ((ExtKeyLen = DgcBase64::decode2(str_key, str_key_len, ExtKey + 2,
                                            MAX_EXT_KEY_LEN - 2)) < 0) {
        ...
        if ((ExtKeyLen = DgcBase16::decode(str_key, str_key_len, ExtKey + 2,
                                           MAX_EXT_KEY_LEN - 2)) < 0) {
            THROWnR(DgcPciExcept(DGC_EC_PCI_ERR,
...
DgcWorker::PLOG.tprintf(0,"createKey => ExtKey[%s] key_no[%u]\n",ext_in_hex,*key_no);
    ...
    dgt_sint32 rtn;
    if ((rtn = PCI_encrypt(&EncContext, ExtKey, ExtKeyLen, (dgt_uint8*)sek,
                           sek_len)) < 0) {
        THROWnR(DgcPciExcept(DGC_EC_PCI_ERR,
    ...
    if ((rtn = PCI_encrypt(&HashContext, ExtKey, ExtKeyLen, (dgt_uint8*)seks,
                           seks_len)) < 0) {
        THROWnR(DgcPciExcept(DGC_EC_PCI_ERR,
                             new DgcError(SPOS, "PCI_encrypt failed, %d:%s",
                                          rtn, PCI_getKmgrErrMsg())),
...
dgt_sint32 PccExternalKey::checkKey(dgt_uint16 key_no, const dgt_schar* sek,
    ...
    dgt_sint32 rtn;
    if ((rtn = PCI_decrypt(&EncContext, (dgt_uint8*)sek, strlen(sek), ExtKey,
                           &ExtKeyLen)) < 0) {
        THROWnR(DgcPciExcept(DGC_EC_PCI_ERR,
                             new DgcError(SPOS, "PCI_encrypt failed, %d:%s",
                                          rtn, PCI_getKmgrErrMsg())),
...
DgcWorker::PLOG.tprintf(0,"checkKey => ExtKey[%s] \n",ext_in_hex);
    ...
    if (key_no != enc_key_no) {
        THROWnR(
            DgcPciExcept(
                DGC_EC_PCI_ERR,
                new DgcError(SPOS, "key_no[%u] mismatch with saved key_no[%u]",
                             key_no, enc_key_no)),
            -1);
    }
    //
    ...
    if ((rtn = PCI_encrypt(&HashContext, ExtKey, ExtKeyLen,
                           (dgt_uint8*)tmp_seks, &tmp_seks_len)) < 0) {
        THROWnR(DgcPciExcept(DGC_EC_PCI_ERR,
                             new DgcError(SPOS, "PCI_encrypt failed, %d:%s",
                                          rtn, PCI_getKmgrErrMsg())),
                -1);
...
dgt_sint32 PccExternalKey::getKey(dgt_uint16 key_no, const dgt_schar* sek,
