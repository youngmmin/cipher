...
class PccExternalKey {
  private:
	...
	inline dgt_sint32 initContext(dgt_uint16 key_no) throw(DgcExcept)
	{
		...
		key_no = key_no % 1000;
		if ((rtn=PCI_getEncryptKey(key_no,PARENT_KEY_SIZE/8,ParentKey)) < 0) {
			THROWnR(DgcPciExcept(DGC_EC_PCI_ERR,
				new DgcError(SPOS,"PCI_getEncryptKey[%u] failed, %d:%s",key_no,rtn,PCI_getKmgrErrMsg())),-1);
		}
  ...
  public:
	...
	dgt_sint32 createKey(const dgt_schar* str_key,dgt_uint16 format_no,dgt_sint64 key_id,
	 	dgt_uint16* key_no,dgt_schar* sek,dgt_uint32* sek_len,dgt_schar* seks,dgt_uint32* seks_len) throw(DgcExcept);

	dgt_sint32 checkKey(dgt_uint16 key_no,const dgt_schar* sek,const dgt_schar* seks) throw(DgcExcept);

	dgt_sint32 getKey(dgt_uint16 key_no,const dgt_schar* sek,const dgt_schar* seks,
			  dgt_uint32 key_len,dgt_uint8* key_buffer) throw(DgcExcept);
};
