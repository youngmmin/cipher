...
#ifdef PCI_BIG_ENDIAN
#undef PCI_PCI_LITTLE_ENDIAN
#else
#ifndef PCI_LITTLE_ENDIAN
#error In order to compile this, you have to	\
  define either PCI_LITTLE_ENDIAN or PCI_BIG_ENDIAN.	\
...
#define XX(NNN, x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, xa, xb, xc, xd, xe, \
           xf)                                                              \
...
#define ROTR_32(x, y) (((x) >> (y)) | ((x) << (32 - (y))))
#define ROTL_32(x, y) (((x) << (y)) | ((x) >> (32 - (y))))

#if defined(_MSC_VER)
#define ReverseWord(W) \
    { (W) = (0xff00ff00 & ROTR_32((W), 8)) ^ (0x00ff00ff & ROTL_32((W), 8)); }
#else
#define ReverseWord(W)                                        \
    {                                                         \
        (W) = (W) << 24 ^ (W) >> 24 ^ ((W)&0x0000ff00) << 8 ^ \
              ((W)&0x00ff0000) >> 8;                          \
    }
#endif

#ifdef PCI_LITTLE_ENDIAN
...
#define WordLoad(ORIG, DEST)                         \
    {                                                \
        (DEST) = (0xff00ff00 & ROTR_32((ORIG), 8)) ^ \
                 (0x00ff00ff & ROTL_32((ORIG), 8));  \
    }
#endif
...
#define SBL1_M(T0, T1, T2, T3)                                    \
    {                                                             \
        T0 = S1[BRF(T0, 24)] ^ S2[BRF(T0, 16)] ^ X1[BRF(T0, 8)] ^ \
             X2[BRF(T0, 0)];                                      \
        T1 = S1[BRF(T1, 24)] ^ S2[BRF(T1, 16)] ^ X1[BRF(T1, 8)] ^ \
             X2[BRF(T1, 0)];                                      \
        T2 = S1[BRF(T2, 24)] ^ S2[BRF(T2, 16)] ^ X1[BRF(T2, 8)] ^ \
             X2[BRF(T2, 0)];                                      \
        T3 = S1[BRF(T3, 24)] ^ S2[BRF(T3, 16)] ^ X1[BRF(T3, 8)] ^ \
             X2[BRF(T3, 0)];                                      \
    }
#define SBL2_M(T0, T1, T2, T3)                                    \
    {                                                             \
        T0 = X1[BRF(T0, 24)] ^ X2[BRF(T0, 16)] ^ S1[BRF(T0, 8)] ^ \
             S2[BRF(T0, 0)];                                      \
        T1 = X1[BRF(T1, 24)] ^ X2[BRF(T1, 16)] ^ S1[BRF(T1, 8)] ^ \
             S2[BRF(T1, 0)];                                      \
        T2 = X1[BRF(T2, 24)] ^ X2[BRF(T2, 16)] ^ S1[BRF(T2, 8)] ^ \
             S2[BRF(T2, 0)];                                      \
        T3 = X1[BRF(T3, 24)] ^ X2[BRF(T3, 16)] ^ S1[BRF(T3, 8)] ^ \
             S2[BRF(T3, 0)];                                      \
    }
...
#define GSRK(X, Y, n)                                      \
    {                                                      \
        ...
        WO(rk, 0) = ((X)[0]) ^ (((Y)[(q) % 4]) >> r) ^     \
                    (((Y)[(q + 3) % 4]) << (32 - r));      \
        WO(rk, 1) = ((X)[1]) ^ (((Y)[(q + 1) % 4]) >> r) ^ \
                    (((Y)[(q) % 4]) << (32 - r));          \
        WO(rk, 2) = ((X)[2]) ^ (((Y)[(q + 2) % 4]) >> r) ^ \
                    (((Y)[(q + 1) % 4]) << (32 - r));      \
        WO(rk, 3) = ((X)[3]) ^ (((Y)[(q + 3) % 4]) >> r) ^ \
                    (((Y)[(q + 2) % 4]) << (32 - r));      \
        rk += 16;                                          \
    }
...
static void Crypt(const Byte* i, pct_aria_key* aria_key, Byte* o) {
    register Word t0, t1, t2, t3;
    Byte* rk = aria_key->k32.round_key;
    ...
    if (aria_key->k32.key_len > 12) {
        KXL FO KXL FE
    }
    if (aria_key->k32.key_len > 14) {
        KXL FO KXL FE
    }
...
static int EncKeySetup(const unsigned char* mk, pct_aria_key* aria_key,
                       int keyBits) {
    register Word t0, t1, t2, t3;
    Byte* rk = aria_key->k32.round_key;
    ...
    return aria_key->k32.key_len = (keyBits + 256) / 32;
}

static int DecKeySetup(const unsigned char* mk, pct_aria_key* aria_key,
                       int keyBits) {
    Byte* rk = aria_key->k32.round_key;
    Word *a, *z;
    int rValue;
    ...
    rValue = EncKeySetup(mk, aria_key, keyBits);
    a = (Word*)(rk);
    z = a + rValue * 4;
    t0 = a[0];
    t1 = a[1];
...
dgt_sint32 PciAriaCipher::encryptBlock(dgt_uint8* iblock, dgt_uint8* oblock) {
    if (EncKeyStatus != KS_ENC) {
        EncKeySetup(Key, &EncKey, KeyBits);
        EncKeyStatus = KS_ENC;
    }
    Crypt(iblock, &EncKey, oblock);
    return 0;
}

dgt_sint32 PciAriaCipher::decryptBlock(dgt_uint8* iblock, dgt_uint8* oblock) {
    if (DecKeyStatus != KS_DEC) {
        DecKeySetup(Key, &DecKey, KeyBits);
        DecKeyStatus = KS_DEC;
    }
    Crypt(iblock, &DecKey, oblock);
    return 0;
}

dgt_sint32 PciAriaCipher::initializeContext() {
    EncKeyStatus = KS_NO;
    DecKeyStatus = KS_NO;
    return 0;
}

dgt_sint32 PciAriaCipher::PCI_ARIA_makeEncKey() {
    return EncKeySetup(Key, &EncKey, KeyBits);
}

dgt_sint32 PciAriaCipher::PCI_ARIA_makeDecKey() {
    return DecKeySetup(Key, &DecKey, KeyBits);
}

PciAriaCipher::PciAriaCipher() : EncKeyStatus(KS_NO), DecKeyStatus(KS_NO) {
    BlockBytes = 16;
}

PciAriaCipher::~PciAriaCipher() {}
