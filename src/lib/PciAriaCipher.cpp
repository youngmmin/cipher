...
#undef WordLoad
#define WordLoad(ORIG, DEST) {						\
    (DEST) = (0xff00ff00 & ROTR_32((ORIG), 8)) ^ (0x00ff00ff & ROTL_32((ORIG), 8)); \
  }
#endif
...
static void Crypt(const Byte *i, pct_aria_key *aria_key, Byte *o)
{
  ...
  if (aria_key->k32.key_len > 12) {KXL FO KXL FE}
  if (aria_key->k32.key_len > 14) {KXL FO KXL FE}
...
static int EncKeySetup(const unsigned char* mk, pct_aria_key* aria_key, int keyBits)
{
  register Word t0, t1, t2, t3;
  Byte*	rk = aria_key->k32.round_key;
...
static int DecKeySetup(const unsigned char* mk, pct_aria_key* aria_key, int keyBits)
{
  Byte*	rk = aria_key->k32.round_key;
  Word *a, *z;
  int rValue;
...
PciAriaCipher::PciAriaCipher()
	: EncKeyStatus(KS_NO), DecKeyStatus(KS_NO)
{
	BlockBytes = 16;
}
