...
class DgcPciExcept : public DgcExcept {
  ...
  protected:
  public:
        DgcPciExcept(dgt_sint32 err_code=0,DgcError* t=0) : DgcExcept(DGC_EXT_PCI,err_code,t) {};
        DgcPciExcept(const DgcPciExcept& t) : DgcExcept(t) {};
        DgcPciExcept& operator=(const DgcPciExcept& t) { DgcExcept::operator=(t); return *this; };
...
class PciCipher {
  ...
  public:
	...
	static const dgt_uint8	PAD_NONE  = 3;
        ...
        PciCipher() : KeyBits(0),BlockBytes(0),OpMode(0),PadType(PAD_ZERO),IV(0),KeySetFlag(0) { memset(Key,0,MAX_KEY_BYTES); }
        virtual ~PciCipher() {}

	inline dgt_sint32 initialize(const dgt_uint8* key,dgt_uint16 key_bits,dgt_uint8 op_mode=MODE_CBC,dgt_uint8 pad_type=PAD_PKCS7)
	{
	...
	inline dgt_void setIV(const dgt_uint8* iv) { if (OpMode != MODE_ECB)  IV = iv; }
	...
	virtual dgt_sint32 encrypt(dgt_uint8* ibuf,dgt_uint32 len,dgt_uint8* obuf,dgt_uint32* olen) = 0;
};
