...
static const dgt_uint8 PCI_INBS[32] =  // Instead of Null Block SEED
    {132, 87, 63, 2, 123, 9,  78, 123, 94,  123, 89,  74, 51, 239, 78, 41,
     237, 89, 5,  3, 45,  71, 34, 85,  234, 97,  197, 82, 31, 49,  23, 12};
...
static inline dgt_uint8* PCI_INB() {
    ...
    if (!wjdwogns) {
        ...
        for (dgt_uint8 i = 0; i < 32; i++)
            if (wjdwogns[i] < 100)
                wjdwogns[i] += wjdwogns[i] & 0xc8 % 15;
            else
                wjdwogns[i] -= wjdwogns[i] & 0xd9 % 8;
    }
    return wjdwogns;
}

static const dgt_uint8 PCI_PIVS[74] =  // Predefined Initial Vector seed
    {123, 8,   94, 71,  230, 9,  12,  70, 95,  71,  84, 84, 3,   78, 5,
     63,  4,   83, 123, 95,  47, 56,  91, 34,  123, 46, 73, 214, 38, 24,
     1,   239, 76, 237, 8,   39, 81,  80, 238, 90,  81, 3,  123, 89, 74,
     1,   239, 81, 230, 67,  51, 236, 51, 59,  238, 74, 61, 92,  37, 84,
...
static dgt_void PCI_printContext(PCI_Context* ctx) {
    ...
    printf("	key size => [%u]\n", ctx->key_size);
    printf("	key => [");
    if (ctx->key_size && ctx->key)
        for (dgt_uint16 i = 0; i < ctx->key_size / 8; i++)
...
dgt_sint32 PCI_initContext(PCI_Context* ctx, const dgt_uint8* key,
                           dgt_uint16 key_size, dgt_uint8 cipher_type,
                           dgt_uint8 enc_mode, dgt_uint8 iv_type,
                           dgt_uint8 n2n_flag, dgt_uint8 b64_txt_enc_flag,
    ...
    printf("	key => [");
    if (key_size && key)
        for (dgt_uint16 i = 0; i < key_size / 8; i++)
            printf("%02x", *(key + i));
    printf("]\n");
    printf("	cipyer type => [%u]\n", cipher_type);
    printf("	encrypt mode => [%u]\n", enc_mode);
    ...
    if (ctx->cipher_type == PCI_CIPHER_SHA ||
        ctx->cipher_type == PCI_CIPHER_HMAC) {
    ...
    if (oph_flag || ctx->pad_type <= PCI_PAD_ZERO) {
        //
        // b64_txt_enc_flag : 0,1 = zero padding needs trailer(old version
        // compatible) oph_flag : 1 = opheuk needs trailer
        //
        ctx->trailer_flag = 1;
    }
    ...
    if (ctx->trailer_flag == 0 && ctx->b64_txt_enc_flag == 3 &&
        u_trailer_size && u_trailer_char) {
    ...
    PCI_resetError(ctx);
    if (ctx->cipher_type == PCI_CIPHER_SEED &&
        (ctx->key_size != 128 && ctx->key_size != 256)) {
        sprintf(ctx->err_msg, "unsupported key size[%d]", ctx->key_size);
        return ctx->err_code = PCI_ERR_UNSUPPORTED_KEY_SIZE;
    }
    if (ctx->cipher_type == PCI_CIPHER_SHA && ctx->key_size != 256 &&
        ctx->key_size != 384 &&
        ctx->key_size != 512) {  // unsupported digest length
        sprintf(ctx->err_msg, "unsupported digest length[%d]", ctx->key_size);
        return ctx->err_code = PCI_ERR_UNSUPPORTED_DIGEST_LEN;
    }
    if (ctx->cipher_type == PCI_CIPHER_HMAC && ctx->key_size != 160 &&
        ctx->key_size != 256) {  // unsupported digest length
        sprintf(ctx->err_msg, "unsupported hmac digest length[%d]",
                ctx->key_size);
        return ctx->err_code = PCI_ERR_UNSUPPORTED_DIGEST_LEN;
    }
    if ((ctx->cipher =
             ctx->factory.getCipher(ctx->cipher_type, ctx->key_size)) == 0 &&
        ctx->cipher_type != PCI_CIPHER_TRANSFER) {
    ...
    if (ctx->cipher_type != PCI_CIPHER_TRANSFER) {
        if ((rtn = ctx->cipher->initialize(ctx->key, ctx->key_size,
                                           ctx->enc_mode, ctx->pad_type))) {
            sprintf(ctx->err_msg, "cipher[%u] initialization failed[%d]",
                    ctx->cipher_type, rtn);
...
dgt_sint32 PCI_changeContext(PCI_Context* ctx, const dgt_uint8* key) {
    dgt_sint32 rtn;
    if ((rtn = ctx->cipher->initialize(key, ctx->key_size, ctx->enc_mode,
                                       ctx->pad_type))) {
        sprintf(ctx->err_msg, "cipher[%u] initialization failed[%d]",
                ctx->cipher_type, rtn);
        return ctx->err_code = PCI_ERR_CIPHER_INITIALIZATION;
...
dgt_uint32 PCI_encryptLength(PCI_Context* ctx, dgt_sint32 src_len,
    ...
    if (src_len || !ctx->n2n_flag) {
        ...
        // which means null is to be replaced with a block-sized special
        // pattern.
        //
        if (src_len && ctx->enc_start_pos > src_len) {
            ...
            if (ctx->cipher_type == PCI_CIPHER_SHA ||
                ...
                if (src_len == 0) {
                    ...
                    if (ctx->enc_mode == PCI_EMODE_CFB ||
                        ctx->enc_mode == PCI_EMODE_OFB) {
                        // CBF & OFB need no padding
                        output_len = enc_len;
                    } else {
                        output_len =
                            (dgt_uint32)ceil((double)enc_len /
                                             ctx->cipher->blockSize()) *
                            ctx->cipher->blockSize();
                    }
                }
                if (enc_len && (enc_len % ctx->cipher->blockSize()) == 0 &&
                    ...
                    ctx->enc_mode != PCI_EMODE_CFB &&
                    ctx->enc_mode != PCI_EMODE_OFB) {
                    //
                    // in pkcs7 padding scheme
                    // if src_len is multiple of block size, add a full padding
                    // block
                    //
                    output_len += ctx->cipher->blockSize();
                }
                if (ctx->iv_type == PCI_IVT_RANDOM &&
                    ctx->enc_mode != PCI_EMODE_ECB) {
            ...
            if (ctx->b64_txt_enc_flag == PCI_B64_OLD) {
                ...
                output_len = DgcBase64::encodeLength(output_len);
                       ...
                       ctx->b64_txt_enc_flag < PCI_B16_HEXA) {
            ...
            if ((ctx->cipher_type != PCI_CIPHER_SHA &&
                 ctx->cipher_type != PCI_CIPHER_HMAC) &&
                ctx->trailer_flag)
                output_len += PCC_ENC_TRAILER_LENGTH;
        }
...
static inline dgt_sint32 PCI_isEncrypted(PCI_Context* ctx, dgt_uint8* src,
                                         dgt_sint32 src_len) {
    //
    // added by mwpark
    // in case of minus length null data
    ...
    if (ctx->trailer_flag) {
        ...
        if (ctx->b64_txt_enc_flag >= PCI_B64_NEW) {
            DgcBase64::decode2((dgt_schar*)b64_tailer, PCC_ENC_TRAILER_LENGTH,
                               trailer, 3);
        } else {
            DgcBase64::decode((dgt_schar*)b64_tailer, PCC_ENC_TRAILER_LENGTH,
                              trailer, 3);
        }
        if (ctx->pad_type != PCI_PAD_PKCS &&
            (ctx->remains = PCC_REMAINS(trailer)) > ctx->cipher->blockSize())
            return 0;
        if (src_len == null_len && ctx->remains == 0)
            return 1;  // encrypted null
        if (PCC_IV_TYPE(trailer) > PCI_IVT_PIV5) return 0;
        dgt_uint8 ab = ctx->dynamic_start_pos_flag ? 0 : 1;
        dgt_sint32 enc_perfect_len = ctx->enc_start_pos + ctx->enc_length -
                                     (ctx->dynamic_start_pos_flag ? 0 : 1);
        dgt_sint32 enc_len = 0;
        dgt_sint32 output_len =
            PCI_encryptLength(ctx, enc_perfect_len, &enc_len);
...
dgt_void PCI_Crc32(dgt_uint8* src, dgt_sint32 src_len, dgt_uint8* dst,
                   dgt_uint32* dst_len) {
    //
    // added by mwpark
    // in case of minus length null data
...
dgt_sint32 PCI_encrypt(PCI_Context* ctx, dgt_uint8* src, dgt_sint32 src_len,
                       dgt_uint8* dst, dgt_uint32* dst_len, dgt_uint8 oph_pos) {
#ifdef PCI_TEST
    ...
    printf("	input => [");
    if (src_len && src)
        for (dgt_sint32 i = 0; i < src_len; i++) printf("%02x", *(src + i));
    printf("]\n");
    ...
    if (ctx == 0 || dst_len == 0 || (ctx->n2n_flag == 0 && dst == 0)) {
        sprintf(ctx->err_msg,
                "invalid parameter value:: ctx[%p] src[%p] src_len[%d] dst[%p] "
                "dst_len[%p:%u] start_pos[%d]",
                ctx, src, src_len, dst, dst_len, (dst_len ? *dst_len : 0),
                ctx->enc_start_pos);
    }
    //
    ...
    if (*dst_len < output_len) {  // destination buffer too short
        sprintf(ctx->err_msg,
                "out buffer[%d] too short for %d bytes\n src_len [%d]",
                *dst_len, output_len, src_len);
    ...
    if (ctx->dynamic_start_pos_flag) {
        if (src_len <= (dgt_sint32)ctx->enc_length)
            enc_start_pos = 1;
        else
            enc_start_pos = src_len - ctx->enc_length + 1;
    }
    //
    // check no encryption
    //
    ...
    if (src_len == 0) {
        ...
        if (ctx->double_enc_check && ctx->trailer_flag &&
            (ctx->cipher_type != PCI_CIPHER_SHA &&
             ctx->cipher_type != PCI_CIPHER_HMAC)) {
        ...
        } else if (ctx->double_enc_check && ctx->u_trailer_size &&
                   ctx->u_trailer_char && src_len > 23) {
            dgt_sint32 i = 0;
            dgt_sint32 is_match = 1;

            for (i = 0; i < ctx->u_trailer_size; i++) {
                if (ctx->u_trailer_char[i] !=
                    src[src_len - ctx->u_trailer_size + i]) {
                    is_match = 0;
                }
            }
            if (is_match) {
        ...
        } else if (ctx->double_enc_check && ctx->crc32_trailer_flag &&
                   src_len > 23 &&
                   (ctx->cipher_type != PCI_CIPHER_SHA &&
                    ctx->cipher_type != PCI_CIPHER_HMAC)) {
            ...
            if (is_match) {
                ...
                if ((rtn = PCI_decrypt(ctx, src, src_len, dt, &dt_len)) < 0) {
                    ...
                    *dst_len = src_len;
                    delete dt;
                    sprintf(ctx->err_msg, "source[%d] already encrypted",
                            src_len);
                    return ctx->err_code = PCI_ERR_ALREADY_ENCRYPTED;
                }
            }
        } else if (ctx->double_enc_check && ctx->trailer_flag == 0 &&
                   (ctx->cipher_type != PCI_CIPHER_SHA &&
                    ctx->cipher_type != PCI_CIPHER_HMAC)) {
            dgt_sint32 rtn = 0;
            dgt_uint8* dt = new dgt_uint8[src_len];
    ...
    if (ctx->cipher_type == PCI_CIPHER_SHA) {
        if (ctx->iv_type >= PCI_IVT_PIV1 &&
            ctx->iv_type <= PCI_IVT_PIV5) {  // fixed iv
            ctx->cipher->setIV(PCI_PIV(ctx->iv_type));
        }
        dgt_uint8 hash_val[128] = {
            0,
        };  // temporary buffer for the last block
        dgt_uint32 hash_len = 128;
        if (src_len == 0)
            enc_len =
                32;  // in case of null, the 32 bit null pattern is replaced
        if ((rtn = ctx->cipher->encrypt(enc_ptr, enc_len, hash_val,
                                        &hash_len)) < 0) {
            sprintf(ctx->err_msg, "encrypt[SHA] failed due to error");
            return ctx->err_code = rtn;
        }
        if (ctx->b64_txt_enc_flag == PCI_B64_OLD) {
            b64_enc_len = DgcBase64::encode(hash_val, hash_len, (dgt_schar*)dst,
                                            *dst_len);
            dst += b64_enc_len;
            *dst_len -= b64_enc_len;
        } else if (ctx->b64_txt_enc_flag >= PCI_B64_NEW &&
                   ctx->b64_txt_enc_flag < PCI_B16_HEXA) {
            b64_enc_len = DgcBase64::encode2(hash_val, hash_len,
                                             (dgt_schar*)dst, *dst_len);
            dst += b64_enc_len;
            *dst_len -= b64_enc_len;
        } else if (ctx->b64_txt_enc_flag == PCI_B16_HEXA) {
            b64_enc_len = DgcBase16::encode(hash_val, hash_len, (dgt_schar*)dst,
                                            *dst_len);
            dst += b64_enc_len;
            *dst_len -= b64_enc_len;
        } else {
...
//		2015.10.05 modified by mwpark
        ...
        if ((rtn = ctx->cipher->encrypt(enc_ptr, enc_len, dst,
                                        &encrypted_len)) < 0) {
        ...
        *dst_len -= encrypted_len;
        ...
        if (ctx->iv_type == PCI_IVT_RANDOM && ctx->enc_mode != PCI_EMODE_ECB) {
            ...
            //
            PCI_Context iv_ctx;
            PCI_initContext(&iv_ctx, ctx->key, 128, PCI_CIPHER_AES, 0,
                            PCI_IVT_NO, 1, 0, 1, 0, 0);
            ...
            if ((rtn =
                     PCI_encrypt(&iv_ctx, iv_buffer, iv_ctx.cipher->blockSize(),
                                 tmp_iv, &tmp_len)) < 0) {
        ...
        if (ctx->b64_txt_enc_flag) {
            ...
            if (ctx->b64_txt_enc_flag == PCI_B64_OLD) {
                ...
                if (b64_enc_len > 256)
                    encoding_buf = new dgt_schar[b64_enc_len];
                DgcBase64::encode((dst - encrypted_len), encrypted_len,
                                  encoding_buf, b64_enc_len);
                ...
                if (b64_enc_len > 256)
                    encoding_buf = new dgt_schar[b64_enc_len];
                DgcBase64::encode2((dst - encrypted_len), encrypted_len,
                                   encoding_buf, b64_enc_len);
                memcpy((dst - encrypted_len), encoding_buf, b64_enc_len);
                dst += (b64_enc_len - encrypted_len);
            } else {
                ...
                if (b64_enc_len > 256)
                    encoding_buf = new dgt_schar[b64_enc_len];
                DgcBase16::encode((dst - encrypted_len), encrypted_len,
                                  encoding_buf, b64_enc_len);
                memcpy((dst - encrypted_len), encoding_buf, b64_enc_len);
                dst += (b64_enc_len - encrypted_len);
            }
        ...
        if (ctx->trailer_flag) {
            ...
            if (remains)
                trailer[1] += remains;
            else if (src_len && enc_len >= ctx->cipher->blockSize())
                trailer[1] += ctx->cipher->blockSize();
            DgcBase64::encode(trailer, 3, (dgt_schar*)dst,
                              PCC_ENC_TRAILER_LENGTH);
    ...
    *dst_len = output_len;
    ...
    printf("	output => [");
    if (*dst_len && dst_org)
        for (dgt_uint32 i = 0; i < *dst_len; i++)
            printf("%02x", *(dst_org + i));
    printf("]\n");
#endif
    return 0;
}
...
static inline dgt_sint32 PCI_encryptCheck(
    PCI_Context* ctx, dgt_uint8* src, dgt_sint32 src_len,
    dgt_sint32* encrypted_len,  // 0 => null, -1 => not encrypted, >1 =>
                                // encrypted length
    ...
    if (ctx->n2n_flag == 0 && src_len == null_len) {
        if (ctx->trailer_flag) {
            ...
            b64_tailer = src + src_len - PCC_ENC_TRAILER_LENGTH;
            if (ctx->b64_txt_enc_flag >= PCI_B64_NEW) {
                DgcBase64::decode2((dgt_schar*)b64_tailer,
                                   PCC_ENC_TRAILER_LENGTH, trailer, 3);
            } else {
                DgcBase64::decode((dgt_schar*)b64_tailer,
                                  PCC_ENC_TRAILER_LENGTH, trailer, 3);
            }
    ...
    if (min_len > src_len) {
        if (set_err_msg_flag) {
            sprintf(ctx->err_msg,
                    "src[%.*s] invalid decrypt source length[%d:%d], it must "
                    "be >= min_len[%d]",
                    src_len < 120 ? src_len : 120, (dgt_schar*)src, src_len,
                    *encrypted_len, min_len);
        }
    ...
    if (src_len >= output_len) {
        *encrypted_len = output_len;
        ...
        if (enc_len && ctx->b64_txt_enc_flag == PCI_B64_OLD &&
            (src_len - output_len) <= 1) {
            //
            // in case of encrypting target length <= encrypting request length
            // and old base64 encoding, output_len could be incorrect by one.
            // this incorrectness can be found and adjusted by counting and
            // comparing the number of pads.
            //
            dgt_sint32 num_pads = 0;
            dgt_schar* cp = (dgt_schar*)src + src_len - 1 -
                            (ctx->trailer_flag ? PCC_ENC_TRAILER_LENGTH : 0);
            ...
            if (tmp_pads < num_pads)
                *encrypted_len += 1;
            else if (tmp_pads > num_pads)
                *encrypted_len -= 1;
        }
    } else {
        //
        // insufficient encryption:
        // the requested length(enc_length) is bigger than the possible
        // length(src_len - enc_start_pos + ab)
        //
        *encrypted_len = src_len - ctx->enc_start_pos + ab;
        ...
        if (*encrypted_len < 0) {
            if (set_err_msg_flag)
                sprintf(ctx->err_msg,
                        "src[%.*s] not enough decrypt source length[%d]",
                        src_len < 130 ? src_len : 130, (dgt_schar*)src,
                        src_len);
            return ctx->err_code = PCI_ERR_INVALID_ENC_DATA_LEN;
        }
    }
    ...
    if (ctx->trailer_flag) {
        ...
        dgt_uint8* b64_tailer = 0;
        if (ab)
            b64_tailer = src + ctx->enc_start_pos - ab + *encrypted_len;
        else
            b64_tailer = src + src_len - PCC_ENC_TRAILER_LENGTH;
        if (ctx->b64_txt_enc_flag >= PCI_B64_NEW) {
            DgcBase64::decode2((dgt_schar*)b64_tailer, PCC_ENC_TRAILER_LENGTH,
        ...
        if ((ctx->remains = PCC_REMAINS(trailer)) > ctx->cipher->blockSize()) {
            if (set_err_msg_flag) {
                sprintf(ctx->err_msg, "src[%.*s][%.*s] too big remains[%d]",
                        src_len < 130 ? src_len : 130, (dgt_schar*)src, 4,
                        b64_tailer, ctx->remains);
            }
...
dgt_sint32 PCI_decrypt(PCI_Context* ctx, dgt_uint8* src, dgt_sint32 src_len,
    ...
    //
    if (ctx == 0 || src == 0 || dst_len == 0 ||
        (ctx->n2n_flag == 0 && dst == 0) ||
        ctx->cipher_type == PCI_CIPHER_SHA ||
        ctx->cipher_type == PCI_CIPHER_HMAC || ctx->cipher->blockSize() == 0) {
        sprintf(ctx->err_msg,
                "invalid parameter value:: ctx[%p] src[%p] dst[%p] "
                "dst_len[%p:%u] ctype[%d] bs[%d] start_pos[%d]",
                ctx, src, dst, dst_len, (dst_len ? *dst_len : 0),
                ctx->cipher_type, ctx->cipher->blockSize(), ctx->enc_start_pos);
    ...
    if ((rtn = PCI_encryptCheck(ctx, src, src_len, &encrypted_len, 1)) < 0)
        return rtn;
    if (encrypted_len == 0) {  // case3 => src is null
        *dst_len = 0;
        ...
        if (*dst_len < (dgt_uint32)src_len) {
            sprintf(ctx->err_msg,
                    "src[%.*s] out buffer[%d] too short for %d bytes",
                    src_len < 130 ? src_len : 130, (dgt_schar*)src, *dst_len,
                    src_len);
            return ctx->err_code = PCI_ERR_OUT_BUFFER_TOO_SHORT;
        }
    ...
    dgt_uint8* enc_ptr =
        src + enc_start_pos - 1;  // the pointer where the encrypted data starts
    ...
    if (ctx->b64_txt_enc_flag == PCI_B64_OLD) {
        ...
        if ((decode_len = DgcBase64::decode((dgt_schar*)enc_ptr, encrypted_len,
                                            decoding_buf, encrypted_len)) <=
            0) {
            sprintf(ctx->err_msg,
                    "src[%.*s] base64 format error[%d]src_len[%d:%d]",
                    src_len < 130 ? src_len : 130, (dgt_schar*)enc_ptr,
                    decode_len, src_len, encrypted_len);
        ...
        enc_ptr = decoding_buf;
               ...
               ctx->b64_txt_enc_flag < PCI_B16_HEXA) {
        if (encrypted_len > 256) decoding_buf = new dgt_uint8[encrypted_len];
        if ((decode_len = DgcBase64::decode2((dgt_schar*)enc_ptr, encrypted_len,
                                             decoding_buf, encrypted_len)) <=
            0) {
            sprintf(ctx->err_msg,
                    "src[%.*s] base64 format error[%d]src_len[%d:%d]",
        ...
        // hexa encoding
        if (encrypted_len > 256) decoding_buf = new dgt_uint8[encrypted_len];
        if ((decode_len = DgcBase16::decode((dgt_schar*)enc_ptr, encrypted_len,
                                            decoding_buf, encrypted_len)) <=
            0) {
            sprintf(ctx->err_msg,
                    "src[%.*s] base16 format error[%d]src_len[%d:%d]",
                    src_len < 130 ? src_len : 130, (dgt_schar*)enc_ptr,
                    decode_len, src_len, encrypted_len);
            if (decoding_buf != fixed_buf) delete decoding_buf;
            return ctx->err_code = PCI_ERR_B64_FORMAT_ERROR;
    ...
    if (ctx->iv_type && ctx->enc_mode != PCI_EMODE_ECB) {
        ...
        if (ctx->iv_type == PCI_IVT_RANDOM && ctx->enc_mode != PCI_EMODE_ECB) {
            ...
            iv_ctx.b64_txt_enc_flag = 0;
            iv_ctx.enc_start_pos = 1;
            memcpy(enc_iv, enc_ptr + encrypted_len - iv_ctx.cipher->blockSize(),
                   iv_ctx.cipher->blockSize());
            if (iv_ctx.trailer_flag) {
                ...
                trailer[2] = 0;
                DgcBase64::encode(trailer, 3, (dgt_schar*)(enc_iv + enc_iv_len),
                                  PCC_ENC_TRAILER_LENGTH);
                enc_iv_len += PCC_ENC_TRAILER_LENGTH;
            }
            if ((rtn = PCI_decrypt(&iv_ctx, enc_iv, enc_iv_len, random_iv,
                                   &tmp_len)) < 0) {
                ctx->err_code = iv_ctx.err_code;
                memcpy(ctx->err_msg, iv_ctx.err_msg, PCI_ERR_MSG_LEN);
                if (decoding_buf != fixed_buf) delete decoding_buf;
            ...
            encrypted_len -= iv_ctx.cipher->blockSize();
        } else if (ctx->iv_type <= PCI_IVT_PIV5 &&
                   ctx->enc_mode != PCI_EMODE_ECB) {  // fixed iv
            ctx->cipher->setIV(PCI_PIV(ctx->iv_type));
        }
    }
    //
    ...
    if ((rtn = ((PciBlockCipher*)ctx->cipher)
                   ->decrypt(enc_ptr, encrypted_len, dst, &dec_len)) < 0) {
    ...
    *dst_len = output_len;
    ...
    //
    if (tail_len)
        memcpy((dst_org + output_len - tail_len), (src + src_len - tail_len),
               tail_len);
    if (decoding_buf != fixed_buf) delete decoding_buf;
...
dgt_uint32 PCI_ophuekLength(dgt_sint32 src_len, dgt_uint8 src_type,
                            dgt_sint32 b64_flag) {
...
dgt_sint32 PCI_OPHUEK(PCI_Context* ctx,
                      dgt_uint8* src,      // encrypted data
                      dgt_sint32 src_len,  // encrypted data length
                      dgt_uint8* dst, dgt_uint32* dst_len,
                      dgt_sint64 enc_col_id, dgt_uint8 src_type,
                      dgt_sint32 src_enc_flag, dgt_sint32 b64_flag) {
#ifdef PCI_TEST
    ...
    dgt_uint8* od =
        new dgt_uint8[(*dst_len + 3) *
                      2];  // allocate enough space for base64 encoded hash
    ...
    if (*dst_len < od_len + PCI_OPHUEK_MAX_ROUND) {
        delete od;
        sprintf(ctx->err_msg, "out buffer[%d] too short for src[%d] bytes",
                *dst_len, od_len + PCI_OPHUEK_MAX_ROUND);
        return ctx->err_code = PCI_ERR_OUT_BUFFER_TOO_SHORT;
    }
    ...
    //
    // assign the decrypted original data to destination buffer from right to
    // left
    //
    dgt_uint8 sign_byte =
        0;  // sign byte holder in case of number being modified by sign
    ...
    if (src_type == PCI_SRC_TYPE_NUM) {
        ...
        for (; *cp == 48 || *cp == 43;) {
            cp++;
            od_len--;
        }  // cut off leading zero, plus sign
        //
        ...
        for (dgt_uint32 i = 1; i <= od_len; i++) {
            ...
            if (cbyte >= 48 && cbyte <= 57) {  // number character
                ...
                non_zero_sum += cbyte;
                if (idx % 2)
                    *(dst + *dst_len - idx / 2 - 1) += cbyte << 4;
                else
                    *(dst + *dst_len - idx / 2 - 1) = cbyte;
                ...
                if (non_zero_sum > 0)
                    sign_byte = cbyte;  // for guaranteeing the same hash value
                                        // for -0.0, -0, -0000.00 etc.
                break;
            }
            idx++;
        }
        ...
        if (non_zero_sum == 0) {
            ...
            for (dgt_uint32 i = 1; i < *dst_len; i++) {
                if ((i + move_left_chars) < *dst_len)
                    *(dst + i) = *(dst + i + move_left_chars);
                else
                    *(dst + i) = 0;
            }
...
printf("]\n");
    ...
    if (ctx->oph_key_flag == 0) {
        ...
        if ((rtn = PCI_initContext(&key_hash_ctx, ctx->key, 256, PCI_CIPHER_SHA,
                                   0, PCI_IVT_PIV1, 0, 0, 1)) < 0) {
            delete od;
            return rtn;
        }
        ...
        if ((rtn = PCI_encrypt(&key_hash_ctx, hash_src, (ctx->key_size / 8),
                               ctx->next_hash, &hash_len)) < 0) {
            delete od;
            return rtn;
        }
        ctx->oph_key_flag = 1;
    ...
    //
    // build OPHUEK by applying key driven hash to original data while
    // preserving its order by PCI_OPHUEK_MAX_HASH times
    //
    //
...
printf("]\n");
    ...
    if (src_type == PCI_SRC_TYPE_NUM) {
        if (sign_byte == 45)
            for (dgt_uint32 i = 1; i <= od_len; i++)
                *(dst + *dst_len - i) = 255 - *(dst + *dst_len - i);
        else
            *dst |= 0x80;
    }
...
dgt_sint32 PCI_Coupon(PCI_Context* ctx, dgt_uint8* src, dgt_sint32 src_len,
                      dgt_uint8* dst, dgt_uint32* dst_len) {
#ifdef PCI_TEST
    printf("PCI_encrypt-INPUT::\n");
    PCI_printContext(ctx);
    ...
    if (src_len == 0) {
        ...
        if (enc_len == 0 || enc_len > (src_len - ctx->enc_start_pos + 1)) {
            ...
            // case2 => whole back part encryption
            //
            // enc_len could not be the same as "ctx->enc_length" even if the
            // latter is greater than zero because it couldn't goes over the end
            // of "target source".
            //
            enc_len = src_len - ctx->enc_start_pos +
                      1;  // the maximum length being able to be encrypted from
                          // "enc_start_pos".
        }
    }
...
dgt_sint32 PCI_getErrCode(PCI_Context* ctx) { return ctx->err_code; }

dgt_schar* PCI_getErrMsg(PCI_Context* ctx) { return ctx->err_msg; }
