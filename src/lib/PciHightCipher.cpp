...
dgt_void PciHightCipher::pciHightKeySched()
{
    ...
    for( i=0; i<8; i++) {
        for( j=0; j<8; j++)
            RoundKey[8+16*i+j  ] = (BYTE) (HightKey[(j-i)&7    ] + PCI_Delta[16*i+j  ]);	
// Use "&7"  instead of the "%8" for Performance

        for( j=0; j<8; j++)
            RoundKey[8+16*i+j+8] = (BYTE) (HightKey[((j-i)&7)+8] + PCI_Delta[16*i+j+8]);
    }
}
...
dgt_sint32 PciHightCipher::encryptBlock(dgt_uint8* iblock,dgt_uint8* oblock)
{
    ...
    #define PCI_HIGHT_ENC(k, i0,i1,i2,i3,i4,i5,i6,i7) {                         \
        XX[i0] = (XX[i0] ^ (PCI_HIGHT_F0[XX[i1]] + RoundKey[4*k+3])) & 0xFF;    \
        XX[i2] = (XX[i2] + (PCI_HIGHT_F1[XX[i3]] ^ RoundKey[4*k+2])) & 0xFF;    \
        XX[i4] = (XX[i4] ^ (PCI_HIGHT_F0[XX[i5]] + RoundKey[4*k+1])) & 0xFF;    \
        XX[i6] = (XX[i6] + (PCI_HIGHT_F1[XX[i7]] ^ RoundKey[4*k+0])) & 0xFF;    \
    }
...
dgt_sint32 PciHightCipher::decryptBlock(dgt_uint8* iblock,dgt_uint8* oblock)
{
    ...
    #define PCI_HIGHT_DEC(k, i0,i1,i2,i3,i4,i5,i6,i7) {                         \
        XX[i1] = (XX[i1] - (PCI_HIGHT_F1[XX[i2]] ^ RoundKey[4*k+2])) & 0xFF;    \
        XX[i3] = (XX[i3] ^ (PCI_HIGHT_F0[XX[i4]] + RoundKey[4*k+1])) & 0xFF;    \
        XX[i5] = (XX[i5] - (PCI_HIGHT_F1[XX[i6]] ^ RoundKey[4*k+0])) & 0xFF;    \
        XX[i7] = (XX[i7] ^ (PCI_HIGHT_F0[XX[i0]] + RoundKey[4*k+3])) & 0xFF;    \
    }
