   ...
   public:
    PciKeyStash(PCT_KEY_STASH* key_stash) {
        ...
        dgt_schar key_seed[PCC_MASTER_KEY_LENGTH + 1] = {
            23,  84, 239, 42, 3,   59, 5,  232, 50,  89, 34,
            202, 95, 7,   93, 123, 98, 7,  122, 9,   23, 173,
            4,   6,  99,  82, 7,   69, 38, 204, 101, 82, 76};
        memcpy(EncKey, key_seed, PCC_MASTER_KEY_LENGTH + 1);
        for (dgt_sint32 i = 0; i < PCC_MASTER_KEY_LENGTH; i++)
            if (i % 2)
                EncKey[i] |= EncKey[i + 1];
            else
                EncKey[i] &= EncKey[i + 1];
        PCI_initContext(&EncContext, EncKey, PCC_MASTER_HASH_SIZE,
                        PCI_CIPHER_AES, PCI_EMODE_CBC, PCI_IVT_PIV1, 0, 0, 1);
        PCI_initContext(&HashContext, 0, PCC_MASTER_HASH_SIZE, PCI_CIPHER_SHA,
                        0, PCI_IVT_PIV1, 0, 0, 1);
    };
    ~PciKeyStash(){};
    ...
    inline dgt_sint32 openKeySet(dgt_uint8* eks) {
        ...
        if (KeyStash->open_status) {
            sprintf(PCI_KmgrErrMsg,
                    "openKeySet failed because the key set already opened");
        ...
        if ((rtn = PCI_encrypt(&EncContext, eks, PCC_MAX_KEY_SET_LENGTH,
                               KeyStash->key_set, &tmp_len)) < 0) {
            sprintf(PCI_KmgrErrMsg, "openKeySet failed due to %s",
                    PCI_getErrMsg(&EncContext));
            return rtn;
        }
        //
        ...
        if ((rtn = PCI_encrypt(&HashContext, eks, PCC_MAX_KEY_SET_LENGTH,
                               KeyStash->key_set_signature, &tmp_len)) < 0) {
            sprintf(PCI_KmgrErrMsg, "openKeySet failed due to %s",
                    PCI_getErrMsg(&HashContext));
            return rtn;
        }
    ...
    inline dgt_sint32 getKey(dgt_uint32 key_idx, dgt_uint32 key_len,
                             dgt_uint8* key_buffer) {
        if (KeyStash->open_status == 0) {
            sprintf(PCI_KmgrErrMsg,
                    "getKey failed because the key set not opened");
            return PCC_ERR_KMGR_KEY_NOT_OPEN;
        }
        if (key_buffer == 0) {
            sprintf(PCI_KmgrErrMsg,
                    "getKey failed due to invalid input parameter");
            return PCC_ERR_KMGR_INVALID_INPUT_PARAM;
        }
        if ((key_idx + key_len) > (dgt_uint32)PCC_MAX_KEY_SET_LENGTH) {
            sprintf(PCI_KmgrErrMsg,
                    "getKey failed due to the requested key[%d] out of the "
                    "range[%d]",
        ...
        if ((rtn = PCI_decrypt(&EncContext, KeyStash->key_set,
                               PCC_MAX_EKEY_SET_LENGTH, eks_buf, &tmp_len)) <
            0) {
            sprintf(PCI_KmgrErrMsg, "getKey failed due to %s",
                    PCI_getErrMsg(&EncContext));
            return rtn;
        }
        //
        ...
        tmp_len = PCC_KEY_SET_SIG_LENGTH;
        if ((rtn = PCI_encrypt(&HashContext, eks_buf, PCC_MAX_KEY_SET_LENGTH,
                               eks_sign_buf, &tmp_len)) < 0) {
            sprintf(PCI_KmgrErrMsg, "getKey failed due to %s",
                    PCI_getErrMsg(&HashContext));
            return rtn;
        }
        if (memcmp(eks_sign_buf, KeyStash->key_set_signature,
                   PCC_KEY_SET_SIG_LENGTH)) {
            sprintf(PCI_KmgrErrMsg,
                    "getKey failed due to corrupted key stash[%s][%s]",
                    eks_sign_buf, KeyStash->key_set_signature);
...
dgt_sint32 PCI_PBKDF2(const dgt_schar* pw, const dgt_uint8* salt,
    ...
    dgt_uint32 pw_len = 0;
    if (pw == 0 || (pw_len = strlen(pw)) == 0 || salt == 0 || salt_len == 0 ||
        mk == 0 || mk_len == 0 || *mk_len == 0) {
        sprintf(PCI_KmgrErrMsg,
                "PCI_PBKDF2 failed due to invalid input "
                "parameter[%p:%u:%p:%u:%p:%p:%u",
                pw, pw_len, salt, salt_len, mk, mk_len, *mk_len);
        return PCC_ERR_KMGR_INVALID_INPUT_PARAM;
    }
    dgt_uint32 n_hash =
        (dgt_uint32)ceil(*mk_len / (PCI_PBKDF2_HASH_SIZE *
                                    1.0));  // the number of hash result for mk
    dgt_uint8 tmp_hash[PCI_PBKDF2_HASH_SIZE] = {
        0,
    };  // temporary hash value
    dgt_uint32 tmp_len =
        PCI_PBKDF2_HASH_SIZE;  // the length of temporary hash value
    HMAC_CTX ctx;              // HMAC context
    //
    // make the first salt
    //
    if (salt_len < PCI_PBKDF2_HASH_SIZE - sizeof(dgt_uint32))
        memcpy(tmp_hash, salt, salt_len);
    else
        memcpy(tmp_hash, salt, PCI_PBKDF2_HASH_SIZE - sizeof(dgt_uint32));
    //
    ...
    for (dgt_uint32 i = 0; i < n_hash; i++) {  // for the number of hashes
        ...
        if (i == n_hash - 1)
            memset(mk + c_pos, 0, *mk_len % PCI_PBKDF2_HASH_SIZE);
        else
            memset(mk + c_pos, 0, PCI_PBKDF2_HASH_SIZE);
        memcpy(tmp_hash + PCI_PBKDF2_HASH_SIZE - sizeof(dgt_uint32), &i,
               sizeof(dgt_uint32));  // strength the salt with index value
        for (dgt_uint32 j = 1; j <= PCI_PBKDF2_ITERATION;
             j++) {  // for the defined iteration count
            //
...
static dgt_sint32 PCI_generateEKMK(const dgt_schar* pw, dgt_uint8* ekmk_buf,
                                   dgt_uint32* ekmk_buf_len,
                                   dgt_sint32 hsm_mode = 0,
                                   const dgt_schar* hsm_password = 0) {
    if (pw == 0) {
        sprintf(PCI_KmgrErrMsg,
                "PCI_generateEKMK failed due to invalid input parameter");
        return PCC_ERR_KMGR_INVALID_INPUT_PARAM;
    }
    if (hsm_mode) {
        sprintf(PCI_KmgrErrMsg,
                "As of June 27, 2023, we no longer support HSM.\n");
        return PCC_ERR_NOT_SUPPORT_HSM;
    } else {
        dgt_sint32 rtn = 0;
        PCI_Context ctx;
        if ((rtn = PCI_initContext(&ctx, 0, PCC_MASTER_HASH_SIZE,
                                   PCI_CIPHER_SHA, 0, PCI_IVT_PIV1, 0, 0, 1)) <
            0) {
            sprintf(PCI_KmgrErrMsg, "PCI_generateEKMK failed due to %s",
                    PCI_getErrMsg(&ctx));
            return rtn;  // 32 bytes key
        }
        if ((rtn = PCI_encrypt(&ctx, (dgt_uint8*)pw, strlen(pw), ekmk_buf,
                               ekmk_buf_len)) < 0) {
            sprintf(PCI_KmgrErrMsg, "PCI_generateEKMK failed due to %s",
                    PCI_getErrMsg(&ctx));
            return rtn;
...
// check key
//
static dgt_sint32 PCI_checkKey(const dgt_schar* pw, dgt_schar* smk,
                               dgt_uint32 smk_len, dgt_schar* seks,
                               dgt_uint32 seks_len, dgt_schar* sks,
                               dgt_uint32 sks_len, PciKeyHolder& key_holder,
                               dgt_sint32 hsm_mode = 0,
                               const dgt_schar* hsm_password = 0) {
    //
    // check input parameters
    //
    if (pw == 0 || smk == 0 || smk_len == 0 || seks == 0 || seks_len == 0 ||
        sks == 0 || sks_len == 0) {
        sprintf(PCI_KmgrErrMsg,
                "PCI_checkKey failed due to invalid input parameter");
        return PCC_ERR_KMGR_INVALID_INPUT_PARAM;
    }
    //
    // generate EKMK - Encryption Key for Master Key
    ...
    if ((rtn = PCI_generateEKMK(pw, EKMK, &dst_len, hsm_mode, hsm_password)) <
        0)
        return rtn;
    ...
    if (hsm_mode) {
        ...
        if ((rtn = PCI_initContext(&ctx, EKMK, PCC_MASTER_HASH_SIZE,
                                   PCI_CIPHER_AES, PCI_EMODE_CBC, PCI_IVT_PIV1,
                                   0, 1, 1)) < 0)
            goto PCI_ERR_RETURN1;
        dst_len = PCC_MASTER_KEY_LENGTH;
        if ((rtn = PCI_decrypt(&ctx, (dgt_uint8*)smk, smk_len, MK, &dst_len)) <
            0)
            goto PCI_ERR_RETURN1;
    }
    ...
    //
    if ((rtn = PCI_initContext(&ctx, MK, PCC_MASTER_HASH_SIZE, PCI_CIPHER_AES,
                               PCI_EMODE_CBC, PCI_IVT_PIV1, 0, 1, 1)) < 0)
        goto PCI_ERR_RETURN1;
    dst_len = PCC_MAX_KEY_SET_LENGTH;
    if ((rtn = PCI_decrypt(&ctx, (dgt_uint8*)seks, seks_len, (dgt_uint8*)EKS,
                           &dst_len)) < 0)
        goto PCI_ERR_RETURN1;
    ...
    //
    if ((rtn = PCI_initContext(&ctx, 0, PCC_MASTER_HASH_SIZE, PCI_CIPHER_SHA, 0,
                               PCI_IVT_PIV1, 0, 1, 1)) < 0)
        goto PCI_ERR_RETURN1;
    if ((rtn = PCI_encrypt(&ctx, (dgt_uint8*)EKS, PCC_MAX_KEY_SET_LENGTH,
                           tmp_sks, &tmp_len)) < 0)
        goto PCI_ERR_RETURN1;
...
PCI_ERR_RETURN1:
    if (rtn)
        sprintf(PCI_KmgrErrMsg, "PCI_checkKey failed due to %s",
                PCI_getErrMsg(&ctx));
    return rtn;
}
...
dgt_sint32 PCI_createKey(const dgt_schar* pw, dgt_void* thread_ptr,
                         ...
                         dgt_uint32* seks_len, dgt_schar* sks,
                         dgt_uint32* sks_len, dgt_sint32 hsm_mode,
    ...
    // check input parameters
    //
    if (pw == 0 || smk == 0 || smk_len == 0 || *smk_len == 0 || seks == 0 ||
        seks_len == 0 || *seks_len == 0 || sks == 0 || sks_len == 0 ||
        *sks_len == 0) {
        sprintf(PCI_KmgrErrMsg,
                "PCI_createKey failed due to invalid input parameter");
        return PCC_ERR_KMGR_INVALID_INPUT_PARAM;
    }
    ...
    if ((rtn = DgcPwChecker::checkStrength(pw, strlen(pw))) < 0) {
        sprintf(PCI_KmgrErrMsg, "PCI_createKey failed due to %s",
                EXCEPT->getErr()->message());
        delete EXCEPTnC;
    ...
    dst_len = PCC_MASTER_KEY_LENGTH;
    if ((rtn = PCI_PBKDF2(pw, rand_bytes, EVP_MAX_KEY_LENGTH, MK, &dst_len)))
        return rtn;
    ...
    RAND_bytes(rand_bytes, EVP_MAX_KEY_LENGTH);
    dst_len = PCC_MAX_KEY_SET_LENGTH;
    if ((rtn = PCI_PBKDF2(pw, rand_bytes, EVP_MAX_KEY_LENGTH, EKS, &dst_len)))
        return rtn;
    //
    ...
    if ((rtn = PCI_initContext(&ctx, 0, PCC_MASTER_HASH_SIZE, PCI_CIPHER_SHA, 0,
                               PCI_IVT_PIV1, 0, 1, 1)) < 0)
        goto PCI_ERR_RETURN2;
    if ((rtn = PCI_encrypt(&ctx, EKS, PCC_MAX_KEY_SET_LENGTH, (dgt_uint8*)sks,
                           sks_len)) < 0)
        goto PCI_ERR_RETURN2;
    //
    ...
    if ((rtn = PCI_initContext(&ctx, MK, PCC_MASTER_HASH_SIZE, PCI_CIPHER_AES,
                               PCI_EMODE_CBC, PCI_IVT_PIV1, 0, 1, 1)) < 0)
        goto PCI_ERR_RETURN2;
    if ((rtn = PCI_encrypt(&ctx, EKS, PCC_MAX_KEY_SET_LENGTH, (dgt_uint8*)seks,
                           seks_len)) < 0)
        goto PCI_ERR_RETURN2;
    //
    ...
    if (hsm_mode) {
        ...
        if ((rtn = PCI_initContext(&ctx, EKMK, PCC_MASTER_HASH_SIZE,
                                   ...
                                   0, 1, 1)) < 0)
            goto PCI_ERR_RETURN2;
        if ((rtn = PCI_encrypt(&ctx, MK, PCC_MASTER_KEY_LENGTH, (dgt_uint8*)smk,
                               smk_len)) < 0)
            goto PCI_ERR_RETURN2;
    }
...
dgt_sint32 PCI_checkPassword(const dgt_schar* pw, dgt_schar* smk,
    ...
    dgt_sint32 rtn = 0;
    PciKeyHolder key_holder;
    rtn = PCI_checkKey(pw, smk, smk_len, seks, seks_len, sks, sks_len,
                       key_holder, hsm_mode, hsm_password);
...
dgt_sint32 PCI_changePassword(const dgt_schar* old_pw, const dgt_schar* new_pw,
    ...
    dgt_sint32 rtn = 0;
    PciKeyHolder key_holder;
    if ((rtn = PCI_checkKey(old_pw, smk, smk_len, seks, seks_len, sks, sks_len,
                            key_holder, hsm_mode, hsm_password)) < 0)
        return rtn;
    //
    // generate EKMK for the new password
    //
    if ((rtn = DgcPwChecker::checkStrength(new_pw, strlen(new_pw))) < 0) {
        sprintf(PCI_KmgrErrMsg, "PCI_changePassword failed due to %s",
                EXCEPT->getErr()->message());
        delete EXCEPTnC;
        return PCC_ERR_KMGR_WEAK_PASSWORD;
    }
    dgt_uint32 dst_len = PCC_MASTER_KEY_LENGTH;
    if ((rtn = PCI_generateEKMK(new_pw, key_holder.eKMK(), &dst_len, hsm_mode,
                                hsm_password)) < 0)
        return rtn;
    //
    // generate SMK - Saved Master Key
    ...
    if (hsm_mode) {
        ...
        if ((rtn = PCI_initContext(&ctx, key_holder.eKMK(),
                                   PCC_MASTER_HASH_SIZE, PCI_CIPHER_AES,
                                   PCI_EMODE_CBC, PCI_IVT_PIV1, 0, 1, 1)) < 0)
            goto PCI_ERR_RETURN3;
        if ((rtn = PCI_encrypt(&ctx, key_holder.mK(), PCC_MASTER_KEY_LENGTH,
                               (dgt_uint8*)smk, &smk_len)) < 0)
            goto PCI_ERR_RETURN3;
    }
...
dgt_sint32 PCI_setKeyStash(PCT_KEY_STASH* key_stash) {
    ...
    for (dgt_uint32 i = 0; i < PCC_KEY_SET_SIG_LENGTH; i++)
        printf("%02x", *(key_stash->key_set_signature + i));
    printf("]\n");
    printf("        key_set => [");
    for (dgt_uint32 i = 0; i < PCC_MAX_EKEY_SET_LENGTH; i++)
        printf("%02x", *(key_stash->key_set + i));
    printf("]\n");
#endif

    if (key_stash == 0) {
        sprintf(PCI_KmgrErrMsg,
                "PCI_setKeyStash failed due to invalid input parameter");
        return PCC_ERR_KMGR_INVALID_INPUT_PARAM;
    }
    if (PCI_KeyStash) {
        sprintf(PCI_KmgrErrMsg,
                "PCI_setKeyStash failed because the key stash's already set");
...
dgt_sint32 PCI_getKeyStash(PCT_KEY_STASH** key_stash) {
    if (PCI_KeyStash == 0 || PCI_KeyStash->keyStash() == 0) {
        sprintf(PCI_KmgrErrMsg,
                "PCI_openKey failed due to no key stash, call PCI_setKeyStash "
                "first");
...
dgt_sint32 PCI_openKey(const dgt_schar* pw, dgt_schar* smk, dgt_uint32 smk_len,
    ...
    dgt_sint32 rtn = 0;
    PciKeyHolder key_holder;
    if ((rtn = PCI_checkKey(pw, smk, smk_len, seks, seks_len, sks, sks_len,
                            key_holder, hsm_mode, hsm_password)) < 0)
        return rtn;
    ...
    printf("        key_signature => [");
    for (dgt_uint32 i = 0; i < PCC_KEY_SET_SIG_LENGTH; i++)
        printf("%02x", *(PCI_KeyStash->keyStash()->key_set_signature + i));
    printf("]\n");
    printf("        key_set => [");
    for (dgt_uint32 i = 0; i < PCC_MAX_EKEY_SET_LENGTH; i++)
        printf("%02x", *(PCI_KeyStash->keyStash()->key_set + i));
    printf("]\n");
#endif
...
dgt_sint32 PCI_closeKey(const dgt_schar* pw, dgt_schar* smk, dgt_uint32 smk_len,
    ...
    if (PCI_KeyStash == 0) {
        sprintf(PCI_KmgrErrMsg,
                "PCI_closeKey failed due to no key stash, call PCI_setKeyStash "
                "first");
        return PCC_ERR_KMGR_NO_KEY_STASH;
    }
    dgt_sint32 rtn = 0;
...
dgt_sint32 PCI_getEncryptKey(dgt_uint32 key_idx,
    ...
    if (key_len == 0 || key_buffer == 0) {
        sprintf(PCI_KmgrErrMsg,
                "PCI_getEncryptKey failed due to invalid input parameter");
        return PCC_ERR_KMGR_INVALID_INPUT_PARAM;
    }
    if (PCI_KeyStash == 0) {
        sprintf(PCI_KmgrErrMsg,
                "PCI_getEncryptKey failed due to no key stash, call "
                "PCI_setKeyStash first");
        return PCC_ERR_KMGR_NO_KEY_STASH;
    }
...
const dgt_schar* PCI_getKmgrErrMsg() { return PCI_KmgrErrMsg; }
