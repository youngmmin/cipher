...
#define loadU32(v)                                      \
    ((unsigned int)((((unsigned char*)(&v))[3] << 24) | \
...
dgt_void PciLeaCipher::PciLeaKeyGeneric(LEA_KEY* key, const unsigned char* mk,
                                        unsigned int mk_len) {
    if (!key)
        return;
    else if (!mk)
...
dgt_sint32 PciLeaCipher::encryptBlock(dgt_uint8* iblock, dgt_uint8* oblock) {
    LEA_KEY* key = &LeaKey;
    unsigned int X0, X1, X2, X3;

    const unsigned int* _pt = (const unsigned int*)iblock;
    unsigned int* _ct = (unsigned int*)oblock;
...
dgt_sint32 PciLeaCipher::decryptBlock(dgt_uint8* iblock, dgt_uint8* oblock) {
    LEA_KEY* key = &LeaKey;
    unsigned int X0, X1, X2, X3;

    unsigned int* _pt = (unsigned int*)oblock;
    const unsigned int* _ct = (const unsigned int*)iblock;
...
dgt_sint32 PciLeaCipher::initializeContext() {
    BlockBytes = 16;
    PciLeaKeyGeneric(&LeaKey, Key, KeyBits / 8);
    return 0;
}

PciLeaCipher::PciLeaCipher() {}
