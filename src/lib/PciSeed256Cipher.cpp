...
void PciSeed256Cipher::SeedEncRoundKey(dgt_uint32 *pdwRoundKey,
                                       dgt_uint8 *pbUserKey) {
    dgt_uint32 A, B, C, D, E, F, G, H, T0, T1, *K = pdwRoundKey;

    seed_char2word(pbUserKey, A);
    seed_char2word(pbUserKey + 4, B);
    seed_char2word(pbUserKey + 8, C);
    seed_char2word(pbUserKey + 12, D);
    seed_char2word(pbUserKey + 16, E);
    seed_char2word(pbUserKey + 20, F);
    seed_char2word(pbUserKey + 24, G);
    seed_char2word(pbUserKey + 28, H);
...
dgt_sint32 PciSeed256Cipher::encryptBlock(dgt_uint8 *iblock,
                                          dgt_uint8 *oblock) {
...
dgt_sint32 PciSeed256Cipher::decryptBlock(dgt_uint8 *iblock,
                                          dgt_uint8 *oblock) {
    memcpy(oblock, iblock, BlockBytes);
...
dgt_sint32 PciSeed256Cipher::initializeContext() {
    memset(Schedule, 0, NoRoundKeys);
    SeedEncRoundKey(Schedule, (dgt_uint8 *)Key);
    return 0;
}

PciSeed256Cipher::PciSeed256Cipher() { BlockBytes = 16; }

PciSeed256Cipher::~PciSeed256Cipher() {}
