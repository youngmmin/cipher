...
#define GetB0(A) ((dgt_uint8)((A)))
#define GetB1(A) ((dgt_uint8)((A) >> 8))
#define GetB2(A) ((dgt_uint8)((A) >> 16))
#define GetB3(A) ((dgt_uint8)((A) >> 24))
...
#define SeedRound(L0, L1, R0, R1, K)                                           \
    {                                                                          \
        ...
        T1 ^= T0;                                                              \
        T1 =                                                                   \
            SS0[GetB0(T1)] ^ SS1[GetB1(T1)] ^ SS2[GetB2(T1)] ^ SS3[GetB3(T1)]; \
        T0 += T1;                                                              \
        T0 =                                                                   \
            SS0[GetB0(T0)] ^ SS1[GetB1(T0)] ^ SS2[GetB2(T0)] ^ SS3[GetB3(T0)]; \
        T1 += T0;                                                              \
        T1 =                                                                   \
            SS0[GetB0(T1)] ^ SS1[GetB1(T1)] ^ SS2[GetB2(T1)] ^ SS3[GetB3(T1)]; \
        T0 += T1;                                                              \
...
#define EndianChange(dwS)                        \
    ((ROTL((dwS), 8) & (dgt_uint32)0x00ff00ff) | \
     (ROTL((dwS), 24) & (dgt_uint32)0xff00ff00))
...
#define seed_char2word(c, i)                                             \
    (i) = ((((dgt_uint32)(c)[0]) << 24) | (((dgt_uint32)(c)[1]) << 16) | \
           (((dgt_uint32)(c)[2]) << 8) | ((dgt_uint32)(c)[3]))

#define seed_word2char(l, c)                        \
...
void PciSeedCipher::SeedEncRoundKey(dgt_uint32 *pdwRoundKey,
                                    dgt_uint8 *pbUserKey) {
    dgt_uint32 A, B, C, D, T0, T1, *K = pdwRoundKey;

    seed_char2word(pbUserKey, A);
    seed_char2word(pbUserKey + 4, B);
    seed_char2word(pbUserKey + 8, C);
    seed_char2word(pbUserKey + 12, D);

    EncRoundKeyUpdate0(K, A, B, C, D, KC0);
    EncRoundKeyUpdate1(K + 2, A, B, C, D, KC1);
    EncRoundKeyUpdate0(K + 4, A, B, C, D, KC2);
    EncRoundKeyUpdate1(K + 6, A, B, C, D, KC3);
    EncRoundKeyUpdate0(K + 8, A, B, C, D, KC4);
    EncRoundKeyUpdate1(K + 10, A, B, C, D, KC5);
    EncRoundKeyUpdate0(K + 12, A, B, C, D, KC6);
    EncRoundKeyUpdate1(K + 14, A, B, C, D, KC7);
    EncRoundKeyUpdate0(K + 16, A, B, C, D, KC8);
    EncRoundKeyUpdate1(K + 18, A, B, C, D, KC9);
    EncRoundKeyUpdate0(K + 20, A, B, C, D, KC10);
    EncRoundKeyUpdate1(K + 22, A, B, C, D, KC11);
    EncRoundKeyUpdate0(K + 24, A, B, C, D, KC12);
    EncRoundKeyUpdate1(K + 26, A, B, C, D, KC13);
    EncRoundKeyUpdate0(K + 28, A, B, C, D, KC14);
    T0 = A + C - KC15;
    T1 = B - D + KC15;
...
dgt_sint32 PciSeedCipher::encryptBlock(dgt_uint8 *iblock, dgt_uint8 *oblock) {
    memcpy(oblock, iblock, BlockBytes);
    dgt_uint32 L0, L1, R0, R1, T0, T1, *K = Schedule;

    seed_char2word(oblock, L0);
    seed_char2word(oblock + 4, L1);
    seed_char2word(oblock + 8, R0);
    seed_char2word(oblock + 12, R1);
    ...
    seed_word2char(R0, oblock);
    seed_word2char(R1, oblock + 4);
    seed_word2char(L0, oblock + 8);
    seed_word2char(L1, oblock + 12);
    return 0;
}

dgt_sint32 PciSeedCipher::decryptBlock(dgt_uint8 *iblock, dgt_uint8 *oblock) {
    memcpy(oblock, iblock, BlockBytes);
...
dgt_sint32 PciSeedCipher::initializeContext() {
    memset(Schedule, 0, NoRoundKeys);
    SeedEncRoundKey(Schedule, (dgt_uint8 *)Key);
    return 0;
}

PciSeedCipher::PciSeedCipher() { BlockBytes = 16; }

PciSeedCipher::~PciSeedCipher() {}
