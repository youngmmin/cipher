...
class PcaApiSession : public PcaHashNode {
   private:
    static const dgt_sint32 PSP_ERR_LOCK_FAIL =
        -30309;  // error code for spin locking failure

    dgt_slock
        FuncLatch;  // spin lock for concurrency control over crypt function
   ...
   protected:
   public:
    static inline dgt_sint32 computeHashValue(
        const char* client_ip, const char* user_id, const char* client_program,
        const dgt_schar* client_mac, const dgt_schar* db_user,
        const dgt_schar* os_user, dgt_uint8 access_protocol,
        dgt_schar* hash_string) {
        dgt_sint32 hash_val = 0;
        sprintf(hash_string, "[%s]^[%s]^[%s]^[%s]^[%s]^[%s]^[%d]",
                client_ip ? client_ip : "", user_id ? user_id : "",
                client_program ? client_program : "",
                client_mac ? client_mac : "", db_user ? db_user : "",
                os_user ? os_user : "", access_protocol ? access_protocol : 0);
        for (dgt_sint32 i = 0; i < PSP_HS_LENGTH && hash_string[i]; i++)
            hash_val += hash_string[i] * (i + 1);
        return hash_val;
    }

    PcaApiSession(const dgt_schar* client_ip, const char* user_id,
                  const char* client_program, const dgt_schar* client_mac,
                  const dgt_schar* db_user, const dgt_schar* os_user,
                  dgt_uint8 access_protocol, PcaSession* pca_session,
                  dgt_sint32 shared_flag = 0)
        : ClientSession(pca_session), SharedFlag(shared_flag) {
        DgcSpinLock::unlock(&FuncLatch);
        memset(HashString, 0, PSP_HS_LENGTH);
        computeHashValue(client_ip, user_id, client_program, client_mac,
                         db_user, os_user, access_protocol, HashString);
    ...
    inline dgt_sint32 encryptLength(const dgt_schar* enc_col_name,
        ...
        if (DgcSpinLock::lock(&FuncLatch)) {
            PcaKeySvrSessionPool::logging(
                PSP_ERR_LOCK_FAIL, "locking the api session pool failed");
    ...
    inline dgt_sint32 encryptLengthWithVirtualKey(
        ...
        PcaPrivilege* priv = ClientSession->getVKeyPrivilege(
...
#if 1  // added by mwpark 2017.06.26 for file encrypt/decrypt logging and
    ...
    inline dgt_sint32 encrypt(dgt_sint64 enc_col_id, dgt_uint8* src,
        ...
        dgt_sint32 rtn =
            ClientSession->encrypt(enc_col_id, src, src_len, dst, dst_len);
        DgcSpinLock::unlock(&FuncLatch);
        return rtn;
    };
    ...
    inline dgt_sint32 encrypt(dgt_sint64 enc_col_id, dgt_uint8* src,
                              dgt_sint32 src_len, dgt_uint8* dst,
                              dgt_uint32* dst_len, int sql_type) {
        if (DgcSpinLock::lock(&FuncLatch)) {
            PcaKeySvrSessionPool::logging(
                PSP_ERR_LOCK_FAIL, "locking the api session pool failed");
            return PSP_ERR_LOCK_FAIL;
        }
        dgt_sint32 rtn = ClientSession->encrypt(enc_col_id, src, src_len, dst,
                                                dst_len, sql_type);
        DgcSpinLock::unlock(&FuncLatch);
        return rtn;
    };
    ...
    inline dgt_sint32 encrypt(const dgt_schar* enc_col_name, dgt_uint8* src,
        ...
        dgt_sint32 rtn =
            ClientSession->encrypt(enc_col_name, src, src_len, dst, dst_len);
        DgcSpinLock::unlock(&FuncLatch);
        return rtn;
    };
    ...
    inline dgt_sint32 encrypt(const dgt_schar* enc_col_name, dgt_uint8* src,
        ...
        dgt_sint32 rtn = ClientSession->encrypt(enc_col_name, src, src_len, dst,
                                                dst_len, header_flag);
        DgcSpinLock::unlock(&FuncLatch);
        return rtn;
    };
    ...
    inline dgt_sint32 crypt_test(dgt_sint64 enc_col_id, dgt_uint8* str,
                                 dgt_sint32 ksv_num) {
        if (DgcSpinLock::lock(&FuncLatch)) {
            PcaKeySvrSessionPool::logging(
                PSP_ERR_LOCK_FAIL, "locking the api session pool failed");
    ...
    inline dgt_sint32 crypt_test(const dgt_schar* enc_col_name, dgt_uint8* str,
                                 dgt_sint32 ksv_num) {
        if (DgcSpinLock::lock(&FuncLatch)) {
            PcaKeySvrSessionPool::logging(
                PSP_ERR_LOCK_FAIL, "locking the api session pool failed");
    ...
    inline dgt_sint32 PtMonCipherApiCall(dgt_sint64 enc_col_id,
                                         const dgt_schar* str,
                                         dgt_sint32* key_stat) {
        if (DgcSpinLock::lock(&FuncLatch)) {
            PcaKeySvrSessionPool::logging(
        ...
        for (dgt_sint32 i = 0; i < 3; i++)
            key_stat[i] =
                ClientSession->crypt_test(enc_col_id, (dgt_uint8*)str, i);

        DgcSpinLock::unlock(&FuncLatch);
        return 0;
    };

    inline dgt_sint32 PtMonCipherApiCall(const dgt_schar* enc_col_name,
                                         const dgt_schar* str,
                                         dgt_sint32* key_stat) {
        if (DgcSpinLock::lock(&FuncLatch)) {
            PcaKeySvrSessionPool::logging(
        ...
        for (dgt_sint32 i = 0; i < 3; i++)
            key_stat[i] =
                ClientSession->crypt_test(enc_col_name, (dgt_uint8*)str, i);
    ...
    inline dgt_sint32 decrypt(dgt_sint64 enc_col_id, dgt_uint8* src,
        ...
        dgt_sint32 rtn =
            ClientSession->decrypt(enc_col_id, src, src_len, dst, dst_len);
        DgcSpinLock::unlock(&FuncLatch);
        return rtn;
    };
    ...
    inline dgt_sint32 decrypt(dgt_sint64 enc_col_id, dgt_uint8* src,
        ...
        dgt_sint32 rtn = ClientSession->decrypt(enc_col_id, src, src_len, dst,
                                                dst_len, sql_type);
        DgcSpinLock::unlock(&FuncLatch);
        return rtn;
    };
    ...
    inline dgt_sint32 decrypt(const dgt_schar* enc_col_name, dgt_uint8* src,
        ...
        dgt_sint32 rtn =
            ClientSession->decrypt(enc_col_name, src, src_len, dst, dst_len);
        DgcSpinLock::unlock(&FuncLatch);
        return rtn;
    };
    ...
    inline dgt_sint32 decrypt(const dgt_schar* enc_col_name, dgt_uint8* src,
        ...
        dgt_sint32 rtn = ClientSession->decrypt(enc_col_name, src, src_len, dst,
                                                dst_len, header_flag);
        DgcSpinLock::unlock(&FuncLatch);
        return rtn;
    };
    ...
    inline dgt_sint32 decrypt_vkey(dgt_sint64 virtual_key_id, dgt_uint8* src,
                                   dgt_sint32 src_len, dgt_uint8* dst,
        ...
        dgt_sint32 rtn = ClientSession->decrypt_vkey(
            virtual_key_id, src, src_len, dst, dst_len, target_type, name1,
    ...
    inline dgt_sint32 OPHUEK(dgt_sint64 enc_col_id, dgt_uint8* src,
        ...
        dgt_sint32 rtn = ClientSession->OPHUEK(enc_col_id, src, src_len, dst,
                                               dst_len, src_enc_flag);
        DgcSpinLock::unlock(&FuncLatch);
        return rtn;
    };
    ...
    inline dgt_sint32 OPHUEK(const dgt_schar* enc_col_name, dgt_uint8* src,
        ...
        dgt_sint32 rtn = ClientSession->OPHUEK(enc_col_name, src, src_len, dst,
                                               dst_len, src_enc_flag);
        DgcSpinLock::unlock(&FuncLatch);
        return rtn;
    };
    ...
    inline dgt_sint32 encryptCpn(dgt_sint64 enc_col_id, dgt_uint8* src,
        ...
        dgt_sint32 rtn = ClientSession->encryptCpn(enc_col_id, src, src_len,
                                                   coupon, coupon_len);
        DgcSpinLock::unlock(&FuncLatch);
        return rtn;
    };
    ...
    inline dgt_sint32 encryptCpn(const dgt_schar* enc_col_name, dgt_uint8* src,
        ...
        dgt_sint32 rtn = ClientSession->encryptCpn(enc_col_name, src, src_len,
                                                   coupon, coupon_len);
        DgcSpinLock::unlock(&FuncLatch);
        return rtn;
    };
    ...
    inline dgt_sint32 decryptCpn(dgt_sint64 enc_col_id, dgt_uint8* coupon,
        ...
        dgt_sint32 rtn = ClientSession->decryptCpn(enc_col_id, coupon,
                                                   coupon_len, dst, dst_len);
        DgcSpinLock::unlock(&FuncLatch);
        return rtn;
    };
    ...
    inline dgt_sint32 decryptCpn(const dgt_schar* enc_col_name,
        ...
        dgt_sint32 rtn = ClientSession->decryptCpn(enc_col_name, coupon,
                                                   coupon_len, dst, dst_len);
        DgcSpinLock::unlock(&FuncLatch);
        return rtn;
    };
    ...
    inline dgt_sint32 getNewSqlFlag() {
        return ClientSession->getNewSqlFlag();
    };
    ...
    inline dgt_sint32 getKey(const dgt_schar* enc_col_name,
        ...
        dgt_sint32 rtn =
            ClientSession->getKey(enc_col_name, key_buffer, key_size);
        DgcSpinLock::unlock(&FuncLatch);
        return rtn;
    };
    ...
    inline dgt_sint32 getKeyInfo(const dgt_schar* enc_col_name,
        ...
        dgt_sint32 rtn = ClientSession->getKeyInfo(enc_col_name, passwd,
                                                   key_info_buf, buf_len);
        DgcSpinLock::unlock(&FuncLatch);
        return rtn;
    }
    ...
    inline dgt_sint32 getRegEngine(dgt_schar* reg_name,
                                   PccRegExprSearchEngine** param) {
        if (DgcSpinLock::lock(&FuncLatch)) {
            PcaKeySvrSessionPool::logging(
                PSP_ERR_LOCK_FAIL, "locking the api session pool failed");
    ...
    inline dgt_uint32 getFileHeaderFlag() {
        return ClientSession->getFileHeaderFlag();
    }
    inline dgt_uint32 getFileRealBytes() {
        return ClientSession->getFileRealBytes();
    }
    inline dgt_schar* getFileKeyName() {
        return ClientSession->getFileKeyName();
    }
    ...
    inline dgt_void setFileRealBytes(dgt_uint32 real_bytes) {
        ClientSession->setFileRealBytes(real_bytes);
    }
...
class PcaApiSessionPool : public DgcObject {
   private:
    static const dgt_sint32 PSP_MAX_SESSIONS =
        10000;  // size of session hash table
    static const dgt_sint32 PSP_ERR_LOCK_FAIL =
        -30309;  // error code for spin locking failure
    ...
    dgt_sint32 LastAssignedIdx;   // the index in the shared api session pool
                                  // that was fetched latest.
    ...
    inline dgt_sint32 findSharedSession(dgt_uint8 no_lock = 0) {
        ...
        if (NumSharedSession == 0) {
            //
            // not initialized shared session pool, initialize shared session
            // pool
            //
            if ((NumSharedSession = PcaSessionPool::numSharedSession()) > 0) {
                ApiSharedSessions =
                    new PcaApiSession*[NumSharedSession];  // create a shared
                                                           // session pool
                ...
                for (dgt_sint32 i = 0; i < NumSharedSession; i++) {
                    ...
                    //
                    PcaSession* pca_session = PcaSessionPool::openSession(
                        0);  // open a shared client session
                    if (pca_session) {
                        if (pca_session->openSession() >=
                            0) {  // open a shared user session
                            ApiSharedSessions[i] = new PcaApiSession(
                                "", "", "", "", "", "", 0, pca_session,
                                1);  // register with the api session pool
                            SharedFreePool[FreePoolSize++] = pca_session->sid();
                        } else {
                            //
                            // failed to open a user session, remove it from the
                            // client session pool
                            //
        ...
        if (FreePoolSize > 0) {
            ...
            // forward search
            //
            for (dgt_sint32 i = LastAssignedIdx + 1; i < NumSharedSession;
                 i++) {
    ...
    inline dgt_void returnSharedSession(dgt_sint32 sid) {
        if (DgcSpinLock::lock(&PoolLatch)) {
            ...
            for (dgt_sint32 i = 0; i < NumSharedSession; i++) {
                if (ApiSharedSessions[i] &&
                    ApiSharedSessions[i]->sid() == sid) {
                    rtn_sid = sid;
                    break;
                }
            }
            if (rtn_sid >= 0 && FreePoolSize < NumSharedSession)
                SharedFreePool[FreePoolSize++] = rtn_sid;
            DgcSpinLock::unlock(&PoolLatch);
        }
    };

    inline dgt_sint32 findApiSession(
        const dgt_schar* client_ip, const dgt_schar* user_id,
        const dgt_schar* client_program, const dgt_schar* client_mac,
        const dgt_schar* db_user, const dgt_schar* os_user,
        dgt_uint8 access_protocol, dgt_uint8 no_lock = 0) {
        dgt_sint32 sid;
        if ((!client_ip || *client_ip == 0) && (!user_id || *user_id == 0) &&
            (!client_program || *client_program == 0) &&
            (!client_mac || *client_mac == 0) && (!db_user || *db_user == 0) &&
            (!os_user || *os_user == 0) && !access_protocol) {
            //
            // a shared session asked
            //
            if ((sid = findSharedSession(no_lock)) > 0) return sid;
            //
            // in case that a client asks a shared session but fails to get one
            // due to problem, the library would better try to create an
            // personal session, which is the reason why an error is not
            // returned at this point.
            //
        }
        dgt_schar hash_string[PSP_HS_LENGTH];
        memset(hash_string, 0, PSP_HS_LENGTH);
        dgt_sint32 hval = PcaApiSession::computeHashValue(
                              client_ip, user_id, client_program, client_mac,
                              db_user, os_user, access_protocol, hash_string) %
                          PSP_MAX_SESSIONS;
        if (DgcSpinLock::lock(&PoolLatch)) {
            PcaKeySvrSessionPool::logging(
                sid = PSP_ERR_LOCK_FAIL, "locking the api session pool failed");
        } else {
            ...
            while (api_session) {
                ...
                //
                if (strncmp(api_session->hashString(), hash_string,
                            PSP_HS_LENGTH) == 0)
                    break;
            ...
            if (!api_session) {
                //
                // not found in the api session pool and then open a client
                // session first
                //
                ...
                if (pca_session) {
                    ...
                    //
                    dgt_sint32 rtn = 0;
                    if ((rtn = pca_session->openSession(
                             0, "", "", client_ip, db_user, os_user,
                             client_program, access_protocol, user_id,
                             client_mac)) >= 0) {
                        if (DgcSpinLock::lock(&PoolLatch)) {
                            PcaSessionPool::closeSession(sid);
                            PcaKeySvrSessionPool::logging(
                                sid = PSP_ERR_LOCK_FAIL,
                                "locking the api session pool failed");
                        } else {
                            //
                            // register with the api session pool
                            //
                            api_session = new PcaApiSession(
                                client_ip, user_id, client_program, client_mac,
                                db_user, os_user, access_protocol, pca_session);
                            if (ApiSessions[hval])
                                api_session->setNext(
                                    ApiSessions[hval]);  // set it as the first
                                                         // node
    ...
    inline dgt_void removeApiSession(const dgt_schar* client_ip,
                                     const dgt_schar* user_id,
                                     const dgt_schar* client_program,
                                     const dgt_schar* client_mac,
                                     const dgt_schar* db_user,
                                     const dgt_schar* os_user,
                                     dgt_uint8 access_protocol) {
        dgt_schar hash_string[PSP_HS_LENGTH];
        dgt_sint32 hval = PcaApiSession::computeHashValue(
                              client_ip, user_id, client_program, client_mac,
                              db_user, os_user, access_protocol, hash_string) %
        ...
        if (DgcSpinLock::lock(&PoolLatch)) {
            ...
            dgt_sint32 sid = -1;
            while (curr) {
                if (strncmp(curr->hashString(), hash_string, PSP_HS_LENGTH) ==
                    0) {
                    //
                    ...
                    if (prev)
                        ...
                        ApiSessions[hval] = (PcaApiSession*)curr->next();
                    curr->setNext();
                    sid = curr->sid();
                    delete curr;
                    break;
   ...
   public:
    ...
    static inline dgt_sint32 initialize(dgt_schar* info_file_path = 0,
                                        const dgt_schar* credentials_pw = 0) {
        return PcaSessionPool::initialize(info_file_path, credentials_pw);
    };

    static inline dgt_sint32 getApiSession(
        const dgt_schar* client_ip, const dgt_schar* user_id,
        const dgt_schar* client_program, const dgt_schar* client_mac,
        const dgt_schar* db_user, const dgt_schar* os_user,
        dgt_uint8 access_protocol, dgt_uint8 no_lock = 0) {
        ...
        return ApiSessionPool->findApiSession(
            client_ip, user_id, client_program, client_mac, db_user, os_user,
            access_protocol, no_lock);
#else
        return ApiSessionPool.findApiSession(client_ip, user_id, client_program,
                                             client_mac, db_user, os_user,
                                             access_protocol, no_lock);
#endif
    };
    ...
    static inline void closeApiSession(const dgt_schar* client_ip,
                                       const dgt_schar* user_id,
                                       const dgt_schar* client_program,
                                       const dgt_schar* client_mac,
                                       const dgt_schar* db_user,
...
#ifdef hpux11
        if (ApiSessionPool == 0) ApiSessionPool = new PcaApiSessionPool();
        ApiSessionPool->removeApiSession(client_ip, user_id, client_program,
                                         client_mac, db_user, os_user,
                                         access_protocol);
#else
        ApiSessionPool.removeApiSession(client_ip, user_id, client_program,
    ...
    static dgt_sint32 putKeyInfo(const dgt_schar* key_info_buffer,
                                 dgt_uint32 buffer_len,
                                 const dgt_schar* passwd) {
