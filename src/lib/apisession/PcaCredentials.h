...
class PcaCredentials : public DgcObject {
   ...
   public:
    ...
    inline dgt_void setSvcHome(const dgt_schar* val) {
        strncpy(SvcHome, val, 128);
    };
    inline dgt_void setIP(const dgt_schar* val) { strncpy(IP, val, 64); };
    inline dgt_void setMAC(const dgt_schar* val) { strncpy(MAC, val, 64); };
    inline dgt_void setInstanceName(const dgt_schar* val) {
        strncpy(InstanceName, val, 32);
    };
    inline dgt_void setDbName(const dgt_schar* val) {
        strncpy(DbName, val, 32);
    };
    inline dgt_void setDbUser(const dgt_schar* val) {
        strncpy(DbUser, val, 32);
    };
    inline dgt_void setOsUser(const dgt_schar* val) {
        strncpy(OsUser, val, 32);
    };
    inline dgt_void setProgram(const dgt_schar* val) {
        strncpy(Program, val, 128);
    };
    inline dgt_void setOrgUserID(const dgt_schar* val) {
        strncpy(OrgUserID, val, 32);
    };

    dgt_sint32 generate(const dgt_schar* svc_name, const dgt_schar* user_id,
                        const dgt_schar* password,
                        const dgt_schar* credentials_pw = 0);
    dgt_sint32 parse(const dgt_schar* credentials,
                     const dgt_schar* credentials_pw = 0);
};
