...
dgt_schar* PcaCryptParamPool::getCryptParam(dgt_schar* crypt_param_name)
{
	...
	dgt_sint64 key_id = 0;
#ifndef WIN32
	key_id = DgcCRC64::calCRC64Case(key_id, (dgt_uint8*)crypt_param_name, dg_strlen(crypt_param_name));
#else
	key_id = DgcCRC64::calCRC64Case(key_id, (dgt_uint8*)crypt_param_name, strlen(crypt_param_name));
#endif
	// find crypt param
...
dgt_schar* PcaCryptParamPool::putCryptParam(dgt_schar* crypt_param_name, dgt_schar* param)
{
	memset(ErrMsg, 0, sizeof(ErrMsg));
	// get crc value
	dgt_sint64 key_id = 0;
