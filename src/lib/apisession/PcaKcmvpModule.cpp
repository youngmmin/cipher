...
static char PcAPIDL_ifs[1][33] = {"K_CurrState"};

PcaKcmvpModule::PcaKcmvpModule() { KCMVP_LIB_PC_API = 0; }
...
#ifndef WIN32
    if (K_CurrState_fp == 0) {
        if (KCMVP_LIB_PC_API == 0 && initializeModule() < 0)
            return KCMVP_ERR_LOAD_FAILED;
        if ((K_CurrState_fp = (KCMVP_K_CurrState_type)dlsym(
                 KCMVP_LIB_PC_API, "K_CurrState")) == 0) {
