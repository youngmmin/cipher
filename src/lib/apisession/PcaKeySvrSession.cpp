...
#ifdef WIN32
static const dgt_schar* PKSS_DEFAULT_CONF_FILE_PATH="C:\\Program Files\\SINSIWAY\\Petra\\api\\petra_cipher_api.conf";
#else
static const dgt_schar* PKSS_DEFAULT_CONF_FILE_PATH="/var/tmp/.petra/petra_cipher_api.conf";
#endif
...
dgt_schar* PcaKeySvrSession::findConfPath(dgt_schar* conf_file_path)
{
	...
	if ((conf_file_path=dg_getenv("ORACLE_SID")) && *conf_file_path) {
		sprintf(ConfFilePath,"/var/tmp/.petra/petra_cipher_api_ora_%s.conf",dg_strnupper(conf_file_path,256));
		if ((fp=fopen(ConfFilePath,"r"))) {
			fclose(fp);
			return ConfFilePath;
		}
	...
	if ((conf_file_path=dg_getenv("TB_SID")) && *conf_file_path) {
                sprintf(ConfFilePath,"/var/tmp/.petra/petra_cipher_api_tib_%s.conf",dg_strnupper(conf_file_path,256));
                if ((fp=fopen(ConfFilePath,"r"))) {
                        fclose(fp);
                        return ConfFilePath;
                }
...
#ifndef WIN32
	: Current(0), ErrCode(0), LoadCount(0)
#else
	: Current(0), ErrCode(0), LoadCount(0)
#endif
{
	...
	Primary.con_timeout = Secondary.con_timeout = Third.con_timeout = PKSS_DFLT_CON_TIMEOUT;
	Primary.in_timeout = Secondary.in_timeout = Third.in_timeout = PKSS_DFLT_IN_TIMEOUT;
	Primary.out_timeout = Secondary.out_timeout = Third.out_timeout = PKSS_DFLT_OUT_TIMEOUT;
...
dgt_sint32 PcaKeySvrSession::loadConfFile(dgt_sint32 is_agent)
{
	...
	if ((ErrCode=nvp.parseFromFile(ConfFilePath))) {
		...
		dgt_sint32	reload_before_conn_flag = 0;
                if ((val=nvp.getValue("keysvr.reload_before_connect_flag"))) reload_before_conn_flag = strtol(val,0,10);
		if (LoadCount++ == 0 || reload_before_conn_flag) {
			if (is_agent) {
				if ((val=nvp.getValue("agent.primary.host"))) strncpy(Primary.host,val,64);
				if ((val=nvp.getValue("agent.primary.port"))) Primary.port=(dgt_uint16)strtol(val,0,10);
				if ((val=nvp.getValue("keysvr.primary.credentials"))) strncpy(Primary.credentials, val, 1023);
				if ((val=nvp.getValue("agent.secondary.host"))) strncpy(Secondary.host,val,64);
				if ((val=nvp.getValue("agent.secondary.port"))) Secondary.port=(dgt_uint16)strtol(val,0,10);
				if ((val=nvp.getValue("keysvr.secondary.credentials"))) strncpy(Secondary.credentials, val, 1023);
				if ((val=nvp.getValue("agent.third.host"))) strncpy(Third.host,val,64);
				if ((val=nvp.getValue("agent.third.port"))) Third.port=(dgt_uint16)strtol(val,0,10);
				if ((val=nvp.getValue("keysvr.third.credentials"))) strncpy(Third.credentials, val, 1023);
			} else {
		                if ((val=nvp.getValue("keysvr.primary.host"))) strncpy(Primary.host, val, 64);
				if ((val=nvp.getValue("keysvr.primary.port"))) Primary.port = (dgt_uint16)strtol(val,0,10);
				if ((val=nvp.getValue("keysvr.primary.con_timeout"))) Primary.con_timeout = strtol(val,0,10);
				if ((val=nvp.getValue("keysvr.primary.In_timeout"))) Primary.in_timeout = strtol(val,0,10);
				if ((val=nvp.getValue("keysvr.primary.out_timeout"))) Primary.out_timeout = strtol(val,0,10);
				if ((val=nvp.getValue("keysvr.primary.credentials"))) strncpy(Primary.credentials, val, 1023);
				if ((val=nvp.getValue("keysvr.secondary.host"))) strncpy(Secondary.host, val, 64);
				if ((val=nvp.getValue("keysvr.secondary.port"))) Secondary.port = (dgt_uint16)strtol(val,0,10);
				if ((val=nvp.getValue("keysvr.secondary.con_timeout"))) Secondary.con_timeout = strtol(val,0,10);
				if ((val=nvp.getValue("keysvr.secondary.In_timeout"))) Secondary.in_timeout = strtol(val,0,10);
				if ((val=nvp.getValue("keysvr.secondary.out_timeout"))) Secondary.out_timeout = strtol(val,0,10);
				if ((val=nvp.getValue("keysvr.secondary.credentials"))) strncpy(Secondary.credentials, val, 1023);
				if ((val=nvp.getValue("keysvr.third.host"))) strncpy(Third.host, val, 64);
				if ((val=nvp.getValue("keysvr.third.port"))) Third.port = (dgt_uint16)strtol(val,0,10);
				if ((val=nvp.getValue("keysvr.third.con_timeout"))) Third.con_timeout = strtol(val,0,10);
				if ((val=nvp.getValue("keysvr.third.In_timeout"))) Third.in_timeout = strtol(val,0,10);
				if ((val=nvp.getValue("keysvr.third.out_timeout"))) Third.out_timeout = strtol(val,0,10);
				if ((val=nvp.getValue("keysvr.third.credentials"))) strncpy(Third.credentials, val, 1023);
#if 0
#ifndef WIN32
