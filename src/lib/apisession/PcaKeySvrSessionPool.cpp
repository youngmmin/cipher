...
dgt_sint32 PcaKeySvrSessionPool::ThreadSessionCheckInterval =
    PcaKeySvrSessionPool::PKSSP_THRD_SESS_CHECK_INTERVAL;
dgt_sint32 PcaKeySvrSessionPool::ThreadSessionCleanInterval =
    PcaKeySvrSessionPool::PKSSP_THRD_SESS_CLEAN_INTERVAL;
dgt_sint32 PcaKeySvrSessionPool::ThreadSessionMaxSleep =
    PcaKeySvrSessionPool::PKSSP_THRD_SESS_MAX_SLEEP;
dgt_sint32 PcaKeySvrSessionPool::NumSharedSession = 10;
dgt_sint32 PcaKeySvrSessionPool::MaxPrivateSession =
    PcaKeySvrSessionPool::PKSSP_MAX_PRIVATE_SESSION;
...
#endif
{
    ...
    for (dgt_sint32 i = 0; i < PKSSP_MAX_STARVE_SESSIONS; i++) {
        StarvingQueue[i].degree = 0;
        StarvingQueue[i].svr_session = 0;
    }
...
dgt_sint32 PcaKeySvrSessionPool::initialize(dgt_schar* conf_file_path,
                                            const dgt_schar* credentials_pw,
                                            dgt_sint32 stand_alone_flag) {
    dgt_sint32 keysvr_sessions = 0;
    PcaNameValuePair nvp;
    dgt_sint32 rtn = 0;

    if ((rtn =
             nvp.parseFromFile(conf_file_path = PcaKeySvrSession::findConfPath(
                                   conf_file_path)))) {
        if (stand_alone_flag == 0) logging(rtn, nvp.errMsg());
    } else {
        dgt_sint32 conf_file_path_len = strlen(conf_file_path);
        dgt_schar* trace_conf_file_path =
            new dgt_schar[conf_file_path_len +
                          15];  // added by shson 20200212 for realtime
                                // tracelevel setting
        ...
        dgt_schar* val = 0;
        if ((val = nvp.getValue("same_err_no_log_interval")))
            no_log_interval = strtol(val, 0, 10);
        ...
        for (dgt_sint32 i = 0; i < strlen(conf_file_path); i++) {
            if (*(conf_file_path + conf_file_path_len - i) == '/') {
                i--;  // for include '/'
                strncpy(trace_conf_file_path, conf_file_path,
                        conf_file_path_len - i);
                break;
            }
        }
#else
        strcpy(trace_conf_file_path,
               "C:\\Program Files\\SINSIWAY\\Petra\\api\\");
#endif
        strcat(trace_conf_file_path, "PetraTrace.conf");
        if (!TraceMemMap) TraceMemMap = new PccFileMemMap;
        if (TraceMemMap && TraceMemMap->isLoaded() == 0 &&
            TraceMemMap->load(trace_conf_file_path, 1, 0) < 0) {
            DgcExcept* e = EXCEPTnC;
            if (e) delete e;
            rtn = -66101;
            logging("error code : %d TraceMemMap load failed [%s]", rtn,
                    trace_conf_file_path);
        }
        delete trace_conf_file_path;
        // if ((val=nvp.getValue("trace_level"))) TraceLevel = strtol(val,0,10);
        if ((val = nvp.getValue("encrypt_local_flag")))
            EncryptLocalFlag = strtol(val, 0, 10);
        if ((val = nvp.getValue("decrypt_local_flag")))
            DecryptLocalFlag = strtol(val, 0, 10);
        // if ((val=nvp.getValue("new_sql_interval"))) NewSqlInterval =
        // strtol(val,0,10);
        if ((val = nvp.getValue("keysvr.sessions")))
            keysvr_sessions = strtol(val, 0, 10);
        if ((val = nvp.getValue("num_shared_session")))
            NumSharedSession = strtol(val, 0, 10);
        if ((val = nvp.getValue("shared_session_ip")))
            strncpy(SharedSessionIP, val, 64);
        if ((val = nvp.getValue("shared_session_uid")))
            strncpy(SharedSessionUID, val, 32);
        if ((val = nvp.getValue("shared_session_program")))
            strncpy(SharedSessionProgram, val, 128);
        if ((val = nvp.getValue("max_private_session")))
            MaxPrivateSession = strtol(val, 0, 10);
        if ((val = nvp.getValue("default_enc_col_name")))
            strncpy(DefaultEncColName, val, 128);
        if ((val = nvp.getValue("ip_prob_cmd_path")))
            strncpy(IpProbCmdPath, val, 128);
        if ((val = nvp.getValue("ip_prob_cmd_args")))
            strncpy(IpProbCmdArgs, val, 128);
        if ((val = nvp.getValue("ip_prob_pos"))) IpProbPos = strtol(val, 0, 10);
        if ((val = nvp.getValue("mac_prob_cmd_path")))
            strncpy(MacProbCmdPath, val, 128);
        if ((val = nvp.getValue("mac_prob_cmd_args")))
            strncpy(MacProbCmdArgs, val, 128);
        if ((val = nvp.getValue("mac_prob_ip_pos")))
            MacProbIpPos = strtol(val, 0, 10);
        if ((val = nvp.getValue("mac_prob_pos")))
            MacProbPos = strtol(val, 0, 10);
        if ((val = nvp.getValue("thread_session_check_interval")))
            ThreadSessionCheckInterval = strtol(val, 0, 10);
        if ((val = nvp.getValue("thread_session_clean_interval")))
            ThreadSessionCleanInterval = strtol(val, 0, 10);
        if ((val = nvp.getValue("thread_session_max_sleep")))
            ThreadSessionMaxSleep = strtol(val, 0, 10);
        if ((val = nvp.getValue("char_set"))) strncpy(CharSet, val, 32);
        if ((val = nvp.getValue("instance_name")))
            strncpy(InstanceName, val, 32);
        if ((val = nvp.getValue("db_name"))) strncpy(DbName, val, 32);
        if ((val = nvp.getValue("double_enc_check")) &&
            strncasecmp("yes", val, 3) == 0)
            DoubleEncCheck = 1;
        if ((val = nvp.getValue("trailer_flag")))
            TrailerFlag = strtol(val, 0, 10);
        if ((val = nvp.getValue("rule_effective_time")))
            RuleEffectiveTime = strtol(val, 0, 10);
        if ((val = nvp.getValue("api_mode"))) ApiMode = strtol(val, 0, 10);
        if ((val = nvp.getValue("op_mode"))) OpMode = strtol(val, 0, 10);
#ifndef WIN32
        if ((val = nvp.getValue("oltp_log_mode")))
            OltpLogMode = (dgt_uint8)strtol(val, 0, 10);
        if ((val = nvp.getValue("udp_host1"))) strncpy(UdpHost1, val, 64);
        if ((val = nvp.getValue("udp_port1")))
            UdpPort1 = (dgt_uint16)strtol(val, 0, 10);
        if ((val = nvp.getValue("udp_host2"))) strncpy(UdpHost2, val, 64);
        if ((val = nvp.getValue("udp_port2")))
            UdpPort2 = (dgt_uint16)strtol(val, 0, 10);
#endif
        if ((val = nvp.getValue("vl_col_id"))) VlColID = strtol(val, 0, 10);
        if ((val = nvp.getValue("vl_length"))) VlLength = strtol(val, 0, 10);
        if ((val = nvp.getValue("vl_enc_length")))
            VlEncLength = strtol(val, 0, 10);
...
#endif
#if 1  // added by chchung 2017.6.20 for allowing optional decrypting error
       // handling
        if ((val = nvp.getValue("decrypt_fail_src_rtn")) &&
            strncasecmp(val, "yes", 3) == 0)
            DecryptFailSrcRtnFlag = 1;
#endif
        ...
        if (traceLevel() > 0) {
            ...
            logging("configuration file[%s] parsed.",
                    conf_file_path ? conf_file_path : "");
            logging("log_file_path => [%s]",
            ...
            logging("thread_session_check_interval => [%d]",
                    ThreadSessionCheckInterval);
            logging("thread_session_clean_interval => [%d]",
                    ThreadSessionCleanInterval);
    ...
    if (stand_alone_flag == 0) {
        ...
        if (traceLevel() > 0)
            logging("%d key server sessions created", CurrNumSessions);

#if 1  // modified by chchung 2015.9.13 for adding test mode
        for (dgt_sint32 i = 0;
             OpMode < PCI_OP_NO_PULL_NO_PUSH && i < CurrNumSessions; i++) {
#else
        for (dgt_sint32 i = 0; i < CurrNumSessions; i++) {
#endif
            dgt_sint32 tmp_rtn = 0;
            if ((tmp_rtn = SessionTable[i]->initialize(credentials_pw)))
                logging(tmp_rtn, SessionTable[i]->errMsg());
        }
        if (traceLevel() > 0)
            logging("%d key server sessions initialized", CurrNumSessions);
    }
#ifndef WIN32
    if (KcmvpMode) {
        ...
        if (tmp_rtn)
            logging("KCMVP KLIB module loading failed[%d] \n", tmp_rtn);
        return tmp_rtn;
    }
#endif
    return rtn;
