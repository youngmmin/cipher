...
class PcaKeySvrSessionPool : public DgcObject {
  private:
	...
	static const dgt_sint32 PKSSP_GET_MAX_TRY=1000;			// get free session max try
	static const dgt_sint32	PKSSP_DEFAULT_ELF=1;			// default encrypt local flag
	static const dgt_sint32	PKSSP_DEFAULT_DLF=1;			// default decrypt local flag
	static const dgt_sint32	PKSSP_DEFAULT_NSI=0;			// default new sql interval in microsecond, 0 means this function is not used
	static const dgt_sint32	PKSSP_DEFAULT_NS=1;			// default number of sessions
	static const dgt_sint32 PKSSP_DEFAULT_SERR_NO_LOG_INTERVAL=30;	// same error no logging interval
	static const dgt_sint32	PKSSP_MAX_STARVE_SESSIONS=2048;		// size of starving session table
	static const dgt_sint32 PKSSP_STARVE_THRESHOLD=600;		// starving threshold
	static const dgt_sint32 PKSSP_MAX_PRIVATE_SESSION=10000;	// the maximum number of private sessions
	static const dgt_sint32 PKSSP_THRD_SESS_CHECK_INTERVAL=10;	// thread session check interval
	static const dgt_sint32 PKSSP_THRD_SESS_CLEAN_INTERVAL=180;	// thread session clean interval
	static const dgt_sint32 PKSSP_THRD_SESS_MAX_SLEEP=600;		// thread session max sleep

	//static dgt_sint32	TraceLevel;				// trace level
	static dgt_sint32	ThreadSessionCheckInterval;		// thread session check interval
	static dgt_sint32	ThreadSessionCleanInterval;		// thread session clean interval
	...
	static dgt_sint32	MaxPrivateSession;			// the maximum number of private sessions
	...
	dgt_sint64		NewSqlInterval;				// no request interval in micro second for defining sql boundary
...
#endif
#if 1 // added by chchung 2017.6.20 for for allowing optional decrypting error handling
        dgt_uint8               DecryptFailSrcRtnFlag;                 // source return when failure of decrypting instead of returning error
#endif
  ...
  public:
	...
	static inline dgt_sint32 traceLevel() { 
		if (TraceMemMap && TraceMemMap->isLoaded()) return (dgt_sint32)*((dgt_uint8*)TraceMemMap->address());
		else return 0; 
	};
	static inline dgt_sint32 threadSessionCheckInterval() { return	ThreadSessionCheckInterval; };
	static inline dgt_sint32 threadSessionCleanInterval() { return ThreadSessionCleanInterval; };
	static inline dgt_sint32 threadSessionMaxSleep() { return ThreadSessionMaxSleep; };
	...
	inline dgt_void	  setNewSqlInterval(dgt_sint64 new_sql_interval) { NewSqlInterval=new_sql_interval; };
			...
			if (DgcSpinLock::lock(&PoolLatch)) {
				...
				if (starving_queue_id >= 0) {
					...
					if (i == PKSSP_STARVE_THRESHOLD && *session == 0) {
						...
						//
						for(dgt_sint32 sq_id=0; sq_id<PKSSP_MAX_STARVE_SESSIONS; sq_id++) {
	...
	inline dgt_sint32 returnSession(PcaKeySvrSession* session)
	{
		dgt_sint32	rtn=0;
		if (DgcSpinLock::lock(&PoolLatch)) {
			logging(PKSSP_ERR_LOCK_FAIL,"locking the server session pool failed");
			rtn = PKSSP_ERR_LOCK_FAIL;
		} else {
			if (NumStarveSessions > 0) {
				...
				if (max_starving_id >= 0) {
					//
					// found a starving API session and give the server session to it
					//
			...
			if (session) {
				logging(PKSSP_ERR_NO_EMPTY_SPACE, "no empty space in server session pool");
	...
	dgt_sint32 initialize(dgt_schar* info_file_path=0,const dgt_schar* credentials_pw=0,dgt_sint32 stand_alone_flag=0);
	...
	inline dgt_sint32 oltpFileLogging(pc_type_file_request_in* log_request)
	{
		...
		//
        	...
        	if (UdpBindFlag2 == 1) {
	                ...
	                mcp8((dgt_uint8*)MarshallFileBuffer, (dgt_uint8*)&log_request->user_sid);
                	...
                	*(MarshallFileBuffer+489)=log_request->mode;
	                ...
	                mcp8((dgt_uint8*)MarshallFileBuffer+620, (dgt_uint8*)&log_request->file_size);
        	        mcp8((dgt_uint8*)MarshallFileBuffer+628, (dgt_uint8*)&log_request->processed_byte);
        	        memcpy(MarshallFileBuffer+636, log_request->zone_name,130);
        	        mcp4((dgt_uint8*)MarshallFileBuffer+766, (dgt_uint8*)&log_request->enc_start_date);
        	        mcp4((dgt_uint8*)MarshallFileBuffer+770, (dgt_uint8*)&log_request->enc_end_date);
        	        memcpy(MarshallFileBuffer+774,log_request->err_msg,256);
                	if (ClientDatagram2.sendData((dgt_uint8*)MarshallFileBuffer,1030) < 0) {
	                        logging("OLTP LogMode Udp[%s-%u] send failed.",UdpHost2,UdpPort2);
        	                return 0;
                	}
	        } else {
        	        //
