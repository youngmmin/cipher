...
dgt_sint32 PcaKeySvrSessionSoha::connectKeySvr() {
    ...
    for (dgt_sint32 i = 0; i < ConnectTryCnt; i++) {
        ...
        //
        if ((ErrCode = Credentials.parse(Current->credentials,
                                         CredentialsPassword))) {
            sprintf(ErrMsg, "parsing credentials failed[%s]",
                    Credentials.errMsg());
        ...
        if (strcasecmp(Current->host, "local") == 0) {
            ...
            continue;
#else
            CurrConn = new PccLocalConnection(Current->in_timeout,
                                              Current->out_timeout);
            if (CurrConn->connect(Credentials.svcHome(), Credentials.svcName(),
            ...
            sprintf(con_string,
                    "(address=(protocol=tcp)(host=%s)(port=%u)(conn_timeout=%d)"
                    "(in_timeout=%d)(out_timeout=%d)(db_name=%s))",
                    Current->host, Current->port, Current->con_timeout,
                    Current->in_timeout, Current->out_timeout,
                    Credentials.svcName());
            CurrConn = new DgcSohaConnection();
            if (CurrConn->connect(con_string, Credentials.svcName(),
                                  Credentials.userID(), Credentials.password(),
                                  "petra_cipher_api")) {
                setError(EXCEPT);
                delete EXCEPTnC;
...
dgt_sint32 PcaKeySvrSessionSoha::crypt(dgt_sint32 msg_type,
                                       dgt_sint64 enc_col_id, dgt_uint8* src,
...
CONNECT_AGAIN:
    ...
    if ((frows = CryptStmt->execute(1000, &CryptBind)) < 0) {
        ...
        delete EXCEPTnC;
        cleanCurrConnection();
        logging("%s, 1connection[%s:%u] closed.", ErrMsg, Current->host,
                Current->port);
    ...
    if (*dst_len < rtn_remains) {
        sprintf(ErrMsg, "output buffer[%u] underflow for %u", *dst_len,
                rtn_remains);
    ...
    *dst_len = rtn_remains;
    ...
    //
    while (rtn_remains > 0) {
        if (rtn_remains < (dgt_uint32)seg_remains)
            seg_remains = (dgt_sint32)rtn_remains;
...
dgt_sint32 PcaKeySvrSessionSoha::connectKeySvr(dgt_sint32 ksv_num) {
    ...
    switch (ksv_num) {
        case 0:
            ...
            break;
        case 1:
            if (Secondary.host[0]) Current = &Secondary;
            break;
        case 2:
            if (Third.host[0]) Current = &Third;
            break;
        default:
            return PKSS_INVALID_HOST;  // invalid Key Server
    }
    if (!Current)
        return PKSS_SESSION_NOT_FOUND;  // key server info. is not exist.

    //
    // parse credentials which has service name, user id, password
    //
    if ((ErrCode =
             Credentials.parse(Current->credentials, CredentialsPassword))) {
    ...
    if (strcasecmp(Current->host, "local") == 0) {
...
#else
        CurrConn =
            new PccLocalConnection(Current->in_timeout, Current->out_timeout);
        if (CurrConn->connect(Credentials.svcHome(), Credentials.svcName(),
                              Credentials.userID(), Credentials.password(),
                              "petra_cipher_api")) {
            setError(EXCEPT);
        ...
        dgt_schar con_string[256];
        sprintf(con_string,
                "(address=(protocol=tcp)(host=%s)(port=%u)(conn_timeout=%d)(in_"
                "timeout=%d)(out_timeout=%d)(db_name=%s))",
                Current->host, Current->port, Current->con_timeout,
                Current->in_timeout, Current->out_timeout,
                Credentials.svcName());
        CurrConn = new DgcSohaConnection();
...
dgt_sint32 PcaKeySvrSessionSoha::encrypt(dgt_sint64 enc_col_id, dgt_uint8* src,
                                         dgt_sint32 src_len, dgt_uint8* dst,
                                         dgt_uint32* dst_len,
                                         dgt_sint32 ksv_num) {
...
dgt_sint32 PcaKeySvrSessionSoha::decrypt(dgt_sint64 enc_col_id, dgt_uint8* src,
                                         dgt_sint32 src_len, dgt_uint8* dst,
                                         dgt_uint32* dst_len,
                                         dgt_sint32 ksv_num) {
...
PcaKeySvrSessionSoha::PcaKeySvrSessionSoha()
    : OpenSessBind(10),
      ...
      GetRsaKeyStmt(0) {
#ifndef WIN32  // added by chchung, 2012.12.8 ensuring the initialization of
               // DgcSession's static variables
...
PcaKeySvrSessionSoha::~PcaKeySvrSessionSoha() { cleanCurrConnection(); }

dgt_sint32 PcaKeySvrSessionSoha::initialize(
    const dgt_schar* credentials_password) {
    if (credentials_password)
        strncpy(CredentialsPassword, credentials_password, 32);
#if 0
...
dgt_sint32 PcaKeySvrSessionSoha::openSession(
    dgt_sint32 db_sid, const dgt_schar* instance_name, const dgt_schar* db_name,
    const dgt_schar* ip, const dgt_schar* db_user, const dgt_schar* os_user,
    const dgt_schar* program, dgt_uint8 protocol, const dgt_schar* user_id,
    const dgt_schar* mac, pc_type_open_sess_out* sess_out) {
    //
    // build bind data
    //
...
CONNECT_AGAIN1:
    ...
    if ((frows = OpenSessStmt->execute(1, &OpenSessBind)) < 0) {
        ...
        delete EXCEPTnC;
        cleanCurrConnection();
        logging("%s, 2connection[%s:%u] closed.", ErrMsg, Current->host,
                Current->port);
    ...
    if ((frows = OpenSessStmt->fetch()) < 0) {
        ...
        delete EXCEPTnC;
        cleanCurrConnection();
        logging("%s, 3connection[%s:%u] closed.", ErrMsg, Current->host,
                Current->port);
        if (ntry++ == 0) goto CONNECT_AGAIN1;
        return ErrCode = PKSS_CONNECT_ERROR;
    }
...
dgt_sint32 PcaKeySvrSessionSoha::getPriv(dgt_sint64 user_sid,
...
CONNECT_AGAIN2:
    ...
    if ((frows = GetPrivStmt->execute(1, &GetPrivBind)) < 0) {
        ...
        delete EXCEPTnC;
        cleanCurrConnection();
        logging("%s, 4connection[%s:%u] closed.", ErrMsg, Current->host,
                Current->port);
    ...
    if ((frows = GetPrivStmt->fetch()) < 0) {
        ...
        delete EXCEPTnC;
        cleanCurrConnection();
        logging("%s, 5connection[%s:%u] closed.", ErrMsg, Current->host,
                Current->port);
        if (ntry++ == 0) goto CONNECT_AGAIN2;
        return ErrCode = PKSS_CONNECT_ERROR;
    }
...
dgt_sint32 PcaKeySvrSessionSoha::getVKeyPriv(
    dgt_sint64 user_sid, dgt_sint64 virtual_key_id, dgt_uint8 crypt_type,
    pc_type_get_vkey_priv_out* priv_out, dgt_uint8 target_type,
    ...
    if (target_type == PCI_VKEY_TARGET_TYPE_DB) {
        return getVKeyDbPriv(user_sid, virtual_key_id, crypt_type, name1, name2,
                             name3, name4, name5, priv_out);
    } else if (target_type == PCI_VKEY_TARGET_TYPE_FILE) {
        return getVKeyFilePriv(user_sid, virtual_key_id, crypt_type, name1,
                               name2, name3, name4, priv_out);
    }
    return 0;
}
...
dgt_sint32 PcaKeySvrSessionSoha::getVKeyDbPriv(
    dgt_sint64 user_sid, dgt_sint64 virtual_key_id, dgt_uint8 crypt_type,
    dgt_schar* db_name, dgt_schar* schema_name, dgt_schar* db_user_name,
    dgt_schar* table_name, dgt_schar* column_name,
    ...
    if (db_name)
        strncpy(priv_in.name1, db_name,
                dg_strlen(db_name) > 32 ? 32 : dg_strlen(db_name));
    if (schema_name)
        strncpy(priv_in.name2, schema_name,
                dg_strlen(schema_name) > 32 ? 32 : dg_strlen(schema_name));
    if (db_user_name)
        strncpy(priv_in.name3, db_user_name,
                dg_strlen(db_user_name) > 32 ? 32 : dg_strlen(db_user_name));
    if (table_name)
        strncpy(priv_in.name4, table_name,
                dg_strlen(table_name) > 32 ? 32 : dg_strlen(table_name));
    if (column_name)
        strncpy(priv_in.name5, column_name,
                dg_strlen(column_name) > 32 ? 32 : dg_strlen(column_name));
...
CONNECT_AGAIN11:
    ...
    if ((frows = GetVKeyDbPrivStmt->execute(1, &GetVKeyDbPrivBind)) < 0) {
        ...
        delete EXCEPTnC;
        cleanCurrConnection();
        logging("%s, 4.1connection[%s:%u] closed.", ErrMsg, Current->host,
                Current->port);
    ...
    if ((frows = GetVKeyDbPrivStmt->fetch()) < 0) {
        ...
        delete EXCEPTnC;
        cleanCurrConnection();
        logging("%s, 5.1connection[%s:%u] closed.", ErrMsg, Current->host,
                Current->port);
        if (ntry++ == 0) goto CONNECT_AGAIN11;
        return ErrCode = PKSS_CONNECT_ERROR;
    }
...
dgt_sint32 PcaKeySvrSessionSoha::getVKeyFilePriv(
    dgt_sint64 user_sid, dgt_sint64 virtual_key_id, dgt_uint8 crypt_type,
    dgt_schar* host_name, dgt_schar* os_user_name, dgt_schar* file_path,
    dgt_schar* file_name, pc_type_get_vkey_priv_out* priv_out) {
    // build bind data
...
CONNECT_AGAIN12:
    ...
    if ((frows = GetVKeyFilePrivStmt->execute(1, &GetVKeyFilePrivBind)) < 0) {
        ...
        delete EXCEPTnC;
        cleanCurrConnection();
        logging("%s, 4.2connection[%s:%u] closed.", ErrMsg, Current->host,
                Current->port);
    ...
    if ((frows = GetVKeyFilePrivStmt->fetch()) < 0) {
        ...
        delete EXCEPTnC;
        cleanCurrConnection();
        logging("%s, 5.2connection[%s:%u] closed.", ErrMsg, Current->host,
                Current->port);
        if (ntry++ == 0) goto CONNECT_AGAIN12;
        return ErrCode = PKSS_CONNECT_ERROR;
    }
...
dgt_sint32 PcaKeySvrSessionSoha::getKey(dgt_sint64 key_id,
...
CONNECT_AGAIN8:
    ...
    if ((frows = GetKeyStmt->execute(1, &GetKeyBind)) < 0) {
        ...
        delete EXCEPTnC;
        cleanCurrConnection();
        logging("%s, 6connection[%s:%u] closed.", ErrMsg, Current->host,
                Current->port);
    ...
    if ((frows = GetKeyStmt->fetch()) < 0) {
        ...
        delete EXCEPTnC;
        cleanCurrConnection();
        logging("%s, 7connection[%s:%u] closed.", ErrMsg, Current->host,
                Current->port);
        if (ntry++ == 0) goto CONNECT_AGAIN8;
        return ErrCode = PKSS_CONNECT_ERROR;
    }
...
CONNECT_AGAIN3:
    ...
    if ((frows = AlertStmt->execute(1, &AlertBind)) < 0) {
        ...
        delete EXCEPTnC;
        cleanCurrConnection();
        logging("%s, 8connection[%s:%u] closed.", ErrMsg, Current->host,
                Current->port);
...
dgt_sint32 PcaKeySvrSessionSoha::approve(dgt_sint64 user_sid,
                                         dgt_sint64 enc_col_id,
...
CONNECT_AGAIN4:
    ...
    if ((frows = ApproveStmt->execute(1, &ApproveBind)) < 0) {
        ...
        delete EXCEPTnC;
        cleanCurrConnection();
        logging("%s, 9connection[%s:%u] closed.", ErrMsg, Current->host,
                Current->port);
    ...
    if ((frows = ApproveStmt->fetch()) < 0) {
        ...
        delete EXCEPTnC;
        cleanCurrConnection();
        logging("%s, 10connection[%s:%u] closed.", ErrMsg, Current->host,
                Current->port);
        if (ntry++ == 0) goto CONNECT_AGAIN4;
        return ErrCode = PKSS_CONNECT_ERROR;
    }
...
dgt_sint32 PcaKeySvrSessionSoha::encryptCpn(dgt_sint64 enc_col_id,
                                            dgt_uint8* src, dgt_sint32 src_len,
                                            dgt_uint8* coupon,
                                            dgt_uint32* coupon_len) {
    return crypt(PCI_MSG_ENCRYPT_COUPON, enc_col_id, src, src_len, coupon,
                 coupon_len);
}

dgt_sint32 PcaKeySvrSessionSoha::decryptCpn(dgt_sint64 enc_col_id,
                                            dgt_uint8* coupon,
                                            dgt_sint32 coupon_len,
                                            dgt_uint8* dst,
                                            dgt_uint32* dst_len) {
    return crypt(PCI_MSG_DECRYPT_COUPON, enc_col_id, coupon, coupon_len, dst,
                 dst_len);
}

dgt_sint32 PcaKeySvrSessionSoha::getEncColID(const dgt_schar* name,
                                             dgt_sint64* enc_col_id) {
    GetEciBind.rewind();
    GetEciBind.next();
...
CONNECT_AGAIN5:
    ...
    if (GetEciStmt->execute(1, &GetEciBind) < 0) {
        ...
        delete EXCEPTnC;
        cleanCurrConnection();
        logging("%s, 11connection[%s:%u] closed.", ErrMsg, Current->host,
                Current->port);
...
dgt_sint32 PcaKeySvrSessionSoha::getZoneID(const dgt_schar* name,
                                           dgt_sint64* zone_id) {
...
dgt_sint32 PcaKeySvrSessionSoha::getRegEngineID(const dgt_schar* name,
...
dgt_sint32 PcaKeySvrSessionSoha::logRequest(
    pc_type_log_request_in* log_request) {
    //
...
CONNECT_AGAIN6:
    ...
    if (LogRqstStmt->execute(1, &LogRqstBind) < 0) {
        ...
        delete EXCEPTnC;
        cleanCurrConnection();
        logging("%s, 12connection[%s:%u] closed.", ErrMsg, Current->host,
                Current->port);
...
dgt_sint32 PcaKeySvrSessionSoha::encCount(dgt_sint64 enc_col_id,
    ...
    EncCountBind.rewind();
    EncCountBind.next();
    pc_type_enc_count_in* enc_count_in =
        (pc_type_enc_count_in*)EncCountBind.data();
...
dgt_sint32 PcaKeySvrSessionSoha::posting(dgt_sint64 user_sid,
                                         dgt_sint64 enc_col_id,
                                         dgt_sint32 err_code) {
    //
...
dgt_sint32 PcaKeySvrSessionSoha::getIV(dgt_uint8 iv_no, dgt_uint16 iv_size,
...
dgt_sint32 PcaKeySvrSessionSoha::putExtKey(const dgt_schar* key_name,
                                           const dgt_schar* key,
    ...
    pc_type_put_ext_key_in* key_in =
        (pc_type_put_ext_key_in*)PutExtKeyBind.data();
...
dgt_sint32 PcaKeySvrSessionSoha::getTrailer(
    dgt_sint64 key_id, pc_type_get_trailer_out* trailer_out) {
    //
    // marshall getTrailer message
    //
    GetTrailerBind.rewind();
    GetTrailerBind.next();
    pc_type_get_trailer_in* trailer_in =
        (pc_type_get_trailer_in*)GetTrailerBind.data();
...
dgt_sint32 PcaKeySvrSessionSoha::logFileRequest(
    pc_type_file_request_in* log_request) {
    //
    // soha connection logging mode
    //
    FileLogRqstBind.rewind();
    FileLogRqstBind.next();
    memcpy(FileLogRqstBind.data(), log_request,
           sizeof(pc_type_file_request_in));
    dgt_sint32 ntry = 0;
CONNECT_AGAIN6:
    //
    // check connection status
    ...
    if (FileLogRqstStmt->execute(1, &FileLogRqstBind) < 0) {
        ...
        delete EXCEPTnC;
        cleanCurrConnection();
        logging("%s, file request connection[%s:%u] closed.", ErrMsg,
...
dgt_sint32 PcaKeySvrSessionSoha::logUserFileRequest(
    ...
    memcpy(UserFileLogRqstBind.data(), log_request,
           sizeof(pc_type_user_file_request_in));
    dgt_sint32 ntry = 0;
CONNECT_AGAIN6:
    //
    // check connection status
...
dgt_sint32 PcaKeySvrSessionSoha::getZoneParam(dgt_sint64 zone_id,
                                              dgt_schar* param) {
    //
...
dgt_sint32 PcaKeySvrSessionSoha::getRegEngine(dgt_sint64 reg_engine_id,
                                              dgt_schar* param) {
    //
...
#endif
        if (result_len > 2048) {
            sprintf(ErrMsg, "big parameter than 2048byte [parameter size : %d]",
                    result_len);
...
dgt_sint32 PcaKeySvrSessionSoha::getCryptParam(dgt_schar* crypt_param_name,
                                               dgt_schar* param) {
    //
    ...
    dg_strncpy((dgt_schar*)GetCryptParamBind.data(), crypt_param_name,
               crypt_param_name_len > 33 ? 33 : crypt_param_name_len);
#else
    dgt_uint32 crypt_param_name_len = strlen(crypt_param_name);
    strncpy((dgt_schar*)GetCryptParamBind.data(), crypt_param_name,
            crypt_param_name_len > 33 ? 33 : crypt_param_name_len);
#endif
...
CONNECT_AGAIN8:
    ...
    if ((frows = GetCryptParamStmt->execute(1, &GetCryptParamBind)) < 0) {
        ...
        delete EXCEPTnC;
        cleanCurrConnection();
        logging("%s, 20connection[%s:%u] closed.", ErrMsg, Current->host,
                Current->port);
        if (ntry++ == 0) goto CONNECT_AGAIN8;
        return ErrCode = PKSS_CONNECT_ERROR;
    }
    if ((frows = GetCryptParamStmt->fetch()) < 0) {
        ...
        delete EXCEPTnC;
        cleanCurrConnection();
        logging("%s, 21connection[%s:%u] closed.", ErrMsg, Current->host,
                Current->port);
        if (ntry++ == 0) goto CONNECT_AGAIN8;
        return ErrCode = PKSS_CONNECT_ERROR;
    }
...
dgt_sint32 PcaKeySvrSessionSoha::logDetectFileRequest(
...
CONNECT_AGAIN14:
    ...
    memcpy(DetectFileLogRqstBind.data(), log_request,
           sizeof(pc_type_detect_file_request_in));
    ...
    while (log_data->next()) {
        memset(DetectFileLogDataBind.data(), 0,
               DetectFileLogDataBind.rowSize());
        memcpy(DetectFileLogDataBind.getColPtr(1), &log_request->job_id,
               sizeof(dgt_sint64));
        memcpy(DetectFileLogDataBind.getColPtr(2), &log_request->dir_id,
               sizeof(dgt_sint64));
        memcpy(DetectFileLogDataBind.getColPtr(3), &log_request->file_id,
               sizeof(dgt_sint64));
        memcpy(DetectFileLogDataBind.getColPtr(4), &log_request->file_name,
               strlen(log_request->file_name));
        memcpy(DetectFileLogDataBind.getColPtr(5), log_data->data(),
               log_data->rowSize());
...
dgt_sint32 PcaKeySvrSessionSoha::getRsaKey(dgt_schar* key_name,
    ...
    dg_strncpy((dgt_schar*)GetRsaKeyBind.data(), key_name,
               key_name_len > 33 ? 33 : key_name_len);
#else
    dgt_uint32 key_name_len = strlen(key_name);
    strncpy((dgt_schar*)GetRsaKeyBind.data(), key_name,
            key_name_len > 33 ? 33 : key_name_len);
#endif
...
CONNECT_AGAIN8:
    ...
    if ((frows = GetRsaKeyStmt->execute(1, &GetRsaKeyBind)) < 0) {
        ...
        delete EXCEPTnC;
        cleanCurrConnection();
        logging("%s, 22connection[%s:%u] closed.", ErrMsg, Current->host,
                Current->port);
        if (ntry++ == 0) goto CONNECT_AGAIN8;
        return ErrCode = PKSS_CONNECT_ERROR;
    }
    if ((frows = GetRsaKeyStmt->fetch()) < 0) {
        ...
        delete EXCEPTnC;
        cleanCurrConnection();
        logging("%s, 23connection[%s:%u] closed.", ErrMsg, Current->host,
                Current->port);
        if (ntry++ == 0) goto CONNECT_AGAIN8;
        return ErrCode = PKSS_CONNECT_ERROR;
    }
