...
class PcaKeySvrSessionSoha : public PcaKeySvrSession {
   private:
    ...
    DgcMemRows
        GetVKeyDbPrivBind;  // bind rows for getting privilege with virtual key
    DgcMemRows GetVKeyFilePrivBind;  // bind rows for getting privilege with
                                     // virtual key
    DgcMemRows GetZoneParamBind;  // bind rows for getting enc_zone_param with
                                  // zone_name
    DgcMemRows
        GetRegEngineBind;  // bind rows for getting regular engine with reg_name
    DgcMemRows GetCryptParamBind;  // bind rows for getting crypt parameter with
                                   // crypt_param_name
    ...
    DgcMemRows
        GetRsaKeyBind;  // bind rows for getting rsa key with rsa_key_name
    ...
    DgcCliStmt*
        GetVKeyDbPrivStmt;  // statement for getting privilege with virtual key
    DgcCliStmt* GetVKeyFilePrivStmt;  // statement for getting privilege with
                                      // virtual key
    DgcCliStmt* GetZoneParamStmt;  // statement for getting enc_zone_param with
                                   // zone_name
    DgcCliStmt*
        GetRegEngineStmt;  // statement for getting regular engine with reg_name
    DgcCliStmt* GetCryptParamStmt;      // statement for getting crypt parameter
                                        // with crypt_param_name
    ...
    DgcCliStmt*
        GetRsaKeyStmt;  // statement for getting rsa key with rsa_key_name
    ...
    inline dgt_void cleanCurrConnection() {
        ...
        if (GetZoneIdStmt) {
            delete GetZoneIdStmt;
            GetZoneIdStmt = 0;
        }
        if (GetRegEngineIdStmt) {
            delete GetRegEngineIdStmt;
        ...
        if (PutExtKeyStmt) {
            delete PutExtKeyStmt;
            PutExtKeyStmt = 0;
        }
        if (GetTrailerStmt) {
            delete GetTrailerStmt;
            GetTrailerStmt = 0;
        }
        if (FileLogRqstStmt) {
            delete FileLogRqstStmt;
            FileLogRqstStmt = 0;
        }
        if (UserFileLogRqstStmt) {
            delete UserFileLogRqstStmt;
            UserFileLogRqstStmt = 0;
        }
        if (GetVKeyDbPrivStmt) {
            delete GetVKeyDbPrivStmt;
            GetVKeyDbPrivStmt = 0;
        }
        if (GetVKeyFilePrivStmt) {
            delete GetVKeyFilePrivStmt;
            GetVKeyFilePrivStmt = 0;
        }
        if (GetZoneParamStmt) {
            delete GetZoneParamStmt;
            GetZoneParamStmt = 0;
        }
        if (GetRegEngineStmt) {
            delete GetRegEngineStmt;
            GetRegEngineStmt = 0;
        }
        if (GetCryptParamStmt) {
            delete GetCryptParamStmt;
            GetCryptParamStmt = 0;
        }
        if (DetectFileLogDataStmt) {
            delete DetectFileLogDataStmt;
            DetectFileLogDataStmt = 0;
        }
        if (DetectFileLogRqstStmt) {
            delete DetectFileLogRqstStmt;
            DetectFileLogRqstStmt = 0;
        }
        if (DetectFileGetRqstStmt) {
            delete DetectFileGetRqstStmt;
            DetectFileGetRqstStmt = 0;
        }
        if (GetRsaKeyStmt) {
            delete GetRsaKeyStmt;
            GetRsaKeyStmt = 0;
        }
        if (CurrConn) {
            delete CurrConn;
    ...
    dgt_sint32 crypt(dgt_sint32 msg_type, dgt_sint64 enc_col_id, dgt_uint8* src,
                     dgt_sint32 src_len, dgt_uint8* dst, dgt_uint32* dst_len);
   ...
   public:
    ...
    virtual dgt_sint32 openSession(
        dgt_sint32 db_sid, const dgt_schar* instance_name,
        const dgt_schar* db_name, const dgt_schar* ip, const dgt_schar* db_user,
        const dgt_schar* os_user, const dgt_schar* program, dgt_uint8 protocol,
        const dgt_schar* user_id, const dgt_schar* mac,
        pc_type_open_sess_out* sess_out);

    virtual dgt_sint32 getPriv(dgt_sint64 user_sid, dgt_sint64 enc_col_id,
    ...
    virtual dgt_sint32 getVKeyPriv(
        ...
        dgt_schar* name1 = 0, dgt_schar* name2 = 0, dgt_schar* name3 = 0,
        dgt_schar* name4 = 0, dgt_schar* name5 = 0, dgt_schar* name6 = 0,
        dgt_schar* name7 = 0, dgt_schar* name8 = 0, dgt_schar* name9 = 0,
        dgt_schar* name10 = 0);
    ...
    virtual dgt_sint32 getKey(dgt_sint64 key_id, pc_type_get_key_out* key_out);

    virtual dgt_sint32 alert(pc_type_alert_in* alert_request);

    virtual dgt_sint32 approve(dgt_sint64 user_sid, dgt_sint64 enc_col_id,
                               dgt_sint64 approve_id, dgt_sint32* result);

    virtual dgt_sint32 encrypt(dgt_sint64 enc_col_id, dgt_uint8* src,
                               dgt_sint32 src_len, dgt_uint8* dst,
                               dgt_uint32* dst_len);

    virtual dgt_sint32 decrypt(dgt_sint64 enc_col_id, dgt_uint8* src,
                               dgt_sint32 src_len, dgt_uint8* dst,
                               dgt_uint32* dst_len);

    virtual dgt_sint32 encrypt(dgt_sint64 enc_col_id, dgt_uint8* src,
                               dgt_sint32 src_len, dgt_uint8* dst,
                               dgt_uint32* dst_len, dgt_sint32 ksv_num);

    virtual dgt_sint32 decrypt(dgt_sint64 enc_col_id, dgt_uint8* src,
                               dgt_sint32 src_len, dgt_uint8* dst,
                               dgt_uint32* dst_len, dgt_sint32 ksv_num);

    virtual dgt_sint32 encryptCpn(dgt_sint64 enc_col_id, dgt_uint8* src,
                                  dgt_sint32 src_len, dgt_uint8* coupon,
                                  dgt_uint32* coupon_len);

    virtual dgt_sint32 decryptCpn(dgt_sint64 enc_col_id, dgt_uint8* coupon,
                                  dgt_sint32 coupon_len, dgt_uint8* dst,
                                  dgt_uint32* dst_len);

    virtual dgt_sint32 getEncColID(const dgt_schar* name,
                                   dgt_sint64* enc_col_id);

    virtual dgt_sint32 getZoneID(const dgt_schar* name, dgt_sint64* zone_id);

    virtual dgt_sint32 getRegEngineID(const dgt_schar* name,
                                      dgt_sint64* reg_engine_id);

    virtual dgt_sint32 encCount(dgt_sint64 enc_col_id, dgt_sint64 enc_count);

    virtual dgt_sint32 posting(dgt_sint64 user_sid, dgt_sint64 enc_col_id,
                               dgt_sint32 err_code);

    virtual dgt_sint32 getIV(dgt_uint8 iv_no, dgt_uint16 iv_size,
                             pc_type_get_iv_out* iv_out);

    virtual dgt_sint32 logRequest(pc_type_log_request_in* log_request);
    virtual dgt_sint32 putExtKey(const dgt_schar* key_name,
                                 const dgt_schar* key, dgt_uint16 foramt_no);
    virtual dgt_sint32 getTrailer(dgt_sint64 key_id,
                                  pc_type_get_trailer_out* trailer_out);

    virtual dgt_sint32 logFileRequest(pc_type_file_request_in* log_request);
    virtual dgt_sint32 logUserFileRequest(
    ...
    virtual dgt_sint32 getCryptParam(dgt_schar* crypt_param_name,
                                     dgt_schar* param);
    virtual dgt_sint32 logDetectFileRequest(
    ...
    virtual dgt_sint32 getRsaKey(dgt_schar* key_param_name,
