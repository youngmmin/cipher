  ...
  public:
	...
	static inline dgt_void logging(const char *fmt, va_list ap)
	{
		...
		fprintf(fp,"\n[%d.%02d.%02d.%02d:%02d:%02d]:",
			cl->tm_year+1900, cl->tm_mon+1, cl->tm_mday, cl->tm_hour, cl->tm_min, cl->tm_sec);
#else
		dg_fprint(fp,"\n[%d.%02d.%02d.%02d:%02d:%02d]:",
			cl->tm_year+1900, cl->tm_mon+1, cl->tm_mday, cl->tm_hour, cl->tm_min, cl->tm_sec);
#endif
	...
	static inline dgt_void tprintf(dgt_uint8 log_level,DgcExcept& except,const dgt_schar* fmt,...)
        {
                va_list ap;
                va_start(ap,fmt);
                if (fmt == NULL) return;
	...
	static inline dgt_void logging(dgt_sint32 err_code,const dgt_schar* log_msg)
	{
		...
		if (err_code && err_code == LastErrCode && (ct - LastLogTime) < SameErrNoLogInterval) {
			LastLogTime = ct;
			return;
		}
		LastLogTime = ct;
	...
	static inline dgt_void initialize(const dgt_schar* log_file_path, dgt_uint32 no_log_interval)
	{
		if (log_file_path && *log_file_path) strncpy(LogFilePath, log_file_path, 255);
