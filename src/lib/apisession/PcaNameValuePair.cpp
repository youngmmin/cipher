...
dgt_sint32 PcaNameValuePair::parse()
{
	...
	for(dgt_sint32 line_no=1; line; line_no++) {
		...
		if (*line != '#') {
			...
			*op=0;
			name=line;
			for(dgt_sint32 i=0; *(name+i); i++) if (*(name+i) == ' ' || *(name+i) == '\t') { *(name+i)=0; break; } // trim tailing white spaces
			value = op + 1;
			for(dgt_sint32 i=0; *value && (*value == ' ' || *value == '\t'); i++) value++;
			for(dgt_sint32 i=strlen(value)-1; i >= 0; i--) if (*(value+i) == ' ' || *(value+i) == '\t') *(value+i)=0; else break;
			addNode(name,value);
		}
#ifdef WIN32
		line=strtok_s(0,"\f\n",&last);
...
dgt_sint32 PcaNameValuePair::parseFromFile(dgt_schar* file_path)
{
	...
	if ((fd=open(file_path,O_RDONLY)) < 0) {
		sprintf(ErrMsg,"open[%s] failed, os_error[%d:%s]",file_path,errno,strerror(errno));
		return PNVP_FILE_ERROR;
	}
	struct stat	fst;
        if (fstat(fd,&fst) < 0) {
		sprintf(ErrMsg,"fstat[%s] failed, os_error[%d:%s]",file_path,errno,strerror(errno));
		close(fd);
		return PNVP_FILE_ERROR;
	}
	delete Text;
	...
	if ((rbytes=read(fd, Text, fst.st_size)) < 0) {
		sprintf(ErrMsg,"read[%s] failed, os_error[%d:%s]",file_path,errno,strerror(errno));
		close(fd);
		return PNVP_FILE_ERROR;
	}
	close(fd);
...
dgt_schar* PcaNameValuePair::getValue(const dgt_schar* name,dgt_sint8 is_case_sensitive)
{
