...
class PcaNameValuePair {
  private:
	...
	class PcaNameValueNode {
	  ...
	  public:
		PcaNameValueNode(dgt_schar* name, dgt_schar* value) : Name(name), Value(value), Next(0) {};
  ...
  public:
	...
	inline dgt_schar* next() { if (Cursor) Cursor=Cursor->next(); if (Cursor) return Cursor->name(); return 0; };
	inline dgt_schar* name() { if (Cursor) return Cursor->name(); return 0; };
	inline dgt_schar* value() { if (Cursor) return Cursor->value(); return 0; };
