...
PccRegExprSearchEngine* PcaRegEnginePool::getRegEngine(dgt_sint64 reg_engine_id)
{
	if (isInitialize == 0) {
		//create decrypt regular engine
		dgt_sint64 key_id = 19901130;//decrypt engine id - this number.. is birthday of shson!
		PccRegExprSearchEngine* SearchEngine=new PccRegExprSearchEngine(0,0);
		if (SearchEngine->addRegExpr(1,PPE_TAG_REG_EXPR,ErrMsg) < 0) {
			sprintf(ErrMsg, "decrypt_regular addRegExpr Failed[%s]", PPE_TAG_REG_EXPR);
			return 0;
		}
		PccHashNode* hnp = RegEngineList.addNode(key_id,SearchEngine);
		if (hnp) isInitialize = 1; 
		else {
...
PccRegExprSearchEngine* PcaRegEnginePool::putRegEngine(dgt_sint64 reg_engine_id, dgt_schar* param)
{
	memset(ErrMsg, 0, sizeof(ErrMsg));
	dgt_sint64 key_id = reg_engine_id;
	DgcBgrammer bg;
	...
	for (dgt_sint32 i = 0; i < NumRegExpr; i++) {
		if (SearchEngine->addRegExpr(RegExprs[i].col_no, RegExprs[i].expr_string, ErrMsg) < 0) {
			delete SearchEngine;
			return 0;
		}
	}
