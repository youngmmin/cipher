...
dgt_schar* PcaRsaKeyPool::getRsaKey(dgt_schar* key_name)
{
	...
	dgt_sint64 key_id = 0;
#ifndef WIN32
	key_id = DgcCRC64::calCRC64Case(key_id, (dgt_uint8*)key_name, dg_strlen(key_name));
#else
	key_id = DgcCRC64::calCRC64Case(key_id, (dgt_uint8*)key_name, strlen(key_name));
#endif
	// find enc zone
...
dgt_schar* PcaRsaKeyPool::putRsaKey(dgt_schar* key_name, dgt_schar* key_string)
{
	memset(ErrMsg, 0, sizeof(ErrMsg));
	// get crc value
	dgt_sint64 key_id = 0;
