...
class PcaSession : public PcaHashNode {
  private:
	...
	dgt_sint32		ApiSid;				//added by shson for use api session with SID of PcaSession
	...
	// this problem happens when the current key server is not the owner of current UserSID.
	// in order to solve this probem, session info must be kept so as to open a new session
	// if the current key server has no session by UserSID.
	// 
...
// added by chchung 2017.5.31 for stand alone session
	...
	inline dgt_void setNewSqlFlag()
	{
		...
		if (CurrRequest.start_date == 0) {
			...
			//
			// caculate the time interval between the last request and the current request
			// so that the current crypt request is considered to come from a new sql
			// if the interval is greater than the specified threshold time in micro-seconds.
			//
			struct timeval	ct;
			gettimeofday(&ct,0);
			if (ct.tv_sec == LastCallTime.tv_sec) interval = ct.tv_usec - LastCallTime.tv_usec;
			else interval = (dgt_sint64)(ct.tv_sec - LastCallTime.tv_sec)*1000000 + ct.tv_usec - LastCallTime.tv_usec;
...
//if (PcaKeySvrSessionPool::traceLevel() > 5) PcaKeySvrSessionPool::logging("SID[%d]:: NewSqlFlag => [%d] itvl[%lld]", SID, NewSqlFlag, interval);
	};

	inline dgt_void setRequest()
	{
		if (KeySvrSessionPool->apiMode() == 0) {
			CurrRequest.start_date = dgtime(&CurrRequest.start_date); // non zero start_date means the current request is a logging target
		} else {
...
#else
	inline dgt_sint32 convCharSet(dgt_schar* src, dgt_sint32 src_len, PcaPrivilege* priv,dgt_uint8 reverse_flag=0)
	{
#if 1 // added by chchung 2013.9.13 for adding test mode, where character set conversion should not be done
	if (KeySvrSessionPool->opMode()) return 0;
#endif
		dgt_sint32	rtn = 0;
		if ((priv->colType() == PCI_SRC_TYPE_CHAR || priv->colType() == PCI_SRC_TYPE_VARCHAR) && priv->multiByteFlag() > 0 && *SessCharSet) {
			dgt_schar*	col_char_set = priv->charSet(&ErrCode);
			if (col_char_set == 0) {
				PcaKeySvrSessionPool::logging(ErrCode, "getKey in charSet failed");
				return -1;
			}
			if (col_char_set && *col_char_set && strncasecmp(col_char_set,SessCharSet,32)) {
				if (CharSetCnvt) {
					if (!CharSetCnvt->isConvertable(col_char_set,SessCharSet)) CharSetCnvt->setCharSet(col_char_set,SessCharSet);
				} else {
					CharSetCnvt = new PcaCharSetCnvt(col_char_set,SessCharSet);
				}
				if (ConvBufferLength < src_len*4) {
					delete ConvBuffer;
					ConvBuffer = new dgt_uint8[ConvBufferLength=(src_len*4)];
				}
				if ((rtn=CharSetCnvt->convert(src,src_len,(dgt_schar*)ConvBuffer,ConvBufferLength,reverse_flag)) < 0) {
					ErrCode = PS_ERR_CHARSET_CONV_FAILURE;
				}
if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("SID[%d]:: charset convert[%s:%s:%d] => result[%d->%d]",SID,col_char_set,SessCharSet,reverse_flag,src_len,rtn);
			}
		}
		return rtn;
	};
  ...
  protected:
  public:
	PcaSession(dgt_sint32 sid,PcaKeySvrSessionPool* key_svr_session_pool, PcaNamePool* name_pool, PcaKeyPool* key_pool, PcaIVPool* iv_pool, PcaPrivHolder* priv_holder=0)
#ifdef WIN32
		: SID(sid),
		  UserSID(0),
	...
	virtual ~PcaSession()
	{
		...
		delete InBuffer;
		delete OutBuffer;
		if (KeySvrSessionPool->apiMode() == 0 || KeySvrSessionPool->apiMode() == 2) logCurrRequest();
		reset();
#else
                delete CharSetCnvt;
                delete ConvBuffer;
	...
	{
		if (char_set) strncpy(SessCharSet, char_set, 32);
		else strncpy(SessCharSet, KeySvrSessionPool->charSet(), 32);
	};
...
if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("SID[%d}:: InBuffer[%d] replaced", SID, InBufferLength);
		}
		return InBuffer;
	};
	...
	inline dgt_sint32 openSession(
		dgt_sint32		db_sid=0,
		const dgt_schar*	instance_name=0,
		const dgt_schar*	db_name=0,
		const dgt_schar*	ip=0,
		const dgt_schar*	db_user=0,
		const dgt_schar*	os_user=0,
		const dgt_schar*	program=0,
		dgt_uint8		protocol=0,
		const dgt_schar*	user_id=0,
		const dgt_schar*	mac = 0)
	{
		ErrCode = 0;
	...
	if (KeySvrSessionPool->opMode() >= PCI_OP_NO_PULL_NO_PUSH) return SID;
		...
		if (PrivHolder) {
			...
			if(!strcasecmp(SessCharSet,"UTF-8")) multi_byte_size=3;
			else multi_byte_size=2;
			for(dgt_sint32 idx=0; idx<PrivHolder->numPrivs(); idx++) {
				PcaPrivilege*	rtn;
				PrivilegePool.putPriv(new PcaPrivilege(PrivHolder->encColID(idx),PrivHolder->privInfo(idx),KeyPool,IVPool,multi_byte_size),&rtn);
			}
			return SID;
		}
#endif
		...
		if (!(ErrCode=KeySvrSessionPool->getSession(&svr_session))) {
			if (!db_sid && (!instance_name || *instance_name == 0) && (!db_name || *db_name == 0) && (!ip || *ip == 0) &&
			    (!db_user || *db_user == 0) && (!os_user || *os_user == 0) && (!program || *program == 0)  && !protocol &&
				...
				pid_t pid=getpid();
				if (pstat_getproc(&psinfo, sizeof(struct pst_status), 0, pid) < 0) {
					program = KeySvrSessionPool->sharedSessionProgram();
				} else {
	...
	#elif WIN32
				DWORD dwFileNameSize=GetModuleFileName(NULL, (LPWCH)cmdline, sizeof(cmdline));
				...
				os_user=os_user_tmp;
#endif
if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("SID[%d]:: open shared session => [%s-%s%s]", SID, ip, user_id, program);
			}
			...
			if(db_user && strlen(db_user) > 1 && (db_user[0] == '-')) {
				...
				os_user=os_user_tmp;
#endif
if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("SID[%d]:: open file agent session => [%s-%s-%s-%s]", SID, ip, os_user, program,db_user);
			}
			...
			if (!program || *program == 0) program = KeySvrSessionPool->sharedSessionProgram();
			...
			if ((val=svr_session->credentials().instanceName()) && *val) instance_name = val;
			if ((val=svr_session->credentials().dbName()) && *val) db_name = val;
			if ((val=svr_session->credentials().dbUser()) && *val) db_user = val;
			if ((val=svr_session->credentials().osUser()) && *val) os_user = val;
			if ((val=svr_session->credentials().program()) && *val) program = val;
			if ((val=svr_session->credentials().orgUserID()) && *val) user_id = val;
			...
			if ((ip == 0 || *ip == 0 || strncmp(ip, "127.0.0.1", 9) == 0) && *KeySvrSessionPool->ipProbCmdPath()) {
				//
				// beq session, need to obtain login IP
				//
				plip.getLoginIp(KeySvrSessionPool->ipProbCmdPath(),KeySvrSessionPool->ipProbCmdArgs(),KeySvrSessionPool->ipProbPos(),client_ip);
			} else if (ip) {
				strncpy(client_ip, ip, 64);
			}
if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("SID[%d]:: get IP => [%s]", SID, client_ip);

			if ((mac == 0 || *mac == 0 || strlen(mac) == 0) && *KeySvrSessionPool->macProbCmdPath()) {
				//
				// get client mac
				//
				plmac.getMac4Ip(KeySvrSessionPool->macProbCmdPath(),
						KeySvrSessionPool->macProbCmdArgs(),
						KeySvrSessionPool->macProbIpPos(),
						client_ip,
						KeySvrSessionPool->macProbPos(),
						client_mac);
if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("SID[%d]:: get mac => [%s][%s]", SID, client_ip, client_mac);
			} else if (mac) {
				strncpy(client_mac, mac, 64);
			}
#endif
			if (instance_name == 0 || *instance_name == 0) instance_name = KeySvrSessionPool->instanceName();
			if (db_name == 0 || *db_name == 0) db_name = KeySvrSessionPool->dbName();

			pc_type_open_sess_out	sess_out;
			memset(&sess_out,0,sizeof(pc_type_open_sess_out));
			if ((ErrCode=svr_session->openSession(db_sid, instance_name, db_name, client_ip, db_user, os_user, program, protocol, user_id, client_mac, &sess_out))) {
				PcaKeySvrSessionPool::logging("client[%s] open session failed:%d:%s.", client_ip, ErrCode, svr_session->errMsg());
			}
			UserSID=sess_out.user_sid;
			AuthFailCode=sess_out.auth_fail_code;
if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("SID[%d]:: openSession => UserSID[%lld]AuthFailCode[%d]", SID, UserSID,AuthFailCode);
			KeySvrSessionPool->returnSession(svr_session);

			//
			// added by chchung, 2012.10.11
			//
			if (*SessInfo.client_ip == 0 && *SessInfo.user_id == 0 && *SessInfo.client_program == 0 && *SessInfo.db_user == 0) {
				SessInfo.db_sid = db_sid;
				if (instance_name) strncpy(SessInfo.instance_name, instance_name, 32);
	...
	inline dgt_uint8 isEncryptAudit(const dgt_schar* enc_col_name)
		...
		ErrCode = 0;
                ...
                if (enc_col_id < 0) {
                        ...
                        sprintf(err_msg, "getID[%s] failed", enc_col_name ? enc_col_name:"");
	...
	if (KeySvrSessionPool->opMode() >= PCI_OP_NO_PULL_NO_PUSH) {
		...
		if (priv == 0) priv = getPrivilege(enc_col_id);
                if (priv && AuthFailCode && priv->authFailEncPriv() > PCI_ENCRYPT_PRIV) {
			//
			...
			//
			if (KeySvrSessionPool->newSqlInterval() == 0 && (priv->encAuditFlag() || priv->decAuditFlag())) {
		...
		if (priv) {
			...
			// if AuditFlag > 0 the setting the NewSqlInterval = 200000
			//
			if (KeySvrSessionPool->newSqlInterval() == 0 && KeySvrSessionPool->apiMode() == 0 && (priv->encAuditFlag() || priv->decAuditFlag())) {
				KeySvrSessionPool->setNewSqlInterval(200000);
			}
			if (priv->getCryptPriv(PCI_CRYPT_OP_ENCRYPT,sql_type) <= PCI_ENCRYPT_PRIV) { // encryption privilege check
				//
				...
				if (rtn >= 0) {
					...
					if (KeySvrSessionPool->opMode() || priv->opMode()) {
						...
						if (KeySvrSessionPool->isEncryptLocal()) {
							// delegate decision of enforcing the same load to PcaPrivilege
							// because PcaPrivilege has key fetching routine.
							priv->encrypt(src,src_len,dst,&tmp_dst_len,(strncmp(header_flag,"on",2) == 0) ? 0 : KeySvrSessionPool->doubleEncCheck(),0,set_key,set_iv,pad_type);
						} else {
							if (svr_session || KeySvrSessionPool->getSession(&svr_session) == 0) {
								if (KeySvrSessionPool->opMode() == PCI_OP_NO_CRYPT_EQUAL_LOAD ||
								    (KeySvrSessionPool->opMode() == 0 && priv->opMode())) {
									// the same load
									if ((ErrCode=svr_session->encrypt(enc_col_id,src,src_len,dst,&tmp_dst_len))) {
						...
						if (KeySvrSessionPool->isEncryptLocal()) {
							ErrCode = priv->encrypt(src,src_len,dst,dst_len,(strncmp(header_flag,"on",2) == 0) ? 0 : KeySvrSessionPool->doubleEncCheck(),0,set_key,set_iv,pad_type);
						} else {
							if (svr_session || KeySvrSessionPool->getSession(&svr_session) == 0) {
								if ((ErrCode=svr_session->encrypt(enc_col_id,src,src_len,dst,dst_len))) {
									PcaKeySvrSessionPool::logging(ErrCode, svr_session->errMsg());
								}
								if (svr_session) KeySvrSessionPool->returnSession(svr_session);
							}
...
#else
					if (KeySvrSessionPool->isEncryptLocal()) {
						ErrCode = priv->encrypt(src,src_len,dst,dst_len,KeySvrSessionPool->doubleEncCheck(),0,set_key,set_iv);
					} else {
						if (svr_session || KeySvrSessionPool->getSession(&svr_session) == 0) {
							if ((ErrCode=svr_session->encrypt(enc_col_id,src,src_len,dst,dst_len))) {
								PcaKeySvrSessionPool::logging(ErrCode, svr_session->errMsg());
				...
				if (priv->encCount() == 1) {
					...
					//
					// in case of no privilege & first try, need to check alerting rule
					//
				 	if (priv->isEncryptAlert()) {
						if (svr_session || KeySvrSessionPool->getSession(&svr_session) == 0) {
							setAlertRequest(enc_col_id,0,0,PS_NO_PRIV_ENCRYPT_ALERT);
							if (svr_session->alert(&AlertRequest)) {
								PcaKeySvrSessionPool::logging(ErrCode, svr_session->errMsg());
							}
							if (svr_session) KeySvrSessionPool->returnSession(svr_session);
if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("SID[%d]:: no priv encrypt alert => [%d]", SID, ErrCode);
						}
					}
				}
			}
		...
		if (priv) {
			...
			dgt_sint32      rtn = 0;
#else
			dgt_sint32      rtn = convCharSet((dgt_schar*)dst, (dgt_sint32)*dst_len, priv,1);
#endif
			if (rtn > 0) {
...
#if 1 // modified by chchung 2017.6.20 for allowing optional decrypting error handling
                if (ErrCode && KeySvrSessionPool->decryptFailSrcRtn() && (strncmp(header_flag,"on",2) != 0)) {
...
#endif
		PcaPrivilege* priv = getPrivilege(enc_col_id); // get a for "enc_col_id"
		if (priv) {
			*dst_len=4000;
			if (OutBufferLength == 0) {
				delete OutBuffer;
			...
			if (src_len > 2000) {
				if (priv->ophFlag()) {
					*dst_len = 4*(dgt_sint32)ceil((double)(src_len+64)/3) + 32 + priv->ophLength();
				} else {
...
if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("SID[%d]:: OutBuffer[%d] replaced", SID, OutBufferLength);
				}
			}
			*dst = OutBuffer;
			encrypt(enc_col_id, src, src_len, OutBuffer, dst_len, sql_type, priv);
		}
		return ErrCode == 0 ? NewSqlFlag : ErrCode;
	};
	...
	if (KeySvrSessionPool->opMode() >= PCI_OP_NO_PULL_NO_PUSH) {
		...
		if (enc_col_id < 0) {
			...
			encrypt(enc_col_id,src,src_len,dst,dst_len,sql_type,0,set_key,set_iv,(dgt_schar*)"off",pad_type);
		}
		return ErrCode == 0 ? NewSqlFlag : ErrCode;
	};
...
if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("SID[%d]:: OutBuffer[%d] replaced", SID, OutBufferLength);
				...
				*dst = OutBuffer;
				encrypt(enc_col_id, src, src_len, OutBuffer, dst_len, sql_type, priv, set_key, set_iv, (dgt_schar*)"off",pad_type);
			}
		}
		return ErrCode == 0 ? NewSqlFlag : ErrCode;
	};
	...
	if (KeySvrSessionPool->opMode() >= PCI_OP_NO_PULL_NO_PUSH) {
		...
		if (enc_col_id < 0) {
			...
			encrypt(enc_col_id,src,src_len,dst,dst_len,0,0,0,0,header_flag);
		}
		return ErrCode == 0 ? NewSqlFlag : ErrCode;
	}
		...
		dgt_sint32	ksv_num)
	{
		...
		dgt_schar	enc[10000]={0,};		dgt_uint32	enc_len=10000;
		dgt_schar	dec[10000]={0,};		dgt_uint32	dec_len=10000;
		...
		if (rtn >= 0) {
			...
			if (KeySvrSessionPool->getSession(&svr_session) != 0) return 2;	//key server not found.
			
			//encrypt
			if ((ErrCode=svr_session->encrypt(enc_col_id,src,src_len,(dgt_uint8*)enc,&enc_len,ksv_num))) {
				PcaKeySvrSessionPool::logging(ErrCode, svr_session->errMsg());
				if (svr_session) KeySvrSessionPool->returnSession(svr_session);
				return ErrCode;
			}
			//decrypt
			if ((ErrCode=svr_session->decrypt(enc_col_id,(dgt_uint8*)enc,enc_len,(dgt_uint8*)dec,&dec_len,ksv_num))) {
				PcaKeySvrSessionPool::logging(ErrCode, svr_session->errMsg());
				if (svr_session) KeySvrSessionPool->returnSession(svr_session);
				return ErrCode;
			}
			//diff
			if (svr_session) KeySvrSessionPool->returnSession(svr_session);
			if(strcmp((const dgt_schar*)src, (const dgt_schar*)dec)) return 3;	//src & dec are different
		} 
		return 0;	//success
	};
	...
	if (KeySvrSessionPool->opMode() >= PCI_OP_NO_PULL_NO_PUSH) {
		...
		if (priv) {
			...
			dgt_sint32      rtn = 0;
                ...
                if (priv && AuthFailCode && priv->authFailDecPriv() > PCI_DEC_PRIV_DEC) {
                        //
                        // Authentaication fail
                        //
                        //
			...
			if (priv->authFailDecPriv()==PCI_DEC_PRIV_MASK) {
                                ...
                                // masking or
                                // decrypt allowed,
                                // which session has decryption privilege & no approve rule or approved as "PCI_DEC_PRIV_DEC".
                                // the decLogPrivFlag() returns "PCI_DEC_PRIV_DEC" if there's no settig,
                                // because decLogPrivFlag was initialized as "PCI_DEC_PRIV_DEC"
                                //
				...
				if (KeySvrSessionPool->opMode() || priv->opMode()) {
						...
						if (svr_session || KeySvrSessionPool->getSession(&svr_session) == 0) {
							if (KeySvrSessionPool->opMode() == PCI_OP_NO_CRYPT_EQUAL_LOAD ||
							    (KeySvrSessionPool->opMode() == 0 && priv->opMode())) {
								// the same load
								if ((ErrCode=svr_session->encrypt(enc_col_id,src,src_len,dst,&dst_buf_len))) {
									PcaKeySvrSessionPool::logging(ErrCode, svr_session->errMsg());
								}
							}
							if (svr_session) KeySvrSessionPool->returnSession(svr_session);
					...
					return ErrCode = 0;
                                	...
                                	if (KeySvrSessionPool->isDecryptLocal()) {
						ErrCode = priv->decrypt(src,src_len,dst,dst_len,set_key,set_iv,(strncmp(header_flag,"on",2) == 0) ? 0 : KeySvrSessionPool->decryptFailSrcRtn(),pad_type);
					} else {
						if (svr_session || KeySvrSessionPool->getSession(&svr_session) == 0) {
							if ((ErrCode = svr_session->decrypt(enc_col_id,src,src_len,dst,dst_len))) {
								PcaKeySvrSessionPool::logging(ErrCode, svr_session->errMsg());
							}
							if (svr_session) KeySvrSessionPool->returnSession(svr_session);
						}
...
#else
                                if (KeySvrSessionPool->isDecryptLocal()) {
                                        ErrCode = priv->decrypt(src,src_len,dst,dst_len,set_key,set_iv,KeySvrSessionPool->decryptFailSrcRtn());
                                } else {
                                        if (svr_session || KeySvrSessionPool->getSession(&svr_session) == 0) {
                                                if ((ErrCode = svr_session->decrypt(enc_col_id,src,src_len,dst,dst_len))) {
                                                        PcaKeySvrSessionPool::logging(ErrCode, svr_session->errMsg());
                        ...
                        if (ErrCode == 0) {
                                ...
                                if (priv->authFailDecPriv() > PCI_DEC_PRIV_DEC) {
                                        ...
                                        //
                                        if (priv->authFailDecPriv() == PCI_DEC_PRIV_ERR) { // error return
                                                ErrCode = PcaPrivilege::PK_NO_DECRYPT_PRIV;
                                        } else if (priv->authFailDecPriv() == PCI_DEC_PRIV_SRC) { // encrypted data return
                                                //
                                                ...
                                                //
                                                // masking in which case the "dst" data has been decrypted already.
                                                //
                                                dgt_uint32      masking_len = priv->maskingLength(*dst_len);
                                                if (priv->colType() == PCI_SRC_TYPE_NUM) {
                                                        if (masking_len > 0) {
                                                                dgt_uint8*      cp = dst + priv->encStartPos(*dst_len) - 1;
                                                                ...
                                                                if (mask_pttn_len == 0) {
                                                                        ...
                                                                        while(masking_len > 0) {
                                                                                if (masking_len <= mask_pttn_len) {
                                                                                        memcpy(cp, priv->maskChar(), masking_len);
                                                                                        masking_len = 0;
                                                                                } else {
                                                                                        memcpy(cp, priv->maskChar(), mask_pttn_len);
		...
		if (priv) {
			...
			PcaKeySvrSession*	svr_session = 0;
			dgt_uint8		dec_priv;
			if ((dec_priv=priv->getCryptPriv(PCI_CRYPT_OP_DECRYPT,sql_type)) <= PCI_DEC_PRIV_DEC) {
				//
			...
			if (priv->decLogPrivFlag() <= PCI_DEC_PRIV_DEC || priv->decLogPrivFlag() == PCI_DEC_PRIV_MASK) {
				//
				// masking or
				// decrypt allowed,
				// which session has decryption privilege & no approve rule or approved as "PCI_DEC_PRIV_DEC".
				...
				dgt_sint32 rtn=priv->isAlert();
                                if (rtn == 1) {
                                        //
                                        // decryption not allowed but too many decryption try so as to alert this event
                                        //
					AlertSqlFlag=0;
					setAlertRequest(enc_col_id,priv->alertDecCount(),2,PS_TOO_MANY_DECRYPT_ALERT);
					if (svr_session || KeySvrSessionPool->getSession(&svr_session) == 0) {
                                                if ((ErrCode=svr_session->alert(&AlertRequest))) {
                                                        PcaKeySvrSessionPool::logging(ErrCode, svr_session->errMsg());
                                                }
                                                if (svr_session) KeySvrSessionPool->returnSession(svr_session);
if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("SID[%d]:: too many[%u] not allowed decrypt alert => ErrCode[%d]", SID, priv->alertDecCount(), ErrCode);
					}
				} else if (rtn < 0) {
					AlertSqlFlag=1;
				}
			...
			if (priv->decCount() == 1 && priv->decLogPrivFlag() > PCI_DEC_PRIV_DEC && priv->isDecryptAlert()) {
				//
				// in case of first descryption try without privilege,
				// need to check whether to send no-privilege-decryption-try alert,
				// alert for a decryption try without privilege.
				//
				if (svr_session || KeySvrSessionPool->getSession(&svr_session) == 0) {
					...
					if (svr_session->alert(&AlertRequest)) {
						PcaKeySvrSessionPool::logging(ErrCode, svr_session->errMsg());
               				...
               				if (svr_session) KeySvrSessionPool->returnSession(svr_session);
if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("SID[%d]:: decrypt no priv alert[%lld] => ErrCode[%d]", SID, enc_col_id, ErrCode);
				}
			}
			if (ErrCode == 0) {
				//
				...
				if (priv->decLogPrivFlag() > PCI_DEC_PRIV_DEC) {
					...
					// no error and no privilege
					//
					if (priv->decLogPrivFlag() == PCI_DEC_PRIV_ERR) { // error return
						ErrCode = PcaPrivilege::PK_NO_DECRYPT_PRIV;
					} else if (priv->decLogPrivFlag() == PCI_DEC_PRIV_SRC) { // encrypted data return
#if 0
						if (*dst_len > (dgt_uint32)src_len) *dst_len = src_len;
						memcpy(dst, src, *dst_len);
#endif
					...
					//
					// decryption allowed but too many decryption try so as to alert this event
					//
					if (rtn < 0) {
						AlertSqlFlag=1;
					} else {
						AlertSqlFlag=0;
						setAlertRequest(enc_col_id,priv->alertDecCount(),1,PS_TOO_MANY_DECRYPT_ALERT);
						if (svr_session || KeySvrSessionPool->getSession(&svr_session) == 0) {
							if ((ErrCode=svr_session->alert(&AlertRequest))) {
								PcaKeySvrSessionPool::logging(ErrCode, svr_session->errMsg());
							}
               						if (svr_session) KeySvrSessionPool->returnSession(svr_session);
if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("SID[%d]:: too many[%u] decrypt alert => ErrCode[%d]", SID, priv->decCount(), ErrCode);
						}
					}
				}
			}
...
if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("SID[%d]:: OutBuffer[%d] replaced", SID, OutBufferLength);
			...
			*dst = OutBuffer;
			decrypt(enc_col_id, src, src_len, OutBuffer, dst_len, sql_type, priv);
		}
		return ErrCode == 0 ? NewSqlFlag : ErrCode;
	};
	...
	if (KeySvrSessionPool->opMode() >= PCI_OP_NO_PULL_NO_PUSH) {
		...
		if (enc_col_id < 0) {
			...
			decrypt(enc_col_id,src,src_len,dst,dst_len,sql_type,0,set_key,set_iv,(dgt_schar*)"off",pad_type);
		}
		return ErrCode == 0 ? NewSqlFlag : ErrCode;
	};
...
if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("SID[%d]:: OutBuffer[%d] replaced", SID, OutBufferLength);
				...
				*dst = OutBuffer;
				decrypt(enc_col_id, src, src_len, OutBuffer, dst_len, sql_type, priv, set_key, set_iv, (dgt_schar*)"off", pad_type);
			}
		}
		return ErrCode == 0 ? NewSqlFlag : ErrCode;
	};
	...
	if (KeySvrSessionPool->opMode() >= PCI_OP_NO_PULL_NO_PUSH) {
		...
		if (enc_col_id < 0) {
			...
			decrypt(enc_col_id,src,src_len,dst,dst_len,0,0,0,0,header_flag);
		}
		return ErrCode == 0 ? NewSqlFlag : ErrCode;
		return 0;
	}

	dgt_sint32 decrypt_vkey(
			dgt_sint64		virtual_key_id,
			dgt_uint8*		src,
			dgt_sint32		src_len,
			dgt_uint8*		dst,
		...
		if (priv) {
			*dst_len=4000;
			...
			if (src_len > 2000) {
				if ((*dst_len+3)*2 > OutBufferLength) {
					delete OutBuffer;
					OutBuffer = new dgt_uint8[OutBufferLength=(*dst_len+3)*2];
if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("SID[%d]:: OutBuffer[%d] replaced", SID, OutBufferLength);
				}
			}
			*dst = OutBuffer;
			OPHUEK(enc_col_id, src, src_len, OutBuffer, dst_len, src_enc_flag, priv);
		}
		return ErrCode;
	};
	...
	if (KeySvrSessionPool->opMode() >= PCI_OP_NO_PULL_NO_PUSH) {
		*dst_len = extendOutBuffer(src_len);
		*dst = OutBuffer;
		OPHUEK(enc_col_name, src, src_len, OutBuffer, dst_len, src_enc_flag);
		return 0;
	}
#endif
		dgt_sint64	enc_col_id = NamePool->getID(enc_col_name);
	...
	if (KeySvrSessionPool->opMode() >= PCI_OP_NO_PULL_NO_PUSH) {
		...
		if (priv) {
			...
			if (priv->getCryptPriv(PCI_CRYPT_OP_ENCRYPT)<=PCI_ENCRYPT_PRIV) {
				...
				dgt_uint32	enc_len = 4*(dgt_uint32)ceil((double)(src_len+64)/3) + 32;
				if (enc_len > MAX_COUPON_SRC_LEN) enc_data = new dgt_uint8[enc_len];
				else enc_len = MAX_COUPON_SRC_LEN;
				...
				if (KeySvrSessionPool->opMode() || priv->opMode()) {
					if (KeySvrSessionPool->opMode() == PCI_OP_NO_CRYPT_EQUAL_LOAD ||
						...
						if ((ErrCode=priv->encrypt(src,src_len,enc_data,&enc_len,KeySvrSessionPool->doubleEncCheck(),1)) == 0) {
							if (svr_session || KeySvrSessionPool->getSession(&svr_session) == 0) {
								if ((ErrCode=svr_session->encryptCpn(enc_col_id,enc_data,enc_len,coupon,&tmp_coupon_len))) {
									PcaKeySvrSessionPool::logging(ErrCode, svr_session->errMsg());
								}
								if (svr_session) KeySvrSessionPool->returnSession(svr_session);
							}
					...
					return ErrCode = 0;
						...
						if ((ErrCode=priv->encrypt(src,src_len,enc_data,&enc_len,KeySvrSessionPool->doubleEncCheck())) == 0) {
							if (svr_session || KeySvrSessionPool->getSession(&svr_session) == 0) {
								if ((ErrCode=svr_session->encryptCpn(enc_col_id,enc_data,enc_len,coupon,coupon_len))) {
									PcaKeySvrSessionPool::logging(ErrCode, svr_session->errMsg());
								}
								if (svr_session) KeySvrSessionPool->returnSession(svr_session);
							}
                                ...
                                if (rtn >= 0) {
                                        ...
                                        if (KeySvrSessionPool->isEncryptLocal()) {
                                                ErrCode=priv->encrypt(src,src_len,(dgt_uint8*)enc_data,&enc_len,KeySvrSessionPool->doubleEncCheck());
                                        } else {
                                                if (svr_session || KeySvrSessionPool->getSession(&svr_session) == 0) {
                                                        if ((ErrCode=svr_session->encrypt(enc_col_id,src,src_len,(dgt_uint8*)enc_data,&enc_len))) {
                                                                PcaKeySvrSessionPool::logging(ErrCode, svr_session->errMsg());
                                                        }
							if (svr_session) KeySvrSessionPool->returnSession(svr_session);
                                                }
				...
				if (svr_session || KeySvrSessionPool->getSession(&svr_session) == 0) {
					if ((ErrCode=svr_session->encryptCpn(enc_col_id,(dgt_uint8*)enc_data,enc_len,coupon,coupon_len))) {
						PcaKeySvrSessionPool::logging(ErrCode, svr_session->errMsg());
					}
					if (svr_session) KeySvrSessionPool->returnSession(svr_session);
				}
...
if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("SID[%d]:: OutBuffer[%d] replaced", SID, OutBufferLength);
	...
	if (KeySvrSessionPool->opMode() >= PCI_OP_NO_PULL_NO_PUSH) {
		...
		if (priv) {
			...
			PcaKeySvrSession*	svr_session=0;
			dgt_uint8		dec_priv;
			if ((dec_priv=priv->getCryptPriv(PCI_CRYPT_OP_DECRYPT)) <= PCI_DEC_PRIV_DEC) {
				if (priv->isMasking()) {
			...
			if (priv->decLogPrivFlag() <= PCI_DEC_PRIV_DEC || priv->decLogPrivFlag() == PCI_DEC_PRIV_MASK) {
				...
				if (KeySvrSessionPool->opMode() || priv->opMode()) {
					if (KeySvrSessionPool->opMode() == PCI_OP_NO_CRYPT_EQUAL_LOAD ||
					    (KeySvrSessionPool->opMode() == 0 && priv->opMode())) {
						// the same load
						if ((ErrCode=priv->encrypt(coupon,coupon_len,enc_data,&enc_len,KeySvrSessionPool->doubleEncCheck(),1)) == 0) {
							if (svr_session || KeySvrSessionPool->getSession(&svr_session) == 0) {
								if ((ErrCode=svr_session->encryptCpn(enc_col_id,enc_data,enc_len,dst,&dst_buf_len))) {
									PcaKeySvrSessionPool::logging(ErrCode, svr_session->errMsg());
								}
								if (svr_session) KeySvrSessionPool->returnSession(svr_session);
							}
					...
					if (svr_session || KeySvrSessionPool->getSession(&svr_session) == 0) {
						if ((ErrCode=svr_session->decryptCpn(enc_col_id,coupon,coupon_len,enc_data,&enc_len))) {
							PcaKeySvrSessionPool::logging(ErrCode, svr_session->errMsg());
						}
						if (svr_session) KeySvrSessionPool->returnSession(svr_session);
					}
                                        ErrCode = priv->decrypt((dgt_uint8*)enc_data,enc_len,dst,dst_len,0,0,KeySvrSessionPool->decryptFailSrcRtn());
				}
#else
				if (svr_session || KeySvrSessionPool->getSession(&svr_session) == 0) {
					if ((ErrCode=svr_session->decryptCpn(enc_col_id,coupon,coupon_len,(dgt_uint8*)enc_data,&enc_len))) {
						PcaKeySvrSessionPool::logging(ErrCode, svr_session->errMsg());
					}
					if (svr_session) KeySvrSessionPool->returnSession(svr_session);
				}
                                if (KeySvrSessionPool->isDecryptLocal()) {
                                        ...
                                        if (svr_session || KeySvrSessionPool->getSession(&svr_session) == 0) {
                                                if ((ErrCode = svr_session->decrypt(enc_col_id,(dgt_uint8*)enc_data,enc_len,dst,dst_len))) {
                                                        PcaKeySvrSessionPool::logging(ErrCode, svr_session->errMsg());
                                                }
						if (svr_session) KeySvrSessionPool->returnSession(svr_session);
                                        }
			...
			if (priv->decCount() == 1 && priv->decLogPrivFlag() > PCI_DEC_PRIV_DEC && priv->isDecryptAlert()) {
				if (svr_session || KeySvrSessionPool->getSession(&svr_session) == 0) {
					...
					if (svr_session) KeySvrSessionPool->returnSession(svr_session);
if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("SID[%d]:: no priv decrypt alert => ErrCode[%d]", SID, ErrCode);
				}
			}
			if (ErrCode == 0) {
				dgt_sint32 rtn=0;
...
if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("SID[%d]:: OutBuffer[%d] replaced", SID, OutBufferLength);
			...
			*dst = OutBuffer;
			decryptCpn(enc_col_id, coupon, coupon_len, OutBuffer, dst_len, priv);
		}
		return ErrCode == 0 ? NewSqlFlag : ErrCode;
	};
...
if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("SID[%d]:: OutBuffer[%d] replaced", SID, OutBufferLength);
	...
	inline dgt_void setSqlHash(dgt_schar* sql_hash,dgt_sint32 sql_type)
	{
		...
		// because the time period being spent on finding "sql hash" by the caller 
		// should be excluded to decide new sql boundary.
		//
		gettimeofday(&FirstCallTime,0);
		LastCallTime=FirstCallTime;
	};

	inline dgt_void setAlertRequest(dgt_sint64 enc_col_id, dgt_sint64 dec_count, dgt_uint8 dec_no_priv_flag, dgt_uint8 op_type) {
		if (op_type != PS_TOO_MANY_DECRYPT_ALERT) {
			memset(&AlertRequest,0,sizeof(AlertRequest));
		}
                AlertRequest.start_date = dgtime(&AlertRequest.start_date); // non zero start_date means the current request is a logging target
	...
	inline dgt_void logCurrRequest(dgt_schar* sql_hash=0,dgt_sint32 sql_type=0,dgt_schar* user_id=0)
	{
		if (KeySvrSessionPool->apiMode()) {
			...
			if (sql_hash) {
				...
				strncpy(CurrRequest.sql_hash, KeySvrSessionPool->sharedSessionProgram(), 64);
			}
			if (user_id) {
				strncpy(CurrRequest.reserved, user_id, 32);
			} else {
				strncpy(CurrRequest.reserved, KeySvrSessionPool->sharedSessionUID(), 32);
			}
			if (sql_type) CurrRequest.sql_type = sql_type;
			else CurrRequest.stmt_id = 128; // for api mode(nong hyup) , all sql logging 
...
if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("SID[%d]:: logRequest => UserSID[%lld] EncColID[%lld]", SID, CurrRequest.user_sid, CurrRequest.enc_col_id);
	                                }
				}
                                KeySvrSessionPool->returnSession(svr_session);
                        }
		...
		if (sql_hash && !strncmp(sql_hash,"reg",3)) {
			...
			if (LastCallTime.tv_sec == FirstCallTime.tv_sec) {
				CurrRequest.lapse_time = LastCallTime.tv_usec - FirstCallTime.tv_usec;
			} else {
				CurrRequest.lapse_time = (dgt_sint64)(LastCallTime.tv_sec - FirstCallTime.tv_sec)*1000000 + 
...
#endif
                        PcaKeySvrSession*       svr_session=0;
                        if (priv->getCryptPriv(PCI_CRYPT_OP_ENCRYPT) <= PCI_ENCRYPT_PRIV) { // encryption privilege check
                                //
                                // charater set conversion
                                //
#ifdef WIN32
                                ...
                                if (rtn >= 0) {
                                        ...
                                        if (KeySvrSessionPool->isEncryptLocal()) {
                                                ErrCode=priv->encrypt(src,src_len,dst,dst_len,KeySvrSessionPool->doubleEncCheck(),1);
                                        } else {
                                                if (svr_session || KeySvrSessionPool->getSession(&svr_session) == 0) {
                                                        if ((ErrCode=svr_session->encrypt(enc_col_id,src,src_len,dst,dst_len))) {
                                                                PcaKeySvrSessionPool::logging(ErrCode, svr_session->errMsg());
...
if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("SID[%d]:: no priv encrypt alert => [%d]", SID, ErrCode);
			...
			if (priv->encCount() == PCI_ENC_LOG_COUNT) { 
				if (svr_session || KeySvrSessionPool->getSession(&svr_session) == 0) {
					if ((ErrCode=svr_session->encCount(enc_col_id,PCI_ENC_LOG_COUNT))) {
						PcaKeySvrSessionPool::logging(ErrCode, svr_session->errMsg());
					}
					if (svr_session) KeySvrSessionPool->returnSession(svr_session);
				}
...
if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("SID[%d]:: OutBuffer[%d] replaced", SID, OutBufferLength);
        ...
        inline dgt_sint32 getKey(const dgt_schar* enc_col_name, dgt_uint8* key_buffer, dgt_sint32* key_size)
        {
                ErrCode = 0;
                dgt_sint64      enc_col_id = NamePool->getID(enc_col_name);
                if (enc_col_id < 0) {
	...
	inline dgt_sint32 putExtKey(const dgt_schar* key_name,const dgt_schar* key,dgt_uint16 format_no)
	{
		ErrCode = 0;
		PcaKeySvrSession*       svr_session=0;
		if (svr_session || KeySvrSessionPool->getSession(&svr_session) == 0) {
...
//added by shson 2018.6.1
	inline dgt_sint64 getZoneID(const dgt_schar* zone_name)
	{
		dgt_sint64      zone_id = NamePool->getZoneID(zone_name);
			...
			dgt_schar err_msg[256];
			memset(err_msg,0,256);
                        sprintf(err_msg, "getZoneID[%s] failed", zone_name ? zone_name:"");
	...
	inline dgt_sint64 getRegEngineID(const dgt_schar* reg_engine_name)
	{
		dgt_sint64      reg_engine_id = NamePool->getRegEngineID(reg_engine_name);
			...
			dgt_schar err_msg[256];
			memset(err_msg,0,256);
                        sprintf(err_msg, "getZoneID[%s] failed", reg_engine_name ? reg_engine_name:"");
	...
	inline dgt_sint32 logFileRequest(pc_type_file_request_in* log_request)
		...
		} else if (svr_session || KeySvrSessionPool->getSession(&svr_session) == 0) {
	...
	inline dgt_sint32 logUserFileRequest(pc_type_user_file_request_in* log_request)
        {
                ErrCode = 0;
                PcaKeySvrSession*       svr_session=0;
	//	if (KeySvrSessionPool->oltpLogMode() > 0) {
	...
	inline dgt_sint32 getKeyInfo(const dgt_schar* enc_col_name,const dgt_schar* passwd,dgt_schar* key_info_buf,dgt_uint32* buf_len)
	{
		...
		if (enc_col_id < 0) {
			...
			if ((ErrCode=priv_compiler.exportKeyInfo(enc_col_name,enc_col_id,passwd,key_info_buf,buf_len)) >= 0) return 0;
		}
		return ErrCode;
	}
#if 1
	inline dgt_sint32 getZoneParam(dgt_sint64 zone_id, dgt_schar** param)
	{
		...
		if (PcaKeySvrSessionPool::traceLevel() > 2) PcaKeySvrSessionPool::logging("SID::[%d] Searching EncZonePool [%lld]",SID,zone_id );
				...
				if ((ErrCode=svr_session->getZoneParam(zone_id,param_out))) {
					...
					memcpy(charset,KeySvrSessionPool->charSet(),32);
                                        if (*charset && strncasecmp(charset,"UTF-8",32)) {
                                                PtCharConv         ConvCharset(KeySvrSessionPool->charSet(),"UTF-8");
                                                dgt_schar       outbuffer[2049];
                                                memset(outbuffer,0,2049);
                                                if (ConvCharset.convCharSet(param_out, 2049, outbuffer, 2049) < 0) {
                                                        if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("charsetconv failed [%lld:%s]",zone_id ,param_out);
                                                } else {
                                                        memset(param_out,0,2049);
...
#endif
					if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("put parameters [%lld:%s]",zone_id ,param_out);
					*param = EncZonePool.putEncZone(zone_id,param_out);
					if (*param == 0) {
						ErrCode = PS_ERR_ENC_ZONE_PARAM_NOT_FOUND;
						delete param_out;
						PcaKeySvrSessionPool::logging("putEncZone Failed [%s]",EncZonePool.getErr());
					}
				}
				KeySvrSessionPool->returnSession(svr_session);
			} else PcaKeySvrSessionPool::logging(ErrCode, "get Server Session Failed");
		}//if(*param == 0)
		return ErrCode;
	}
#endif
	...
	inline dgt_sint32 getRegEngine(dgt_sint64 reg_engine_id, PccRegExprSearchEngine** param)
        {
                ErrCode=0;
		if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("SID::[%d] Searching RegEnginePool [%lld]",SID,reg_engine_id );
                *param = RegEnginePool.getRegEngine(reg_engine_id);
                if(*param == 0) {
                        ...
                        if (!(ErrCode=KeySvrSessionPool->getSession(&svr_session))) {
                                ...
                                memset(param_out,0,2049);
                                dgt_sint32              retry_cnt = 0;
                                if ((ErrCode=svr_session->getRegEngine(reg_engine_id,param_out))) {
                                        PcaKeySvrSessionPool::logging(ErrCode, svr_session->errMsg());
					...
					memcpy(charset,KeySvrSessionPool->charSet(),32);
						...
						dgt_schar       outbuffer[2049];
				                memset(outbuffer,0,2049);
				                if (ConvCharset.convCharSet(param_out, 2049, outbuffer, 2049) < 0) {
							if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("charsetconv failed [%lld:%s]",reg_engine_id ,param_out);
                        			} else {
							memset(param_out,0,2049);
							memcpy(param_out, outbuffer, 2049);
						}
...
#endif
					if (PcaKeySvrSessionPool::traceLevel() > 0) PcaKeySvrSessionPool::logging("put parameters [%lld:%s]",reg_engine_id ,param_out);
					*param = RegEnginePool.putRegEngine(reg_engine_id,param_out);
					if(*param == 0) {
						ErrCode = PS_ERR_REG_PARAM_NOT_FOUND;
						PcaKeySvrSessionPool::logging("putRegEngine Failed [%s]",RegEnginePool.getErr());
					}
				}
	...
	inline dgt_sint32 getRegEngine(dgt_schar* reg_engine_name, PccRegExprSearchEngine** param)
	{
		ErrCode=0;
	...
	{
		ErrCode=0;
			if (PcaKeySvrSessionPool::traceLevel() > 2) PcaKeySvrSessionPool::logging("SID[%d] Searching CryptParamPool [%s]", SID, crypt_param_name);
		*param = CryptParamPool.getCryptParam(crypt_param_name);
		if(*param == 0) {
			...
			if (!(ErrCode=KeySvrSessionPool->getSession(&svr_session))) {
				...
				memset(param_out,0,2049);
				dgt_sint32		retry_cnt = 0;
				if ((ErrCode=svr_session->getCryptParam(crypt_param_name,param_out))) {
					PcaKeySvrSessionPool::logging(ErrCode, svr_session->errMsg());
					ErrCode = PS_ERR_CRYPT_PARAM_NOT_FOUND;
				} else {
					if (PcaKeySvrSessionPool::traceLevel() > 2) PcaKeySvrSessionPool::logging("put parameters [%s:%s]", crypt_param_name, param_out);
					*param = CryptParamPool.putCryptParam(crypt_param_name, param_out);
					if(*param == 0) {
						ErrCode = PS_ERR_CRYPT_PARAM_NOT_FOUND;
						PcaKeySvrSessionPool::logging("putCryptParam Failed [%s]", CryptParamPool.getErr());
					}
				}
				KeySvrSessionPool->returnSession(svr_session);
			} else PcaKeySvrSessionPool::logging(ErrCode, "get Server Session Failed");
        ...
        inline dgt_sint32 encryptLengthWithVirtualKey(
                        ...
                        dgt_schar*      name10=0)
        {
                PcaPrivilege*   priv = getVKeyPrivilege(virtual_key_id,crypt_type,target_type,                                                                                               name1, name2, name3, name4, name5, name6, name7, name8, name9, name10);
                dgt_sint32 rtn = 0;
	...
	//for file crypt stream
        inline dgt_uint32 getFileHeaderFlag() { return FileHeaderFlag; };	// read: header skip / write: header write
	inline dgt_uint32 getFileRealBytes() { return FileRealBytes; };		// read: read bytes / write : wirte bytes
	...
	inline dgt_void setFileHeaderFlag() { FileHeaderFlag = 1; };
        inline dgt_void setFileRealBytes(dgt_uint32 real_bytes) { FileRealBytes = real_bytes; };
	...
	inline dgt_sint32 logDetectFileRequest(pc_type_detect_file_request_in* log_request, DgcMemRows* log_data)
	{
		ErrCode = 0;
		PcaKeySvrSession*	svr_session=0;
		if (svr_session || KeySvrSessionPool->getSession(&svr_session) == 0) {
			if ((ErrCode=svr_session->logDetectFileRequest(log_request,log_data))) {
				PcaKeySvrSessionPool::logging(ErrCode, svr_session->errMsg());
			}
			if (svr_session) KeySvrSessionPool->returnSession(svr_session);
		}
	...
	// for get rsa key
        ...
        {
                ErrCode=0;
                if (PcaKeySvrSessionPool::traceLevel() > 2) PcaKeySvrSessionPool::logging("SID::[%d] Searching RsaKeyPool [%s]",SID,key_name );
                ...
                if(*key_string == 0) {
                        ...
                        if (!(ErrCode=KeySvrSessionPool->getSession(&svr_session))) {
                                ...
                                if ((ErrCode=svr_session->getRsaKey(key_name,key_out))) {
                                        ...
                                        if (PcaKeySvrSessionPool::traceLevel() > 2) PcaKeySvrSessionPool::logging("put rsa_key [%s]",key_name);
                                        *key_string = RsaKeyPool.putRsaKey(key_name,key_out);
                                        if (*key_string == 0) {
                                                ErrCode = PS_ERR_RSA_KEY_NOT_FOUND;
                                                PcaKeySvrSessionPool::logging("putRsaKey Failed [%s]",RsaKeyPool.getErr());
                                                delete key_out;
                                        }
                                }
                                KeySvrSessionPool->returnSession(svr_session);
