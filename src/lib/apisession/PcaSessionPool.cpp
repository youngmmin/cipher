...
#ifndef WIN32
__attribute__((constructor)) void __pcApiCoreInitializer(void) {
    PcaSessionPool::initializer();
}
__attribute__((destructor)) void __pcApiCoreFinalizer(void) {
    PcaSessionPool::finalizer();
}
...
PcaSession* PcaSessionPool::newSession(dgt_sint32 sid) {
    ...
    if (DgcSpinLock::lock(&PoolLatch)) {
        ...
        while (session) {
            if (session->sid() == sid) {
                //
                // any session with the same sid is considered as an closed
                // session. reset & reuse
                //
                session->reset();
                break;
        ...
        if (!session) {
            //
            // create a new session and set it as the first node in its hash
            // list
            //
            session =
                new PcaSession(sid, KeySvrSessionPool, NamePool, KeyPool,
                               IVPool, (StandAloneFlag ? &PrivHolder : 0));
            if (PcaKeySvrSessionPool::traceLevel() > 0)
                PcaKeySvrSessionPool::logging("new session[%d] created.", sid);
            if (SessionTable[hval])
                session->setNext(
                    SessionTable[hval]);  // set it as the first node
...
dgt_void PcaSessionPool::removeSession(dgt_sint32 sid) {
            ...
            while (curr) {
                if (curr->sid() == sid) {
                    if (prev)
                        ...
                        SessionTable[hval] = (PcaSession*)curr->next();
                    curr->setNext();
                    delete curr;
                    break;
                }
    ...
    if (PcaKeySvrSessionPool::traceLevel() > 0)
        PcaKeySvrSessionPool::logging("session[%d] removed.", sid);
}

PcaSessionPool::~PcaSessionPool() {
#ifndef WIN32
    if (PcaKeySvrSessionPool::traceLevel() > 0)
        PcaKeySvrSessionPool::logging("SessionPool[%u] destructed", getpid());
#endif
