...
class PcaSessionPool : public DgcObject {
   private:
    static const dgt_sint32 PSP_MAX_SESSIONS =
        4096;  // size of session hash table
    static const dgt_sint32 PSP_RAMDOM_SEED =
        100000000;  // random session id seed
    ...
    dgt_sint32 setStandAlone(const dgt_schar* key_info_buffer,
                             dgt_uint32 buffer_len, const dgt_schar* passwd) {
        dgt_sint32 rtn;
        PcaPrivCompiler priv_compiler;
        if ((rtn = priv_compiler.importKeyInfo(passwd, key_info_buffer,
                                               buffer_len)) < 0)
            return rtn;
        if ((rtn = PrivHolder.putPriv(priv_compiler.encColID(),
                                      priv_compiler.privInfo())) < 0)
            return rtn;
        StandAloneFlag = 1;
#ifndef WIN32
        ...
        KeyPool->putKeyInfo(priv_compiler.privInfo()->key_id,
                            priv_compiler.keyInfo(),
                            priv_compiler.trailerInfo());
        return 0;
    }
#endif

    inline dgt_sint32 initializeKeySvrSessionPool(
        dgt_schar* file_path, const dgt_schar* credentials_pw) {
        dgt_sint32 rtn = 0;
        if (InitializeFlag == 0) {
            if (!(rtn = KeySvrSessionPool->initialize(file_path, credentials_pw,
                                                      StandAloneFlag))) {
            }
            InitializeFlag = 1;
            ...
            if (*(KeySvrSessionPool->keyInfoFilePath())) {
                ...
                if (EXCEPT) {
                    ...
                    PcaKeySvrSessionPool::logging(
                        "key_info_file[%s] compilation failed:%d",
                        KeySvrSessionPool->keyInfoFilePath(), e->errCode());
                    delete e;
                } else {
                    DgcBgrammer* bg = 0;
                    while ((bg = key_infos.getNext())) {
                        ...
                        if (key_info_buffer) {
                            dgt_sint32 rtn = setStandAlone(
                                key_info_buffer, strlen(key_info_buffer), 0);
                            if (rtn)
                                PcaKeySvrSessionPool::logging(
                                    "key_info[%s] load failed:%d",
                                    key_info_buffer, rtn);
                        }
                    }
                }
    ...
    inline PcaSession* findSession(dgt_sint32 sid) {
        if (sid < 0)
            ...
            return SessionTable[SingleHashVal];  // single session
        dgt_sint32 hval = sid % PSP_MAX_SESSIONS;
        PcaSession* session = 0;
        if (DgcSpinLock::lock(&PoolLatch)) {
            PcaKeySvrSessionPool::logging(PSP_ERR_LOCK_FAIL,
                                          "locking the session pool failed");
        } else {
   ...
   public:
...
#ifndef WIN32
        return SessionPool->initializeKeySvrSessionPool(info_file_path,
                                                        credentials_pw);
#else
        return SessionPool.initializeKeySvrSessionPool(info_file_path,
                                                       credentials_pw);
    ...
    static inline PcaSession* getSession(dgt_sint32 sid,
                                         dgt_uint8 no_lock = 0) {
#ifndef WIN32
...
#ifndef WIN32
    static inline dgt_void initializer() {
        SessionPool = new PcaSessionPool();
    };
