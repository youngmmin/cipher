...
class PccCipher : public DgcWorker {
  ...
  public:
	PccCipher(PccCryptBufFifoQueue& dq,PccCryptList& cq,PccPttnSearchEngine* pse,PccCryptor* cryptor,dgt_sint32* last_err_code)
  	  : DataQueue(dq),CryptQueue(cq),SearchEngine(pse),Cryptor(cryptor),LastErrCode(last_err_code),DataQueueWaits(0),CryptBuffers(0),OutBufLen(0){}
	virtual ~PccCipher() {}
