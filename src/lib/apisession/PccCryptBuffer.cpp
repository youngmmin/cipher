...
PccSegment::PccSegment(dgt_uint32 so, dgt_uint32 eo, dgt_uint8 st,
                       dgt_uint16 col_no, dgt_sint32 expr_no, dgt_schar* expr)
    : Soffset(so),
      Eoffset(eo),
      Type(st),
      ColNo(col_no),
...
dgt_sint8 PccSegment::compare(DgcObject* obj) {
    ...
    if ((soffset <= Soffset && Soffset < eoffset) ||
        (Soffset <= soffset && soffset < Eoffset))
        return 0;
    else if (Soffset > soffset)
        return 1;
    return -1;
}
            ...
            for (; curr;) {
                ...
                if ((cmp_val = seg->compare(curr)) > 0) {
                    ...
                    if (next)
                        ...
                        Last = seg;  // change the Last because the curr is the
                                     // Last
                    seg = 0;
                    break;
                } else if (cmp_val == 0) {
...
dgt_void PccSortedSegList::complete(
    dgt_sint32 last_eo)  // last_eo is the length of the text to be searched
{
    //
    ...
    // should be added.
    // complete is a process that all text segments have been added at their
    // correct position.
    //
    rewind();
    //
    ...
    if (curr && curr->sOffset()) {
        // insert the first text segment only if the first pattern segment's
        // starting offset > 0
    ...
    if (prev) {
        for (; (curr = curr->next());) {
            if (prev->eOffset() != curr->sOffset()) {
                ...
                tmp = new PccSegment(prev->eOffset(), curr->sOffset(),
                                     PccSegment::SEG_T_TEXT);
                tmp->setNext(curr);
                tmp->setPrev(prev);
                prev->setNext(tmp);
                curr->setPrev(tmp);
    ...
    if (prev) {  // there're pattern segments
        if (prev->eOffset() < last_eo) {
            // insert the last text segment only if the last pattern segment's
            // ending offset < text length
            tmp = new PccSegment(prev->eOffset(), last_eo,
                                 PccSegment::SEG_T_TEXT);
            tmp->setPrev(prev);
            prev->setNext(tmp);
            NumTextSegs++;
...
dgt_void PccCryptBuffer::readReset(dgt_uint64 seq_no, dgt_sint32 src_length,
