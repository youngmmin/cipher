...
class PccSegment : public DgcObject {
  ...
  public:
	...
	static const dgt_uint8 SEG_T_PTTN_NULL = 5; //this type is only space character and apply padding

	PccSegment(dgt_uint32 so,dgt_uint32 eo,dgt_uint8 st,dgt_uint16 col_no=0,dgt_sint32 expr_no=0,dgt_schar* expr=0);
	virtual ~PccSegment();
	...
	dgt_void update(PccSegment* seg) { if (seg) { Soffset=seg->sOffset(); Eoffset=seg->eOffset(); Type=seg->type(); } }
...
class PccSortedSegList : public DgcObject {
  ...
  public:
	...
	PccSegment* next() { PccSegment* rtn = Curr; if (Curr) Curr=Curr->next(); return rtn; }
...
class PccCryptBuffer : public DgcObject {
  public:
	...
	dgt_void readReset(dgt_uint64 seq_no,dgt_sint32 src_length,dgt_sint32 dst_length);
};
