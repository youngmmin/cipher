...
PccCryptDivision::PccCryptDivision(PccSearchEngineFactory& sef,
                                   PccCryptorFactory& cf, PccHeaderManager& hm)
    : SearchEngineFactory(sef),
      CryptorFactory(cf),
      HeaderManager(hm),
      LastErrCode(0),
      OutBufLen(0),
...
dgt_sint32 PccCryptDivision::crypt(const dgt_schar* in_file,
                                   const dgt_schar* out_file,
                                   dgt_uint8 force_target_write,
    ...
    if (CryptorFactory.parallelism() == 0) {
        ...
        if ((e = EXCEPTnC)) {
            ...
            DgcWorker::PLOG.tprintf(
                0, *e, "crypt_division crypt failed : [%s]\n", err_string);
            delete e;
            return LastErrCode;
        }
        ...
        if (!InFileSize) {
            ...
            if (force_target_write)
                file_flag = O_CREAT | O_TRUNC | O_WRONLY;
            else
...
#else
        if (force_target_write)
            file_flag = O_CREAT | O_TRUNC | O_WRONLY | _O_BINARY;
        else
            file_flag = O_CREAT | O_EXCL | O_WRONLY | _O_BINARY;
#endif
        ...
        if (HeaderManager.headerFlag() == 3)
            file_flag = O_CREAT | O_RDWR | _O_BINARY;
#endif
        DgcFileStream out(out_file, file_flag, 0664);
        if ((e = EXCEPTnC)) {
#ifndef WIN32
...
#ifndef WIN32
                sprintf(err_string, "out_file[%s] open failed : err_code[%d]",
                        out_file, e->errCode());
#else
                sprintf(err_string,
                        "out_file[%s] open failed : err_code[%d:%d]", out_file,
                        e->errCode(), GetLastError());
#endif
            }
            DgcWorker::PLOG.tprintf(
                0, *e, "crypt_division crypt failed : [%s]\n", err_string);
        ...
        dgt_sint32 new_stream_file_flag =
            0;  // if first stream encryption, value 1
        dgt_sint64 old_in_file_size = 0;
        if (HeaderManager.headerFlag() == 3 && out.fileSize() == 0)
            new_stream_file_flag = 1;
        ...
        // if you when encrypting set header_flag=on, must give same header_flag
        // when decrypting read and check header from in file
        if (HeaderManager.headerFlag()) {
            rtn = HeaderManager.checkHeader(&in);
            // rtn 1 : encrypting file
            //    0 : text file
            //   -1 : broken file
            if (rtn == PFC_HEADER_FILE_TYPE_ENCRYPT &&
                CryptorFactory.cryptMode() == PFC_CRYPT_MODE_ENCRYPT) {
		...
		return 0;
                    ...
                    LastErrCode =
                        PFC_DVS_ERR_CODE_ALREADY_ENCRYPTED;  // error for
                                                             // duplication
                                                             // encrypt
                    sprintf(err_string,
                            "integrity check faild : already encrypted file");
                    DgcWorker::PLOG.tprintf(
                        0, "crypt_division crypt failed : [%s]\n", err_string);
                    return LastErrCode;
                }
            } else if (rtn == PFC_HEADER_FILE_TYPE_ORIGINAL &&
                       CryptorFactory.cryptMode() == PFC_CRYPT_MODE_DECRYPT) {
                ...
                if (CryptorFactory.bypassCheck()) {
                    ...
                    sprintf(err_string,
                            "integrity check faild : original text file");
                    DgcWorker::PLOG.tprintf(
                        0, "crypt_division crypt failed : [%s]\n", err_string);
                    return LastErrCode;
                }
            ...
            if (HeaderManager.headerFlag() == 3 &&
                CryptorFactory.cryptMode() == PFC_CRYPT_MODE_ENCRYPT &&
                new_stream_file_flag == 0) {  // case already exist out file
                file_indicator = out.seek(0, SEEK_SET);
                if (file_indicator != 0)
                    ATHROWnR(
                        DgcError(SPOS, "out.seek failed [%d]", file_indicator),
                        -1);
                ...
                if (rtn == PFC_HEADER_FILE_TYPE_ORIGINAL) {
                    ...
                    if ((e = EXCEPTnC)) {
                        ...
                        LastErrCode = PFC_DVS_ERR_CODE_BROKEN_FILE;
                        sprintf(err_string,
                                "integrity check faild : broken file");
                        DgcWorker::PLOG.tprintf(
                            0, "crypt_division crypt failed : [%s]\n",
                            err_string);
                        return LastErrCode;
                ...
                if (HeaderManager.inFileSize() == InFileSize)
                    return 0;  // unchanged file, this not target
                old_in_file_size = HeaderManager.inFileSize();
            }  // if (HeaderManager.headerFlag() == 3 && out.fileSize() > 0) end

            // header version 3,4 encryption
            if (CryptorFactory.cryptMode() == PFC_CRYPT_MODE_ENCRYPT &&
                (HeaderManager.headerFlag() == 3 ||
                 HeaderManager.headerFlag() == 4))
                CryptorFactory.setBufferSize(1024);
            // write header to out file
            if (!CryptorFactory.bypassFlag() && CryptorFactory.cryptMode()) {
                if (HeaderManager.writeHeader(
                        &out, HeaderManager.headerFlag(),
                        CryptorFactory.cryptMode() == PFC_CRYPT_MODE_MIGRATION
                            ? HeaderManager.inFileSize()
                            : InFileSize,
                        CryptorFactory.cryptMode() == PFC_CRYPT_MODE_MIGRATION
                            ? HeaderManager.bufferSize()
        ...
        if (EXCEPT) {
            ...
            DgcWorker::PLOG.tprintf(
                0, *e,
                "crypt_division crypt failed : unkwon exception occured\n");
            delete e;
            return LastErrCode;
        }
        ...
        if (HeaderManager.headerFlag() == 3 &&
            CryptorFactory.cryptMode() == PFC_CRYPT_MODE_ENCRYPT &&
            new_stream_file_flag == 0) {
            // for appending, infile offset moving at infilesize offset of
            // header
            file_indicator = in.seek(out_header_info.in_file_size, SEEK_SET);
            if (file_indicator != 0)
                ATHROWnR(DgcError(SPOS, "in.seek failed [%d]", file_indicator),
                         -1);
            // for appending, outfile offset moving at end offset
            file_indicator = out.seek(0, SEEK_END);
            if (file_indicator != 0)
                ATHROWnR(DgcError(SPOS, "out.seek failed [%d]", file_indicator),
                         -1);
        }

        // converting buffer size for stream encryption and kernel encryption
        if ((HeaderManager.headerVersion() == 3 ||
             HeaderManager.headerVersion() == 4) &&
            !CryptorFactory.cryptMode()) {
            CryptorFactory.setBufferSize(CryptorFactory.streamDecBufSize());
        }

        PccCryptUnit crypt_unit(&in, &out, SearchEngineFactory, CryptorFactory,
                                HeaderManager, &LastErrCode);
        if ((HeaderManager.headerVersion() == 3 &&
             CryptorFactory.cryptMode() == PFC_CRYPT_MODE_ENCRYPT))
            crypt_unit.setInFileSize(InFileSize);
        ...
        // commitHeader function write hash value, encrypting data of filesize
        // through SHA256
        if (!CryptorFactory.bypassFlag() && HeaderManager.headerFlag() &&
            CryptorFactory.cryptMode()) {
        ...
        if (CryptorFactory.cryptMode() == PFC_CRYPT_MODE_ENCRYPT &&
            HeaderManager.headerFlag() == 3) {  // convert original data
            ...
            dgt_uint8* convert_buffer =
                new dgt_uint8[CryptorFactory.bufferSize()];
            ...
            while (remain_bytes > 0) {
                ...
                if ((nbytes = in.sendData(convert_buffer, delete_size) < 0)) {
                    ...
                    ATHROWnR(
                        DgcError(
                            SPOS,
                            "sendData failed: convert_buffer insert failed"),
                        -1);
                }
           ...
           // HeaderManager.headerFlag() == 3) end
        ...
        //
        PccFileSpliter file_spliter(in_file, SearchEngineFactory.getEngine(),
                                    CryptorFactory);
        if ((e = EXCEPTnC)) {
        ...
        if (force_target_write)
            ...
            file_flag = O_CREAT | O_EXCL | O_WRONLY | _O_BINARY;
#endif
        PccFileMerger file_merger(CryptorFactory, out_file,
                                  file_spliter.runSize(), file_flag);
        if ((e = EXCEPTnC)) {
            if (e->errCode() == EEXIST) {
                LastErrCode = PFC_DVS_ERR_CODE_OUT_FILE_ALREADY_EXIST;
                sprintf(err_string, "out_file[%s] already exist", out_file);
        ...
        if (HeaderManager.headerFlag()) {
            ...
            if ((in = file_spliter.getRun()) == 0) {
                if ((e = EXCEPTnC)) {
                    ...
                    sprintf(err_string,
                            "file_streams are not allocated : in[%p]\n", in);
                    DgcWorker::PLOG.tprintf(
                        0, "crypt_division crypt failed : [%s]\n", err_string);
                    return LastErrCode;
                }
            ...
            if (rtn == 1 && CryptorFactory.cryptMode()) {
                ...
                if (CryptorFactory.bypassCheck()) {
                    ...
                    sprintf(err_string,
                            "integrity check faild : already encrypt file");
                    DgcWorker::PLOG.tprintf(
                        0, "crypt_division crypt failed : [%s]\n", err_string);
                    return LastErrCode;
                }
            ...
            if (CryptorFactory.cryptMode() && HeaderManager.headerFlag() == 4)
                CryptorFactory.setBufferSize(1024);
            // write header to out file
            if (!CryptorFactory.bypassFlag() && CryptorFactory.cryptMode()) {
                if (HeaderManager.writeHeader(
                        &file_merger, HeaderManager.headerFlag(),
                        CryptorFactory.cryptMode() == PFC_CRYPT_MODE_MIGRATION
                            ? HeaderManager.inFileSize()
                            : InFileSize,
                        CryptorFactory.bufferSize(), CryptorFactory.encZoneId(),
                        CryptorFactory.keyId()) < 0) {
                    LastErrCode = PFC_DVS_ERR_CODE_WRITE_HEADER_FAILED;
                    sprintf(err_string, "writeHeader[%s] failed\n", out_file);
                    if ((e = EXCEPTnC)) {
        ...
        num_runs = 1;
        PccCryptUnit*
            crypt_units[1];  // for windows but deleted by jhpark 2017.11.06
#endif
        // converting buffer size for stream encryption and kernel encryption
        if ((HeaderManager.headerVersion() == 3 ||
             HeaderManager.headerVersion() == 4) &&
        ...
        for (dgt_uint32 i = 0; i < num_runs; i++) {
            ...
            if ((in = file_spliter.getRun()) == 0 ||
                ...
                if ((e = EXCEPTnC)) {
                    ...
                    LastErrCode = PFC_DVS_ERR_CODE_FSTREAM_NOT_ALLOCATED;
                    sprintf(err_string,
                            "file_streams are not allocated : in[%p] out[%p]\n",
                            in, out);
                    DgcWorker::PLOG.tprintf(
                        0, "crypt_division crypt failed : [%s]\n", err_string);
                }
            ...
            crypt_units[i] =
                new PccCryptUnit(in, out, SearchEngineFactory, CryptorFactory,
                                 HeaderManager, &LastErrCode);
        }
        ...
        if (LastErrCode == 0 && num_runs) {
            ...
            do {
                ...
                alive_units = 0;
                for (dgt_uint32 i = 0; i < num_runs; i++) {
                    if (crypt_units[i]->isAlive() &&
                        crypt_units[i]->status() != DGC_WS_CREAT)
                        alive_units++;
                }
            } while (alive_units);
        }
    ...
    if (CryptorFactory.cryptMode() == PFC_CRYPT_MODE_MIGRATION &&
...
dgt_sint32 PccCryptDivision::detect(const dgt_schar* in_file,
                                    DgcMemRows* detect_data,
                                    dgt_schar* err_string) {
    LastErrCode = 0;
    DgcExcept* e = 0;
    ...
    if (CryptorFactory.parallelism() == 0) {
        ...
        if ((e = EXCEPTnC)) {
            ...
            sprintf(err_string, "in_file[%s] open failed", in_file);
            DgcWorker::PLOG.tprintf(
                0, *e, "crypt_division detect failed : [%s]\n", err_string);
            delete e;
            return LastErrCode;
        }
        InFileSize = in.fileSize();
        ...
        PccDetectUnit detect_unit(&in, SearchEngineFactory, CryptorFactory,
                                  &LastErrCode);
        ...
        num_runs = 1;
        PccDetectUnit*
            detect_units[1];  // for windows but deleted by jhpark 2017.11.06
#endif
        for (dgt_uint32 i = 0; i < num_runs; i++) {
            DgcFileStream* in = 0;
            DgcFileStream* out = 0;
            if ((in = file_spliter.getRun()) == 0) {
                if ((e = EXCEPTnC)) {
                    ...
                    sprintf(err_string,
                            ...
                            in, out);
                    DgcWorker::PLOG.tprintf(
                        0, "crypt_division detect failed : [%s]\n", err_string);
                }
                for (dgt_uint32 j = 0; j < i; j++) delete detect_units[j];
                return LastErrCode;
            }
            detect_units[i] = new PccDetectUnit(in, SearchEngineFactory,
                                                CryptorFactory, &LastErrCode);
        }
        ...
        if (LastErrCode == 0 && num_runs) {
            ...
            do {
                ...
                alive_units = 0;
                for (dgt_uint32 i = 0; i < num_runs; i++) {
                    if (detect_units[i]->isAlive() &&
                        detect_units[i]->status() != DGC_WS_CREAT)
                        alive_units++;
                }
            } while (alive_units);
        }
        ...
        for (dgt_uint32 i = 0; i < num_runs; i++) {
            ...
            while (rtn_rows->next()) {
                DetectData->add();
                DetectData->next();
                memcpy(DetectData->data(), rtn_rows->data(),
                       rtn_rows->rowSize());
            }
        ...
        if (EXCEPT) {
            ...
            DgcWorker::PLOG.tprintf(
                0, *e,
                "crypt_division detect failed : unkwon exception occured\n");
            delete e;
            return LastErrCode;
        }
    }  // multiple processing else end
