...
class PccCryptDivision : public DgcObject {
  private :
	...
	dgt_sint64		OutFileSize; // added by shson  2017.07.02 for collecting statistic
	dgt_sint32		IsSkip;
	DgcMemRows*		DetectData;
  protected :
  public :
	PccCryptDivision(PccSearchEngineFactory& sef,PccCryptorFactory& cf,PccHeaderManager& hm);
	...
	dgt_sint32 crypt(const dgt_schar* in_file,const dgt_schar* out_file, dgt_uint8 force_target_write, dgt_schar* err_string);
	dgt_sint32 detect(const dgt_schar* in_file,DgcMemRows* detect_data, dgt_schar* err_string);
};
#endif
