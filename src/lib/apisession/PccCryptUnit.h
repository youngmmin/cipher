...
class PccCryptUnit : public DgcWorker {
  ...
  protected :
  public :
	PccCryptUnit(DgcFileStream* in,DgcFileStream* out,PccSearchEngineFactory& sef,PccCryptorFactory& cf,PccHeaderManager& hm,dgt_sint32* last_err_code);
	virtual ~PccCryptUnit();

	inline void setInFileSize(dgt_sint64 in_file_size) { InFileSize = in_file_size; };
