...
PccCryptorFactory::PccCryptorFactory(PccKeyMap& key_map,
                                     PccSearchEngineFactory& sef,
                                     const dgt_schar* pgm_name,
                                     dgt_sint32 manager_id)
    : KeyMap(key_map),
...
dgt_sint32 PccCryptorFactory::initialize(DgcBgrammer* dg,
    ...
    if (dg) {
        ...
        if ((val = dg->getValue("parallel.buffer.size")) &&
            (tmp = strtol(val, 0, 10)))
            BufferSize = tmp;
#else  // modifed by shson 2017.07.24 - fix buffer size has a few problem
        BufferSize = 2097152;
#endif
        if ((val = dg->getValue("parallel.threads")) &&
            (tmp = strtol(val, 0, 10)))
            NumThreads = tmp;
        if (NumThreads > 3) {
            NumBuffers = (dgt_sint32)floor(NumThreads * 1.5);
        }
        if ((val = dg->getValue("parallel.binary_flag")) &&
            (tmp = strtol(val, 0, 10)))
            BinaryFlag = tmp;
...
#endif
            if (tmp) RunSize = tmp;
            if (dgt_sint32 adjustSize = BufferSize % 16) {
                BufferSize += adjustSize;
            }
        }
    }

#ifdef WIN32  // added by chchung 2017.6.20 because windows do not support
              // thread yet
    RunSize = 0;
#endif
    if (BufferSize && RunSize && (dgt_sint64)BufferSize > RunSize) {
        sprintf(err_string,
                "buffer_size[%d] should be smaller than run_size[%lld]",
                BufferSize, RunSize);
        return -1;
    }
    if (NumBuffers < 3) {
        sprintf(err_string, "buffer amount[%d] should be greater than 2",
                NumBuffers);
        return -1;
    }
    return 0;
...
#if 1  // added by chchung 2017.6.12 for supporting shared or private session
       // depending on user's choice
// private session is used when ProgramName is defined
PcaApiSession* PccCryptorFactory::getSession(dgt_schar* err_string) throw(
    DgcExcept) {
    dgt_sint32 sid = -1;
    if ((sid = SessionID) < 0) {  // a private session in case of ProgramName is
                                  // defined and SessionID is not set yet
        // added by shson 18.02.28
        // when use multiple threads,
        // each session should be private session
        // because remove dependency num_shared_session parameter in
        // petra_cipher_api.conf so input parameter that no mean db_user
        dgt_schar db_user[16];
        memset(db_user, 0, sizeof(db_user));
        sprintf(db_user, "%d-%d", SessionID,
                ManagerID);  // this SessionID is no mean(simply number has been
                             // increasing)
        //
        ...
        //
        if (OsUser && *OsUser) {
            if ((sid = PcaApiSessionPool::getApiSession("", "", "", "", db_user,
                                                        OsUser, 0)) < 0) {
                sprintf(err_string, "getApiSession failed[%d]", sid);
                THROWnR(
                    DgcBgmrExcept(
                        DGC_EC_BG_INCOMPLETE,
                        new DgcError(SPOS, "getApiSession failed[%d]", sid)),
                    0);
            }
        } else {
            if ((sid = PcaApiSessionPool::getApiSession("", "", "", "", db_user,
    ...
    if ((session = PcaApiSessionPool::getApiSession(sid)) == 0) {
        ...
        THROWnR(
            DgcBgmrExcept(DGC_EC_BG_INCOMPLETE,
                          new DgcError(SPOS, "getApiSession[%d] failed", sid)),
            0);
    }
    return session;
}
...
PccCryptor* PccCryptorFactory::getCryptor(
    dgt_schar* err_string, dgt_sint32 shared_flag) throw(DgcExcept) {
    PcaApiSession* session = 0;
