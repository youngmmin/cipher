...
class PccCryptor : public DgcObject {
  ...
  protected:
	...
	PccKeyMap&	KeyMap;
  public:
	PccCryptor(PcaApiSession* session,PccKeyMap& key_map) : Session(session), KeyMap(key_map) {}
	virtual ~PccCryptor() {}
...
class PccLocalCryptor : public PccCryptor {
  ...
  public:
	PccLocalCryptor(PcaApiSession* session,PccKeyMap& key_map, dgt_sint32 header_flag = 0) : PccCryptor(session,key_map) { HeaderFlag = header_flag; }
...
class PccPatternEncryptor : public PccLocalCryptor {
  ...
  protected:
  public:
	PccPatternEncryptor(PcaApiSession* session,PccKeyMap& key_map) : PccLocalCryptor(session,key_map) {}
	virtual ~PccPatternEncryptor() {}

	virtual dgt_sint32 crypt(PccCryptBuffer* crypt_buf)
	{
		unsigned char* cebp = crypt_buf->DstDataPtr; // the current encrypting buffer pointer
		PccSegment* seg;
		...
		while((seg=crypt_buf->SegList->next())) {
			...
			if (seg->type() == PccSegment::SEG_T_PTTN) {
				...
				if((rtn=Session->encrypt(KeyMap.encName(seg->colNo()),sp,seg->length(),cebp,&dst_len)) < 0) {
#if 1
					return rtn;
#else
...
class PccPatternDecryptor : public PccLocalCryptor {
  private:
	dgt_uint16 colNo(const dgt_uint8* tag) { return (*(tag+2)-48)*10 + (*(tag+3)-48); }
	dgt_uint16 encLen(const dgt_uint8* tag) { return (*(tag+5)-48)*100 + (*(tag+6)-48)*10 + (*(tag+7)-48); }
  protected:
  public:
	PccPatternDecryptor(PcaApiSession* session,PccKeyMap& key_map) : PccLocalCryptor(session,key_map) {}
	virtual ~PccPatternDecryptor() {}
	virtual dgt_sint32 crypt(PccCryptBuffer* crypt_buf)
	{
		unsigned char* cdbp = crypt_buf->DstDataPtr; // the current encrypting buffer pointer
		PccSegment* seg;
		dgt_uint32 dst_buf_len = crypt_buf->DstLength;
		crypt_buf->DstLength = 0;
		crypt_buf->SegList->rewind();
		while((seg=crypt_buf->SegList->next())) {
			...
			if (seg->type() == PccSegment::SEG_T_TEXT && seg->next() && seg->next()->type() == PccSegment::SEG_T_PTTN
				...
				if (KeyMap.keyType() == USE_KEY_TYPE_VIRTUAL_KEY) {
					rtn = Session->decrypt_vkey(KeyMap.virtualKeyID(colNo(enc_tag)),enc_start_pos,enc_len,cdbp,&dst_len,
							PCI_VKEY_TARGET_TYPE_FILE,
							(dgt_schar*)KeyMap.hostName(),
							(dgt_schar*)KeyMap.osUser(),
							(dgt_schar*)KeyMap.inputFileNamePtr());
				} else {
					rtn = Session->decrypt(KeyMap.encName(colNo(enc_tag)),enc_start_pos,enc_len,cdbp,&dst_len);
				}
				if(rtn < 0) {
#if 1
					return rtn;
...
class PccFormatEncryptor : public PccLocalCryptor {
  ...
  protected:
  public:
	PccFormatEncryptor(PcaApiSession* session,PccKeyMap& key_map) : PccLocalCryptor(session,key_map) {}
	...
	virtual dgt_sint32 crypt(PccCryptBuffer* crypt_buf)
	{
		...
		while((seg=crypt_buf->SegList->next())) {
			unsigned char* sp = crypt_buf->SrcDataPtr + seg->sOffset();
			if (seg->type() == PccSegment::SEG_T_PTTN || seg->type() == PccSegment::SEG_T_PTTN_NULL) {
				// pattern segment should be encrypted
				dgt_sint32 rtn;
				dgt_uint32 dst_len = dst_buf_len - crypt_buf->DstLength;
				if((seg->type() == PccSegment::SEG_T_PTTN) && 
				   (rtn=Session->encrypt(KeyMap.encName(seg->colNo()),sp,seg->length(),cebp,&dst_len)) < 0) {
#if 1
					return rtn;
#else
					DgcWorker::PLOG.tprintf(0,"encrypt failed[%d]\n",rtn);
					...
					if (seg->type() == PccSegment::SEG_T_PTTN_NULL) {	//SEG_T_PTTN_NULL is space string, 
						memcpy(cebp, sp, seg->length());                //copy source buffer to dst buffer because of do not execute encrypt function
						dst_len = seg->length();
					}
					cebp += dst_len;
					crypt_buf->DstLength += dst_len;
...
class PccFormatDecryptor : public PccLocalCryptor {
  ...
  protected:
  public:
	PccFormatDecryptor(PcaApiSession* session,PccKeyMap& key_map) : PccLocalCryptor(session,key_map) {}
	...
	virtual dgt_sint32 crypt(PccCryptBuffer* crypt_buf)
	{
		...
		while((seg=crypt_buf->SegList->next())) {
			...
			if (seg->type() == PccSegment::SEG_T_PTTN || seg->type() == PccSegment::SEG_T_PTTN_NULL) {
				...
				dgt_sint32 rtn=0;
				dgt_uint32 dst_len = dst_buf_len - crypt_buf->DstLength;
				if (KeyMap.keyType() == USE_KEY_TYPE_VIRTUAL_KEY && seg->type() == PccSegment::SEG_T_PTTN) {
					rtn = Session->decrypt_vkey(KeyMap.virtualKeyID(seg->colNo()),sp,seg->length(),cebp,&dst_len,
							PCI_VKEY_TARGET_TYPE_FILE,
							(dgt_schar*)KeyMap.hostName(),
							(dgt_schar*)KeyMap.osUser(),
							(dgt_schar*)KeyMap.inputFileNamePtr());
				} else if (seg->type() == PccSegment::SEG_T_PTTN) {
					rtn = Session->decrypt(KeyMap.encName(seg->colNo()),sp,seg->length(),cebp,&dst_len);
				}
				...
				if(rtn < 0) {
					...
					if (seg->type() == PccSegment::SEG_T_PTTN_NULL) { //SEG_T_PTTN_NULL is space string, 
						memcpy(cebp, sp, seg->length());			  //copy source buffer to dst buffer because of do not execute decrypt function
						dst_len = seg->length();
					}
					cebp += dst_len;
					crypt_buf->DstLength += dst_len;
...
class PccWholeEncryptor : public PccLocalCryptor {
  ...
  protected:
  public:
	PccWholeEncryptor(PcaApiSession* session,PccKeyMap& key_map, dgt_sint32 header_flag = 0) : PccLocalCryptor(session,key_map,header_flag) {}
	virtual ~PccWholeEncryptor() {}
	virtual dgt_sint32 crypt(PccCryptBuffer* crypt_buf)
	{
		dgt_sint32 rtn;
		dgt_schar* header_flag_str = 0;
		if (HeaderFlag > 0) header_flag_str = (dgt_schar*)"on";
		else header_flag_str = (dgt_schar*)"off";
		dgt_uint32 dst_len = crypt_buf->DstLength;
		if((rtn=Session->encrypt(KeyMap.encName(1),crypt_buf->SrcDataPtr,crypt_buf->SrcLength,crypt_buf->DstDataPtr,&dst_len,header_flag_str)) < 0) {
#if 1
			return rtn;
#else
			DgcWorker::PLOG.tprintf(0,"encrypt failed[%d]\n",rtn);
			memcpy(crypt_buf->DstDataPtr,crypt_buf->SrcDataPtr,crypt_buf->SrcLength);
...
class PccWholeDecryptor : public PccLocalCryptor {
  ...
  protected:
  public:
	PccWholeDecryptor(PcaApiSession* session,PccKeyMap& key_map, dgt_sint32 header_flag = 0) : PccLocalCryptor(session,key_map,header_flag) {}
	...
	virtual dgt_sint32 crypt(PccCryptBuffer* crypt_buf)
	{
		...
		if (KeyMap.keyType() == USE_KEY_TYPE_VIRTUAL_KEY) {
			rtn = Session->decrypt_vkey(KeyMap.virtualKeyID(1),crypt_buf->SrcDataPtr,crypt_buf->SrcLength,crypt_buf->DstDataPtr,&dst_len,
					PCI_VKEY_TARGET_TYPE_FILE,
					(dgt_schar*)KeyMap.hostName(),
					(dgt_schar*)KeyMap.osUser(),
					(dgt_schar*)KeyMap.inputFileNamePtr());
		} else {
			rtn=Session->decrypt(KeyMap.encName(1),crypt_buf->SrcDataPtr,crypt_buf->SrcLength,crypt_buf->DstDataPtr,&dst_len,header_flag_str);
		}
...
#else
			DgcWorker::PLOG.tprintf(0,"decrypt failed[%d]\n",rtn);
			memcpy(crypt_buf->DstDataPtr,crypt_buf->SrcDataPtr,crypt_buf->DstLength);
#endif
		} else {
			crypt_buf->DstLength = dst_len;
		}
...
class PccBypassEncryptor : public PccLocalCryptor {
  ...
  protected:
  public:
        PccBypassEncryptor(PcaApiSession* session,PccKeyMap& key_map) : PccLocalCryptor(session,key_map) {}
...
class PccWholeMigrator : public PccLocalCryptor {
  ...
  protected:
  public:
	PccWholeMigrator(PcaApiSession* session,PccKeyMap& key_map, dgt_sint32 header_flag = 0) : PccLocalCryptor(session,key_map,header_flag) { CurrentKeyId = 0; }
	...
	virtual dgt_sint32 crypt(PccCryptBuffer* crypt_buf)
	{
		...
		if (CurrentKeyId == (dgt_sint64)*(crypt_buf->SrcDataPtr -1)) {
			delete decrypt_buffer;
			return -920514; //this number mean not migration target file, origin is birthday of daryu~
		}

		//4. decryption old encrypting data
		if ((rtn=Session->decrypt(KeyMap.encName(1),crypt_buf->SrcDataPtr,crypt_buf->SrcLength,decrypt_buffer,&decrypt_buffer_len,header_flag_str)) < 0) {
			delete decrypt_buffer;
			return rtn;
		} else { //decrypt success
			//5. new encrypt decrypting data
			if((rtn=Session->encrypt(KeyMap.encName(1),decrypt_buffer,decrypt_buffer_len,crypt_buf->DstDataPtr,&dst_len,header_flag_str)) < 0) { //encrypt failed
				delete decrypt_buffer;
				return rtn;
			} //encrypt end
			else crypt_buf->DstLength = dst_len; //encrypt success
		} //migration end
		delete decrypt_buffer;
		return crypt_buf->DstLength;
	}
...
class PccCryptorFactory : public DgcObject {
  ...
  public :
	...
	PccCryptorFactory(PccKeyMap& key_map,PccSearchEngineFactory& sef,const dgt_schar* pgm_name = 0, dgt_sint32 manager_id=0);
	virtual ~PccCryptorFactory();

	inline dgt_void setProgramName(const dgt_schar* pgm_name) { ProgramName = pgm_name; };
	...
	inline dgt_void setBufferSize(dgt_sint32 buf_size) { BufferSize = buf_size; };
	inline dgt_void setEncZoneId(dgt_sint64 enc_zone_id) { EncZoneId = enc_zone_id; };
	inline dgt_void setTraceLevel(dgt_sint32 trace_level) { TraceLevel = trace_level; };
	...
	inline dgt_void setHeaderFlag(dgt_sint32 header_flag) { HeaderFlag=header_flag; };
	inline dgt_void setKeyId(dgt_sint64 key_id) { KeyID=key_id; };
	inline dgt_void setStreamBufferSize(dgt_sint32 stream_dec_buf_size) { StreamDecBufSize=stream_dec_buf_size; };
	...
	PccCryptor* getCryptor(dgt_schar* err_string, dgt_sint32 shared_flag=0) throw(DgcExcept);
};
