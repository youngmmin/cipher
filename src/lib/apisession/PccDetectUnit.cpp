...
PccDetectUnit::PccDetectUnit(DgcFileStream* in, PccSearchEngineFactory& sef,
                             PccCryptorFactory& cf, dgt_sint32* last_err_code)
    : InStream(in),
...
dgt_void PccDetectUnit::in() throw(DgcExcept) {
    ...
    Reader = new PccReader(InStream, CryptBufList, search_engine, DataQueue, 0,
                           LastErrCode);
    Reader->start(1);
    if (EXCEPT) {
    ...
    for (dgt_sint32 i = 0;
         i < CryptorFactory.numThreads() && ++NumDetectors <= MAX_CIPHERS;
         i++) {
        Detectors[i] =
            new PccDetector(DataQueue, CryptQueue,
                            SearchEngineFactory.getEngine(), LastErrCode);
        Detectors[i]->start(1);
        if (EXCEPT) {
            *LastErrCode = PFC_UNIT_ERR_CODE_START_CIPHER_FAILED;
            sprintf(ErrString, "%d-th detector start failed:%d", i + 1,
                    *LastErrCode);
...
dgt_void PccDetectUnit::out() throw(DgcExcept) {
    if (Reader->isAlive()) {  // abnormal stop by force in middle of work
        ...
        for (dgt_sint32 i = 0; i < NumDetectors; i++) {
            if (Detectors[i])
                while (Detectors[i]->isAlive()) napAtick();
            //      Detectors[i]->stop();
        }
    } else {
...
dgt_sint32 PccDetectUnit::detect() throw(DgcExcept) {
    ...
    PccReader reader(InStream, CryptBufList, search_engine, DataQueue, 0,
                     LastErrCode, 0);
    ...
    while ((rtn = reader.readAndHandover(&curr)) > 0 && curr) {
        ...
        if ((rtn = detector.detect(curr)) < 0) {
            THROWnR(DgcBgmrExcept(DGC_EC_BG_INCOMPLETE,
                                  new DgcError(SPOS, "detector detect failed")),
                    -1);
        }
