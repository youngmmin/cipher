...
PccFileCryptor::PccFileCryptor(const dgt_schar* pgm_name,const dgt_schar* crypt_mode, dgt_sint32 trace_level, dgt_sint32 manager_id)
	: SearchEngineFactory(KeyMap,crypt_mode), CryptorFactory(KeyMap,SearchEngineFactory,pgm_name,manager_id), HeaderManager(),
	  ParamList(0), NumParams(0), InFileName(0), OutFileName(0), LogFileName(0), ForceTargetWrite(0), OutBufLen(0), InFileSize(0), OutFileSize(0), LastErrCode(0), ErrString(0), BypassCheck(0), UserLogging(0), PtuId(0), ValidationFlag(0), JobId(0), DirId(0), NumPttns(0), IsSkip(0)
{
...
dgt_void PccFileCryptor::openLogStream()
{
	for (;DgcWorker::PLOG.logStream() == 0;) {
		DgcFileStream* fs = new DgcFileStream(PcaLogger::logFilePath(),O_CREAT|O_APPEND|O_WRONLY,0666);
		if (EXCEPT) {
			delete EXCEPTnC;
			fs = 0;
		}
		...
		fs = new DgcFileStream("/tmp/petra_cipher_api.log",O_CREAT|O_APPEND|O_WRONLY,0666);
		if (EXCEPT) {
			delete EXCEPTnC;
			fs = 0;
		} 
...
dgt_sint32 PccFileCryptor::getFileParams(DgcBgrammer* bg)
{
	...
	if ((LogFileName=bg->getValue("file.log"))) {
		DgcFileStream*  fs = new DgcFileStream(LogFileName,O_CREAT|O_APPEND|O_WRONLY,0666);
		if (EXCEPT) {
			LastErrCode = PFC_FC_ERR_CODE_OPEN_LOG_FILE_FAILED;
			DgcExcept* e = EXCEPTnC;
			openLogStream();
			sprintf(ErrString,"log file open failed[%s]",LogFileName);
			DgcWorker::PLOG.tprintf(0,*e,"file_cryptor getFileParams failed:[%s]\n",ErrString);
			delete e;
            return LastErrCode;
		} else {
			DgcWorker::PLOG.setLog(10,DGC_TRUE,new DgcBufferStream(fs,1));
	...
	for(col_no=1;;col_no++) {
		...
		if ((enc_name=bg->getValue(node_name))) {
			...
			if ((col_string=bg->getValue(node_name))) {
				...
				sprintf(ErrString,"not define columns in key node [col_no:%d]",col_no);
				if (CryptorFactory.traceLevel() > 10) if (CryptorFactory.traceLevel() > 10) DgcWorker::PLOG.tprintf(0,"file_cryptor getKeyParams failed:[%s]\n",ErrString);
				return LastErrCode;
			}
		} else {	//not define key name
			if (col_no == 1) {	//when not exist col_no 1 is error
				LastErrCode = PFC_FC_ERR_CODE_KEY_NAME_NOT_DEFINED;
				sprintf(ErrString,"not define name in key node [col_no:%d]",col_no);
				if (CryptorFactory.traceLevel() > 10) DgcWorker::PLOG.tprintf(0,"file_cryptor getKeyParams failed:[%s]\n",ErrString);
				return LastErrCode;
			}
			break;
		}
...
{
	dgt_schar* val;
	if ((val=bg->getValue("session.program_id"))) CryptorFactory.setProgramName(val);
	return 0;
}
...
dgt_sint32 PccFileCryptor::getModeParams(DgcBgrammer* bg)
{
	...
	if ((val=bg->getValue("mode.crypt"))) { 
		...
		//if crypt paramter equel "validation", It't just only change ValidationFlag
		//other operating be same decrypt machanism
		//ValidationFlag used for logging

		if (strlen(val) && (strncasecmp(val,"validation",10) == 0)) { //when validation
			ValidationFlag = 1;
			val = (dgt_schar*)"decrypt"; //change decrypt mode
		} 
		SearchEngineFactory.setCryptMode(val);
	}
	if ((val=bg->getValue("mode.crypt.detect")) || (val=bg->getValue("mode.detect"))) {
#ifndef	WIN32
	...
	if ((val=bg->getValue("mode.overwrite_flag")) || (val=bg->getValue("mode.force_target_write"))) {
	if(val && strncmp(val,"on",2) == 0)
		ForceTargetWrite = 1;
	}
...
dgt_sint32 PccFileCryptor::importKeyInfo(DgcBgrammer* bg)
{
	...
	if ((val=bg->getValue("key_info"))) {
		...
		if ((rtn=PcaSessionPool::putKeyInfo(val,strlen(val),0)) < 0) {
			...
			if ((e=EXCEPTnC)) {
				if (CryptorFactory.traceLevel() > 10) DgcWorker::PLOG.tprintf(0,*e,"file_cryptor importKeyInfo failed : [%s]\n",ErrString);
				delete e;
			} else {
				if (CryptorFactory.traceLevel() > 10) DgcWorker::PLOG.tprintf(0,"file_cryptor importKeyInfo failed : [%s]\n",ErrString);
			}
			return LastErrCode;
		}
	}
...
dgt_sint32 PccFileCryptor::getSystemInfo(DgcBgrammer* bg)
{
	...
	dgt_schar* val;
	if ((val=bg->getValue("system_info.system_name"))) strcat(SystemName, val); 
...
{
	dgt_sint32 rtn=0;
	if (bg->getNode("file")) rtn=getFileParams(bg);
	else if (bg->getNode("key")) rtn=getKeyParams(bg);
	else if (bg->getNode("delimiter")) rtn=getDelimiterParams(bg);
	else if (bg->getNode("fixed")) rtn=getFixedParams(bg);
	else if (bg->getNode("regular")) rtn=getRegularParams(bg);
	else if (bg->getNode("parallel")) rtn=getCryptParams(bg);
	else if (bg->getNode("session")) rtn=getSessionParams(bg);
	else if (bg->getNode("mode")) rtn=getModeParams(bg);
	else if (bg->getNode("key_info")) rtn=importKeyInfo(bg);
	else if (bg->getNode("system_info")) rtn=getSystemInfo(bg);
	else if (bg->getNode("logging_info")) rtn=getLoggingInfo(bg);
	else if (bg->getNode("out_extension")) rtn=0; //not to do
	else {
		LastErrCode = PFC_FC_ERR_CODE_UNSUPPORTED_PARAM;
		sprintf(ErrString,"unsupported parameter[%s]",bg->getText());
		if (CryptorFactory.traceLevel() > 10) DgcWorker::PLOG.tprintf(0,"file_cryptor getParams failed : [%s]\n",ErrString);
		return LastErrCode;
	}
	return rtn;
}
...
dgt_sint32 PccFileCryptor::compileParamList(const dgt_schar* param_list)
{
    ...
    while(param_list[idx] != '(') {
    	if(param_list[idx] == ' ') {
            ...
            if (CryptorFactory.traceLevel() > 10) DgcWorker::PLOG.tprintf(0,"file_cryptor compileParamList failed : [%s]\n",ErrString);
            return LastErrCode;
        }
    }
	...
	if (EXCEPT) {
		...
		if (e) {
			...
			if (CryptorFactory.traceLevel() > 10) DgcWorker::PLOG.tprintf(0,*e,"file_cryptor compileParamList failed : [%s]\n",ErrString);
			delete e;
		}
		return LastErrCode;
	}
...
dgt_sint32 PccFileCryptor::compileParamFile(const dgt_schar* param_file)
{
	...
	if (EXCEPT) {
		...
		if (e) {
			...
			DgcWorker::PLOG.tprintf(0,*e,"file_cryptor compileParamFile failed : [%s]\n",ErrString);
			delete e;
		}
		return LastErrCode;
	}
...
dgt_sint32 PccFileCryptor::crypt(const dgt_schar* in_file,const dgt_schar* out_file)
{
	openLogStream();
	if (in_file == 0 || (in_file && strlen(in_file) == 0)) in_file = InFileName;
	if (out_file == 0 || (out_file && strlen(out_file) == 0)) out_file = OutFileName;
	if (in_file == 0) {
		LastErrCode = PFC_FC_ERR_CODE_IN_FILE_NOT_DEFINED;
		sprintf(ErrString,"input file is not defined");
		if (CryptorFactory.traceLevel() > 10) DgcWorker::PLOG.tprintf(0,"file_cryptor crypt failed : [%s]\n",ErrString);
		return LastErrCode;
	}
...
#if 1 // added by shson 2017.6.5 for automatic calculation of the read buffer size when decrypting a whole file
// based on the read buffer size being used when encrypting,
// which means the size of buffer in decrypting parameters should be the same as the size of buffer in encrypting parameters
//
		if(SearchEngineFactory.engineType() == SearchEngineFactory.WHOLE_DECRYPTOR ||
			SearchEngineFactory.engineType() == SearchEngineFactory.WHOLE_MIGRATOR) {
			...
			if (!session) {
				...
				if (EXCEPT) {
					DgcExcept* e = EXCEPTnC;
					if (CryptorFactory.traceLevel() > 10) DgcWorker::PLOG.tprintf(0,*e,"file_cryptor crypt failed : [%s]\n",ErrString);
					delete e;
				}
				rtn = LastErrCode;
				break;
			...
			if (KeyMap.keyType() == USE_KEY_TYPE_VIRTUAL_KEY) {
				decrypt_read_buf_size = session->encryptLengthWithVirtualKey(
										KeyMap.virtualKeyID(1),
										CryptorFactory.bufferSize(),
										PCI_VKEY_CRYPT_TYPE_DEC,
										PCI_VKEY_TARGET_TYPE_FILE,
				...
				if(decrypt_read_buf_size < 0) {
					...
					sprintf(ErrString,"encryptLengthWithVirtualKey faild:[%d] ",decrypt_read_buf_size);
					if (CryptorFactory.traceLevel() > 10) DgcWorker::PLOG.tprintf(0,"file_cryptor crypt failed : [%s]\n",ErrString);
					rtn = LastErrCode;
					break;
				}
			} else {
				decrypt_read_buf_size = session->encryptLength(KeyMap.encName(1),CryptorFactory.bufferSize());
				stream_dec_buf_size = session->encryptLength(KeyMap.encName(1),1024);
				if(decrypt_read_buf_size < 0) {
					LastErrCode = decrypt_read_buf_size;
					sprintf(ErrString,"getEncryptLength faild:buffer_size[%d] ",decrypt_read_buf_size);
					if (CryptorFactory.traceLevel() > 10) DgcWorker::PLOG.tprintf(0,"file_cryptor crypt failed : [%s]\n",ErrString);
					rtn = LastErrCode;
					break;
				}
			}
//			if (session->sharedFlag()) PcaApiSessionPool::returnApiSession(session->sid());
			...
			if (KeyMap.keyType() != USE_KEY_TYPE_VIRTUAL_KEY) {
				dgt_sint64 ctl_bytes=session->maskingDecCount(KeyMap.encName(1));
				if (ctl_bytes) {
					DgcFileStream ctl_file(in_file,O_RDONLY);
					DgcExcept* e = 0;
					if ((e=EXCEPTnC)) {
						LastErrCode = PFC_FC_ERR_CODE_OPEN_IN_FILE_FAILED;
						sprintf(ErrString,"in_file[%s] open failed for massive data decrypt control",in_file);
						if (CryptorFactory.traceLevel() > 10) DgcWorker::PLOG.tprintf(0,*e,"file_cryptor crypt failed : [%s]\n",ErrString);
						delete e;
						rtn = LastErrCode;
						break;
					...
					if (ctl_file.fileSize() > ctl_bytes) {
						...
						sprintf(ErrString,"no privilege by size control : ctl_bytes[%lld]",ctl_bytes);
						if (CryptorFactory.traceLevel() > 10) DgcWorker::PLOG.tprintf(0,"file_cryptor crypt failed : [%s]\n",ErrString);
						rtn = LastErrCode;
						break;
					}
		...
		PcaApiSession*  session = CryptorFactory.getSession(ErrString);
			...
			if (SearchEngineFactory.engineType() == SearchEngineFactory.WHOLE_DECRYPTOR || 
				SearchEngineFactory.engineType() == SearchEngineFactory.WHOLE_ENCRYPTOR ||
				SearchEngineFactory.engineType() == SearchEngineFactory.WHOLE_MIGRATOR) {
				dgt_sint64 key_id = session->getKeyId(KeyMap.encName(1));
				if (key_id < 0) {
					LastErrCode = key_id;
					sprintf(ErrString,"getKeyId faild:Error Code[%lld] ",key_id);
					if (CryptorFactory.traceLevel() > 10) DgcWorker::PLOG.tprintf(0,"file_cryptor crypt failed : [%s]\n",ErrString);
					rtn = LastErrCode;
					break;
				}
				CryptorFactory.setKeyId(key_id);
			} //if (SearchEngineFactory.engineType() == SearchEngineFactory.WHOLE_DECRYPTOR) end
#endif
		}
		PccCryptDivision cd(SearchEngineFactory,CryptorFactory,HeaderManager);
		if ((rtn=cd.crypt(in_file,out_file,ForceTargetWrite,ErrString)) < 0) {
			LastErrCode = rtn;
			DgcWorker::PLOG.tprintf(0,"crypt failed[%d] : in_file [%s] out_file [%s]",LastErrCode,in_file,out_file);
			break;
		}
	...
	if (rtn < 0) {
		...
		if (HeaderManager.headerFlag() != 3) {
			if (rtn == PFC_DVS_ERR_CODE_OUT_FILE_ALREADY_EXIST || rtn == PFC_DVS_ERR_CODE_OPEN_OUT_FILE_FAILED) {
		...
		if (unlink(in_file) < 0) {
			if (CryptorFactory.traceLevel() > 10) DgcWorker::PLOG.tprintf(0,"unlink[%s] failed:%s\n",in_file,strerror(errno));
		} else {
			if (rename(out_file,in_file) < 0) { // rename the output file
				if (CryptorFactory.traceLevel() > 10) DgcWorker::PLOG.tprintf(0,"rename old[%s] new[%s] failed:%s\n",out_file,in_file,strerror(errno));
			}
		}
	}
...
dgt_sint32 PccFileCryptor::crypt(dgt_sint32 sid,const dgt_schar* parameters,const dgt_schar* in_file,const dgt_schar* out_file,dgt_sint32 agent_mode,const dgt_schar* enc_col_name,const dgt_schar* header_flag, dgt_sint32 buffer_size)
{
	dgt_sint32 rtn;
	...
	if (agent_mode > 1 && enc_col_name && *enc_col_name) {
		...
		if (buffer_size) CryptorFactory.setBufferSize(buffer_size);
                ...
                if (rtn == 1) //if rtn values 1, execute compileParamList()
                        rtn=compileParamList(parameters); //case parameters is bgrammer string
	...
	if (UserLogging) userCryptLogging(start_time, end_time);
	else cryptLogging(start_time, end_time);
	return rtn;
}

dgt_sint32 PccFileCryptor::detect(const dgt_schar* parameters, const dgt_schar* in_file)
{
	openLogStream();
	...
	if (in_file == 0) {
		LastErrCode = PFC_FC_ERR_CODE_IN_FILE_NOT_DEFINED;
		sprintf(ErrString,"input file is not defined");
		if (CryptorFactory.traceLevel() > 10) DgcWorker::PLOG.tprintf(0,"file_cryptor detect failed : [%s]\n",ErrString);
		return LastErrCode;
	}
	...
	if (SearchEngineFactory.detectMode()) {
		if (!file_type) {
			...
			dgt_uint8 buf[32] = {0};
			dgt_sint32 nbytes = 0;
			if ((nbytes=in.recvData(buf,sizeof(buf))) < 0) ATHROWnR(DgcError(SPOS,"src_file[%s] read failed\n", in_file), -1);
			for (dgt_sint32 i = 0; i < 32 && i < nbytes; i++) {
				// 1. If the control character(ex. 0x00 ~ 0x1F) exists in 32bytes, judge it is a binary file
				// 2. The newline character(ex. 0x0A, 0x0D, 0x1E) is used on new lines so they are excluded from judgment.
				// 3. The tab character(ex. 0x09) is excluded
				if (buf[i] >= 0x00 && buf[i] <= 0x1F && buf[i] != 0x09 && buf[i] != 0x0A && buf[i] != 0x0D && buf[i] != 0x1E) {
		...
		if (!(SearchEngineFactory.detectMode() & file_type)) {
			...
			sprintf(ErrString,"the file[%s]'s type does not match the detect mode", in_file);
			return LastErrCode;
		}
	}
	for(;in_file;) {
		...
		if (!session) {
			...
			if (EXCEPT) {
				DgcExcept* e = EXCEPTnC;
				if (CryptorFactory.traceLevel() > 10) DgcWorker::PLOG.tprintf(0,*e,"file_cryptor detect failed : [%s]\n",ErrString);
				delete e;
			}
			rtn = LastErrCode;
			break;
...
dgt_sint32 PccFileCryptor::detect(dgt_sint32 sid,const dgt_schar* parameters,const dgt_schar* in_file,dgt_sint64 max_detection, dgt_sint64 job_id, dgt_sint64 dir_id)
{
	...
	if (rtn == 1) { //if rtn values 1, execute compileParamList()
		...
		if (file_size > 1024) {
			...
			dgt_sint32 nbytes = 0;
			if ((nbytes=param_file.recvData((dgt_uint8*)params,file_size)) < 0) rtn = nbytes;
		}
...
dgt_void PccFileCryptor::cryptLogging(dgt_uint32 start_time, dgt_uint32 end_time)
{
	PcaApiSession*  session = CryptorFactory.getSession(ErrString);
	if (session == 0) {
		delete EXCEPTnC;
	...
	for (i=1; i<KeyMap.maxCols(); i++) {
		if (KeyMap.keyType() == USE_KEY_TYPE_VIRTUAL_KEY) {
			...
			if (strlen(KeyMap.encName(i)) > 0) {
				memcpy(enc_col_name,KeyMap.encName(i),strlen(KeyMap.encName(i)));
				break;
			}
		}
	...
	if (SearchEngineFactory.cryptMode()) {
		...
		if (session->isEncryptAudit(enc_col_name)) {
			...
			if (SearchEngineFactory.engineType() == PccSearchEngineFactory::FORMAT_ENCRYPTOR) {
				sprintf(file_request.enc_type,"SAM");
			} else if (SearchEngineFactory.engineType() == PccSearchEngineFactory::PATTERN_ENCRYPTOR) {
			...
			if (strlen(ErrString)) memcpy(file_request.err_msg,ErrString,255);
			else sprintf(file_request.err_msg,"success");
		...
		if (is_audit_flag) {
			...
			memcpy(file_request.system_ip,SystemIp,strlen(SystemIp));
			memcpy(file_request.zone_name,ZoneName,strlen(ZoneName));
			if (SearchEngineFactory.engineType() == PccSearchEngineFactory::FORMAT_DECRYPTOR) {
				sprintf(file_request.enc_type,"SAM");
			} else if (SearchEngineFactory.engineType() == PccSearchEngineFactory::PATTERN_DECRYPTOR) {
				sprintf(file_request.enc_type,"PATTERN");
			} else {
				sprintf(file_request.enc_type,"FULL");
			}
			if (ValidationFlag) file_request.mode=3; else file_request.mode=2;
			if (KeyMap.keyType() == USE_KEY_TYPE_VIRTUAL_KEY) {
...
dgt_void PccFileCryptor::userCryptLogging(dgt_uint32 start_time, dgt_uint32 end_time)
{
	PcaApiSession*  session = CryptorFactory.getSession(ErrString);
	if (session == 0) {
		delete EXCEPTnC;
...
dgt_void PccFileCryptor::detectLogging(dgt_uint32 start_time, dgt_uint32 end_time)
{
	PcaApiSession*	session = CryptorFactory.getSession(ErrString);
	if (session == 0) {
		delete EXCEPTnC;
