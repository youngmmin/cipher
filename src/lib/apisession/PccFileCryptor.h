...
class PccFileCryptor : public DgcObject {
   ...
   protected:
   public:
    PccFileCryptor(const dgt_schar* pgm_name = 0,
                   const dgt_schar* crypt_mode = 0, dgt_sint32 trace_level = 0,
                   dgt_sint32 manager_id = 0);
    virtual ~PccFileCryptor();
    inline const dgt_schar* errString() { return ErrString; };
    inline dgt_sint32 errCode() { return LastErrCode; };
    inline dgt_void setCryptMode(const dgt_schar* crypt_mode) {
        SearchEngineFactory.setCryptMode(crypt_mode);
    };
    inline dgt_void setMaxDetection(dgt_sint64 max_detection) {
        SearchEngineFactory.setMaxDetection(max_detection);
    };
    inline dgt_void setProgramName(const dgt_schar* pgm_name) {
        CryptorFactory.setProgramName(pgm_name);
    };
    inline dgt_void setOsUser(const dgt_schar* os_user) {
        CryptorFactory.setOsUser(os_user);
    };
    inline dgt_void setSessionID(dgt_sint32 sid) {
        CryptorFactory.setSessionID(sid);
    };
    ...
    dgt_sint32 crypt(const dgt_schar* in_file = 0,
                     const dgt_schar* out_file = 0);
    dgt_sint32 crypt(dgt_sint32 sid, const dgt_schar* parameters,
                     const dgt_schar* in_file = 0,
                     const dgt_schar* out_file = 0, dgt_sint32 agent_mode = 0,
                     const dgt_schar* enc_col_name = 0,
                     const dgt_schar* header_flag = 0,
                     dgt_sint32 buffer_size = 0);
    dgt_sint32 detect(const dgt_schar* parameters,
                      const dgt_schar* in_file = 0);
    dgt_sint32 detect(dgt_sint32 sid, const dgt_schar* parameters,
                      const dgt_schar* in_file = 0,
                      dgt_sint64 max_detection = 10, dgt_sint64 job_id = 0,
                      dgt_sint64 dir_id = 0);
};
