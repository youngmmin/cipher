...
class PccFileMemMap : public DgcObject {
  private:
	...
	dgt_sint32 openFile(const dgt_schar* file_name, dgt_sint32 o_flag) throw(DgcExcept);
  ...
  public:
	...
	inline dgt_uint8 isLoaded() { if (Address) return 1; return 0; };
	inline dgt_void* address() { return (dgt_void*)Address; };

	dgt_sint32 load(const dgt_schar* file_name, dgt_sint32 size, dgt_uint8* buf, dgt_uint8 create_flag=1) throw(DgcExcept);
