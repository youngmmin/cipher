...
PccFileMerger::PccFileMerger(PccCryptorFactory& cf, const dgt_schar* file_name,
                             dgt_sint64 run_size, dgt_sint32 file_flag)
    : DgcFileStream(file_name, file_flag, 0644),
      CryptorFactory(cf),
      BinaryFlag(cf.binaryFlag()),
      FileName(file_name),
      RunSize(run_size),
      NumRuns(0),
      CopyBuffer(0) {
...
#ifndef WIN32
            rtn_run =
                new PccRunStream(FileName, O_RDWR, 0644, (ith - 1) * RunSize);
#else
            rtn_run = new PccRunStream(FileName, O_RDWR | _O_BINARY, 0644,
                                       (ith - 1) * RunSize);
#endif
...
#ifndef WIN32
            rtn_run = new PccRunStream(run_file_name,
                                       O_CREAT | O_TRUNC | O_RDWR, 0644);
#else
            rtn_run = new PccRunStream(
                run_file_name, O_CREAT | O_TRUNC | O_RDWR | _O_BINARY, 0644);
#endif
    ...
    for (dgt_sint32 i = 1; i < NumRuns; i++) {
        ...
        while ((rbytes = Runs[i]->recvData(CopyBuffer, COPY_BUF_SIZE)) > 0) {
            if ((rbytes = sendData(CopyBuffer, rbytes)) < 0) {
                ATHROWnR(
                    DgcError(SPOS, "sendData[%s] or sendData failed", FileName),
                    -1);
                break;
            }
        }
        ATHROWnR(DgcError(SPOS, "recvData[%s] failed",
                          ((PccRunStream*)Runs[i])->fileName()),
                 -1);

        if (unlink(((PccRunStream*)Runs[i])->fileName())) {
            if (CryptorFactory.traceLevel() > 10)
                DgcWorker::PLOG.tprintf(0, "mergeRuns:unlink[%s] failed:%s\n",
                                        ((PccRunStream*)Runs[i])->fileName(),
                                        strerror(errno));
        }
    }
        ...
        if (unlink(((PccRunStream*)Runs[i])->fileName())) {
            if (CryptorFactory.traceLevel() > 10)
                DgcWorker::PLOG.tprintf(
                    0, "removeRunFiles:unlink[%s] failed:%s\n",
