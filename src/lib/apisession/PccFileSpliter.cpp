...
PccFileSpliter::PccFileSpliter(const dgt_schar* file_name,PccPttnSearchEngine* se,PccCryptorFactory& cf)
#ifndef WIN32
	: DgcFileStream(file_name,O_RDONLY), FileName(file_name),SearchEngine(se),CryptorFactory(cf),CurrOffset(0),CurrRuns(0)
#else
	: DgcFileStream(file_name,O_RDONLY|_O_BINARY), FileName(file_name),SearchEngine(se),CryptorFactory(cf),CurrOffset(0),CurrRuns(0)
#endif
{
	if (EXCEPT) {
		FileName = file_name;
...
#ifndef WIN32
			if ((NumRuns=(dgt_sint32)floor(DgcFileStream::fileSize()/CryptorFactory.runSize())) == 0) NumRuns = 1;
...
dgt_sint32 PccFileSpliter::computeNextOffset() throw(DgcExcept)
{
	...
	if (DgcFileStream::seek(CurrOffset,SEEK_SET) < 0) { // move to the next run's starting point
		delete buffer;
		ATHROWnR(DgcError(SPOS,"seek failed"),-1);
	}
	dgt_sint32 rbytes;
	if ((rbytes=DgcFileStream::recvData(buffer,crypt_buf.SrcLength)) < 0) { // read a buffer
		delete buffer;
		ATHROWnR(DgcError(SPOS,"recvData failed"),-1);
	} else {
		crypt_buf.SrcLength = rbytes;
	}
	CurrOffset += (crypt_buf.SrcLength - SearchEngine->getHandoverSize(&crypt_buf)); // adjust CurrOffset
	delete buffer;
	return 0;
}
...
#ifndef WIN32
	PccRunStream* rtn_run = new PccRunStream(FileName,O_RDONLY,0666,start_offset,CurrOffset-start_offset);
#else
	PccRunStream* rtn_run = new PccRunStream(FileName,O_RDONLY|_O_BINARY,0666,start_offset,CurrOffset-start_offset);
#endif
