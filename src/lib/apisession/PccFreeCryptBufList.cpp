...
PccFreeCryptBufList::PccFreeCryptBufList(dgt_sint32 crypt_mode,dgt_sint32 num_buffers,dgt_sint32 src_buf_len)
: SeqNo(0),SrcDatas(0),DstDatas(0),CryptBuffers(0),FirstFree(0),LastFree(0),
	NumBuffers(num_buffers),SrcBufLen(src_buf_len), DstBufLen(0),CryptMode(crypt_mode)
{
	...
	if ((SrcDatas=new dgt_uint8[(SrcBufLen+1)*NumBuffers]) == 0) {
		DgcWorker::PLOG.tprintf(0,"fail to get memory[(%d)*%d] for text buffer datas.\n",SrcBufLen,NumBuffers);
		return;
	}
	// create the array of destination buffers
	if (CryptMode == PFC_CRYPT_MODE_ENCRYPT) DstBufLen = SrcBufLen*3*1.4; // in case of encrypting the length of output is bigger than input
	else if (CryptMode == PFC_CRYPT_MODE_DECRYPT || CryptMode == PFC_CRYPT_MODE_MIGRATION) DstBufLen = SrcBufLen;
	else {
