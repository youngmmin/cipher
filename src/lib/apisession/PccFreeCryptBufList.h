...
class PccFreeCryptBufList : public DgcObject {
   ...
   public:
    PccFreeCryptBufList(dgt_sint32 crypt_mode, dgt_sint32 num_buffers,
                        dgt_sint32 src_buf_len);
    virtual ~PccFreeCryptBufList() {
        delete[] CryptBuffers;
        delete DstDatas;
        delete SrcDatas;
    }
...
class PccCryptBufFifoQueue {
   ...
   public:
    ...
    dgt_void setTotalBuffers(dgt_uint64 total_buffers) {
    ...
    dgt_sint32 get(PccCryptBuffer** rtn, PccWaiter* waiter = 0) {
        *rtn = 0;
        if (DgcSpinLock::lock(&QueueLock) == 0) {
            ...
            if (TotalBuffers && (Size == 0 || OutCount))
                return -1;  // the end of job to do
...
class PccCryptBufSortQueue : public DgcObject {
   ...
   public:
    ...
    PccCryptBuffer* put(
        PccCryptBuffer* crypt_buf)  // return a crypted buffer list if the
                                    // crypt_buf is the next crypted buffer
    {
        PccCryptBuffer* rtn = 0;
        if (crypt_buf) {
            for (;;) {  // until holding the queue lock
                if (DgcSpinLock::lock(&QueueLock) == 0) {
                    *(BufferPtrs + ((HeaderIdx + crypt_buf->SeqNo - Sequence) %
                                    NumBuffers)) =
                        crypt_buf;  // put in the queue
                    Size++;
                    InCount++;
                        ...
                        do {
                            ...
                            *(BufferPtrs + HeaderIdx) = 0;
                            ...
                            HeaderIdx = (HeaderIdx + 1) %
                                        NumBuffers;  // move the HeaderIdx to
                                                     // the next slot
                            Sequence++;
...
class PccCryptList {
   ...
   protected:
   public:
    PccCryptList(dgt_sint32 num_buffers) : SortQueue(num_buffers) {
        DgcSpinLock::unlock(&ListLock);
    }
    virtual ~PccCryptList() {}
    PccCryptBufFifoQueue& writeQueue() { return WriteQueue; };
    dgt_void setTotalBuffers(dgt_uint64 total_buffers) {
        WriteQueue.setTotalBuffers(total_buffers);
    }
    dgt_sint32 sortQueueSize() { return SortQueue.size(); }
    dgt_uint64 missCount() {
        return SortQueue.missCount() + WriteQueue.missCount();
    }
    ...
    dgt_void put(PccCryptBuffer* crypt_buf) {
        for (;;) {  // until holding the queue lock
            if (DgcSpinLock::lock(&ListLock) == 0) {
                if ((crypt_buf = SortQueue.put(crypt_buf)))
                    WriteQueue.put(crypt_buf);
                DgcSpinLock::unlock(&ListLock);
                break;
            }
        }
    ...
    dgt_sint32 get(PccCryptBuffer** rtn, PccWaiter* waiter = 0) {
        return WriteQueue.get(rtn, waiter);
    }
};
