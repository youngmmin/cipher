...
class PccHashNode {
  ...
  protected:
  public:
	PccHashNode(dgt_uint64 key,dgt_uint8* value) : Key(key),Value(value),Next(0) {}
	~PccHashNode() {}
	...
	dgt_void reset(dgt_uint64 key=0,dgt_void* value=0) { Key=key; Value=value; Next=0; }
};
...
class PccHashTable : public DgcObject {
  ...
  public:
	...
	inline dgt_void rewind() { CurrPos = -1; CurrNode = 0; }
