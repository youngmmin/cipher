...
class PccHeaderManager : public DgcObject {
  private :
  protected : // added by faoji on 2018. 07. 08. to move private properties into protected region
      dgt_uint8                 Version                             ;
  ...
  public :
	...
	dgt_sint32 checkHeader(dgt_schar* in_file, dgt_header_info *header_info = 0);
	...
	virtual dgt_sint32 checkHeader(DgcFileStream* in_stream, dgt_header_info *header_info = 0);
	virtual dgt_sint32 writeHeader(DgcFileStream* out_stream, dgt_uint8 version = 1,dgt_sint64 infile_size = 0, dgt_sint32 buffer_size = 0, dgt_sint64 enc_zone_id = 0, dgt_sint64 key_id = 0);
	virtual dgt_sint32 writeRTHeader(DgcFileStream* out_stream, dgt_uint8 version = 3);
	virtual dgt_sint32 commitHeader();
#endif
	dgt_sint64 inFileSize() { if (InFileSize) return InFileSize; return 0; }
	dgt_sint32 bufferSize() { if (BufferSize) return BufferSize; return 0; }
	dgt_sint64 encZoneId() { if (EncZoneId) return EncZoneId; return 0; }
	dgt_sint64 keyId() { if (KeyID) return KeyID; return 0; }
	dgt_uint16 headerSize() { if (HeaderSize) return HeaderSize; return 0; }
    ...
    static dgt_sint32 isEncrypted(const dgt_schar* file_path, dgt_header_info *header_info = 0) 
    {
		dgt_sint32 rtn = 0;
#ifndef WIN32
