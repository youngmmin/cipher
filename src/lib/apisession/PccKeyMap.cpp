...
dgt_void PccKeyMap::addKeyMap(const dgt_schar* enc_name,
...
dgt_void PccKeyMap::addVirtualKeyMap(dgt_sint64 vkey_id,
                                     dgt_schar* col_string) {
    dgt_schar* last;
#ifndef WIN32
    dgt_schar* cp = strtok_r(col_string, " ,\f\n", &last);
...
dgt_void PccKeyMap::setVKeyTargetEnv(const dgt_schar* target_file) {
    ...
    if (os_user) {
        strncpy(OsUser, dg_getenv("LOGNAME"),
                dg_strlen(os_user) > 64 ? 64 : dg_strlen(os_user));
    }
#else
