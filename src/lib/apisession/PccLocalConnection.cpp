...
PccLocalConnection::PccLocalConnection(dgt_uint16 in_timeout,
                                       dgt_uint16 out_timeout,
                                       dgt_uint32 max_open_cursors)
...
dgt_sint8 PccLocalConnection::connect(dgt_schar* shome, const dgt_schar* sid,
                                      const dgt_schar* uid,
                                      const dgt_schar* pswd,
                                      const dgt_schar* pname) throw(DgcExcept) {
    if (status() == DGC_CON_ST_OPEN) {
        THROWnR(
            DgcDbNetExcept(
                DGC_EC_DN_INVALID_ST,
                new DgcError(SPOS,
                             "invalid status[%d] for connect, disconnect first",
                             status())),
            -1);
    } else {
    ...
    dg_sprintf(cstring,
               "(address=(protocol=beq)(soha_home=%s)(soha_svc=%s)(svr_name="
               "pcp_kredsvr)(in_timeout=%d)(out_timeout=%d)(db_name=%s))",
               shome, sid, InTimeOut, OutTimeOut, sid);
    ...
    if (status() != DGC_CON_ST_OPEN) {
        THROWnR(
            DgcDbNetExcept(DGC_EC_DN_INVALID_ST,
                           new DgcError(SPOS, "not connected, connect first")),
            0);
    }
