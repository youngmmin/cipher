...
dgt_void PccReader::in() throw(DgcExcept)
{
	...
	dgt_sint32 nbytes = 0;
	if ((nbytes=InStream->recvData(CurrBuf->SrcDataPtr,CurrBuf->SrcLength)) < 0) { // empty file
		*LastErrCode = -66001;
		ATHROW(DgcError(SPOS,"recvData failed"));
	} else if (nbytes < CurrBuf->SrcLength) {
		CurrBuf->SrcLength = nbytes;
	}
	*(CurrBuf->SrcDataPtr+CurrBuf->SrcLength) = 0; // null terminating for pattern search
	if ((RemainBytes-=nbytes) < 0) {
		*LastErrCode = -66002;
		THROW(DgcBgmrExcept(DGC_EC_BG_INCOMPLETE,new DgcError(SPOS,"file not closed, getting smaller[%lld]",RemainBytes)));
	}
}
...
dgt_sint32 PccReader::run() throw(DgcExcept)
{
	...
	if (RemainBytes) {
		...
		dgt_sint32 handover_size = RemainBytes ? SearchEngine->getHandoverSize(CurrBuf) : 0;
		...
		if (handover_size) {
			memcpy(NextBuf->SrcDataPtr,CurrBuf->SrcDataPtr+CurrBuf->SrcLength-handover_size,handover_size);
			cp += handover_size;
			buf_len -= handover_size;
			CurrBuf->SrcLength -= handover_size; // decrease SrcLength by handover_size
			*(CurrBuf->SrcDataPtr + CurrBuf->SrcLength) = 0; // null terminating SrcDataPtr for pattern search
		}
		NextBuf->SrcLength = handover_size;
	}

	// put the CurrBuf into DataQueue for Ciphers
	if (RemainBytes == 0) CurrBuf->LastFlag = 1;	//2017.09.13 added by shson for tail_line
	...
	if (RemainBytes) { // read data into the next buffer
		...
		if ((nbytes=InStream->recvData(cp,buf_len)) < 0) {
			...
			*LastErrCode = -66004;
			THROWnR(DgcBgmrExcept(DGC_EC_BG_INCOMPLETE,new DgcError(SPOS,"file not closed, getting smaller[%lld]",RemainBytes)),-1);
		}
		if ((RemainBytes-=nbytes) < 0) {
			*LastErrCode = -66005;
			THROWnR(DgcBgmrExcept(DGC_EC_BG_INCOMPLETE,new DgcError(SPOS,"file not closed, getting bigger[%lld]",RemainBytes)),-1);
		}
		NextBuf->SrcLength += nbytes;
		*(NextBuf->SrcDataPtr + NextBuf->SrcLength) = 0; // null terminating for pattern search
	}
...
dgt_sint32 PccReader::readAndHandover(PccCryptBuffer** rtn_buf) throw(DgcExcept)
{
	if (StopFlag) return 1;
	//if file seek pointer 0 and header_flag = on, 
	//reader is skip header size
	...
	if (CurrBuf == 0 && RemainBytes) {
		...
		if ((nbytes=InStream->recvData(CurrBuf->SrcDataPtr,CurrBuf->SrcLength)) < 0) { // empty file
			*LastErrCode = PFC_RD_ERR_CODE_RECV_DATA_FAILED;
			...
			if (StreamFlag == 0) CurrBuf->SrcLength = nbytes;
			else memset(CurrBuf->SrcDataPtr+nbytes, 0, CurrBuf->SrcLength - nbytes);
		}
		*(CurrBuf->SrcDataPtr+CurrBuf->SrcLength) = 0; // null terminating for pattern search
		if ((RemainBytes-=nbytes) < 0) {
			if (StreamFlag == 0) {
	...
	if (RemainBytes == 0 && CurrBuf) CurrBuf->LastFlag = 1;	//2017.09.13 added by shson for tail_line
	...
	if (RemainBytes) { // handover to and read into the next buffer
		...
		if (StreamFlag == 0) NextBuf->SrcLength += nbytes;
		else {
			NextBuf->SrcLength += buf_len;
			memset(NextBuf->SrcDataPtr+nbytes, 0, NextBuf->SrcLength - nbytes);
		}
		*(NextBuf->SrcDataPtr + NextBuf->SrcLength) = 0; // null terminating for pattern search
	}
	*rtn_buf = CurrBuf;
