...
class PccReader : public DgcWorker {
  ...
  protected:
  public:
	PccReader(DgcFileStream* in_stream,PccFreeCryptBufList& cbl,PccPttnSearchEngine* pse,PccCryptBufFifoQueue& dq,dgt_uint16 header_size,dgt_sint32* last_err_code, dgt_sint32 stream_flag = 0)
  	  : InStream(in_stream), CryptBufList(cbl), SearchEngine(pse), DataQueue(dq), HeaderSize(header_size), LastErrCode(last_err_code),
		CurrBuf(0), NextBuf(0), FreeListWaits(0), ReadBuffers(0), RemainBytes(0), StreamFlag(stream_flag), StopFlag(0) 
	{ 
	...
	inline void setRemainBytes(dgt_sint64 remain_bytes) { RemainBytes = remain_bytes; };
