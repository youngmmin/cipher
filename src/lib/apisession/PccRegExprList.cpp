...
PccRegExprList::~PccRegExprList()
{
        ...
        exp_type* curr_preg;
        ExprList.rewind();
        while((curr_preg=(exp_type*)ExprList.nextObject())) regfree(&curr_preg->reg);
#endif
}

dgt_sint32 PccRegExprList::compileStr(const dgt_schar* str,dgt_schar* err_string)
{
        dgt_sint32 errcode;
        ...
        if (curr_preg) {
        	...
        	if ((errcode=regcomp(&tmp_preg, str, REG_EXTENDED))) {
	                ...
	                sprintf(err_string,"pattern[%s] compilation failed:[%s]",str,ErrBuffer);
	                return -1;
	        }
