...
PccRunStream::PccRunStream(const dgt_schar* file_name, dgt_sint32 oflag,
                           mode_t perm, dgt_sint64 start_offset,
                           dgt_sint64 length)
    : DgcFileStream(file_name, oflag, perm),
...
dgt_sint32 PccRunStream::recvData(dgt_uint8* buf, dgt_sint32 len,
                                  dgt_sint32 timeout) throw(DgcOsExcept) {
    dgt_sint32 rbytes;
    if (Length) {
        // limited run
        if (Remains >= (dgt_uint32)len)
            rbytes = DgcFileStream::recvData(buf, len, timeout);
        else if (Remains == 0)
            rbytes = 0;
        else
            rbytes = DgcFileStream::recvData(buf, Remains, timeout);
...
dgt_sint32 PccRunStream::sendData(const dgt_uint8* buf, dgt_sint32 len,
                                  dgt_sint32 timeout) throw(DgcOsExcept) {
    dgt_sint32 wbytes;
    if (Length) {
        // limited run
        if (Remains >= (dgt_uint32)len)
            ...
            wbytes = DgcFileStream::sendData(buf, Remains, timeout);
