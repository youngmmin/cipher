...
// pttn search engine
PccPttnSearchEngine::PccPttnSearchEngine(dgt_sint32 max_pattern_len,dgt_sint32 max_line_len)
...
PccRegExprSearchEngine::PccRegExprSearchEngine(dgt_sint32 max_pattern_len,dgt_sint32 max_line_len) : PccPttnSearchEngine(max_pattern_len,max_line_len)
{
...
dgt_sint32 PccRegExprSearchEngine::pttnSearch(dgt_schar* buf,PccSortedSegList* seg_list, dgt_uint32 buf_size, dgt_uint64 seq_no)
{
	exp_type*	preg;
	...
	while(1)
	{
		for(dgt_uint16 col_no=0; col_no<MAX_ENC_COLS; col_no++) {
			...
			if (ExprList[col_no]) {
				...
				while((preg=ExprList[col_no]->nextPttn())) { // each regular expression
					expr_no++;
					dgt_schar* str = working_buf; // search target string
					dgt_sint32 str_os = buf_offset; // search target string offset
//printf("str [%s], str_os [%d] strlen(str) [%d]\n",str,str_os,(dgt_sint32)strlen(str));
#ifndef WIN32
					for(;str;) {
						int errcode;
						if ((errcode=regexec(&preg->reg,str,NUM_PTTN,PttnMatch,0))) {
							if (errcode != REG_NOMATCH) {
								memset(ErrBuffer,0,MAX_ERR_STRING);
								regerror(errcode, &preg->reg, ErrBuffer, MAX_ERR_STRING-1);
								DgcWorker::PLOG.tprintf(0,"regexec failed:[%s]\n",ErrBuffer);
							}
							str = 0;
						} else {
							dgt_sint32 i;
							for(i=0; i<NUM_PTTN; i++) {
								if (PttnMatch[i].rm_so >= 0) {
									...
									seg_list->add(new PccSegment(PttnMatch[i].rm_so+str_os,
											PttnMatch[i].rm_eo+str_os,PccSegment::SEG_T_PTTN,col_no+1,expr_no,(dgt_schar*)preg->exp));
									num_pttn++;
									pttn_len = PttnMatch[i].rm_eo - PttnMatch[i].rm_so;
								}
							}
					...
					try {
						...
						while (std::tr1::regex_search(tmp_str, m, pattern)) {
							// found a pattern text segment
							std::string tmp=m[0];
							seg_list->add(new PccSegment(m.position()+str_os,
									m.position()+tmp.length()+str_os,PccSegment::SEG_T_PTTN,col_no+1));
							num_pttn++;
							tmp_str = m.suffix();
		...
		if(buf_size <= buf_offset) break;
		else working_buf=buf + buf_offset;
	} //while(1)
	return num_pttn;
}

dgt_sint32 PccRegExprSearchEngine::addRegExpr(dgt_sint32 col_no,const dgt_schar* reg_expr,dgt_schar* err_string)
{
	if (col_no >= MAX_ENC_COLS) return 0; // column number out of MAX_ENC_COLS, which case is automatically discarded
...
//	if (MaxLineLen && crypt_buf->SrcLength >= MaxLineLen) return getLineHandoverSize(crypt_buf);
	...
	if (MaxPatternLen && crypt_buf->SrcLength >= MaxPatternLen) {
		//
		// "MaxLineLen > 0" or "MaxPatternLen == 0" means there's no split pattern
		//
		if (crypt_buf->SegList == 0) crypt_buf->SegList = new PccSortedSegList();
		//
		// split pattern search
		//
		dgt_sint32 split_pttn_area_offset = (crypt_buf->SrcLength - (MaxPatternLen*2 - 1));
		dgt_schar* split_pttn_area = (dgt_schar*)crypt_buf->SrcDataPtr + split_pttn_area_offset;
		if (pttnSearch(split_pttn_area,crypt_buf->SegList,crypt_buf->SrcLength - split_pttn_area_offset,crypt_buf->SeqNo)) {
			//
			...
			while((seg=crypt_buf->SegList->next())) {
				...
				if (handover_offset < seg->eOffset()) handover_offset = seg->eOffset();
			}
			handover_size = crypt_buf->SrcLength - handover_offset;
			crypt_buf->FirstSplitPttnChar = *(crypt_buf->SrcDataPtr + crypt_buf->FirstSplitPttnOffset);
			*(crypt_buf->SrcDataPtr + crypt_buf->FirstSplitPttnOffset) = 0; // from prevent double pattern search
		} else {
			// not found
...
dgt_sint32 PccRegExprSearchEngine::patternSearch(PccCryptBuffer* crypt_buf)
{
	...
	dgt_sint32 num_pttn = pttnSearch((dgt_schar*)crypt_buf->SrcDataPtr,crypt_buf->SegList, crypt_buf->SrcLength,crypt_buf->SeqNo);
	crypt_buf->SegList->complete(crypt_buf->SrcLength);
	if (crypt_buf->FirstSplitPttnChar)
		*(crypt_buf->SrcDataPtr + crypt_buf->FirstSplitPttnOffset) = crypt_buf->FirstSplitPttnChar;
	return num_pttn;
}
...
PccFormatSearchEngine::PccFormatSearchEngine(dgt_sint32 max_pattern_len,dgt_sint32 max_line_len)
	: PccPttnSearchEngine(max_pattern_len,max_line_len), NumCryptCols(0), RowDelimiterLen(0), ContinueDelimiterLen(0), IBK_flag(0), MultiRecordLength(0), HeaderLine(0), TailLine(0), HeaderSkipSize(0), TailSkipSize(0)
{
}
...
dgt_uint8 PccFormatSearchEngine::isRowDelimiter(dgt_uint8 *cv)
{
	...
	if (strlen(cp) < RowDelimiterLen)
		return 1;
#endif
	if (strncmp((dgt_schar*)RowDelimiters,cp,strlen((dgt_schar*)RowDelimiters)) == 0) {
		IBK_flag = 1;
		return 1;
	}
	//*************************important***************************************
...
//added 2017.08.14 by shson - exist history with IBK bank, check with shson
	if (IBK_flag == 0) {
		if (strncmp((dgt_schar*)CRRowDelimiters,cp,strlen((dgt_schar*)CRRowDelimiters)) == 0) {
	...
	//dgt_uint8* cp;
	//for(dgt_sint32 i=0; i<MAX_DELIMITERS && (cp=*(Delimiters+i)); i++) if (cp == cv) return 1;
	return 0;
}

dgt_uint8 PccFormatSearchEngine::isIncludeContinueDelimiter(dgt_uint8 *cv,dgt_sint32 col_len)
{
	if(strlen((dgt_schar*)ContinueDelimiters) == 0) return 0;
	dgt_schar* cp;
	cp = (dgt_schar*)cv;
//printf("cv [%s] return [%d]\n",cp,strncmp((dgt_schar*)ContinueDelimiters,cp,strlen((dgt_schar*)ContinueDelimiters)));
	if (strncmp((dgt_schar*)ContinueDelimiters,cp,strlen((dgt_schar*)ContinueDelimiters)) == 0) {
		IBK_flag = 1;
		return 1; //[continue delimiter]column
	}
	if (strncmp((dgt_schar*)ContinueDelimiters,cp +col_len-strlen((dgt_schar*)ContinueDelimiters),strlen((dgt_schar*)ContinueDelimiters)) == 0) {
		IBK_flag = 1;
...
{
	if (col_no && NumCryptCols < MAX_ENC_COLS) CryptCols[NumCryptCols++] = col_no;
	return NumCryptCols;
}

dgt_void PccFormatSearchEngine::addRowDelimiters(const dgt_schar* row_delimiters)
{
	...
	for(dgt_sint32 i = 0 ; tmp_delimiter[i] != 0x00 ; i++)
	{
		if (tmp_delimiter[i] == '\\' ) { // convert escape character
			...
			for(dgt_sint32 idx = i + 1 ; idx < MAX_DELIMITERS - 2 ; idx++) //pull up tmp_delimiter
			{
...
dgt_void PccFormatSearchEngine::addContinueDelimiters(const dgt_schar* continue_delimiters)
{
#ifndef WIN32
...
dgt_sint32 PccFormatSearchEngine::addMultiLecordLength(dgt_schar* record_length)
{
		MultiRecordLength = strtol(record_length,0,10);
		return 0;
}

dgt_sint32 PccFormatSearchEngine::getSkipSize(PccCryptBuffer* crypt_buf, dgt_sint32 start_flag)
{
	...
	//0 is started back search to buffer tail
		...
		if (start_flag == 0) {//when last_buffer
			...
			for(dgt_uint32 i = 1/*null size*/; i <= (dgt_uint32)crypt_buf->SrcLength; i++) { //when exist header_line parameter
				if (isRowDelimiter(crypt_buf->SrcDataPtr + crypt_buf->SrcLength - i) == 1) {
					numDelimiter++;
					if (numDelimiter -1 == TailLine) {
						skip_size = i - rowDelimiterLen();
						break;
					} //if (numDelimiter -1 == TailLine) end
				} //if (isRowDelimiter(crypt_buf->SrcDataPtr + crypt_buf->SrcLength - i) end
			} //for(dgt_sint32 i = skip_size + 1/*null size*/; i <= crypt_buf->SrcLength; i++) end
		} else {//when first buffer
			if(HeaderSkipSize) {//when exist header_skip_size parameter
				return HeaderSkipSize;
			}
			for(dgt_uint32 i = 0; i < (dgt_uint32)crypt_buf->SrcLength; i++) { //when exist header_line parameter
...
dgt_sint32 PccFormatSearchEngine::getHandoverSize(PccCryptBuffer* crypt_buf)
{
		...
		if (MultiRecordLength) {	//2017.09.12 by shson for MultiRecordLength
			...
			handover_size =	(crypt_buf->SrcLength - header_skip_size)%MultiRecordLength;
			return handover_size;
		}
...
PccDelimiterSearchEngine::PccDelimiterSearchEngine(dgt_sint32 max_pattern_len,dgt_sint32 max_line_len)
...
dgt_sint32 PccDelimiterSearchEngine::patternSearch(PccCryptBuffer* crypt_buf)
{
	...
	if ((header_skip_size + tail_skip_size) >= (dgt_uint32)crypt_buf->SrcLength) { //for ibk, whole file is skiped
		crypt_buf->SegList->complete(crypt_buf->SrcLength);
		return 0;
	}
	for(dgt_sint32 col_offset=header_skip_size; col_offset<crypt_buf->SrcLength - (dgt_sint32)tail_skip_size; col_offset++) {
		...
		if (isDelimiter(cv) || isRowDelimiter(cv)) { // end of a column or line
			if (col_len && isCryptCol(col_no)) {
				dgt_uint8 except_flag = 0; // 1: exist except string to front 2: exist except string to back 0: not dxist except string
				except_flag = isIncludeContinueDelimiter(cv-col_len,col_len);
				if (except_flag == 1)
				crypt_buf->SegList->add(new PccSegment(col_offset-col_len+continueDelimiterLen(),col_offset,PccSegment::SEG_T_PTTN,col_no));
				else if (except_flag == 2)
				crypt_buf->SegList->add(new PccSegment(col_offset-col_len,col_offset-continueDelimiterLen(),PccSegment::SEG_T_PTTN,col_no));
				else
				crypt_buf->SegList->add(new PccSegment(col_offset-col_len,col_offset,PccSegment::SEG_T_PTTN,col_no));
				num_pttn++;
			}
	...
	crypt_buf->SegList->complete(crypt_buf->SrcLength);
	return NumColDelimiter == 0 ? PFC_SE_ERR_CODE_DILIMETER_NOT_FOUND : num_pttn;
}


// fixed search engine
PccFixedSearchEngine::PccFixedSearchEngine(dgt_sint32 max_pattern_len,dgt_sint32 max_line_len)
	: PccFormatSearchEngine(max_pattern_len,max_line_len),NumCols(1),LeadSpaceTrimFlag(0),TailSpaceTrimFlag(0)
{
...
if ((header_skip_size + tail_skip_size) >= (dgt_uint32)crypt_buf->SrcLength) {
	crypt_buf->SegList->complete(crypt_buf->SrcLength);
	return 0;
}
#endif
	do {
		...
		for(dgt_uint16 i=0; i<NumCryptCols; i++) {
			...
			if (CryptCols[i] && CryptCols[i] <= NumCols) { // valid column number
				...
				if (LeadSpaceTrimFlag) { // leading space trimming
					while(*(crypt_buf->SrcDataPtr+line_offset+col_soffset+lsc) == ' ' && (lsc < col_eoffset - col_soffset)) lsc++;
					if (lsc && (lsc < col_eoffset - col_soffset)) { // add a passing segment for discarding leading space
						crypt_buf->SegList->add(new PccSegment(line_offset+col_soffset,
									line_offset+col_soffset+lsc,PccSegment::SEG_T_PASS));
					}
				}
				if (TailSpaceTrimFlag) { // tailing space trimming
					while(*(crypt_buf->SrcDataPtr+line_offset+col_eoffset-1-tsc) == ' ' && (tsc < col_eoffset - col_soffset)) tsc++;
					if (tsc && (tsc < col_eoffset - col_soffset)) { // add a passing segment for discarding leading space
						crypt_buf->SegList->add(new PccSegment(line_offset+col_eoffset-tsc,
									line_offset+col_eoffset,PccSegment::SEG_T_PASS));
					}
				}

				if ((lsc == col_eoffset - col_soffset) || (tsc == col_eoffset - col_soffset)) { //this case is only filled ' '(space) in segment
					crypt_buf->SegList->add(new PccSegment(line_offset+col_soffset,
								line_offset+col_eoffset,PccSegment::SEG_T_PTTN_NULL,CryptCols[i]));
				} else {
				crypt_buf->SegList->add(new PccSegment(line_offset+col_soffset+lsc,
							line_offset+col_eoffset-tsc,PccSegment::SEG_T_PTTN,CryptCols[i]));
				}
		...
		while((line_offset < crypt_buf->SrcLength) && isRowDelimiter(crypt_buf->SrcDataPtr+line_offset++) != 1); // move to the end of line
		if ((line_offset < crypt_buf->SrcLength)) line_offset+=rowDelimiterLen()-1; //move offset so rowDelimiterLen, -1 is added because of first line_offset++ to upline
		} else { //MultiRecordLength encrypt case
			if (MultiRecordLength == header_skip_size) header_skip_size = 0; 
			while ((line_offset % MultiRecordLength) != header_skip_size) line_offset++; // move to the end of line
			if ((dgt_uint32)(crypt_buf->SrcLength - line_offset - tail_skip_size) < MultiRecordLength) break;
		}
	} while(line_offset < crypt_buf->SrcLength-1 - (dgt_sint32)tail_skip_size);
	crypt_buf->SegList->complete(crypt_buf->SrcLength);
	return num_pttn;
}
...
// factory
PccSearchEngineFactory::PccSearchEngineFactory(PccKeyMap& key_map,const dgt_schar* crypt_mode)
	: KeyMap(key_map),CryptMode(1),DetectMode(1),MaxLineLen(0),MaxPatternLen(0),MaxDetection(10),EngineType(WHOLE_ENCRYPTOR),NumEngine(0),NumRegExpr(0)
{
...
{
	if (crypt_mode && strlen(crypt_mode) && strncasecmp(crypt_mode,"decrypt",7) == 0) { //when decrypt mode
		CryptMode = 0;
		...
		if(EngineType == WHOLE_ENCRYPTOR)
            EngineType = WHOLE_DECRYPTOR;

	} else if (crypt_mode && strlen(crypt_mode) && strncasecmp(crypt_mode,"encrypt",7) == 0) { //when encrypt mode
		CryptMode = 1;
	} else if (crypt_mode && strlen(crypt_mode) && strncasecmp(crypt_mode,"migration",9) == 0) { //when migration mode
		CryptMode = 3;
...
dgt_sint32 PccSearchEngineFactory::initDelimiter(DgcBgrammer* dg,dgt_schar* err_string)
{
	if (EngineType >= WHOLE_DECRYPTOR) { // no engine defined yet
		dgt_schar* val = 0;
		dgt_sint32 max_line_len = 0;
		dgt_sint32 max_pattern_len = 0;
		if ((val=dg->getValue("delimiter.max_line_len"))) max_line_len = strtol(val,0,10);
		if ((val=dg->getValue("delimiter.max_pattern_len"))) max_pattern_len = strtol(val,0,10);
		if (CryptMode) EngineType = FORMAT_ENCRYPTOR;
		else EngineType = FORMAT_DECRYPTOR;
//		if (max_line_len && max_pattern_len && (val=dg->getValue("delimiter.chars")) && strlen(val)) {
		if ((val=dg->getValue("delimiter.chars")) && strlen(val)) {
			MaxLineLen = max_line_len;
			MaxPatternLen = max_pattern_len;
			PccDelimiterSearchEngine* tmp = new PccDelimiterSearchEngine(MaxPatternLen,MaxLineLen);
			...
			//except string set
			if ((val=dg->getValue("delimiter.continue_delimiter")) && strlen(val))
		...
		if ((val=dg->getValue("delimiter.header_line"))) tmp->setHeaderLine(strtol(val,0,10));
		if ((val=dg->getValue("delimiter.tail_line"))) tmp->setTailLine(strtol(val,0,10));
...
dgt_sint32 PccSearchEngineFactory::initFixed(DgcBgrammer* dg,dgt_schar* err_string)
{
	if (EngineType >= WHOLE_DECRYPTOR) { // no engine defined yet
		dgt_schar* val = 0;
		dgt_sint32 num = 0;
		if ((val=dg->getValue("fixed.max_line_len")) && (num=strtol(val,0,10))) MaxLineLen=num;
		if ((val=dg->getValue("fixed.max_pattern_len")) && (num=strtol(val,0,10))) MaxPatternLen=num;
		PccFixedSearchEngine* engine = new PccFixedSearchEngine(MaxPatternLen,MaxLineLen);
		if ((val=dg->getValue("fixed.col_offsets")) && strlen(val)) engine->addColOffsets(val);
		if ((val=dg->getValue("fixed.col_lengths")) && strlen(val)) engine->addColLengths(val);
		if ((val=dg->getValue("fixed.lead_space_trim")) && strcasecmp(val,"yes") == 0) engine->setLeadSpaceTrim(1);
		if ((val=dg->getValue("fixed.tail_space_trim")) && strcasecmp(val,"yes") == 0) engine->setTailSpaceTrim(1);

		//added row_delimiter and continue_delimiter by shson 2017.07.25 for IBK bank 
		//row delimiter set
		...
		if ((val=dg->getValue("fixed.header_line"))) engine->setHeaderLine(strtol(val,0,10));
		if ((val=dg->getValue("fixed.tail_line"))) engine->setTailLine(strtol(val,0,10));
		if ((val=dg->getValue("fixed.header_skip_size"))) engine->setHeaderSkipSize(strtol(val,0,10));
		if ((val=dg->getValue("fixed.tail_skip_size"))) engine->setTailSkipSize(strtol(val,0,10));
		...
		if ((val=dg->getValue("fixed.out_col_lengths")) && strlen(val)) KeyMap.addOutColLengths(val);
...
dgt_sint32 PccSearchEngineFactory::initRegular(DgcBgrammer* dg,dgt_schar* err_string)
{
	if (EngineType >= WHOLE_DECRYPTOR) { // no engine defined yet
		dgt_schar* val = 0;
		dgt_sint32 max_line_len = 0;
		...
		if ((val=dg->getValue("regular.max_pattern_len"))) max_pattern_len = strtol(val,0,10);
		...
		if ((val=dg->getValue("regular.max_line_len"))) max_line_len = strtol(val,0,10);
...
PccPttnSearchEngine* PccSearchEngineFactory::getEngine()
{
	...
	for(;;) {
		dgt_schar err_string[513] = {0,};
		if (DgcSpinLock::lock(&FactoryLock) == 0) {
			if (EngineType == PATTERN_ENCRYPTOR) {
				//
				// reglar expression search engine can't be shared because it needs state variables
				// each thread's PccCipher is need it
				// 
				PccRegExprSearchEngine*	tmp = new PccRegExprSearchEngine(MaxPatternLen,MaxLineLen);
				for(dgt_sint32 i=0; i<NumRegExpr; i++) {
					if (tmp->addRegExpr(RegExprs[i].col_no,RegExprs[i].expr_string,err_string) < 0) {
...
pr_debug("EngineType[%d] NumEngine[%d]\n",EngineType,NumEngine);
	...
	return engine ? engine : Engines[0];  // engine shoulde be only one in any case except reg_expr case
}
