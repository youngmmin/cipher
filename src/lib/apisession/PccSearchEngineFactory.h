...
class PccPttnSearchEngine : public DgcObject {
  ...
  public:
	...
	dgt_sint32 maxHandoverSize() { return MaxLineLen ? MaxLineLen : (MaxPatternLen*2 - 1); }
...
class PccRegExprSearchEngine : public PccPttnSearchEngine {
  ...
  protected:
	...
	virtual dgt_sint32 pttnSearch(dgt_schar* buf,PccSortedSegList* seg_list, dgt_uint32 buf_size, dgt_uint64 seq_no=0);
  public:
	PccRegExprSearchEngine(dgt_sint32 max_pattern_len,dgt_sint32 max_line_len);
	virtual ~PccRegExprSearchEngine();

	dgt_sint32 addRegExpr(dgt_sint32 col_no,const dgt_schar* reg_expr,dgt_schar* err_string);
...
class PccFormatSearchEngine : public PccPttnSearchEngine {
  ...
  protected:
	...
	dgt_uint8	IBK_flag; //added 2017.08.14 by shson - exist history with IBK bank, check with shson
	dgt_uint32	MultiRecordLength; //added 2017.09.12 by shson - for milti_record_length
	...
	dgt_uint32	HeaderSkipSize; //added 2017.10.11 by shson - for skip header_size
  ...
  public:
	...
	inline dgt_uint8 setHeaderLine(dgt_uint32 header_line) { return HeaderLine = header_line; }
	inline dgt_uint8 setTailLine(dgt_uint32 tail_line) { return TailLine = tail_line; }
	inline dgt_uint8 setHeaderSkipSize(dgt_uint32 header_skip_size) { return HeaderSkipSize = header_skip_size; }
	inline dgt_uint8 setTailSkipSize(dgt_uint32 tail_skip_size) { return TailSkipSize = tail_skip_size; }
...
class PccDelimiterSearchEngine : public PccFormatSearchEngine {
  private:
	...
	inline dgt_uint8 isDelimiter(dgt_uint8* cv) {
		dgt_schar* cp;
		cp = (dgt_schar*)cv;
		if (strncmp((dgt_schar*)Delimiters,cp,strlen((dgt_schar*)Delimiters)) == 0) {
			NumColDelimiter++;
			return 1;
		}
		return 0;
	...
	inline dgt_sint32 isCryptCol(dgt_uint16 col_no) {
		for(dgt_uint16 i=0; i<NumCryptCols; i++) if (CryptCols[i] == col_no) return 1;
		return 0;
	};
  protected:
  public:
	PccDelimiterSearchEngine(dgt_sint32 max_pattern_len,dgt_sint32 max_line_len);
...
class PccFixedSearchEngine : public PccFormatSearchEngine {
  ...
  public:
	...
	inline dgt_void setLeadSpaceTrim(dgt_sint32 flag) { LeadSpaceTrimFlag = flag; }
	inline dgt_void setTailSpaceTrim(dgt_sint32 flag) { TailSpaceTrimFlag = flag; }
	inline dgt_sint32 paramCheck(dgt_schar* err_string) {
		if (MaxLineLen == 0 && ColOffsets[NumCols]) MaxLineLen = ColOffsets[NumCols] + rowDelimiterLen()*3;
		if (MaxLineLen == 0) {
...
class PccSearchEngineFactory : public DgcObject {
  ...
  public :
	...
	dgt_void setDetectMode(dgt_sint32 detect_mode) { DetectMode = detect_mode; };
	dgt_void setMaxDetection(dgt_sint64 max_detection) { MaxDetection= max_detection; };
