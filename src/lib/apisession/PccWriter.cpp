...
dgt_sint32 PccWriter::write(PccCryptBuffer* curr) throw(DgcExcept) {
    ...
    if (StreamFlag) {
        ...
        if (curr->LastFlag == 1 && OrgSize)
            wbytes = OrgSize - OutStream->fileSize();
        else
            wbytes = curr->DstLength;
...
dgt_sint32 PccWriter::run() throw(DgcExcept) {
    ...
    while (crypted_bufs) {
        if ((nbytes = OutStream->sendData(crypted_bufs->DstDataPtr, wbytes)) <
            0) {
    ...
    CryptBufList.put(
        curr);  // return the crypted buffers into the free buffer list
    return 0;
}
