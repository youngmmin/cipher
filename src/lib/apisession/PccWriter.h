...
class PccWriter : public DgcWorker {
  ...
  protected:
  public:
	PccWriter(DgcFileStream* out_stream,PccCryptList& cq,PccFreeCryptBufList& cbl,dgt_sint32* last_err_code, dgt_sint32 stream_flag = 0)
  	  : OutStream(out_stream),CryptQueue(cq), CryptBufList(cbl), LastErrCode(last_err_code), CryptQueueWaits(0), WriteBuffers(0), StreamFlag(stream_flag), StopFlag(0), OrgSize(0) {}
	virtual ~PccWriter() {}
