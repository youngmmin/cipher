...
extern "C"
{
	...
	char *ext_pls_encrypt_b64_id(char *src, int src_len, int enc_col_id, int *err_code);
	char *ext_pls_decrypt_b64_id(char *src, int src_len, int enc_col_id, int *err_code);
	int opn_session();
	int db_sid = 0;
	...
	void entryfunction(char *func_name, int arg_count, void **args, void **returnArg)
	{
		...
		if (strcmp(func_name, "ext_pls_encrypt_b64_id") == 0)
		{
			if (returnArg != NULL)
			{
				*(char **)returnArg = ext_pls_encrypt_b64_id((char *)args[0], *((int *)args[1]), *((int *)args[2]), (int *)args[3]);
			}
		}
		else if (strcmp(func_name, "ext_pls_decrypt_b64_id") == 0)
		{
			if (returnArg != NULL)
			{
				*(char **)returnArg = ext_pls_decrypt_b64_id((char *)args[0], *((int *)args[1]), *((int *)args[2]), (int *)args[3]);
			}
		}
	}
	...
	char *ext_pls_encrypt_b64_id(char *src, int src_len, int enc_col_id, int *err_code)
	{
		...
		if (session->encrypt(enc_col_id, (dgt_uint8 *)src, src_len, (dgt_uint8 *)enc_buf, &dst_len) < 0)
		{
			//
	...
	char *ext_pls_decrypt_b64_id(char *src, int src_len, int enc_col_id, int *err_code)
	{
		...
		if (session->decrypt(enc_col_id, (dgt_uint8 *)src, src_len, (dgt_uint8 *)dec_buf, &dst_len) < 0)
		{
			//
			// encrypt failed, session has this last error code which can be gotten by ECODE
			//
