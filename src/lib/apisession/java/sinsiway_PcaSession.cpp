 ...
 * Signature: ([B[B)I
 */
JNIEXPORT jint JNICALL Java_sinsiway_PcaSession_INIT(JNIEnv *env, jclass jc,
													 ...
													 jbyteArray credentials_password)
{
	...
	dgt_schar cpw[33];
	dgt_sint32 blen;
	if ((blen = env->GetArrayLength(conf_file_path)) > 256)
		blen = 256;
	memset(cfp, 0, 257);
	env->GetByteArrayRegion(conf_file_path, 0, blen, (jbyte *)cfp);
	if ((blen = env->GetArrayLength(credentials_password)) > 32)
		blen = 32;
	memset(cpw, 0, 33);
 ...
 */
JNIEXPORT jint JNICALL Java_sinsiway_PcaSession_OPN(JNIEnv *env, jclass jc,
													jint db_sid,
													jbyteArray instance_name,
													jbyteArray db_name,
													jbyteArray client_ip,
													jbyteArray db_user,
													jbyteArray os_user,
													jbyteArray client_program,
													jint protocol,
													jbyteArray user_id,
													jbyteArray client_mac)
{
	pc_type_open_sess_in sess_in;
	dgt_sint32 blen;
	memset(&sess_in, 0, sizeof(sess_in));
	sess_in.db_sid = db_sid;
	if ((blen = env->GetArrayLength(instance_name)) > 32)
		blen = 32;
	env->GetByteArrayRegion(instance_name, 0, blen, (jbyte *)sess_in.instance_name);
	if ((blen = env->GetArrayLength(db_name)) > 32)
		blen = 32;
	env->GetByteArrayRegion(db_name, 0, blen, (jbyte *)sess_in.db_name);
	if ((blen = env->GetArrayLength(client_ip)) > 64)
		blen = 64;
	env->GetByteArrayRegion(client_ip, 0, blen, (jbyte *)sess_in.client_ip);
	if ((blen = env->GetArrayLength(db_user)) > 32)
		blen = 32;
	env->GetByteArrayRegion(db_user, 0, blen, (jbyte *)sess_in.db_user);
	if ((blen = env->GetArrayLength(os_user)) > 32)
		blen = 32;
	env->GetByteArrayRegion(os_user, 0, blen, (jbyte *)sess_in.os_user);
	if ((blen = env->GetArrayLength(client_program)) > 128)
		blen = 128;
	env->GetByteArrayRegion(client_program, 0, blen, (jbyte *)sess_in.client_program);
	sess_in.protocol = (dgt_uint8)protocol;
	if ((blen = env->GetArrayLength(user_id)) > 32)
		blen = 32;
	env->GetByteArrayRegion(user_id, 0, blen, (jbyte *)sess_in.user_id);
	if ((blen = env->GetArrayLength(client_mac)) > 64)
		blen = 64;
	...
	if (!session)
		return PcAPI_ERR_INVALID_SID;
	//	return session->openSession(sess_in.db_sid, sess_in.instance_name, sess_in.db_name, sess_in.client_ip, sess_in.db_user,
	//			sess_in.os_user, sess_in.client_program, sess_in.protocol, sess_in.user_id, sess_in.client_mac);
	dgt_sint32 rtn = session->openSession(sess_in.db_sid, sess_in.instance_name, sess_in.db_name, sess_in.client_ip, sess_in.db_user,
										  sess_in.os_user, sess_in.client_program, sess_in.protocol, sess_in.user_id, sess_in.client_mac);
	// added by shson 2018.10.01 for use api session for cryptFile
	dgt_sint32 api_sid = PcaApiSessionPool::getApiSession(sess_in.client_ip, "", sess_in.client_program, "", "", sess_in.os_user, 0);
 ...
 * Signature: (I)V
 */
JNIEXPORT void JNICALL Java_sinsiway_PcaSession_CLS(JNIEnv *env, jclass jc, jint db_sid)
{
 ...
 * Signature: (II[B)[B
 */
JNIEXPORT jbyteArray JNICALL Java_sinsiway_PcaSession_ENC__II_3B(JNIEnv *env, jclass jc,
																 jint db_sid,
																 jint enc_col_id,
																 jbyteArray src)
{
 ...
 * Signature: (I[B[B)[B
 */
JNIEXPORT jbyteArray JNICALL Java_sinsiway_PcaSession_ENC_1NM(JNIEnv *env, jclass jc,
															  jint db_sid,
															  ...
															  jbyteArray src)
{
	...
	dgt_schar ecn[132];
	memset(ecn, 0, 132);
	env->GetByteArrayRegion(enc_col_name, 0, env->GetArrayLength(enc_col_name), (jbyte *)ecn);
	env->GetByteArrayRegion(src, 0, in_len, (jbyte *)in_buffer);
 ...
 * Signature: (II[B)[B
 */
JNIEXPORT jbyteArray JNICALL Java_sinsiway_PcaSession_DEC__II_3B(JNIEnv *env, jclass jc,
																 jint db_sid,
																 jint enc_col_id,
																 jbyteArray src)
{
 ...
 * Signature: (I[B[B)[B
 */
JNIEXPORT jbyteArray JNICALL Java_sinsiway_PcaSession_DEC_1NM(JNIEnv *env, jclass jc,
															  jint db_sid,
															  jbyteArray enc_col_name,
															  jbyteArray src)
{
 ...
 * Signature: (I)I
 */
JNIEXPORT jint JNICALL Java_sinsiway_PcaSession_ECODE(JNIEnv *env, jclass jc, jint db_sid)
{
	PcaSession *session = PcaSessionPool::getSession(db_sid, 1);
	if (!session)
		return PcAPI_ERR_INVALID_SID;
 ...
 * Signature: (I[B[B[B)I
 */
JNIEXPORT jint JNICALL Java_sinsiway_PcaSession_CRYPTFILE(JNIEnv *env, jclass jc, jint db_sid, jbyteArray param_string, jbyteArray input_file_path, jbyteArray output_file_path)
{
	...
	if (cryptor.crypt(api_sid, (const dgt_schar *)parameters, (const dgt_schar *)in_file, (const dgt_schar *)out_file) < 0)
	{
 ...
 * Signature: ([B)I
 */
JNIEXPORT jint JNICALL Java_sinsiway_PcaSession_ISENCRYPTED(JNIEnv *env, jclass jc, jbyteArray file_name)
{
