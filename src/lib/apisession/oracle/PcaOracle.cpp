 ...
 */
JNIEXPORT jint JNICALL Java_PcaOracle_OPN(JNIEnv *env, jclass jc,
                                          jint db_sid,
 ...
 * Signature: (I[B)V
 */
JNIEXPORT void JNICALL Java_PcaOracle_CCS(JNIEnv *env, jclass jc, jint db_sid, jbyteArray char_set_bytes)
{
        ...
        if (session)
        {
                ...
                env->GetByteArrayRegion(char_set_bytes, 0, env->GetArrayLength(char_set_bytes), (jbyte *)char_set);
                                                   ...
                                                   jint src_enc_flag)
{
        ...
        dgt_uint8 *in_buffer = session->inBuffer(in_len);
        env->GetByteArrayRegion(src, 0, in_len, (jbyte *)in_buffer);
        if (session->OPHUEK(enc_col_id, in_buffer, in_len, &dst, &dst_len, src_enc_flag) < 0)
        {
                return 0;
        }
        else if (dst_len == 0)
                                                       ...
                                                       jint src_enc_flag)
{
        ...
        env->GetByteArrayRegion(enc_col_name, 0, env->GetArrayLength(enc_col_name), (jbyte *)ecn);
        env->GetByteArrayRegion(src, 0, in_len, (jbyte *)in_buffer);
        if (session->OPHUEK(ecn, in_buffer, in_len, &dst, &dst_len, src_enc_flag) < 0)
        {
                return 0;
        }
        else if (dst_len == 0)
                                                     ...
                                                     jbyteArray src)
{
        ...
        dgt_uint8 *in_buffer = session->inBuffer(in_len);
        env->GetByteArrayRegion(src, 0, in_len, (jbyte *)in_buffer);
        if (session->encryptCpn(enc_col_id, in_buffer, in_len, &dst, &dst_len) < 0)
        {
                return 0;
        }
        else if (dst_len == 0)
...
JNIEXPORT jbyteArray JNICALL Java_PcaOracle_ENC_1CPN_1NM(JNIEnv *env, jclass jc,
                                                         jint db_sid,
                                                         jbyteArray enc_col_name,
                                                         jbyteArray src)
{
                                                     ...
                                                     jbyteArray src)
{
        ...
        dgt_uint8 *in_buffer = session->inBuffer(in_len);
        env->GetByteArrayRegion(src, 0, in_len, (jbyte *)in_buffer);
        if (session->decryptCpn(enc_col_id, in_buffer, in_len, &dst, &dst_len) < 0)
        {
                return 0;
        }
        else if (dst_len == 0)
...
JNIEXPORT jbyteArray JNICALL Java_PcaOracle_DEC_1CPN_1NM(JNIEnv *env, jclass jc,
                                                         jint db_sid,
                                                         jbyteArray enc_col_name,
                                                         jbyteArray src)
{
                                             ...
                                             jint sql_type)
{
        ...
        dgt_schar sh[65];
        dgt_sint32 blen;
        if ((blen = env->GetArrayLength(sql_hash)) > 64)
                blen = 64;
        memset(sh, 0, 65);
 ...
 * Signature: (I[BI)V
 */
JNIEXPORT jint JNICALL Java_PcaOracle_LCR64(JNIEnv *env, jclass jc,
                                            jint db_sid,
                                            jbyteArray sql_hash,
                                            jint sql_type)
{
 ...
 * Signature: (I)I
 */
JNIEXPORT jint JNICALL Java_PcaOracle_ECODE(JNIEnv *env, jclass jc, jint db_sid)
{
        PcaSession *session = PcaSessionPool::getSession(db_sid);
        if (!session)
                return PcAPI_ERR_INVALID_SID;
 ...
 * Signature: (I)I
 */
JNIEXPORT jint JNICALL Java_PcaOracle_GNSF(JNIEnv *env, jclass jc, jint db_sid)
{
        PcaSession *session = PcaSessionPool::getSession(db_sid);
        if (session)
                return session->getNewSqlFlag();
        return 0;
}
 ...
 * Signature: (I[B)V
 */
JNIEXPORT void JNICALL Java_PcaOracle_LOGGING(JNIEnv *env, jclass jc, jint ecode, jbyteArray msg)
{
        dgt_schar m[257];
        dgt_sint32 blen;
        if ((blen = env->GetArrayLength(msg)) > 256)
                blen = 256;
        memset(m, 0, 257);
