...
dgt_sint32 clib_sessinfo() {
    ...
    if (!session) return -1;
    dgt_sint32 rtn =
        session->openSession(1, "pgsql", "pgsql", "127.0.0.1", "dbuser",
                             "os_user", "pgm", 1, " ", " ");
    return rtn;
}
...
Datum pca_encrypt(PG_FUNCTION_ARGS) {
    ...
    memcpy(enc_col_nm, (void*)VARDATA(enc_col_name),
           VARSIZE(enc_col_name) - VARHDRSZ);
    if (session->encrypt((dgt_schar*)enc_col_nm, (dgt_uint8*)VARDATA(src_data),
                         VARSIZE(src_data) - VARHDRSZ, &dst, &dst_len) < 0) {
        //
...
Datum pca_decrypt(PG_FUNCTION_ARGS) {
    ...
    memcpy(enc_col_nm, (void*)VARDATA(enc_col_name),
           VARSIZE(enc_col_name) - VARHDRSZ);
    if (session->decrypt((dgt_schar*)enc_col_nm, (dgt_uint8*)VARDATA(src_data),
                         VARSIZE(src_data) - VARHDRSZ, &dst, &dst_len) < 0) {
        //
        // decrypt failed, session has this last error code which can be gotten
        // by ECODE
        //
        ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
