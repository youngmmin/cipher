...
static const dgt_uint8 test_key[32] = {
    185, 127, 59, 99, 81,  116, 20, 119, 14,  21, 80, 143, 17,  60,  52,  106,
    69,  158, 26, 6,  252, 183, 74, 42,  254, 23, 87, 37,  152, 118, 138, 206};
...
extern "C" {
CS_INT sybesp_dll_version() { return CS_CURRENT_VERSION; }
/*
** Function:
**	xp_message
...
**
*/
void xp_message(SRV_PROC *srvproc,         /*
    ...
    */
    ...
    if (errmsgp->textlen >= CS_MAX_MSG) return;
...
{
    if (init_flag == 0) {
        PCI_initContext(&ctx, test_key, 256, PCI_CIPHER_ARIA, 2, PCI_IVT_NO, 1,
                        2, 1, 0, 0);
        PCI_initContext(&ctx2, test_key, 256, PCI_CIPHER_SHA, 0, PCI_IVT_NO, 1,
                        2, 1);
        init_flag = 1;
    }
    ...
    */
    if (paramnum != 3) {
        ...
        */
        if ((result == CS_SUCCEED) && srv_descfmt(srvproc, CS_GET, SRV_RPCDATA,
        ...
        if (result == CS_SUCCEED) {
            ...
            */
            ...
            if (!strncasecmp((char *)enc_col_id, "dbo.VIMS_KEY.PASSWORD", 21)) {
                if ((ret = PCI_encrypt(&ctx2, (dgt_uint8 *)src, src_len,
                                       (dgt_uint8 *)dst,
                                       (dgt_uint32 *)&dst_len)) < 0) {
                    result = CS_FAIL;
                }
            } else {
                if ((ret = PCI_encrypt(&ctx, (dgt_uint8 *)src, src_len,
                                       (dgt_uint8 *)dst,
                                       (dgt_uint32 *)&dst_len)) < 0) {
                    result = CS_FAIL;
                }
            ...
            */
            if ((result == CS_SUCCEED) &&
                (srv_bind(srvproc, CS_SET, SRV_RPCDATA, 1, &paramfmt,
                          (CS_BYTE *)dst, &dst_len, &outlen) != CS_SUCCEED)) {
                result = CS_FAIL;
            }
            ...
            */
            if ((result == CS_SUCCEED) &&
                srv_xferdata(srvproc, CS_SET, SRV_RPCDATA) != CS_SUCCEED) {
                result = CS_FAIL;
            }
        }
        ...
        */
        if (result == CS_SUCCEED) {
            ...
            */
            ...
            memset(dst, 0, 256);
            if (!strncasecmp((char *)enc_col_id, "dbo.VIMS_KEY.PASSWORD", 21)) {
                if ((ret = PCI_decrypt(&ctx2, (dgt_uint8 *)src, src_len,
                                       (dgt_uint8 *)dst,
                                       (dgt_uint32 *)&dst_len)) < 0) {
                    result = CS_FAIL;
                }
            } else {
                if ((ret = PCI_decrypt(&ctx, (dgt_uint8 *)src, src_len,
                                       (dgt_uint8 *)dst,
                                       (dgt_uint32 *)&dst_len)) < 0) {
                    result = CS_FAIL;
                }
