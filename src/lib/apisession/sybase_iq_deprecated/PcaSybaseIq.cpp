...
static void pls_encrypt_b64_id_evaluate(a_v3_extfn_scalar_context *cntxt, void *arg_handle)
{
	...
	unsigned int dst_len=4000;
        //if ((rtn=PcAPI_encrypt_sess_buf(sid, enc_col_id, (unsigned char*)src_value.data, src_len, &dst, &dst_len)) < 0) {
        if ((rtn=PcAPI_encrypt(sid, enc_col_id, (unsigned char*)src_value.data, src_len, dst, &dst_len)) < 0) {
...
static void pls_decrypt_b64_id_evaluate(a_v3_extfn_scalar_context *cntxt, void *arg_handle)
{
        an_extfn_value  src_value;
        an_extfn_value  enc_col_id_value;
        an_extfn_value  outval;
	...
	memset(dst,0,4000);
        unsigned int dst_len=4000;
        if ((rtn=PcAPI_decrypt(sid, enc_col_id, (unsigned char*)src_value.data, src_len, dst, &dst_len)) < 0) {
